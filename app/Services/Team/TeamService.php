...
class TeamService
{
    ...
    public function teamCategories($tc_id = 0)
    {
        ...
        if ($team_list) {
            foreach ($team_list as $key => $val) {
                ...
                $list[$key]['tc_img'] = $this->sanmuRepository->getImagePath($val['tc_img']);
            }
        }
        ...
        if ($this->config['virtual_order'] == 1) {
            ...
            if ($info) {
                foreach ($info as $key => $value) {
                    ...
                    $list[$key]['user_picture'] = $this->sanmuRepository->getImagePath($value['user_picture']);
                    //随机秒数
    ...
    public function getGoods($tc_id = 0, $page = 1, $size = 10)
    {
        ...
        if ($team_list) {
            foreach ($team_list as $key => $val) {
                ...
                $list[$key]['shop_price'] = $this->sanmuRepository->getPriceFormat($val['shop_price']);
                $list[$key]['goods_thumb'] = $this->sanmuRepository->getImagePath($val['goods_thumb']);
                $list[$key]['team_price'] = $this->sanmuRepository->getPriceFormat($val['team_price']);
    ...
    public function goodsDetail($goods_id = 0, $user_id = 0)
    {
        ...
        if (empty($res['desc_mobile']) && !empty($res['goods_desc'])) {
            $desc_preg = $this->sanmuRepository->descImagesPreg($res['goods_desc']);
            $res['goods_desc'] = $desc_preg['goods_desc'];
        }

        if (!empty($res['desc_mobile'])) {
            // 处理手机端商品详情 图片（手机相册图） data/gallery_album/
            $desc_preg = $this->sanmuRepository->descImagesPreg($res['desc_mobile'], 'desc_mobile', 1);
            $res['desc_mobile'] = $desc_preg['desc_mobile'];
            $res['goods_desc'] = $desc_preg['desc_mobile'];
        }


        $res['goods_thumb'] = $this->sanmuRepository->getImagePath($res['goods_thumb']);
        $res['goods_img'] = $this->sanmuRepository->getImagePath($res['goods_img']);
        if ($user_id) {
    ...
    public function teamNewGoods($type = 'is_new', $user_id = 0, $size = 10)
    {
        ...
        if ($team_list) {
            foreach ($team_list as $key => $val) {
                ...
                $list[$key]['goods_id'] = $val['goods_id'];
                $list[$key]['goods_thumb'] = $this->sanmuRepository->getImagePath($val['goods_thumb']);
                $list[$key]['shop_price'] = $this->sanmuRepository->getPriceFormat($val['shop_price'], true);
                $list[$key]['team_price'] = $this->sanmuRepository->getPriceFormat($val['team_price'], true);
                $list[$key]['goods_name'] = $val['goods_name'];
            }
        }
    ...
    public function teamInfo($team_id = 0)
    {
        ...
        if ($team_info) {
            ...
            $team_info['user_picture'] = $this->sanmuRepository->getImagePath($team_info['user_picture']);

            $team_info['goods_thumb'] = $this->sanmuRepository->getImagePath($team_info['goods_thumb']);
            $team_info['team_price'] = $this->sanmuRepository->getPriceFormat($team_info['team_price']);
            // 当前时间
    ...
    public function teamUserOrderRefund()
    {
        ...
        if ($team_list) {
            foreach ($team_list as $key => $val) {
                ...
                $val = $val['get_team_goods'] ? array_merge($val, $val['get_team_goods']) : $val;
                $val = $val['get_goods'] ? array_merge($val, $val['get_goods']) : $val;
                $val['shop_price'] = $this->sanmuRepository->getPriceFormat($val['shop_price']);
                $val['goods_thumb'] = $this->sanmuRepository->getImagePath($val['goods_thumb']);
                $val['team_price'] = $this->sanmuRepository->getPriceFormat($val['team_price']);
                $team_num = $this->surplusNum($val['team_id']);  //统计几人参团
                ...
                if ($team_order) {
                    ...
                    if (is_wechat_browser() && file_exists(MOBILE_WECHAT)) {
                        foreach ($team_order as $key => $vo) {
                            ...
                            $url = sanmu_url('/#/team/wait') . '?' . http_build_query(['team_id' => $team_id], '', '&');
