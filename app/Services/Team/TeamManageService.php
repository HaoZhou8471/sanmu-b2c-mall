    ...
    public function getTeamGoodsList($filter = [], $offset = [])
    {
        ...
        if ($list) {
            foreach ($list as $key => $val) {
                ...
                $list[$key]['market_price'] = $this->sanmuRepository->getPriceFormat($val['market_price']);
                $list[$key]['goods_img'] = $this->sanmuRepository->getImagePath($val['goods_img']);
    ...
    public function getTeamOrder($offset = [], $team_id = 0)
    {
        ...
        if ($list) {
            foreach ($list as $key => $val) {
                ...
                $list[$key]['formated_order_amount'] = $this->sanmuRepository->getPriceFormat($val['order_amount']);
                $list[$key]['formated_total_fee'] = $this->sanmuRepository->getPriceFormat($val['goods_amount']);
                ...
                if (isset($this->config['show_mobile']) && $this->config['show_mobile'] == 0) {
                    $list[$key]['mobile'] = $this->sanmuRepository->stringToStar($list[$key]['mobile']);
                    $list[$key]['tel'] = $this->sanmuRepository->stringToStar($list[$key]['tel']);
                    $list[$key]['email'] = $this->sanmuRepository->stringToStar($list[$key]['email']);
                    $list[$key]['user_name'] = $this->sanmuRepository->stringToStar($list[$key]['user_name']);
                }
            }
        }
