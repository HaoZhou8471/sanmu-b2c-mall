...
class RightsCardCommonService
{
    ...
    public function transFormRightsCardInfo($info = [])
    {
        ...
        if (!empty($info)) {
            if (isset($info['background_img']) && !empty($info['background_img'])) {
                ...
                $info['background_img'] = $this->sanmuRepository->getImagePath($info['background_img']);
            } else {
            ...
            if (!empty($info['receive_value'])) {
                foreach ($info['receive_value'] as $k => $item) {
                    if ($item['type'] == 'order' || $item['type'] == 'buy') {
                        $info['receive_value'][$k]['value_format'] = $this->sanmuRepository->getPriceFormat($item['value'], true);
                    } else {
    ...
    public function transFormCardRightsList($item = [])
    {
        ...
        foreach ($list as $k => $value) {
            ...
            if (isset($value['user_membership_rights']) && !empty($value['user_membership_rights'])) {
                ...
                $list[$k]['icon'] = empty($list[$k]['icon']) ? '' : ((stripos($list[$k]['icon'], 'assets') !== false) ? asset($list[$k]['icon']) : $this->sanmuRepository->getImagePath($list[$k]['icon']));
    ...
    public function transFormRightsList($item = [])
    {
        ...
        foreach ($list as $k => $value) {
            ...
            $list[$k]['icon'] = empty($value['icon']) ? '' : ((stripos($value['icon'], 'assets') !== false) ? asset($value['icon']) : $this->sanmuRepository->getImagePath($value['icon']));
