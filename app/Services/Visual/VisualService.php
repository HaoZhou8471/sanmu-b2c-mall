...
class VisualService
{
    ...
    public function Article($cat_id = 0, $num = 10)
    {
        ...
        if (!empty($article_msg)) {
            foreach ($article_msg as $key => $value) {
                ...
                $article_msg[$key]['url'] = sanmu_url('/#/articleDetail/' . $value['article_id']);
        ...
        if (!empty($goods_id)) {
            ...
            if ($res) {
                foreach ($res as $key => $row) {
                    ...
                    if ($promote_price > 0) {
                        $goods[$key]['shop_price_formated'] = $this->sanmuRepository->getPriceFormat($promote_price);
                    } else {
                        $goods[$key]['shop_price_formated'] = $this->sanmuRepository->getPriceFormat($row['shop_price']);
                    }
                    $goods[$key]['market_price'] = $row['market_price'] ?? '';
                    $goods[$key]['market_price_formated'] = $this->sanmuRepository->getPriceFormat($row['market_price']);
                    ...
                    $goods[$key]['goods_thumb'] = $this->sanmuRepository->getImagePath($row['goods_thumb']);
                    $goods[$key]['goods_img'] = $this->sanmuRepository->getImagePath($row['goods_img']);
                    $goods[$key]['url'] = sanmu_url('/#/goods/' . $row['goods_id']);
    ...
    public function Seckill($number = 10)
    {
        ...
        if ($sec_goods) {
            foreach ($sec_goods as $key => $value) {
                ...
                $arr['goods'][$key]['price_formated'] = $this->sanmuRepository->getPriceFormat($value['sec_price']);
                ...
                $arr['goods'][$key]['market_price_formated'] = $this->sanmuRepository->getPriceFormat($arr['goods'][$key]['market_price']);
                $arr['goods'][$key]['title'] = $goods['goods_name'];
                $arr['goods'][$key]['goods_thumb'] = empty($goods['goods_thumb']) ? '' : $this->sanmuRepository->getImagePath($goods['goods_thumb']);
                ...
                foreach ($store as $key => $value) {
                    ...
                    if ($goods) {
                        foreach ($goods as $a => $val) {
                            $goods[$a]['goods_thumb'] = $this->sanmuRepository->getImagePath($val['goods_thumb']);
                        }
                    }

                    $store[$key]['goods'] = $goods;
                    $store[$key]['total'] = count($goods);
                    $store[$key]['logo_thumb'] = $this->sanmuRepository->getImagePath(str_replace('../', '', $value['logo_thumb']));
                    $store[$key]['street_thumb'] = $this->sanmuRepository->getImagePath($value['street_thumb']);
    ...
    public function View($id = 0, $type = 'index', $default = 0, $ru_id = 0, $number = 10, $page_id = 0)
    {
        ...
        if ($id) {
            ...
            if ($res) {
                foreach ($res as $k => $v) {
                    $res[$k]['thumb_pic'] = (isset($v['thumb_pic']) && !empty($v['thumb_pic'])) ? $this->sanmuRepository->getImagePath($v['thumb_pic']) : '';
                }
            }
    ...
    public function cat_list($cat_id = 0, $level = 0)
    {
        ...
        foreach ($res as $k => $row) {
            ...
            $three_arr[$k]['url'] = sanmu_url('/#/list/' . $row['cat_id']);
    ...
    public function article_list($cat_id = 0, $level = 0)
    {
        ...
        foreach ($article as $k => $row) {
            $three_arr[$k]['cat_id'] = $row['cat_id'];
            $three_arr[$k]['cat_name'] = $row['cat_name'];
            $three_arr[$k]['url'] = sanmu_url('/#/article') . '?cat_id=' . $row['cat_id'];
            $three_arr[$k]['parent_id'] = $row['parent_id'];
            $three_arr[$k]['haschild'] = 0;
    ...
    public function store_category($cat_id = 0, $ru_id = 0, $level = 0)
    {
        ...
        foreach ($res as $k => $row) {
            ...
            $three_arr[$k]['cat_name'] = $row['cat_name'];
            $three_arr[$k]['parent_id'] = $row['parent_id'];
            $three_arr[$k]['url'] = sanmu_url('/#/ShopGoodsList') . '?' . http_build_query(['cat_id' => $row['cat_id'], 'ru_id' => $ru_id], '', '&');
    ...
    public function brand_list($num = 100)
    {
        ...
        if ($brand) {
            foreach ($brand as $k => $val) {
                $brand[$k]['brand_logo'] = $this->sanmuRepository->getImagePath($val['brand_logo']);
                $brand[$k]['index_img'] = $this->sanmuRepository->getImagePath($val['index_img']);
            }
        }
    ...
    public function picture_list($ru_id = 0, $album_id = 0, $thumb = '', $pageSize = 15)
    {
        ...
        foreach ($list as $key => $vo) {
            ...
            $res[$key]['img'] = $this->sanmuRepository->getImagePath($vo['pic_file']);
        ...
        if ($type == 'thumb') {
            ...
            if ($pic) {
                foreach ($pic as $key => $value) {
                    $pic[$key]['pic_file'] = $this->sanmuRepository->getImagePath($value['pic_file']);
                }
            }
    ...
    public function get_url($type = '', $pageSize = 10, $currentPage = 1)
    {
        ...
        if ($type == 'function') {
            $url = [
                ['cat_id' => '1', 'cat_name' => lang('common.category'), 'parent_id' => 0, 'url' => sanmu_url('/#/catalog'), 'app_page' => config('route.catalog.index')],
                ['cat_id' => '2', 'cat_name' => lang('common.cat_list'), 'parent_id' => 0, 'url' => sanmu_url('/#/cart'), 'app_page' => config('route.cart.index')],
                ['cat_id' => '3', 'cat_name' => lang('common.user_center'), 'parent_id' => 0, 'url' => sanmu_url('/#/user'), 'app_page' => config('route.user.index')],
                ['cat_id' => '4', 'cat_name' => lang('common.store_street'), 'parent_id' => 0, 'url' => sanmu_url('/#/shop'), 'app_page' => config('route.shop.index')],
                ['cat_id' => '5', 'cat_name' => lang('common.brand_street'), 'parent_id' => 0, 'url' => sanmu_url('/#/brand'), 'app_page' => config('route.brand.index')],
                ['cat_id' => '6', 'cat_name' => lang('common.mini_sns'), 'parent_id' => 0, 'url' => sanmu_url('/#/discover'), 'app_page' => config('route.discover.index')],
            ];

            return ['url' => $url, 'total' => count($url), 'page' => $currentPage];
        } elseif ($type == 'activity') {
            $url = [
                ['cat_id' => 1, 'cat_name' => lang('common.rec_txt.2'), 'parent_id' => 0, 'url' => sanmu_url('/#/groupbuy'), 'app_page' => config('route.groupbuy.index')],
                ['cat_id' => 2, 'cat_name' => lang('common.rec_txt.5'), 'parent_id' => 0, 'url' => sanmu_url('/#/exchange'), 'app_page' => config('route.exchange.index')],
                ...
                ['cat_id' => 4, 'cat_name' => lang('common.rec_txt.7'), 'parent_id' => 0, 'url' => sanmu_url('/#/topic'), 'app_page' => config('route.topic.index')],
                ['cat_id' => 5, 'cat_name' => lang('common.rec_txt.9'), 'parent_id' => 0, 'url' => sanmu_url('/#/activity'), 'app_page' => config('route.activity.index')],
                ['cat_id' => 6, 'cat_name' => lang('common.rec_txt.3'), 'parent_id' => 0, 'url' => sanmu_url('/#/auction'), 'app_page' => config('route.auction.index')],
                ['cat_id' => 7, 'cat_name' => lang('common.rec_txt.10'), 'parent_id' => 0, 'url' => sanmu_url('/#/seckill'), 'app_page' => config('route.seckill.index')],
                ['cat_id' => 8, 'cat_name' => lang('common.rec_txt.11'), 'parent_id' => 0, 'url' => sanmu_url('/#/package'), 'app_page' => config('route.package.index')]
            ];
            // 如果有拼团模块则显示
            if (file_exists(MOBILE_TEAM)) {
                $team = [['cat_id' => 9, 'cat_name' => lang('common.rec_txt.12'), 'parent_id' => 0, 'url' => sanmu_url('/#/team'), 'app_page' => config('route.team.index')]];
                $url = collect($url)->merge($team)->all();
            }

            // 如果有砍价模块则显示
            if (file_exists(MOBILE_BARGAIN)) {
                $team = [['cat_id' => 10, 'cat_name' => lang('common.rec_txt.13'), 'parent_id' => 0, 'url' => sanmu_url('/#/bargain'), 'app_page' => config('route.bargain.index')]];
                $url = collect($url)->merge($team)->all();
            }
            ...
            if ($list) {
                foreach ($list as $key => $value) {
                    $url[$key] = [
                        ...
                        'topic_img' => $this->sanmuRepository->getImagePath($value['topic_img']),
                        'url' => sanmu_url('/#/topic/detail/' . $value['cat_id']),
    ...
    public function get_seller_url($type = '', $ru_id = 0, $pageSize = 10, $currentPage = 1)
    {
        ...
        if ($type == 'category') {
            ...
            foreach ($url as $key => $value) {
                $url[$key]['url'] = sanmu_url('/#/ShopGoodsList') . '?' . http_build_query(['ru_id' => $ru_id, 'cat_id' => $value['cat_id']], '', '&');
            }
    ...
    public function add_topic_page($id = 0, $type = 'topic', $ru_id = 0, $page_id = 0, $title = '', $description = '', $data = [])
    {
        ...
        if ($id) {
            ...
            if ($view) {
                ...
                $page['thumb_pic'] = (isset($page['thumb_pic']) && !empty($page['thumb_pic'])) ? $this->sanmuRepository->getImagePath($page['thumb_pic']) : '';
    ...
    public function search_goods($ru_id = 0, $keywords = '', $cat_id = 0, $brand_id = 0, $warehouse_id = 0, $area_id = 0, $area_city = 0, $pageSize = 10, $currentPage = 1)
    {
        ...
        foreach ($goods as $key => $val) {
            ...
            if ($promote_price > 0) {
                ...
                $goods[$key]['shop_price_formated'] = $this->sanmuRepository->getPriceFormat($val['shop_price']);
            }
            $goods[$key]['market_price_formated'] = $this->sanmuRepository->getPriceFormat($val['market_price']);

            $goods[$key]['goods_img'] = $this->sanmuRepository->getImagePath($val['goods_img']);
            $goods[$key]['goods_thumb'] = $this->sanmuRepository->getImagePath($val['goods_thumb']);
            $goods[$key]['goods_img'] = $this->sanmuRepository->getImagePath($val['goods_img']);
        }
