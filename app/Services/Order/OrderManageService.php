...
class OrderManageService
{
    ...
    public function orderList($page = 0)
    {
        ...
        if ($GLOBALS['_CFG']['region_store_enabled']) {
            ...
            $res = $this->sanmuRepository->getWhereRsid($res, $filter['rs_id']);
        }
        ...
        if ($row) {
            foreach ($row as $key => $value) {
                ...
                $row[$key]['formated_order_amount'] = $this->sanmuRepository->getPriceFormat($value['order_amount']);
                $row[$key]['formated_money_paid'] = $this->sanmuRepository->getPriceFormat($value['money_paid']);
                $row[$key]['formated_total_fee'] = $this->sanmuRepository->getPriceFormat($value['total_fee']);
                $row[$key]['old_shipping_fee'] = $value['shipping_fee'];
                $row[$key]['shipping_fee'] = $this->sanmuRepository->getPriceFormat($value['shipping_fee']);
                ...
                $row[$key]['formated_value_card'] = $this->sanmuRepository->getPriceFormat($value_card);

                $row[$key]['formated_total_fee_order'] = $this->sanmuRepository->getPriceFormat($value['total_fee_order']);
                ...
                if (isset($value['is_zc_order']) && $value['is_zc_order'] == 1) {
                    ...
                    $zcg = [
                        ...
                        'goods_thumb' => $this->sanmuRepository->getImagePath($zc_project['title_img']),
                    ...
                    if ($goods_list) {
                        ...
                        foreach ($goods_list as $idx => $goods) {
                            ...
                            $goods_list[$idx]['goods_thumb'] = isset($goods['get_goods']['goods_thumb']) ? $this->sanmuRepository->getImagePath($goods['get_goods']['goods_thumb']) : '';
                            $goods_list[$idx]['goods_img'] = isset($goods['get_goods']['goods_img']) ? $this->sanmuRepository->getImagePath($goods['get_goods']['goods_img']) : '';
                            $goods_list[$idx]['original_img'] = isset($goods['get_goods']['original_img']) ? $this->sanmuRepository->getImagePath($goods['get_goods']['original_img']) : '';

                            $goods_list[$idx]['format_goods_price'] = $this->sanmuRepository->getPriceFormat($goods['goods_price']);
                            ...
                            if ($trade_id) {
                                $goods_list[$idx]['trade_url'] = $this->sanmuRepository->sanmuUrl("trade_snapshot.php?act=trade&user_id=" . $value['user_id'] . "&tradeId=" . $trade_id . "&snapshot=true");
                            }
                ...
                if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                    $row[$key]['mobile'] = $this->sanmuRepository->stringToStar($row[$key]['mobile']);
                    $row[$key]['buyer'] = $this->sanmuRepository->stringToStar($row[$key]['buyer']);
                    $row[$key]['tel'] = $this->sanmuRepository->stringToStar($row[$key]['tel']);
                    $row[$key]['email'] = $this->sanmuRepository->stringToStar($row[$key]['email']);
                }
            }
        }
    ...
    public function getOrderGoods($order)
    {
        ...
        foreach ($res as $row) {
            ...
            $row['formated_subtotal'] = $this->sanmuRepository->getPriceFormat($row['goods_price'] * $row['goods_number']);
            $row['formated_goods_price'] = $this->sanmuRepository->getPriceFormat($row['goods_price']);
            ...
            if ($row['extension_code'] == 'package_buy') {
                ...
                if ($activity) {
                    $row['goods_thumb'] = $this->sanmuRepository->getImagePath($activity['activity_thumb']);
                }
            }
            ...
            if ($trade_id) {
                $row['trade_url'] = $this->sanmuRepository->sanmuUrl("trade_snapshot.php?act=trade&user_id=" . $row['user_id'] . "&tradeId=" . $trade_id . "&snapshot=true");
            }
    ...
    public function deliveryList()
    {
        ...
        foreach ($row as $key => $value) {
            ...
            if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                ...
                $row[$key]['consignee'] = $this->sanmuRepository->stringToStar($value['consignee']);
            ...
            $_goods_thumb = isset($row[$key]['goods_thumb']) && $row[$key]['goods_thumb'] ? $this->sanmuRepository->getImagePath($row[$key]['goods_thumb']) : '';
    ...
    public function deliveryOrderInfo($delivery_id, $delivery_sn = '')
    {
        ...
        if ($delivery) {
            ...
            /* 格式化金额字段 */
            $delivery['formated_insure_fee'] = $this->sanmuRepository->getPriceFormat($delivery['insure_fee'], false);
            $delivery['formated_shipping_fee'] = $this->sanmuRepository->getPriceFormat($delivery['shipping_fee'], false);
            ...
            if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                ...
                $delivery['consignee'] = $this->sanmuRepository->stringToStar($delivery['consignee']);
            }
    ...
    public function backOrderInfo($back_id = 0)
    {
        ...
        if ($back) {
            ...
            /* 格式化金额字段 */
            $back['formated_insure_fee'] = $this->sanmuRepository->getPriceFormat($back['insure_fee'], false);
            $back['formated_shipping_fee'] = $this->sanmuRepository->getPriceFormat($back['shipping_fee'], false);
    ...
    {
        $this->sanmuRepository->helpersLang(['order'], 'admin');
