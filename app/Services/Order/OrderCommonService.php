...
class OrderCommonService
{
        ...
        if ($res) {
            foreach ($res as $key => $item) {
                if (isset($item['order_id']) && $item['order_id']) {
                    ...
                    $res[$key]['wvera_price'] = $this->sanmuRepository->getPriceFormat($item['goods_num'] ? $item['turnover'] / $item['goods_num'] : 0);
                    $res[$key]['short_name'] = $this->sanmuRepository->subStr($item['goods_name'], 30, true);
                    $res[$key]['turnover'] = $this->sanmuRepository->getPriceFormat($item['turnover']);
                    $res[$key]['taxis'] = $key + 1;
    ...
    public function unconfirmedNum($start_date = 0, $end_date = 0, $rs_id = 0, $type = 0)
    {
        ...
        if ($rs_id > 0) {
            $unconfirmed_num = $this->sanmuRepository->getWhereRsid($unconfirmed_num, 'ru_id', $rs_id);
        } else {
    ...
    public function confirmedNum($start_date = 0, $end_date = 0, $rs_id = 0, $type = 0)
    {
        ...
        if ($rs_id > 0) {
            $confirmed_num = $this->sanmuRepository->getWhereRsid($confirmed_num, 'ru_id', $rs_id);
        } else {
    ...
    public function succeedNum($start_date = 0, $end_date = 0, $rs_id = 0, $type = 0)
    {
        ...
        if ($rs_id > 0) {
            $succeed_num = $this->sanmuRepository->getWhereRsid($succeed_num, 'ru_id', $rs_id);
        } else {
    ...
    public function invalidNum($start_date = 0, $end_date = 0, $rs_id = 0, $type = 0)
    {
        ...
        if ($rs_id > 0) {
            $invalid_num = $this->sanmuRepository->getWhereRsid($invalid_num, 'ru_id', $rs_id);
        } else {
        ...
        if ($res) {
            foreach ($res as $items) {
                ...
                if (isset($this->config['show_mobile']) && $this->config['show_mobile'] == 0) {
                    $items['mobile_phone'] = $this->sanmuRepository->stringToStar($items['mobile_phone']);
                    $items['user_name'] = $this->sanmuRepository->stringToStar($items['user_name']);
                    $items['email'] = $this->sanmuRepository->stringToStar($items['email']);
                }
    ...
    public function userAreaStats($page = 0)
    {
        ...
        if ($result === false) {
            ...
            if ($filter['area']) {
                $sql = " SELECT region_id FROM " . $GLOBALS['sanmu']->table('merchants_region_info') . " WHERE ra_id = '$filter[area]' ";
            ...
            /* 关联查询 */
            $leftJoin = '';
            $leftJoin .= " LEFT JOIN " . $GLOBALS['sanmu']->table('region') . " AS r ON r.region_id = o.district ";

            /* 记录总数 */
            $sql = "SELECT o.district FROM " . $GLOBALS['sanmu']->table('order_info') . " AS o " .
                $leftJoin .
                $where_o . $groupBy;
    ...
    public function userSaleStats($page = 0)
    {
        ...
        if ($result === false) {
            ...
            /* 关联查询 */
            $leftJoin = '';
            $leftJoin .= " LEFT JOIN " . $GLOBALS['sanmu']->table('users') . " AS u ON u.user_id = o.user_id ";
            $leftJoin .= " LEFT JOIN " . $GLOBALS['sanmu']->table('order_return') . " AS re ON re.order_id = o.order_id ";

            /* 记录总数 */
            $sql = "SELECT o.user_id FROM " . $GLOBALS['sanmu']->table('order_info') . " AS o " .
                $leftJoin .
        ...
        /* 格式化数据 */
        foreach ($row as $key => $value) {
            $row[$key]['formated_return_fee'] = $this->sanmuRepository->getPriceFormat($value['return_fee']);
            $row[$key]['formated_total_fee'] = $this->sanmuRepository->getPriceFormat($value['total_fee']);

            if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                $row[$key]['user_name'] = $this->sanmuRepository->stringToStar($value['user_name']);
                $row[$key]['mobile_phone'] = $this->sanmuRepository->stringToStar($value['mobile_phone']);
            }
        }
    ...
    public function shopSaleStats($page = 0)
    {
        ...
        /* 关联查询 */
        $leftJoin = " LEFT JOIN " . $GLOBALS['sanmu']->table('order_info') . " AS o ON o.ru_id = spi.ru_id ";
        $leftJoin .= " LEFT JOIN " . $GLOBALS['sanmu']->table('order_return') . " AS re ON re.order_id = o.order_id ";
        $leftJoin .= " LEFT JOIN " . $GLOBALS['sanmu']->table('merchants_shop_information') . " AS msi ON msi.user_id = spi.ru_id ";

        /* 记录总数 */
        $sql = "SELECT spi.ru_id FROM " . $GLOBALS['sanmu']->table('seller_shopinfo') . " AS spi " .
            $leftJoin .
        ...
        foreach ($row as $key => $value) {
            ...
            $row[$key]['formated_total_fee'] = $this->sanmuRepository->getPriceFormat($value['total_fee']);
            $row[$key]['formated_valid_fee'] = $this->sanmuRepository->getPriceFormat($value['valid_fee']);
            $row[$key]['formated_return_amount'] = $this->sanmuRepository->getPriceFormat($value['return_amount']);
        }
    ...
    public function shopAreaStats($page = 0)
    {
        ...
        if ($result === false) {
            ...
            $leftJoin = '';
            $leftJoin .= " LEFT JOIN " . $GLOBALS['sanmu']->table('merchants_shop_information') . " AS msi ON msi.user_id = spi.ru_id ";
            $leftJoin .= " LEFT JOIN " . $GLOBALS['sanmu']->table('region') . " AS r ON r.region_id = spi.district ";

            /* 记录总数 */
            $sql = "SELECT spi.district FROM " . $GLOBALS['sanmu']->table('seller_shopinfo') . " AS spi " .
                $leftJoin .
