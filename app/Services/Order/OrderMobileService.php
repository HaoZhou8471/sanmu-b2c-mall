...
class OrderMobileService
{
        ...
        foreach ($List as $k => $v) {
            ...
            if ($goods) {
                foreach ($goods as $key => $val) {
                    ...
                    $orderList[$k]['order_goods'][$key]['goods_price'] = $this->sanmuRepository->getPriceFormat($val['goods_price'] + $val['shipping_fee']);

                    $val['goods_thumb'] = $val['goods_thumb'] ?? '';
                    $orderList[$k]['order_goods'][$key]['goods_thumb'] = $this->sanmuRepository->getImagePath($val['goods_thumb']);
                    ...
                    if ($val['extension_code'] == 'package_buy') {
                        ...
                        $orderList[$k]['order_goods'][$key]['goods_thumb'] = $this->sanmuRepository->getImagePath($activity['goods_thumb']);
                    }
                    $orderList[$k]['order_goods'][$key]['goods_name'] = $val['goods_name'];
                    $orderList[$k]['order_goods'][$key]['goods_sn'] = $val['goods_sn'];
                    $orderList[$k]['order_goods'][$key]['drp_money'] = $this->sanmuRepository->getPriceFormat($val['drp_money']);
            ...
            $orderList[$k]['goods_amount_formated'] = $this->sanmuRepository->getPriceFormat($v['goods_amount']);
            $orderList[$k]['money_paid_formated'] = $this->sanmuRepository->getPriceFormat($v['money_paid']);
            $orderList[$k]['order_amount_formated'] = $this->sanmuRepository->getPriceFormat($v['order_amount']);
            $orderList[$k]['shipping_fee_formated'] = $this->sanmuRepository->getPriceFormat($v['shipping_fee']);
            ...
            $orderList[$k]['total_amount_formated'] = $this->sanmuRepository->getPriceFormat($total_fee_order);
        }
    ...
    public function orderDetail($args)
    {
        ...
        $list = [
            ...
            'money_paid_formated' => $this->sanmuRepository->getPriceFormat($order['money_paid'], false),
            'goods_amount' => $order['goods_amount'],
            'goods_amount_formated' => $this->sanmuRepository->getPriceFormat($order['goods_amount'], false),
            'order_amount' => $order['order_amount'],
            'order_amount_formated' => $this->sanmuRepository->getPriceFormat($order['order_amount'], false),
            ...
            'pay_fee_formated' => $this->sanmuRepository->getPriceFormat($order['pay_fee'], false),
            ...
            'bonus' => $this->sanmuRepository->getPriceFormat($order['bonus']),
            'discount' => $order['discount'],
            'shipping_fee_formated' => $this->sanmuRepository->getPriceFormat($order['shipping_fee'], false),
            'discount_formated' => $this->sanmuRepository->getPriceFormat($order['discount'], false),
            ...
            'total_amount_formated' => $this->sanmuRepository->getPriceFormat($order['order_amount'], false),
            'coupons_type' => $order['coupons'] > 0 ? 1 : 0,
            'coupons' => $this->sanmuRepository->getPriceFormat($order['coupons']),
            'integral' => $order['integral'],
            'integral_money' => $this->sanmuRepository->getPriceFormat($order['integral_money']),
            'surplus' => $order['surplus'],
            'surplus_formated' => $this->sanmuRepository->getPriceFormat($order['surplus']),
        ...
        if (CROSS_BORDER === true) // 跨境多商户
        {
            ...
            $list['rate'] = $this->sanmuRepository->getPriceFormat($list['rate_fee']);
        ...
        if (file_exists(MOBILE_DRP)) {
            ...
            if (!empty($order_membership_card)) {
                ...
                $list['membership_card_buy_money_formated'] = $this->sanmuRepository->getPriceFormat($order_membership_card['membership_card_buy_money']);
                $list['membership_card_discount_price_formated'] = $this->sanmuRepository->getPriceFormat($order_membership_card['membership_card_discount_price']);
            } else {
        ...
        if (!empty($list)) {
            ...
            if ($orderGoods) {
                foreach ($orderGoods as $k => $v) {
                    ...
                    $goodsList[$k]['goods_thumb'] = $this->sanmuRepository->getImagePath($v['goods_thumb']);

                    $goodsList[$k]['goods_price'] = $v['goods_price'];
                    $goodsList[$k]['goods_price_formated'] = $this->sanmuRepository->getPriceFormat($v['goods_price'], false);
                    ...
                    if ($v['extension_code'] == 'package_buy') {
                        ...
                        $goodsList[$k]['format_package_list_total'] = $this->sanmuRepository->getPriceFormat($goodsList[$k]['package_list_total']);
                        $goodsList[$k]['format_package_list_saving'] = $this->sanmuRepository->getPriceFormat($goodsList[$k]['package_list_saving']);
                    } else {
                        $goods_count++;
                    }
    ...
    public function value_card_record($order_id)
    {
        ...
        $res = $res ? $res->toArray() : [];
        if ($res) {
            $res['use_val'] = $this->sanmuRepository->getPriceFormat($res['use_val']);
        }
