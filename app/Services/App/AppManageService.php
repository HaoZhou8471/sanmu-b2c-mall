        ...
        if (!empty($list)) {
            foreach ($list as $k => $value) {
                if ($value['media_type'] == 0) {
                    ...
                    $list[$k]['ad_code'] = empty($value['ad_code']) ? '' : $this->sanmuRepository->getImagePath($value['ad_code']);
                }
    ...
    public function adInfo($ad_id = 0)
    {
        ...
        if (!empty($res)) {
            if ($res['media_type'] == 0) {
                ...
                $res['ad_code'] = empty($res['ad_code']) ? '' : $this->sanmuRepository->getImagePath($res['ad_code']);
            }
        }
