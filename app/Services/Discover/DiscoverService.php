...
class DiscoverService
{
    ...
    public function Show($uid = 0, $goods_id = 0)
    {
        ...
        $order['goods_thumb'] = $this->sanmuRepository->getImagePath($goods['goods_thumb']);
    ...
    public function Detail($uid = 0, $dis_type = 0, $dis_id = 0)
    {
        ...
        if ($res) {
            ...
            $res['user_picture'] = $this->sanmuRepository->getImagePath($users['user_picture'], '', asset('img/user_default.png'));
            ...
            if ($dis_type == 4) {
                ...
                if ($img_list) {
                    foreach ($img_list as $key => $list) {
                        $img_list[$key]['comment_img'] = $this->sanmuRepository->getImagePath($list['comment_img']);
                    }
                }
            ...
            if (!empty($link_good)) {
                $link_good['goods_thumb'] = $this->sanmuRepository->getImagePath($link_good['goods_thumb']);
                $link_good['url'] = sanmu_url('/#/goods/' . $link_good['goods_id']);
        ...
        if ($users) {
            $res['avatar'] = $this->sanmuRepository->getImagePath($users['user_picture'], '', asset('img/user_default.png'));
        }
        ...
        if ($res['goods_id']) {
            if ($dis_type == 4) {
                ...
                foreach ($dis_comment as $k => $v) {
                    ...
                    $dis_comment[$k]['user_picture'] = $this->sanmuRepository->getImagePath($usersnick['user_picture'], '', asset('img/user_default.png'));
    ...
    public function CommentList($type = 0, $page = 1, $size = 10, $user_id = 0, $goods_id = 0)
    {
        ...
        if ($list) {
            foreach ($list as $k => $v) {
                ...
                $list[$k]['user_picture'] = $this->sanmuRepository->getImagePath($users['user_picture'], '', asset('img/user_default.png'));
                $list[$k]['community_num'] = $this->community_num(0, $v['dis_id']);
                $list[$k]['delete_com'] = ($user_id == $v['user_id']) ? 1 : 0; // 是否显示删除按钮
                $list[$k]['dis_text'] = $this->sanmuRepository->subStr(strip_tags($this->html_out($v['dis_text'])), 50);
            }
        }
    ...
    public function My($dis_type = 1, $page = 1, $size = 10, $uid)
    {
        ...
        $res['avatar'] = $this->sanmuRepository->getImagePath($arr['user_picture'], '', asset('img/user_default.png'));
    ...
    public function Reply($uid, $page, $size)
    {
        ...
        if ($list) {
            foreach ($list as $k => $v) {
                ...
                $list[$k]['user_picture'] = $this->sanmuRepository->getImagePath($usersnick['user_picture'], '', asset('img/user_default.png'));
                $list[$k]['add_time'] = $this->mdate($v['add_time']);
            }
        }
    ...
    public function comment_list($uid, $page = 1, $size = 10)
    {
        ...
        if ($res) {
            foreach ($res as $k => $v) {
                ...
                $res[$k]['user_picture'] = $this->sanmuRepository->getImagePath($users['user_picture'], '', asset('img/user_default.png'));
                $res[$k]['dis_type'] = 4;
                $res[$k]['community_num'] = $this->comment_num(4, $v['new_dis_id']);
                $res[$k]['dis_title'] = $this->sanmuRepository->subStr($v['content'], 20); //晒单贴没有标题，从内容截取
                $res[$k]['delete_com'] = ($uid == $v['user_id']) ? 1 : 0; // 是否显示删除按钮
                $res[$k]['dis_text'] = $this->sanmuRepository->subStr(strip_tags($this->html_out($v['content'])), 50);
