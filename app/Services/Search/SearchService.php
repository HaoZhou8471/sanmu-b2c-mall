                    ...
                    $res = $res->orWhere(function ($query) use ($where) {
                        ...
                        if (is_array($where['val'])) {
                            ...
                            if ($where['pickout']) {
                                ...
                                $query->where('attr_id', $where['attr_id'])
                                    ->where('attr_value', 'like', '%' . $this->sanmuRepository->mysqlLikeQuote($where['val']) . '%');
                            }
        ...
        if ($res) {
            ...
            foreach ($res as $row) {
                ...
                if ($presale && $presale['act_id']) {
                    ...
                    $arr[$idx]['thumb'] = $this->sanmuRepository->getImagePath($row['goods_thumb']);
                    $arr[$idx]['goods_img'] = $this->sanmuRepository->getImagePath($row['goods_img']);
                    $arr[$idx]['purl'] = $this->sanmuRepository->buildUri('presale', ['act' => 'view', 'presaleid' => $presale['act_id']], $presale['goods_name']);
                ...
                if ($display == 'grid') {
                    //@Author guan 关键字高亮显示 start
                    $row['goods_name'] = $this->sanmuRepository->subStr($row['goods_name'], $this->config['goods_name_length']);
                ...
                $arr[$idx]['market_price'] = $this->sanmuRepository->getPriceFormat($row['market_price']);
                $arr[$idx]['shop_price'] = $this->sanmuRepository->getPriceFormat($row['shop_price']);
                $arr[$idx]['promote_price'] = ($promote_price > 0) ? $this->sanmuRepository->getPriceFormat($promote_price) : '';
                $arr[$idx]['goods_brief'] = $row['goods_brief'];
                $arr[$idx]['goods_thumb'] = $this->sanmuRepository->getImagePath($row['goods_thumb']);
                $arr[$idx]['goods_img'] = $this->sanmuRepository->getImagePath($row['goods_img']);
                $arr[$idx]['url'] = $this->sanmuRepository->buildUri('goods', ['gid' => $row['goods_id']], $row['goods_name']);
                ...
                if ($seller_id == 0) {
                    ...
                    if ($kf_im_switch) {
                        $arr[$idx]['is_sanmu'] = true;
                    } else {
                        $arr[$idx]['is_sanmu'] = false;
                    }
                } else {
                    $arr[$idx]['is_sanmu'] = false;
                }
