...
class RefoundService
{
        ...
        if ($goods_list) {
            ...
            foreach ($goods_list as $key => $value) {
                ...
                if ($is_package_buy == 0) {
                    ...
                    $order['goods'][$key]['goods_thumb'] = $this->sanmuRepository->getImagePath($value['get_goods']['goods_thumb'] ?? '');
                    $order['goods'][$key]['goods_cause'] = $this->goodsCommonService->getGoodsCause($value['get_goods']['goods_cause'] ?? '');

                    $price[] = $value['subtotal'];
                    $order['goods'][$key]['market_price'] = $this->sanmuRepository->getPriceFormat($value['market_price'], false);
                    $order['goods'][$key]['goods_number'] = $value['goods_number'];
                    $order['goods'][$key]['goods_price'] = $this->sanmuRepository->getPriceFormat($value['goods_price'], false);
                    $order['goods'][$key]['subtotal'] = $this->sanmuRepository->getPriceFormat($value['subtotal'], false);
    ...
    public function getRefoundList($user_id = 0, $order_id = 0, $start = 1, $size = 10)
    {
        ...
        if ($res) {
            ...
            foreach ($res as $row) {
                ...
                $row['should_return'] = $this->sanmuRepository->getPriceFormat($row['should_return']);
                ...
                $row['goods_thumb'] = $this->sanmuRepository->getImagePath($row['get_goods']['goods_thumb']);
    ...
    public function rec_goods($rec_id)
    {
        ...
        $res['market_price'] = $this->sanmuRepository->getPriceFormat($res['market_price'], false);

        $res['goods_price1'] = $res['goods_price'];
        $res['goods_price'] = $this->sanmuRepository->getPriceFormat($res['goods_price'], false);
        $res['shop_price_formated'] = $this->sanmuRepository->getPriceFormat($res['goods_price1'], false);
        $res['subtotal'] = $this->sanmuRepository->getPriceFormat($res['subtotal'], false);
        $res['attr_name'] = $res['goods_attr'];

        $res['formated_goods_coupons'] = $this->sanmuRepository->getPriceFormat($res['goods_coupons'], false);
        $res['formated_goods_bonus'] = $this->sanmuRepository->getPriceFormat($res['goods_bonus'], false);
        ...
        /* 修正商品图片 */
        $res['goods_img'] = $this->sanmuRepository->getImagePath($goods['goods_img']);
        $res['goods_thumb'] = $this->sanmuRepository->getImagePath($goods['goods_thumb']);
    ...
    public function returnDetail($user_id = 0, $ret_id = 0)
    {
        ...
        if ($shipping_list) {
            foreach ($shipping_list as $key => $val) {
                ...
                $shipping_list[$key]['format_shipping_fee'] = $this->sanmuRepository->getPriceFormat($shipping_fee);
                $shipping_list[$key]['shipping_fee'] = $shipping_fee;
                $shipping_list[$key]['free_money'] = $this->sanmuRepository->getPriceFormat($shipping_cfg['free_money']);
                if (isset($val['insure'])) {
                    $shipping_list[$key]['insure_formated'] = strpos($val['insure'], '%') === false ? $this->sanmuRepository->getPriceFormat($val['insure']) : $val['insure'];
                }
            }
    ...
    public function return_order_info($ret_id = 0, $order_sn = '', $order_id = 0, $user_id = 0)
    {
        ...
        if ($order) {
            ...
            $order['formated_goods_coupons'] = $this->sanmuRepository->getPriceFormat($order['goods_coupons']);
            $order['formated_goods_bonus'] = $this->sanmuRepository->getPriceFormat($order['goods_bonus']);
            ...
            {
                $order['formated_return_rate_price'] = $this->sanmuRepository->getPriceFormat($order['return_rate_price'], false);
            ...
            if (file_exists(MOBILE_DRP) && $ret_id > 0) {
                $order['membership_card_discount_price_formated'] = $this->sanmuRepository->getPriceFormat($order['membership_card_discount_price'], false);
            }
            $order['formated_goods_amount'] = $this->sanmuRepository->getPriceFormat($order['should_return']);
            $order['formated_discount_amount'] = $this->sanmuRepository->getPriceFormat($order['discount_amount']);
            $order['formated_should_return'] = $this->sanmuRepository->getPriceFormat($should_return);
            $order['formated_return_shipping_fee'] = $this->sanmuRepository->getPriceFormat($order['return_shipping_fee']);
            $order['formated_return_amount'] = $this->sanmuRepository->getPriceFormat($return_amount);
            $order['formated_actual_return'] = $this->sanmuRepository->getPriceFormat($order['actual_return']);
            ...
            $order['shop_price'] = $this->sanmuRepository->getPriceFormat($order['shop_price']);
            ...
            $order['goods_thumb'] = $this->sanmuRepository->getImagePath($order['goods_thumb']);
            ...
            $goods_price = OrderGoods::where('order_id', $order['order_id'])
                ...
                ->value('goods_price');
            $order['goods_price'] = $this->sanmuRepository->getPriceFormat($goods_price);
