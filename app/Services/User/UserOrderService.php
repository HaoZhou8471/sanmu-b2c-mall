    ...
    public function getDefaultUserOrders($user_id = 0, $is_zc_order = 0)
    {
        ...
        if ($res) {
            ...
            foreach ($res as $key => $row) {
                ...
                $arr[$key]['total_fee'] = $this->sanmuRepository->getPriceFormat($row['total_fee'], false);
                ...
                if ($row['get_order_goods_list']) {
                    foreach ($row['get_order_goods_list'] as $idx => $order_goods) {
                        ...
                        if ($goods) {
                            $arr[$key]['goods'][$idx]['goods_thumb'] = $this->sanmuRepository->getImagePath($goods['goods_thumb']);
                        }

                        $arr[$key]['goods'][$idx]['url'] = $this->sanmuRepository->buildUri('goods', ['gid' => $order_goods['goods_id']], $order_goods['goods_name']);
                    }
                }
            }
        }
