    ...
    public function getCollectionGoods($user_id = 0, $record_count = 0, $page = 1, $pageFunc = '', $size = 10, $warehouse_id = 0, $area_id = 0, $area_city = 0)
    {
        ...
        if ($res) {
            foreach ($res as $key => $row) {
                ...
                $goods_list[$row['goods_id']]['market_price'] = $this->sanmuRepository->getPriceFormat($row['market_price']);
                $goods_list[$row['goods_id']]['shop_price'] = $this->sanmuRepository->getPriceFormat($row['shop_price']);
                $goods_list[$row['goods_id']]['promote_price'] = ($promote_price > 0) ? $this->sanmuRepository->getPriceFormat($promote_price) : '';
                $goods_list[$row['goods_id']]['url'] = $this->sanmuRepository->buildUri('goods', ['gid' => $row['goods_id']], $row['goods_name']);
                $goods_list[$row['goods_id']]['goods_thumb'] = $this->sanmuRepository->getImagePath($row['goods_thumb']);
        ...
        if ($res) {
            foreach ($res as $row) {
                ...
                if ($ru_id == 0) {
                    ...
                    if ($kf_im_switch) {
                        $goods_list[$row['goods_id']]['is_sanmu'] = true;
                    } else {
                        $goods_list[$row['goods_id']]['is_sanmu'] = false;
                    }
                } else {
                    $goods_list[$row['goods_id']]['is_sanmu'] = false;
                }
                ...
                $goods_list[$row['goods_id']]['promote_price'] = ($promote_price > 0) ? $this->sanmuRepository->getPriceFormat($promote_price) : '';
                $goods_list[$row['goods_id']]['url'] = $this->sanmuRepository->buildUri('goods', ['gid' => $row['goods_id']], $row['goods_name']);
                $goods_list[$row['goods_id']]['shop_url'] = $this->sanmuRepository->buildUri('merchants_store', ['urid' => $row['user_id']]);
                $goods_list[$row['goods_id']]['goods_thumb'] = $this->sanmuRepository->getImagePath($row['goods_thumb']);
            }
        }
