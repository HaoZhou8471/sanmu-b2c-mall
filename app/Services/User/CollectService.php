    ...
    public function getUserShopList($user_id = 0, $page = 1, $size = 10, $region_id = 0, $area_id = 0, $area_city = 0)
    {
        ...
        if ($res) {
            foreach ($res as $key => $row) {
                ...
                $store_list[$key]['shop_bg_logo'] = $this->sanmuRepository->getImagePath(str_replace('../', '', $row['shop_logo']));
                $store_list[$key]['shop_logo'] = $this->sanmuRepository->getImagePath(str_replace('../', '', $row['logo_thumb']));
                ...
                $store_list[$key]['brand_thumb'] = $this->sanmuRepository->getImagePath($row['brand_thumb']);
        ...
        if ($res) {
            foreach ($res as $key => $row) {
                ...
                if ($ru_id == 0) {
                    ...
                    if ($kf_im_switch) {
                        $goods_list[$key]['is_sanmu'] = true;
                    } else {
                        $goods_list[$key]['is_sanmu'] = false;
                    }
                } else {
                    $goods_list[$key]['is_sanmu'] = false;
                }
                ...
                $goods_list[$key]['market_price'] = $this->sanmuRepository->getPriceFormat($row['market_price']);
                $goods_list[$key]['shop_price'] = $this->sanmuRepository->getPriceFormat($row['shop_price']);
                $goods_list[$key]['promote_price'] = ($promote_price > 0) ? $this->sanmuRepository->getPriceFormat($promote_price) : '';
                $store_list[$key]['cancel_collect_goods'] = route('api.collect.collectgoods', ['rec_id' => $row['rec_id']]);
                $goods_list[$key]['url'] = sanmu_url('/#/goods/' . $row['goods_id']);
                $goods_list[$key]['app_page'] = config('route.goods.detail') . $row['goods_id'];
                $goods_list[$key]['shop_url'] = sanmu_url('/#/shopHome/' . $row['user_id']);
                $goods_list[$key]['goods_thumb'] = $this->sanmuRepository->getImagePath($row['goods_thumb']);
            }
        }
