        ...
        if ($recommend_brands) {
            foreach ($recommend_brands as $key => $row) {
                ...
                if ($row['site_url'] && strlen($row['site_url']) > 8) {
                    ...
                    $recommend_brands[$key]['url'] = $this->sanmuRepository->buildUri('brandn', ['bid' => $row['brand_id']], $row['brand_name']);
                }

                $recommend_brands[$key]['is_collect'] = $row['collect_count'];

                $recommend_brands[$key]['brand_logo'] = $this->sanmuRepository->getImagePath(DATA_DIR . '/brandlogo/' . $row['brand_logo']);
            }
        }
