                ...
                foreach ($res as $k => $v) {
                    ...
                    if ($child) {
                        foreach ($child as $key => $val) {
                            ...
                            if (isset($val['charge'])) {
                                $child[$key]['format_charge'] = $this->sanmuRepository->getPriceFormat($val['charge']);
                            }
                        }
                        $res[$k]['child'] = $child;
                    }

                    $res['format_count_charge'] = $this->sanmuRepository->getPriceFormat($res['count_charge']);
                    $res['format_no_cumulative_price'] = $this->sanmuRepository->getPriceFormat($res['no_cumulative_price']);
                }
            }
        }
        ...
        if ($res) {
            foreach ($res as $k => $v) {
                if ($v['entry_criteria']) {
                    ...
                    $res[$k]['grade_img'] = $this->sanmuRepository->getImagePath($v['grade_img']);
