...
class VerifyService
{
    ...
    public function infoVerify($user_id = 0)
    {
        ...
        if (!empty($res)) {
            $res['front_of_id_card'] = $this->sanmuRepository->getImagePath($res['front_of_id_card']);
            $res['reverse_of_id_card'] = $this->sanmuRepository->getImagePath($res['reverse_of_id_card']);
