...
class UserService
{
    ...
    public function userInfo($where = [])
    {
        ...
        if ($user) {
            ...
            if (isset($this->config['show_mobile']) && $this->config['show_mobile'] == 0) {
                $user['mobile_phone'] = $this->sanmuRepository->stringToStar($user['mobile_phone']);
                $user['user_name'] = $this->sanmuRepository->stringToStar($user['user_name']);
                $user['email'] = $this->sanmuRepository->stringToStar($user['email']);
            }

            $user['formated_user_money'] = $this->sanmuRepository->getPriceFormat($user['user_money'], false);
            $user['formated_frozen_money'] = $this->sanmuRepository->getPriceFormat($user['frozen_money'], false);
        }
    ...
    */
    ...
    public function getGoodsReportList($num = 10, $start = 0)
    {
        ...
        if ($row) {
            foreach ($row as $k => $v) {
                ...
                $row[$k]['url'] = $this->sanmuRepository->buildUri('goods', ['gid' => $v['goods_id']], $v['goods_name']);

                $ru_id = $v['get_goods']['user_id'] ?? 0;
                $row[$k]['shop_name'] = $this->merchantCommonService->getShopName($ru_id, 1);

                $row[$k]['goods_thumb'] = $this->sanmuRepository->getImagePath($v['goods_image']);
            }
        }
        ...
        if ($res) {
            foreach ($res as $row) {
                ...
                $arr[] = ['order_id' => $row['order_id'],
                    ...
                    'kf_ww' => $chat['kf_ww'],
                    'kf_qq' => $chat['kf_qq'],
                    'total_fee' => isset($row['total_fee']) ? $this->sanmuRepository->getPriceFormat($row['total_fee'], false) : 0,
    ...
    public function ReportImagesList($where = [])
    {
        ...
        if ($img_list) {
            foreach ($img_list as $key => $row) {
                $img_list[$key]['comment_img'] = $this->sanmuRepository->getImagePath($row['comment_img']);
            }
        }
    ...
    public function GetUserAccountLogList($user_id = 0, $account_type = '', $pager = [])
    {
        ...
        if ($res) {
            foreach ($res as $row) {
                ...
                $row['user_money'] = $this->sanmuRepository->getPriceFormat(abs($row['user_money']), false);
                $row['frozen_money'] = $this->sanmuRepository->getPriceFormat(abs($row['frozen_money']), false);
                $row['rank_points'] = abs($row['rank_points']);
                $row['pay_points'] = abs($row['pay_points']);
                $row['short_change_desc'] = $this->sanmuRepository->subStr($row['change_desc'], 60);
    ...
    public function getGoodsReportImgList($where = [])
    {
        ...
        if ($img_list) {
            foreach ($img_list as $key => $row) {
                $img_list[$key]['comment_img'] = $this->sanmuRepository->getImagePath($row['img_file']);
            }
        }
    ...
    public function getComplaintImgList($where = [])
    {
        ...
        if ($img_list) {
            foreach ($img_list as $key => $row) {
                $img_list[$key]['img_file'] = $this->sanmuRepository->getImagePath($row['img_file']);
                $img_list[$key]['comment_img'] = $this->sanmuRepository->getImagePath($row['comment_img']);
            }
        }
    ...
    public function getAccountLog($user_id, $size, $start)
    {
        ...
        if ($res) {
            foreach ($res as $rows) {
                ...
                $rows['short_admin_note'] = ($rows['admin_note'] > '') ? $this->sanmuRepository->subStr($rows['admin_note'], 30) : 'N/A';
                $rows['user_note'] = nl2br(htmlspecialchars($rows['user_note']));
                $rows['short_user_note'] = ($rows['user_note'] > '') ? $this->sanmuRepository->subStr($rows['user_note'], 30) : 'N/A';
                $rows['pay_status'] = ($rows['is_paid'] == 0) ? $GLOBALS['_LANG']['un_confirmed'] : $GLOBALS['_LANG']['is_confirmed'];
                $rows['amount'] = $this->sanmuRepository->getPriceFormat(abs($rows['amount']), false);
    ...
    public function getUserPayPoints($user_id, $page = 1, $size = 10, $type)
    {
        ...
        if ($res) {
            foreach ($res as $k => $row) {
                ...
                $row['type'] = $row[$type] > 0 ? '+' : '-';
                $row['user_money'] = $this->sanmuRepository->getPriceFormat(abs($row['user_money']), false);
                $row['frozen_money_format'] = $this->sanmuRepository->getPriceFormat(abs($row['frozen_money']), false);
                $row['rank_points'] = abs($row['rank_points']);
                $row['pay_points'] = abs($row['pay_points']);
                $row['short_change_desc'] = $this->sanmuRepository->subStr($row['change_desc'], 60);
        ...
        if (!empty($article_id)) {
            ...
            if ($res) {
                foreach ($res as $key => $row) {
                    ...
                    $arr[$key]['url'] = $this->sanmuRepository->buildUri('article', ['aid' => $row['article_id']], $row['title']);
                }
            }
        }
