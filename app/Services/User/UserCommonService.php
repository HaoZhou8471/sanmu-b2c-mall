...
class UserCommonService
{
    ...
    public function getUserByName($username)
    {
        ...
        if ($this->sanmuRepository->isEmail($username)) {
            $condition['field'] = 'email';
        } elseif ($this->sanmuRepository->isMobile($username)) {
    ...
    public function getUserDefault($user_id = 0)
    {
        ...
        if ($row) {
            ...
            $info['formated_credit_line'] = $this->sanmuRepository->getPriceFormat($info['credit_line'], false);
            $info['nick_name'] = !empty($row['nick_name']) ? $row['nick_name'] : $info['username'];
            $info['user_picture'] = $this->sanmuRepository->getImagePath($row['user_picture']);
            ...
            $info['surplus'] = $this->sanmuRepository->getPriceFormat($row['user_money'], false);
            $info['bonus'] = sprintf(lang('user.user_bonus_info'), $user_bonus['bonus_count'], $this->sanmuRepository->getPriceFormat($user_bonus['bonus_value'], false));

            $info['bonus_count'] = $user_bonus['bonus_count'];
            $info['bonus_value'] = $this->sanmuRepository->getPriceFormat($user_bonus['bonus_value']);
    ...
    {
        $modules = $this->sanmuRepository->readModules(plugin_path('Connect'));

        for ($i = 0; $i < count($modules); $i++) {

            $type = Str::studly($modules[$i]['type']);

            $this->sanmuRepository->helpersLang($modules[$i]['type'], 'Connect/' . $type . '/Languages/' . $this->config['lang'], 1);
