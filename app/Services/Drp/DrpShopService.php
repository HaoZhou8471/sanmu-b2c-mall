            ...
            foreach ($list as $key => $val) {
                ...
                if ($list[$key]['drp_parent_id'] > 0 && !empty($list[$key]['get_drp_parent'])) {
                    ...
                    if (isset($this->config['show_mobile']) && $this->config['show_mobile'] == 0 && !empty($list[$key]['parent_name'])) {
                        $list[$key]['parent_name'] = $this->sanmuRepository->stringToStar($list[$key]['parent_name']);
                    }
                }
                ...
                if (isset($this->config['show_mobile']) && $this->config['show_mobile'] == 0 && isset($val['get_users']) && !empty($val['get_users'])) {
                    ...
                    $list[$key]['shop_name'] = $this->sanmuRepository->stringToStar($list[$key]['shop_name']);
                }

                // 分销商头像
                if (isset($val['shop_portrait']) && $val['shop_portrait']) {
                    $list[$key]['shop_portrait'] = $this->sanmuRepository->getImagePath($val['shop_portrait']);
                } else {
                    $list[$key]['shop_portrait'] = $this->sanmuRepository->getImagePath($list[$key]['user_picture']);
                }
            }
        }
