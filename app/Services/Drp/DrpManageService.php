            ...
            foreach ($list as $k => $rt) {
                ...
                if (isset($rt['get_users']) && !empty($rt['get_users'])) {
                    ...
                    if (isset($this->config['show_mobile']) && $this->config['show_mobile'] == 0) {
                        $rt['user_name'] = $this->sanmuRepository->stringToStar($rt['user_name']);
                    }
                }
                ...
                if (!empty($rt['get_drp_logs'])) {
                    ...
                    foreach ($rt['get_drp_logs'] as $key => $val) {
                        ...
                        if (isset($this->config['show_mobile']) && $this->config['show_mobile'] == 0) {
                            $val['auser'] = $this->sanmuRepository->stringToStar($val['auser']);
                        }
        ...
        if (!empty($list)) {
            ...
            foreach ($list as $k => $rt) {
                ...
                if (isset($this->config['show_mobile']) && $this->config['show_mobile'] == 0 && isset($rt['shop_name'])) {
                    $rt['shop_name'] = $this->sanmuRepository->stringToStar($rt['shop_name']);
                }
                ...
                if (isset($rt['shop_portrait']) && $rt['shop_portrait']) {
                    $rt['shop_portrait'] = $this->sanmuRepository->getImagePath($rt['shop_portrait']);
                }
...
//            $list[$key]['credit_name'] = $res['credit_name'] ?? '';
            ...
            if (isset($this->config['show_mobile']) && $this->config['show_mobile'] == 0) {
                $list[$key]['shop_name'] = $this->sanmuRepository->stringToStar($val['shop_name']);
                $list[$key]['mobile'] = $this->sanmuRepository->stringToStar($val['mobile']);
                $list[$key]['real_name'] = $this->sanmuRepository->stringToStar($val['real_name']);
                $list[$key]['name'] = $this->sanmuRepository->stringToStar($list[$key]['name']);
            }
        }
                ...
                if ($row['is_separate'] == 0) {
                    ...
                    if (empty($row['user_id']) || empty($row['user_name'])) {
                        ...
                        if (file_exists(MOBILE_WECHAT)) {
                            ...
                            $url = sanmu_url('/#/drp/orderDetail', ['order_id' => $order_id]);
...
//                $val['day'] = isset($val['time']) ? $this->timeRepository->getLocalDate('y-m-d', $val['time'] + $time_diff) : '';
    ...
    public function checkUser($user_name = '')
    {
        ...
        if (!empty($res)) {
            ...
            $res['user_picture'] = $this->sanmuRepository->getImagePath($res['user_picture']);
