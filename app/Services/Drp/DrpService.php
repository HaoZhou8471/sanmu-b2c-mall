...
class DrpService
{
    ...
    public function shopInfo($user_id = 0, $shop_id = 0)
    {
        ...
        $info['shop_img'] = $this->sanmuRepository->getImagePath($info['shop_img']);
        if ($info['shop_portrait']) {
            $info['headimgurl'] = $this->sanmuRepository->getImagePath($info['shop_portrait']);
        } else {
            //用户名、头像
            $info['headimgurl'] = $this->sanmuRepository->getImagePath($info['user_picture']);
        }
        ...
        if ($model == 0) {
            ...
            if ($res) {
                foreach ($res as $k => $v) {
                    ...
                    $res[$k]['shop_price_formated'] = $this->sanmuRepository->getPriceFormat($v['shop_price'], true);
                    $res[$k]['url'] = sanmu_url('/#/goods/' . $v['goods_id']) . '?' . http_build_query(['parent_id' => $user_id], '', '&');
...
//        $credit_id = DrpShop::where('user_id', $user_id)->value('credit_id');
        ...
        if (!empty($res)) {
            foreach ($res as $key => $val) {
                ...
                if (isset($val['shop_portrait']) && $val['shop_portrait']) {
                    $list[$key]['user_picture'] = $this->sanmuRepository->getImagePath($val['shop_portrait']);
                } else {
                    $list[$key]['user_picture'] = $this->sanmuRepository->getImagePath($val['user_picture']);
                }
        ...
        if ($article) {
            ...
            if ($res) {
                foreach ($res as $key => $val) {
                    ...
                    // 过滤样式 手机自适应
                    $content = $this->sanmuRepository->contentStyleReplace($val['content']);
                    // 显示文章详情图片 （本地或OSS）
                    $content = $this->sanmuRepository->getContentImgReplace($content);
    ...
    public function createUserCard($user_id = 0, $type = 'url')
    {
        ...
        if (!file_exists($out_img) || $generate == true) {
            ...
            if ($type == 'qrcode' && file_exists(MOBILE_WECHAT)) {
                ...
                // 店铺申请页面链接
                $shop_id = $info['id'];
                $url = sanmu_url('/#/drp/register') . '?' . http_build_query(['parent_id' => $user_id, 'shop_id' => $shop_id], '', '&');
            }
    ...
    public function getMobileCategoryChild($cat_id = 0, $user_id = 0)
    {
        ...
        if ($arr) {
            foreach ($arr as $key => $v) {
                ...
                $arr[$key]['cat_icon'] = $this->sanmuRepository->getImagePath($v['cat_icon']);
                $arr[$key]['touch_icon'] = $this->sanmuRepository->getImagePath($v['touch_icon']);
                $arr[$key]['touch_catads'] = empty($v['touch_catads']) ? '' : $this->sanmuRepository->getImagePath($v['touch_catads']);
    ...
    public function getCategoryChild($cat_id = 0, $user_id = 0)
    {
        ...
        if ($arr) {
            foreach ($arr as $k => $v) {
                ...
                $arr[$k]['touch_icon'] = $this->sanmuRepository->getImagePath($v['touch_icon']);
                $arr[$k]['cat_icon'] = $this->sanmuRepository->getImagePath($v['cat_icon']);
    ...
    public function getCategoryGetGoods($uid = 0, $cat_id = 0)
    {
        ...
        foreach ($list as $k => $v) {
            $list[$k]['goods_thumb'] = $this->sanmuRepository->getImagePath($v['goods_thumb']);
            $list[$k]['shop_price_formated'] = $this->sanmuRepository->getPriceFormat($v['shop_price'], true);
