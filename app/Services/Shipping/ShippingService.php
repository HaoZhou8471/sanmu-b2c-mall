...
class ShippingService
{
    ...
    public function getRuShippngInfo($rec_ids, $user_id, $ru_id, $consignee = '', $whereCart = [])
    {
        ...
        if ($freight) {
            $freight = $this->sanmuRepository->delStrComma($freight);
        }
        ...
        if ($is_freight) {
            ...
            if ($shipping_list) {
                ...
                $str_shipping = $this->sanmuRepository->delStrComma($str_shipping);
                ...
                foreach ($shipping_list as $key => $val) {
                    if (substr($val['shipping_code'], 0, 5) != 'ship_') {
                        if ($this->config['freight_model'] == 0) {
                            ...
                            $shipping_list[$key]['free_money'] = $this->sanmuRepository->getPriceFormat(0, false);
                        }
                        ...
                        $shipping_list[$key]['format_shipping_fee'] = $this->sanmuRepository->getPriceFormat($shipping_fee, false);
                        $shipping_list[$key]['shipping_fee'] = $shipping_fee;

                        if (isset($val['insure']) && $val['insure']) {
                            $shipping_list[$key]['insure_formated'] = strpos($val['insure'], '%') === false ? $this->sanmuRepository->getPriceFormat($val['insure'], false) : $val['insure'];
                        }
            ...
            $shipping_list[0]['free_money'] = $this->sanmuRepository->getPriceFormat(0, false);
            $shipping_list[0]['format_shipping_fee'] = $this->sanmuRepository->getPriceFormat($shipping_fee, false);
