...
class CategoryManageService
{
    ...
    public function getCategoryInfo($cat_id = 0)
    {
        ...
        if ($info) {
            if ($info['cat_icon']) {
                $info['cat_icon'] = $this->sanmuRepository->getImagePath($info['cat_icon']);
            }

            if ($info['touch_icon']) {
                $info['touch_icon'] = $this->sanmuRepository->getImagePath($info['touch_icon']);
            }

            if ($info['touch_catads']) {
                $info['touch_catads'] = $this->sanmuRepository->getImagePath($info['touch_catads']);
            }
        }
    ...
    public function getTouchImagesUpload($name = '', $tmp_name = '', $size = 0, $path = [], $link = [])
    {
        ...
        if (!empty($name)) {
            ...
            $this->sanmuRepository->getOssAddFile([$icon]); //oss存储图片
        }
