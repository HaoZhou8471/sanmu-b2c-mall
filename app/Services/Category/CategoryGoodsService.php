...
class CategoryGoodsService
{
    ...
    public function getCategoryRecommendGoods($cats = '', $type = '', $brand_id = 0, $warehouse_id = 0, $area_id = 0, $area_city = 0, $where_ext = [], $min = 0, $max = 0, $num = 10, $start = 0)
    {
        ...
        if ($goods === false) {
            ...
            $res = $this->sanmuRepository->getWhereRsid($res, 'user_id', 0, $this->city);
    ...
    public function getCatCompare($goods_ids, $compare = '', $highlight = '', $warehouse_id = 0, $area_id = 0, $area_city = 0)
    {
        ...
        if ($res) {
            foreach ($res as $row) {
                ...
                $goods_list[$goods_id]['url'] = $this->sanmuRepository->buildUri('goods', ['gid' => $row['goods_id']], $row['goods_name']);
                $goods_list[$goods_id]['goods_name'] = $row['goods_name'];
                $goods_list[$goods_id]['shop_price'] = $this->sanmuRepository->getPriceFormat($row['shop_price']);
                $goods_list[$goods_id]['promote_price'] = ($promote_price > 0) ? $this->sanmuRepository->getPriceFormat($promote_price) : '';
                ...
                $goods_list[$goods_id]['goods_thumb'] = $this->sanmuRepository->getImagePath($row['goods_thumb']);
    ...
    public function getMobileCategoryGoodsList($uid = 0, $keywords = [], $children = '', $brand_id = 0, $price_min = 0, $price_max = 0, $filter_attr = [], $where_ext = [], $goods_num = 0, $size = 10, $page = 1, $sort = 'goods_id', $order = 'DESC')
    {
                ...
                $query->whereHas('getPresaleActivity', function ($query) use ($keywordsParam) {
                    $query->where(function ($query) use ($keywordsParam) {
                        foreach ($keywordsParam['keywords'] as $key => $val) {
                            ...
                            $query->where(function ($query) use ($val) {
                                $val = $this->sanmuRepository->mysqlLikeQuote(trim($val));
        ...
        if ($res) {
            foreach ($res as $k => $row) {
                ...
                $arr[$k]['market_price'] = $this->sanmuRepository->getPriceFormat($row['market_price']);
                $arr[$k]['shop_price'] = $row['shop_price'];
                if ($promote_price > 0) {
                    $arr[$k]['shop_price_formated'] = $this->sanmuRepository->getPriceFormat($row['promote_price']);
                } else {
                    $arr[$k]['shop_price_formated'] = $this->sanmuRepository->getPriceFormat($row['shop_price']);
                }
                $arr[$k]['type'] = $row['goods_type'];
                $arr[$k]['promote_price'] = ($promote_price > 0) ? $this->sanmuRepository->getPriceFormat($row['promote_price']) : '';
                $arr[$k]['goods_thumb'] = $this->sanmuRepository->getImagePath($row['goods_thumb']);
                $arr[$k]['goods_img'] = $this->sanmuRepository->getImagePath($row['goods_img']);
                $arr[$k]['original_img'] = $this->sanmuRepository->getImagePath($row['original_img']);
                ...
                $arr[$k]['url'] = sanmu_url('/#/goods/' . $row['goods_id']);
 ...
 /*        $res = $res->with([
        ...
        if ($res) {
            foreach ($res as $row) {
                ...
                $arr[$row['goods_id']]['goods_style_name'] = $this->goodsCommonService->addStyle($row['goods_name'], '');

                $arr[$row['goods_id']]['market_price'] = $this->sanmuRepository->getPriceFormat($row['market_price']);
                ...
                $arr[$row['goods_id']]['goods_img'] = $this->sanmuRepository->getImagePath($row['goods_img']);
                ...
                if ($seller_id == 0) {
                   ...
                   // $kf_im_switch = SellerShopinfo::where('ru_id', 0)->value('kf_im_switch');
                    ...
                    if ($kf_im_switch) {
                        $arr[$row['goods_id']]['is_sanmu'] = true;
                    } else {
                        $arr[$row['goods_id']]['is_sanmu'] = false;
                    }
                } else {
                    $arr[$row['goods_id']]['is_sanmu'] = false;
                }
    ...
    public function getGoodsPriceMaxMin($children = [], $brand_id = 0, $warehouse_id = 0, $area_id = 0, $area_city = 0, $where_ext = [], $goods_ids = [], $keywords = [], $sc_ds = '', $cat_type = 'cat_id')
    {
        ...
        $goodsParam = [
            ...
            'cat_type' => $cat_type
                 ...
                 /* 查询分类商品数据 */
        $res = Goods::select('shop_price', 'market_price')->where('is_on_sale', 1)
            ->where('is_alone_sale', 1)
            ->where('is_delete', 0)
            ->where(function ($query) use ($goodsParam) {
                if ($goodsParam['children']) {
