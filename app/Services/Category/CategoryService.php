...
class CategoryService
{
        ...
        if ($arr === false) {
            ...
            if ($categories_child) {
                foreach ($categories_child as $key => $val) {
                    ...
                    $val['cat_icon'] = !empty($val['cat_icon']) ? $this->sanmuRepository->getImagePath($val['cat_icon']) : '';
                    $val['touch_catads'] = !empty($val['touch_catads']) ? $this->sanmuRepository->getImagePath($val['touch_catads']) : '';
                    $val['touch_icon'] = !empty($val['touch_icon']) ? $this->sanmuRepository->getImagePath($val['touch_icon']) : '';
        ...
        if ($arr === false) {
            ...
            if ($arr) {
                foreach ($arr as $key => $v) {
                    ...
                    $arr[$key]['touch_catads'] = $this->sanmuRepository->getImagePath($v['touch_catads']);

                    if ($v['touch_icon']) {
                        $arr[$key]['touch_icon'] = $this->sanmuRepository->getImagePath($v['touch_icon']);
                    } else {
                        $arr[$key]['touch_icon'] = $this->sanmuRepository->getImagePath($v['get_goods']['goods_thumb'] ?? '');
                    }
    ...
    public function catList($cat_id = 0, $type = 0, $getrid = 0, $table = 'category', $seller_shop_cat = [], $cat_level = 0, $user_id = 0)
    {
        ...
        if ($seller_shop_cat) {
            if ($seller_shop_cat['parent'] && $seller_shop_cat['parent'] && $cat_level < 3) {
                $seller_shop_cat['parent'] = $this->sanmuRepository->delStrComma($seller_shop_cat['parent']);
            }
        }
                ...
                foreach ($res as $key => $row) {
                    if ($getrid == 0) {
                        ...
                        if ($table == 'merchants_category') {
                            ...
                            $arr[$row['cat_id']]['url'] = $this->sanmuRepository->buildUri('category', ['cid' => $row['cat_id']], $row['cat_name']);
                        }
                    } else {
                ...
                foreach ($res as $row) {
                    ...
                    if ($getrid == 0) {
                        ...
                        if ($table == 'merchants_category') {
                            ...
                            $three_arr[$row['cat_id']]['url'] = $this->sanmuRepository->buildUri('category', ['cid' => $row['cat_id']], $row['cat_name']);
                        }
    ...
    public function getArrayKeysCat($cat_id = 0, $type = 0, $table = 'category')
    {
        ...
        if ($type == 1) {
            if ($list) {
                $list = implode(',', $list);
                $list = $this->sanmuRepository->delStrComma($list);
            }
        }
