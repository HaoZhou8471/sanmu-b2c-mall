        ...
        if ($res && $type == 0) {
            foreach ($res as $k => $v) {
                ...
                if ($v['extension_code'] == 'package_buy') {
                    ...
                    if ($package) {
                        $res[$k]['goods_thumb'] = !empty($package['activity_thumb']) ? $this->sanmuRepository->getImagePath($package['activity_thumb']) : $this->sanmuRepository->sanmuUrl('themes/ecmoban_sanmu2017/images/17184624079016pa.jpg');
                    }

                    $res[$k]['package_goods_list'] = $this->packageGoodsService->getPackageGoods($v['goods_id']);
                } else {
                    $res[$k]['url'] = $this->sanmuRepository->buildUri('goods', ['gid' => $v['goods_id']], $v['goods_name']);
                    $res[$k]['goods_thumb'] = $this->sanmuRepository->getImagePath($goods['goods_thumb']);
                }
                ...
                $res[$k]['shop_price'] = $this->sanmuRepository->getPriceFormat($res[$k]['goods_price']);
                $res[$k]['subtotal'] = $v['goods_price'] * $v['goods_number'];
                $res[$k]['format_subtotal'] = $this->sanmuRepository->getPriceFormat($res[$k]['subtotal']);
                 ...
                 */
                $integral = [
                    $this->sanmuRepository->integralOfValue($v['goods_price'] * $v['goods_number']),
                    $this->sanmuRepository->integralOfValue($v['integral'] * $v['goods_number'])
                ];

                $integral = $this->baseRepository->getArrayMin($integral);
                $res[$k]['integral_total'] = $this->sanmuRepository->valueOfIntegral($integral);
