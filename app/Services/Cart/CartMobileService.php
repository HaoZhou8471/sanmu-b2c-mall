...
class CartMobileService
{
    ...
    public function getGoodsCartListMobile($where = [])
    {
        ...
        if ($res) {
            ...
            foreach ($res as $k => $v) {
                ...
                $res[$k]['short_name'] = $this->config['goods_name_length'] > 0 ? $this->sanmuRepository->subStr($v['goods_name'], $this->config['goods_name_length']) : $v['goods_name'];
                $res[$k]['goods_number'] = $v['goods_number'];
                $res[$k]['goods_name'] = $v['goods_name'];
                $res[$k]['goods_price'] = $v['goods_price'];
                $res[$k]['goods_price_format'] = $this->sanmuRepository->getPriceFormat($v['goods_price']);
                $res[$k]['market_price_format'] = $this->sanmuRepository->getPriceFormat($v['market_price']);
        ...
        foreach ($merchant_goods as $key => $row) { // 第一层 遍历商家
            ...
            if ($user_cart_goods) {
                foreach ($user_cart_goods as $goods_key => $goods_row) {
                    // 第二层 遍历购物车中商家的商品
                    $goods_row['market_price_formated'] = $this->sanmuRepository->getPriceFormat($goods_row['market_price'], false);
                    $goods_row['goods_price_formated'] = $this->sanmuRepository->getPriceFormat($goods_row['goods_price'], false);
                    $goods_row['goods_thumb'] = $this->sanmuRepository->getImagePath($goods_row['goods_thumb']);
    ...
    public function favourable_list($user_id = 0, $ru_id = 0, $act_sel_id = [])
    {
        ...
        if ($list) {
            foreach ($list as $favourable) {
                ...
                $favourable['formated_min_amount'] = $this->sanmuRepository->getPriceFormat($favourable['min_amount']);
                $favourable['formated_max_amount'] = $this->sanmuRepository->getPriceFormat($favourable['max_amount']);
                ...
                foreach ($favourable['gift'] as $key => $value) {
                    ...
                    if (!empty($goods)) {
                        ...
                        $favourable['gift'][$key]['formated_price'] = $this->sanmuRepository->getPriceFormat($value['price'], false);
                        // 赠品缩略图
                        $favourable['gift'][$key]['thumb_img'] = $this->sanmuRepository->getImagePath($goods['goods_thumb']);
    ...
    public function getCartGoods($user_id, $type = CART_GENERAL_GOODS, $cart_value = '', $is_checked = true)
    {
        ...
        foreach ($res as $k => $v) {
            ...
            $res[$k]['goods_price_format'] = $this->sanmuRepository->getPriceFormat($res[$k]['goods_price']);
            ...
            if (isset($goods['get_goods_consumption']) && $goods['get_goods_consumption']) {
                $res[$k]['amount'] = $this->sanmuRepository->getGoodsConsumptionPrice($goods['get_goods_consumption'], $row['goods_amount']);
            } else {
                $res[$k]['amount'] = $row['goods_amount'];
            }

            $res[$k]['subtotal'] = $row['goods_amount'];
            $res[$k]['formated_subtotal'] = $this->sanmuRepository->getPriceFormat($row['goods_amount'], false);
            $res[$k]['dis_amount'] = $row['goods_amount'] - $res[$k]['amount'];
            $res[$k]['dis_amount'] = number_format($res[$k]['dis_amount'], 2, '.', '');
            $res[$k]['discount_amount'] = $this->sanmuRepository->getPriceFormat($res[$k]['dis_amount'], false);
        }
