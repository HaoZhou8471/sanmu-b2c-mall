    ...
    public function getSelfPointCart($where = [])
    {
        ...
        if ($self_point) {
            foreach ($self_point as $key => $row) {
                ...
                $self_point[$key]['img_url'] = isset($row['img_url']) && !empty($row['img_url']) ? $this->sanmuRepository->getImagePath($row['img_url']) : '';
    ...
    public function getUserCartInfo($type = 0)
    {
        ...
        if ($row) {
            foreach ($row as $k => $v) {
                ...
                $arr[$k]['goods_thumb'] = $this->sanmuRepository->getImagePath($v['goods_thumb']);
                $arr[$k]['short_name'] = $this->config['goods_name_length'] > 0 ?
                    $this->sanmuRepository->subStr($v['goods_name'], $this->config['goods_name_length']) : $v['goods_name'];
                $arr[$k]['url'] = $this->sanmuRepository->buildUri('goods', ['gid' => $v['goods_id']], $v['goods_name']);
                $arr[$k]['goods_number'] = $v['goods_number'];
                $arr[$k]['goods_name'] = $v['goods_name'];
                $arr[$k]['goods_price'] = $this->sanmuRepository->getPriceFormat($v['goods_price']);
        ...
        if ($type == 1) {
            $cart = ['goods_list' => $arr, 'number' => $number, 'amount' => $this->sanmuRepository->getPriceFormat($amount, false)];
            return $cart;
        } elseif ($type == 2) {
            $cart = ['goods_list' => $arr, 'number' => $number, 'amount' => $this->sanmuRepository->getPriceFormat($amount, false)];
            return $cart;
                ...
                if ($row['get_goods_attr_list']) {
                    foreach ($row['get_goods_attr_list'] as $idx => $val) {
                        ...
                        $attr = [
                            ...
                            'price' => $attr_price,
                            'format_price' => $this->sanmuRepository->getPriceFormat($attr_price, false),
