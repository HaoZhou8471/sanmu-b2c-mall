                ...
                switch ($row['media_type']) {
                    case 0:
                        ...
                        if ((strpos($row['ad_code'], 'http://') === false && strpos($row['ad_code'], 'https://') === false)) {
                            $row['ad_code'] = $this->sanmuRepository->getImagePath('data/afficheimg/' . $row['ad_code']);
                        }
    ...
    public function getAdPostiChild($cat_n_child = '', $warehouse_id = 0, $area_id = 0, $area_city = 0)
    {
        ...
        if ($res) {
            foreach ($res as $key => $row) {
                ...
                if ($row['ad_code']) {
                    if (strpos($row['ad_code'], 'http://') === false && strpos($row['ad_code'], 'https://') === false) {
                        $arr[$key]['ad_code'] = $this->sanmuRepository->getImagePath(DATA_DIR . '/afficheimg/' . $row['ad_code']);
                    } else {
                        $src = $row['ad_code'];
                        $src = str_replace('../', '', $src);
                        $arr[$key]['ad_code'] = $this->sanmuRepository->getImagePath($src);
                    }
                }

                if ($row['ad_bg_code']) {
                    if (strpos($row['ad_bg_code'], 'http://') === false && strpos($row['ad_bg_code'], 'https://') === false) {
                        $arr[$key]['ad_bg_code'] = $this->sanmuRepository->getImagePath(DATA_DIR . '/afficheimg/' . $row['ad_bg_code']);
                    } else {
                        $src = $row['ad_bg_code'];
                        $src = str_replace('../', '', $src);
                        $arr[$key]['ad_bg_code'] = $this->sanmuRepository->getImagePath($src);
                    }
                }
