...
class ExchangeService
{
    ...
    public function getExchangeRecommendGoods($where = [])
    {
        ...
        if ($res) {
            foreach ($res as $row) {
                ...
                $goods[$idx]['url'] = $this->sanmuRepository->buildUri('exchange_goods', ['gid' => $row['goods_id']], $row['goods_name']);
    ...
    public function getExchangeGoodsInfo($user_id = 0, $goods_id = 0, $warehouse_id = 0, $area_id = 0, $area_city = 0)
    {
        ...
        if (!empty($row)) {
            ...
            $row['market_price_formated'] = $this->sanmuRepository->getPriceFormat($row['market_price']);
            $row['goods_price'] = $this->sanmuRepository->getPriceFormat($row['exchange_integral'] * $this->config['integral_scale'] / 100);
            ...
            $row['integral_money'] = $this->sanmuRepository->getPriceFormat($this->config['integral_scale']);
