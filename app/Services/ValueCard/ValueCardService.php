    ...
    public function valueCardList($user_id, $page = 1, $size = 10)
    {
        ...
        if ($res) {
            foreach ($res as $key => $row) {
                ...
                $res[$key]['vc_value'] = $this->sanmuRepository->getPriceFormat($row['vc_value']);
                $res[$key]['use_condition'] = condition_format($row['use_condition']);
                $res[$key]['card_money'] = $this->sanmuRepository->getPriceFormat($row['card_money']);
                ...
                foreach ($res as $key => $row) {
                    if (!empty($row['get_order']) || $row['order_id'] == 0) {
                        if ($row['use_val'] > 0 && $row['add_val'] > 0) {
                            $row['add_val'] = 0;
                            $arr[$key]['use_val'] = $row['use_val'] > 0 ? '+' . $this->sanmuRepository->getPriceFormat($row['use_val']) : $this->sanmuRepository->getPriceFormat($row['use_val']);
                        } else {
                            $arr[$key]['use_val'] = $row['use_val'] > 0 ? '-' . $this->sanmuRepository->getPriceFormat($row['use_val']) : $this->sanmuRepository->getPriceFormat($row['use_val']);
                        }

                        $arr[$key]['add_val'] = $row['add_val'] > 0 ? '+' . $this->sanmuRepository->getPriceFormat($row['add_val']) : $this->sanmuRepository->getPriceFormat($row['add_val']);
