...
class CommissionService
{
    ...
    public function getBillDetail($id = 0, $start_time = 0, $end_time = 0)
    {
        ...
        if ($row) {
            ...
            $row['negative_bill']['format_amount'] = $this->sanmuRepository->getPriceFormat($negative_bill['amount'], false);
            $row['negative_bill']['format_shippingfee'] = $this->sanmuRepository->getPriceFormat($negative_bill['shippingfee'], false);
            $row['negative_bill']['format_total'] = $this->sanmuRepository->getPriceFormat($negative_bill['total'], false);
            ...
            $row['format_rate_fee'] = $this->sanmuRepository->getPriceFormat($row['rate_fee'], false);
            $row['format_order_amount'] = $this->sanmuRepository->getPriceFormat($row['order_amount'], false);

            $row['format_return_fee'] = $this->sanmuRepository->getPriceFormat($row['return_amount'], false);
            $row['format_shipping_fee'] = $this->sanmuRepository->getPriceFormat($row['return_shippingfee'], false);

            $row['format_shipping_amount'] = $this->sanmuRepository->getPriceFormat($row['shipping_amount'], false);
            $row['format_return_amount'] = $this->sanmuRepository->getPriceFormat($row['return_amount'] + $row['return_shippingfee'], false);
            ...
            if ($row['negative_amount'] == 0) {
                $row['format_negative_amount'] = $this->sanmuRepository->getPriceFormat($row['negative_bill']['total'], false);
            } else {
                $row['format_negative_amount'] = $this->sanmuRepository->getPriceFormat($row['negative_amount'], false);
            }

            $row['format_gain_commission'] = $this->sanmuRepository->getPriceFormat($row['gain_commission'], false);
            $row['format_should_amount'] = $this->sanmuRepository->getPriceFormat($row['should_amount'] - $row['settle_accounts'], false);
            $row['format_frozen_money'] = $this->sanmuRepository->getPriceFormat($row['frozen_money'], false);
        ...
        if ($row) {
            foreach ($row as $key => $value) {
                ...
                if (empty($value['chargeoff_status'])) {
                    ...
                    $row[$key]['format_gain_commission'] = $this->sanmuRepository->getPriceFormat($detail['gain_commission'], false);
                    $row[$key]['format_should_amount'] = $this->sanmuRepository->getPriceFormat($should_amount, false);
                } else {
                    ...
                    $row[$key]['format_gain_commission'] = $this->sanmuRepository->getPriceFormat($value['gain_commission'], false);
                    $row[$key]['format_should_amount'] = $this->sanmuRepository->getPriceFormat($value['should_amount'], false);
                }
                ...
                /** 合计已单独结算订单佣金金额 end * */
                $row[$key]['format_order_amount'] = $this->sanmuRepository->getPriceFormat($value['order_amount'], false);
                $row[$key]['format_shipping_amount'] = $this->sanmuRepository->getPriceFormat($value['shipping_amount'], false);
                $row[$key]['format_return_amount'] = $this->sanmuRepository->getPriceFormat($value['return_amount'], false);
                $row[$key]['format_drp_money'] = $this->sanmuRepository->getPriceFormat($value['drp_money'], false);
                $row[$key]['format_frozen_money'] = $this->sanmuRepository->getPriceFormat($value['frozen_money'], false);
        ...
        if ($row) {
            foreach ($row as $key => $value) {
                ...
                if ($value['commission_model']) {
                    ...
                    $row[$key]['format_gain_commission'] = $this->sanmuRepository->getPriceFormat($format_gain_commission, false);

                    $row[$key]['format_drp_money'] = $this->sanmuRepository->getPriceFormat($value['drp_money'], false);
                    $row[$key]['format_integral_money'] = $this->sanmuRepository->getPriceFormat($value['integral_money'], false);
                ...
                $should_amount = $this->sanmuRepository->changeFloat($should_amount);       //应结佣金

                if ($should_amount == 0 && $goods_rate) {
                    $value['commission_total_fee'] = $goods_rate['total_fee'] + $goods_rate['rate_activity'];
                }

                $row[$key]['gain_commission'] = $this->sanmuRepository->changeFloat($gain_commission);   //收取佣金
                ...
                $format_should_amount = $this->sanmuRepository->changeFloat($format_should_amount);       //应结佣金
                ...
                $row[$key]['format_gain_commission'] = $this->sanmuRepository->getPriceFormat($format_gain_commission, false);
                $row[$key]['format_should_amount'] = $this->sanmuRepository->getPriceFormat($format_should_amount, false);

                $row[$key]['format_drp_money'] = $this->sanmuRepository->getPriceFormat($value['drp_money'], false);
                $row[$key]['format_integral_money'] = $this->sanmuRepository->getPriceFormat($value['integral_money'], false);
                $row[$key]['format_order_amount'] = $this->sanmuRepository->getPriceFormat($value['order_amount'], false);
                $row[$key]['commission_total_fee'] = $value['commission_total_fee'];
                $row[$key]['format_commission_total_fee'] = $this->sanmuRepository->getPriceFormat($value['commission_total_fee'], false);
                $row[$key]['format_shipping_fee'] = $this->sanmuRepository->getPriceFormat($value['shipping_fee'], false);
                $row[$key]['format_return_amount'] = $this->sanmuRepository->getPriceFormat($return_amount + $return_shippingfee, false);
                ...
                if ($value['order_status'] == OS_RETURNED) {
                    $row[$key]['gain_commission'] = $this->sanmuRepository->getPriceFormat(0, false);
                    $row[$key]['should_amount'] = $this->sanmuRepository->getPriceFormat(0, false);
                    $row[$key]['goods_rate']['gain_commission'] = $this->sanmuRepository->getPriceFormat(0, false);
                    $row[$key]['goods_rate']['should_amount'] = $this->sanmuRepository->getPriceFormat(0, false);
                }
            }
        }
    ...
    public function getAloneGoodsRate($order_id = 0, $ru_id = 0, $order = [])
    {
        ...
        if ($goods_list) {
            ...
            $row['format_total_fee'] = $this->sanmuRepository->changeFloat($row['total_fee']);
            ...
            $row['gain_commission'] = $this->sanmuRepository->changeFloat($gain_commission);
            $row['should_amount'] = $this->sanmuRepository->changeFloat($should_amount);

            /* 有效佣金商品金额 */
            $row['total_fee'] = $this->sanmuRepository->changeFloat($row['format_total_fee'] - $res['rate_activity']);
            $row['format_total_fee'] = $this->sanmuRepository->getPriceFormat($row['total_fee'], false);
    ...
    public function getOrderDrpMoney($total_fee = 0, $ru_id = 0, $order_id = 0, $order = [])
    {
        ...
        if ($order_id) {
            ...
            $res['goods_rate_total'] = isset($goods_rate['total_fee']) ? $this->sanmuRepository->changeFloat($goods_rate['total_fee']) : 0;
            $res['order_activity'] = isset($goods_rate['order_activity']) ? $this->sanmuRepository->changeFloat($goods_rate['order_activity']) : 0;
            $res['rate_activity'] = isset($goods_rate['rate_activity']) ? $this->sanmuRepository->changeFloat($goods_rate['rate_activity']) : 0;
    ...
    public function getSellerSettlementAmount($order_id = 0, $ru_id = 0)
    {
        ...
        $actual_amount = $this->sanmuRepository->changeFloat($actual_amount);
    ...
    public function getOrderGoodsCommission($order_id = 0, $type = 0)
    {
        ...
        if ($type == 1) {
            $arr = [
                'gain_commission' => $this->sanmuRepository->changeFloat($gain_commission),
                'should_amount' => $this->sanmuRepository->changeFloat($should_amount),
                'cat' => $cat,
        ...
        if ($other) {
            ...
            if ($goods_list) {
                ...
                if ($bill_id > 0) {
                    ...
                    $gain_amount = $this->sanmuRepository->changeFloat($gain_amount);
    ...
    public function billGoodsList()
    {
        ...
        if ($row) {
            foreach ($row as $key => $value) {
                ...
                $row[$key]['format_goods_price'] = $this->sanmuRepository->getPriceFormat($value['goods_price'], false);
                $row[$key]['format_drp_money'] = $this->sanmuRepository->getPriceFormat($value['drp_money'], false);
    ...
    public function merchantsIsSettlement($ru_id = 0, $state = '', $filter = [])
    {
        ...
        if ($filter) {
            if (isset($filter['order_sn']) && $filter['order_sn']) {
                $row = $row->where('order_sn', 'like', '%' . $this->sanmuRepository->mysqlLikeQuote($filter['order_sn']) . '%');
            }

            if (isset($filter['consignee']) && $filter['consignee']) {
                $row = $row->where('consignee', 'like', '%' . $this->sanmuRepository->mysqlLikeQuote($filter['consignee']) . '%');
            }
        ...
        if ($row) {
            for ($i = 0; $i < count($row); $i++) {
                ...
                $row[$i]['formated_order_amount'] = $this->sanmuRepository->getPriceFormat($row[$i]['order_amount'], true);

                $row[$i]['formated_money_paid'] = $this->sanmuRepository->getPriceFormat($row[$i]['money_paid'], true);
                if (!isset($row[$i]['total_fee'])) {
                    $row[$i]['total_fee'] = 0;
                }
                $row[$i]['formated_total_fee'] = $this->sanmuRepository->getPriceFormat($row[$i]['total_fee'], true);

                $effective_amount = isset($row[$i]['return_amount']) ? $row[$i]['total_fee'] - $row[$i]['return_amount'] : $row[$i]['total_fee'];
                $row[$i]['formated_brokerage_amount'] = $this->sanmuRepository->getPriceFormat($effective_amount * $percent_value, true);
                $row[$i]['formated_effective_amount'] = $this->sanmuRepository->getPriceFormat($effective_amount, true);
                ...
                $row[$i]['formated_return_amount'] = $this->sanmuRepository->getPriceFormat($row[$i]['return_amount'], true);
                ...
                if (file_exists(MOBILE_DRP)) {
                    ...
                    $row[$i]['formated_brokerage_amount'] = $this->sanmuRepository->getPriceFormat($total_return_amount * $percent_value, true);
                    $row[$i]['formated_effective_amount'] = $this->sanmuRepository->getPriceFormat($total_return_amount, true);
        ...
        if (file_exists(MOBILE_DRP)) {
            if ($row) {
                ...
                $row['all_gain_commission'] = $this->sanmuRepository->changeFloat($row['gain_commission_amount']);
                $row['all_price'] = $this->sanmuRepository->changeFloat($row['all_brokerage_amount']);
                $row['all_drp_price'] = $this->sanmuRepository->changeFloat($row['all_drp']);

                $row['gain_all'] = $this->sanmuRepository->getPriceFormat($row['gain_commission_amount'], true);
                $row['all'] = $this->sanmuRepository->getPriceFormat($row['all_brokerage_amount'], true);
                $row['all_drp'] = $this->sanmuRepository->getPriceFormat($row['all_drp'], true);
            }
    ...
    public function getSettlementPrice($ru_id = 0, $order_id = 0, $type = -1, $sumType = '')
    {
        ...
        $total = $total > 0 ? $this->sanmuRepository->changeFloat($total) : 0;
