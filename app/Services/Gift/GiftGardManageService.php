...
class GiftGardManageService
{
    ...
    public function getGiftGardTypeList($ru_id)
    {
        ...
        //卖场
        $res = $this->sanmuRepository->getWhereRsid($res, 'ru_id', $filter['rs_id']);
    ...
    public function getUserGiftGardList($ru_id = 0)
    {
        ...
        foreach ($row as $key => $val) {
            ...
            if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                $val['user_name'] = $this->sanmuRepository->stringToStar($val['user_name']);
                $val['mobile'] = $this->sanmuRepository->stringToStar($val['mobile']);
            }
    ...
    public function sendBonusMail($bonus_type_id, $bonus_id_list)
    {
        ...
        foreach ($bonus_list as $bonus) {
            ...
            $GLOBALS['smarty']->assign('money', $this->sanmuRepository->getPriceFormat($bonus_type['type_money']));
