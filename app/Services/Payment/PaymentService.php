...
class PaymentService
{
    ...
    public function availablePaymentList($support_cod = 0, $cod_fee = 0, $is_online = 0, $is_balance = 0)
    {
        ...
        foreach ($payment_list as $key => $payment) {
            ...
            $payment['format_pay_fee'] = strpos($payment['pay_fee'], '%') !== false ? $payment['pay_fee'] : $this->sanmuRepository->getPriceFormat($payment['pay_fee'], false);
            ...
            if (isset($log_id) && !empty($log_id)) {
                ...
                if ($pay_log['order_type'] == PAY_ORDER) {
                    ...
                    if ($order['extension_code'] == 'team_buy') {
                        // 拼团
                        $result['extension_code'] = $order['extension_code'];
                        $result['url'] = sanmu_url('/#/team/wait') . '?' . http_build_query(['team_id' => $order['team_id'], 'status' => 1], '', '&');
                    } elseif ($order['is_zc_order'] == 1 && $order['zc_goods_id'] > 0) {
                        // 众筹
                        $result['extension_code'] = 'crowd_buy';
                        $result['url'] = sanmu_url('/#/crowdfunding/order');
                    } else {
                        // 普通订单
                        $result['url'] = sanmu_url('/#/user/orderDetail/' . $pay_log['order_id']);
                    }

                } elseif ($pay_log['order_type'] == PAY_WHOLESALE) {
                    // 供求订单支付
                    $result['url'] = sanmu_url('/#/supplier/orderlist');
                } else {
                    // 无订单类支付
                    if ($pay_log['order_type'] == PAY_REGISTERED) {
                        // 分销购买
                        $result['extension_code'] = 'drp_buy';
                        $result['url'] = sanmu_url('/#/drp/drpinfo');
                    } elseif ($pay_log['order_type'] == PAY_SURPLUS) {
                        // 会员充值
                        $result['url'] = sanmu_url('/#/user/account');
                    }
                }
    ...
    public function Wxh5($code = '', $log_id = 0)
    {
        ...
        if (!empty($log_id)) {
            ...
            if ($pay_log['order_type'] == PAY_ORDER) {
                $order_url = sanmu_url('/#/user/orderDetail') . '/' . $pay_log['order_id'];
            } elseif ($pay_log['order_type'] == PAY_SURPLUS) {
                $order_url = sanmu_url('/#/user/account');
            } elseif ($pay_log['order_type'] == PAY_REGISTERED) {
                //分销购买
                $order_url = sanmu_url('/#/drp');
            } elseif ($pay_log['order_type'] == PAY_TOPUP) {
                //拼团
                $order_url = sanmu_url('/#/team/order');
            } elseif ($pay_log['order_type'] == PAY_WHOLESALE) {
                //供求
                $result['url'] = sanmu_url('/#/supplier/orderlist');
            } else {
                $order_url = sanmu_url('/#/user/order');
            }
            // 支付状态
            $args = [
                'code' => $code,
            ...
            $respond_url = sanmu_url('/#/respond?' . http_build_query($args, '', '&'));
        } else {
            $args = [
                'code' => $code,
                'status' => 0
    ...
    public function wxpayQrcode($code_url = '')
    {
        ...
        $qrcode_right = asset('themes/ecmoban_sanmu2017/images/weixin-qrcode.jpg');
        $sj = asset('themes/ecmoban_sanmu2017/images/sj.png');
