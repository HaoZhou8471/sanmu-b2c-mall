    ...
    public function getPackageGoods($package_id, $seller_id = 0, $type = 0)
    {
        ...
        if ($resource) {
            foreach ($resource as $_row) {
                ...
                $_row['goods_thumb'] = $this->sanmuRepository->getImagePath($_row['goods_thumb']);
                ...
                $_row['url'] = $this->sanmuRepository->buildUri('goods', ['gid' => $_row['goods_id']], $_row['goods_name']);
                $_row['shop_price'] = $this->sanmuRepository->getPriceFormat($_row['shop_price']);
