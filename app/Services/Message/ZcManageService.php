        ...
        if ($res) {
            foreach ($res as $key => $val) {
                ...
                if (isset($this->config['show_mobile']) && $this->config['show_mobile'] == 0) {
                    $res[$key]['user_name'] = $this->sanmuRepository->stringToStar($res[$key]['user_name']);
                    $res[$key]['nick_name'] = $this->sanmuRepository->stringToStar($res[$key]['nick_name']);
                }
    ...
    public function zcInitiatorList()
    {
        ...
        if ($res) {
            foreach ($res as $k => $v) {//处理等级标识
                $res[$k]['img'] = $this->sanmuRepository->getImagePath($v['img']);
                $logo = explode(',', $v['rank']);
                if ($logo) {
                    foreach ($logo as $val) {
                        $res[$k]['logo'][] = $this->getRankLogo($val);
                        foreach ($res[$k]['logo'] as $logo_key => $logo_val) {
                            $res[$k]['logo'][$logo_key]['img'] = $this->sanmuRepository->getImagePath($logo_val['img']);
                        }
                    }
                }
            }
    ...
    public function zcRankLogoList()
    {
        ...
        if ($row) {
            foreach ($row as $key => $val) {
                $row[$key]['img'] = $this->sanmuRepository->getImagePath($val['img']);
            }
        }
