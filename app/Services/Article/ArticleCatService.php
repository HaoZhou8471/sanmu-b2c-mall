    ...
    public function getArticleCategoriesTree($cat_id = 0, $cat_type = 1)
    {
        ...
        if ($res) {
            foreach ($res as $row) {
                ...
                $cat_arr[$row['cat_id']]['url'] = $this->sanmuRepository->buildUri('article_cat', array('acid' => $row['cat_id']), $row['cat_name']);
    ...
    public function articleCategoryAll($cat_id = 0, $columns = ['*'], $size = 20)
    {
        ...
        if (!empty($category['data'])) {
            foreach ($category['data'] as $k => $value) {
                $category['data'][$k]['url'] = sanmu_url('/#/article') . '?cat_id=' . $value['cat_id'];
            }
        }
