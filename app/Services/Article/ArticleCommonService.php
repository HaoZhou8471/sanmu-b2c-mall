        ...
        if ($arr === false) {
            ...
            if ($res) {
                foreach ($res as $key => $row) {
                    $arr[$row['cat_id']]['cat_id'] = $this->sanmuRepository->buildUri('article_cat', ['acid' => $row['cat_id']], $row['cat_name']);
                    ...
                    if ($row['get_article_list']) {
                        foreach ($row['get_article_list'] as $k => $article) {
                            ...
                            $arr[$row['cat_id']]['article'][$k]['short_title'] = $GLOBALS['_CFG']['article_title_length'] > 0 ?
                                $this->sanmuRepository->subStr($article['title'], $GLOBALS['_CFG']['article_title_length']) : $article['title'];
                            $arr[$row['cat_id']]['article'][$k]['url'] = $article['open_type'] != 1 ?
                                $this->sanmuRepository->buildUri('article', ['aid' => $article['article_id']], $article['title']) : trim($article['file_url']);
                        }
                    }
                }
