    ...
    public function getNewsCatArticles($where = [])
    {
        ...
        if ($res) {
            foreach ($res as $row) {
                ...
                $arr[$article_id]['short_title'] = $this->config['article_title_length'] > 0 ? $this->sanmuRepository->subStr($row['title'], $this->config['article_title_length']) : $row['title'];
                $arr[$article_id]['author'] = empty($row['author']) || $row['author'] == '_SHOPHELP' ? $this->config['shop_name'] : $row['author'];
                $arr[$article_id]['url'] = $row['open_type'] != 1 ? $this->sanmuRepository->buildUri('article', ['aid' => $article_id], $row['title']) : trim($row['file_url']);
    ...
    public function getNewArticles($cat_id = 0, $limit = 12)
    {
        ...
        if ($res) {
            foreach ($res as $idx => $row) {
                ...
                $row['cat_url'] = $this->sanmuRepository->buildUri('article_cat', ['acid' => $row['cat_id']], $row['cat_name']);
