...
class OrderManageService
{
    ...
    public function wholesaleOrderGoodsToInfo($order_id = 0)
    {
        ...
        if ($res) {
            foreach ($res as $key => $row) {
                ...
                $res[$key]['url'] = $this->sanmuRepository->buildUri('wholesale_goods', ['aid' => $row['goods_id']], $row['goods_name']);
                $res[$key]['goods_thumb'] = isset($goods['goods_thumb']) && $goods['goods_thumb'] ? $this->sanmuRepository->getImagePath($goods['goods_thumb']) : '';
            }
        }
    ...
    public function wholesaleOrderInfo($order_id = 0, $order_sn = '')
    {
        ...
        if ($order) {
            ...
            $order['formated_adjust_fee'] = $this->sanmuRepository->getPriceFormat($conv_adjust_fee, false);
            $order['formated_goods_amount'] = $this->sanmuRepository->getPriceFormat($order['goods_amount'], false);
            $order['formated_order_amount'] = $this->sanmuRepository->getPriceFormat(abs($order['order_amount']), false);
            ...
            if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                $order['mobile'] = $this->sanmuRepository->stringToStar($order['mobile']);
                $order['tel'] = $this->sanmuRepository->stringToStar($order['tel']);
                $order['email'] = $this->sanmuRepository->stringToStar($order['email']);
            }
        }
