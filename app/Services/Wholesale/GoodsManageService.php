...
class GoodsManageService
{
    ...
    public function getWholesaleList($is_delete = 0, $seller = [])
    {
        ...
        if ($list) {
            foreach ($list as $key => $row) {
                ...
                //处理商品链接
                $list[$key]['url'] = $this->sanmuRepository->buildUri('wholesale_goods', ['gid' => $row['goods_id']], $row['goods_name']);

                //补充处理 by wu
                $list[$key]['formated_shop_price'] = $this->sanmuRepository->getPriceFormat($row['shop_price']);
    ...
    public function getWholesaleVolumePriceList($goods_id, $price_type = '1')
    {
        ...
        if ($res) {
            foreach ($res as $k => $v) {
                ...
                $res[$k]['format_price'] = $this->sanmuRepository->getPriceFormat($v['volume_price']);
            }
        }
    ...
    public function deleteGoods($goods_id = 0)
    {
        ...
        if ($wholesale) {
            ...
            sanmu_unlink($img, storage_public());
        }
