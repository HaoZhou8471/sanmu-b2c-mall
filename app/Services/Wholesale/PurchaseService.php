        ...
        if ($res) {
            foreach ($res as $key => $row) {
                ...
                if ($goods_img) {
                    ...
                    $row['img'] = $this->sanmuRepository->getImagePath(reset($goods_img));
                }
                ...
                $row['url'] = $this->sanmuRepository->buildUri('wholesale_purchase', array('gid' => $row['purchase_id'], 'act' => 'info'), $row['subject']);
                $arr[] = $row;
            }
        }
    ...
    public function getPurchaseInfo($purchase_id = 0)
    {
        ...
        if ($purchase_info) {
                ...
                foreach ($goods_list as $key => $val) {
                    if ($val['goods_img']) {
                        ...
                        foreach ($goods_img as $k => $img) {
                            $goods_img[$k] = $this->sanmuRepository->getImagePath($img);
                        }
            ...
            if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                $purchase_info['mobile'] = $this->sanmuRepository->stringToStar($purchase_info['user_name']);
                $purchase_info['contact_phone'] = $this->sanmuRepository->stringToStar($purchase_info['contact_phone']);
                $purchase_info['contact_email'] = $this->sanmuRepository->stringToStar($purchase_info['contact_email']);
                $purchase_info['supplier_contact_phone'] = $this->sanmuRepository->stringToStar($purchase_info['supplier_contact_phone']);
            }
