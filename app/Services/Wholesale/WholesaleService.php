...
class WholesaleService
{
    ...
    public function getWholesaleLimit()
    {
        ...
        if ($res) {
            foreach ($res as $key => $row) {
                ...
                $res[$key]['goods_thumb'] = $this->sanmuRepository->getImagePath($row['goods_thumb']);
    ...
    public function getWholsaleNavigator()
    {
        ...
        if ($res) {
            foreach ($res as $k => $row) {
                $res[$k]['url'] = $this->sanmuRepository->buildUri('wholesale_cat', array('act' => 'list', 'cid' => $row['cat_id']), $row['cat_name']);
    ...
    public function assignCatPager($app, $cat, $record_count, $keywords, $size, $sort, $order, $page = 1)
    {
        ...
        if (!empty($url_format)) {
            ...
            $pager['page_first'] = ($page - $_offset > 1 && $_pagenum < $page_count) ? $this->sanmuRepository->buildUri($app, $uri_args, '', 1, $keywords) : '';
            $pager['page_prev'] = ($page > 1) ? $this->sanmuRepository->buildUri($app, $uri_args, '', $page_prev, $keywords) : '';
            $pager['page_next'] = ($page < $page_count) ? $this->sanmuRepository->buildUri($app, $uri_args, '', $page_next, $keywords) : '';
            $pager['page_last'] = ($_to < $page_count) ? $this->sanmuRepository->buildUri($app, $uri_args, '', $page_count, $keywords) : '';
            $pager['page_kbd'] = ($_pagenum < $page_count) ? true : false;
            $pager['page_number'] = array();
            for ($i = $_from; $i <= $_to; ++$i) {
                $pager['page_number'][$i] = $this->sanmuRepository->buildUri($app, $uri_args, '', $i, $keywords);
            }
        }
 ...
 */
    ...
    public function getSuppliersOrderValidRefund($suppliers_id = 0)
    {
        ...
        $res['brokerage_amount'] = $this->sanmuRepository->getPriceFormat($res['brokerage_amount']);
        $res['formated_is_settlement_amout'] = $this->sanmuRepository->getPriceFormat($res['is_settlement_amout']);
        $res['formated_no_settlement_amout'] = $this->sanmuRepository->getPriceFormat($res['no_settlement_amout']);
        $res['formated_return_amount'] = $this->sanmuRepository->getPriceFormat($res['return_amount']);
        ...
        if ($res) {
            foreach ($res as $row) {
                if (!empty($row['position_id'])) {
                    ...
                    $ads[] = [
                        'pic' => $this->sanmuRepository->getImagePath($src),
