    ...
    public function getwholesalegoodsorder($order_id = 0)
    {
        ...
        foreach ($goods_list as $key => $value) {
            if ($value['extension_code'] != 'package_buy') {
                ...
                $goods_list[$key]['market_price'] = $this->sanmuRepository->getPriceFormat($value['market_price'], false);
                $goods_list[$key]['goods_price'] = $this->sanmuRepository->getPriceFormat($value['goods_price'], false);
                $goods_list[$key]['subtotal'] = $this->sanmuRepository->getPriceFormat($value['subtotal'], false);
        ...
        $data['formated_goods_amount'] = $this->sanmuRepository->getPriceFormat(array_sum($price), false);
    ...
    public function getReturnOrdersList($data = [])
    {
        ...
        if ($res) {
            foreach ($res as $key => $row) {
                ...
                $row['refound_money'] = $this->sanmuRepository->getPriceFormat($wholesale_goods['refound'], false);
