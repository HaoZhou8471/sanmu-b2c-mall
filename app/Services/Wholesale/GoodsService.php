...
class GoodsService
{
    ...
    public function getWholesaleCatGoods($parent_id = 0)
    {
        ...
        if ($res) {
            foreach ($res as $key => $row) {
                ...
                $res[$key]['cat_url'] = $this->sanmuRepository->buildUri('wholesale_cat', array('act' => 'list', 'cid' => $row['cat_id']), $row['cat_name']);
            }
        }
    ...
    public function getBusinessGoods($cat_id = 0)
    {
        ...
        if ($res) {
            foreach ($res as $key => $row) {
                ...
                $res[$key]['thumb'] = $this->sanmuRepository->getImagePath($row['goods_thumb']);
                $res[$key]['goods_img'] = $this->sanmuRepository->getImagePath($row['goods_img']);
                $res[$key]['url'] = $this->sanmuRepository->buildUri('wholesale_goods', array('aid' => $row['goods_id']), $row['goods_name']);
            }
        }
    ...
    public function getWholesaleList($cat_id = 0, $size = 10, $page = 1, $sort, $order)
    {
        ...
        if ($res) {
            foreach ($res as $key => $row) {
                ...
                $row['goods_thumb'] = $this->sanmuRepository->getImagePath($row['goods_thumb']); //处理图片地址
                ...
                $row['goods_url'] = $this->sanmuRepository->buildUri('wholesale_goods', array('aid' => $row['goods_id']), $row['goods_name']);
                ...
                $row['suppliers_url'] = $this->sanmuRepository->buildUri('wholesale_suppliers', array('sid' => $suppliers['suppliers_id']));
                ...
                $row['shop_price'] = $this->sanmuRepository->getPriceFormat($row['goods_price']);
                $list[] = $row;
            }
        }
    ...
    public function getSupplierHome($suppliers_id = 0)
    {
        ...
        if (!empty($row)) {
            ...
            $row['suppliers_logo'] = $this->sanmuRepository->getImagePath($row['suppliers_logo']); //处理图片地址
        ...
        if (!empty($goods_attr)) {
            ...
            foreach ($goods_attr as $k => $value) {
                ...
                if ($GLOBALS['_CFG']['goods_attr_price'] == 1) {
                    ...
                    $attr_price = $this->sanmuRepository->getPriceFormat($attr_price, false);
                }
    ...
    public function calculateGoodsPrice($goods_id = 0, $goods_number = 0)
    {
        ...
        $data['unit_price_formatted'] = $this->sanmuRepository->getPriceFormat($data['unit_price']);
    ...
    public function getWholesaleGoodsInfo($goods_id = 0)
    {
        ...
        if ($row) {
            ...
            $row['suppliers_logo'] = isset($row['suppliers_logo']) ? $this->sanmuRepository->getImagePath($row['suppliers_logo']) : '';
            ...
            $row['store_url'] = $domain_url['domain_name'];
            $row['suppliers_url'] = $this->sanmuRepository->buildUri('wholesale_suppliers', array('sid' => $row['suppliers_id']));
        }
    ...
    public function getGalleryList($goods_id = 0)
    {
        ...
        if ($res) {
            foreach ($res as $k => $v) {
                $res[$k] = $this->sanmuRepository->getImagePath(($v['img_url']));
            }
        }
        return $res;
    }
        ...
        if ($row) {
            foreach ($row as $key => $gallery_img) {
                if (!empty($gallery_img['external_url'])) {
                    ...
                    $row[$key]['img_url'] = $this->sanmuRepository->getImagePath($gallery_img['img_url']);
                    $row[$key]['thumb_url'] = $this->sanmuRepository->getImagePath($gallery_img['thumb_url']);
                }
            }
            ...
            if (!$row) {
                ...
                $row = [
                    [
                        'img_url' => $this->sanmuRepository->getImagePath($goods_thumb),
                        'thumb_url' => $this->sanmuRepository->getImagePath($goods_thumb)
                    ]
                ];
            }
        }
