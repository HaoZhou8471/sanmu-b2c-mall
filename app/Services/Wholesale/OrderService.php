...
class OrderService
{
    ...
    public function wholesaleRecGoods($rec_id)
    {
        ...
        if ($res) {
            ...
            $res['subtotal'] = $this->sanmuRepository->getPriceFormat($subtotal, false);
            ...
            if ($goods) {
                ...
                $chat = $this->sanmuRepository->chatQq($shopInfo);
            ...
            $res['url'] = $this->sanmuRepository->buildUri('wholesale_goods', ['aid' => $res['goods_id']], $res['goods_name']);
        }
    ...
    public function wholesaleReturnOrder($size = 0, $start = 0)
    {
        ...
        if ($res) {
            foreach ($res as $key => $row) {
                ...
                $row['goods_thumb'] = $row['goods_thumb'] ? $this->sanmuRepository->getImagePath($row['goods_thumb']) : '';

                $row['apply_time'] = $this->timeRepository->getLocalDate($this->config['time_format'], $row['apply_time']);
                $row['should_return'] = $this->sanmuRepository->getPriceFormat($row['should_return'], false);
    ...
    public function wholesaleReturnOrderInfo($ret_id = 0, $order_sn = '', $order_id = 0)
    {
        ...
        if ($ret_id > 0) {
            ...
            if ($order) {
                ...
                $order['goods_thumb'] = isset($wholesale['goods_thumb']) ? $this->sanmuRepository->getImagePath($wholesale['goods_thumb']) : '';
                ...
                $order['formated_goods_amount'] = $this->sanmuRepository->getPriceFormat($order['should_return'], false);
                $order['formated_discount_amount'] = $this->sanmuRepository->getPriceFormat($order['discount_amount'], false);
                $order['formated_should_return'] = $this->sanmuRepository->getPriceFormat($order['should_return'] - $order['discount_amount'], false);
                $order['formated_return_shipping_fee'] = $this->sanmuRepository->getPriceFormat($order['return_shipping_fee'], false);
                $order['formated_return_amount'] = $this->sanmuRepository->getPriceFormat($order['should_return'] + $order['return_shipping_fee'] - $order['discount_amount'], false);
                $order['formated_actual_return'] = $this->sanmuRepository->getPriceFormat($order['actual_return'], false);
                ...
                $order['goods_price'] = $this->sanmuRepository->getPriceFormat($order['goods_price'], false);
                ...
                $order['goods_price'] = $this->sanmuRepository->getPriceFormat($goods_price, false);
                ...
                if ($img_list) {
                    foreach ($img_list as $key => $val) {
                        $img_list[$key]['img_file'] = $this->sanmuRepository->getImagePath($val['img_file']);
                    }
                }
                ...
                $order['url'] = $this->sanmuRepository->buildUri('wholesale_goods', ['aid' => $order['goods_id']], $order['goods_name']);
                ...
                $order['shop_url'] = $this->sanmuRepository->buildUri('merchants_store', array('urid' => $ru_id), $order['shop_name']);
    ...
    public function wholesaleCancelReturn($ret_id = 0, $user_id = 0)
    {
        ...
        if ($delete) {
            ...
            if ($img_file) {
                sanmu_unlink($img_file, storage_public());

                $this->sanmuRepository->getOssDelFile($img_file);
    ...
    public function getWholesaleReturnOrderGoods($rec_id = 0)
    {
        ...
        if ($goods_list) {
            foreach ($goods_list as $key => $row) {
                ...
                //图片显示
                $goods_list[$key]['goods_thumb'] = $wholesale && $wholesale['goods_thumb'] ? $this->sanmuRepository->getImagePath($wholesale['goods_thumb']) : '';
                $goods_list[$key]['url'] = $this->sanmuRepository->buildUri('wholesale_goods', ['aid' => $row['goods_id']], $row['goods_name']);
        ...
        if ($res) {
            foreach ($res as $key => $row) {
                ...
                $arr[] = [
                    ...
                    'kf_ww' => $chat['kf_ww'],
                    'kf_qq' => $chat['kf_qq'],
                    'total_fee' => $this->sanmuRepository->getPriceFormat($row['total_fee'], false),
