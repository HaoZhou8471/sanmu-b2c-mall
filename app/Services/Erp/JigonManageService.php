...
class JigonManageService
{
    ...
    public function pushJigonOrderGoods($cart_goods = [], $order = [], $type = 'pc') //0、pc返回  1、 api返回
    {
        ...
        if (!$this->config['cloud_sanmu_appkey']) {
            return $request;
        }
        ...
        if (!empty($order_detaillist)) {
            ...
            $request = sanmu_decode($request, true);
    ...
    public function jigonConfirmOrder($order_id)
    {
        ...
        if ($order_id > 0) {
            ...
            if ($cloud_order) {
                ...
                $cloud_order['notifyUrl'] = $this->sanmuRepository->sanmuUrl("api.php?app_key=" . $this->config['cloud_sanmu_appkey'] . "&method=sanmu.order.confirmorder.post&format=json&interface_type=1");
        ...
        if (!empty($order_cloud)) {
            ...
            if (!empty($images_list)) {
                foreach ($images_list as $k => $v) {
                    if ($v) {
                        $img = $this->sanmuRepository->getImagePath($v);
