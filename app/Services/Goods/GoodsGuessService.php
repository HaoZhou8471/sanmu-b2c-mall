    ...
    public function getGuessGoods($where = [])
    {
        ...
        if ($query) {
            foreach ($query as $row) {
                ...
                $guess_goods[$row['goods_id']]['short_name'] = $this->config['goods_name_length'] > 0 ? $this->sanmuRepository->subStr($row['goods_name'], $this->config['goods_name_length']) : $row['goods_name'];
                $guess_goods[$row['goods_id']]['goods_thumb'] = $this->sanmuRepository->getImagePath($row['goods_thumb']);
                $guess_goods[$row['goods_id']]['shop_price'] = $this->sanmuRepository->getPriceFormat($row['shop_price']);
                $guess_goods[$row['goods_id']]['promote_price'] = ($promote_price > 0) ? $this->sanmuRepository->getPriceFormat($promote_price) : '';
                $guess_goods[$row['goods_id']]['url'] = $this->sanmuRepository->buildUri('goods', ['gid' => $row['goods_id']], $row['goods_name']);

                $guess_goods[$row['goods_id']]['shop_name'] = $this->merchantCommonService->getShopName($row['user_id'], 1);
                $guess_goods[$row['goods_id']]['shopUrl'] = $this->sanmuRepository->buildUri('merchants_store', ['urid' => $row['user_id']]);
