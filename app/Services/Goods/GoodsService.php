...
class GoodsService
{
        ...
        $content = cache()->rememberForever('get_recommend_goods.' . $cache_id, function () use ($where) {
            ...
            if (isset($where['type']) && empty($type_goods[$where['type']])) {
                ...
                if (isset($where['seller_id'])) {
                    $goods_res = Goods::selectRaw('goods_id, brand_id, is_new, is_hot, is_best');
                } else {
                ...
                $goods_res = $this->sanmuRepository->getAreaLinkGoods($goods_res, $where['area_id'], $where['area_city']);
                ...
                if (isset($where['seller_id'])) {
                    ...
                    $goods_res = $this->sanmuRepository->getWhereRsid($goods_res, 'user_id', 0, $this->city);
                }
                ...
                $result = $this->sanmuRepository->getAreaLinkGoods($result, $where['area_id'], $where['area_city']);
                ...
                if (isset($where['seller_id'])) {
                    ...
                    $result = $this->sanmuRepository->getWhereRsid($result, 'user_id', 0, $this->city);
                }
                ...
                if ($result) {
                    foreach ($result as $idx => $row) {
                        ...
                        $goods[$idx]['short_name'] = $this->config['goods_name_length'] > 0 ? $this->sanmuRepository->subStr($row['goods_name'], $this->config['goods_name_length']) : $row['goods_name'];
                        ...
                        $goods[$idx]['promote_price'] = $promote_price > 0 ? $this->sanmuRepository->getPriceFormat($promote_price) : '';
                        $goods[$idx]['thumb'] = $this->sanmuRepository->getImagePath($row['goods_thumb']);
                        $goods[$idx]['goods_img'] = $this->sanmuRepository->getImagePath($row['goods_img']);
                        $goods[$idx]['shop_name'] = $this->merchantCommonService->getShopName($row['user_id'], 1);
                        $goods[$idx]['url'] = $this->sanmuRepository->buildUri('goods', ['gid' => $row['goods_id']], $row['goods_name']);
                        $goods[$idx]['shopUrl'] = $this->sanmuRepository->buildUri('merchants_store', ['urid' => $row['user_id']]);
    ...
    public function getPromoteGoods($where = [], $num = 3)
    {
        ...
        $result = $result->with([
            ...
            'getWarehouseGoods' => function ($query) use ($where) {
                $query->where('amount', $where['warehouse_id']);
            },
            'getWarehouseAreaGoods' => function ($query) use ($where) {
                $query = $query->where('amount', $where['area_id']);
    ...
    public function getGoodsInfo($where = [])
    {
        ...
        if ($row) {
            ...
            if ($brand) {
                $brand['url'] = $this->sanmuRepository->buildUri('brand', ['bid' => $brand['brand_id']], $brand['brand_name']);
                $brand['brand_logo'] = $this->sanmuRepository->getImagePath($this->sanmuRepository->dataDir() . '/brandlogo/' . $brand['brand_logo']);
            ...
            if ($row['brand_id']) {
                ...
                $row['brand_url'] = $this->sanmuRepository->buildUri('brand', ['bid' => $row['brand_id']], $row['brand_name']);
            }
            ...
            $row['promote_price'] = ($promote_price > 0) ? $this->sanmuRepository->getPriceFormat($promote_price) : '';
            ...
            if ($row['user_id'] > 0 && $grade_rank) {
                ...
                $row['grade_img'] = empty($grade_rank['grade_img']) ? '' : $this->sanmuRepository->getImagePath($grade_rank['grade_img']);
            ...
            $desc_preg = $this->sanmuRepository->descImagesPreg($row['goods_desc']);
            $row['goods_desc'] = $desc_preg['goods_desc'];

            $row['goods_img'] = $this->sanmuRepository->getImagePath($row['goods_img']);
            $row['goods_thumb'] = $this->sanmuRepository->getImagePath($row['goods_thumb']);
            $row['original_img'] = $this->sanmuRepository->getImagePath($row['original_img']);
            $row['goods_video_path'] = !empty($row['goods_video']) ? $this->sanmuRepository->getImagePath($row['goods_video']) : '';

            /* 获得商品的销售价格 */
            $row['marketPrice'] = $row['market_price'];
            $row['market_price'] = $this->sanmuRepository->getPriceFormat($row['market_price']);
            if ($promote_price > 0) {
                $row['shop_price_formated'] = $row['promote_price'];
                $row['goods_price'] = $promote_price;
            } else {
                $row['shop_price_formated'] = $this->sanmuRepository->getPriceFormat($row['shop_price']);
            ...
            if (isset($seller_info['shopinfo'])) {
                ...
                $row['shopinfo']['brand_thumb'] = $this->sanmuRepository->getImagePath($row['shopinfo']['brand_thumb']);
            }

            $row['goods_url'] = $this->sanmuRepository->buildUri('goods', ['gid' => $row['goods_id']], $row['goods_name']);
    ...
    public function getLinkedGoods($goods_id = 0, $warehouse_id = 0, $area_id = 0, $area_city = 0)
    {
        ...
        $res = $res->whereHas('getGoods', function ($query) use ($where) {
            ...
            $this->sanmuRepository->getAreaLinkGoods($query, $where['area_id'], $where['area_city']);
        });
        ...
        $res = $res->with([
            ...
            'getWarehouseGoods' => function ($query) use ($warehouse_id) {
                $query->where('amount', $warehouse_id);
            },
            'getWarehouseAreaGoods' => function ($query) use ($where) {
                $query = $query->where('amount', $where['area_id']);
        ...
        if ($res) {
            foreach ($res as $key => $row) {
                ...
                $arr[$goods_id]['short_name'] = $this->config['goods_name_length'] > 0 ?
                    $this->sanmuRepository->subStr($row['goods_name'], $this->config['goods_name_length']) : $row['goods_name'];
                $arr[$goods_id]['goods_thumb'] = $this->sanmuRepository->getImagePath($row['goods_thumb']);
                $arr[$goods_id]['goods_img'] = $this->sanmuRepository->getImagePath($row['goods_img']);
                $arr[$goods_id]['market_price'] = $this->sanmuRepository->getPriceFormat($row['market_price']);
                $arr[$goods_id]['shop_price'] = $this->sanmuRepository->getPriceFormat($row['shop_price']);
                $arr[$goods_id]['promote_price'] = ($promote_price > 0) ? $this->sanmuRepository->getPriceFormat($promote_price) : '';
                $arr[$goods_id]['url'] = $this->sanmuRepository->buildUri('goods', ['gid' => $row['goods_id']], $row['goods_name']);
        ...
        if ($res) {
            foreach ($res as $key => $row) {
                ...
                $row['url'] = $row['open_type'] != 1 ?
                    $this->sanmuRepository->buildUri('article', ['aid' => $row['article_id']], $row['title']) : trim($row['file_url']);
                ...
                $row['short_title'] = $this->config['article_title_length'] > 0 ?
                    $this->sanmuRepository->subStr($row['title'], $this->config['article_title_length']) : $row['title'];
        ...
        if ($res) {
            foreach ($res as $row) {
                ...
                $arr[$row['rank_id']] = [
                    ...
                    'price' => $this->sanmuRepository->getPriceFormat($row['price'])];
            }
        }
        ...
        if ($res) {
            foreach ($res as $key => $row) {
                ...
                if ($row['promote_price'] > 0) {
                    ...
                    $row['formated_promote_price'] = $this->sanmuRepository->getPriceFormat($row['promote_price']);
                } else {
                    $row['promote_price'] = 0;
                }
                    ...
                    'getGoods' => function ($query) use ($whereGoods) {
                        $query->with([
                            ...
                            'getWarehouseGoods' => function ($query) use ($whereGoods) {
                                $query->where('amount', $whereGoods['warehouse_id']);
                            },
                            'getWarehouseAreaGoods' => function ($query) use ($whereGoods) {
                                $query = $query->where('amount', $whereGoods['area_id']);
            ...
            foreach ($res as $tempkey => $value) {
                ...
                if ($goods_res) {
                    foreach ($goods_res as $key => $val) {
                        ...
                        $goods_res[$key]['goods_thumb'] = $this->sanmuRepository->getImagePath($val['goods_thumb']);
                        $goods_res[$key]['market_price'] = $this->sanmuRepository->getPriceFormat($val['market_price']);
                        $goods_res[$key]['rank_price'] = $this->sanmuRepository->getPriceFormat($val['shop_price']);
                        $goods_res[$key]['promote_price'] = $this->sanmuRepository->getPriceFormat($val['promote_price']);
                ...
                $res[$tempkey]['subtotal'] = $this->sanmuRepository->getPriceFormat($subtotal);
                $res[$tempkey]['saving'] = $this->sanmuRepository->getPriceFormat(($subtotal - $res[$tempkey]['package_price']));
                $res[$tempkey]['package_price'] = $this->sanmuRepository->getPriceFormat($res[$tempkey]['package_price']);
            }
        }
    ...
    public function getGoodsRelatedCat($cat_id)
    {
        ...
        if ($res) {
            foreach ($res as $key => $row) {
                ...
                $res[$key]['url'] = $this->sanmuRepository->buildUri('category', ['cid' => $row['cat_id']], $row['cat_name']);
            }
        }
    ...
    public function getGoodsSimilarBrand($cat_id = 0)
    {
        ...
        if ($brand) {
            ...
            if ($res) {
                foreach ($res as $key => $row) {
                    $res[$key]['url'] = $this->sanmuRepository->buildUri('brand', ['bid' => $row['brand_id']], $row['brand_name']);
                }
            }
        }
    ...
    public function getFilterGoodsList($filter = ['goods_ids' => '', 'cat_ids' => '', 'brand_ids' => '', 'user_id' => 0, 'mer_ids' => ''], $type = '', $warehouse_id = 0, $area_id = 0, $area_city = 0, $size = 10, $page = 1, $sort = "sort_order", $order = "ASC")
    {
        ...
        $res = $this->sanmuRepository->getAreaLinkGoods($res, $area_id, $area_city);
        ...
        if ($res) {
            foreach ($res as $row) {
                ...
                $arr[$row['goods_id']]['goods_thumb'] = $this->sanmuRepository->getImagePath($row['goods_thumb']);
                $arr[$row['goods_id']]['goods_video'] = $this->sanmuRepository->getImagePath($row['goods_video']);
                $arr[$row['goods_id']]['url'] = $this->sanmuRepository->buildUri('goods', ['gid' => $row['goods_id']], $row['goods_name']);
                $arr[$row['goods_id']]['shop_price'] = $this->sanmuRepository->getPriceFormat($row['shop_price']);
                $arr[$row['goods_id']]['promote_price'] = ($promote_price) > 0 ? $this->sanmuRepository->getPriceFormat($promote_price) : '';
            }
        }
    ...
    public function getShopQrcode($ru_id = 0)
    {
        ...
        $this->sanmuRepository->getOssAddFile([$image_name]);

        return $this->sanmuRepository->getImagePath($image_name);
    ...
    public function getGoodsQrcode($goods = [])
    {
        ...
        if (!file_exists($out_img) || $generate == true) {
            ...
            $logo_picture = empty($two_code_logo) ? $this->sanmuRepository->getImagePath($goods_img) : str_replace('../', '', $two_code_logo);
            ...
            $linkExists = $this->sanmuRepository->remoteLinkExists($logo);
            if ($linkExists) {
        ...
        return [
            'url' => $this->sanmuRepository->getImagePath($image_name) . '?v=' . Str::random(32)
        ];
    }
