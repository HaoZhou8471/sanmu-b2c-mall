...
class GoodsBookingManageService
{
    ...
    public function getBooKingList()
    {
        ...
        //卖场
        $res = $this->sanmuRepository->getWhereRsid($res, 'user_id', $filter['rs_id']);
        $ru_id = $adminru['ru_id'];
    ...
    public function getBooKingInfo($id)
    {
        ...
        if (isset($res['get_users']) && !empty($res['get_users'])) {
            ...
            if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                $res['user_name'] = $this->sanmuRepository->stringToStar($res['user_name']);
            }
        }

        if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
            $res['email'] = $this->sanmuRepository->stringToStar($res['email']);
            $res['tel'] = $this->sanmuRepository->stringToStar($res['tel']);
        }
