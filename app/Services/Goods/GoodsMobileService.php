...
class GoodsMobileService
{
    ...
    public function getPromoteGoods($where = [])
    {
        ...
        $res = $this->sanmuRepository->getAreaLinkGoods($res, $where['area_id'], $where['area_city']);
        ...
        if ($row) {
            ...
            if (CROSS_BORDER === true) {
                ...
                if (!empty($cbec)) {
                    ...
                    $row['formated_goods_rate'] = $this->sanmuRepository->getPriceFormat($row['goods_rate']);
                    if ($row['user_id'] > 0) {
            ...
            if ($row['gallery_list']) {
                foreach ($row['gallery_list'] as $k => $v) {
                    $row['gallery_list'][$k]['img_original'] = $this->sanmuRepository->getImagePath($v['img_original']);
                    $row['gallery_list'][$k]['img_url'] = $this->sanmuRepository->getImagePath($v['img_url']);
                    $row['gallery_list'][$k]['thumb_url'] = $this->sanmuRepository->getImagePath($v['thumb_url']);
                }
            }
            ...
            if (!empty($row['desc_mobile'])) {
                ...
                $desc_preg = $this->sanmuRepository->descImagesPreg($row['desc_mobile'], 'desc_mobile', 1);
            ...
            $row['goods_video'] = empty($row['goods_video']) ? '' : $this->sanmuRepository->getImagePath($row['goods_video']);
            ...
            $row['market_price_formated'] = $this->sanmuRepository->getPriceFormat($row['marketPrice']);
            $row['shop_price_formated'] = $this->sanmuRepository->getPriceFormat($row['shop_price']);
            $row['promote_price_formated'] = $is_promote == 1 ? $this->sanmuRepository->getPriceFormat($promote_price) : '';
            ...
            if (file_exists(MOBILE_DRP)) {
                ...
                if (empty($row['drp_shop'])) {
                    ...
                    $row['membership_card_discount_price_formated'] = $this->sanmuRepository->getPriceFormat($row['membership_card_discount_price']);
                }
            } else {
                $row['drp_shop'] = 0;
            }
        ...
        if (is_null($res)) {
            ...
            if ($res) {
                foreach ($res as $k => $v) {
                    ...
                    $res[$k]['promote_price_formated'] = ($promote_price > 0) ? $this->sanmuRepository->getPriceFormat($promote_price) : '';

                    $res[$k]['goods_thumb'] = $this->sanmuRepository->getImagePath($v['goods_thumb']);
                    $res[$k]['goods_img'] = $this->sanmuRepository->getImagePath($v['goods_img']);
                    $res[$k]['original_img'] = $this->sanmuRepository->getImagePath($v['original_img']);
                    $res[$k]['shop_price_formated'] = $this->sanmuRepository->getPriceFormat($v['shop_price']);
                    $res[$k]['market_price_formated'] = $this->sanmuRepository->getPriceFormat($v['market_price']);
                }
    ...
    public function goodsPropertiesPrice($uid, $goods_id, $attr_id, $num = 1, $warehouse_id = 0, $area_id = 0, $area_city = 0, $store_id = 0)
    {
        ...
        $result['market_price_formated'] = $this->sanmuRepository->getPriceFormat($result['market_price'], true);
        $result['qty'] = $num;

        $result['spec_price'] = app(GoodsProdutsService::class)->goodsPropertyPrice($goods_id, $attr_id, $warehouse_id, $area_id, $area_city);
        $result['spec_price_formated'] = $this->sanmuRepository->getPriceFormat($result['spec_price'], true);
        ...
        $result['spec_promote_price_formated'] = $this->sanmuRepository->getPriceFormat($result['spec_promote_price'], true);

        $result['goods_price'] = $this->getFinalPrice($uid, $goods_id, $num, true, $attr_id, $warehouse_id, $area_id, $area_city);
        $result['goods_price_formated'] = $this->sanmuRepository->getPriceFormat($result['goods_price'], true);
        ...
        if ($attr_id) {
            ...
            if (!empty($attr_img['attr_img_flie'])) {
                $result['attr_img'] = $this->sanmuRepository->getImagePath($attr_img['attr_img_flie']);
            }
            $name = [];
    ...
    public function createsharePoster($user_id, $goods_id = 0, $warehouse_id = 0, $area_id = 0, $area_city = 0, $share_type = 0)
    {
        ...
        $goods_url = sanmu_url('/#/goods/' . $goods_id) . '?' . http_build_query($param, '', '&');
        ...
        if (!file_exists($out_img) || $generate == true) {
            ...
            $goods_price = $this->sanmuRepository->getPriceFormat($goods['shop_price']);
            ...
            // 生成二维码+微信头像
            $user_picture = empty($users['user_picture']) ? public_path('img/user_default.png') : $this->sanmuRepository->getImagePath($users['user_picture']);
            // 生成微信头像缩略图
            ...
            $nickname = empty($users['nick_name']) ? $users['user_name'] : $this->sanmuRepository->subStr($users['nick_name'], 10, false); //用户昵称
            if ($share_type == 1) {
                // 分销文案
    ...
    public function getVideoList($size = 10, $page = 1, $user_id = 0, $where)
    {
        ...
        foreach ($res as $key => $val) {
            ...
            $res[$key]['shop_price_formated'] = $this->sanmuRepository->getPriceFormat($price['shop_price']);
            if ($user_id == 0) {
                $res[$key]['is_collect'] = 0;
            }
            $res[$key]['goods_thumb'] = empty($val['goods_thumb']) ? '' : $this->sanmuRepository->getImagePath($val['goods_thumb']);
            $res[$key]['goods_video'] = empty($val['goods_video']) ? '' : $this->sanmuRepository->getImagePath($val['goods_video']);
            $res[$key]['shop_logo'] = empty($val['get_seller_shop_info']['shop_logo']) ? '' : $this->sanmuRepository->getImagePath($val['get_seller_shop_info']['shop_logo']);
