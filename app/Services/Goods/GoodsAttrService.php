    ...
    public function goodsAttr($goods_id = 0)
    {
        ...
        if ($res) {
            foreach ($res as $key => $val) {
                ...
                $res[$key]['attr_img_flie'] = $val['attr_img_flie'] ? $this->sanmuRepository->getImagePath($val['attr_img_flie']) : '';
            }
        }
    ...
    public function getGoodsAttrId($where_select = [], $attr_type = 0, $retuen_db = 1)
    {
        ...
        if ($retuen_db == 2) {
            ...
            if ($res) {
                foreach ($res as $key => $row) {
                    foreach ($row['get_goods_attr_list'] as $idx => $val) {
                        ...
                        $list[$goods_attr_id]['attr_img_flie'] = $this->sanmuRepository->getImagePath($val['attr_img_flie']);
                        $list[$goods_attr_id]['attr_gallery_flie'] = $this->sanmuRepository->getImagePath($val['attr_gallery_flie']);
            ...
            if ($res) {
                ...
                $res['attr_img_flie'] = $this->sanmuRepository->getImagePath($res['attr_img_flie']);
                $res['attr_gallery_flie'] = $this->sanmuRepository->getImagePath($res['attr_gallery_flie']);
            }
                    ...
                    if ($goods_attr_list) {
                        foreach ($goods_attr_list as $idx => $val) {
                            ...
                            if ($val['attr_img_flie']) {
                                $goods_attr_list[$idx]['img_flie'] = $this->sanmuRepository->getImagePath($val['attr_img_flie']);
                            } else {
                                $goods_attr_list[$idx]['img_flie'] = $attributeImg ? $this->sanmuRepository->getImagePath($attributeImg['attr_img']) : '';
                            }
                            ...
                            $goods_attr_list[$idx]['format_price'] = $this->sanmuRepository->getPriceFormat($attr_price, false);
