...
class GoodsCommonService
{
    ...
    public function getVolumePriceList($goods_id, $price_type = 1, $is_pc = 0)
    {
        ...
        if ($res) {
            foreach ($res as $k => $v) {
                ...
                $volume_price[$k]['format_price'] = $this->sanmuRepository->getPriceFormat($v['volume_price']);
                //pc前台显示区分阶梯价格
                if ($is_pc > 0) {
                    if (($res_count - 1) > $k) {
    ...
    public function getFloorAjaxGoods($children = [], $num = 0, $warehouse_id = 0, $area_id = 0, $area_city = 0, $goods_ids = '', $ru_id = 0)
    {
        ...
        if (is_null($goods_res)) {
            ...
            $goods_res = $this->sanmuRepository->getAreaLinkGoods($goods_res, $area_id, $area_city);
            ...
            if ($goods_res) {
                foreach ($goods_res as $idx => $row) {
                    ...
                    $goods_res[$idx]['goods_img'] = $this->sanmuRepository->getImagePath($row['goods_img']);
                    $goods_res[$idx]['goods_thumb'] = $this->sanmuRepository->getImagePath($row['goods_thumb']);
                    $goods_res[$idx]['market_price'] = $this->sanmuRepository->getPriceFormat($row['market_price']);
                    $goods_res[$idx]['shop_price'] = $this->sanmuRepository->getPriceFormat($row['shop_price']);
                    $goods_res[$idx]['promote_price'] = ($promote_price > 0) ? $this->sanmuRepository->getPriceFormat($promote_price) : '';
                    $goods_res[$idx]['shop_price'] = $this->sanmuRepository->getPriceFormat($row['shop_price']);
                    $goods_res[$idx]['short_name'] = $this->config['goods_name_length'] > 0 ? $this->sanmuRepository->subStr($row['goods_name'], $this->config['goods_name_length']) : $row['goods_name'];
                    $goods_res[$idx]['url'] = $this->sanmuRepository->buildUri('goods', ['gid' => $row['goods_id']], $row['goods_name']);
                }
            }
