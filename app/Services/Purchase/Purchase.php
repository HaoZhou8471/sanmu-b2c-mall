...
class Purchase
{
                ...
                switch ($row['media_type']) {
                    case 0: // 图片广告
                        $src = (strpos($row['ad_code'], 'http://') === false && strpos($row['ad_code'], 'https://') === false) ?
                            $this->sanmuRepository->getImagePath($row['ad_code']) : $row['ad_code'];
    ...
    public function get_wholesale_limit()
    {
        ...
        if ($res) {
            foreach ($res as $key => $row) {
                ...
                $res[$key]['thumb'] = $this->sanmuRepository->getImagePath($row['get_goods']['goods_thumb']);
                $res[$key]['goods_thumb'] = $this->sanmuRepository->getImagePath($row['get_goods']['goods_thumb']);
                $res[$key]['goods_img'] = $this->sanmuRepository->getImagePath($row['get_goods']['goods_img']);
    ...
    public function get_wholesale_list($cat_id, $size, $page = 1)
    {
        ...
        if ($res) {
            foreach ($res as $row) {
                $row['goods_thumb'] = $this->sanmuRepository->getImagePath($row['get_goods']['goods_thumb']); //处理图片地址
                ...
                $row['shop_price'] = $this->sanmuRepository->getPriceFormat($row['get_goods']['shop_price']);
                $row['market_price'] = $this->sanmuRepository->getPriceFormat($row['get_goods']['market_price']);
    ...
    public function get_search_goods_list($keyword, $page = 1, $size = 10)
    {
        ...
        if (!empty($keyword)) {
            ...
            foreach ($arr as $key => $val) {
                ...
                $sql = 'SELECT DISTINCT goods_id FROM ' . $GLOBALS['sanmu']->table('tag') . " WHERE tag_words LIKE '%$val%' ";
        ...
        /* 获得符合条件的商品总数 */
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['sanmu']->table('wholesale') . " AS w " .
        ...
        $sql = "SELECT w.*, g.goods_thumb, g.goods_img, MIN(wvp.volume_number) AS volume_number, wvp.volume_price " .
            "FROM " . $GLOBALS['sanmu']->table('wholesale') . " AS w "
            . " LEFT JOIN " . $GLOBALS['sanmu']->table('goods') . " AS g ON w.goods_id = g.goods_id "
            . " LEFT JOIN " . $GLOBALS['sanmu']->table('wholesale_volume_price') . " AS wvp ON wvp.goods_id = g.goods_id "
            . "WHERE w.enabled = 1 AND w.review_status = 3 " .
            $keywords . $tag_where .
    ...
    public function get_wholesale_goods_info($act_id)
    {
        ...
        if ($row !== false) {
            ...
            $row['goods_price_formatted'] = $this->sanmuRepository->getPriceFormat($row['goods_price']);
    ...
    public function getWholesaleGoodsProperties($goods_id, $goods_attr_id = '', $attr_type = 0)
    {
        ...
        foreach ($res as $row) {
            ...
            if ($row['attr_type'] == 0) {
                ...
                if ($attr_info) {
                    $row['img_flie'] = !empty($attr_info['attr_img']) ? $this->sanmuRepository->getImagePath($attr_info['attr_img']) : '';
                ...
                $arr['spe'][$row['attr_id']]['values'][] = [
                    ...
                    'price' => $attr_price,
                    'format_price' => $this->sanmuRepository->getPriceFormat(abs($attr_price), false),
    ...
    public function get_brand_url($brand_id = 0)
    {
        ...
        $res = $res ? $res->toArray() : [];
        if ($res) {
            $res['url'] = $this->sanmuRepository->buildUri('brand', ['bid' => $res['brand_id']], $res['brand_name']);
    ...
    public function wholesale_cart_info($goods_id = 0, $rec_ids = '')
    {
        ...
        $sql = " SELECT goods_price, goods_number FROM " . $GLOBALS['sanmu']->table('wholesale_cart') . " AS c WHERE $sess_id ";
        ...
        $cart_info['total_price_formatted'] = $this->sanmuRepository->getPriceFormat($cart_info['total_price']);
        return $cart_info;
    }
    ...
    public function get_wholesale_cart_info($user_id = 0)
    {
        ...
        foreach ($row as $k => $v) {
            ...
            $arr[$k]['url'] = $this->sanmuRepository->buildUri('wholesale_goods', ['aid' => $v['get_wholesale']['act_id']], $v['goods_name']);
            $arr[$k]['goods_thumb'] = $this->sanmuRepository->getImagePath($v['get_goods']['goods_thumb']);
        ...
        if ($row) {
            ...
            $amount = $this->sanmuRepository->getPriceFormat(floatval($row['amount']));
        } else {
            $cart_number = 0;
            $number = 0;
            $amount = 0;
    ...
    public function wholesale_cart_goods($goods_id = 0, $rec_ids = '')
    {
        ...
        $cart_goods = [];
        //区分商家
        $sql = " SELECT DISTINCT ru_id FROM " . $GLOBALS['sanmu']->table('wholesale_cart') . " AS c WHERE $sess_id ";
        ...
        foreach ($ru_ids as $key => $val) {
            ...
            if ($val == 0) {
                //判断平台是否开启了IM在线客服
                if ($GLOBALS['db']->getOne("SELECT kf_im_switch FROM " . $GLOBALS['sanmu']->table('seller_shopinfo') . "WHERE ru_id = 0", true)) {
                    $data['is_sanmu'] = true;
                } else {
                    $data['is_sanmu'] = false;
                }
            ...
            /*  @author-bylu  end */
            //自营有自提点--val=ru_id
            $sql = "select * from " . $GLOBALS['sanmu']->table('seller_shopinfo') . " where ru_id='" . $val . "'";
            $basic_info = $GLOBALS['db']->getRow($sql);

            $chat = $this->sanmuRepository->chatQq($basic_info);
            $data['kf_type'] = $chat['kf_type'];
            $data['kf_ww'] = $chat['kf_ww'];
            $data['kf_qq'] = $chat['kf_qq'];

            //区分商品
            $sql = " SELECT DISTINCT goods_id FROM " . $GLOBALS['sanmu']->table('wholesale_cart') . " AS c WHERE $sess_id AND c.ru_id = '$val' ";
            ...
            foreach ($goods_ids as $a => $g) {
                ...
                $sql = " SELECT c.rec_id, c.goods_price, c.goods_number, c.goods_attr_id " .
                    " FROM " . $GLOBALS['sanmu']->table('wholesale_cart') . " AS c " .
                ...
                $total_price = 0;
                foreach ($res as $k => $v) {
                    $res[$k]['goods_price_formatted'] = $this->sanmuRepository->getPriceFormat($v['goods_price']);
                    $res[$k]['total_price'] = $v['goods_price'] * $v['goods_number'];
                    $res[$k]['total_price_formatted'] = $this->sanmuRepository->getPriceFormat($res[$k]['total_price']);
                ...
                $sql = " select goods_thumb from " . $GLOBALS['sanmu']->table('goods') . "  where goods_id='$goods_data[goods_id]'";
                $goods_thumb = $GLOBALS['db']->getOne($sql);
                $goods_data['goods_thumb'] = $this->sanmuRepository->getImagePath($goods_thumb);
