...
class StoreStreetMobileService
{
    ...
    public function getCatStoreList($cat_id = 0, $warehouse_id = 0, $area_id = 0, $area_city = 0, $size = 10, $page = 1, $sort = 'goods_id', $order = 'DESC', $user_id = 0, $lat = 0, $lng = 0, $city_id = 0)
    {
        ...
        $store = $store->with([
            ...
            'getGoods' => function ($query) use ($where) {
                ...
                /* 关联地区 */
                $query = $this->sanmuRepository->getAreaLinkGoods($query, $where['area_id'], $where['area_city']);
        ...
        if ($store) {
            foreach ($store as $key => $val) {
                ...
                $store[$key]['logo_thumb'] = $this->sanmuRepository->getImagePath(str_replace('../', '', $val['logo_thumb']));
                ...
                if ($res) {
                    foreach ($res as $idx => $goods) {
                        ...
                        $res[$idx]['goods_thumb'] = $this->sanmuRepository->getImagePath($goods['goods_thumb']);
                    }
                }
    ...
    public function getStoreGoodsList($uid = 0, $ru_id = 0, $children = 0, $keywords = '', $brand_id = 0, $size = 10, $page = 1, $sort = 'goods_id', $order = 'DESC', $filter_attr = [], $where_ext = [], $type = '')
    {
        ...
        if ($res) {
            foreach ($res as $k => $row) {
                ...
                $arr[$k]['promote_price'] = ($promote_price > 0) ? $this->sanmuRepository->getPriceFormat($promote_price) : '';
    ...
    public function StoreDetail($ru_id, $user_id = 0)
    {
        ...
        if ($data) {
            ...
            $info['grade_img'] = $this->sanmuRepository->getImagePath($grade_info['grade_img']);
            $info['logo_thumb'] = $this->sanmuRepository->getImagePath(str_replace('../', '', $data['logo_thumb']));
            $info['shop_logo'] = $this->sanmuRepository->getImagePath(str_replace('../', '', $data['shop_logo']));
            ...
            $info['shop_qrcode'] = $this->sanmuRepository->getImagePath($image_name);
    ...
    protected function shopdata($data = [])
    {
        ...
        $info['shop_logo'] = $this->sanmuRepository->getImagePath(str_replace('../', '', $data['logo_thumb']));
        $info['street_thumb'] = $this->sanmuRepository->getImagePath(str_replace('../', '', $data['street_thumb']));
        ...
        $info['shop_flash'] = $this->sanmuRepository->getImagePath($data['street_thumb']);
