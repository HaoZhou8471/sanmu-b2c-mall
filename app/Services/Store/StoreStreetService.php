...
class StoreStreetService
{
    ...
    public function getStoreShopCount($keywords = '', $sort = 'shop_id', $store_province = 0, $store_city = 0, $store_district = 0, $store_user = '', $libType = 0)
    {
        ...
        if (!empty($keywords)) {
            ...
            if (!empty($user_list)) {
                $user_list = $this->sanmuRepository->delStrComma($user_list);
            ...
            if ($store_user) {
                $store_user = $this->sanmuRepository->delStrComma($store_user);
    ...
    public function getStoreShopList($libType = 0, $keywords = '', $count = 0, $size = 16, $page = 1, $sort = 'shop_id', $order = 'DESC', $warehouse_id = 0, $area_id = 0, $area_city = 0, $store_province = 0, $store_city = 0, $store_district = 0, $store_user = '', $goods_type = 0)
    {
        ...
        if ($res) {
            foreach ($res as $row) {
                ...
                $arr[$row['shop_id']]['grade_img'] = $this->sanmuRepository->getImagePath($grade_info['grade_img']);
                $arr[$row['shop_id']]['grade_name'] = $this->sanmuRepository->getImagePath($grade_info['grade_name']);
                ...
                $arr[$row['shop_id']]['shop_logo'] = $this->sanmuRepository->getImagePath($arr[$row['shop_id']]['shop_logo']);
                $arr[$row['shop_id']]['logo_thumb'] = $this->sanmuRepository->getImagePath($arr[$row['shop_id']]['logo_thumb']);
                $arr[$row['shop_id']]['street_thumb'] = $this->sanmuRepository->getImagePath($shop_info['street_thumb']);
                $arr[$row['shop_id']]['brand_thumb'] = $this->sanmuRepository->getImagePath($shop_info['brand_thumb']);
                ...
                $arr[$row['shop_id']]['store_shop_url'] = $this->sanmuRepository->buildUri('merchants_store_shop', ['urid' => $row['user_id']], $arr[$row['shop_id']]['shop_name']);
                ...
                if ($row['user_id'] == 0) {
                    ...
                    if ($kf_im_switch) {
                        $arr[$row['shop_id']]['is_sanmu'] = true;
                    } else {
                        $arr[$row['shop_id']]['is_sanmu'] = false;
                    }
                } else {
                    $arr[$row['shop_id']]['is_sanmu'] = false;
                }
        ...
        if ($res) {
            foreach ($res as $row) {
                if ($row) {
                    ...
                    $arr[$row['goods_id']]['goods_url'] = $this->sanmuRepository->buildUri('goods', ['gid' => $row['goods_id']], $row['goods_name']);
        ...
        if ($type == 1) {
            ...
            if ($res) {
                foreach ($res as $key => $row) {
                    ...
                    $arr[$key]['market_price'] = $this->sanmuRepository->getPriceFormat($row['market_price']);
                    $arr[$key]['shop_price'] = $this->sanmuRepository->getPriceFormat($row['shop_price']);
                    $arr[$key]['promote_price'] = ($promote_price > 0) ? $this->sanmuRepository->getPriceFormat($promote_price) : '';

                    $arr[$key]['goods_id'] = $row['goods_id'];
                    $arr[$key]['goods_name'] = $row['goods_name'];
                    $arr[$key]['goods_thumb'] = $this->sanmuRepository->getImagePath($row['goods_thumb']);
                    $arr[$key]['goods_url'] = $this->sanmuRepository->buildUri('goods', ['gid' => $row['goods_id']], $row['goods_name']);
                    ...
                    $arr[$key]['shop_url'] = $this->sanmuRepository->buildUri('merchants_store', ['cid' => 0, 'urid' => $row['user_id']], $arr[$key]['shop_name']);
