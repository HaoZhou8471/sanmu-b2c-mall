    ...
    public function GoodsList($page = 1, $size = 10, $type = '')
    {
        ...
        if ($list) {
            foreach ($list as $key => $val) {
                ...
                $list[$key]['shop_price'] = $this->sanmuRepository->getPriceFormat($val['shop_price'], false);
                $list[$key]['target_price'] = $this->sanmuRepository->getPriceFormat($val['target_price'], false);
                if ($val['get_bargain_target_price']) {//获取砍价商品属性最低价格
                    ...
                    if ($target_price) {
                        $list[$key]['target_price'] = $this->sanmuRepository->getPriceFormat($target_price, false);
                    }
                }
    ...
    public function goodsInfo($bargain_id = 0)
    {
        ...
        if ($info) {
            ...
            if (empty($info['desc_mobile']) && !empty($info['goods_desc'])) {
                $desc_preg = $this->sanmuRepository->descImagesPreg($info['goods_desc']);
                $info['goods_desc'] = $desc_preg['goods_desc'];
            }

            if (!empty($info['desc_mobile'])) {
                // 处理手机端商品详情 图片（手机相册图） data/gallery_album/
                $desc_preg = $this->sanmuRepository->descImagesPreg($info['desc_mobile'], 'desc_mobile', 1);
    ...
    public function getBargainRanking($bargain_id = 0)
    {
        ...
        if ($list) {
            foreach ($list as $key => $val) {
                ...
                $info[$key]['rank'] = $key + 1;
                $info[$key]['money'] = $this->sanmuRepository->getPriceFormat($val['money']);
                $user_name = $val['nick_name'] ? $val['nick_name'] : $val['user_name'];
                $info[$key]['user_name'] = setAnonymous($user_name);
                $info[$key]['user_picture'] = $this->sanmuRepository->getImagePath($val['user_picture']);
    ...
    public function getBargainStatistics($bs_id = 0)
    {
        ...
        if ($res) {
            foreach ($res as $key => $val) {
                $val = array_merge($val, $val['get_users']);
                $list[$key]['subtract_price'] = $this->sanmuRepository->getPriceFormat($val['subtract_price'], false);
    ...
    public function myBargain($user_id = 0, $page = 1, $size = 10)
    {
        ...
        if ($res) {
            foreach ($res as $key => $v) {
                ...
                $list[$key]['goods_thumb'] = $this->sanmuRepository->getImagePath($v['goods_thumb']);
                $list[$key]['goods_img'] = $this->sanmuRepository->getImagePath($v['goods_img']);
                $list[$key]['shop_price'] = $this->sanmuRepository->getPriceFormat($v['shop_price'], false);
                $list[$key]['target_price'] = $this->sanmuRepository->getPriceFormat($v['target_price'], false);
