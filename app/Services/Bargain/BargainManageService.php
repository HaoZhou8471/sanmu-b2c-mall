...
class BargainManageService
{
    ...
    public function getBargainGoodsList($filter = [], $offset = [])
    {
        ...
        foreach ($list as $key => $val) {
            ...
            $list[$key]['shop_price'] = $this->sanmuRepository->getPriceFormat($list[$key]['shop_price']);
            $list[$key]['target_price'] = $this->sanmuRepository->getPriceFormat($val['target_price']);
            ...
            $list[$key]['goods_img'] = $this->sanmuRepository->getImagePath($list[$key]['goods_img']);
            $list[$key]['goods_thumb'] = $this->sanmuRepository->getImagePath($list[$key]['goods_thumb']);
        ...
        if ($list) {
            foreach ($list as $key => $val) {
                ...
                if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                    $list[$key]['user_name'] = $this->sanmuRepository->stringToStar($val['user_name']);
                }
