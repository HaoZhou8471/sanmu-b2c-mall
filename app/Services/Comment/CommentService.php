...
class CommentService
{
    ...
    public function getCommentImgList($where = [], $order = ['sort' => 'id', 'order' => 'desc'])
    {
        ...
        if ($res) {
            foreach ($res as $key => $row) {
                $row['comment_img'] = $this->sanmuRepository->getImagePath($row['comment_img']);
                $row['img_thumb'] = $this->sanmuRepository->getImagePath($row['img_thumb']);

                $row['path_comment_img'] = $this->sanmuRepository->getImagePath($row['comment_img']);
                $row['path_img_thumb'] = $this->sanmuRepository->getImagePath($row['img_thumb']);
    ...
    public function getOrderCommentList($user_id, $sign = 0, $rec_id = 0)
    {
        ...
        if ($row) {
            ...
            $row['goods_price'] = $this->sanmuRepository->getPriceFormat($row['goods_price']);
        ...
        if ($res) {
            foreach ($res as $k => $v) {
                ...
                if (isset($v['user']['user_picture']) && $v['user']['user_picture']) {
                    $commentlist[$k]['user_picture'] = $this->sanmuRepository->getImagePath($v['user']['user_picture']);
                } else {
                    $user_default = $this->sanmuRepository->sanmuUrl('img/user_default.png');
                    $commentlist[$k]['user_picture'] = $this->sanmuRepository->getImagePath($user_default);
                }
                ...
                if (isset($v['get_comment_img']) && $v['get_comment_img']) {
                    ...
                    foreach ($v['get_comment_img'] as $key => $val) {
                        $img[$key] = $this->sanmuRepository->getImagePath($val['comment_img']);
                    }
    ...
    public function getOrderGoods($rec_id)
    {
        ...
        if ($ordergoods) {
            ...
            if ($goods) {
                $ordergoods['shop_price'] = $this->sanmuRepository->getPriceFormat($goods['shop_price']);
                $ordergoods['goods_thumb'] = $this->sanmuRepository->getImagePath($goods['goods_thumb']);
            }
        }
    ...
    public function getArticleCommentList($article_id, $page = 1, $size = 10)
    {
        ...
        if ($comment_list) {
            foreach ($comment_list as $key => $val) {
                ...
                $comment_list[$key]['user_name'] = $this->sanmuRepository->stringToStar($user_name, $name_len, 6);
                ...
                $comment_list[$key]['user_picture'] = $this->sanmuRepository->getImagePath($user_picture);
            }
        }
    ...
    public function getAssignArticleComment($id, $type, $page = 1)
    {
        ...
        if ($res) {
            foreach ($res as $row) {
                ...
                $row['user_name'] = $this->sanmuRepository->stringToStar($row['user_name'], $name_len, 3);
                ...
                $arr[$row['comment_id']]['user_picture'] = $this->sanmuRepository->getImagePath($user_picture);
                ...
                $arr[$row['comment_id']]['user_picture'] = $this->sanmuRepository->getImagePath($arr[$row['comment_id']]['user_picture']);
            }
        }
