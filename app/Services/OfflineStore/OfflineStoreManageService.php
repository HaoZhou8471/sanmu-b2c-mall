...
class OfflineStoreManageService
{
    ...
    public function getOfflineStoreList()
    {
        ...
        foreach ($row as $k => $v) {
            ...
            $v['stores_img'] = $this->sanmuRepository->getImagePath($v['stores_img']);

            if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                $v['stores_tel'] = $this->sanmuRepository->stringToStar($v['stores_tel']);
            }
    ...
    public function getDataList($type = 0)
    {
        ...
        if ($type != 0) {
            ...
            $arr = [
                ...
                'store_total' => $this->sanmuRepository->getPriceFormat($store_total)
            ];
