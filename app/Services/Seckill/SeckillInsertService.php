...
class SeckillInsertService
{
    ...
    public function insertIndexSeckillGoods($seckillid = [], $temp = '')
    {
        ...
        if ($seckill_goods) {
            ...
            $GLOBALS['smarty']->assign('url_seckill', $this->sanmuRepository->setRewriteUrl('seckill.php'));
        }
    ...
    public function getIndexSeckillGoods($seckillid = [])
    {
        ...
        if ($res) {
            foreach ($res as $k => $v) {
                ...
                if ($i) {
                    $res[$k]['sec_price'] = $this->sanmuRepository->getPriceFormat($v['sec_price']);
                    $res[$k]['market_price'] = $this->sanmuRepository->getPriceFormat($v['market_price']);
                    $res[$k]['url'] = $this->sanmuRepository->buildUri('seckill', ['act' => "view", 'secid' => $v['id']], $v['goods_name']);
                    $res[$k]['list_url'] = $this->sanmuRepository->buildUri('seckill', ['act' => "list", 'secid' => $v['id']], $v['goods_name']);
            ...
            if ($soon) {
                foreach ($soon as $k => $v) {
                    ...
                    if ($i) {
                        $soon[$k]['sec_price'] = $this->sanmuRepository->getPriceFormat($v['sec_price']);
                        $soon[$k]['market_price'] = $this->sanmuRepository->getPriceFormat($v['market_price']);
                        $soon[$k]['url'] = $this->sanmuRepository->buildUri('seckill', ['act' => "view", 'secid' => $v['id']], $v['goods_name']);
                        $res[$k]['list_url'] = $this->sanmuRepository->buildUri('seckill', ['act' => "list", 'secid' => $v['id']], $v['goods_name']);
                        $res[$k]['goods_thumb'] = $this->sanmuRepository->getImagePath($v['goods_thumb']);
                    }
                }
