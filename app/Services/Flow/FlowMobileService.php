...
class FlowMobileService
{
    ...
    public function OrderInfo($uid, $rec_type = 0, $t_id = 0, $team_id = 0, $bs_id = 0, $store_id = 0, $type_id = 0)
    {
        ...
        if (CROSS_BORDER === true) // 跨境多商户
        {
            ...
            if (!empty($cbec)) {
                ...
                if (!empty($excess)) {
                    ...
                    $msg['error'] = 'excess';
                    $msg['msg'] = $store_name . lang('common.cross_order_excess') . $this->sanmuRepository->getPriceFormat($excess['limited_amount']);
                    return $msg;
                }
            }
        }
        ...
        foreach ($ret as $k => $v) {
            foreach ($v as $key => $val) {
                ...
                $goods_list[$k]['amount'] = $this->sanmuRepository->getPriceFormat($this->SumGoodsPrice($val['goods']));
        ...
        $list['total']['amount_formated'] = $this->sanmuRepository->getPriceFormat($list['total']['amount']);
        $list['total']['goods_price_formated'] = $this->sanmuRepository->getPriceFormat($list['total']['goods_price']);
        $list['total']['integral'] = 0;
        $list['total']['integral_money'] = 0;
        $list['total']['integral_money_formated'] = $this->sanmuRepository->getPriceFormat($list['total']['integral_money']);
        ...
        if ($this->config['use_integral'] == 1 && ($flow_type == CART_GENERAL_GOODS || $flow_type == CART_ONESTEP_GOODS)) {
            ...
            if ($order_integral > 0) {
                $order_point = $this->sanmuRepository->integralOfValue($order_integral);

                if ($user_info['pay_points'] >= $order_point) {
                    $list['integral'][0]['integral'] = $order_point;
                    $list['integral'][0]['integral_money'] = $this->sanmuRepository->valueOfIntegral($order_point);
                    $list['integral'][0]['integral_money_formated'] = $this->sanmuRepository->getPriceFormat($list['integral'][0]['integral_money']);
                }
        ...
        if ($this->config['use_surplus'] == 1) {
            ...
            if ($use_surplus > 0 && empty($list['is_kj'])) {
                ...
                $list['user_money_formated'] = $this->sanmuRepository->getPriceFormat($use_surplus);
            }
        }
    ...
    public function Done($uid = 0, $flow = [])
    {
        ...
        if ($this->config['use_paypwd'] == 1 && ($payment['pay_code'] == 'balance' || $order['vc_id'] > 0 || $order['is_surplus'] == 1)) {
            if (empty($users_paypwd)) {
                // 请启用支付密码
                return ['error' => 1, 'msg' => lang('flow.paypwd_must_open'), 'url' => sanmu_url('/#/user/accountsafe')];
            } else {
                if (empty($pay_pwd)) {
        ...
        if ($order['extension_code'] == 'exchange_goods') {
            $order['integral_money'] = $this->sanmuRepository->valueOfIntegral($total['exchange_integral']);
        ...
        if ($new_order_id > 0) {
            ...
            foreach ($cart_goods as $k => $v) {
                ...
                if ($is_general_goods === true && $order['bonus'] > 0 && $bonusSubtotal > 0) {
                    if ($order_goods['goods_price'] > 0 && $isShareAlike == 1) {
                        ...
                        $order_goods['goods_bonus'] = $this->sanmuRepository->changeFloat($order_goods['goods_bonus']);
                    } else {
                ...
                if ($is_general_goods === true && $order['coupons'] > 0 && $couponSubtotal > 0) {
                    ...
                    if ($couponsGoods && $order_goods['goods_price'] > 0 && $couponsGoods['ru_id'] == $v['ru_id']) {
                        ...
                        if ($couponsGoods['is_coupons'] == 1) {
                            ...
                            $order_goods['goods_coupons'] = $this->sanmuRepository->changeFloat($order_goods['goods_coupons']);
            ...
            /* 核对均摊优惠券商品金额 */
            $this->sanmuRepository->collateOrderGoodsBonus($coupons_list, $order['coupons'] ?? 0, $goods_coupons);

            /* 核对均摊红包商品金额 */
            $this->sanmuRepository->collateOrderGoodsBonus($bonus_list, $order['bonus'], $goods_bonus);
        }
        ...
        if (is_wechat_browser() && file_exists(MOBILE_WECHAT)) {
            ...
            $url = sanmu_url('/#/user/orderDetail/' . $order_id);
        ...
        if (($payment['pay_code'] == 'balance' || $order['is_surplus'] == 1) && $order['order_amount'] == 0 && $order['surplus'] > 0) {
            ...
            if (is_wechat_browser() && file_exists(MOBILE_WECHAT)) {
                ...
                $url = sanmu_url('/#/user/account'); // 进入用户余额页面
    ...
    public function PayCheck($uid, $order_sn = '', $store_id = 0)
    {
        ...
        if ($order_info) {
            ...
            $order['order_amount_format'] = $this->sanmuRepository->getPriceFormat($order_info['order_amount']);
            ...
            if ($order['extension_code'] == 'team_buy') {
                ...
                $order['url'] = sanmu_url('/#/team/wait') . '?' . http_build_query(['team_id' => $team_id, 'status' => 1], '', '&');
    ...
    public function ChangeCou($uc_id = 0, $uid = 0, $total = [])
    {
        ...
        if ($uc_id > 0 && $coupons) {
            ...
            $total['coupons_money_formated'] = $this->sanmuRepository->getPriceFormat($coupons['cou_money']);
            ...
            if ($coupons['cou_type'] == VOUCHER_SHIPPING) {
                ...
                $total['shopping_fee_formated'] = $this->sanmuRepository->getPriceFormat($shopping_fee);
            }
        } else {
        ...
        $total['amount_formated'] = $this->sanmuRepository->getPriceFormat($total['amount']);
        $total['card_money_formated'] = $this->sanmuRepository->getPriceFormat($total['card_money']);
        $total['coupons_money_formated'] = $this->sanmuRepository->getPriceFormat($total['coupons_money']);
    ...
    public function ChangeBon($bonus_id, $uid, $total = [])
    {
        ...
        $total['amount_formated'] = $this->sanmuRepository->getPriceFormat($total['amount']);
        $total['card_money_formated'] = $this->sanmuRepository->getPriceFormat($total['card_money']);
        $total['bonus_money_formated'] = $this->sanmuRepository->getPriceFormat($total['bonus_money']);
    ...
    public function ChangeInt($uid, $total = [], $integral_type = 0, $cart_value = [], $flow_type)
    {
        ...
        $integral_money = $this->sanmuRepository->getValueOfIntegral($integral);
        ...
        if ($integral_type == 1) {
            if ($pay_points > $integral && $integral > 0) {
                ...
                if ($amount >= $integral_money) {
                    ...
                    $total['integral'] = $this->sanmuRepository->getIntegralOfValue($amount);
        ...
        $total['vc_dis'] = $total['vc_dis'] * 10;
        $total['amount_formated'] = $this->sanmuRepository->getPriceFormat($total['amount']);
        $total['integral_money_formated'] = $this->sanmuRepository->getPriceFormat($total['integral_money']);
        $total['card_money_formated'] = $this->sanmuRepository->getPriceFormat($total['card_money']);
    ...
    public function ChangeSurplus($uid, $total = [], $surplus = 0, $shopping_fee = 0)
    {
        ...
        $total['amount_formated'] = $this->sanmuRepository->getPriceFormat($total['amount']);
        $total['card_money_formated'] = $this->sanmuRepository->getPriceFormat($total['card_money']);
        $total['surplus_formated'] = $this->sanmuRepository->getPriceFormat($total['surplus']);
    ...
    public function ChangeCard($vid, $uid, $total = [])
    {
        ...
        $total['card_formated'] = $this->sanmuRepository->getPriceFormat($total['card_money']);
        $total['amount_formated'] = $this->sanmuRepository->getPriceFormat($total['amount']);
        $total['card_money_formated'] = $this->sanmuRepository->getPriceFormat($total['card_money']);
    ...
    public function GoodsInCartByUser($uid = 0, $cart_goods = [])
    {
        ...
        foreach ($goods_list as $key => $row) {
            ...
            $row['goods']['market_price_format'] = $this->sanmuRepository->getPriceFormat($row['market_price']);
            $row['goods']['goods_price'] = $row['goods_price'];
            $row['goods']['goods_price_format'] = $this->sanmuRepository->getPriceFormat($row['goods_price']);
            ...
            if ($row['extension_code'] == 'package_buy') {
                ...
                if (!empty($package_goods)) {
                    foreach ($package_goods as $k => $val) {
                        ...
                        $package_goods[$k]['goods_thumb'] = $this->sanmuRepository->getImagePath($val['goods_thumb']);
                        $package_goods[$k]['market_price_format'] = $this->sanmuRepository->getPriceFormat($val['market_price']);
                        $package_goods[$k]['rank_price_format'] = $this->sanmuRepository->getPriceFormat($val['shop_price']);
                    }
                }
                ...
                if (!empty($package_goods)) {
                    ...
                    $row['goods']['format_package_list_total'] = $this->sanmuRepository->getPriceFormat($row['goods']['package_list_total']);
                    $row['goods']['format_package_list_saving'] = $this->sanmuRepository->getPriceFormat($row['goods']['package_list_saving']);
                }
            }

            $row['goods']['goods_thumb'] = $this->sanmuRepository->getImagePath($row['goods_thumb']);
        ...
        $total['saving_formated'] = $this->sanmuRepository->getPriceFormat($total['saving'], false);
        $total['discount_formated'] = $this->sanmuRepository->getPriceFormat($total['discount'], false);
        $total['dis_amount_formated'] = $this->sanmuRepository->getPriceFormat($total['dis_amount'], false);
        $total['goods_price_formated'] = $this->sanmuRepository->getPriceFormat($total['goods_price'], false);
        $total['market_price_formated'] = $this->sanmuRepository->getPriceFormat($total['market_price'], false);
    ...
    public function bonus_info($bonus_id, $bonus_psd = '', $cart_value = 0)
    {
        ...
        if ($cart_value != 0 || !empty($cart_value)) {
            ...
            if (!empty($goods_user)) {
                ...
                $goods_user = $this->sanmuRepository->delStrComma($goods_user);
            }
        }
            ...
            if ($res) {
                ...
                if ($cant_buy_goods) {
                    foreach ($cant_buy_goods as $key => $v) {
                        ...
                        $cant_buy_goods[$key]['goods_thumb'] = $this->sanmuRepository->getImagePath($goods_thumb);
                    }
                }
