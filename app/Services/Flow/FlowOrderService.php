    ...
    public function OrderSeparateBill($order_id = 0)
    {
        ...
        foreach ($newInfo as $key => $info) {
            ...
            if ($row['order_amount'] > 0 && $integral_money > 0) {
                ...
                if ($integral_ratio > 0) {
                    if ($integral_money >= $integral_ratio && $row['order_amount'] > $integral_ratio) {
                        ...
                        $row['integral'] = $this->sanmuRepository->integralOfValue($integral_ratio);
                    } else {
                        if ($integral_money > $row['order_amount']) {
                            $integral_money -= $row['order_amount'];
                            $row['integral_money'] = $row['order_amount'];
                            $row['integral'] = $this->sanmuRepository->integralOfValue($row['order_amount']);

                            $row['order_amount'] = 0;
                        } else {
                            $row['order_amount'] -= $integral_money;
                            $row['integral_money'] = $integral_money;
                            $row['integral'] = $this->sanmuRepository->integralOfValue($integral_money);
            ...
            if ($new_orderId > 0) {
                ...
                $this->sanmuRepository->collateOrderGoodsBonus($bonus_list, $row['bonus'], $goods_bonus);
        ...
        if ($res) {
            foreach ($res as $key => $v) {
                ...
                $integral_list = [
                    ...
                    $this->sanmuRepository->integralOfValue($integral * $v['goods_number'])
                ];

                $integral = $this->baseRepository->getArrayMin($integral_list);
                $integral = $this->sanmuRepository->valueOfIntegral($integral);

                $v['integral'] = $this->sanmuRepository->integralOfValue($integral);
    ...
    public function AddToOrderGoods($where = [], $order = [], $all_ru_id = [])
    {
        ...
        if ($where['order_id'] > 0) {
            ...
            if ($res) {
                ...
                foreach ($res as $key => $val) {
                    ...
                    if ($is_general_goods === true && $order['bonus'] > 0 && $bonusSubtotal > 0) {
                        if ($val['goods_price'] > 0 && $isShareAlike == 1) {
                            ...
                            $cartOther[$key]['goods_bonus'] = $this->sanmuRepository->changeFloat($cartOther[$key]['goods_bonus']);
                        } else {
                    ...
                    if ($is_general_goods === true && $order['coupons'] > 0 && $couponSubtotal > 0) {
                        ...
                        if ($couponsGoods && $val['goods_price'] > 0 && $couponsGoods['ru_id'] == $val['ru_id']) {
                            ...
                            if ($couponsGoods['is_coupons'] == 1) {
                                ...
                                $cartOther[$key]['goods_coupons'] = $this->sanmuRepository->changeFloat($cartOther[$key]['goods_coupons']);
                ...
                /* 核对均摊红包商品金额 */
                $this->sanmuRepository->collateOrderGoodsCoupons($bonus_list, $order['bonus'] ?? 0, $goods_bonus);
