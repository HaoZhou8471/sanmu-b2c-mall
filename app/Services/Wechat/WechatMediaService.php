...
class WechatMediaService
{
    ...
    public function WechatMediaDetail($id = 0)
    {
        ...
        if (isset($res['content']) && $res['content']) {
            // 过滤样式 手机自适应
            $res['content'] = $this->sanmuRepository->contentStyleReplace($res['content']);
            // 显示文章详情图片 （本地或OSS）
            $res['content'] = $this->sanmuRepository->getContentImgReplace($res['content']);

            $res['content'] = html_out($res['content']);
        }
        // 封面图
        $res['file'] = $this->sanmuRepository->getImagePath($res['file']);
