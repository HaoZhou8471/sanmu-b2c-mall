...
class WechatPluginService
{
    ...
    public function recommendGoods($where = [], $limit = 5, $wechat_ru_id = 0)
    {
        ...
        if (!empty($goods)) {
            foreach ($goods as $key => $value) {
                $goods[$key]['goods_img'] = !empty($value['goods_img']) ? $this->sanmuRepository->getImagePath($value['goods_img']) : '';
            }
        }
    ...
    {
        if (empty($goods_img)) {
            $picUrl = empty(config('shop.wap_logo')) ? '' : $this->sanmuRepository->getImagePath(config('shop.wap_logo'));
        } else {
            $picUrl = $this->sanmuRepository->getImagePath($goods_img);
        }
    ...
    public function userInfo($user_id = 0)
    {
        ...
        if (!empty($user)) {
            $user['user_money_format'] = strip_tags($this->sanmuRepository->getPriceFormat($user['user_money'], false));
        }
