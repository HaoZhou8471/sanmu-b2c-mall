...
class WechatManageService
{
    ...
    public function getWechatMediaArticle($wechat_id = 0, $offset = [])
    {
        ...
        if (!empty($list)) {
            foreach ($list as $k => $v) {
                ...
                $list[$k]['content'] = empty($v['digest']) ? (empty($v['content']) ? '' : $this->sanmuRepository->subStr(strip_tags(html_out($v['content'])), 50)) : $v['digest'];
            }
        }
    ...
    public function transformWechatMediaData($id = 0, $data = [])
    {
        ...
        if (empty($data['digest']) && !empty($data['content'])) {
            $data['digest'] = $this->sanmuRepository->subStr(strip_tags(html_out($data['content'])), 50);
        }
    ...
    public function transformWechatMassHistoryList($list = [])
    {
        ...
        if ($list) {
            foreach ($list as $key => $val) {
                ...
                $artinfo['content'] = empty($artinfo['digest']) ? (empty($artinfo['content']) ? '' : $this->sanmuRepository->subStr(strip_tags(html_out($artinfo['content'])), 50)) : $artinfo['digest'];
        ...
        if (!empty($list)) {
            foreach ($list as $key => $val) {
                ...
                if (!empty($val['media_id'])) {
                    ...
                    $media['content'] = empty($media['digest']) ? (empty($media['content']) ? '' : $this->sanmuRepository->subStr(strip_tags(html_out($media['content'])), 50)) : $media['digest'];
                    if (!empty($media['article_id'])) {
                        $artids = explode(',', $media['article_id']);
