...
class CommonService
{
    ...
    public function actLogin()
    {
        ...
        $is_jsonp = $this->sanmuRepository->isJsonp($back_act, '&');

        if ($is_jsonp == 1 && strpos($back_act, 'merchants_store') !== false) {
            $seller_id = $this->sanmuRepository->isJsonp($back_act, '&', 'merchant_id');
            $back_act = $this->sanmuRepository->sellerUrl(intval($seller_id));
    ...
    public function getContent($user_id = 0, $warehouse_id = 0, $area_id = 0, $area_city = 0)
    {
        ...
        $urlHtml = $this->sanmuRepository->getUrlHtml($urlHtmlKey);
    ...
    public function ajaxUpdateCart($user_id = 0, $session_id = '', $warehouse_id = 0, $area_id = 0, $area_city = 0)
    {
        ...
        if (request()->exists('rec_id') && request()->exists('goods_number')) {
                ...
                if ($row['extension_code'] == 'package_buy') {
                    ...
                    if ($cartInfo['extension_code'] != 'package_buy' && $cartInfo['is_gift'] == 0 && $cartInfo['parent_id'] == 0) {
                        ...
                        if ($goods_price > 0) {
                            ...
                            $result['goods_price'] = $this->sanmuRepository->getPriceFormat($goods_price);
                        }
                    }
            ...
            foreach ($cart_goods['goods_list'] as $goods) {
                if ($goods['rec_id'] == $key) {
                    if ($goods['dis_amount'] > 0) {
                        $result['goods_subtotal'] = $this->sanmuRepository->getPriceFormat($goods['subtotal']);
                        $result['discount_amount'] = $goods['discount_amount'];
                    } else {
                        $result['goods_subtotal'] = $this->sanmuRepository->getPriceFormat($goods['subtotal']);
                        $result['discount_amount'] = $this->sanmuRepository->getPriceFormat(0);
                    }
                    ...
                    if (CROSS_BORDER === true) // 跨境多商户
                    {
                        ...
                        if ($goods['rate_price'] > 0) {
                            $rate_price = $this->sanmuRepository->getPriceFormat($goods['rate_price']);
            ...
            $result['save_total_amount'] = $this->sanmuRepository->getPriceFormat($fav_amount + $goods_discount_amount['save_amount']);
            ...
            foreach ($cart_goods['goods_list'] as $goods) {
                ...
                if (isset($result['rec_goods']) && $goods['parent_id'] > 0 && $result['rec_goods'] == $goods['parent_id']) {
                    if ($goods['rec_id'] != $key) {
                        ...
                        $result['group'][$goods['rec_id']]['rec_group_subtotal'] = $this->sanmuRepository->getPriceFormat($goods['goods_amount'], false);
            ...
            $total_goods_price = $this->sanmuRepository->getPriceFormat($goods_amount, false);
            ...
            $result['goods_amount_fromated'] = $this->sanmuRepository->getPriceFormat($goods_amount, false);
    ...
    public function ajaxDeleteCartGoods()
    {
        ...
        $GLOBALS['smarty']->assign('str', sprintf(lang('common.cart_info'), $number, $this->sanmuRepository->getPriceFormat($amount, false)));
        $GLOBALS['smarty']->assign('goods', $arr);

        $cart_info = ['goods_list' => $arr, 'number' => $number, 'amount' => $this->sanmuRepository->getPriceFormat($amount, false)];
        $GLOBALS['smarty']->assign('cart_info', $cart_info);
