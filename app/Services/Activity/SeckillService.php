...
class SeckillService
{
    ...
    public function seckill_detail($uid = 0, $seckill_id = 0, $tomorrow = 0)
    {
        ...
        if ($seckill) {
            $seckill['goods_thumb'] = $this->sanmuRepository->getImagePath($seckill['goods_thumb']);
            $seckill['goods_img'] = $this->sanmuRepository->getImagePath($seckill['goods_img']);
            ...
            if (empty($seckill['desc_mobile']) && !empty($seckill['goods_desc'])) {
                $desc_preg = $this->sanmuRepository->descImagesPreg($seckill['goods_desc']);
                $seckill['goods_desc'] = $desc_preg['goods_desc'];

            }
            if (!empty($seckill['desc_mobile'])) {
                // 处理手机端商品详情 图片（手机相册图） data/gallery_album/
                $desc_preg = $this->sanmuRepository->descImagesPreg($seckill['desc_mobile'], 'desc_mobile', 1);
    ...
    public function seckill_info($seckill_id = 0, $tomorrow = 0)
    {
        ...
        $seckill['sec_price_format'] = $this->sanmuRepository->getPriceFormat($seckill['sec_price']);
    ...
    public function seckill_goods_results($id, $page = 1, $size = 10, $tomorrow = 0)
    {
        ...
        if ($res) {
            foreach ($res as $k => $v) {
                ...
                $res[$k]['sec_price_formated'] = $this->sanmuRepository->getPriceFormat($v['sec_price']);
                ...
                $res[$k]['url'] = sanmu_url('/#/seckill/detail') . '?' . http_build_query(['seckill_id' => $v['id'], 'tomorrow' => $tomorrow], '', '&');
    ...
    public function getTopSeckillGoods()
    {
        ...
        if ($res) {
            foreach ($res as $key => $look_top) {
                ...
                $look_top['goods_thumb'] = $this->sanmuRepository->getImagePath($look_top['goods_thumb']);
                $look_top['url'] = $this->sanmuRepository->buildUri('seckill', ['act' => "view", 'secid' => $look_top['id']], $look_top['goods_name']);
    ...
    public function getMerchantSeckillGoods($sec_goods_id = 0, $ru_id = 0)
    {
        ...
        foreach ($res as $key => $row) {
            ...
            $row['shop_price'] = $this->sanmuRepository->getPriceFormat($row['sec_price'], false);
            $row['goods_thumb'] = $this->sanmuRepository->getImagePath($row['goods_thumb']);
            $row['url'] = $this->sanmuRepository->buildUri('seckill', ['act' => "view", 'secid' => $row['id']], $row['goods_name']);
