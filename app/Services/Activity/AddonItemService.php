...
class AddonItemService
{
    ...
    public function getFavourableGoodsList($user_rank, $favourable_id, $sort = '', $order = '', $size = 15, $page = 1, $warehouse_id = 0, $area_id = 0, $area_city = 0)
    {
        ...
        if ($res) {
            foreach ($res as $row) {
                ...
                $arr[$key]['format_shop_price'] = $this->sanmuRepository->getPriceFormat($row['shop_price']);
                $arr[$key]['format_promote_price'] = ($promote_price > 0) ? $this->sanmuRepository->getImagePath($promote_price) : '';
    ...
    public function getCartFavourableGoods($user_rank = 0, $favourable_id, $warehouse_id = 0, $area_id = 0, $area_city = 0)
    {
        ...
        $res = $res->whereHas('getGoods', function ($query) use ($goodsWhere) {
            ...
            $this->sanmuRepository->getAreaLinkGoods($query, $goodsWhere['area_id'], $goodsWhere['area_city']);
        });
        ...
        if ($res) {
            foreach ($res as $key => $row) {
                ...
                $cart_favourable_goods[$key]['goods_thumb'] = $this->sanmuRepository->getImagePath($row['goods_thumb']);
                ...
                $cart_favourable_goods[$key]['goods_url'] = $this->sanmuRepository->buildUri('goods', ['gid' => $row['goods_id']], $row['goods_name']);
            }
        }
