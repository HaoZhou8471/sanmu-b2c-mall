...
class ExchangeService
{
    ...
    public function getExchangeGetGoods($children = [], $min = 0, $max = 0, $size = 0, $page = 1, $sort = 'g.goods_id', $order = 'desc')
    {
        ...
        if ($res) {
            foreach ($res as $row) {
                ...
                $arr[$row['goods_id']]['url'] = $this->sanmuRepository->buildUri('exchange_goods', ['gid' => $row['goods_id']], $row['goods_name']);
            }
        }
    ...
    public function getExchangeGoodsInfo($goods_id = 0, $warehouse_id = 0, $area_id = 0, $area_city = 0)
    {
        ...
        if ($row) {
            ...
            $row['goods_price'] = $this->sanmuRepository->getPriceFormat($row['exchange_integral'] * $GLOBALS['_CFG']['integral_scale'] / 100);
