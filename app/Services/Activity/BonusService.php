...
class BonusService
{
    ...
    public function getUserBounsNewList($user_id = 0, $page = 1, $type = 0, $pageFunc = '', $amount = 0, $size = 10, $cart_ru_id = -1)
    {
        ...
        if ($res) {
            foreach ($res as $key => $row) {
                ...
                $arr[$key]['type_money'] = $this->sanmuRepository->getPriceFormat($row['type_money']);
                $arr[$key]['min_goods_amount'] = $this->sanmuRepository->getPriceFormat($row['min_goods_amount']);
    ...
    public function getBounsAmountList($bouns_list)
    {
        ...
        return $this->sanmuRepository->getPriceFormat($bouns_amount);
    }
