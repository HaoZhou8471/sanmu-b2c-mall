...
class DiscountService
{
    ...
    public function activityList()
    {
        ...
        if ($activity_list) {
            foreach ($activity_list as $row) {
                $row['activity_thumb'] = $this->sanmuRepository->getImagePath($row['activity_thumb']);
    ...
    public function activityDetail($act_id = 0)
    {
        ...
        $list['activity_thumb'] = $this->sanmuRepository->getImagePath($list['activity_thumb']);
        $list['min_amount'] = $this->sanmuRepository->getPriceFormat($list['min_amount'], false);
        $list['max_amount'] = $this->sanmuRepository->getPriceFormat($list['max_amount'], false);
            ...
            case 0:
                ...
                if (is_array($list['gift'])) {
                    foreach ($list['gift'] as $k => $v) {
                        ...
                        $list['gift'][$k]['thumb'] = $this->sanmuRepository->getImagePath($goods_thumb);
                        $list['gift'][$k]['price'] = $v['price'];
                        $list['gift'][$k]['price_formated'] = $this->sanmuRepository->getPriceFormat($v['price']);
                        $list['gift'][$k]['url'] = sanmu_url('/#/goods/' . $v['id']);
    ...
    public function activityGoods($filter = ['goods_ids' => '', 'cat_ids' => '', 'brand_ids' => '', 'user_id' => 0], $sort = 0, $order = 'desc', $page = 1, $size = 10, $uid)
    {
        ...
        foreach ($goods_list as $key => $row) {
            ...
            $list[$key]['goods_thumb'] = $this->sanmuRepository->getImagePath($row['goods_thumb']);
            $list[$key]['shop_price'] = $row['shop_price'] * ($discount / 100);//显示会员等级价格
            $list[$key]['market_price'] = $row['market_price'];
            $list[$key]['shop_price_formated'] = $this->sanmuRepository->getPriceFormat($row['shop_price'] * ($discount / 100));//显示会员等级价格
            $list[$key]['market_price_format'] = $this->sanmuRepository->getPriceFormat($row['market_price']);
            $list[$key]['goods_number'] = $row['goods_number'];
            $list[$key]['sales_volume'] = $row['sales_volume'];
            $list[$key]['url'] = sanmu_url('/#/goods/' . $row['goods_id']);
        ...
        if ($favourable) {
            ...
            if ($res) {
                foreach ($res as $key => $row) {
                    ...
                    $list[$key]['shop_price_format'] = $this->sanmuRepository->getPriceFormat($row['goods_price']);
                    $list[$key]['goods_number'] = $row['goods_number'];
                    $list[$key]['url'] = sanmu_url('/#/goods/' . $row['goods_id']);
                    $list[$key]['app_page'] = config('route.goods.detail') . $row['goods_id'];
                }
                    ...
                    case 0:
                        ...
                        if (is_array($row['gift'])) {
                            foreach ($row['gift'] as $k => $v) {
                                ...
                                $row['gift'][$k]['thumb'] = $this->sanmuRepository->getImagePath($goods_thumb);
                            }
                        }
                        break;
                    case 1:
    ...
    public function getActRangeExt($act_range_ext, $warehouse_id = 0, $area_id = 0, $area_city = 0)
    {
        ...
        if ($res) {
            foreach ($res as $key => $val) {
                ...
                $arr[$key]['price'] = $this->sanmuRepository->getPriceFormat($val['shop_price']);
