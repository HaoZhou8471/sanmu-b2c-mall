...
class GroupBuyService
{
    ...
    public function getGroupBuyList($children = [], $keywords = '', $type = '', $goods_num = 0, $size = 10, $page = 1, $sort = 'act_id', $order = 'DESC')
    {
        ...
        if ($res) {
            foreach ($res as $group_buy) {
                ...
                /* 格式化保证金 */
                $group_buy['formated_deposit'] = $this->sanmuRepository->getPriceFormat($group_buy['deposit'], false);
                ...
                if (!is_array($price_ladder) || empty($price_ladder)) {
                    $price_ladder = [
                        [
                            'amount' => 0,
                            'price' => 0,
                            'formated_amount' => $this->sanmuRepository->getPriceFormat(0),
                            'formated_price' => $this->sanmuRepository->getPriceFormat(0)
                        ]
                    ];
                } else {
                    foreach ($price_ladder as $key => $amount_price) {
                        $price_ladder[$key]['formated_price'] = $this->sanmuRepository->getPriceFormat($amount_price['price']);
                    }
                }
                ...
                $group_buy['goods_thumb'] = $this->sanmuRepository->getImagePath($goods['goods_thumb']);

                /* 处理链接 */
                $group_buy['url'] = $this->sanmuRepository->buildUri('group_buy', ['gbid' => $group_buy['group_buy_id']]);

                $group_buy['end_date_day'] = $this->timeRepository->getBuyDate($group_buy['end_date']);
                $group_buy['price'] = $this->sanmuRepository->getPriceFormat($nowprice);
    ...
    public function getGroupBuyInfo($where = [])
    {
        ...
        if ($goods) {
            ...
            $goods['short_name'] = $this->config['goods_name_length'] > 0 ?
                $this->sanmuRepository->subStr($goods['goods_name'], $this->config['goods_name_length']) : $goods['goods_name'];
            $goods['goods_thumb'] = $this->sanmuRepository->getImagePath($goods['goods_thumb']);
            $goods['goods_img'] = $this->sanmuRepository->getImagePath($goods['goods_img']);
            $goods['market_price'] = $this->sanmuRepository->getPriceFormat($goods['market_price']);
            $goods['shop_price'] = $this->sanmuRepository->getPriceFormat($goods['shop_price']);
            $goods['promote_price'] = ($promote_price > 0) ? $this->sanmuRepository->getPriceFormat($promote_price) : '';
            $goods['url'] = $this->sanmuRepository->buildUri('goods', ['gid' => $goods['goods_id']], $goods['goods_name']);
        ...
        if (!is_array($price_ladder) || empty($price_ladder)) {
            ...
            foreach ($price_ladder as $key => $amount_price) {
                $price_ladder[$key]['formated_price'] = $this->sanmuRepository->getPriceFormat($amount_price['price'], false);
            }
        }
        $group_buy['price_ladder'] = $price_ladder;
        ...
        $group_buy['formated_cur_price'] = $this->sanmuRepository->getPriceFormat($cur_price, false);
        ...
        /* 视频路径格式化 */
        $group_buy['goods_video_path'] = $this->sanmuRepository->getImagePath($goods['goods_video']);
        ...
        if ($group_buy['user_id']) {
            ...
            $group_buy['shopinfo']['brand_thumb'] = $this->sanmuRepository->getImagePath($group_buy['shopinfo']['brand_thumb']);
        }
            ...
            foreach ($res as $key => $look_top) {
                ...
                if (!is_array($price_ladder) || empty($price_ladder)) {
                    ...
                    foreach ($price_ladder as $k => $amount_price) {
                        $price_ladder[$k]['formated_price'] = $this->sanmuRepository->getPriceFormat($amount_price['price'], false);
                    }
                }
                ...
                $look_top['goods_thumb'] = $this->sanmuRepository->getImagePath($goods['goods_thumb']);

                $look_top['ext_info']['cur_price'] = $this->sanmuRepository->getPriceFormat($cur_price, false); //现价
    ...
    public function getMerchantGroupGoods($group_buy_id = 0, $seller_id = 0)
    {
        ...
        if ($res) {
            foreach ($res as $key => $row) {
                ...
                $res[$key]['goods_thumb'] = $this->sanmuRepository->getImagePath($goods['goods_thumb']);
            }
        }
    ...
    public function getMonthDayStartEndGoods($group_buy_id, $first_month_day = 0, $last_month_day = 0)
    {
        ...
        if ($res) {
            foreach ($res as $key => $row) {
                ...
                $arr[$key]['cur_price'] = $this->sanmuRepository->getPriceFormat($cur_price, false); //现价

                /* 团购节省和折扣计算 by ecmoban start */
                $arr[$key]['market_price'] = $this->sanmuRepository->getPriceFormat($goods['market_price'], false); //原价
                $price = $goods['market_price']; //原价
                $nowprice = $cur_price; //现价
                $arr[$key]['jiesheng'] = $this->sanmuRepository->getPriceFormat($price - $nowprice, false); //节省金额
                if ($nowprice > 0) {
                ...
                $arr[$key]['goods_thumb'] = $this->sanmuRepository->getImagePath($goods['goods_thumb']);
            }
    ...
    public function get_brand_url($brand_id = 0)
    {
        ...
        if ($res) {
            ...
            $res['brand_logo'] = !empty($res['brand_logo']) ? $this->sanmuRepository->getImagePath($this->shop->data_dir() . '/brandlogo/' . $res['brand_logo']) : '';
        }
