...
class SnatchService
{
    ...
    public function getExchangeRecommendGoods($where = [], $num = 11)
    {
        ...
        if ($res) {
            foreach ($res as $key => $row) {
                ...
                $row['thumb'] = $this->sanmuRepository->getImagePath($row['goods_thumb']);
                $row['goods_img'] = $this->sanmuRepository->getImagePath($row['goods_img']);
                $row['url'] = $this->sanmuRepository->buildUri('snatch', ['sid' => $row['act_id']]);

                $promote_price = $this->goodsCommonService->getBargainPrice($row['promote_price'], $row['promote_start_date'], $row['promote_end_date']);
                $row['formated_shop_price'] = $this->sanmuRepository->getPriceFormat($row['shop_price']);
                $row['formated_shop_price'] = ($promote_price > 0) ? $this->sanmuRepository->getPriceFormat($promote_price) : $row['formated_shop_price'];
                $row['formated_market_price'] = $this->sanmuRepository->getPriceFormat($row['market_price']);
    ...
    public function getMyPrice($id)
    {
        ...
        if (!empty(session('user_id'))) {
            ...
            for ($i = 0, $count = count($my_price_time); $i < $count; $i++) {
                $bid_price[] = [
                    'price' => $this->sanmuRepository->getPriceFormat($my_price_time[$i]['bid_price'], false),
                    'bid_price' => $this->sanmuRepository->getPriceFormat($my_price_time[$i]['bid_price'], false),
    ...
    public function getPriceList($id)
    {
        ...
        if ($res) {
            foreach ($res as $key => $row) {
                ...
                $row['bid_price'] = $this->sanmuRepository->getPriceFormat($row['bid_price'], false);
    ...
    public function getSnatchList($keywords = '', $size = 15, $page = 1, $sort = 'goods_id', $order = 'desc', $warehouse_id = 0, $area_id = 0, $area_city = 0)
    {
        ...
        if ($res) {
            foreach ($res as $key => $row) {
                ...
                $snatchInfo['max_price'] = $this->sanmuRepository->getPriceFormat($snatchInfo['max_price']);
                ...
                $snatch_list[] = [
                    ...
                    'overtime' => $overtime,
                    'formated_market_price' => $this->sanmuRepository->getPriceFormat($row['market_price']),
                    'formated_shop_price' => $this->sanmuRepository->getPriceFormat($shop_price),
                    'goods_thumb' => $this->sanmuRepository->getImagePath($row['goods_thumb']),
                    'price_list_count' => $row['price_list_count'], // 围观次数
                    'url' => $this->sanmuRepository->buildUri('snatch', ['sid' => $row['snatch_id']])
                ];
            }
        }
    ...
    public function getSnatch($where = [])
    {
        ...
        if ($goods) {
            ...
            $goods['formated_market_price'] = $this->sanmuRepository->getPriceFormat($goods['market_price']);
            $goods['formated_shop_price'] = $this->sanmuRepository->getPriceFormat($goods['shop_price']);
            $goods['formated_promote_price'] = ($promote_price > 0) ? $this->sanmuRepository->getPriceFormat($promote_price) : '';
            ...
            if ($info) {
                ...
                $goods['formated_start_price'] = $this->sanmuRepository->getPriceFormat($goods['start_price']);
                $goods['formated_end_price'] = $this->sanmuRepository->getPriceFormat($goods['end_price']);
                $goods['formated_max_price'] = $this->sanmuRepository->getPriceFormat($goods['max_price']);
            }
    ...
    public function getSnatchGoodsList($user_id, $record_count, $page, $list = '', $size = 10)
    {
        ...
        if ($res) {
            ...
            foreach ($res as $row) {
                ...
                $arr['bid_price'] = isset($row['bid_price']) ? $this->sanmuRepository->getPriceFormat($row['bid_price']) : $this->sanmuRepository->getPriceFormat(0);
