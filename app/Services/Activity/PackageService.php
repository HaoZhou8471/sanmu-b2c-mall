...
class PackageService
{
    ...
    public function getPackageList($where = [])
    {
        ...
        if ($res) {
            foreach ($res as $row) {
                $row['activity_thumb'] = !empty($row['activity_thumb']) ? $this->sanmuRepository->getImagePath($row['activity_thumb']) : $this->sanmuRepository->sanmuUrl('themes/ecmoban_sanmu2017/images/17184624079016pa.jpg');
                ...
                if ($goods_res) {
                    foreach ($goods_res as $key => $val) {
                        ...
                        $goods_res[$key]['shop_price'] = $this->sanmuRepository->getPriceFormat($val['shop_price']);
                        $goods_res[$key]['rank_price'] = $this->sanmuRepository->getPriceFormat($val['rank_price']);

                        $goods_res[$key]['package_amounte'] = $this->sanmuRepository->getPriceFormat($val['shop_price'] * $val['goods_number'] - $package_price);
                ...
                $row['subtotal'] = $this->sanmuRepository->getPriceFormat($subtotal);
                $row['saving'] = $this->sanmuRepository->getPriceFormat(($subtotal - $row['package_price']));
                $row['package_price'] = $this->sanmuRepository->getPriceFormat($package_price);
                $row['package_amounte'] = $this->sanmuRepository->getPriceFormat($subtotal - $package_price);
    ...
    public function getPackageInfo($package_id = 0)
    {
        ...
        if ($package) {
            ...
            if ($goods_res) {
                foreach ($goods_res as $key => $val) {
                    $goods_res[$key]['goods_thumb'] = $this->sanmuRepository->getImagePath($val['goods_thumb']);
                    $goods_res[$key]['market_price_format'] = $this->sanmuRepository->getPriceFormat($val['market_price']);
                    $goods_res[$key]['rank_price_format'] = $this->sanmuRepository->getPriceFormat($val['shop_price']);
            ...
            $package['market_package_format'] = $this->sanmuRepository->getPriceFormat($market_price);
            $package['package_price_format'] = $this->sanmuRepository->getPriceFormat($package['package_price']);
        } else {
            //移除购物车中的无效超值礼包
            if ($package_id) {
                Cart::where([
