...
class PresaleService
{
    ...
    public function getPreGoods($children = [], $min = 0, $max = 0, $start_time = 0, $end_time = 0, $sort, $status = 0, $order = 'DESC', $keywords = '')
    {
        ...
        if ($res) {
            foreach ($res as $key => $row) {
                ...
                $res[$key]['original_img'] = $this->sanmuRepository->getImagePath($row['original_img']);
                ...
                $res[$key]['format_shop_price'] = $this->sanmuRepository->getPriceFormat($row['shop_price']);
                $res[$key]['format_market_price'] = $this->sanmuRepository->getPriceFormat($row['market_price']);
    ...
    public function getCatGoods($cat_id = 0)
    {
        ...
        if ($res) {
            foreach ($res as $key => $row) {
                ...
                $res[$key]['goods_img'] = $this->sanmuRepository->getImagePath($row['goods_img']);
                $res[$key]['url'] = route('presale', ['act' => 'view', 'id' => $row['act_id']]);
                $res[$key]['goods_desc'] = $this->sanmuRepository->getContentImgReplace($row['goods_desc']);
    ...
    {
        return $this->sanmuRepository->buildUri('presale', ['act' => $act, 'cid' => $cat_id, 'status' => $status], $cat_name);
    }
    ...
    public function getPresaleCatListChildren($parent_id = 0)
    {
        ...
        if ($cat_list === false) {
            ...
            if ($cat_list) {
                $cat_list = $this->sanmuRepository->getCatVal($cat_list);
    ...
    public function presaleInfo($presale_id, $current_num = 0, $user_id = 0, $path = '')
    {
        ...
        /* 格式化保证金 */
        $presale['formated_deposit'] = $this->sanmuRepository->getPriceFormat($presale['deposit'], false);
        /* 尾款 */
        $presale['final_payment'] = $presale['shop_price'] - $presale['deposit'];
        $presale['formated_final_payment'] = $this->sanmuRepository->getPriceFormat($presale['final_payment'], false);
        ...
        if (!$presale['act_desc']) {
            $presale['act_desc'] = $this->sanmuRepository->getContentImgReplace($presale['goods_desc']);
        }
        ...
        $presale['goods_img'] = $this->sanmuRepository->getImagePath($presale['goods_img']);
        $presale['goods_thumb'] = $this->sanmuRepository->getImagePath($presale['goods_thumb']);
        $presale['goods_desc'] = $this->sanmuRepository->getContentImgReplace($presale['goods_desc']);
