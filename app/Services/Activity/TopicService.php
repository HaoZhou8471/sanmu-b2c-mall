...
class TopicService
{
        ...
        if ($topic) {
            foreach ($topic as $k => $v) {
                if ($v['page_id'] > 0) {
                    ...
                    if ($pctopic) {
                        $arr[$k]['topic_img'] = $this->sanmuRepository->getImagePath($v['thumb_pic']);
                        $arr[$k]['topic_id'] = $v['id'];
                        $arr[$k]['title'] = $v['title'];
                    }
                } else {
    ...
    public function getDetail($uid, $topic_id = 0, $warehouse_id = 0, $area_id = 0, $area_city = 0)
    {
        ...
        if ($topic) {
            $topic['topic_img'] = $this->sanmuRepository->getImagePath($topic['topic_img']);
            ...
            if ($goods_id) {
                ...
                if ($goods_list) {
                    foreach ($goods_list as $key => $val) {
                        ...
                        $goods_list[$key]['market_price_formated'] = $this->sanmuRepository->getPriceFormat($val['market_price']);
                        $goods_list[$key]['rank_price_formated'] = $this->sanmuRepository->getPriceFormat($val['shop_price']);
                        if ($promote_price > 0) {
                            $goods_list[$key]['shop_price_formated'] = $this->sanmuRepository->getPriceFormat($promote_price);
                        } else {
                            $goods_list[$key]['shop_price_formated'] = $this->sanmuRepository->getPriceFormat($val['shop_price']);
                        }

                        $goods_list[$key]['goods_thumb'] = $this->sanmuRepository->getImagePath($val['goods_thumb']);
                    }
