        ...
        if ($count > 0) {
            ...
            if ($row) {
                foreach ($row as $key => $val) {
                    ...
                    $res = $res ? $res->toArray() : [];
                    $row[$key]['format_shop_price'] = $this->sanmuRepository->getPriceFormat($res['shop_price']);
                    if (empty($val['get_goods']['goods_thumb'])) {
                        $row[$key]['goods_thumb'] = $this->config['no_picture'];
                    } else {
                        $row[$key]['goods_thumb'] = $this->sanmuRepository->getImagePath($val['get_goods']['goods_thumb']);
                    }
                }
    ...
    public function getWholeSaleDetail($goods_id, $user_id)
    {
        ...
        foreach ($prices as $k => $v) {
            ...
            foreach ($v['qp_list'] as $kitem => $vitem) {
                ...
                $prices[$k]['qp_list'][$kitem]['price'] = $this->sanmuRepository->getPriceFormat($vitem['price']);
