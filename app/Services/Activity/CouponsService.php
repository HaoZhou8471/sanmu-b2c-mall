...
class CouponsService
{
    ...
    public function getFromatCoupons($cou_data = [], $user_id = 0)
    {
        ...
        foreach ($cou_data as $k => $v) {
            ...
            if (!empty($v['cou_goods'])) {
                $v['cou_goods'] = $this->sanmuRepository->delStrComma($v['cou_goods']);
                ...
                if (!empty($cou_goods_arr)) {
                    foreach ($cou_goods_arr as $g_key => $g_val) {
                        if ($g_val['goods_thumb']) {
                            $cou_goods_arr[$g_key]['goods_thumb'] = $this->sanmuRepository->getImagePath($g_val['goods_thumb']);
                        }
                    }
                }
    ...
    public function getCouponsGoods($cou_type = [], $num = 0)
    {
        ...
        if ($cou_goods) {
            foreach ($cou_goods as $k => $v) {
                ...
                //商品图片(没有指定商品时为默认图片)
                if ($v['cou_ok_goods']) {
                    $v['cou_ok_goods'] = $this->sanmuRepository->delStrComma($v['cou_ok_goods']);
                    ...
                    $cou_goods[$k]['cou_ok_goods_name'][0]['goods_thumb'] = $this->sanmuRepository->getImagePath("images/coupons_default.png");
                }
                //可使用的店铺;
            ...
            foreach ($cou_data as $k => $v) {
                ...
                if ($cou_surplus) {
                    ...
                    if ($v['cou_ok_goods']) {
                        ...
                        $cou_data[$k]['cou_ok_goods_name'][0]['goods_thumb'] = $this->sanmuRepository->getImagePath("images/coupons_default.png");
                    }
                    ...
                    $cou_data[$k]['format_cou_money'] = $this->sanmuRepository->getPriceFormat($v['cou_money']);
    ...
    public function getUserCouponsList($user_id = '', $is_use = false, $total = '', $cart_goods = [], $user = true, $cart_ru_id = -1, $act_type = 'user', $province = 0)
    {
        ...
        if ($is_use && isset($total) && $cart_goods) {
            ...
            $couarr = [];
            foreach ($res as $key => $row) {
                $row['goods_id'] = $this->sanmuRepository->delStrComma($row['goods_id']);
                $row['cat_id'] = $this->sanmuRepository->delStrComma($row['cat_id']);
    ...
    public function getCouChildren($cat = '')
    {
        ...
        if ($cat) {
            ...
            $catlist = $this->sanmuRepository->delStrComma($catlist, 0, -1);
            $catlist = array_unique(explode(",", $catlist));
            $catlist = implode(",", $catlist);
            $cat = implode(",", $cat);
            $catlist = !empty($catlist) ? $catlist . "," . $cat : $cat;

            $catlist = $this->sanmuRepository->delStrComma($catlist);
        }
            ...
            foreach ($row as $key => $val) {
                ...
                if ($val['user_id']) {
                    ...
                    if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                        $row[$key]['user_name'] = $this->sanmuRepository->stringToStar($row[$key]['user_name']);
                    }
                }
            }
        }
