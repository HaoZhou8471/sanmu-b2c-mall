...
class AuctionService
{
    ...
    public function auctionList($user_id = 0, $keywords = '', $sort = 'act_id', $order = 'desc', $page = 1, $size = 10)
    {
        ...
        if ($res) {
            ...
            foreach ($res as $row) {
                ...
                $auction['formated_start_price'] = $this->sanmuRepository->getPriceFormat($auction['start_price']);
                $auction['formated_end_price'] = $this->sanmuRepository->getPriceFormat($auction['end_price']);
                $auction['formated_deposit'] = $this->sanmuRepository->getPriceFormat($auction['deposit']);
                $auction['goods_thumb'] = $this->sanmuRepository->getImagePath($row['goods_thumb']);
                ...
                if ($auction['bid_user_count'] > 0) {
                    ...
                    if (!empty($log)) {
                        ...
                        $log['formated_bid_price'] = $this->sanmuRepository->getPriceFormat($log['bid_price'], false); //最后出价
    ...
    public function getAuctionInfo($act_id = 0, $config = false, $path = '')
    {
        ...
        $auction['formated_start_price'] = $this->sanmuRepository->getPriceFormat($auction['start_price']);
        $auction['formated_end_price'] = $this->sanmuRepository->getPriceFormat($auction['end_price']);
        $auction['formated_amplitude'] = $this->sanmuRepository->getPriceFormat($auction['amplitude']);
        $auction['formated_deposit'] = $this->sanmuRepository->getPriceFormat($auction['deposit']);
        ...
        if ($auction['bid_user_count'] > 0) {
            ...
            if (!empty($row)) {
                ...
                $row['formated_bid_price'] = $this->sanmuRepository->getPriceFormat($row['bid_price'], false);
        ...
        $auction['formated_current_price'] = $this->sanmuRepository->getPriceFormat($auction['current_price'], false);
    ...
    public function getAuctionList($keywords, $sort = 'act_id', $order = 'desc', $size = 20, $page = 1, $cats = [])
    {
        ...
        if ($res) {
            ...
            foreach ($res as $row) {
                ...
                $auction['formated_end_price'] = $this->sanmuRepository->getPriceFormat($auction['end_price']);
                $auction['formated_deposit'] = $this->sanmuRepository->getPriceFormat($auction['deposit']);
                $auction['goods_thumb'] = isset($row['goods_thumb']) ? $this->sanmuRepository->getImagePath($row['goods_thumb']) : '';
                $auction['url'] = $this->sanmuRepository->buildUri('auction', ['auid' => $auction['act_id']]);
        ...
        if ($res) {
            ...
            foreach ($res as $key => $row) {
                ...
                $auction[$key]['short_name'] = $GLOBALS['_CFG']['goods_name_length'] > 0 ?
                    $this->sanmuRepository->subStr($row['goods_name'], $GLOBALS['_CFG']['goods_name_length']) : $row['goods_name'];
                $auction[$key]['thumb'] = $this->sanmuRepository->getImagePath($row['goods_thumb']);
                $auction[$key]['goods_img'] = $this->sanmuRepository->getImagePath($row['goods_img']);
                $auction[$key]['url'] = $this->sanmuRepository->buildUri('auction', ['auid' => $row['act_id'], $row['act_name']]);
                ...
                $auction_info['formated_start_price'] = $this->sanmuRepository->getPriceFormat($auction_info['start_price']);

                $auction[$key]['auction'] = $auction_info;
                $auction[$key]['status_no'] = $this->activityRepository->getAuctionStatus($auction_info);
                $auction[$key]['start_price'] = $this->sanmuRepository->getPriceFormat($auction_info['start_price']);
    ...
    public function getAuctionGoodsList($user_id, $record_count, $page, $list = [], $size = 10)
    {
        ...
        if ($res) {
            ...
            foreach ($res as $row) {
                ...
                $arr['goods_thumb'] = empty($row['goods_thumb']) ? '' : $this->sanmuRepository->getImagePath($row['goods_thumb']);
                ...
                $arr['bid_price'] = $this->sanmuRepository->getPriceFormat($row['bid_price']);
    ...
    public function getAuctionBidGoodsList($user_id, $page, $list = [], $size = 10)
    {
        ...
        if ($res) {
            ...
            foreach ($res as $row) {
                ...
                $arr['act_id'] = $row['act_id'];
                $arr['act_name'] = $row['act_name'];
                $arr['goods_thumb'] = $this->sanmuRepository->getImagePath($row['goods_thumb']);
                $arr['goods_name'] = $row['goods_name'];
                $arr['start_time'] = $row['start_time'];
                $arr['end_time'] = $row['end_time'];
                $arr['bid_time'] = $this->timeRepository->getLocalDate($timeFormat, $row['bid_time']);
