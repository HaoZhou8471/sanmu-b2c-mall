...
class BrandMobileService
{
    ...
    public function index($num = 20)
    {
        ...
        foreach ($brand as $key => $val) {
            if ($key == 0) {
                ...
                $res['best']['brand_logo'] = $this->sanmuRepository->getImagePath('data/brandlogo/' . $val['brand_logo']);
                $res['best']['index_img'] = $this->sanmuRepository->getImagePath('data/indeximg/' . $val['index_img']);
            } elseif ($key > 0 && $key < 5) {
                $res['hot'][$key] = $val;
                $res['hot'][$key]['brand_logo'] = $this->sanmuRepository->getImagePath('data/brandlogo/' . $val['brand_logo']);
                $res['hot'][$key]['index_img'] = $this->sanmuRepository->getImagePath('data/indeximg/' . $val['index_img']);
                ...
                $res['hot'][$key]['brand_msg']['brand_logo'] = $this->sanmuRepository->getImagePath('data/brandlogo/' . $val['brand_logo']);
                ...
                $res['recommend'][$key]['brand_logo'] = $this->sanmuRepository->getImagePath('data/brandlogo/' . $val['brand_logo']);
                $res['recommend'][$key]['index_img'] = $this->sanmuRepository->getImagePath('data/indeximg/' . $val['index_img']);
            }
        }
    ...
    public function detail($brand_id, $cat_id = 0)
    {
        ...
        if ($brand) {
            ...
            $res['brand_msg']['brand_logo'] = $this->sanmuRepository->getImagePath('data/brandlogo/' . $brand['brand_logo']);
    ...
    public function BrandGoodsList($uid, $brand_id = 0, $cat_id = 0, $type = '', $size = 10, $page = 1, $sort = '', $order = 'DESC', $ext = [])
    {
        ...
        if ($res) {
            foreach ($res as $k => $row) {
                ...
                $arr[$k]['market_price_format'] = $this->sanmuRepository->getPriceFormat($row['market_price']);
                $arr[$k]['shop_price'] = $row['shop_price'];
                $arr[$k]['shop_price_format'] = $this->sanmuRepository->getPriceFormat($row['shop_price']);
    ...
    public function BrandGoods($brand_id, $num = 3)
    {
        ...
        foreach ($res as $k => $val) {
            $res[$k]['goods_thumb'] = $this->sanmuRepository->getImagePath($val['goods_thumb']);
        }
    ...
    public function get_brands_letter()
    {
        ...
        foreach ($res as $row) {
            ...
            $brand['brand_logo'] = $this->sanmuRepository->getImagePath('data/brandlogo/' . $row['brand_logo']);
