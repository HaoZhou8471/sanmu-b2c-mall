...
class BrandService
{
    ...
    public function getBrandInfo($id_name, $act = '', $selType = 0)
    {
        ...
        if ($brand) {
            $brand['brand_logo'] = !empty($brand['brand_logo']) ? $this->sanmuRepository->getImagePath(DATA_DIR . '/brandlogo/' . $brand['brand_logo']) : '';
            $brand['index_img'] = !empty($brand['index_img']) ? $this->sanmuRepository->getImagePath(DATA_DIR . '/indeximg/' . $brand['index_img']) : '';
            $brand['brand_bg'] = !empty($brand['brand_bg']) ? $this->sanmuRepository->getImagePath(DATA_DIR . '/brandbg/' . $brand['brand_bg']) : '';
        }
    ...
    public function getBrands($cat = 0, $app = 'brand', $num = 0, $page = 1, $page_size = 8)
    {
        ...
        if ($row) {
            foreach ($row as $key => $val) {
                if ($val['site_url'] && strlen($val['site_url']) > 8) {
                    ...
                    $row[$key]['url'] = $this->sanmuRepository->buildUri($app, ['cid' => $cat, 'bid' => $val['brand_id']], $val['brand_name']);
                }
                $row[$key]['brand_desc'] = htmlspecialchars($val['brand_desc'], ENT_QUOTES);
                $row[$key]['brand_logo'] = $this->sanmuRepository->getImagePath(DATA_DIR . '/brandlogo/' . $val['brand_logo']);
                $row[$key]['index_img'] = $this->sanmuRepository->getImagePath(DATA_DIR . '/indeximg/' . $val['index_img']); //品牌专区大图 by wu
                $row[$key]['brand_bg'] = $this->sanmuRepository->getImagePath(DATA_DIR . '/brandbg/' . $val['brand_bg']); //品牌专区大图 by wu
        ...
        $page_array = $this->sanmuRepository->pageArray($page_size, $page, $row);
    ...
    public function getBrandRelatedCat($brand)
    {
        $arr[] = ['cat_id' => 0,
            ...
            'url' => $this->sanmuRepository->buildUri('brand', ['bid' => $brand], $GLOBALS['_LANG']['all_category'])];
        ...
        if ($res) {
            foreach ($res as $row) {
                $row['url'] = $this->sanmuRepository->buildUri('brand', ['cid' => $row['cat_id'], 'bid' => $brand], $row['cat_name']);
                $arr[] = $row;
            }
        }
