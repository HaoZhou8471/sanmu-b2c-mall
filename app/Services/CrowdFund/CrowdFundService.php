...
class CrowdFundService
{
    ...
    public function getZcCategoryParents($parent_id = 0, $type = 0)
    {
        ...
        $arr = [
            'cate_one' => $cate_one,
            'cate_two' => $cate_two,
            'str_id' => $this->sanmuRepository->delStrComma($str_id)
        ];
    ...
    public function getZcProjectList($keyword = '', $str_id = '', $order = 'id', $page = 1, $size = 10, $type = '')
    {
        ...
        if ($zc_arr) {
            foreach ($zc_arr as $k => $z_val) {
                ...
                $list[$k]['amount_formated'] = $this->sanmuRepository->getPriceFormat($z_val['amount'], false);
                $list[$k]['join_money'] = $z_val['join_money'];
                $list[$k]['join_money_formated'] = $this->sanmuRepository->getPriceFormat($z_val['join_money'], false);
                $min_price = $this->plan_min_price($z_val['id']);
                $list[$k]['min_price'] = $this->sanmuRepository->getPriceFormat($min_price, false);
                ...
                $list[$k]['title_img'] = $this->sanmuRepository->getImagePath($z_val['title_img']);
    ...
    public function zcGoodsInfo($id = 0)
    {
        ...
        if ($info) {
            $info['title_img'] = $this->sanmuRepository->getImagePath($info['title_img']);
    ...
    public function getZcGoods($pid = 0)
    {
        ...
        if ($goods_arr) {
            foreach ($goods_arr as $key => $row) {
                $goods_arr[$key]['img'] = $this->sanmuRepository->getImagePath($row['img']);
            ...
            foreach ($progress as $key => $row) {
                ...
                if (!empty($row['img'])) {
                    foreach ($row['img'] as $k2 => $v2) {
                        $row['img'][$k2] = $this->sanmuRepository->getImagePath($v2);
                    }
    ...
    public function getBackerList($id = 0)
    {
        ...
        if ($list) {
            foreach ($list as $key => $value) {
                ...
                $backer[$key]['user_picture'] = $this->sanmuRepository->getImagePath($value['user_picture'], '', asset('img/user_default.png'));
    ...
    public function getTopicList($pid = 0, $type = false, $page = 1, $size = 10)
    {
        ...
        if ($list) {
            foreach ($list as $key => $value) {
                ...
                $topic[$key]['user_picture'] = $this->sanmuRepository->getImagePath($value['user_picture'], '', asset('img/user_default.png'));
                ...
                if ($childlist) {
                    foreach ($childlist as $k => $val) {
                        ...
                        $child_topic[$k]['user_picture'] = $this->sanmuRepository->getImagePath($val['user_picture'], '', asset('img/user_default.png'));
    ...
    public function getMyFocus($user_id = 0, $status = 0, $page = 10, $size = 0)
    {
        ...
        if ($res) {
            foreach ($res as $k => $val) {
                ...
                $list[$k]['amount_formated'] = $this->sanmuRepository->getPriceFormat($val['amount'], false);
                $list[$k]['join_money'] = $val['join_money'];
                $list[$k]['join_money_formated'] = $this->sanmuRepository->getPriceFormat($val['join_money'], false);
                ...
                $list[$k]['title_img'] = $this->sanmuRepository->getImagePath($val['title_img']);
            }
        }
        ...
        if ($res) {
            ...
            foreach ($res as $key => $row) {
                ...
                $list[] = [
                    ...
                    'title_img' => $this->sanmuRepository->getImagePath($row['title_img']), //项目图片
                    'content' => $row['content'], //项目商品名称
                    'price' => $row['price'], //项目商品价格
                    'zc_status' => $zc_status, //众筹活动状态
                    'handler' => $row['handler'],
//                    'handler_this' => $row['handler_this'],
                    'total_fee' => $this->sanmuRepository->getPriceFormat($row['goods_amount'] + $row['shipping_fee'], false), // 总额
    ...
    public function getOrderDetail($user_id = 0, $order_id = 0)
    {
        ...
        if ($res) {
            ...
            $res['title_img'] = $this->sanmuRepository->getImagePath($res['title_img']); //项目图片
            if ($res['pay_time'] > 0) {
            ...
            // 格式化金额字段
            $res['formated_join_money'] = $this->sanmuRepository->getPriceFormat($res['join_money'], false);
            $res['formated_price'] = $this->sanmuRepository->getPriceFormat($res['price'], false);
            $res['formated_goods_amount'] = $this->sanmuRepository->getPriceFormat($res['goods_amount'], false);
            $res['formated_shipping_fee'] = $this->sanmuRepository->getPriceFormat($res['shipping_fee'], false);
    ...
    public function getSpecialZcList($type = 0, $num = 5, $sort = 'id', $order = "DESC")
    {
        ...
        if ($res) {
            foreach ($res as $k => $val) {
                $res[$k]['title_img'] = $this->sanmuRepository->getImagePath($val['title_img']);
            }
        }
    ...
    public function getInitiatorInfo($cid)
    {
        ...
        if ($row) {
            $row['img'] = $this->sanmuRepository->getImagePath($row['img']);
            //不存在图片，则输出默认图片
            if ($this->sanmuRepository->remoteLinkExists($row['img']) === false) {
                $row['img'] = $this->sanmuRepository->getImagePath('');
            }
            //处理等级标识
        ...
        if (!empty(request()->cookie('zc_history'))) {
            ...
            if ($res) {
                foreach ($res as $row) {
                    ...
                    $arr[$row['id']]['title_img'] = $this->sanmuRepository->getImagePath($row['title_img']);
                }
            }
        }
    ...
    public function getUserZcProjectList($user_id = 0)
    {
        ...
        if ($res) {
            foreach ($res as $key => $row) {
                ...
                $row['title_img'] = $this->sanmuRepository->getImagePath($row['title_img']);
                $row['format_join_money'] = $this->sanmuRepository->getPriceFormat($row['join_money']);
    ...
    public function getUserZcGoodsList($user_id = 0)
    {
        ...
        if ($res) {
            foreach ($res as $key => $row) {
                $res[$key]['title_img'] = $this->sanmuRepository->getImagePath($row['title_img']);
                $res[$key]['format_join_money'] = $this->sanmuRepository->getPriceFormat($row['join_money']);
                $res[$key]['format_goods_amount'] = $this->sanmuRepository->getPriceFormat($row['goods_amount']);
            }
        }
