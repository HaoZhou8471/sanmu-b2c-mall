...
class CrowdFlowService
{
    ...
    public function getCartGoods($pid = 0, $id = 0, $number = 0)
    {
        ...
        if ($res) {
            ...
            $list['formated_amount'] = $this->sanmuRepository->getPriceFormat($res['amount'], false);
            $list['join_money'] = $res['join_money'];
            $list['formated_join_money'] = $this->sanmuRepository->getPriceFormat($res['join_money'], false);
            ...
            $list['title_img'] = $this->sanmuRepository->getImagePath($res['title_img']);
            $list['price'] = $res['price'];
            $list['formated_price'] = $this->sanmuRepository->getPriceFormat($res['price'], false);
    ...
    public function getOrderFee($goods = [])
    {
        ...
        $total['shipping_fee'] = $goods['shipping_fee'] ? $this->sanmuRepository->getPriceFormat($goods['shipping_fee'], false) : 0;
        // 格式化订单总额
        $total['amount_formated'] = $this->sanmuRepository->getPriceFormat($total['amount'], false);
