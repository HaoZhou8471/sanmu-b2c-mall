...
class MerchantsUsersListManageService
{
    ...
    public function stepsUsersList()
    {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0 && (strlen($users_list[$i]['user_name']) == 11 && is_numeric($users_list[$i]['user_name']))) {
                $users_list[$i]['user_name'] = $this->sanmuRepository->stringToStar($users_list[$i]['user_name']);
            }
            ...
            $users_list[$i]['authorizeFile'] = $this->sanmuRepository->getImagePath($users_list[$i]['authorizeFile']);
            $users_list[$i]['grade_img'] = isset($grade['grade_img']) ? $this->sanmuRepository->getImagePath($grade['grade_img']) : '';
        }
        ...
        foreach ($res as $key => $row) {
            ...
            if ($row['steps_style'] == 1) {
                $parent = [//店铺类型数据插入
                    ...
                    'authorizeFile' => isset($shop_info['authorizeFile']) && !empty($shop_info['authorizeFile']) ? $this->sanmuRepository->getImagePath($shop_info['authorizeFile']) : '',
                ...
                $parent = [
                    ...
                    'brandLogo' => isset($brand_info['brandLogo']) ? $this->sanmuRepository->getImagePath($brand_info['brandLogo']) : '',
    ...
    public function getAdminMerchantsStepsTitle($user_id = 0, $addImg = '')
    {
        ...
        if (isset($_FILES['ec_brandLogo']) && !empty($_FILES['ec_brandLogo'])) {
            ...
            $this->sanmuRepository->getOssAddFile([$brandLogo]);
        }
        ...
        foreach ($res as $key => $row) {
            ...
            if ($row['steps_style'] == 1) {
                if (isset($_FILES['ec_authorizeFile'])) {
                    ...
                    $this->sanmuRepository->getOssAddFile([$ec_authorizeFile]);
                }
                ...
                if (isset($_FILES['ec_authorizeFile'])) {
                    ...
                    $this->sanmuRepository->getOssAddFile([$ec_shop_hypermarketFile]);
                }
