...
class MerchantsCommissionManageService
{
    ...
    public function merchantsCommissionList()
    {
        ...
        for ($i = 0; $i < count($row); $i++) {
            ...
            if ($valid) {
                $row[$i]['order_valid_total'] = $this->sanmuRepository->getPriceFormat($valid['total_fee']);
                ...
                if (file_exists(MOBILE_DRP)) {
                    $row[$i]['order_drp_commission'] = $this->sanmuRepository->getPriceFormat($valid['drp_money']); //dis liu
                }
            }

            $row[$i]['order_refund_total'] = $this->sanmuRepository->getPriceFormat($valid['return_amount']);
            ...
            if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                $row[$i]['user_name'] = $this->sanmuRepository->stringToStar($row[$i]['user_name']);
            }
        }

        $admin_commission['is_settlement'] = $this->sanmuRepository->getPriceFormat($admin_commission['is_settlement']);
        $admin_commission['no_settlement'] = $this->sanmuRepository->getPriceFormat($admin_commission['no_settlement']);
    ...
    public function merchantsOrderList($page = 0)
    {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            $row[$i]['formated_discount'] = $this->sanmuRepository->getPriceFormat($row[$i]['discount'], true);
            $row[$i]['formated_coupons'] = $this->sanmuRepository->getPriceFormat($row[$i]['coupons'], true);
            $row[$i]['formated_integral_money'] = $this->sanmuRepository->getPriceFormat($row[$i]['integral_money'], true);
            $row[$i]['formated_bonus'] = $this->sanmuRepository->getPriceFormat($row[$i]['bonus'], true);
            $row[$i]['formated_order_total_fee'] = $this->sanmuRepository->getPriceFormat($row[$i]['order_total_fee'] - $row[$i]['discount'], true);
            $row[$i]['formated_order_amount_field'] = $this->sanmuRepository->getPriceFormat($row[$i]['total_fee'] + $row[$i]['shipping_fee'], true);
            $row[$i]['formated_shipping_fee'] = $this->sanmuRepository->getPriceFormat($row[$i]['shipping_fee'], true);
            ...
            if (file_exists(MOBILE_DRP)) {
                ...
                if (isset($commission_info['commission_model']) && $commission_info['commission_model']) {
                    ...
                    $row[$i]['formated_brokerage_amount'] = $this->sanmuRepository->getPriceFormat($order_commission + $row[$i]['shipping_fee'], true);

                    $effective_amount_price = $order_commission;
                    $row[$i]['effective_amount_price'] = $this->sanmuRepository->changeFloat($effective_amount_price);
                    ...
                    $row[$i]['formated_brokerage_amount'] = $this->sanmuRepository->getPriceFormat($total_return_amount * $percent_value + $row[$i]['shipping_fee'] + $brokerage_amount['should_amount'], true);
                    ...
                    $row[$i]['gain_commission'] = $this->sanmuRepository->changeFloat($gain_commission);
                }
                ...
                $row[$i]{'formated_drp_commission'} = isset($row[$i]['drp_money']) ? $this->sanmuRepository->getPriceFormat($row[$i]['drp_money'], true) : $this->sanmuRepository->getPriceFormat(0);
                $row[$i]['brokerage_amount_price'] = $total_return_amount;
                $row[$i]['formated_drp_commission'] = $this->sanmuRepository->getPriceFormat($brokerage_amount['drp_money'], true);
                ...
                if ($commission_info['commission_model']) {
                    ...
                    $row[$i]['formated_brokerage_amount'] = $this->sanmuRepository->getPriceFormat($total_return_amount * $percent_value + $row[$i]['shipping_fee'] + $goods_rate['should_amount'], true);
            ...
            $row[$i]['formated_effective_amount_price'] = $this->sanmuRepository->getPriceFormat($row[$i]['effective_amount_price'], true);
            ...
            if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                $row[$i]['mobile'] = $this->sanmuRepository->stringToStar($row[$i]['mobile']);
                $row[$i]['buyer'] = $this->sanmuRepository->stringToStar($row[$i]['buyer']);
                $row[$i]['tel'] = $this->sanmuRepository->stringToStar($row[$i]['tel']);
                $row[$i]['email'] = $this->sanmuRepository->stringToStar($row[$i]['email']);
            }
        }
        ...
        $brokerage_amount['is_settlement'] = $this->sanmuRepository->getPriceFormat($settlemen['is_settlement'], true);
        $brokerage_amount['no_settlement'] = $this->sanmuRepository->getPriceFormat($settlemen['no_settlement'], true);

        $brokerage_amount['all_price'] = $settlemen['settlement_all'];
        $brokerage_amount['all'] = $this->sanmuRepository->getPriceFormat($settlemen['settlement_all'], true);
    ...
    public function negativeBillList()
    {
        ...
        $filter['bill_sn'] = isset($_REQUEST['bill_sn']) && !empty($_REQUEST['bill_sn']) ? sanmu_addslashes($_REQUEST['bill_sn']) : '';
        $filter['commission_bill_sn'] = isset($_REQUEST['commission_bill_sn']) && !empty($_REQUEST['commission_bill_sn']) ? sanmu_addslashes($_REQUEST['commission_bill_sn']) : '';
        ...
        if ($res) {
            foreach ($res as $key => $val) {
                $res[$key]['format_return_amount'] = $this->sanmuRepository->getPriceFormat($val['return_amount']);
                $res[$key]['format_return_shippingfee'] = $this->sanmuRepository->getPriceFormat($val['return_shippingfee']);
                $res[$key]['format_return_rate_price'] = $this->sanmuRepository->getPriceFormat($val['return_rate_price']);
                $res[$key]['format_return_total'] = $this->sanmuRepository->getPriceFormat($val['return_amount'] + $val['return_shippingfee']);
    ...
    public function negativeOrderList()
    {
        ...
        $filter['return_sn'] = isset($_REQUEST['return_sn']) && !empty($_REQUEST['return_sn']) ? sanmu_addslashes($_REQUEST['return_sn']) : '';
        $filter['order_sn'] = isset($_REQUEST['order_sn']) && !empty($_REQUEST['order_sn']) ? sanmu_addslashes($_REQUEST['order_sn']) : '';
        $filter['start_time'] = isset($_REQUEST['start_time']) && !empty($_REQUEST['start_time']) ? sanmu_addslashes($_REQUEST['start_time']) : '';
        $filter['end_time'] = isset($_REQUEST['end_time']) && !empty($_REQUEST['end_time']) ? sanmu_addslashes($_REQUEST['end_time']) : '';
        ...
        $filter['format_return_amount'] = $this->sanmuRepository->getPriceFormat($filter['format_return_amount']);
        $filter['format_return_shippingfee'] = $this->sanmuRepository->getPriceFormat($filter['format_return_shippingfee']);
        $filter['format_return_rate_price'] = $this->sanmuRepository->getPriceFormat($filter['format_return_rate_price']);
