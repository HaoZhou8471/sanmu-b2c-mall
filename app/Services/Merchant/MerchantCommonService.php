...
class MerchantCommonService
{
     ...
     * @throws \Exception
     */
    public function getShopName($supplier_id = 0, $type = 0)
    {
        $Shopinfo_cache_name = 'SellerShopinfo_' . $supplier_id;
        $shopinfo = cache($Shopinfo_cache_name);

/*        if (is_null($shopinfo)) {
            $shopinfo = SellerShopinfo::where('supplier_id', $supplier_id);
            $shopinfo = $this->baseRepository->getToArrayFirst($shopinfo);

            $shopinfo['shop_url'] = $this->sanmuRepository->sellerUrl($supplier_id);

            cache()->forever($Shopinfo_cache_name, $shopinfo);
        }*/

        $MerchantsShopInformation_cache_name = 'MerchantsShopInformation_' . $supplier_id;
        $shop_information = cache($MerchantsShopInformation_cache_name);

/*        if (is_null($shop_information)) {
            $shop_information = MerchantsShopInformation::where('user_id', $supplier_id);
     ...
     * @throws \Exception
     */
    public function getSellerDomainUrl($supplier_id = 0, $build_uri = [])
    {
        $build_uri['cid'] = isset($build_uri['cid']) ? $build_uri['cid'] : 0;
        $build_uri['urid'] = isset($build_uri['urid']) ? $build_uri['urid'] : $supplier_id;
        ...
        $res = [];
        $res['domain_name'] = $this->sanmuRepository->sellerUrl($build_uri['urid'], $other);
     ...
     * @return mixed
     */
    public function getSellerDomainInfo($supplier_id = 0)
    {
        $row = SellerDomain::where('supplier_id', $supplier_id);
        $row = $this->baseRepository->getToArrayFirst($row);
    ...
    public function getMerchantsStepsProcess($process_steps = 0)
    {
        ...
        if ($result['process_article'] > 0) {
            ...
            if ($article) {
                if ($article['content']) {
                    // 过滤样式 手机自适应
                    $article['content'] = $this->sanmuRepository->contentStyleReplace($article['content']);
                    // 显示文章详情图片 （本地或OSS）
                    $article['content'] = $this->sanmuRepository->getContentImgReplace($article['content']);
                }
            }
