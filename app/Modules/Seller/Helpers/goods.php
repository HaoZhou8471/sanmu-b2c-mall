...
function handle_goods_area($goods_id)
{
    $sql = "UPDATE " . $GLOBALS['sanmu']->table('link_area_goods') . " SET " .
        " goods_id = '$goods_id' " .
        " WHERE goods_id = '0'" .
        " AND ru_id = (SELECT ru_id FROM " . $GLOBALS['sanmu']->table('admin_user') . "  WHERE user_id = '" . session('seller_id') . "') ";
    $GLOBALS['db']->query($sql);
}
...
{ //ecmoban模板堂 --zhuo  $content = ''
    if ($goods_id) {
        ...
        if ($field == 'is_on_sale') {
            if ($value == 1) {
                $sql = "SELECT act_id FROM " . $GLOBALS['sanmu']->table('presale_activity') . " WHERE goods_id = '$goods_id'";
                if ($GLOBALS['db']->getOne($sql, true)) {
                    $GLOBALS['db']->query("DELETE FROM " . $GLOBALS['sanmu']->table('presale_activity') . " WHERE goods_id = '$goods_id' ");
                    $GLOBALS['db']->query("DELETE FROM " . $GLOBALS['sanmu']->table('cart') . " WHERE goods_id = '$goods_id' ");
                }
            } else {
...
{
    app(SanmuRepository::class)->helpersLang('goods', 'seller');
...
function goods_list($is_delete = 0, $real_goods = 1, $conditions = '')
{
    ...
    if ($result === false) {
        ...
        $sql = "SELECT g.goods_id, g.goods_name, g.model_price, g.model_inventory, g.user_id, g.goods_type, g.goods_sn, g.shop_price, g.is_on_sale, g.is_best, g.is_new, g.is_hot, g.sort_order, g.goods_number, g.integral, " .
            ...
            " LEFT JOIN " . $GLOBALS['sanmu']->table('goods_cat') . " as gc on g.goods_id = gc.goods_id" . " WHERE $where GROUP BY g.goods_id";
        ...
        $sql = "SELECT g.goods_id, g.goods_name, g.user_id, g.brand_id, g.goods_type, g.goods_sn, g.shop_price, g.is_on_sale, " .
            ...
            " FROM " . $GLOBALS['sanmu']->table('goods') . " AS g " .
            " LEFT JOIN " . $GLOBALS['sanmu']->table('goods_cat') . " as gc on g.goods_id = gc.goods_id" . " WHERE $where" .
    ...
    for ($i = 0; $i < $count; $i++) {
        ...
        if (!empty($row[$i]['url']) && (strpos($row[$i]['url'], 'http://') === false && strpos($row[$i]['url'], 'https://') === false && strpos($row[$i]['url'], 'errorImg.png') === false)) {
            $row[$i]['url'] = $GLOBALS['sanmu']->seller_url() . $row[$i]['url'];
        }
    }
...
function product_number_count($goods_id, $conditions = '', $warehouse_id = 0)
{
    ...
    $sql = "SELECT product_number
            FROM " . $GLOBALS['sanmu']->table('products') . "
            WHERE goods_id = '$goods_id'
            " . $conditions;
...
function get_product_info($product_id, $filed = '')
{
    ...
    $sql = "SELECT $filed FROM  " . $GLOBALS['sanmu']->table('products') . " WHERE product_id = '$product_id'";
