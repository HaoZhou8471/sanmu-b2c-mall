...
function admin_log($sn = '', $action, $content)
{
    ...
    AdminLog::insert([
        ...
        'log_info' => $log_info,
        'ip_address' => app(SanmuRepository::class)->sanmuIp()
    ]);
}
...
{
    $sql = 'SELECT id FROM ' . $GLOBALS['sanmu']->table('shop_config') . " WHERE code = '$parent' AND type = 1";
    $parent_id = $GLOBALS['db']->getOne($sql);

    $sql = 'INSERT INTO ' . $GLOBALS['sanmu']->table('shop_config') . ' (parent_id, code, value) ' .
...
function admin_priv($priv_str, $msg_type = '', $msg_output = true)
{
    if (!session()->has('seller_action_list')) {
        ...
        $sql = 'SELECT action_list ' .
            ' FROM ' . $GLOBALS['sanmu']->table('admin_user') .
...
{
    $bonus = [];
    $sql = 'SELECT type_id, type_name, type_money FROM ' . $GLOBALS['sanmu']->table('bonus_type') .
        ' WHERE send_type = 3';
...
{
    $rank_list = [];
    $sql = 'SELECT rank_id, rank_name, min_points FROM ' . $GLOBALS['sanmu']->table('user_rank');
    if ($is_special) {
...
{
    $user_list = [];
    $sql = 'SELECT user_id, user_name FROM ' . $GLOBALS['sanmu']->table('users') . $where .
...
function get_position_list()
{
    ...
    $sql = 'SELECT position_id, position_name, ad_width, ad_height ' .
        'FROM ' . $GLOBALS['sanmu']->table('ad_position') . $where;
    $res = $GLOBALS['db']->query($sql);
...
function get_goods_list($filter, $limit = ['start' => 0, 'number' => 50])
{
    ...
    $sql = 'SELECT goods_id, goods_name, shop_price ' .
        'FROM ' . $GLOBALS['sanmu']->table('goods') . ' AS g ' . $where;
...
function get_article_list($filter)
{
    ...
    $sql = 'SELECT a.article_id, a.title ' .
        'FROM ' . $GLOBALS['sanmu']->table('article') . ' AS a, ' . $GLOBALS['sanmu']->table('article_cat') . ' AS c ' . $where;
    $res = $GLOBALS['db']->query($sql);
...
function get_where_sql($filter)
{
    ...
    if ($brand_keyword) {
        if ($sel_mode == 1 && !empty($brand_keyword)) {
            ...
            $sql = "SELECT brand_id FROM " . $GLOBALS['sanmu']->table('brand') . " WHERE brand_name LIKE '%$brand_keyword%' ";
...
function area_list($region_id = 0)
{
    ...
    $sql = 'SELECT * FROM ' . $GLOBALS['sanmu']->table('region') .
...
function goods_type_list($selected, $goods_id = 0, $type = 'html', $c_id = 0)
{
    ...
    $sql = 'SELECT cat_id, cat_name ,c_id FROM ' . $GLOBALS['sanmu']->table('goods_type') . ' WHERE enabled = 1 AND suppliers_id = 0' . $ruCat;
    $res = $GLOBALS['db']->query($sql);
...
function get_pay_ids()
{
    ...
    $sql = 'SELECT pay_id, is_cod FROM ' . $GLOBALS['sanmu']->table('payment') . ' WHERE enabled = 1';
    $res = $GLOBALS['db']->query($sql);
...
{
    $sql = 'TRUNCATE TABLE ' . $GLOBALS['sanmu']->table($table_name);
...
{
    $sql = "SELECT attr_group FROM " . $GLOBALS['sanmu']->table('goods_type') . " WHERE cat_id='$cat_id'";
...
{
    if ($ru_id > 0) {
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['sanmu']->table('merchants_category') .
            " WHERE parent_id = '$parent_cat' AND cat_name = '$cat_name' AND  cat_id <> '$exclude' AND user_id = '$ru_id'";

        return ($GLOBALS['db']->getOne($sql) > 0) ? true : false;
    } else {
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['sanmu']->table('category') .
...
{
    $sql = "SELECT COUNT(*) FROM " . $GLOBALS['sanmu']->table('brand') .
...
function admin_info()
{
    $sql = "SELECT * FROM " . $GLOBALS['sanmu']->table('admin_user') . "
...
function suppliers_list_info($conditions = '')
{
    ...
    $sql = "SELECT suppliers_id, suppliers_name, suppliers_desc
            FROM " . $GLOBALS['sanmu']->table("suppliers") . "
            $where";
...
{
    $sql = "select attr_values from " . $GLOBALS['sanmu']->table('attribute') . " where attr_id = '$attr_id'";
    ...
    if (!empty($attr_values)) {
        ...
        for ($i = 0; $i < count($attr_values); $i++) {
            $sql = "select attr_img, attr_site from " . $GLOBALS['sanmu']->table('attribute_img') . " where attr_id = '$attr_id' and attr_values = '" . $attr_values[$i] . "'";
...
function get_attrimg_insert_update($attr_id, $attr_values)
{
    ...
    if (count($attr_values) > 0) {
        for ($i = 0; $i < count($attr_values); $i++) {
            ...
            $sql = "select id, attr_img from " . $GLOBALS['sanmu']->table('attribute_img') . " where attr_id = '$attr_id' and attr_values = '" . $attr_values[$i]['values'] . "'";
            $res = $GLOBALS['db']->getRow($sql);
            ...
            if (!empty($upFile)) {
                if ($res['id'] > 0) {
                    ...
                    $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table('attribute_img'), $other, "UPDATE", "attr_id = '$attr_id' and attr_values = '" . $attr_values[$i]['values'] . "'");
                } else {
                    $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table('attribute_img'), $other, "INSERT");
                }
            }
        }
    }
...
function get_add_edit_link_desc($linked_array, $type = 0, $id = 0)
{
    ...
    if ($linked_array) {
        ...
        if ($id > 0) {
            $sql = "SELECT goods_id FROM " . $GLOBALS['sanmu']->table('link_goods_desc') . " WHERE id = '$id'";
        ...
        $sql = "SELECT goods_id FROM " . $GLOBALS['sanmu']->table('link_desc_temporary') . " WHERE ru_id = '" . $adminru['ru_id'] . "'";
        $tgoods = $GLOBALS['db']->getOne($sql, true);

        if ($type == 1) { //删除 由右至左
            if (!empty($tgoods)) {
                $other['goods_id'] = get_del_in_val($tgoods, $other['goods_id']);
                $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table('link_desc_temporary'), $other, "UPDATE", "1");
            } else {
                $other['goods_id'] = get_del_in_val($desc_goods_id, $other['goods_id']);
                $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table('link_desc_temporary'), $other, "INSERT");
            }
        } else { //添加 由左至右
...
function get_linked_goods_desc($id = 0)
{
    ...
    $sql = "SELECT goods_id FROM " . $GLOBALS['sanmu']->table($table) . $where;
    ...
    if (!empty($goods_id)) {
        ...
        for ($i = 0; $i < count($goods_id); $i++) {
            $sql = "SELECT goods_name FROM " . $GLOBALS['sanmu']->table('goods') . " WHERE goods_id = '" . $goods_id[$i] . "'";
...
function get_add_desc_goodsId($goods_id, $id)
{
    if (!empty($goods_id)) {
        ...
        for ($i = 0; $i < count($goods_id); $i++) {
            ...
            $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table('link_desc_goodsid'), $other, "INSERT");
        }
    }
}
...
function get_bacth_category($cat_name, $cat, $ru_id)
{
    for ($i = 0; $i < count($cat_name); $i++) {
        if (!empty($cat_name)) {
            ...
            if ($GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table('merchants_category'), $cat) !== false) {
                $cat_id = $GLOBALS['db']->insert_id();
                if ($cat['show_in_nav'] == 1) {
                    $vieworder = $GLOBALS['db']->getOne("SELECT max(vieworder) FROM " . $GLOBALS['sanmu']->table('merchants_nav') . " WHERE type = 'middle'");
                    $vieworder += 2;
                    //显示在自定义导航栏中
                    $sql = "INSERT INTO " . $GLOBALS['sanmu']->table('merchants_nav') .
                        " (name,ctype,cid,ifshow,vieworder,opennew,url,type)" .
                        " VALUES('" . $cat['cat_name'] . "', 'c', '$cat_id','1','$vieworder','0', '" . app(SanmuRepository::class)->buildUri('merchants_store', ['cid' => $cat_id, 'urid' => $ru_id], $cat['cat_name']) . "','middle')";
                    $GLOBALS['db']->query($sql);
                }
...
function cause_exists($cause_name, $c_id = 0)
{
    ...
    $sql = "SELECT COUNT(*) FROM " . $GLOBALS['sanmu']->table('return_cause') .
...
function return_order_list()
{
    ...
    if ($result === false) {
        ...
        $sql = "SELECT agency_id FROM " . $GLOBALS['sanmu']->table('admin_user') . " WHERE user_id = '" . session('seller_id') . "'";
        ...
        if ($filter['store_search'] > -1) {
            if ($adminru['ru_id'] == 0) {
                if ($filter['store_search'] > 0) {
                    ...
                    if ($filter['store_search'] > 1) {
                        $where .= " AND (SELECT COUNT(*) FROM " . $GLOBALS['sanmu']->table('merchants_shop_information') . ' as msi ' .
        ...
        if ($store_search == 0 && $adminru['ru_id'] == 0) {
            $where_store = " AND (SELECT COUNT(*) FROM " . $GLOBALS['sanmu']->table('order_goods') . " AS og " . " WHERE o.order_id = og.order_id AND og.ru_id = 0 limit 0,1) > 0 " .
                " AND o.main_count = 0";
        } else {
            $where_store = '';
        }
        ...
        /* 记录总数 */
        if ($filter['user_name']) {
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['sanmu']->table('order_return') . " AS o ," .
                $GLOBALS['sanmu']->table('order_info') . " as o, " .
                $GLOBALS['sanmu']->table('users') . " AS u " . $where . $where_store;
        } else {
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['sanmu']->table('order_return') . " AS r, " .
                $GLOBALS['sanmu']->table('order_info') . " as o " .
        ...
        $sql = "SELECT o.order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid, o.goods_amount, o.discount," .
            ...
            "FROM " . $GLOBALS['sanmu']->table('order_return') . " AS r " .
            "LEFT JOIN " . $GLOBALS['sanmu']->table('order_info') . " AS o ON r.order_id = o.order_id " .
            "LEFT JOIN " . $GLOBALS['sanmu']->table('users') . " AS u ON u.user_id=o.user_id  " . $where . $where_store .
    ...
    foreach ($row as $key => $value) {
        ...
        $sql = "SELECT return_number, refound FROM " . $GLOBALS['sanmu']->table('return_goods') . " WHERE rec_id = '" . $value['rec_id'] . "' LIMIT 1";
...
{
    $sql = 'INSERT INTO ' . $GLOBALS['sanmu']->table('notice_log') . ' (goods_id, email, send_ok, send_time, send_type) ' .
...
function get_delete_seller_info($table = '', $where = '')
{
    if (!empty($table) && !empty($where)) {
        $sql = "DELETE FROM " . $GLOBALS['sanmu']->table($table) . " WHERE $where";
        $GLOBALS['db']->query($sql);
    }
}
...
{
    $sql = "SELECT order_id FROM " . $GLOBALS['sanmu']->table('order_goods') . " WHERE ru_id = '$ru_id'";
    $order_id = $GLOBALS['db']->getOne($sql);

    $sql = "SELECT ret_id FROM " . $GLOBALS['sanmu']->table('order_return') . " WHERE order_id = '$order_id'";
    $ret_list = $GLOBALS['db']->getAll($sql);

    foreach ($ret_list as $key => $row) {
        $GLOBALS['db']->query("DELETE FROM " . $GLOBALS['sanmu']->table('return_goods') . " WHERE rec_id = '" . $row['rec_id'] . "'");
        $GLOBALS['db']->query("DELETE FROM " . $GLOBALS['sanmu']->table('return_action') . " WHERE ret_id = '" . $row['ret_id'] . "'");
    }

    $GLOBALS['db']->query("DELETE FROM " . $GLOBALS['sanmu']->table('order_return') . " WHERE order_id = '$order_id'");
    $GLOBALS['db']->query("DELETE FROM " . $GLOBALS['sanmu']->table('order_info') . " WHERE order_id = '$order_id'");
    $GLOBALS['db']->query("DELETE FROM " . $GLOBALS['sanmu']->table('order_goods') . " WHERE ru_id = '$ru_id'");
}
...
function get_seller_delete_goods_list($ru_id)
{
    ...
    //删除商家属性
    $sql = "SELECT cat_id FROM " . $GLOBALS['sanmu']->table('goods_type') . " WHERE user_id = '$ru_id'";
    $goods_type = $GLOBALS['db']->getAll($sql);

    foreach ($goods_type as $key => $row) {
        $sql = "SELECT attr_id FROM " . $GLOBALS['sanmu']->table('attribute') . " WHERE cat_id = '" . $row['cat_id'] . "'";
        $attribute_list = $GLOBALS['db']->getAll($sql);

        foreach ($attribute_list as $arow) {
            $GLOBALS['db']->query("DELETE FROM " . $GLOBALS['sanmu']->table('goods_attr') . " WHERE attr_id = '" . $row['attr_id'] . "'");
        }
    }
}
...
function get_order_detection_list($is_ajax = 0)
{
    ...
    if ($result === false) {
        ...
        if ($is_ajax == 1) {
            $sql = "UPDATE " . $GLOBALS['sanmu']->table('order_info') .
        ...
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['sanmu']->table('order_info') . " AS o " .
            $where . $where_store;
        ...
        $sql = "SELECT ifnull(bai.is_stages,0) is_stages,og.ru_id, o.order_id, o.main_order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid, o.is_delete," .
            ...
            " FROM " . $GLOBALS['sanmu']->table('order_info') . " AS o " .
            " LEFT JOIN " . $GLOBALS['sanmu']->table('users') . " AS u ON u.user_id=o.user_id " .
            " LEFT JOIN " . $GLOBALS['sanmu']->table('order_goods') . " AS og ON o.order_id=og.order_id " .
            " LEFT JOIN " . $GLOBALS['sanmu']->table('baitiao_log') . " AS bai ON o.order_id=bai.order_id " .
    ...
    foreach ($row as $key => $value) {
        ...
        $sql = "SELECT concat(IFNULL(c.region_name, ''), '  ', IFNULL(p.region_name, ''), " .
            ...
            "FROM " . $GLOBALS['sanmu']->table('order_info') . " AS o " .
            "LEFT JOIN " . $GLOBALS['sanmu']->table('region') . " AS c ON o.country = c.region_id " .
            "LEFT JOIN " . $GLOBALS['sanmu']->table('region') . " AS p ON o.province = p.region_id " .
            "LEFT JOIN " . $GLOBALS['sanmu']->table('region') . " AS t ON o.city = t.region_id " .
            "LEFT JOIN " . $GLOBALS['sanmu']->table('region') . " AS d ON o.district = d.region_id " .
...
{
    $sql = "SELECT * FROM " . $GLOBALS['sanmu']->table('brand') . " WHERE brand_id = '$brand_id' LIMIT 1";
...
{
    $sql = "SELECT count(*) FROM " . $GLOBALS['sanmu']->table('favourable_activity') . " WHERE user_id = '$ru_id'";
...
function get_favourable_dateout_count($ru_id)
{
    ...
    $time = gmtime();
    $sql = "SELECT count(*) FROM " . $GLOBALS['sanmu']->table('favourable_activity') . " WHERE user_id = '$ru_id' AND (end_time - '$time') < '$firstSecToday' AND (end_time - '$time') > 0";
    return $GLOBALS['db']->getOne($sql);
}
...
{
    $where = "(SELECT count(*) FROM " . $GLOBALS['sanmu']->table('comment') . " AS c2 WHERE c2.parent_id = c1.comment_id LIMIT 1) < 1";
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['sanmu']->table('comment') . " AS c1 WHERE c1.comment_type = 0 AND c1.parent_id = 0 AND c1.ru_id = '$ru_id' AND c1.order_id > 0 AND $where";
...
function get_goods_special_count($ru_id, $type = '')
{
    ...
    $sql = "SELECT count(*) FROM " . $GLOBALS['sanmu']->table('goods') . " WHERE user_id = '$ru_id' $where AND is_delete = 0";
...
function set_goods_attribute($goods_type = 0, $goods_id = 0, $goods_model = 0)
{
    ...
    $sql = " SELECT a.attr_id, a.attr_name, a.attr_input_type, a.attr_type, a.attr_values " .
        " FROM " . $GLOBALS['sanmu']->table('attribute') . " AS a " .
    ...
    $sql = " SELECT v.attr_id, v.attr_value, v.attr_price, v.attr_sort, v.attr_checked " .
        " FROM " . $GLOBALS['sanmu']->table('goods_attr') . " AS v " .
        ...
        foreach ($attribute_list as $key => $val) {
            ...
            if ($val['attr_type'] > 0) {
                if ($val['attr_values']) {
                    ...
                    $sql = "SELECT attr_value FROM " . $GLOBALS['sanmu']->table('goods_attr') . " WHERE goods_id = '$goods_id' AND attr_id = '" . $val['attr_id'] . "' ORDER BY attr_sort, goods_attr_id";
                ...
                if ($attr_values) {
                    for ($i = 0; $i < count($attr_values); $i++) {
                        $goods_attr = $GLOBALS['db']->getRow("SELECT goods_attr_id, attr_price, attr_sort FROM " . $GLOBALS['sanmu']->table('goods_attr') . " WHERE goods_id = '$goods_id' AND attr_value = '" . $attr_values[$i] . "' AND attr_id = '" . $val['attr_id'] . "' LIMIT 1");
            ...
            if ($val['attr_type'] == 0) {
                $goods_attr = $GLOBALS['db']->getRow("SELECT goods_attr_id FROM " . $GLOBALS['sanmu']->table('goods_attr') . " WHERE goods_id = '$goods_id' AND attr_value = '$this_value' AND attr_id = '" . $val['attr_id'] . "' LIMIT 1");
...
function insert_attr_changelog($goods_id = 0, $attr_info = [], $goods_model = 0, $warehouse_id = 0, $region_id = 0, $city_id = 0)
{
    if (!empty($attr_info)) {
        ...
        $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table('products_changelog'), $changelog, 'INSERT');
...
function get_product_info_by_attr($goods_id = 0, $attr_arr = [], $goods_model = 0, $region_id = 0, $changelog = 0, $city_id = 0)
{
    if (!empty($attr_arr)) {
        ...
        $sql = " SELECT * FROM " . $GLOBALS['sanmu']->table($table) . " WHERE 1 AND goods_id = '$goods_id' " . $where . " ORDER BY product_id DESC LIMIT 1 ";
...
{
    $sql = "SELECT region_id, region_name FROM " . $GLOBALS['sanmu']->table('region_warehouse') . " WHERE 1 AND region_type = 0";
    $warehouse_list = $GLOBALS['db']->getAll($sql);

    foreach ($warehouse_list as $key => $val) {
        $sql = "SELECT region_id, region_name FROM " . $GLOBALS['sanmu']->table('region_warehouse') . " WHERE parent_id = '$val[region_id]'";
...
function get_goods_payfull($is_fullcut = 0, $full, $reduce, $id, $goods_id, $table, $type = 0)
{
    if ($is_fullcut) {
        if (count($reduce) > 0) {
            for ($i = 0; $i < count($reduce); $i++) {
                if (!empty($full[$i])) {
                    ...
                    if (!empty($id[$i])) {
                        $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table($table), $other, 'UPDATE', "id='" . $id[$i] . "'");
                    } else {
                        $other['goods_id'] = $goods_id;
                        $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table($table), $other, 'INSERT');
                    }
                    //添加或修改 end
                }
            }
        ...
        $sql = "DELETE FROM " . $GLOBALS['sanmu']->table($table) . " WHERE goods_id = '$goods_id'";
        $GLOBALS['db']->query($sql);
    }
}
...
function get_user_menu_list()
{
    ...
    if (isset($adminru['ru_id']) && $adminru['ru_id'] > 0) {
        $sql = " SELECT user_menu FROM " . $GLOBALS['sanmu']->table('seller_shopinfo') . " WHERE ru_id = '" . $adminru['ru_id'] . "' ";
...
function delete_invalid_goods_attr($attr_group = [], $goods_id = 0, $goods_model = 0, $region_id = 0, $city_id = 0)
{
    ...
    if (!empty($attr_group)) {
        ...
        //获取商品的所有零时数据
        $sql = " SELECT product_id,goods_attr FROM " . $GLOBALS['sanmu']->table('products_changelog') . " WHERE 1 AND goods_id = '$goods_id' " . $where;
        ...
        if (!empty($product_info)) {
            foreach ($product_info as $k => $v) {
                ...
                if (count($goods_attr) != $count_attr) {
                    $sql = "DELETE FROM" . $GLOBALS['sanmu']->table('products_changelog') . "WHERE product_id = '" . $v['product_id'] . "'";
                    $GLOBALS['db']->query($sql);
                }
            }
        }
