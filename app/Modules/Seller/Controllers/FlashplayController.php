    ...
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            if ($GLOBALS['_CFG']['index_ad'] == 'cus') {
                return sanmu_header("Location: flashplay.php?act=custom_list\n");
            }
            ...
            return sanmu_header("Location: flashplay.php?act=list\n");
            ...
            if ($GLOBALS['_CFG']['flash_theme'] != $flash_theme) {
                $sql = "UPDATE " . $this->sanmu->table('shop_config') . " SET value = '$flash_theme' WHERE code = 'flash_theme'";
...
//        }
        ...
        elseif ($_REQUEST['act'] == 'custom_insert') {
            ...
            $this->db->autoExecute($this->sanmu->table('ad_custom'), $ad, 'INSERT', '', 'SILENT');
        ...
        elseif ($_REQUEST['act'] == 'custom_del') {
            ...
            $query = $this->db->query("DELETE FROM " . $this->sanmu->table('ad_custom') . " WHERE ad_id = $id");
        ...
        elseif ($_REQUEST['act'] == 'custom_status') {
            ...
            if ($this->modfiy_ad_status($id, $ad_status)) {
                ...
                /* 标签初始化 */
                $sql = "SELECT  value FROM " . $this->sanmu->table("shop_config") . " WHERE id =337";
        ...
        elseif ($_REQUEST['act'] == 'custom_edit') {
            ...
            /* 查询自定义广告信息 */
            $sql = "SELECT ad_id, ad_type, content, url, ad_status, ad_name FROM " . $this->sanmu->table("ad_custom") . " WHERE ad_id = $id LIMIT 0, 1";
        ...
        elseif ($_REQUEST['act'] == 'custom_update') {
            ...
            /* 查询自定义广告信息 */
            $sql = "SELECT ad_id, ad_type, content, url, ad_status, ad_name FROM " . $this->sanmu->table("ad_custom") . " WHERE ad_id = " . $filter['ad']['id'] . " LIMIT 0, 1";
            ...
            $this->db->autoExecute($this->sanmu->table('ad_custom'), $ad, 'UPDATE', 'ad_id = ' . $ad_info['ad_id'], 'SILENT');
    ...
    private function ad_list()
    {
        ...
        if ($result === false) {
            ...
            $sql = "SELECT ad_id, CASE WHEN ad_type = 0 THEN '" . lang('seller/common.imgage') . "'
                ...
                FROM " . $this->sanmu->table("ad_custom") . "
                $where
    ...
    private function modfiy_ad_status($ad_id, $ad_status = 0)
    {
        ...
        /* 查询自定义广告信息 */
        $sql = "SELECT ad_type, content, url, ad_status FROM " . $this->sanmu->table("ad_custom") . " WHERE ad_id = $ad_id LIMIT 0, 1";
        $ad = $this->db->getRow($sql);

        if ($ad_status == 1) {
            /* 如果当前自定义广告是关闭状态 则修改其状态为启用 */
            if ($ad['ad_status'] == 0) {
                $sql = "UPDATE " . $this->sanmu->table("ad_custom") . " SET ad_status = 1 WHERE ad_id = $ad_id";
                $this->db->query($sql);
            }

            /* 关闭 其它自定义广告 */
            $sql = "UPDATE " . $this->sanmu->table("ad_custom") . " SET ad_status = 0 WHERE ad_id <> $ad_id";
            $this->db->query($sql);

            /* 用户自定义广告开启 */
            $sql = "UPDATE " . $this->sanmu->table("shop_config") . " SET value = 'cus' WHERE id =337";
            $this->db->query($sql);
        } else {
            /* 如果当前自定义广告是关闭状态 则检查是否存在启用的自定义广告 */
            /* 如果无 则启用系统默认广告播放器 */
            if ($ad['ad_status'] == 0) {
                $sql = "SELECT COUNT(ad_id) FROM " . $this->sanmu->table("ad_custom") . " WHERE ad_status = 1";
                $ad_status_1 = $this->db->getOne($sql);
                if (empty($ad_status_1)) {
                    $sql = "UPDATE " . $this->sanmu->table("shop_config") . " SET value = 'sys' WHERE id =337";
                    $this->db->query($sql);
                } else {
                    $sql = "UPDATE " . $this->sanmu->table("shop_config") . " SET value = 'cus' WHERE id =337";
                    $this->db->query($sql);
