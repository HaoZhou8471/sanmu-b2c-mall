...
class GoodsInventoryLogsController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'batch_drop') {
            ...
            foreach ($_POST['checkboxes'] as $key => $id) {
                $sql = "DELETE FROM " . $this->sanmu->table('goods_inventory_logs') . " WHERE id = '$id'";
    ...
    private function get_goods_inventory_logs($ru_id)
    {
        ...
        if ($result === false) {
            ...
            /* 获得总记录数据 */
            $sql = 'SELECT COUNT(*) FROM ' . $this->sanmu->table('goods_inventory_logs') . " as gil " .
                " LEFT JOIN " . $this->sanmu->table('goods') . " as g ON gil.goods_id = g.goods_id" .
                " LEFT JOIN " . $this->sanmu->table('order_info') . " as oi ON gil.order_id = oi.order_id " .
                $where;
            ...
            /* 获取管理员日志记录 */
            $list = [];
            $sql = 'SELECT gil.*, g.user_id,g.goods_id,g.goods_thumb,g.brand_id, g.goods_name, oi.order_sn, au.user_name AS admin_name, og.goods_attr FROM ' . $this->sanmu->table('goods_inventory_logs') . " as gil " .
                " LEFT JOIN " . $this->sanmu->table('goods') . " as g ON gil.goods_id = g.goods_id" .
                " LEFT JOIN " . $this->sanmu->table('order_info') . " as oi ON gil.order_id = oi.order_id " .
                " LEFT JOIN " . $this->sanmu->table('order_goods') . " as og ON gil.goods_id = og.goods_id AND gil.order_id = og.order_id " .
                " LEFT JOIN " . $this->sanmu->table('admin_user') . " as au ON gil.admin_id = au.user_id " .
        ...
        foreach ($res as $rows) {
            ...
            if ($rows['brand_id'] > 0) {
                $rows['brand_name'] = $this->db->getOne("SELECT brand_name  FROM" . $this->sanmu->table("brand") . " WHERE brand_id = '" . $rows['brand_id'] . "'");
            }
            ...
            if ($rows['use_storage'] == 9) {
                ...
                $sql = "SELECT goods_attr FROM " . $this->sanmu->table($table) . " WHERE product_id = '" . $rows['product_id'] . "' LIMIT 1";
            ...
            if (!empty($rows['url']) && (strpos($rows['url'], 'http://') === false && strpos($rows['url'], 'https://') === false && strpos($rows['url'], 'errorImg.png') === false)) {
                $rows['url'] = $GLOBALS['sanmu']->seller_url() . $rows['url'];
            }
