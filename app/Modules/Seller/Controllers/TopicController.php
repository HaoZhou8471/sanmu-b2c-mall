...
class TopicController extends InitController
{
    ...
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit') {
            ...
            if (!$isadd) {
                $sql = "SELECT * FROM " . $this->sanmu->table('topic') . " WHERE topic_id = '$topic_id' LIMIT 1";
        ...
        elseif ($_REQUEST['act'] == 'visual') {
            ...
            $sql = "SELECT user_id FROM " . $this->sanmu->table('topic') . " WHERE topic_id = '$topic_id' LIMIT 1";
            $topic = $this->db->getRow($sql);
            ...
            //删除图片
            $this->sanmuRepository->getDelBatch($_POST['checkboxes'], intval($_GET['id']), ['topic_img', 'title_pic'], 'topic_id', Topic::whereRaw(1), 1);

            $sql = "DELETE FROM " . $this->sanmu->table('topic') . " WHERE ";
            if (!empty($_POST['checkboxes'])) {
                $is_use = 0;
                foreach ($_POST['checkboxes'] as $v) {
                    $sql_v = "SELECT * FROM " . $this->sanmu->table('topic') . " WHERE topic_id = '$v' LIMIT 1";
                ...
                $sql_v = "SELECT * FROM " . $this->sanmu->table('topic') . " WHERE topic_id = '" . $_GET['id'] . "' LIMIT 1";
                $topic = $this->db->getRow($sql_v);
                if ($topic['user_id'] != $adminru['ru_id']) {
                }
    ...
    private function get_topic_list()
    {
        ...
        foreach ($query as $topic) {
            $topic['start_time'] = local_date('Y-m-d H:i:s', $topic['start_time']);
            $topic['end_time'] = local_date('Y-m-d H:i:s', $topic['end_time']);
            $topic['url'] = $this->sanmu->seller_url() . 'topic.php?topic_id=' . $topic['topic_id'];
