...
class MessageController extends InitController
{
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'send') {
            ...
            /* 获取管理员列表 */
            $admin_list = $this->db->getAll('SELECT user_id, user_name FROM ' . $this->sanmu->table('admin_user') . " WHERE parent_id > 0 AND ru_id = '" . $adminru['ru_id'] . "' OR action_list = 'all'");
        ...
        elseif ($_REQUEST['act'] == 'insert') {
            ...
            if ($rec_arr[0] == 0) {
                /* 获取管理员信息 */
                $result = $this->db->query('SELECT user_id FROM ' . $this->sanmu->table('admin_user') . 'WHERE user_id !=' . intval(session('seller_id')));
                foreach ($result as $rows) {
                    $sql = "INSERT INTO " . $this->sanmu->table('admin_message') . " (sender_id, receiver_id, sent_time, " .
                ...
                foreach ($rec_arr as $key => $id) {
                    $sql = "INSERT INTO " . $this->sanmu->table('admin_message') . " (sender_id, receiver_id, " .
        ...
        elseif ($_REQUEST['act'] == 'edit') {
            ...
            /* 获取管理员列表 */
            $admin_list = $this->db->getAll('SELECT user_id, user_name FROM ' . $this->sanmu->table('admin_user'));

            /* 获得留言数据*/
            $sql = 'SELECT message_id, receiver_id, title, message' .
                'FROM ' . $this->sanmu->table('admin_message') . " WHERE message_id='$id'";
            ...
            /* 获得留言数据*/
            $msg_arr = [];
            $msg_arr = $this->db->getRow('SELECT * FROM ' . $this->sanmu->table('admin_message') . " WHERE message_id='$_POST[id]'");

            $sql = "UPDATE " . $this->sanmu->table('admin_message') . " SET " .
                "title = '$_POST[title]'," .
        ...
        elseif ($_REQUEST['act'] == 'view') {
            ...
            $sql = "SELECT a.*, b.user_name " .
                "FROM " . $this->sanmu->table('admin_message') . " AS a " .
                "LEFT JOIN " . $this->sanmu->table('admin_user') . " AS b ON b.user_id = a.sender_id " .
        ...
        elseif ($_REQUEST['act'] == 'remove') {
            ...
            $sql = "UPDATE " . $this->sanmu->table('admin_message') . " SET deleted=1 " .
                " WHERE message_id=$id AND (sender_id='" . session('seller_id') . "' OR receiver_id='" . session('seller_id') . "')";
            $this->db->query($sql);
    ...
    private function get_message_list()
    {
        ...
        $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('admin_message') . " AS a WHERE 1 AND " . $where;
        ...
        $sql = "SELECT a.message_id,a.sender_id,a.receiver_id,a.sent_time,a.read_time,a.deleted,a.title,a.message,b.user_name" .
            " FROM " . $this->sanmu->table('admin_message') . " AS a," . $this->sanmu->table('admin_user') . " AS b " .
