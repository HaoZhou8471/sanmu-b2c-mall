...
class PackageController extends InitController
{
    ...
    {
        $exc = new Exchange($this->sanmu->table("goods_activity"), $this->db, 'act_id', 'act_name');
        $menus = session('menus', '');
        ...
        elseif ($_REQUEST['act'] == 'add') {
            ...
            /* 组合商品 */
            $group_goods_list = [];
            $sql = "DELETE FROM " . $this->sanmu->table('package_goods') .
            ...
            $sql = "SELECT COUNT(*) " .
                " FROM " . $this->sanmu->table('goods_activity') .
            ...
            $this->db->AutoExecute($this->sanmu->table('goods_activity'), $record, 'INSERT');
        ...
        elseif ($_REQUEST['act'] == 'edit') {
            ...
            $this->db->autoExecute($this->sanmu->table('goods_activity'), $record, 'UPDATE', "act_id = '$act_id' AND act_type = " . GAT_PACKAGE);
        ...
        elseif ($_REQUEST['act'] == 'drop_thumb') {
            ...
            /* 取得logo名称 */
            $sql = "SELECT activity_thumb FROM " . $this->sanmu->table('goods_activity') . " WHERE act_id = '$act_id'";
            $activity_thumb = $this->db->getOne($sql);

            if (!empty($activity_thumb)) {
                $this->sanmuRepository->getDelBatch('', $act_id, ['activity_thumb'], 'act_id', GoodsActivity::whereRaw(1), 1); //删除图片

                $sql = "UPDATE " . $this->sanmu->table('goods_activity') . " SET activity_thumb = '' WHERE act_id = '$act_id'";
                $this->db->query($sql);
            }
        ...
        elseif ($_REQUEST['act'] == 'remove') {
            ...
            $this->sanmuRepository->getDelBatch('', $id, ['activity_thumb'], 'act_id', GoodsActivity::whereRaw(1), 1); //删除图片
            ...
            if ($goods_ids) {
                ...
                if (!empty($goods_ids_arr)) {
                    foreach ($goods_ids_arr as $v) {
                        ...
                        $sql = "SELECT COUNT(*) FROM" . $this->sanmu->table('package_goods') . "WHERE goods_id = '" . $v . "' AND package_id = '$package_id' $where LIMIT 1";
                        $goods_count = $this->db->getOne($sql);

                        if ($goods_count == 0) {
                            $sql = "INSERT INTO " . $this->sanmu->table('package_goods') . " (package_id, goods_id, goods_number, admin_id) " .
        ...
        elseif ($_REQUEST['act'] == 'edit_package_nuber') {
            ...
            if ($num > 0) {
                $sql = "UPDATE" . $this->sanmu->table('package_goods') .
                    " SET goods_number = '$num' " . $where;

                $this->db->query($sql);
            } else {
                $sql = "SELECT goods_number FROM" . $this->sanmu->table('package_goods') . $where;
        ...
        elseif ($_REQUEST['act'] == 'edit_package_product') {
            ...
            if ($product_id > 0) {
                //判断改属性商品是否存在
                $sql = "SELECT COUNT(*) FROM" . $this->sanmu->table('package_goods') . $where . " AND product_id = '$product_id'";
    ...
    private function get_packagelist($ru_id)
    {
        ...
        if ($result === false) {
            ...
            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('goods_activity') . " AS ga " .
            ...
            $sql = "SELECT ga.act_id, ga.act_name AS package_name, ga.start_time, ga.end_time, ga.is_finished, ga.ext_info, ga.user_id, ga.goods_id, ga.review_status, ga.review_content " .
                " FROM " . $this->sanmu->table('goods_activity') . " AS ga " .
                " WHERE ga.act_type = " . GAT_PACKAGE . $where .
    ...
    {
        $sql = "UPDATE " . $this->sanmu->table('package_goods') . " SET " .
