...
class UserMsgController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'view') {
            ...
            if (empty($_REQUEST['parent_id'])) {
                ...
                $sql = "UPDATE " . $this->sanmu->table('feedback') . " SET user_email = '" . $_POST['user_email'] . "', msg_content='" . $_POST['msg_content'] . "', msg_time = '" . gmtime() . "' WHERE msg_id = '" . $_REQUEST['parent_id'] . "'";
                $this->db->query($sql);
            }
            ...
            return sanmu_header("Location: ?act=view&id=" . $_REQUEST['msg_id'] . "&send_ok=$send_ok\n");
        }
        ...
        elseif ($_REQUEST['act'] == 'drop_file') {
            ...
            /* 更新数据库 */
            $this->db->query("UPDATE " . $this->sanmu->table('feedback') . " SET message_img = '' WHERE msg_id = '$_GET[id]'");

            return sanmu_header("Location: user_msg.php?act=view&amp;id=" . $_GET['id'] . "\n");
        }
    }
    ...
    private function get_feedback_detail($id)
    {
        $sql = "SELECT T1.*, T2.msg_id AS reply_id, T2.user_name  AS reply_name, u.email AS reply_email, " .
            ...
            "LEFT JOIN " . $this->sanmu->table('admin_user') . " AS u ON u.user_id='" . session('seller_id') . "' " .
