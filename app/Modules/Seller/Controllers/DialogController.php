...
class DialogController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'insert_attr_img') {
            ...
            if ($other['attr_checked'] == 1) {
                $this->db->autoExecute($this->sanmu->table('goods_attr'), ['attr_checked' => 0], 'UPDATE', 'attr_id = ' . $attr_id . ' and goods_id = ' . $goods_id);
                $result['is_checked'] = 1;
            }
            ...
            if ($other['attr_checked'] == 1) {
                if ($GLOBALS['_CFG']['add_shop_price'] == 0 && $goods['model_attr'] == 0) {
                    ...
                    if (!empty($goodsAttrId)) {
                        ...
                        if ($products) {
                            ...
                            $this->db->autoExecute($this->sanmu->table('goods'), $other, 'UPDATE', "goods_id = '$goods_id'");
                        }
                    }
                }
            } else {
                if ($goods['model_attr'] > 0) {
                    ...
                    $this->db->autoExecute($this->sanmu->table('goods'), $goods_other, 'UPDATE', "goods_id = '$goods_id'");
                }
            }
        ...
        elseif ($_REQUEST['act'] == 'choose_attrImg') {
            ...
            /* 删除数据 */
            $sql = "SELECT img_id, thumb_url, img_url FROM " . $this->sanmu->table('goods_gallery') . " WHERE $where";
        ...
        elseif ($_REQUEST['act'] == 'add_goods_model_price') {
            ...
            if ($warehouse_list) {
                ...
                $sql = "SELECT region_id FROM " . $this->sanmu->table('region_warehouse') . " WHERE parent_id = '" . $warehouse_list[0]['region_id'] . "'";
        ...
        elseif ($_REQUEST['act'] == 'del_goods_attr') {
            ...
            //删除相关货品
            $sql = "SELECT product_id,goods_attr FROM" . $this->sanmu->table($table) . "WHERE goods_id = '$goods_id' $attr_where";
                ...
                foreach ($products as $k => $v) {
                    if ($v['goods_attr']) {
                        ...
                        if (in_array($goods_attr_id, $goods_attr)) {
                            $sql = "DELETE FROM" . $this->sanmu->table("products") . "WHERE product_id = '" . $v['product_id'] . "' AND goods_id = '$goods_id'";
                            $this->db->query($sql);
                        }
                    }
                }
            ...
            $admin_id = get_admin_id();
            //删除零时货品表
            $sql = "SELECT product_id,goods_attr FROM" . $this->sanmu->table('products_changelog') . "WHERE goods_id = '$goods_id' AND admin_id = '$admin_id' $attr_where";
            ...
            if (!empty($products_changelog)) {
                foreach ($products_changelog as $k => $v) {
                    if ($v['goods_attr']) {
                        $goods_attr = explode('|', $v['goods_attr']);
                        if (in_array($goods_attr_id, $goods_attr)) {
                            $sql = "DELETE FROM" . $this->sanmu->table("products_changelog") . "WHERE product_id = '" . $v['product_id'] . "' AND goods_id = '$goods_id'";
                            $this->db->query($sql);
                        }
                    }
                }
            }
            $sql = "DELETE FROM " . $this->sanmu->table("goods_attr") . " WHERE $where";
            $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'add_sku') {
            ...
            if ($warehouse_list) {
                ...
                $sql = "SELECT region_id FROM " . $this->sanmu->table('region_warehouse') . " WHERE parent_id = '$area_id'";
            ...
            foreach ($attr_value_list as $key => $attr_value) {
                if ($attr_value) {
                    ...
                    if ($goods_attr_id[$key]) {
                        $sql = "UPDATE " . $this->sanmu->table('goods_attr') . " SET attr_value = '$attr_value' WHERE goods_attr_id = '" . $goods_attr_id[$key] . "' LIMIT 1";
                        $this->db->query($sql);
                    } else {
                        $sql = "SELECT MAX(attr_sort) AS attr_sort FROM " . $this->sanmu->table('goods_attr') . " WHERE attr_id = '$attr_id'" . $where;
                        ...
                        $sql = "SELECT goods_attr_id FROM " . $this->sanmu->table('goods_attr') . " WHERE attr_value = '$attr_value' AND attr_id = '$attr_id' AND goods_id = '$goods_id'";
                        if (!$this->db->getOne($sql, true)) {
                            $sql = "INSERT INTO " . $this->sanmu->table('goods_attr') . " (attr_id, goods_id, attr_value, attr_sort, admin_id)" .
        ...
        elseif ($_REQUEST['act'] == 'del_input_type') {
            ...
            $sql = "DELETE FROM " . $this->sanmu->table('goods_attr') . " WHERE goods_attr_id = '$goods_attr_id'";
            $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'del_volume') {
            ...
            $sql = "DELETE FROM " . $this->sanmu->table('volume_price') . " WHERE id = '$volume_id'";
            $this->db->query($sql);

            $volume_price_list = $this->goodsCommonService->getVolumePriceList($goods_id);
            if (!$volume_price_list) {
                $sql = "UPDATE " . $this->sanmu->table('goods') . " SET is_volume = 0 WHERE goods_id = '$goods_id'";
                $this->db->query($sql);
            }
        ...
        elseif ($_REQUEST['act'] == 'del_wholesale_volume') {
            ...
            $sql = "DELETE FROM " . $this->sanmu->table('wholesale_volume_price') . " WHERE id = '$volume_id'";
            $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'del_cfull') {
            ...
            $sql = "DELETE FROM " . $this->sanmu->table('goods_consumption') . " WHERE id = '$volume_id'";
            $this->db->query($sql);

            $consumption_list = $this->cartCommonService->getGoodsConList($goods_id, 'goods_consumption'); //满减订单金额
            if (!$consumption_list) {
                $sql = "UPDATE " . $this->sanmu->table('goods') . " SET is_fullcut = 0 WHERE goods_id = '$goods_id'";
                $this->db->query($sql);
            }
        ...
        elseif ($_REQUEST['act'] == 'insert_external_url') {
            ...
            //当前域名协议
            $http = $this->sanmu->http();

            if ($external_url_list) {
                $sql = "SELECT MAX(img_desc) FROM " . $this->sanmu->table('goods_gallery') . " WHERE goods_id = '$goods_id'";
                ...
                foreach ($external_url_list as $key => $image_urls) {
                    if ($image_urls) {
                        ...
                        $this->sanmuRepository->getOssAddFile([$img_url, $thumb_url, $img_original]);
                    }
                }
            ...
            $sql = "SELECT * FROM " . $this->sanmu->table('goods_gallery') . " WHERE goods_id = '$goods_id' $where  ORDER BY img_desc";
            $img_list = $this->db->getAll($sql);
        ...
        elseif ($_REQUEST['act'] == 'insert_gallery_url') {
            ...
            $sql = "SELECT img_id FROM " . $this->sanmu->table('goods_gallery') . " WHERE external_url = '$external_url' AND goods_id = '$goods_id' AND img_id <> $img_id";
            if ($this->db->getOne($sql, true) && !empty($external_url)) {
                $result['error'] = 1;
            } else {
                $sql = "UPDATE " . $this->sanmu->table('goods_gallery') . " SET external_url = '$external_url'" . " WHERE img_id = '$img_id'";
                $this->db->query($sql);
            }
            ...
            if (!empty($external_url)) {
                ...
                $sql = "SELECT thumb_url FROM " . $this->sanmu->table('goods_gallery') . " WHERE img_id = '$img_id'";
        ...
        elseif ($_REQUEST['act'] == 'set_free_shipping') {
            ...
            $sql = "SELECT ra_id, ra_name " .
                " FROM " . $this->sanmu->table('merchants_region_area');
        ...
        elseif ($_REQUEST['act'] == 'add_goods_type_cat') {
            ...
            if ($type == 'add_goods_type_cat' || $type == 'add_goods_type') {
                ...
                $this->sanmuRepository->helpersLang('attribute', 'seller');
        ...
        elseif ($_REQUEST['act'] == 'ajaxTransport') {
            ...
            $this->sanmuRepository->helpersLang('goods_transport', 'seller');
            ...
            if (!$tid) {
                ...
                $sql = "DELETE FROM" . $this->sanmu->table("goods_transport_tpl") . "WHERE tid = 0 AND admin_id = '$admin_id'";
                $this->db->query($sql);
            } else {
        ...
        elseif ($_REQUEST['act'] == 'transport_insert' || $_REQUEST['act'] == 'transport_update') {
            ...
            if ($_REQUEST['act'] == 'transport_update') {
                ...
                $this->db->autoExecute($this->sanmu->table('goods_transport'), $data, "UPDATE", "tid = '$data[tid]'");
                $tid = $s_tid;

                $where = " tid = '$tid'";
            } else {
                $result['message'] = $GLOBALS['_LANG']['add_express_tpl_success'];
                $this->db->autoExecute($this->sanmu->table('goods_transport'), $data, 'INSERT');
                $tid = $this->db->insert_id();
                $this->db->autoExecute($this->sanmu->table('goods_transport_extend'), ['tid' => $tid], "UPDATE", "tid = '0' AND admin_id = '$admin_id' ");
                $this->db->autoExecute($this->sanmu->table('goods_transport_express'), ['tid' => $tid], "UPDATE", "tid = '0' AND admin_id = '$admin_id' ");
            ...
            if ($data['freight_type'] > 0) {
                ...
                if (!session()->has($s_tid . '.tpl_id') && empty(session()->get($s_tid . '.tpl_id'))) {
                    $sql = "SELECT GROUP_CONCAT(id) AS id FROM " . $GLOBALS['sanmu']->table('goods_transport_tpl') . " WHERE " . $where;
                ...
                if (!empty($tpl_id)) {
                    $sql = "UPDATE" . $this->sanmu->table("goods_transport_tpl") . " SET tid = '$tid' WHERE admin_id = '$admin_id' AND tid = 0 AND id " . db_create_in($tpl_id);
                    $this->db->query($sql);
            ...
            if (isset($_REQUEST['sprice']) && count($_REQUEST['sprice']) > 0) {
                foreach ($_REQUEST['sprice'] as $key => $val) {
                    $info = [];
                    $info['sprice'] = $val;
                    $this->db->autoExecute($this->sanmu->table('goods_transport_extend'), $info, "UPDATE", "id = '$key'");
                }
            }
            ...
            if (isset($_REQUEST['shipping_fee']) && count($_REQUEST['shipping_fee']) > 0) {
                foreach ($_REQUEST['shipping_fee'] as $key => $val) {
                    ...
                    $this->db->autoExecute($this->sanmu->table('goods_transport_express'), $info, "UPDATE", "id = '$key'");
                }
            }
        ...
        elseif ($_REQUEST['act'] == 'ajaxCate') {
            ...
            $this->sanmuRepository->helpersLang('category', 'seller');
            ...
            $sql = "SELECT a.attr_id, a.cat_id, a.attr_name " .
                ...
                $GLOBALS['sanmu']->table('goods_type') . " AS c " .
        ...
        elseif ($_REQUEST['act'] == 'cate_insert') {
            ...
            if (!empty($_FILES['touch_icon']['name'])) {
                ...
                $this->sanmuRepository->getOssAddFile([$cat['touch_icon']]); //oss存储图片
                // 删除文件
                if (!empty($cat_id)) {
            ...
            if (count($cat_name) > 1) {
                ...
                if ($this->db->autoExecute($this->sanmu->table('merchants_category'), $cat) !== false) {
                    $cat_id = $this->db->insert_id();
                    if ($cat['show_in_nav'] == 1) {
                        $vieworder = $this->db->getOne("SELECT max(vieworder) FROM " . $this->sanmu->table('merchants_nav') . " WHERE type = 'middle'");
                        $vieworder += 2;
                        // 显示在自定义导航栏中
                        $sql = "INSERT INTO " . $this->sanmu->table('merchants_nav') .
                            " (name,ctype,cid,ifshow,vieworder,opennew,url,type,ru_id)" .
                            " VALUES('" . $cat['cat_name'] . "', 'c', '" . $this->db->insert_id() . "','1','$vieworder','0', '" . $this->sanmuRepository->buildUri('merchants_store', ['cid' => $cat_id, 'urid' => $adminru['ru_id']], $cat['cat_name']) . "','middle','" . $adminru['ru_id'] . "')";
                        $this->db->query($sql);
                    }
    ...
    {
        $sql = "select r.region_id, r.region_name from " . $this->sanmu->table('merchants_region_info') . " as mri" .
            " left join " . $this->sanmu->table('region') . " as r on mri.region_id = r.region_id" .
    ...
    private function get_transport_area($tid = 0)
    {
        ...
        $sql = " SELECT * FROM " . $GLOBALS['sanmu']->table('goods_transport_extend') . " WHERE tid = '$tid' $where ORDER BY id DESC ";
        ...
        foreach ($transport_area as $key => $val) {
            if (!empty($val['top_area_id']) && !empty($val['area_id'])) {
                ...
                foreach ($top_area_arr as $k => $v) {
                    ...
                    $sql = " SELECT region_name FROM " . $GLOBALS['sanmu']->table('region') . " WHERE parent_id = '$v' AND region_id IN ($val[area_id]) ";
    ...
    private function get_transport_express($tid = 0)
    {
        ...
        $sql = " SELECT * FROM " . $GLOBALS['sanmu']->table('goods_transport_express') . " WHERE tid = '$tid' $where ORDER BY id DESC ";
    ...
    private function get_express_list($shipping_id = '')
    {
        ...
        if (!empty($shipping_id)) {
            $sql = " SELECT shipping_name FROM " . $GLOBALS['sanmu']->table('shipping') . " WHERE shipping_id IN ($shipping_id) ";
