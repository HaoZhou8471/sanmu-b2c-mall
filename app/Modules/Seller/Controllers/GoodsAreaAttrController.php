...
class GoodsAreaAttrController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'edit_product_sn') {
            ...
            /* 修改 */
            $sql = "UPDATE " . $this->sanmu->table('products_area') . " SET product_sn = '$product_sn' WHERE product_id = '$product_id'";
        ...
        elseif ($_REQUEST['act'] == 'edit_bar_code') {
            ...
            /* 修改 */
            $sql = "UPDATE " . $this->sanmu->table('products_area') . " SET bar_code = '$bar_code' WHERE product_id = '$product_id'";
            $result = $this->db->query($sql);
            if ($result) {
                clear_cache_files();
        ...
        elseif ($_REQUEST['act'] == 'edit_product_number') {
            ...
            /* 修改货品库存 */
            $sql = "UPDATE " . $this->sanmu->table('products_area') . " SET product_number = '$product_number' WHERE product_id = '$product_id'";
            $result = $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'product_remove') {
            ...
            /* 删除货品 */
            $sql = "DELETE FROM " . $this->sanmu->table('products_area') . " WHERE product_id = '$product_id'";
            $result = $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'product_add_execute') {
            ...
            foreach ($product['product_sn'] as $key => $value) {
                ...
                /* 插入货品表 */
                $sql = "INSERT INTO " . $this->sanmu->table('products_area') . " (goods_id, goods_attr, product_sn, bar_code, product_price, product_number, area_id)  VALUES ('" . $product['goods_id'] . "', '$goods_attr', '$value', '" . $product['bar_code'][$key] . "', '" . $product['product_price'][$key] . "', '" . $product['product_number'][$key] . "', '" . $product['area_id'] . "')";
    ...
    private function area_product_list()
    {
        ...
        foreach ($area_list as $k => $v) {
            $area_list[$k]['attr_typeNum'] = $this->db->getOne("SELECT COUNT(*) FROM " . $this->sanmu->table('products_area') . " WHERE area_id='" . $v['region_id'] . "' ");
        }
        /*  @author-bylu  end */
    ...
    private function product_area_list($goods_id, $conditions = '', $area_id)
    {
        ...
        if ($result === false) {
            ...
            /* 记录总数 */
            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('products_area') . " AS p WHERE goods_id = $goods_id $where";
            $filter['record_count'] = $this->db->getOne($sql);

            $sql = "SELECT product_id, goods_id, goods_attr, product_sn, product_price, product_number, bar_code
                FROM " . $this->sanmu->table('products_area') . " AS g
    ...
    private function check_product_area_sn_exist($product_sn, $product_id = 0, $ru_id = 0, $type = 0)
    {
        ...
        if (empty($product_id)) {
            if ($type == 1) {
                $sql = "SELECT p.product_id FROM " . $this->sanmu->table('products_area') . " AS p " . "
    ...
    private function product_warehouse_number_count($goods_id, $conditions = '', $area_id = 0)
    {
        ...
        $sql = "SELECT product_number
            FROM " . $this->sanmu->table('products_area') . "
    ...
    private function check_goods_attr_exist($goods_attr, $goods_id, $product_id = 0, $area_id = 0)
    {
        ...
        if (empty($product_id)) {
            $sql = "SELECT product_id FROM " . $this->sanmu->table('products_area') . "
