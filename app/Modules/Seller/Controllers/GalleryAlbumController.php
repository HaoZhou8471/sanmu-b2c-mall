...
class GalleryAlbumController extends InitController
{
    ...
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            if ($parent_id > 0) {
                $sql = "SELECT parent_album_id FROM" . $this->sanmu->table("gallery_album") . "WHERE album_id = '$parent_id'";
            ...
            if ($_REQUEST['act'] == 'insert') {
                ...
                $this->sanmuRepository->getOssAddFile([$file_url]);

                $time = gmtime();
                $sql = "INSERT INTO" . $this->sanmu->table("gallery_album") . "(`parent_album_id`,`album_mame`,`album_cover`,`album_desc`,`sort_order`,`add_time`,`ru_id`)"
                ...
                if ($old_url != '' && $old_url != $file_url && strpos($old_url, 'http: ') === false && strpos($old_url, 'https: ') === false) {
                    ...
                    $this->sanmuRepository->getOssDelFile($del_arr_img);
                }

                $sql = "UPDATE" . $this->sanmu->table('gallery_album') . " SET album_mame='$album_mame',album_cover='$file_url'"
        ...
        elseif ($_REQUEST['act'] == 'view') {
            $album_id = isset($_REQUEST['id']) ? intval($_REQUEST['id']) : 0;
            $sql = "SELECT album_mame FROM " . $this->sanmu->table("gallery_album") . " WHERE album_id = '$album_id'";
        ...
        elseif ($_REQUEST['act'] == 'remove') {
            ...
            //获取下级相册数量
            $sql = "SELECT COUNT(*) FROM" . $this->sanmu->table('gallery_album') . " WHERE parent_album_id = '$album_id'";
            ...
            if ($album_count > 0) {
                ...
                //删除图片数据库
                $sql = "DELETE FROM" . $this->sanmu->table('pic_album') . "WHERE album_id = " . $album_id;
                $this->db->query($sql);
                $exc->drop($album_id);
        ...
        elseif ($_REQUEST['act'] == 'pic_remove') {
            ...
            if ($pic_info['pic_file'] != '' && @strpos($pic_info['pic_file'], 'http://') === false && @strpos($pic_info['pic_file'], 'https://') === false) {
                sanmu_unlink(storage_public($pic_info['pic_file']));
            ...
            if ($pic_info['pic_thumb'] != '' && @strpos($pic_info['pic_thumb'], 'http://') === false && @strpos($pic_info['pic_thumb'], 'https://') === false) {
                sanmu_unlink(storage_public($pic_info['pic_thumb']));
            ...
            if ($pic_info['pic_image'] != '' && @strpos($pic_info['pic_image'], 'http://') === false && @strpos($pic_info['pic_image'], 'https://') === false) {
                sanmu_unlink(storage_public($pic_info['pic_image']));
                $arr_img[] = $pic_info['pic_image'];
            }

            $this->sanmuRepository->getOssDelFile($arr_img);
            ...
            if ($arr_img) {
                ...
                sanmu_unlink($arr_img);
            }

            $sql = "DELETE FROM" . $this->sanmu->table('pic_album') . " WHERE pic_id = '$id' ";
        ...
        elseif ($_REQUEST['act'] == 'upload_pic') {
            ...
            if ($extname == 'mp4') {
                ...
                if ($upload->uploadFile('file')) {
                    ...
                    $arr_img = [$video];
                    $this->sanmuRepository->getOssAddFile($arr_img);
                ...
                if ($pic_id) {
                    ...
                    if ($GLOBALS['_CFG']['open_oss'] == 1 && $bucket_info['is_delimg'] == 1) {
                        sanmu_unlink(storage_public($video));
                    }
                    /* 删除本地视频 start */
                }
            } else {
        ...
        elseif ($_REQUEST['act'] == 'goods_video') {
            ...
            if ($extname == 'mp4') {
                ...
                if ($upload->uploadFile('file')) {
                    ...
                    $this->sanmuRepository->getOssAddFile($goodsOther);

                    if ($goods_id) {
                        $sql = "SELECT goods_video FROM" . $GLOBALS['sanmu']->table('goods') . " WHERE goods_id = '$goods_id'";
                        $video_old = $GLOBALS['db']->getOne($sql, true);

                        sanmu_unlink($video_old);

                        $arr[] = $video_old;
                        $this->sanmuRepository->getOssDelFile($arr);

                        $sql = "UPDATE " . $GLOBALS['sanmu']->table('goods') . " SET goods_video = '$goods_video' WHERE goods_id = '$goods_id'";
        ...
        elseif ($_REQUEST['act'] == 'del_video') {
            ...
            if ($goods_id) {
                ...
                sanmu_unlink(storage_public($video_old));
                $arr[] = $video_old;
                $this->sanmuRepository->getOssDelFile($arr);

                $sql = "UPDATE " . $GLOBALS['sanmu']->table('goods') . " SET goods_video = '' WHERE goods_id = '$goods_id'";
                $GLOBALS['db']->query($sql);
            } else {
                get_del_goods_video();
            }
        ...
        elseif ($_REQUEST['act'] == 'batch') {
            ...
            if (!empty($checkboxes)) {
                if ($type == 'remove') {
                    ...
                    /* 删除活动 */
                    $sql = 'DELETE FROM' . $this->sanmu->table('pic_album') . " WHERE ru_id='" . $adminru['ru_id'] . "' AND  pic_id" . db_create_in($checkboxes);
                    ...
                    /* 转移相册 */
                    if ($album_id > 0) {
                        $sql = "UPDATE" . $this->sanmu->table('pic_album') . " SET album_id = '" . $album_id . "' WHERE ru_id='" . $adminru['ru_id'] . "' AND pic_id" . db_create_in($checkboxes);
                        if ($this->db->query($sql) == true) {
                            $link[] = ['text' => $GLOBALS['_LANG']['bank_list'], 'href' => 'gallery_album.php?act=view&id=' . $old_album_id];
        ...
        elseif ($_REQUEST['act'] == 'submit_pic') {
            ...
            $sql = "UPDATE" . $this->sanmu->table('pic_album') . " SET album_id = '" . $target_album_id . "' WHERE album_id" . db_create_in($cat_select) . " AND ru_id = '" . $adminru['ru_id'] . "'";
            $this->db->query($sql);

            $sql = "SELECT  parent_album_id FROM " . $this->sanmu->table('gallery_album') .
    ...
    private function getgallery_child($album_id = 0, $type = 0)
    {
        ...
        if ($album_id > 0) {
            ...
            $sql = "SELECT  album_id FROM " . $this->sanmu->table('gallery_album') .
        ...
        $child_arr = $this->sanmuRepository->delStrComma($child_arr);
        return $child_arr;
    }
    ...
    private function getGalleryAlbumList($ru_id = 0)
    {
        ...
        if ($result === false) {
            ...
            //管理员查询的权限 -- 店铺查询 end
            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('gallery_album') . " AS ga" . $where;
            $filter['record_count'] = $this->db->getOne($sql);
            $filter = page_and_size($filter);
            /* 获活动数据 */
            $sql = "SELECT album_id,ru_id,album_mame,album_cover,album_desc,sort_order FROM" . $this->sanmu->table('gallery_album') . " AS ga" . "  $where ORDER BY sort_order ASC LIMIT " . $filter['start'] . "," . $filter['page_size'];
        ...
        foreach ($row as $k => $v) {
            ...
            $row[$k]['gallery_count'] = $this->db->getOne("SELECT COUNT('pic_id') FROM" . $this->sanmu->table('pic_album') . " WHERE album_id = '" . $v['album_id'] . "'");
        }
