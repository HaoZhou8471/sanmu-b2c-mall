...
class GoodsController extends InitController
{
    ...
    public function index()
    {
        ...
        $exc = new Exchange($this->sanmu->table('goods'), $this->db, 'goods_id', 'goods_name');
        $exc_extend = new Exchange($this->sanmu->table('goods_extend'), $this->db, 'goods_id', 'extend_id');
        ...
        elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy') {
            ...
            if ($is_add) {
                ...
                if ($adminru['ru_id'] > 0) {
                    ...
                    if ($grade_rank['goods_sun'] != -1) {
                        /*获取商家商品总数*/
                        $sql = " SELECT COUNT(*) FROM" . $this->sanmu->table("goods") . " WHERE user_id = '" . $adminru['ru_id'] . "'";
                ...
                /* 关联商品 */
                $link_goods_list = [];
                $sql = "DELETE FROM " . $this->sanmu->table('link_goods') .
                ...
                /* 组合商品 */
                $group_goods_list = [];
                $sql = "DELETE FROM " . $this->sanmu->table('group_goods') .
                    " WHERE parent_id = 0 AND admin_id = '" . session('seller_id') . "'";
                $this->db->query($sql);

                /* 关联文章 */
                $goods_article_list = [];
                $sql = "DELETE FROM " . $this->sanmu->table('goods_article') .
                    " WHERE goods_id = 0 AND admin_id = '" . session('seller_id') . "'";
                $this->db->query($sql);

                /* 属性 */
                $sql = "DELETE FROM " . $this->sanmu->table('goods_attr') . " WHERE goods_id = 0 AND admin_id = '$admin_id'";
                $this->db->query($sql);
                ...
                //获取拓展分类id数组
                $other_cat_list1 = [];
                $sql = "SELECT ga.cat_id FROM " . $this->sanmu->table('goods_cat') . " as ga " .
                ...
                if ($_REQUEST['act'] == 'copy') {
                    ...
                    $sql = "SELECT '0' AS goods_id, link_goods_id, is_double, '" . session('seller_id') . "' AS admin_id" .
                        " FROM " . $this->sanmu->table('link_goods') .
                        " WHERE goods_id = '" . intval($_REQUEST['goods_id']) . "' ";
                    $res = $this->db->query($sql);
                    foreach ($res as $row) {
                        $this->db->autoExecute($this->sanmu->table('link_goods'), $row, 'INSERT');
                    }
                    ...
                    $sql = "SELECT 0 AS parent_id, goods_id, goods_price, '" . session('seller_id') . "' AS admin_id " .
                        "FROM " . $this->sanmu->table('group_goods') .
                        " WHERE parent_id = '" . intval($_REQUEST['goods_id']) . "' ";
                    $res = $this->db->query($sql);
                    foreach ($res as $row) {
                        $this->db->autoExecute($this->sanmu->table('group_goods'), $row, 'INSERT');
                    }
                    ...
                    $sql = "SELECT 0 AS goods_id, article_id, '" . session('seller_id') . "' AS admin_id " .
                        "FROM " . $this->sanmu->table('goods_article') .
                        " WHERE goods_id = '" . intval($_REQUEST['goods_id']) . "' ";
                    $res = $this->db->query($sql);
                    foreach ($res as $row) {
                        $this->db->autoExecute($this->sanmu->table('goods_article'), $row, 'INSERT');
                    }
                    ...
                    $sql = "SELECT 0 AS goods_id, attr_id, attr_value, attr_price " .
                        "FROM " . $this->sanmu->table('goods_attr') .
                        " WHERE goods_id = '" . intval($_REQUEST['goods_id']) . "' ";
                    $res = $this->db->query($sql);
                    foreach ($res as $row) {
                        $this->db->autoExecute($this->sanmu->table('goods_attr'), addslashes_deep($row), 'INSERT');
                    }
                }
                ...
                /* 图片列表 */
                $sql = "SELECT * FROM " . $this->sanmu->table('goods_gallery') . " WHERE goods_id = '$goods_id'";
                ...
                $min_img_id = $this->db->getOne(" SELECT img_id   FROM " . $this->sanmu->table("goods_gallery") . " WHERE goods_id = '" . $goods_id . "' AND img_desc = '$img_default' ORDER BY img_desc   LIMIT 1");
            ...
            if ($GLOBALS['_CFG']['open_oss'] == 1) {
                $bucket_info = $this->sanmuRepository->getBucketInfo();
            ...
            //获取分类数组
            $type_c_id = $this->db->getOne("SELECT c_id FROM" . $this->sanmu->table("goods_type") . "WHERE cat_id = '" . $goods['goods_type'] . "' " . $where . " LIMIT 1");//获取属性分类id
            ...
            if (count($attr_arr) > 0) {
                ...
                foreach ($attr_arr as $key => $val) {
                    ...
                    foreach ($val as $k => $v) {
                        ...
                        if (!$data) {
                            $sql = "SELECT MAX(goods_attr_id) AS goods_attr_id FROM " . $this->sanmu->table('goods_attr') . " WHERE 1 ";
                            $max_goods_attr_id = $this->db->getOne($sql);
                            $attr_sort = $max_goods_attr_id + 1;

                            $sql = " INSERT INTO " . $this->sanmu->table('goods_attr') . " (goods_id, attr_id, attr_value, attr_sort, admin_id) " .
                                " VALUES " .
                ...
                $products_list = $this->sanmu->page_array($filter['page_size'], $filter['page'], $attr_group, 0, $filter);
        ...
        elseif ($_REQUEST['act'] == 'add_desc') {
            ...
            $sql = "DELETE FROM " . $this->sanmu->table('link_desc_temporary') . " WHERE ru_id = '" . $adminru['ru_id'] . "'";
            $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'add_link_desc') {
            ...
            $linked_array = sanmu_decode($_GET['add_ids'], true);
            $linked_goods = sanmu_decode($_GET['JSON'], true);
            $id = $linked_goods[0];
        ...
        elseif ($_REQUEST['act'] == 'drop_link_desc') {
            ...
            $drop_goods = sanmu_decode($_GET['drop_ids'], true);
            $linked_goods = sanmu_decode($_GET['JSON'], true);
            $id = $linked_goods[0];
        ...
        elseif ($_REQUEST['act'] == 'insert_link_desc' || $_REQUEST['act'] == 'update_link_desc') {
            ...
            $sql = "SELECT goods_id FROM " . $this->sanmu->table('link_desc_temporary') . " WHERE 1 AND ru_id = '" . $adminru['ru_id'] . "'";
            ...
            if (!empty($desc_name)) {
                $sql = "DELETE FROM " . $this->sanmu->table('link_desc_goodsid') . " WHERE d_id = '$id'";
                $this->db->query($sql);

                if ($id > 0) {
                    $this->db->autoExecute($this->sanmu->table('link_goods_desc'), $other, "UPDATE", "id = '$id'");
                    $link_cnt = $GLOBALS['_LANG']['edit_success'];
                } else {
                    $this->db->autoExecute($this->sanmu->table('link_goods_desc'), $other, "INSERT");
        ...
        elseif ($_REQUEST['act'] == 'del_link_desc') {
            ...
            $sql = "DELETE FROM " . $this->sanmu->table('link_goods_desc') . " WHERE id = '$id'";
            $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
            ...
            if ($_POST['goods_sn']) {
                ...
                $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('goods') .
            ...
            if (empty($_POST['goods_sn'])) {
                $max_id = $is_insert ? $this->db->getOne("SELECT MAX(goods_id) + 1 FROM " . $this->sanmu->table('goods')) : $_REQUEST['goods_id'];
            ...
            if ($GLOBALS['_CFG']['review_goods'] == 0) {
                ...
                if ($adminru['ru_id'] > 0) {
                    $sql = "SELECT review_goods FROM " . $this->sanmu->table('merchants_shop_information') . " where user_id = '" . $adminru['ru_id'] . "'";
            ...
            if ($is_insert) {
                ...
                $volume_price_sql = " SELECT * FROM " . $this->sanmu->table('volume_price') . " WHERE goods_id = '$goods_id' ";
                ...
                $this->db->autoExecute($this->sanmu->table('goods_change_log'), $logs_change_old, 'INSERT');
                ...
                $this->db->autoExecute($this->sanmu->table('goods_change_log'), $logs_change, 'INSERT');
            }
            ...
            if ($is_insert) {
                if ($other_catids) {
                    $other_catids = $this->sanmuRepository->delStrComma($other_catids);
                    $sql = "UPDATE" . $this->sanmu->table("goods_cat") . " SET goods_id='$goods_id' WHERE goods_id = 0 AND cat_id in ($other_catids)";
                    $this->db->query($sql);
                }
                ...
                if (file_exists(storage_public($goods_video)) && $goods_video) {
                    if (move_upload_file(storage_public($goods_video), $video_path . $new_goods_video)) {
                        ...
                        $sql = "UPDATE" . $this->sanmu->table("goods") . " SET goods_video = '$goods_video' WHERE goods_id = '$goods_id'";
                        $this->db->query($sql);
                    }
                }
                 ...
                 * $shipping_fee
                 */
                $sql = "UPDATE" . $this->sanmu->table("cart") . " SET freight = '$freight', tid = '$tid', shipping_fee = '$shipping_fee' WHERE goods_id = '$goods_id'";
                $this->db->query($sql);
                ...
                if ($old_commission_rate <> $commission_rate) {
                    $sql = "UPDATE " . $this->sanmu->table('cart') . " SET commission_rate = '$commission_rate' WHERE ru_id = '" . $adminru['ru_id'] . "' AND goods_id = '$goods_id' AND is_real = 1 AND is_gift = 0";
                    $this->db->query($sql);
                }
            }
            ...
            if ($goods_id) {
                ...
                $extend = $this->db->getOne("select count(*) from " . $this->sanmu->table('goods_extend') . " where goods_id='$goods_id'");
                if ($extend > 0) {
                    //跟新商品扩展信息
                    $extend_sql = "update " . $this->sanmu->table('goods_extend') . " SET `is_reality`='$is_reality',`is_return`='$is_return',`is_fast`='$is_fast' WHERE goods_id='$goods_id'";
                } else {
                    //插入商品扩展信息
                    $extend_sql = "INSERT INTO " . $this->sanmu->table('goods_extend') . "(`goods_id`, `is_reality`, `is_return`, `is_fast`) VALUES ('$goods_id','$is_reality','$is_return','$is_fast')";
                }
            ...
            $this->db->autoExecute($this->sanmu->table('goods_extend'), $extend_arr, "UPDATE", "goods_id = '$goods_id'");
            //扩展信息 by wu end
            ...
            if ($is_insert) {
                ...
                if ($model_price == 1) {
                    ...
                    if ($warehouse_id) {
                        ...
                        $this->db->query(" UPDATE " . $this->sanmu->table("warehouse_goods") . " SET goods_id = '$goods_id' WHERE w_id " . db_create_in($warehouse_id));
                    }
                    ...
                    if ($warehouse_area_id) {
                        ...
                        $this->db->query(" UPDATE " . $this->sanmu->table("warehouse_area_goods") . " SET goods_id = '$goods_id' WHERE a_id " . db_create_in($warehouse_area_id));
                    }
                }
                ...
                //降价通知
                $sql = "SELECT * FROM " . $this->sanmu->table('sale_notice') . " WHERE goods_id='$goods_id' AND STATUS!=1";
                $notice_list = $this->db->getAll($sql);

                foreach ($notice_list as $key => $val) {
                    //查询会员名称 by wu
                    $sql = " select user_name from " . $this->sanmu->table('users') . " where user_id='" . $val['user_id'] . "' ";
                    ...
                    if ($shop_price <= $val['hopeDiscount'] && $val['cellphone'] && $GLOBALS['_CFG']['sms_price_notice'] == '1') {
                        ...
                        $smsParams = [
                            ...
                            'goodsname' => $this->sanmuRepository->subStr($goods_name, 20),
                        ...
                        $send_type = 2;
                        if ($res) {
                            $sql = "UPDATE " . $this->sanmu->table('sale_notice') . " SET status = 1, send_type=2 WHERE goods_id = '$goods_id' AND user_id='$val[user_id]'";
                            $this->db->query($sql);
                            $send_ok = 1;
                            notice_log($goods_id, $val['cellphone'], $send_ok, $send_type);
                        } else {
                            $sql = "UPDATE " . $this->sanmu->table('sale_notice') . " SET status = 3, send_type=2 WHERE goods_id = '$goods_id' AND user_id='$val[user_id]'";
                            $this->db->query($sql);
                            $send_ok = 0;
                            notice_log($goods_id, $val['cellphone'], $send_ok, $send_type);
                        }
                    ...
                    if ($send_ok == 0 && $shop_price <= $val['hopeDiscount'] && $val['email']) {
                        ...
                        $this->smarty->assign('goods_link', $this->sanmu->seller_url() . "goods.php?id=" . $goods_id);
                        ...
                        if ($this->commonRepository->sendEmail($user_name, $val['email'], $template['template_subject'], $content, $template['is_html'])) {
                            $sql = "UPDATE " . $this->sanmu->table('sale_notice') . " SET status = 1, send_type=1 WHERE goods_id = '$goods_id' AND user_id='$val[user_id]'";
                            $this->db->query($sql);
                            $send_ok = 1;
                            notice_log($goods_id, $val['email'], $send_ok, $send_type);
                        } else {
                            $sql = "UPDATE " . $this->sanmu->table('sale_notice') . " SET status = 3, send_type=1 WHERE goods_id = '$goods_id' AND user_id='$val[user_id]'";
                            $this->db->query($sql);
                            $send_ok = 0;
                            notice_log($goods_id, $val['email'], $send_ok, $send_type);
                        }
            ...
            if ((isset($_POST['attr_id_list']) && isset($_POST['attr_value_list'])) || (empty($_POST['attr_id_list']) && empty($_POST['attr_value_list']))) {
                ...
                $sql = "SELECT attr_id, attr_index FROM " . $this->sanmu->table('attribute') . " WHERE cat_id = '$goods_type'";
                ...
                FROM " . $this->sanmu->table('goods_attr') . " AS g
                    LEFT JOIN " . $this->sanmu->table('attribute') . " AS a
                ...
                foreach ($goods_attr_list as $attr_id => $attr_value_list) {
                    foreach ($attr_value_list as $attr_value => $info) {
                        if ($info['sign'] == 'insert') { //ecmoban模板堂 --zhuo attr_sort
                            $sql = "INSERT INTO " . $this->sanmu->table('goods_attr') . " (attr_id, goods_id, attr_value, attr_price, attr_sort)" .
                                "VALUES ('$attr_id', '$goods_id', '$attr_value', '$info[attr_price]', '$info[attr_sort]')";
                        } elseif ($info['sign'] == 'update') { //ecmoban模板堂 --zhuo attr_sort
                            $sql = "UPDATE " . $this->sanmu->table('goods_attr') . " SET attr_price = '$info[attr_price]', attr_sort = '$info[attr_sort]' WHERE goods_attr_id = '$info[goods_attr_id]' LIMIT 1";
                        } else {
                            if ($model_attr == 1) {
                            ...
                            $sql = "DELETE FROM " . $this->sanmu->table('goods_attr') . " WHERE goods_attr_id = '" . $info['goods_attr_id'] . "' LIMIT 1";
                        }
                        $this->db->query($sql);
                    }
                }
            ...
            if ($is_insert) {
                ...
                if ($thumb_img_id) {
                    $sql = " UPDATE " . $this->sanmu->table('goods_gallery') . " SET goods_id = '" . $goods_id . "' WHERE goods_id = 0 AND img_id " . db_create_in($thumb_img_id);
                    $this->db->query($sql);
                }
            ...
            if (!empty($_POST['goods_img_url']) && $is_img_url == 1) {
                ...
                // 处理商品图片
                $sql = " UPDATE " . $this->sanmu->table('goods') . " SET goods_thumb = '$goods_thumb', goods_img = '$goods_img', original_img = '$original_img' WHERE goods_id = '$goods_id' ";
                $this->db->query($sql);
                ...
                if (isset($img)) {
                    ...
                    $sql = "INSERT INTO " . $this->sanmu->table('goods_gallery') . " (goods_id, img_url, thumb_url, img_original) " .
                        "VALUES ('$goods_id', '$gallery_img', '$gallery_thumb', '$img')";
                    $this->db->query($sql);
                }

                $this->sanmuRepository->getOssAddFile([$goods_img, $goods_thumb, $original_img, $gallery_img, $gallery_thumb, $img]);
            } else {
                $this->sanmuRepository->getOssAddFile([$goods_img, $goods_thumb, $original_img]);
            }
            ...
            if ($is_insert) {
                $sql = "UPDATE" . $this->sanmu->table($table) . " SET goods_id = '$goods_id' WHERE goods_id = 0 AND admin_id = '$admin_id'";
                $this->db->query($sql);
            } else {
                //下架商品购物车设置失效
                if ($is_on_sale == 0) {
                    $this->db->query(" UPDATE " . $this->sanmu->table('cart') . " SET is_invalid = 1 WHERE goods_id = '$goods_id' ");
                }
            }
            ...
            /* 取出商品信息 */
            $sql = "SELECT goods_sn, goods_name, goods_type, shop_price, model_inventory, model_attr FROM " . $this->sanmu->table('goods') . " WHERE goods_id = '$goods_id' LIMIT 1";
            ...
            foreach ($product['product_sn'] as $key => $value) {
                ...
                if ($product['product_id'][$key]) {
                    ...
                    $sql = "UPDATE " . $this->sanmu->table($table) . " SET product_number = '" . $product['product_number'][$key] . "', " .
                    ...
                    $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table($table), $product_other, 'INSERT');
                    ...
                    if (!$product_id) {
                        ...
                        //货品号为空 自动补货品号
                        if (empty($value)) {
                            $sql = "UPDATE " . $this->sanmu->table($table) . "
            ...
            $sql = "SELECT goods_attr,product_sn,bar_code,product_number,product_price,product_market_price,product_promote_price,product_warn_number,warehouse_id,area_id,admin_id FROM" .
                $this->sanmu->table('products_changelog') . $changelog_where . $where_products;
            ...
            if (!empty($products_changelog)) {
                foreach ($products_changelog as $k => $v) {
                    ...
                    /* 插入货品表 */
                    $sql = "INSERT INTO " . $this->sanmu->table($table) .
            ...
            if ($GLOBALS['_CFG']['add_shop_price'] == 0 && $goods['model_attr'] == 0) {
                ...
                if ($spe) {
                    ...
                    $goodsAttrId = $this->sanmuRepository->delStrComma($goodsAttrId);
                }
        ...
        elseif ($_REQUEST['act'] == 'batch') {
            ...
            if (isset($_POST['type'])) {
                ...
                if ($_POST['type'] == 'trash') {
                    ...
                    if ($is_promotion) {
                        foreach ($is_promotion as $res) {
                            $res[$res['type']]['goods_sn'] = isset($res[$res['type']]['goods_sn']) ? $this->sanmuRepository->delStrComma($res[$res['type']]['goods_sn']) : '';
                ...
                elseif ($_POST['type'] == 'goods_transport') {
                    ...
                    $data['freight'] = 2;
                    $data['tid'] = $_POST['tid'];
                    $this->db->autoExecute($this->sanmu->table('goods'), $data, "UPDATE", "goods_id " . db_create_in($goods_id) . " AND user_id = '" . $adminru['ru_id'] . "'");
                     ...
                     * $shipping_fee
                     */
                    $sql = "UPDATE" . $this->sanmu->table("cart") . " SET freight = '" . $data['freight'] . "', tid = '" . $data['tid'] . "' WHERE goods_id " . db_create_in($goods_id) . " AND ru_id = '" . $adminru['ru_id'] . "'";
                    $this->db->query($sql);
                ...
                elseif ($_POST['type'] == 'return_type') {
                    //修改退换货标识
                    $sql = "UPDATE" . $this->sanmu->table('goods') . "SET goods_cause = '0,1,2,3' WHERE goods_id " . db_create_in($goods_id);
                    $this->db->query($sql);
                    //查找商品拓展
                    $goods_id = explode(',', $goods_id);
                    if (!empty($goods_id)) {
                        foreach ($goods_id as $v) {
                            $sql = "SELECT COUNT(*) FROM" . $this->sanmu->table('goods_extend') . "WHERE goods_id = '$v'";
                            $goods_extend = $this->db->getOne($sql);
                            if ($goods_extend > 0) {
                                $sql = " UPDATE" . $this->sanmu->table('goods_extend') . "SET is_return = 1 WHERE goods_id = '$v'";
                            } else {
                                $sql = "INSERT INTO" . $this->sanmu->table('goods_extend') . "(`goods_id`,`is_return`)VALUES('$v',1)";
                            }
                            $this->db->query($sql);
                        }
                    }
        ...
        elseif ($_REQUEST['act'] == 'edit_goods_sn') {
            ...
            $where = " AND (SELECT g.user_id FROM " . $this->sanmu->table('goods') . " AS g WHERE g.goods_id = p.goods_id LIMIT 1) = '" . $adminru['ru_id'] . "'";
            $sql = "SELECT p.goods_id FROM " . $this->sanmu->table('products') . " AS p WHERE p.product_sn='$goods_sn'" . $where;
        ...
        elseif ($_REQUEST['act'] == 'edit_goods_bar_code') {
            ...
            $where = " AND (SELECT g.user_id FROM " . $this->sanmu->table('goods') . " AS g WHERE g.goods_id = p.goods_id LIMIT 1) = '" . $adminru['ru_id'] . "'";
            $sql = "SELECT p.goods_id FROM " . $this->sanmu->table('products') . " AS p WHERE p.bar_code = '$bar_code'" . $where;
            if ($this->db->getOne($sql)) {
                return make_json_error($GLOBALS['_LANG']['goods_bar_code_exists']);
            }
        ...
        elseif ($_REQUEST['act'] == 'check_goods_sn') {
            ...
            if (!empty($goods_sn)) {
                ...
                if (!empty($goods_sn)) {
                    $sql = "SELECT p.product_id FROM " . $this->sanmu->table('products') . " AS p," .
        ...
        elseif ($_REQUEST['act'] == 'check_products_goods_sn') {
            ...
            if (!is_array($products_sn)) {
                ...
                foreach ($products_sn as $val) {
                    ...
                    $sql = "SELECT goods_id FROM " . $this->sanmu->table('products') . "WHERE product_sn='$val'";
                    if ($this->db->getOne($sql)) {
                        return make_json_error($val . $GLOBALS['_LANG']['goods_sn_exists']);
                    }
                }
        ...
        elseif ($_REQUEST['act'] == 'toggle_on_sale') {
            ...
            if ($exc->edit($change, $goods_id)) {
                ...
                if ($on_sale == 0) {
                    ...
                    $sql = "SELECT act_id FROM " . $this->sanmu->table('presale_activity') . " WHERE goods_id = '$goods_id'";
                    if ($this->db->getOne($sql, true)) {
                        $this->db->query("DELETE FROM " . $GLOBALS['sanmu']->table('presale_activity') . " WHERE goods_id = '$goods_id' ");
                        $this->db->query("DELETE FROM " . $GLOBALS['sanmu']->table('cart') . " WHERE goods_id = '$goods_id' ");
                    }
                }
        ...
        elseif ($_REQUEST['act'] == 'edit_img_desc') {
            ...
            $exc_gallery = new Exchange($this->sanmu->table('goods_gallery'), $this->db, 'img_id', 'goods_id');
        ...
        } elseif ($_REQUEST['act'] == 'main_sanmu') {
            $data = read_static_cache('seller_goods_str');
            if ($data === false) {
                $shop_url = urlencode($this->sanmu->seller_url());
                ...
                $shop_country = $this->db->getOne("SELECT region_name FROM " . $this->sanmu->table('region') . " WHERE region_id='$shop_country'");
                $shop_province = $this->db->getOne("SELECT region_name FROM " . $this->sanmu->table('region') . " WHERE region_id='$shop_province'");
                $shop_city = $this->db->getOne("SELECT region_name FROM " . $this->sanmu->table('region') . " WHERE region_id='$shop_city'");

                $httpData = [
                    'domain' => $this->sanmu->get_domain(), //当前域名
        ...
        elseif ($_REQUEST['act'] == 'remove') {
            ...
            $sql = "SELECT goods_id, user_id " . "FROM " . $this->sanmu->table('goods') .
        ...
        elseif ($_REQUEST['act'] == 'drop_goods') {
            ...
            //删除商品扩展信息by wang
            $sql = "delete from " . $this->sanmu->table('goods_extend') . " where goods_id='$goods_id'";
            $this->db->query($sql);

            /* 删除商品的货品记录 */
            $sql = "DELETE FROM " . $this->sanmu->table('products') .
                " WHERE goods_id = '$goods_id'";
            $this->db->query($sql);

            $sql = "DELETE FROM " . $this->sanmu->table('products_warehouse') .
                " WHERE goods_id = '$goods_id'";
            $this->db->query($sql);

            $sql = "DELETE FROM " . $this->sanmu->table('products_area') .
                " WHERE goods_id = '$goods_id'";
            $this->db->query($sql);

            //清楚商品零时货品表数据
            $sql = "DELETE FROM" . $this->sanmu->table('products_changelog') .
                " WHERE goods_id = '$goods_id'";
            $this->db->query($sql);
            ...
            $sql = "SELECT img_url, thumb_url, img_original " .
                "FROM " . $this->sanmu->table('goods_gallery') .
                " WHERE goods_id = '$goods_id'";
            $res = $this->db->query($sql);
            foreach ($res as $row) {
                $arr = [];
                if (!empty($row['img_url']) && strpos($row['img_url'], "data/gallery_album") === false) {
                    $arr[] = $row['img_url'];
                    sanmu_unlink(storage_public($row['img_url']));
                }
                if (!empty($row['thumb_url']) && strpos($row['thumb_url'], "data/gallery_album") === false) {
                    $arr[] = $row['thumb_url'];
                    sanmu_unlink(storage_public($row['thumb_url']));
                }
                if (!empty($row['img_original']) && strpos($row['img_original'], "data/gallery_album") === false) {
                    $arr[] = $row['img_original'];
                    sanmu_unlink(storage_public($row['img_original']));
                }

                $this->sanmuRepository->getOssDelFile($arr);
            }

            $sql = "DELETE FROM " . $this->sanmu->table('goods_gallery') . " WHERE goods_id = '$goods_id'";
            $this->db->query($sql);

            /* 删除相关表记录 */
            $sql = "DELETE FROM " . $this->sanmu->table('collect_goods') . " WHERE goods_id = '$goods_id'";
            $this->db->query($sql);
            $sql = "DELETE FROM " . $this->sanmu->table('goods_article') . " WHERE goods_id = '$goods_id'";
            $this->db->query($sql);
            $sql = "DELETE FROM " . $this->sanmu->table('goods_attr') . " WHERE goods_id = '$goods_id'";
            $this->db->query($sql);
            $sql = "DELETE FROM " . $this->sanmu->table('goods_cat') . " WHERE goods_id = '$goods_id'";
            $this->db->query($sql);
            $sql = "DELETE FROM " . $this->sanmu->table('member_price') . " WHERE goods_id = '$goods_id'";
            $this->db->query($sql);
            $sql = "DELETE FROM " . $this->sanmu->table('group_goods') . " WHERE parent_id = '$goods_id'";
            $this->db->query($sql);
            $sql = "DELETE FROM " . $this->sanmu->table('group_goods') . " WHERE goods_id = '$goods_id'";
            $this->db->query($sql);
            $sql = "DELETE FROM " . $this->sanmu->table('link_goods') . " WHERE goods_id = '$goods_id'";
            $this->db->query($sql);
            $sql = "DELETE FROM " . $this->sanmu->table('link_goods') . " WHERE link_goods_id = '$goods_id'";
            $this->db->query($sql);
            $sql = "DELETE FROM " . $this->sanmu->table('tag') . " WHERE goods_id = '$goods_id'";
            $this->db->query($sql);
            $sql = "DELETE FROM " . $this->sanmu->table('comment') . " WHERE comment_type = 0 AND id_value = '$goods_id'";
            $this->db->query($sql);
            $sql = "DELETE FROM " . $this->sanmu->table('collect_goods') . " WHERE goods_id = '$goods_id'";
            $this->db->query($sql);
            $sql = "DELETE FROM " . $this->sanmu->table('booking_goods') . " WHERE goods_id = '$goods_id'";
            $this->db->query($sql);
            $sql = "DELETE FROM " . $this->sanmu->table('goods_activity') . " WHERE goods_id = '$goods_id'";
            $this->db->query($sql);
            $sql = "DELETE FROM " . $this->sanmu->table('cart') . " WHERE goods_id = '$goods_id'";
            $this->db->query($sql);

            $sql = "DELETE FROM " . $this->sanmu->table('warehouse_goods') . " WHERE goods_id = '$goods_id'";
            $this->db->query($sql);
            $sql = "DELETE FROM " . $this->sanmu->table('warehouse_attr') . " WHERE goods_id = '$goods_id'";
            $this->db->query($sql);
            $sql = "DELETE FROM " . $this->sanmu->table('warehouse_area_goods') . " WHERE goods_id = '$goods_id'";
            $this->db->query($sql);
            $sql = "DELETE FROM " . $this->sanmu->table('warehouse_area_attr') . " WHERE goods_id = '$goods_id'";
            $this->db->query($sql);

            /* 如果不是实体商品，删除相应虚拟商品记录 */
            if ($goods['is_real'] != 1) {
                $sql = "DELETE FROM " . $this->sanmu->table('virtual_card') . " WHERE goods_id = '$goods_id'";
        ...
        elseif ($_REQUEST['act'] == 'drop_image') {
            ...
            /* 删除数据 */
            $sql = "DELETE FROM " . $this->sanmu->table('goods_gallery') . " WHERE img_id = '$img_id' LIMIT 1";
            $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'drop_product') {
            ...
            $group_attr = sanmu_decode($group_attr, true);
            ...
            $sql = "SELECT $select FROM " . $this->sanmu->table($table) . " WHERE product_id = '$product_id' LIMIT 1";
            ...
            /* 删除数据 */
            $sql = "DELETE FROM " . $this->sanmu->table($table) . " WHERE product_id = '$product_id' LIMIT 1";
            $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'drop_warehouse') {
            ...
            /* 删除数据 */
            $sql = "DELETE FROM " . $this->sanmu->table('warehouse_goods') . " WHERE w_id = '$w_id' LIMIT 1";
            $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'edit_warehouse_number') {
            ...
            $sql = "SELECT goods_id, region_number, region_id FROM " . $this->sanmu->table("warehouse_goods") . " WHERE w_id = '$w_id' LIMIT 1";
            ...
            $sql = "update " . $this->sanmu->table('warehouse_goods') . " set region_number = '$region_number' where w_id = '$w_id' ";
            $res = $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'edit_warehouse_sn') {
            ...
            $sql = "update " . $this->sanmu->table('warehouse_goods') . " set region_sn = '$region_sn' where w_id = '$w_id' ";
            $res = $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'edit_warehouse_price') {
            ...
            $sql = "update " . $this->sanmu->table('warehouse_goods') . " set warehouse_price = '$warehouse_price' where w_id = '$w_id' ";
            $res = $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'edit_warehouse_promote_price') {
            ...
            $sql = "update " . $this->sanmu->table('warehouse_goods') . " set warehouse_promote_price = '$warehouse_promote_price' where w_id = '$w_id' ";
            $res = $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'edit_warehouse_give_integral') {
            ...
            $sql = "update " . $this->sanmu->table('warehouse_goods') . " set give_integral = '$give_integral' where w_id = '$w_id' ";
            $res = $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'edit_warehouse_rank_integral') {
            ...
            $sql = "update " . $this->sanmu->table('warehouse_goods') . " set rank_integral = '$rank_integral' where w_id = '$w_id' ";
            $res = $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'edit_warehouse_pay_integral') {
            ...
            $sql = "update " . $this->sanmu->table('warehouse_goods') . " set pay_integral = '$pay_integral' where w_id = '$w_id' ";
            $res = $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'edit_region_sn') {
            ...
            $sql = "update " . $this->sanmu->table('warehouse_area_goods') . " set region_sn = '$region_sn' where a_id = '$a_id' ";
            $res = $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'drop_warehouse_area') {
            ...
            /* 删除数据 */
            $sql = "DELETE FROM " . $this->sanmu->table('warehouse_area_goods') . " WHERE a_id = '$a_id' LIMIT 1";
            $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'edit_region_price') {
            ...
            $sql = "update " . $this->sanmu->table('warehouse_area_goods') . " set region_price = '$region_price' where a_id = '$a_id' ";
            $res = $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'edit_region_number') {
            ...
            $sql = "SELECT goods_id, region_number, region_id, city_id FROM " . $this->sanmu->table("warehouse_area_goods") . " WHERE a_id = '$a_id' LIMIT 1";
            ...
            $sql = "UPDATE " . $this->sanmu->table('warehouse_area_goods') . " SET region_number = '$region_number' WHERE a_id = '$a_id' ";
            $res = $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'edit_region_promote_price') {
            ...
            $sql = "update " . $this->sanmu->table('warehouse_area_goods') . " set region_promote_price = '$region_promote_price' where a_id = '$a_id' ";
            $res = $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'city_region') {
            ...
            if ($area_id > 0) {
                $sql = "select region_id, region_name from " . $GLOBALS['sanmu']->table('region_warehouse') . " where parent_id = '$area_id'";
        ...
        elseif ($_REQUEST['act'] == 'edit_region_give_integral') {
            ...
            $sql = "update " . $this->sanmu->table('warehouse_area_goods') . " set give_integral = '$give_integral' where a_id = '$a_id' ";
            $res = $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'edit_region_rank_integral') {
            ...
            $sql = "update " . $this->sanmu->table('warehouse_area_goods') . " set rank_integral = '$rank_integral' where a_id = '$a_id' ";
            $res = $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'edit_region_pay_integral') {
            ...
            $sql = "update " . $this->sanmu->table('warehouse_area_goods') . " set pay_integral = '$pay_integral' where a_id = '$a_id' ";
            $res = $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'edit_region_sort') {
            ...
            $sql = "update " . $this->sanmu->table('warehouse_area_goods') . " set region_sort = '$region_sort' where a_id = '$a_id' ";
            $res = $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'insert_attr_img') {
            ...
            $this->sanmuRepository->getOssAddFile([$other['attr_img_flie']]);
            ...
            $this->db->autoExecute($this->sanmu->table('goods_attr'), $other, 'UPDATE', 'goods_attr_id = ' . $goods_attr_id . ' and attr_id = ' . $attr_id . ' and goods_id = ' . $goods_id);
        ...
        elseif ($_REQUEST['act'] == 'drop_attr_img') {
            ...
            $sql = "select attr_img_flie from " . $this->sanmu->table('goods_attr') . " where goods_attr_id = '$goods_attr_id'";
            $attr_img_flie = $this->db->getOne($sql);

            $this->sanmuRepository->getOssDelFile([$attr_img_flie]);

            @unlink(storage_public($attr_img_flie));
            $other['attr_img_flie'] = '';
            $this->db->autoExecute($this->sanmu->table('goods_attr'), $other, "UPDATE", "goods_attr_id = '$goods_attr_id'");
        ...
        elseif ($_REQUEST['act'] == 'choose_attrImg') {
            ...
            $sql = "SELECT attr_gallery_flie FROM " . $this->sanmu->table('goods_attr') . " WHERE goods_attr_id = '$goods_attr_id' AND goods_id = '$goods_id'";
            $attr_gallery_flie = $this->db->getOne($sql);

            /* 删除数据 */
            $sql = "SELECT img_id, thumb_url, img_url FROM " . $this->sanmu->table('goods_gallery') . " WHERE goods_id = '$goods_id'";
            $img_list = $this->db->getAll($sql);
        ...
        elseif ($_REQUEST['act'] == 'insert_gallery_attr') {
            ...
            if (!empty($gallery_id)) {
                $sql = "SELECT img_id, img_url FROM " . $this->sanmu->table('goods_gallery') . "WHERE img_id='$gallery_id'";
                $img = $this->db->getRow($sql);
                $result = $img['img_id'];

                $sql = "UPDATE " . $this->sanmu->table('goods_attr') . " SET attr_gallery_flie = '" . $img['img_url'] . "' WHERE goods_attr_id = '$goods_attr_id' AND goods_id = '$goods_id'";
                $this->db->query($sql);
            } else {
        ...
        elseif ($_REQUEST['act'] == 'get_goods_list') {
            ...
            $filters = sanmu_decode($_GET['JSON']);
        ...
        elseif ($_REQUEST['act'] == 'add_link_goods') {
            ...
            foreach ($linked_array as $val) {
                if ($is_double) {
                    /* 双向关联 */
                    $sql = "INSERT INTO " . $this->sanmu->table('link_goods') . " (goods_id, link_goods_id, is_double, admin_id) " .
        ...
        elseif ($_REQUEST['act'] == 'drop_link_goods') {
            ...
            if (!$is_signle) {
                ...
                $sql = "UPDATE " . $this->sanmu->table('link_goods') . " SET is_double = 0 " .
                    " WHERE link_goods_id = '$goods_id' AND goods_id " . $drop_goods_ids;
            }
            if ($goods_id == 0) {
        ...
        elseif ($_REQUEST['act'] == 'add_group_goods') {
            ...
            $fittings = sanmu_decode($_GET['add_ids'], true);
            $arguments = sanmu_decode($_GET['JSON'], true);
            $goods_id = $arguments[0];
            $price = $arguments[1];
            $group_id = $arguments[2];//by mike add

            $sql = "select count(*) from " . $this->sanmu->table('group_goods') . " where parent_id = '$goods_id' and group_id = '$group_id' and admin_id = '" . session('seller_id') . "'";
            $groupCount = $this->db->getOne($sql);

            $message = "";
            if ($groupCount < 1000) {
                foreach ($fittings as $val) {
                    $sql = "SELECT id FROM " . $this->sanmu->table('group_goods') . " WHERE parent_id = '$goods_id' AND goods_id = '$val' AND group_id = '$group_id'";
                    if (!$this->db->getOne($sql)) {
                        $sql = "INSERT INTO " . $this->sanmu->table('group_goods') . " (parent_id, goods_id, goods_price, admin_id, group_id) " .
        ...
        elseif ($_REQUEST['act'] == 'drop_group_goods') {
            ...
            $fittings = sanmu_decode($_GET['drop_ids'], true);
            $arguments = sanmu_decode($_GET['JSON'], true);
            $goods_id = $arguments[0];
            $price = $arguments[1];
        ...
        elseif ($_REQUEST['act'] == 'add_area_goods') {
            ...
            $sql = "SELECT user_id FROM " . $this->sanmu->table('goods') . " WHERE goods_id = '$goods_id'";
            $ru_id = $this->db->getOne($sql);

            foreach ($fittings as $val) {
                $sql = "INSERT INTO " . $this->sanmu->table('link_area_goods') . " (goods_id, region_id, ru_id) " .
        ...
        elseif ($_REQUEST['act'] == 'drop_area_goods') {
            ...
            $sql = "DELETE FROM " . $this->sanmu->table('link_area_goods') . " WHERE region_id" . $drop_goods_ids . " and goods_id = '$goods_id'";
            if ($goods_id == 0) {
        ...
        elseif ($_REQUEST['act'] == 'get_article_list') {
            ...
            $filters = (array)sanmu_decode(json_str_iconv($_GET['JSON']), true);
            ...
            $sql = 'SELECT article_id, title FROM ' . $this->sanmu->table('article') . $where .
        ...
        elseif ($_REQUEST['act'] == 'add_goods_article') {
            ...
            $articles = sanmu_decode($_GET['add_ids'], true);
            $arguments = sanmu_decode($_GET['JSON'], true);
            $goods_id = $arguments[0];

            foreach ($articles as $val) {
                $sql = "INSERT INTO " . $this->sanmu->table('goods_article') . " (goods_id, article_id, admin_id) " .
        ...
        elseif ($_REQUEST['act'] == 'drop_goods_article') {
            ...
            $articles = sanmu_decode($_GET['drop_ids'], true);
            $arguments = sanmu_decode($_GET['JSON'], true);
            $goods_id = $arguments[0];

            $sql = "DELETE FROM " . $this->sanmu->table('goods_article') . " WHERE " . db_create_in($articles, "article_id") . " AND goods_id = '$goods_id'";
            $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'product_list') {
            ...
            /* 取出商品信息 */
            $sql = "SELECT goods_sn, goods_name, goods_type, shop_price, model_attr FROM " . $this->sanmu->table('goods') . " WHERE goods_id = '$goods_id'";
            $goods = $this->db->getRow($sql);
            if (empty($goods)) {
        ...
        elseif ($_REQUEST['act'] == 'product_query') {
            ...
            /* 取出商品信息 */
            $sql = "SELECT goods_sn, goods_name, goods_type, shop_price FROM " . $this->sanmu->table('goods') . " WHERE goods_id = '$goods_id'";
            $goods = $this->db->getRow($sql);
            if (empty($goods)) {
                return make_json_error($GLOBALS['_LANG']['sys']['wrong'] . $GLOBALS['_LANG']['cannot_found_goods']);
            }
        ...
        elseif ($_REQUEST['act'] == 'edit_bar_code') {
            ...
            /* 修改 */
            $sql = "UPDATE " . $this->sanmu->table('products') . " SET bar_code = '$bar_code' WHERE product_id = '$product_id'";
        ...
        elseif ($_REQUEST['act'] == 'edit_product_number') {
            ...
            /* 修改货品库存 */
            $sql = "UPDATE " . $this->sanmu->table($table) . " SET product_number = '$product_number' WHERE product_id = '$product_id'";
            $result = $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'edit_product_warn_number') {
            ...
            /* 修改货品库存 */
            $sql = "UPDATE " . $this->sanmu->table($table) . " SET product_warn_number = '$product_warn_number' WHERE product_id = '$product_id'";
            $result = $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'edit_product_market_price') {
            ...
            /* 修改 */
            $sql = "UPDATE " . $this->sanmu->table($table) . " SET product_market_price = '$market_price' WHERE product_id = '$product_id'";
            $result = $this->db->query($sql);
            if ($result) {
                clear_cache_files();
        ...
        elseif ($_REQUEST['act'] == 'synchronization_attr') {
            ...
            if ($field) {
                ...
                /* 修改临时表 */
                $sql = "UPDATE " . $this->sanmu->table('products_changelog') . " SET $field = '$val'" . $changelog_where;
                $this->db->query($sql);
                ...
                /* 修改 */
                $sql = "UPDATE " . $this->sanmu->table($table) . " SET $field = '$val'" . $where;
                if ($this->db->query($sql)) {
                    clear_cache_files();
                }
            } else {
        ...
        elseif ($_REQUEST['act'] == 'edit_product_price') {
            ...
            if ($GLOBALS['_CFG']['goods_attr_price'] == 1 && $changelog == 0) {
                $sql = "SELECT goods_id FROM " . $this->sanmu->table($table) . " WHERE product_id = '$product_id'";
                $goods_id = $this->db->getOne($sql, true);
                ...
                $this->db->autoExecute($this->sanmu->table('goods'), $goods_other, 'UPDATE', "goods_id = '$goods_id' AND product_id = '$product_id' AND product_table = '$table'");
            }

            /* 修改货品库存 */
            $sql = "UPDATE " . $this->sanmu->table($table) . " SET product_price = '$product_price' WHERE product_id = '$product_id'";
            $result = $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'edit_product_promote_price') {
            ...
            /* 修改 */
            $sql = "UPDATE " . $this->sanmu->table($table) . " SET product_promote_price = '$promote_price' WHERE product_id = '$product_id'";
            $result = $this->db->query($sql);
            if ($result) {
                clear_cache_files();
        ...
        elseif ($_REQUEST['act'] == 'product_add_execute') {
            ...
            /* 取出商品信息 */
            $sql = "SELECT goods_sn, goods_name, goods_type, shop_price, model_inventory, model_attr FROM " . $this->sanmu->table('goods') . " WHERE goods_id = '" . $product['goods_id'] . "'";
            $goods = $this->db->getRow($sql);
            if (empty($goods)) {
                return sys_msg($GLOBALS['_LANG']['sys']['wrong'] . $GLOBALS['_LANG']['cannot_found_goods'], 1, [], false);
            }
            ...
            foreach ($product['product_sn'] as $key => $value) {
                ...
                /* 插入货品表 */
                $sql = "INSERT INTO " . $this->sanmu->table('products') . " (goods_id, goods_attr, product_sn, bar_code, product_price, product_number)  VALUES ('" . $product['goods_id'] . "', '$goods_attr', '$value', '" . $product['bar_code'][$key] . "', '" . $product['product_price'][$key] . "', '" . $product['product_number'][$key] . "')";
                if (!$this->db->query($sql)) {
                    continue;
                }
        ...
        elseif ($_REQUEST['act'] == 'batch_product') {
            ...
            if ($_POST['type'] == 'drop') {
                ...
                $sum = 0;
                $goods_id = 0;
                $sql = "SELECT product_id, goods_id, product_number FROM  " . $this->sanmu->table('products') . " WHERE product_id $product_bound";
                ...
                if (!empty($product_array)) {
                    ...
                    /* 删除货品 */
                    $sql = "DELETE FROM " . $this->sanmu->table('products') . " WHERE product_id $product_bound";
                    if ($this->db->query($sql)) {
                        //记录日志
            ...
            if (!empty($keyword)) {
                if ($adminru['ru_id'] == 0) {
                    $sql = "SELECT `cat_id`,`cat_name` FROM " . $this->sanmu->table('category') . "WHERE `cat_name` like '%$keyword%' AND parent_id = '$parent_id'";
                    $options = $this->db->getAll($sql);
                } else {
                    $sql = "select user_shopMain_category from " . $this->sanmu->table('merchants_shop_information') . " where user_id = '" . $adminru['ru_id'] . "'";
                    $shopMain_category = $this->db->getOne($sql);
                    $cat_ids = explode(',', get_category_child_tree($shopMain_category));
                    $sql = "SELECT `cat_id`,`cat_name` FROM " . $this->sanmu->table('category') . "WHERE `cat_name` like '%$keyword%' and cat_id " . db_create_in($cat_ids) . " AND parent_id = '$parent_id'";
                    $options = $this->db->getAll($sql);
                }
        ...
        elseif ($_REQUEST['act'] == 'edit_attr_sort') {
            ...
            /* 修改 */
            $sql = "UPDATE " . $this->sanmu->table('goods_attr') . " SET attr_sort = '$attr_sort' WHERE goods_attr_id = '$goods_attr_id'";
            $result = $this->db->query($sql);
            if ($result) {
                clear_cache_files();
        ...
        elseif ($_REQUEST['act'] == 'addWarehouse') {
            ...
            if (empty($ware_name)) {
                ...
                $sql = "select w_id from " . $this->sanmu->table('warehouse_goods') . " where goods_id = '$goods_id' and region_id = '" . $ware_name . "' AND user_id = '$user_id'";
                ...
                if ($w_id > 0) {
                    ...
                    if ($ware_number == 0) {
                        ...
                        $sql = "insert into " . $this->sanmu->table('warehouse_goods') .
            ...
            if (empty($ware_name)) {
                ...
                for ($i = 0; $i < count($ware_name); $i++) {
                    if (!empty($ware_name[$i])) {
                        ...
                        $sql = "SELECT w_id FROM " . $this->sanmu->table('warehouse_goods') . " WHERE goods_id = '$goods_id' AND region_id = '" . $ware_name[$i] . "'";
        ...
        elseif ($_REQUEST['act'] == 'addRegion') {
            ...
            if (empty($area_name)) {
                ...
                if ($region_number == 0) {
                    ...
                    $sql = "select a_id from " . $this->sanmu->table('warehouse_area_goods') . " where goods_id = '$goods_id' and region_id = '" . $area_name . "'" . $where;
                ...
                if (empty($region_number)) {
                    ...
                    for ($i = 0; $i < count($area_pricetype); $i++) {
                        if (!empty($area_pricetype[$i])) {
                            ...
                            $sql = "select a_id from " . $this->sanmu->table('warehouse_area_goods') . " where goods_id = '$goods_id' and region_id = '" . $area_name[$i] . "'" . $where;
        ...
        elseif ($_REQUEST['act'] == 'addImg') {
            ...
            if ($goods_id > 0) {
                ...
                $sql = "SELECT * FROM " . $this->sanmu->table('goods_gallery') . " WHERE goods_id='' $where ORDER BY img_desc ASC";
            }
            $img_list = $this->db->getAll($sql);
            /* 格式化相册图片路径 */
            if (isset($GLOBALS['shop_id']) && ($GLOBALS['shop_id'] > 0)) {
        ...
        elseif ($_REQUEST['act'] == 'img_default') {
            ...
            if ($img_id > 0) {
                $goods_id = $this->db->getOne(" SELECT goods_id FROM" . $this->sanmu->table('goods_gallery') . " WHERE img_id= '$img_id'");
                $this->db->query("UPDATE" . $this->sanmu->table('goods_gallery') . " SET img_desc = img_desc+1 WHERE goods_id = '$goods_id' ");
                $sql = $this->db->query("UPDATE" . $this->sanmu->table('goods_gallery') . " SET img_desc = 1 WHERE img_id = '$img_id'");
                if ($sql = true) {
                    $where = " 1 ";
                    ...
                    $sql = "SELECT * FROM " . $this->sanmu->table('goods_gallery') . " WHERE $where ORDER BY img_desc ASC";
                    $img_list = $this->db->getAll($sql);
            ...
            $goods_id = !empty($_REQUEST['goods_id']) ? intval($_REQUEST['goods_id']) : '0';
            if ($con_id > 0) {
                $sql = "DELETE FROM" . $this->sanmu->table('goods_consumption') . " WHERE id = '$con_id' AND goods_id = '$goods_id'";
                if ($this->db->query($sql)) {
                    $result['error'] = 2;
        ...
        elseif ($_REQUEST['act'] == 'gallery_album_dialog') {
            ...
            // 获取相册信息 qin
            $sql = "SELECT album_id,ru_id,album_mame,album_cover,album_desc,sort_order FROM " . $this->sanmu->table('gallery_album') . " "
            ...
            $sql = "SELECT * FROM " . $this->sanmu->table('pic_album') . " WHERE ru_id = '$adminru[ru_id]'";
        ...
        elseif ($_REQUEST['act'] == 'view_detail') {
            ...
            if ($step == 'member') {
                $res = $this->db->getOne(" SELECT member_price FROM " . $this->sanmu->table('goods_change_log') . " WHERE log_id = '$log_id' ");
                $res = unserialize($res);
                if ($res) {
                    foreach ($res as $k => $v) {
                        $member_price[$k]['rank_name'] = $this->db->getOne(" SELECT rank_name FROM " . $this->sanmu->table('user_rank') . " WHERE rank_id = '$k' ");
                        $member_price[$k]['member_price'] = $v;
                    }
                }
                $this->smarty->assign('res', $member_price);
            } elseif ($step == 'volume') {
                $res = $this->db->getOne(" SELECT volume_price FROM " . $this->sanmu->table('goods_change_log') . " WHERE log_id = '$log_id' ");
                $res = unserialize($res);
                if ($res) {
                    foreach ($res as $k => $v) {
    ...
    protected function get_order_no_comment_goods($ru_id = 0, $sign = 0)
    {
        ...
        if ($sign == 0) {
            $where .= " AND (SELECT count(*) FROM " . $this->sanmu->table('comment') . " AS c WHERE c.comment_type = 0 AND c.id_value = g.goods_id AND c.rec_id = og.rec_id AND c.parent_id = 0 AND c.ru_id = '$ru_id') = 0 ";
        }
        //记录总数
        $sql = "SELECT count(*) FROM " . $this->sanmu->table('order_goods') . " AS og " .
            "LEFT JOIN " . $this->sanmu->table('order_info') . " AS oi ON og.order_id = oi.order_id " .
            "LEFT JOIN  " . $this->sanmu->table('goods') . " AS g ON og.goods_id = g.goods_id " .
            "WHERE og.ru_id = '$ru_id' $where ";
        $filter['record_count'] = $this->db->getOne($sql);
        /* 分页大小 */
        $filter = page_and_size($filter);
        $sql = "SELECT og.*, oi.*,g.goods_thumb, u.user_name FROM " . $this->sanmu->table('order_goods') . " AS og " .
            "LEFT JOIN " . $this->sanmu->table('order_info') . " AS oi ON og.order_id = oi.order_id " .
            "LEFT JOIN  " . $this->sanmu->table('goods') . " AS g ON og.goods_id = g.goods_id " .
            "LEFT JOIN  " . $this->sanmu->table('users') . " AS u ON u.user_id = oi.user_id " .
            "WHERE og.ru_id = '$ru_id' $where " .
    ...
    {
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['sanmu']->table('order_info') . " AS oi WHERE " .
            "(SELECT order_id FROM " . $GLOBALS['sanmu']->table('order_goods') . " AS og WHERE og.goods_id = '$goods_id' LIMIT 1) = oi.order_id";
