...
class CommentManageController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'single_reply') {
            ...
            $this->smarty->assign('primary_cat', $GLOBALS['_LANG']['02_cat_and_goods']);
            /* 获取评论详细信息并进行字符处理 */
            $sql = $sql = "SELECT * FROM " . $this->sanmu->table('single') . " WHERE single_id = '$_REQUEST[id]'";
            ...
            /* 获得图片 */
            $sql = $sql = "SELECT id, img_file, cont_desc FROM " . $this->sanmu->table('single_sun_images') . " WHERE single_id = '$id' order by id DESC";
            ...
            /* 获取管理员的用户名和Email地址 */
            $sql = "SELECT user_name, email FROM " . $this->sanmu->table('admin_user') .
        ...
        elseif ($_REQUEST['act'] == 'drop_single_image') {
            ...
            /* 删除图片文件 */
            $sql = "SELECT img_file  " . " FROM " . $this->sanmu->table('single_sun_images') .
                " WHERE id = '$img_id'";
            ...
            /* 删除数据 */
            $sql = "DELETE FROM " . $this->sanmu->table('single_sun_images') . " WHERE id = '$img_id'";
            $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'single_check') {
            ...
            if ($_REQUEST['check'] == 'allow') {
                $sql = "UPDATE " . $this->sanmu->table('order_goods') . " SET is_single = 2 WHERE order_id = '$order_id' AND goods_id = '$goods_id'";
                $this->db->query($sql);
                $sql = "UPDATE " . $this->sanmu->table('single') . " SET is_audit = 1, integ='$integ' WHERE order_id = '$order_id'";
                $this->db->query($sql);
                ...
                return sanmu_header("Location: comment_manage.php?act=single_list\n");
            } else {
                $sql = "UPDATE " . $this->sanmu->table('order_goods') . " SET is_single = 3 WHERE order_id = '$order_id' AND goods_id = '$goods_id'";
                $this->db->query($sql);
                $sql = "UPDATE " . $this->sanmu->table('single') . " SET is_audit = 0, integ='-$integ' WHERE order_id = '$order_id'";
                $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'reply') {
            ...
            /* 获取评论详细信息并进行字符处理 */
            $sql = "SELECT * FROM " . $this->sanmu->table('comment') . " WHERE comment_id = '$comment_id'";
            ...
            //晒单图片
            $sql = "SELECT comment_img, img_thumb FROM " . $this->sanmu->table('comment_img') . " WHERE comment_id = '$comment_id'";
            ...
            /* 获得评论回复内容 */
            $sql = "SELECT * FROM " . $this->sanmu->table('comment') . " WHERE parent_id = '$comment_id'" .
            ...
            if ($comment_info['comment_type'] == 0) {
                ...
                $sql = "SELECT title FROM " . $this->sanmu->table('article') .
        ...
        elseif ($_REQUEST['act'] == 'action') {
            ...
            /* 获取管理员的用户名和Email地址 */
            $sql = "SELECT user_id, ru_id FROM " . $this->sanmu->table('admin_user') .
                " WHERE user_id = '" . session('seller_id') . "'";
            $admin_info = $this->db->getRow($sql);

            /* 获取IP地址 */
            $ip = $this->sanmuRepository->sanmuIp();

            $comment_id = isset($_REQUEST['comment_id']) && !empty($_REQUEST['comment_id']) ? intval($_REQUEST['comment_id']) : 0;
            $comment_info = $this->db->getRow("SELECT comment_id,ru_id FROM " . $this->sanmu->table('comment') . " WHERE comment_id = '$comment_id' AND ru_id='" . $adminru['ru_id'] . "'");

            /* 获得评论是否有回复 */
            $sql = "SELECT comment_id,content,parent_id,ru_id FROM " . $this->sanmu->table('comment') .
            ...
            if (!empty($reply_info['content']) && $adminru['ru_id'] == $comment_info['ru_id']) {
                /* 更新回复的内容 */
                $sql = "UPDATE " . $this->sanmu->table('comment') . " SET " .
                    "email     = '$email', " .
                    "user_name = '$user_name', " .
                    "content   = '$content', " .
                    "add_time  =  '" . gmtime() . "', " .
                ...
                $sql = "INSERT INTO " . $this->sanmu->table('comment') . " (comment_type, id_value, email, user_name , " .
            ...
            $sql = "UPDATE " . $this->sanmu->table('comment') . " SET status = 1 WHERE comment_id = '$comment_id'";
            $this->db->query($sql);
            ...
            if (!empty($send_email_notice) || (isset($remail) && !empty($remail))) {
                ...
                $this->smarty->assign('shop_name', "<a href='" . $this->sanmu->seller_url() . "'>" . $GLOBALS['_CFG']['shop_name'] . '</a>');
            ...
            return sanmu_header("Location: comment_manage.php?act=reply&id=$comment_id&send_ok=$send_ok\n");
        }
        /*------------------------------------------------------ */
        //-- 更新评论的状态为显示或者        禁止
        /*------------------------------------------------------ */
        elseif ($_REQUEST['act'] == 'check') {
            ...
            if ($_REQUEST['check'] == 'allow') {
                ...
                $sql = 'SELECT id_value FROM ' . $this->sanmu->table('comment') . " WHERE comment_id = '$comment_id'";
                $goods_id = $this->db->getOne($sql);

                $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('comment') . " WHERE id_value = '$goods_id' AND comment_type = 0 AND status = 1 AND parent_id = 0 ";
                $count = $this->db->getOne($sql);


                $sql = "UPDATE " . $this->sanmu->table('goods') . " SET comments_number = '$count' WHERE goods_id = '$goods_id'";
                $this->db->query($sql);

                /* 清除缓存 */
                clear_cache_files();

                return sanmu_header("Location: comment_manage.php?act=reply&id=$comment_id\n");
            } else {
                /* 禁止评论显示 */
                $sql = "UPDATE " . $this->sanmu->table('comment') . " SET status = 0 WHERE comment_id = '$comment_id'";
                $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'single_remove') {
            ...
            $sql = "SELECT order_id FROM " . $this->sanmu->table('single') . " WHERE single_id = '$single_id'";
            $res = $this->db->getRow($sql);
            $order_id = $res['order_id'];
            $this->db->query("UPDATE " . $this->sanmu->table('order_info') . " SET is_single='4'" . " WHERE order_id = '$order_id'");
            $sql = "DELETE FROM " . $this->sanmu->table('single') . " WHERE single_id = '$single_id'";
            $res = $this->db->query($sql);
            if ($res) {
                $this->db->query("DELETE FROM " . $this->sanmu->table('goods_gallery') . " WHERE single_id = '$single_id'");
            }
        ...
        elseif ($_REQUEST['act'] == 'remove') {
            ...
            if ($img) {
                for ($i = 0; $i < count($img); $i++) {
                    ...
                    $this->sanmuRepository->getOssDelFile([$img[$i]['comment_img'], $img[$i]['img_thumb']]);
                }
            }

            $sql = "DELETE FROM " . $this->sanmu->table('comment_img') . " WHERE comment_id = '$comment_id'";
            $res = $this->db->query($sql);

            $sql = "DELETE FROM " . $this->sanmu->table('comment') . " WHERE comment_id = '$comment_id'";
            $res = $this->db->query($sql);
            if ($res) {
                $this->db->query("DELETE FROM " . $this->sanmu->table('comment') . " WHERE parent_id = '$comment_id'");
            }
            ...
            if (isset($_POST['checkboxes'])) {
                switch ($action) {
                    case 'remove':
                        ...
                        $sql = "SELECT comment_img, img_thumb FROM " . $this->sanmu->table('comment_img') . " WHERE " . db_create_in($_POST['checkboxes'], 'comment_id');
                        $img = $this->db->getAll($sql);
                        ...
                        $this->db->query("DELETE FROM " . $this->sanmu->table('comment_img') . " WHERE " . db_create_in($_POST['checkboxes'], 'comment_id'));
                        $this->db->query("DELETE FROM " . $this->sanmu->table('comment') . " WHERE " . db_create_in($_POST['checkboxes'], 'comment_id'));
                        $this->db->query("DELETE FROM " . $this->sanmu->table('comment') . " WHERE " . db_create_in($_POST['checkboxes'], 'parent_id'));
                        break;

                    case 'allow':
                        $this->db->query("UPDATE " . $this->sanmu->table('comment') . " SET status = 1  WHERE " . db_create_in($_POST['checkboxes'], 'comment_id'));
                        break;

                    case 'deny':
                        $this->db->query("UPDATE " . $this->sanmu->table('comment') . " SET status = 0  WHERE " . db_create_in($_POST['checkboxes'], 'comment_id'));
                        break;
    ...
    private function get_comment_list($ru_id)
    {
        ...
        //ecmoban模板堂 --zhuo start
        $sql = "select user_id from " . $this->sanmu->table('merchants_shop_information') . " where shoprz_brandName LIKE '%" . mysql_like_quote($filter['keywords']) . "%' OR shopNameSuffix LIKE '%" . mysql_like_quote($filter['keywords']) . "%'";
        ...
        //ecmoban模板堂 --zhuo end
        if ($filter['reply']) {
            $where .= " AND c1.order_id > 0 AND c1.comment_type = 0 AND (SELECT count(*) FROM " . $this->sanmu->table('comment') . " AS c2 WHERE c2.parent_id = c1.comment_id LIMIT 1) < 1";
        }
        $where .= " AND (c1.parent_id = 0 OR (c1.parent_id > 0 AND c1.user_id > 0))";

        $sql = "SELECT count(*) FROM " . $this->sanmu->table('comment') . " AS c1 WHERE $where";
        ...
        /* 获取评论数据 */
        $arr = [];
        $sql = "SELECT * FROM " . $this->sanmu->table('comment') . " AS c1 WHERE $where " .
        ...
        foreach ($res as $row) {
            if ($row['comment_type'] == 2) {
                $sql = "SELECT goods_name FROM " . $this->sanmu->table('goods') . " WHERE goods_id='$row[id_value]'";
                ...
                $sql = ($row['comment_type'] == 0) ?
                    "SELECT goods_name FROM " . $this->sanmu->table('goods') . " WHERE goods_id='$row[id_value]'" :
                    "SELECT title FROM " . $this->sanmu->table('article') . " WHERE article_id='$row[id_value]'";
                $row['title'] = $this->db->getOne($sql);
            }
            ...
            if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                $row['email'] = $this->sanmuRepository->stringToStar($row['email']);
    ...
    private function get_single_list($ru_id)
    {
        ...
        $sql = "SELECT s.* FROM " . $this->sanmu->table('single') . " as s, " . $this->sanmu->table('goods') . " as g " . " WHERE s.goods_id = g.goods_id AND 1=1 $where ";
        ...
        /* 获取晒单列表 */
        $arr = [];
        $sql = "SELECT s.*, g.user_id as ru_id FROM " . $this->sanmu->table('single') . " as s, " . $this->sanmu->table('goods') . " as g " . " WHERE s.goods_id = g.goods_id AND 1=1 $where " .
            " ORDER BY $filter[sort_by] $filter[sort_order] " .
            " LIMIT " . $filter['start'] . ", $filter[page_size]";
        $res = $this->db->query($sql);
        ...
        foreach ($res as $row) {
            $sql = "SELECT goods_name FROM " . $this->sanmu->table('goods') . " WHERE goods_id='$row[goods_id]'";
