        ...
        elseif ($_REQUEST['act'] == 'ajax_print') {
            ...
            if ($order_sn) {
                ...
                if (empty($order)) {
                    ...
                    //商家店铺信息打印到订单和快递单上
                    $sql = "select shop_name,country,province,city,district,shop_address,kf_tel from " . $this->sanmu->table('seller_shopinfo') . " where ru_id='" . $adminru['ru_id'] . "'";
                    ...
                    if ($shipping['print_model'] == 2) {
                        ...
                        $sql = "select oi.ru_id from " . $this->sanmu->table('order_info') . " as oi " .
                            " where oi.order_id = '" . $order['order_id'] . "'";
                        $ru_id = $this->db->getOne($sql);

                        if ($ru_id > 0) {
                            $sql = "select shoprz_brandName, shopNameSuffix from " . $this->sanmu->table('merchants_shop_information') . " where user_id = '$ru_id'";
