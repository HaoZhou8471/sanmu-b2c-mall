...
class OrderDelayController extends InitController
{
    public function index()
    {
        ...
        //-- 延迟收货申请列表
        /*------------------------------------------------------ */
        $exc = new Exchange($this->sanmu->table("order_delayed"), $this->db, 'delayed_id', 'apply_day', 'order_id');//
        ...
        elseif ($_REQUEST['act'] == 'batch') {
            ...
            if (isset($_POST['type'])) {
                ...
                if ($_POST['type'] == 'batch_remove') {
                    $sql = "DELETE FROM " . $this->sanmu->table('order_delayed') .
                ...
                elseif ($_POST['type'] == 'review_to') {
                    ...
                    // 查询是否有已审核的订单
                    $sql = "SELECT od.review_status,od.apply_day,o.order_sn ,od.delayed_id FROM " . $this->sanmu->table('order_delayed') . "AS od"
                        . " LEFT JOIN " . $this->sanmu->table('order_info') . "AS o ON od.order_id = o.order_id WHERE od.delayed_id " . db_create_in($delay_id_arr);
                    ...
                    $sql = "UPDATE " . $this->sanmu->table('order_delayed') . " SET review_status = '$review_status', review_time = '$time', review_admin = '" . session('seller_id') . "' "
                        . " WHERE delayed_id " . db_create_in($delay_id_arr);

                    if ($this->db->query($sql)) {
                        // 更新订单表的确认收货天数
                        $sql = "SELECT order_id, apply_day FROM " . $this->sanmu->table('order_delayed') . " WHERE delayed_id " . db_create_in($delay_id_arr);
                        $order_id_list = $this->db->getAll($sql);
                        foreach ($order_id_list as $key => $value) {
                            $sql = "UPDATE " . $this->sanmu->table('order_info') . "SET auto_delivery_time=auto_delivery_time+'$value[apply_day]' WHERE order_id='$value[order_id]'";
                            $this->db->query($sql);
                        }
