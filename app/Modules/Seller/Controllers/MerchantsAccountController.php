    ...
    public function index()
    {
        ...
        if ($act == 'account_manage') {
            ...
            elseif ($act_type == 'topup_pay') {
                ...
                $sql = "SELECT * FROM " . $this->sanmu->table('seller_account_log') . " WHERE log_id = '$log_id' LIMIT 1";
                $account_log = $this->db->getRow($sql);

                $sql = "SELECT * FROM " . $this->sanmu->table('pay_log') . " WHERE order_id = '$log_id' AND order_type = '" . PAY_TOPUP . "' LIMIT 1";
                ...
                if ($payment_info['pay_code'] == 'balance') {
                    //查询出当前用户的剩余余额;
                    $user_money = $this->db->getOne("SELECT user_money FROM " . $this->sanmu->table('users') . " WHERE user_id='" . $account_log['ru_id'] . "'");
                    //如果用户余额足够支付订单;
                    if ($user_money >= $account_log['amount']) {

                        /* 改变商家金额 */
                        $sql = " UPDATE " . $this->sanmu->table('seller_shopinfo') . " SET seller_money = seller_money + " . $account_log['amount'] . " WHERE ru_id = '" . $account_log['ru_id'] . "'";
                        $this->db->query($sql);

                        /* 修改申请的支付状态 */
                        $sql = " UPDATE " . $this->sanmu->table('seller_account_log') . " SET is_paid = 1, pay_time = '" . gmtime() . "' WHERE log_id = '$log_id'";
                        $this->db->query($sql);

                        load_helper('clips');

                        /* 改变会员金额 */
                        $sql = " UPDATE " . $this->sanmu->table('users') . " SET user_money = user_money - " . $account_log['amount'] . " WHERE user_id = '" . $account_log['ru_id'] . "'";
                        $this->db->query($sql);
                        ...
                        $this->db->autoExecute($this->sanmu->table('account_log'), $user_account_log, 'INSERT');

                        //记录支付log
                        $sql = "UPDATE " . $this->sanmu->table('pay_log') . "SET is_paid = 1 WHERE order_id = '$log_id' AND order_type = '" . PAY_TOPUP . "'";
                        $this->db->query($sql);
        ...
        elseif ($act == 'checkorder') {
            ...
            $sql = "SELECT is_paid FROM " . $this->sanmu->table('seller_account_log') . " WHERE log_id = '$apply_id' LIMIT 1";
        ...
        elseif ($act == 'account_edit') {
            ...
            if ($is_insert == 'insert') {
                ...
                $this->db->autoExecute($this->sanmu->table('users_real'), $other, 'INSERT');
            } else {
                ...
                $this->db->autoExecute($this->sanmu->table('users_real'), $other, 'UPDTAE', "user_id = '" . $adminru['ru_id'] . "' AND user_type = 1");
            }
        ...
        elseif ($act == 'deposit_insert') {
            ...
            //商家申请提现记录
            $this->db->autoExecute($this->sanmu->table('seller_account_log'), $other, 'INSERT');
        ...
        elseif ($act == 'del_pay') {
            ...
            $sql = "DELETE FROM " . $this->sanmu->table('seller_account_log') . " WHERE log_id = '$log_id'";
            $this->db->query($sql);
