...
class GoodsBookingController extends InitController
{
    ...
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'remove') {
            ...
            $this->db->query("DELETE FROM " . $this->sanmu->table('booking_goods') . " WHERE rec_id='$id'");
        ...
        if ($_REQUEST['act'] == 'update') {
            ...
            $sql = "UPDATE  " . $this->sanmu->table('booking_goods') .
            ...
            if (!empty($_POST['send_email_notice']) or isset($_POST['remail'])) {
                ...
                $sql = 'SELECT bg.email, bg.link_man, bg.goods_id, g.goods_name ' .
                    'FROM ' . $this->sanmu->table('booking_goods') . ' AS bg, ' . $this->sanmu->table('goods') . ' AS g ' .
                    "WHERE bg.goods_id = g.goods_id AND bg.rec_id = '$rec_id'";
                $booking_info = $this->db->getRow($sql);

                /* 设置缺货回复模板所需要的内容信息 */
                $template = get_mail_template('goods_booking');
                $goods_link = $this->sanmu->seller_url() . 'goods.php?id=' . $booking_info['goods_id'];
            ...
            return sanmu_header("Location: ?act=detail&id=" . $rec_id . "&send_ok=$send_ok\n");
        }
    }
    ...
    private function get_booking_info($id)
    {
        $sql = "SELECT bg.rec_id, bg.user_id, IFNULL(u.user_name, '" . $GLOBALS['_LANG']['guest_user'] . "') AS user_name, " .
            ...
            "FROM " . $this->sanmu->table('booking_goods') . " AS bg " .
            "LEFT JOIN " . $this->sanmu->table('goods') . " AS g ON g.goods_id=bg.goods_id " .
            "LEFT JOIN " . $this->sanmu->table('users') . " AS u ON u.user_id=bg.user_id " .
            "WHERE bg.rec_id ='$id'";
