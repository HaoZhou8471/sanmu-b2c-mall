...
class MerchantsTemplateController extends InitController
{
    public function index()
    {
        ...
        $sql = "select id,seller_theme,store_style from " . $this->sanmu->table('seller_shopinfo') . " where ru_id = '" . $adminru['ru_id'] . "'";
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            if ($adminru['ru_id'] > 0) {
                $sql = "SELECT sg.seller_temp FROM" . $this->sanmu->table('seller_grade') . " AS sg LEFT JOIN " . $this->sanmu->table('merchants_grade') . " AS mg ON sg.id = mg.grade_id WHERE mg.ru_id = '$adminru[ru_id]'";
        ...
        if ($_REQUEST['act'] == 'install') {
            ...
            $custom_dirname = $this->sanmu->seller_url();

            //正则去掉js代码
            $preg = "/<script[\s\S]*?<\/script>/i";

            $template_info = $this->get_seller_template_info($tpl_name);
            $sql = "UPDATE " . $this->sanmu->table('seller_shopinfo') . " SET seller_theme = '$tpl_name', store_style = '$tpl_fg', win_goods_type = '" . $template_info['win_goods_type'] . "'" .
                " WHERE ru_id = '" . $adminru['ru_id'] . "'";
            $step_install = $this->db->query($sql, 'SILENT');

            $sql = " delete from " . $this->sanmu->table('seller_shopheader') . " where seller_theme='$tpl_name' and ru_id = '" . $adminru['ru_id'] . "'";
            $this->db->query($sql);

            //安装模板装修代码
            //店铺头部装修
            //检测是否设置过此模板的头部
            $sql = "select count(*) as count, content from " . $this->sanmu->table('seller_shopheader') . " where seller_theme='$tpl_name' and ru_id = '" . $adminru['ru_id'] . "'";
            ...
            if ($header_info['count'] == 0) {
                ...
                if (file_exists($header_path)) {
                    ...
                    if (strlen($header_content) >= 3) {//默认有3个字符
                        ...
                        $sql = "insert into" . $this->sanmu->table('seller_shopheader') . "(content,seller_theme,ru_id) values ('$header_content','$tpl_name'," . $adminru['ru_id'] . ")";
                ...
                if (file_exists($header_path)) {
                    ...
                    if (strlen($header_content) >= 3) {//默认有3个字符
                        ...
                        $header_content = preg_replace($patterns, $replacements, $header_content);
                        $sql = "UPDATE " . $this->sanmu->table('seller_shopheader') . " SET content = '$header_content' WHERE seller_theme = '$tpl_name' AND ru_id = '" . $adminru['ru_id'] . "'";
                        $this->db->query($sql);
                    }
                }
            }
            ...
            //模板幻灯片安装
            //检测是否设置过此模板的幻灯片
            $sql = "select count(id) from " . $this->sanmu->table('seller_shopslide') . " where seller_theme='$tpl_name' and ru_id = '" . $adminru['ru_id'] . "'";
            ...
            if ($count == 0) {
                ...
                if (file_exists($silde_path)) {
                    ...
                    if ($slide_arr) {
                        $sql = "insert into " . $this->sanmu->table('seller_shopslide') . " (ru_id,img_url,img_link,img_desc,is_show,seller_theme,install_img) values ";
            ...
            //橱窗自定义装饰代码
            //检测是否设置过此模板的自定义区域
            $sql = "select count(*) from " . $this->sanmu->table('seller_shopwindow') . " where seller_theme='$tpl_name' and win_type=0 and ru_id = '" . $adminru['ru_id'] . "'";
            $count = $this->db->getOne($sql);
                    ...
                    if ($file != '.' && $file != '..' && !is_dir(storage_public('seller_themes/' . $file))) {
                        ...
                        if ($ext['extension'] == 'txt') {
                            ...
                            if (strlen($custom_content) >= 3) {//默认有3个字符
                                ...
                                $sql = "insert into" . $this->sanmu->table('seller_shopwindow') . "(win_type,win_name,win_order,ru_id,is_show,win_custom,seller_theme) values ('0','$cus_name','$win_order'," . $adminru['ru_id'] . ",1,'$custom_content','$tpl_name')";
        ...
        elseif ($_REQUEST['act'] == 'user_default') {
            $adminru = get_admin_ru_id();
            $sql = "UPDATE " . $this->sanmu->table('seller_shopinfo') . " SET seller_theme = '' WHERE ru_id = '" . $adminru['ru_id'] . "'";
            $this->db->query($sql);
