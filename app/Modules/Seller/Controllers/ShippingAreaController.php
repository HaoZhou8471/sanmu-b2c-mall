    ...
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'edit') {
            ...
            if ($this->db->getOne($sql) > 0) {
                ...
                $shipping_code = $this->db->getOne("SELECT shipping_code FROM " . $this->sanmu->table('shipping') . " WHERE shipping_id='$_POST[shipping]'");
                $plugin = '../includes/modules/shipping/' . $shipping_code . ".php";
                ...
                /* 清除原有的城市和地区 */
                $this->db->query("DELETE FROM " . $this->sanmu->table("area_region") . " WHERE shipping_area_id='$_POST[id]' and ru_id='" . $adminru['ru_id'] . "'");
                ...
                if ($shipping_code == "cac") {
                    ...
                    $sql = "INSERT INTO " . $this->sanmu->table('area_region') . " (shipping_area_id, region_id) VALUES ('$_POST[id]', '$district')";
                    $this->db->query($sql);
                } else {
                    /* 添加选定的城市和地区 */
                    foreach ($selected_regions as $key => $val) {
                        $sql = "INSERT INTO " . $this->sanmu->table('area_region') . " (shipping_area_id, region_id,ru_id) VALUES ('$_POST[id]', '$val','" . $adminru['ru_id'] . "')";
                        $this->db->query($sql);
                    }
                }
