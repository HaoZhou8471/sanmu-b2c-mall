...
class CategoryStoreController extends InitController
{
    ...
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            if (isset($_REQUEST['back_level']) && $_REQUEST['back_level'] > 0) {
                ...
                $_REQUEST['parent_id'] = $this->db->getOne("SELECT parent_id FROM " . $this->sanmu->table('merchants_category') . " WHERE cat_id = '" . $_REQUEST['parent_id'] . "'", true);
            } else {
        ...
        if ($_REQUEST['act'] == 'insert') {
            ...
            if (!empty($_FILES['touch_icon']['name'])) {
                ...
                if (!empty($cat_id)) {
                    ...
                    sanmu_unlink(storage_public($cat_info['touch_icon']));
                    $this->sanmuRepository->getOssDelFile([$cat_info['touch_icon']]);
                }
            }
                ...
                if ($cat['show_in_nav'] != $dat['show_in_nav']) {
                    ...
                    if ($cat['show_in_nav'] == 1) {
                        ...
                        if (empty($nid)) {
                            ...
                            $vieworder = $vieworder->max_vieworder ?? 0;
                            $vieworder += 2;
                            $uri = $this->sanmuRepository->buildUri('merchants_store', ['urid' => $adminru['ru_id'], 'cid' => $cat_id], $cat['cat_name']);
        ...
        if ($_REQUEST['act'] == 'move_cat') {
            ...
            /* 更新商品分类 */
            $sql = "UPDATE " . $this->sanmu->table('goods') . " SET user_cat = '$target_cat_id' " .
    ...
    private function cat_update($cat_id, $args)
    {
        ...
        return $this->db->autoExecute($this->sanmu->table('merchants_category'), $args, 'update', "cat_id='$cat_id'");
    }
