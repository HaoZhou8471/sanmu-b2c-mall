...
class ComplaintController extends InitController
{
    ...
    public function index()
    {
        ...
        $exc = new Exchange($this->sanmu->table("complaint"), $this->db, 'complaint_id', 'title_id');
        ...
        elseif ($_REQUEST['act'] == 'upload_img') {
            ...
            $this->sanmuRepository->getOssAddFile([$img_file]);
            ...
            $sql = "SELECT count(*) FROM " . $this->sanmu->table('appeal_img') . " WHERE complaint_id = '$complaint_id' AND order_id = '$order_id'";
            $img_count = $this->db->getOne($sql);

            if ($img_count < 5 && $img_file) {
                $this->db->autoExecute($this->sanmu->table('appeal_img'), $report, 'INSERT');
        ...
        elseif ($_REQUEST['act'] == 'del_img') {
            ...
            if ($img_id > 0) {
                $sql = "SELECT img_file FROM" . $this->sanmu->table('appeal_img') . " WHERE img_id = '$img_id' AND complaint_id = '$complaint_id' LIMIT 1";
                ...
                $sql = "DELETE FROM " . $this->sanmu->table('appeal_img') . " WHERE img_id = '$img_id'";
                $this->db->query($sql);
            } else {
                $result['error'] = "1";
        ...
        elseif ($_REQUEST['act'] == 'appeal_submit') {
            ...
            $sql = " UPDATE" . $this->sanmu->table('complaint') . " SET complaint_state = complaint_state+1 $set WHERE complaint_id = '$complaint_id'";
            $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'talk_release') {
            ...
            if ($type == 0) {
                ...
                $this->db->autoExecute($this->sanmu->table('complaint_talk'), $complaint_talk, 'INSERT');
            }
            $talk_list = checkTalkView($complaint_id, 'seller');
            $this->smarty->assign('talk_list', $talk_list);
    ...
    private function get_complaint_list()
    {
        ...
        if ($result === false) {
            ...
            /* 查询记录总数，计算分页数 */
            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('complaint') . $where;
            ...
            $sql = "SELECT complaint_id,order_id,order_sn,user_id,user_name,ru_id,shop_name,title_id,complaint_content,add_time,complaint_handle_time,"
                . "admin_id,appeal_messg,appeal_time,end_handle_time,end_admin_id,complaint_state,complaint_active FROM" . $this->sanmu->table('complaint')
        ...
        foreach ($res as $rows) {
            if ($rows['title_id'] > 0) {
                $sql_title = "SELECT title_name FROM " . $this->sanmu->table("complain_title") . "WHERE title_id = '" . $rows['title_id'] . "'";
                $rows['title_name'] = $this->db->getOne($sql_title);
            }
            //获取举报图片列表
            $sql = "SELECT img_file ,img_id FROM " . $GLOBALS["sanmu"]->table('complaint_img') . " WHERE complaint_id = '" . $rows['complaint_id'] . "' ORDER BY  img_id DESC";
            ...
            if ($rows['complaint_state'] > 1) {
                $sql = "SELECT view_state FROM" . $this->sanmu->table('complaint_talk') . "WHERE complaint_id='" . $rows['complaint_id'] . "' ORDER BY talk_time DESC";
