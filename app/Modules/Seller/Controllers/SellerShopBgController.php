...
class SellerShopBgController extends InitController
{
    ...
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'first') {
            ...
            $sql = "select * from " . $this->sanmu->table('seller_shopbg') . " where ru_id='" . $adminru['ru_id'] . "' and seller_theme='$seller_theme'";
        ...
        elseif ($_REQUEST['act'] == 'second') {
            ...
            $this->sanmuRepository->getOssAddFile([$shop_bg['bgimg']]);

            if ($data_op == 'add') {
                $this->db->autoExecute($this->sanmu->table('seller_shopbg'), $shop_bg, 'INSERT');
                ...
                $this->db->autoExecute($this->sanmu->table('seller_shopbg'), $shop_bg, 'update', "ru_id='" . $adminru['ru_id'] . "' and seller_theme='$seller_theme'");
                $lnk[] = ['text' => $GLOBALS['_LANG']['go_back'], 'href' => 'seller_shop_bg.php?act=first'];
