...
class GoodsTransportController extends InitController
{
    ...
    public function index()
    {
        ...
        $exc = new Exchange($this->sanmu->table('goods_transport'), $this->db, 'tid', 'title');
        $exc_extend = new Exchange($this->sanmu->table('goods_transport_extend'), $this->db, 'id', 'tid');
        $exc_express = new Exchange($this->sanmu->table('goods_transport_express'), $this->db, 'id', 'tid');
        ...
        elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit') {
            ...
            $sql = " SELECT * FROM " . $this->sanmu->table('goods_transport') . " WHERE tid = '$tid' ";
        ...
        elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
            ...
            if ($data['freight_type'] > 0) {
                if (session()->has($s_tid . '.tpl_id') && empty(session()->get($s_tid . '.tpl_id'))) {
                    $sql = "SELECT GROUP_CONCAT(id) AS id FROM " . $this->sanmu->table('goods_transport_tpl') . " WHERE " . $where;
        ...
        elseif ($_REQUEST['act'] == 'remove') {
            ...
            //删除拓展数据
            $sql = " DELETE FROM " . $this->sanmu->table('goods_transport_extend') . " WHERE tid = '$id' ";
            $this->db->query($sql);

            $sql = " DELETE FROM " . $this->sanmu->table('goods_transport_express') . " WHERE tid = '$id' ";
            $this->db->query($sql);

            $sql = " DELETE FROM " . $this->sanmu->table('goods_transport_tpl') . " WHERE tid = '$id' ";
            $this->db->query($sql);

            $sql = " UPDATE " . $this->sanmu->table('goods') . " SET tid = 0 WHERE tid = '$id' ";
            $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'add_area') {
            ...
            $this->db->autoExecute($this->sanmu->table('goods_transport_extend'), $data, 'INSERT');
        ...
        elseif ($_REQUEST['act'] == 'save_area') {
            ...
            $this->db->autoExecute($this->sanmu->table('goods_transport_extend'), $data, "UPDATE", "id = '$id'");
            $this->smarty->assign('transport_area', $this->get_transport_area($tid));
            $html = $this->smarty->fetch('library/goods_transport_area.lbi');
            return make_json_result($html);
        }
        ...
        elseif ($_REQUEST['act'] == 'add_express') {
            ...
            $data['ru_id'] = $adminru['ru_id'];
            $data['admin_id'] = $admin_id;
            $this->db->autoExecute($this->sanmu->table('goods_transport_express'), $data, 'INSERT');
        ...
        elseif ($_REQUEST['act'] == 'edit_express') {
            ...
            $express_disabled = '';
            $sql = " SELECT shipping_id FROM " . $this->sanmu->table('goods_transport_express') . " WHERE tid='$tid' $where AND id!='$id' ";
        ...
        elseif ($_REQUEST['act'] == 'save_express') {
            ...
            $this->db->autoExecute($this->sanmu->table('goods_transport_express'), $data, "UPDATE", "id = '$id'");
        ...
        elseif ($_REQUEST['act'] == 'get_shipping_tem') {
            ...
            $sql = "SELECT a.shipping_name, a.shipping_code, a.support_cod, b.* " .
                " FROM " . $this->sanmu->table('goods_transport_tpl') . " AS b " .
            ...
            if (!empty($row['region_id'])) {
                $sql = " SELECT region_id,region_name from " . $this->sanmu->table('region') . " where region_id in (" . $row['region_id'] . ") ";
        ...
        elseif ($_REQUEST['act'] == 'the_national') {
            ...
            $sql = "SELECT GROUP_CONCAT(region_id) AS region_id FROM " . $this->sanmu->table('goods_transport_tpl') . " WHERE user_id = '" . $adminru['ru_id'] . "'" .
                " AND tid = '$tid' AND shipping_id = '$shipping_id'";
            $region_list = $this->db->getOne($sql);
            $region_list = !empty($region_list) ? explode(",", $region_list) : [];

            $sql = "SELECT GROUP_CONCAT(region_id) AS region_id FROM " . $this->sanmu->table('region') . " WHERE 1";
        ...
        elseif ($_REQUEST['act'] == 'add_shipping_tpl') {
            ...
            if ($shipping_id == 0 || empty($regionId)) {
                ...
                $sql = "SELECT count(*) FROM " . $this->sanmu->table('goods_transport_tpl') . " WHERE id = '$id'";
                $res = $this->db->getOne($sql);
                if ($res > 0) {
                    $this->db->autoExecute($this->sanmu->table('goods_transport_tpl'), $other, 'UPDATE', "id = '$id'");
                } else {
                    $other['admin_id'] = $admin_id;
                    $this->db->autoExecute($this->sanmu->table('goods_transport_tpl'), $other, 'INSERT');
        ...
        elseif ($_REQUEST['act'] == 'drop_shipping') {
            ...
            $sql = "DELETE FROM" . $this->sanmu->table("goods_transport_tpl") . "WHERE id = '$id'";
            $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'select_area') {
            ...
            $region = $this->sanmuRepository->delStrComma($region);
    ...
    private function get_transport_list($ru_id = 0)
    {
        ...
        /* 查询记录总数，计算分页数 */
        $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('goods_transport') . $where;
        $filter['record_count'] = $this->db->getOne($sql);
        $filter = page_and_size($filter);

        /* 查询记录 */
        $sql = "SELECT * FROM " . $this->sanmu->table('goods_transport') . $where .
            " ORDER BY tid DESC";
    ...
    private function get_transport_area($tid = 0)
    {
        ...
        $sql = " SELECT * FROM " . $this->sanmu->table('goods_transport_extend') . " WHERE tid = '$tid' $where ORDER BY id DESC ";
        ...
        foreach ($transport_area as $key => $val) {
            if (!empty($val['top_area_id']) && !empty($val['area_id'])) {
                ...
                foreach ($top_area_arr as $k => $v) {
                    ...
                    $sql = " SELECT region_name FROM " . $this->sanmu->table('region') . " WHERE parent_id = '$v' AND region_id IN ($val[area_id]) ";
    ...
    private function get_area_list($area_id = '')
    {
        $area_list = '';
        if (!empty($area_id)) {
            $sql = " SELECT region_name FROM " . $this->sanmu->table('region') . " WHERE region_id IN ($area_id) ";
    ...
    private function get_transport_express($tid = 0)
    {
        ...
        $sql = " SELECT * FROM " . $this->sanmu->table('goods_transport_express') . " WHERE tid = '$tid' $where ORDER BY id DESC ";
    ...
    private function get_express_list($shipping_id = '')
    {
        ...
        if (!empty($shipping_id)) {
            $sql = " SELECT shipping_name FROM " . $this->sanmu->table('shipping') . " WHERE shipping_id IN ($shipping_id) ";
