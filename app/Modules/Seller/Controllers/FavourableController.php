...
class FavourableController extends InitController
{
    ...
    public function index()
    {
        ...
        $exc = new Exchange($this->sanmu->table('favourable_activity'), $this->db, 'act_id', 'act_name');
        ...
        elseif ($_REQUEST['act'] == 'remove') {
            ...
            $this->sanmuRepository->getDelBatch('', $id, ['activity_thumb'], 'act_id', FavourableActivity::whereRaw(1), 1); //删除图片
        ...
        elseif ($_REQUEST['act'] == 'batch') {
            ...
            if (empty($_POST['checkboxes'])) {
                ...
                if (isset($_POST['drop'])) {
                    $this->sanmuRepository->getDelBatch($ids, '', ['activity_thumb'], 'act_id', FavourableActivity::whereRaw(1), 1);

                    /* 删除记录 */
                    $sql = "DELETE FROM " . $this->sanmu->table('favourable_activity') .
        ...
        elseif ($_REQUEST['act'] == 'edit_sort_order') {
            ...
            $sql = "UPDATE " . $this->sanmu->table('favourable_activity') .
                " SET sort_order = '$val'" .
        ...
        elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit') {
            ...
            if ($favourable['act_range'] != FAR_ALL && !empty($favourable['act_range_ext'])) {
                if ($favourable['act_range'] == FAR_CATEGORY) {
                    $sql = "SELECT cat_id AS id, cat_name AS name FROM " . $this->sanmu->table('category') .
                        " WHERE cat_id " . db_create_in($favourable['act_range_ext']);
                } elseif ($favourable['act_range'] == FAR_BRAND) {
                    $sql = "SELECT brand_id AS id, brand_name AS name FROM " . $this->sanmu->table('brand') .
                        " WHERE brand_id " . db_create_in($favourable['act_range_ext']);
                } else {
                    $sql = "SELECT goods_id AS id, goods_name AS name FROM " . $this->sanmu->table('goods') .
        ...
        elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
            ...
            /* 检查名称是否重复 */
            $act_name = $this->sanmuRepository->subStr($act_name, 255, false);
            ...
            $this->sanmuRepository->getOssAddFile([$activity_thumb]);
            ...
            if ($is_add) {
                ...
                $this->db->autoExecute($this->sanmu->table('favourable_activity'), $favourable, 'INSERT');
                ...
                $this->db->autoExecute($this->sanmu->table('favourable_activity'), $favourable, 'UPDATE', "act_id = '$favourable[act_id]'");
            }
        ...
        elseif ($_REQUEST['act'] == 'drop_thumb') {
            ...
            $this->sanmuRepository->getDelBatch('', $act_id, ['activity_thumb'], 'act_id', FavourableActivity::whereRaw(1), 1); //删除图片
            $sql = "UPDATE " . $this->sanmu->table('favourable_activity') . " SET activity_thumb = '' WHERE act_id = '$act_id'";
            $this->db->query($sql);
    ...
    private function favourable_list($ru_id)
    {
        ...
        if ($result === false) {
            ...
            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('favourable_activity') . " AS fa " .
                " WHERE 1 $where";
            ...
            /* 查询 */
            $sql = "SELECT fa.* " .
                "FROM " . $this->sanmu->table('favourable_activity') . " AS fa " .
                " WHERE 1 $where " .
    ...
    {
        $sql = "SELECT user_shopMain_category FROM " . $this->sanmu->table('merchants_shop_information') . " WHERE user_id = '$ru_id'";
        ...
        if (!empty($user_cat)) {
            ...
            foreach ($arr as $key => $row) {
                $new_arr[$key]['id'] = $row;
                $new_arr[$key]['name'] = $this->db->getOne("SELECT cat_name as name FROM " . $this->sanmu->table('category') . " WHERE cat_id = '$row'");
            }
    ...
    {
        $res = $this->db->getAll("SELECT cat_id FROM " . $this->sanmu->table('category') . " WHERE parent_id = '$arr'");
    ...
    {
        $sql = "SELECT mc.cat_id as id, c.cat_name as name FROM " . $this->sanmu->table('merchants_category') . " as mc, " . $this->sanmu->table('category') . " as c " .
