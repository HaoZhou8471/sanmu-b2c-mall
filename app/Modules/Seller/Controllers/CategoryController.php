...
class CategoryController extends InitController
{
    ...
    public function index()
    {
        ...
        if ($act == 'list') {
            ...
            if ($back_level > 0) {
                $level = $back_level - 1;
                $parent_id = $this->db->getOne("SELECT parent_id FROM " . $this->sanmu->table('category') . " WHERE cat_id = '$parent_id'", true);
            } else {
        ...
        if ($act == 'delete_icon') {
            ...
            if (!empty($cat_info)) {
                $sql = " update " . $this->sanmu->table('category') . " set cat_icon='' where cat_id= " . $cat_id;
        ...
        if ($act == 'insert') {
            ...
            if (count($cat_name) > 1) {
                ...
                if ($this->db->autoExecute($this->sanmu->table('category'), $cat) !== false) {
                    $cat_id = $this->db->insert_id();
                    if ($cat['show_in_nav'] == 1) {
                        $vieworder = $this->db->getOne("SELECT max(vieworder) FROM " . $this->sanmu->table('nav') . " WHERE type = 'middle'");
                        $vieworder += 2;
                        //显示在自定义导航栏中
                        $sql = "INSERT INTO " . $this->sanmu->table('nav') .
                            " (name,ctype,cid,ifshow,vieworder,opennew,url,type)" .
                            " VALUES('" . $cat['cat_name'] . "', 'c', '" . $this->db->insert_id() . "','1','$vieworder','0', '" . $this->sanmuRepository->buildUri('category', ['cid' => $cat_id], $cat['cat_name']) . "','middle')";
                        $this->db->query($sql);
                    }
                    ...
                    if ($adminru['ru_id'] > 0) {
                        ...
                        $this->db->autoExecute($this->sanmu->table('merchants_category'), $parent, 'INSERT');
                    }
                    //ecmoban模板堂 --zhuo end
        ...
        if ($act == 'edit') {
            ...
            if ($cat_info['filter_attr']) {
                ...
                foreach ($filter_attr as $k => $v) {
                    $attr_cat_id = $this->db->getOne("SELECT cat_id FROM " . $this->sanmu->table('attribute') . " WHERE attr_id = '" . intval($v) . "'");
            ...
            //分类是否存在首页推荐
            $res = $this->db->getAll("SELECT recommend_type FROM " . $this->sanmu->table("cat_recommend") . " WHERE cat_id=" . $cat_id);
            if (!empty($res)) {
                $cat_recommend = [];
                foreach ($res as $data) {
            ...
            //ecmoban模板堂 --zhuo start
            $sql = "select dt_id, dt_title from " . $this->sanmu->table('merchants_documenttitle') . " where cat_id = '$cat_id'";
        ...
        elseif ($act == 'titleFileView') {
            ...
            $sql = "select cat_name from " . $this->sanmu->table('category') . " where cat_id = '$cat_id'";
        ...
        elseif ($act == 'add_category') {
            ...
            if (cat_exists($category, $parent_id)) {
                ...
                $sql = "INSERT INTO " . $this->sanmu->table('category') . "(cat_name, parent_id, is_show)" .
        ...
        if ($act == 'update') {
            ...
            $dat = $this->db->getRow("SELECT cat_name, show_in_nav FROM " . $this->sanmu->table('category') . " WHERE cat_id = '$cat_id'");

            if ($this->db->autoExecute($this->sanmu->table('category'), $cat, 'UPDATE', "cat_id='$cat_id'")) {
                if ($cat['cat_name'] != $dat['cat_name']) {
                    //如果分类名称发生了改变
                    $sql = "UPDATE " . $this->sanmu->table('nav') . " SET name = '" . $cat['cat_name'] . "' WHERE ctype = 'c' AND cid = '" . $cat_id . "' AND type = 'middle'";
                    $this->db->query($sql);
                }
                if ($cat['show_in_nav'] != $dat['show_in_nav']) {
                    ...
                    if ($cat['show_in_nav'] == 1) {
                        //显示
                        $nid = $this->db->getOne("SELECT id FROM " . $this->sanmu->table('nav') . " WHERE ctype = 'c' AND cid = '" . $cat_id . "' AND type = 'middle'");
                        if (empty($nid)) {
                            //不存在
                            $vieworder = $this->db->getOne("SELECT max(vieworder) FROM " . $this->sanmu->table('nav') . " WHERE type = 'middle'");
                            $vieworder += 2;
                            $uri = $this->sanmuRepository->buildUri('category', ['cid' => $cat_id], $cat['cat_name']);

                            $sql = "INSERT INTO " . $this->sanmu->table('nav') . " (name,ctype,cid,ifshow,vieworder,opennew,url,type) VALUES('" . $cat['cat_name'] . "', 'c', '$cat_id','1','$vieworder','0', '" . $uri . "','middle')";
                        } else {
                            $sql = "UPDATE " . $this->sanmu->table('nav') . " SET ifshow = 1 WHERE ctype = 'c' AND cid = '" . $cat_id . "' AND type = 'middle'";
                        }
                        $this->db->query($sql);
                    } else {
                        //去除
                        $this->db->query("UPDATE " . $this->sanmu->table('nav') . " SET ifshow = 0 WHERE ctype = 'c' AND cid = '" . $cat_id . "' AND type = 'middle'");
                    }
                }
                ...
                $this->db->query("UPDATE " . $this->sanmu->table('merchants_category') . " SET is_show = '" . $is_show_merchants . "' WHERE cat_id = '$cat_id'");
                //ecmoban模板堂 --zhuo end
        ...
        if ($act == 'move_cat') {
            ...
            /* 更新商品分类 */
            $sql = "UPDATE " . $this->sanmu->table('goods') . " SET cat_id = '$target_cat_id' " .
        ...
        if ($act == 'toggle_show_in_nav') {
            ...
            if ($this->cat_update($id, ['show_in_nav' => $val]) != false) {
                if ($val == 1) {
                    ...
                    $vieworder = $this->db->getOne("SELECT max(vieworder) FROM " . $this->sanmu->table('nav') . " WHERE type = 'middle'");
                    $vieworder += 2;
                    $catname = $this->db->getOne("SELECT cat_name FROM " . $this->sanmu->table('category') . " WHERE cat_id = '$id'");
                    //显示在自定义导航栏中
                    $GLOBALS['_CFG']['rewrite'] = 0;
                    $uri = $this->sanmuRepository->buildUri('category', ['cid' => $id], $catname);

                    $nid = $this->db->getOne("SELECT id FROM " . $this->sanmu->table('nav') . " WHERE ctype = 'c' AND cid = '" . $id . "' AND type = 'middle'");
                    if (empty($nid)) {
                        //不存在
                        $sql = "INSERT INTO " . $this->sanmu->table('nav') . " (name,ctype,cid,ifshow,vieworder,opennew,url,type) VALUES('" . $catname . "', 'c', '$id','1','$vieworder','0', '" . $uri . "','middle')";
                    } else {
                        $sql = "UPDATE " . $this->sanmu->table('nav') . " SET ifshow = 1 WHERE ctype = 'c' AND cid = '" . $id . "' AND type = 'middle'";
                    }
                    $this->db->query($sql);
                } else {
                    //去除
                    $this->db->query("UPDATE " . $this->sanmu->table('nav') . "SET ifshow = 0 WHERE ctype = 'c' AND cid = '" . $id . "' AND type = 'middle'");
                }
                clear_cache_files();
                return make_json_result($val);
            } else {
        ...
        if ($act == 'title_remove') {
            ...
            $sql = "delete from " . $this->sanmu->table('merchants_documenttitle') . " where dt_id = '$dt_id'";
            $this->db->query($sql);
        ...
        elseif ($act == 'remove_cat') {
            ...
            $cat_name = $this->db->getOne('SELECT cat_name FROM ' . $this->sanmu->table('category') . " WHERE cat_id='$cat_id'");

            /* 当前分类下是否有子分类 */
            $cat_count = $this->db->getOne('SELECT COUNT(*) FROM ' . $this->sanmu->table('category') . " WHERE parent_id='$cat_id'");

            /* 当前分类下是否存在商品 */
            $goods_count = $this->db->getOne('SELECT COUNT(*) FROM ' . $this->sanmu->table('goods') . " WHERE cat_id='$cat_id'");

            /* 如果不存在下级子分类和商品，则删除之 */
            if ($cat_count == 0 && $goods_count == 0) {
                /* 删除分类 */
                $sql = 'DELETE FROM ' . $this->sanmu->table('category') . " WHERE cat_id = '$cat_id'";
                if ($this->db->query($sql)) {
                    $this->db->query("DELETE FROM " . $this->sanmu->table('nav') . "WHERE ctype = 'c' AND cid = '" . $cat_id . "' AND type = 'middle'");
                    clear_cache_files();
                    admin_log($cat_name, 'remove', 'category');
                    $result['error'] = 1;
                }

                $sql = "delete from " . $this->sanmu->table('merchants_documenttitle') . " where cat_id = '$cat_id'";
                $this->db->query($sql);

                $sql = "delete from " . $this->sanmu->table('merchants_category') . " where cat_id = '$cat_id'";
                $this->db->query($sql);
            } else {
                $result['error'] = 2;
    ...
    private function cat_update($cat_id, $args)
    {
        ...
        return $this->db->autoExecute($this->sanmu->table('category'), $args, 'update', "cat_id='$cat_id'");
    }
    ...
    private function get_attr_list()
    {
        $sql = "SELECT a.attr_id, a.cat_id, a.attr_name " .
            " FROM " . $this->sanmu->table('attribute') . " AS a,  " .
            $this->sanmu->table('goods_type') . " AS c " .
    ...
    private function get_cat_level($parent_id = 0, $level = 0, $ru_id = 0)
    {
        ...
        if ($level == 2) {
            ...
            if ($level == 1) {
                ...
                $parent_id = $this->sanmuRepository->delStrComma($parent);
        ...
        $sql = "SELECT c.cat_id, c.cat_name, c.measure_unit, c.parent_id, c.is_show, c.show_in_nav, c.grade, c.sort_order " .
            " FROM " . $this->sanmu->table('category') . " AS c WHERE  $where" .
        ...
        if ($res) {
            foreach ($res as $k => $row) {
                ...
                $res[$k]['cat_child'] = isset($res[$k]['cat_child']) && !empty($res[$k]['cat_child']) ? $this->sanmuRepository->delStrComma($res[$k]['cat_child']) : '';
                ...
                $goodsNums = $this->db->getAll("SELECT g.goods_id FROM " . $this->sanmu->table('goods') . " AS g " . " WHERE g.is_delete = 0 " . $cat_in . " AND g.user_id = '$ru_id' ");
