...
class PrivilegeController extends InitController
{
    ...
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'logout') {
            ...
            return sanmu_header("Location: privilege.php?act=login\n");
        }
        ...
        if ($_REQUEST['act'] == 'login') {
            ...
            $sql = "SELECT value FROM " . $this->sanmu->table('shop_config') . " WHERE code = 'seller_login_logo'";
        ...
        elseif ($_REQUEST['act'] == 'signin') {
            ...
            if ($row) {
                ...
                if (empty($row['ec_salt'])) {
                    ...
                    $this->db->query("UPDATE " . $this->sanmu->table('admin_user') .
                ...
                // 更新最后登录时间和IP
                $this->db->query("UPDATE " . $this->sanmu->table('admin_user') .
                    " SET last_login='" . gmtime() . "', last_ip='" . $this->sanmuRepository->sanmuIp() . "'" .
                    " WHERE user_id='" . session('seller_id') . "'");
                ...
                return sanmu_header("Location: index.php\n");
            } else {
        ...
        elseif ($_REQUEST['act'] == 'insert') {
            ...
            if (!empty($_POST['select_role'])) {
                $sql = "SELECT action_list FROM " . $this->sanmu->table('role') . " WHERE role_id = '" . $_POST['select_role'] . "'";
                $row = $this->db->getRow($sql);
                $action_list = $row['action_list'];
                $role_id = $_POST['select_role'];
            }

            $sql = "SELECT nav_list FROM " . $this->sanmu->table('admin_user') . " WHERE action_list = 'all'";
            $row = $this->db->getRow($sql);


            $sql = "INSERT INTO " . $this->sanmu->table('admin_user') . " (user_name, email, password, add_time, nav_list, action_list, role_id) " .
        ...
        elseif ($_REQUEST['act'] == 'edit') {
            ...
            /* 获取管理员信息 */
            $sql = "SELECT user_id, user_name, email, password, agency_id, role_id FROM " . $this->sanmu->table('admin_user') .
                " WHERE user_id = '$id'";
            ...
            if ($user_info['agency_id'] > 0) {
                $sql = "SELECT agency_name FROM " . $this->sanmu->table('agency') . " WHERE agency_id = '$user_info[agency_id]'";
            ...
            /* 获得该管理员的权限 */
            $priv_str = $this->db->getOne("SELECT action_list FROM " . $this->sanmu->table('admin_user') . " WHERE user_id = '$id'");
        ...
        elseif ($_REQUEST['act'] == 'update' || $_REQUEST['act'] == 'update_self') {
            ...
            //给商家发短信，邮件
            $sql = "SELECT ru_id FROM " . $this->sanmu->table('admin_user') . " WHERE user_id = '$admin_id'";
            ...
            if ($ru_id && $GLOBALS['_CFG']['sms_seller_signin'] == '1') {
                ...
                $sql = " SELECT mobile, seller_email FROM " . $this->sanmu->table('seller_shopinfo') . " WHERE ru_id = '$ru_id' LIMIT 1";
            ...
            //更新管理员信息
            if ($pwd_modified) {
                $sql = "UPDATE " . $this->sanmu->table('admin_user') . " SET " .
            ...
            $this->db->autoExecute($this->sanmu->table('seller_shopinfo'), $seller_shopinfo, 'UPDATE', "ru_id = '" . $adminru['ru_id'] . "'");
        ...
        elseif ($_REQUEST['act'] == 'modif') {
            ...
            /* 包含插件菜单语言项 */
            $sql = "SELECT code FROM " . $this->sanmu->table('plugins');
            ...
            $sql = "SELECT user_id, user_name, email, nav_list, ru_id " .
                "FROM " . $this->sanmu->table('admin_user') . " WHERE user_id = '" . session('seller_id') . "'";
            $user_info = $this->db->getRow($sql);
            ...
            $priv_str = $this->db->getOne("SELECT action_list FROM " . $this->sanmu->table('admin_user') . " WHERE user_id = '$seller_id'");
        ...
        elseif ($_REQUEST['act'] == 'allot') {
            ...
            $this->sanmuRepository->helpersLang('priv_action', 'seller');
        ...
        elseif ($_REQUEST['act'] == 'update_allot') {
            ...
            /* 取得当前管理员用户名 */
            $admin_name = $this->db->getOne("SELECT user_name FROM " . $this->sanmu->table('admin_user') . " WHERE user_id = '$id'");
        ...
        elseif ($_REQUEST['act'] == 'check_user_name') {
            ...
            if ($user_name) {
                $sql = " SELECT user_id FROM " . $this->sanmu->table('admin_user') . " WHERE user_name = '$user_name' LIMIT 1";
        ...
        elseif ($_REQUEST['act'] == 'check_user_password') {
            ...
            $sql = "SELECT `ec_salt` FROM " . $this->sanmu->table('admin_user') . " WHERE user_name = '" . $user_name . "'";
            ...
            if (!empty($ec_salt)) {
                ...
                $sql = "SELECT user_id,ru_id, user_name, password, last_login, action_list, last_login,suppliers_id,ec_salt" .
                    " FROM " . $this->sanmu->table('admin_user') .
    ...
    private function get_admin_userlist($ru_id)
    {
        ...
        if ($result === false) {
            ...
            /* 记录总数 */
            $sql = 'SELECT COUNT(*) FROM ' . $this->sanmu->table('admin_user') . " AS au " . " WHERE 1 AND parent_id = 0 $where";
            ...
            $sql = 'SELECT user_id, user_name, au.ru_id, email, add_time, last_login ' .
                'FROM ' . $this->sanmu->table('admin_user') . ' AS au ' .
    ...
    private function clear_cart()
    {
        ...
        $sql = "SELECT DISTINCT session_id " .
            "FROM " . $this->sanmu->table('cart') . " AS c, " .
            $this->sanmu->table('sessions') . " AS s " .
            "WHERE c.session_id = s.sesskey ";
        $valid_sess = $this->db->getCol($sql);

        // 删除cart中无效的数据
        $sql = "DELETE FROM " . $this->sanmu->table('cart') .
            " WHERE session_id NOT " . db_create_in($valid_sess);
        $this->db->query($sql);
        // 删除cart_combo中无效的数据 by mike
        $sql = "DELETE FROM " . $this->sanmu->table('cart_combo') .
            " WHERE session_id NOT " . db_create_in($valid_sess);
        $this->db->query($sql);
    }
