...
class MerchantsCustomController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'add') {
            ...
            //检查名称是否重复
            $sql = 'SELECT COUNT(*) FROM ' . $this->sanmu->table('seller_shopwindow') . " WHERE win_name = '$_POST[winname]' and ru_id='" . session('ru_id') . "'";
        ...
        elseif ($_REQUEST['act'] == 'remove') {
            ...
            /* 删除该橱窗的图标 */
            $sql = "SELECT win_img, win_custom FROM " . $this->sanmu->table('seller_shopwindow') . " WHERE id = '$id' and ru_id='" . $adminru['ru_id'] . "'";
        ...
        elseif ($_REQUEST['act'] == 'drop_win_goods') {
            ...
            if ($checkboxes) {
                if ($type == 1) {
                    ...
                    $sql = "DELETE FROM " . $this->sanmu->table('seller_shopwindow') . " WHERE win_type = 0 AND id in($id)";
                    $this->db->query($sql);
                }
            }
    ...
    {
        $adminru = get_admin_ru_id();
        $sql = "SELECT * FROM " . $this->sanmu->table('seller_shopwindow') . ' WHERE ru_id = \'' . $adminru['ru_id'] . '\' AND seller_theme = \'' . $seller_theme . '\' AND	win_type = 0';
