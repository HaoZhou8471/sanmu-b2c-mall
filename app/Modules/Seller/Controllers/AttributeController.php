...
class AttributeController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($act == 'add' || $act == 'edit') {
            ...
            if ($is_add) {
                ...
                $sql = "SELECT * FROM " . $this->sanmu->table('attribute') . " WHERE attr_id = '$attr_id'";
        ...
        elseif ($act == 'insert' || $act == 'update') {
            ...
            if ($is_insert) {
                ...
                $this->db->autoExecute($this->sanmu->table('attribute'), $attr, 'UPDATE', "attr_id = '$exclude'");
        ...
        if ($act == 'set_gcolor') {
            ...
            $SQL = "SELECT color_values FROM " . $this->sanmu->table('attribute') . " WHERE attr_id='$attr_id'";
        ...
        if ($act == 'gcolor_insert') {
            ...
            foreach ($request as $k => $v) {
                $sql = "UPDATE " . $this->sanmu->table('goods_attr') . " SET color_value = '$v'" . " WHERE attr_id = '$attr_id' AND attr_value = '$k'";
                $this->db->query($sql);
            }
            ...
            $update_color = "UPDATE " . $this->sanmu->table('attribute') . " SET `color_values` = '$str' WHERE `attr_id` = '$attr_id'";
        ...
        elseif ($act == 'batch') {
            ...
            if (request()->exists('checkboxes')) {
                ...
                $sql = "DELETE FROM " . $this->sanmu->table('attribute') . " WHERE attr_id " . db_create_in($ids);
                $this->db->query($sql);

                $sql = "DELETE FROM " . $this->sanmu->table('goods_attr') . " WHERE attr_id " . db_create_in($ids);
                $this->db->query($sql);
        ...
        elseif ($act == 'remove') {
            ...
            $this->db->query("DELETE FROM " . $this->sanmu->table('attribute') . " WHERE attr_id='$id'");
            $this->db->query("DELETE FROM " . $this->sanmu->table('goods_attr') . " WHERE attr_id='$id'");
        ...
        elseif ($act == 'get_attr_num') {
            ...
            $sql = "SELECT COUNT(*) " .
                " FROM " . $this->sanmu->table('goods_attr') . " AS a, " .
    ...
    private function get_attrlist()
    {
        ...
        $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('attribute') . " AS a " . " LEFT JOIN " . $this->sanmu->table('goods_type') . " AS t ON a.cat_id = t.cat_id " . " $where";
        ...
        $sql = "SELECT a.*, t.cat_name " .
            ...
            " LEFT JOIN " . $this->sanmu->table('goods_type') . " AS t ON a.cat_id = t.cat_id " . $where .
