...
class MerchantsBrandController extends InitController
{
    ...
    public function index()
    {
        ...
        $exc = new Exchange($this->sanmu->table("merchants_shop_brand"), $this->db, 'bid', 'brandName');
        ...
        elseif ($_REQUEST['act'] == 'insert') {
            ...
            if (empty($linkBrand)) {
                ...
                $sql = "SELECT brand_id FROM " . $this->sanmu->table('brand') . " WHERE brand_name = '$brand_name'";
                $brand_id = $this->db->getOne($sql);
                if (!$brand_id) {
                    $sql = 'INSERT INTO ' . $this->sanmu->table('brand') . " (`brand_name`, `brand_letter`) VALUES ('$brand_name', '$brand_letter')";
                    $this->db->query($sql);
            ...
            if ($linkBrand > 0) {
                ...
                $this->db->autoExecute($this->sanmu->table('link_brand'), $link_brand, 'INSERT'); //更新关联品牌
            }
        ...
        elseif ($_REQUEST['act'] == 'edit') {
            ...
            $sql = "SELECT bid as brand_id, brandName as brand_name, bank_name_letter, site_url, brandLogo as brand_logo, brand_desc, is_show, sort_order, audit_status, major_business " .
                "FROM " . $this->sanmu->table('merchants_shop_brand') . " WHERE bid = '$brand_id'";
        ...
        elseif ($_REQUEST['act'] == 'updata') {
            ...
            if (isset($_POST['link_brand']) && $adminru['ru_id'] > 0) {
                ...
                $this->db->autoExecute($this->sanmu->table('admin_user'), $parent, 'UPDTAE', 'ru_id =' . $adminru['ru_id']);
            }
            ...
            if (empty($_POST['link_brand'])) {
                ...
                $sql = "SELECT brand_id FROM " . $this->sanmu->table('brand') . " WHERE brand_id = '$linkBrand'";
            }
            $brand_id = $this->db->getOne($sql);
            if (!$brand_id) {
                $sql = 'INSERT INTO ' . $this->sanmu->table('brand') . " (`brand_name`, `brand_letter`) VALUES ('$brand_name', '$brand_letter')";
            ...
            if ($linkBrand > 0) {
                $sql = "SELECT id FROM " . $this->sanmu->table('link_brand') . " WHERE bid= '$bid'";
                ...
                if ($lid) {
                    $this->db->autoExecute($this->sanmu->table('link_brand'), $link_brand, 'UPDTAE', 'bid=' . $bid); //更新关联品牌
                } else {
                    $this->db->autoExecute($this->sanmu->table('link_brand'), $link_brand, 'INSERT'); //更新关联品牌
                }
            } else {
                $this->db->query(" DELETE FROM " . $this->sanmu->table('link_brand') . " WHERE bid = '$bid' ");
            }
        ...
        elseif ($_REQUEST['act'] == 'remove') {
            ...
            $id = intval($_GET['id']);
            $brand_id = $this->db->getOne("SELECT brand_id FROM " . $this->sanmu->table('link_brand') . " WHERE bid ='$id'", true);

            $this->sanmuRepository->getDelBatch('', $id, ['brandLogo'], 'bid', MerchantsShopBrand::whereRaw(1), 1); //删除图片

            $exc->drop($id, 'merchants_shop_brand', 'bid');

            $sql = 'DELETE FROM ' . $this->sanmu->table('merchants_shop_brandfile') . " WHERE bid = '$id'";
            $this->db->query($sql);

            $sql = 'DELETE FROM ' . $this->sanmu->table('link_brand') . " WHERE bid = '$id'";
            $this->db->query($sql);

            /* 更新商品的品牌编号 */
            $sql = "UPDATE " . $this->sanmu->table('goods') . " SET brand_id = 0 WHERE brand_id = '$brand_id' AND user_id = '" . $adminru['ru_id'] . "'";
            $this->db->query($sql);

            sanmu_unlink(storage_public(DATA_DIR . "/sc_file/seller_brand/seller_brand_" . $adminru['ru_id']));
        ...
        elseif ($_REQUEST['act'] == 'drop_logo') {
            ...
            $this->sanmuRepository->getDelBatch('', $brand_id, ['brandLogo'], 'bid', MerchantsShopBrand::whereRaw(1), 1); //删除图片

            $sql = "UPDATE " . $this->sanmu->table('merchants_shop_brand') . " SET brandLogo = '' WHERE bid = '$brand_id'";
            $this->db->query($sql);
    ...
    private function get_brandlist($ru_id)
    {
        ...
        if ($result === false) {
            ...
            /* 记录总数以及页数 */
            if (!empty($brand_name)) {
                $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('merchants_shop_brand') . " AS msb " . " WHERE msb.brandName LIKE '%" . mysql_like_quote($brand_name) . "%'" . $where;
            } else {
                $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('merchants_shop_brand') . " AS msb " . " where 1 " . $where;
            }
            ...
            if (!empty($brand_name)) {
                ...
                $sql = "SELECT msb.* FROM " . $this->sanmu->table('merchants_shop_brand') . " AS msb " . " WHERE msb.brandName like '%{$keyword}%' " . $where . " ORDER BY $filter[sort_by] $filter[sort_order]";
            } else {
                $sql = "SELECT msb.* FROM " . $this->sanmu->table('merchants_shop_brand') . " AS msb " . " where 1 " . $where . " ORDER BY $filter[sort_by] $filter[sort_order]";
            }
