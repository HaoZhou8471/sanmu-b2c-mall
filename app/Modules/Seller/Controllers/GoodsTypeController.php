...
class GoodsTypeController extends InitController
{
    ...
    public function index()
    {
        ...
        $exc = new Exchange($this->sanmu->table("goods_type"), $this->db, 'cat_id', 'cat_name');
        ...
        if ($_REQUEST['act'] == 'manage') {
            ...
            $query = $this->db->query("SELECT a.cat_id FROM " . $this->sanmu->table('attribute') . " AS a RIGHT JOIN " . $this->sanmu->table('goods_attr') . " AS g ON g.attr_id = a.attr_id GROUP BY a.cat_id ORDER BY a.sort_order, a.attr_id, g.goods_attr_id");
        ...
        elseif ($_REQUEST['act'] == 'cat_add' || $_REQUEST['act'] == 'cat_edit') {
            ...
            if ($cat_id > 0) {
                $sql = "SELECT cat_id ,cat_name ,parent_id ,sort_order ,level FROM" . $this->sanmu->table('goods_type_cat') . "WHERE cat_id = '$cat_id' LIMIT 1";
        ...
        elseif ($_REQUEST['act'] == 'cat_insert' || $_REQUEST['act'] == 'cat_update') {
            ...
            if ($_REQUEST['act'] == 'cat_insert') {
                ...
                $this->db->autoExecute($this->sanmu->table('goods_type_cat'), $cat_info, 'UPDATE', "cat_id = '$cat_id'");
        ...
        elseif ($_REQUEST['act'] == 'remove_cat') {
            ...
            $id = intval($_GET['id']);
            //判断是否存在下级
            $sql = "SELECT COUNT(*) FROM" . $this->sanmu->table('goods_type_cat') . "WHERE parent_id = '$id'";
            $cat_count = $this->db->getOne($sql);
            //判断分类下是否存在类型
            $sql = "SELECT COUNT(*) FROM" . $this->sanmu->table('goods_type') . "WHERE c_id = '$id'";
        ...
        elseif ($_REQUEST['act'] == 'remove') {
            ...
            if ($exc->drop($id)) {
                ...
                /* 清除该类型下的所有属性 */
                $sql = "SELECT attr_id FROM " . $this->sanmu->table('attribute') . " WHERE cat_id = '$id'";
                $arr = $this->db->getCol($sql);

                $this->db->query("DELETE FROM " . $this->sanmu->table('attribute') . " WHERE attr_id " . db_create_in($arr));
                $this->db->query("DELETE FROM " . $this->sanmu->table('goods_attr') . " WHERE attr_id " . db_create_in($arr));
        ...
        elseif ($_REQUEST['act'] == 'get_childtype') {
            ...
            $sql = "SELECT cat_id,cat_name FROM" . $this->sanmu->table("goods_type") . $where;
    ...
    private function get_goodstype($ru_id)
    {
        ...
        if ($result === false) {
            ...
            /* 记录总数以及页数 */
            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('goods_type') . " AS t " .
                "LEFT JOIN " . $this->sanmu->table('attribute') . " AS a ON a.cat_id=t.cat_id " .
            ...
            $sql = "SELECT t.*, COUNT(a.cat_id) AS attr_count ,gt.cat_name as gt_cat_name " .
                "FROM " . $this->sanmu->table('goods_type') . " AS t " .
                "LEFT JOIN " . $this->sanmu->table('attribute') . " AS a ON a.cat_id=t.cat_id " .
                "LEFT JOIN " . $this->sanmu->table('goods_type_cat') . " AS gt ON gt.cat_id=t.c_id " .
                $where . "GROUP BY t.cat_id " .
    ...
    {
        $sql = "SELECT * FROM " . $this->sanmu->table('goods_type') . " WHERE cat_id='$cat_id'";
    ...
    {
        $sql = "UPDATE " . $this->sanmu->table('attribute') .
