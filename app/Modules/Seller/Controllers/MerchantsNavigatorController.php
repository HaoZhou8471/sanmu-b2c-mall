...
class MerchantsNavigatorController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'add') {
            ...
            if (empty($_REQUEST['step'])) {
                ...
                $vieworder = $this->db->getOne("SELECT max(vieworder) FROM " . $this->sanmu->table('merchants_nav') . " WHERE type = '" . $item_type . "'");
                ...
                if ($item_ifshow == 1 && $item_type == 'middle') {
                    ...
                    if ($arr) {
                        ...
                        $sql = "INSERT INTO " . $this->sanmu->table('merchants_nav') . " (name,ctype,cid,ifshow,cat_id,vieworder,opennew,url,type,ru_id) VALUES('$item_name','" . $arr['type'] . "','" . $arr['id'] . "','$item_ifshow','$item_catId','$item_vieworder','$item_opennew','$item_url','$item_type', '" . $adminru['ru_id'] . "')";
                    }
                }

                if (empty($sql)) {
                    $sql = "INSERT INTO " . $this->sanmu->table('merchants_nav') . " (name,ifshow,vieworder,opennew,url,type,ru_id) VALUES('$item_name','$item_ifshow','$item_vieworder','$item_opennew','$item_url','$item_type', '" . $adminru['ru_id'] . "')";
                }
                $this->db->query($sql);
                clear_cache_files();
        ...
        elseif ($_REQUEST['act'] == 'edit') {
            ...
            if (empty($_REQUEST['step'])) {
                $rt = ['act' => 'edit', 'id' => $id];
                $row = $this->db->getRow("SELECT * FROM " . $this->sanmu->table('merchants_nav') . " WHERE id='$id'");
                ...
                $row = $this->db->getRow("SELECT ctype,cid,ifshow,type FROM " . $this->sanmu->table('merchants_nav') . " WHERE id = '$id'");
                ...
                if ($arr) {
                    ...
                    $sql = "UPDATE " . $this->sanmu->table('merchants_nav') .
        ...
        elseif ($_REQUEST['act'] == 'del') {
            ...
            $this->db->query($sql);
            clear_cache_files();
            return sanmu_header("Location: merchants_navigator.php?act=list\n");
        }
        ...
        if ($_REQUEST['act'] == 'toggle_ifshow') {
            ...
            $row = $this->db->getRow("SELECT type,ctype,cid FROM " . $this->sanmu->table('merchants_nav') . " WHERE id = '$id' LIMIT 1");
    ...
    private function get_nav()
    {
        ...
        if ($result === false) {
            ...
            $sql = "SELECT count(*) FROM " . $this->sanmu->table('merchants_nav') . $where;
    ...
    private function get_sysnav()
    {
        ...
        foreach ($catlist as $key => $val) {
            $val['url'] = $this->sanmuRepository->buildUri('merchants_store', ['cid' => $val['cat_id'], 'urid' => $adminru['ru_id']], $val['cat_name']);
            $sysmain[] = [
                'cat_id' => $val['cat_id'],
    ...
    private function nav_update($id, $args)
    {
        ...
        return $this->db->autoExecute($this->sanmu->table('merchants_nav'), $args, 'update', "id='$id'");
    }
