...
class DiscussCircleController extends InitController
{
    ...
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'insert') {
            ...
            $sql = "SELECT user_id, user_name FROM " . $this->sanmu->table('users') . " WHERE user_name='$user_name'";
        ...
        if ($_REQUEST['act'] == 'update') {
            ...
            /* 插入数据库。 */
            $sql = "UPDATE " . $this->sanmu->table('discuss_circle') . " SET
			dis_title='$dis_title',
			dis_text='$dis_text',
			add_time='$add_time',
			dis_type='$discuss_type'
        ...
        if ($_REQUEST['act'] == 'reply') {
            ...
            /* 获取评论详细信息并进行字符处理 */
            $sql = "SELECT * FROM " . $this->sanmu->table('discuss_circle') . " WHERE dis_id = '$_REQUEST[id]'";
            ...
            //取得商品名称
            $sql = "SELECT goods_name, original_img FROM " . $this->sanmu->table('goods') . " WHERE goods_id='$discuss_info[goods_id]'";
            ...
            //取得图片地址
            $sql = "SELECT * FROM " . $this->sanmu->table('goods_gallery') . " WHERE dis_id = '$discuss_info[dis_id]'";
        ...
        if ($_REQUEST['act'] == 'action') {
            ...
            /* 获得评论是否有回复 */
            $sql = "SELECT comment_id, content, parent_id FROM " . $this->sanmu->table('comment') .
            ...
            $sql = "UPDATE " . $this->sanmu->table('comment') . " SET status = 1 WHERE comment_id = '$_POST[comment_id]'";
            $this->db->query($sql);
            ...
            return sanmu_header("Location: comment_manage.php?act=reply&id=$_REQUEST[comment_id]&send_ok=$send_ok\n");
        }
        /*------------------------------------------------------ */
        //-- 更新评论的状态为显示或者        禁止
        /*------------------------------------------------------ */
        if ($_REQUEST['act'] == 'check') {
            if ($_REQUEST['check'] == 'allow') {
                /* 允许评论显示 */
                $sql = "UPDATE " . $this->sanmu->table('comment') . " SET status = 1 WHERE comment_id = '$_REQUEST[id]'";
                $this->db->query($sql);
                ...
                return sanmu_header("Location: comment_manage.php?act=reply&id=$_REQUEST[id]\n");
            } else {
                /* 禁止评论显示 */
                $sql = "UPDATE " . $this->sanmu->table('comment') . " SET status = 0 WHERE comment_id = '$_REQUEST[id]'";
                $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'remove') {
            ...
            $sql = "DELETE FROM " . $this->sanmu->table('discuss_circle') . " WHERE dis_id = '$id'";
            $this->db->query($sql);
        ...
        if ($_REQUEST['act'] == 'batch_drop') {
            ...
            if (isset($_POST['checkboxes'])) {
                switch ($action) {
                    case 'remove':
                        $this->db->query("DELETE FROM " . $this->sanmu->table('discuss_circle') . " WHERE " . db_create_in($_POST['checkboxes'], 'dis_id'));
                        break;
    ...
    private function get_discuss_list($ru_id)
    {
        ...
        $sql = "SELECT count(*) FROM " . $this->sanmu->table('discuss_circle') . " as dc, " . $this->sanmu->table('goods') . " g " . " $where" . " AND dc.goods_id = g.goods_id";
        ...
        /* 获取评论数据 */
        $arr = [];
        $sql = "SELECT dc.*, g.goods_name, g.user_id as ru_id FROM " . $this->sanmu->table('discuss_circle') . " as dc, " . $this->sanmu->table('goods') . " g " . " $where " . " AND dc.goods_id = g.goods_id" .
    ...
    private function get_discuss_user_reply_list()
    {
        ...
        $sql = "SELECT count(*) FROM " . $this->sanmu->table('discuss_circle') . " as dc " . " $where" . " AND dc.parent_id = '" . $filter['dis_id'] . "'";
        $filter['record_count'] = $this->db->getOne($sql);
        ...
        /* 获取评论数据 */
        $arr = [];
        $sql = "SELECT dc.* FROM " . $this->sanmu->table('discuss_circle') . " as dc " . " $where " . " AND dc.parent_id = '" . $filter['dis_id'] . "'" .
            " ORDER BY $filter[sort_by] $filter[sort_order] " .
            " LIMIT " . $filter['start'] . ", $filter[page_size]";
        $res = $this->db->query($sql);
        foreach ($res as $row) {
            ...
            $sql = "SELECT u.user_name FROM " . $this->sanmu->table('users') . " AS u, " . $this->sanmu->table('discuss_circle') . " AS dc " . " WHERE u.user_id = dc.user_id AND dc.dis_id = '" . $row['quote_id'] . "'";
