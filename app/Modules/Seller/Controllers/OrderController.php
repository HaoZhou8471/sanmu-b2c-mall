...
class OrderController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'edit_auto_delivery_time') {
            ...
            /* 删除数据 */
            $sql = "UPDATE " . $this->sanmu->table('order_info') . " SET auto_delivery_time = '$delivery_time'" . " WHERE order_id = '$order_id'";
            $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'info') {
                    ...
                    if ($payment && strpos($payment['pay_code'], 'pay_') === false) {
                        ...
                        if (!is_null($payObj)) {
                            ...
                            if (is_callable([$payObj, 'orderQuery'])) {
                                ...
                                $sql = "SELECT order_status, shipping_status, pay_status, pay_time FROM " . $this->sanmu->table('order_info') . " WHERE order_id = '" . $order['order_id'] . "' LIMIT 1";
            ...
            if ($order['ru_id'] != $adminru['ru_id']) {
                ...
                return sanmu_header("Location: $Loaction\n");
            }
            //获取支付方式code
            $sql = "SELECT pay_code FROM " . $this->sanmu->table('payment') . " WHERE pay_id = '" . $order['pay_id'] . "'";
            ...
            $order['delivery_id'] = $this->db->getOne("SELECT delivery_id FROM " . $this->sanmu->table('delivery_order') . " WHERE order_sn = '" . $order['order_sn'] . "'", true);
            ...
            if ($GLOBALS['_CFG']['open_delivery_time'] == 1) {
                ...
                if (($orderInfo['order_status'] == OS_CONFIRMED || $orderInfo['order_status'] == OS_SPLITED) && $orderInfo['shipping_status'] == SS_SHIPPED && $orderInfo['pay_status'] == PS_PAYED) { //发货状态
                    ...
                    if ($confirm_take_time > $delivery_time) { //自动确认发货操作
                        ...
                        $sql = "UPDATE " . $this->sanmu->table('order_info') . " SET order_status = '" . $orderInfo['order_status'] . "', " .
                        ...
                        $value_card = $this->db->getOne("SELECT use_val FROM " . $this->sanmu->table('value_card_record') . " WHERE order_id = '" . $order['order_id'] . "'", true);
                    ...
                    if (empty($order['confirm_take_time'])) {
                        $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('seller_bill_order') . " WHERE order_id = '" . $order['order_id'] . "'";
                        $bill_order_count = $this->db->getOne($sql, true);

                        if ($bill_order_count > 0 && $order['shipping_status'] == SS_RECEIVED) {
                            $sql = "SELECT MAX(log_time) AS log_time FROM " . $this->sanmu->table('order_action') . " WHERE order_id = '" . $order['order_id'] . "' AND shipping_status = '" . SS_RECEIVED . "'";
                            ...
                            $this->db->autoExecute($this->sanmu->table('order_info'), $log_other, 'UPDATE', "order_id = '" . $order['order_id'] . "'");
                            $this->db->autoExecute($this->sanmu->table('seller_bill_order'), $log_other, 'UPDATE', "order_id = '" . $order['order_id'] . "'");
                        }
                    }
                }
            }
            ...
            $sql = "SELECT agency_id FROM " . $this->sanmu->table('admin_user') . " WHERE user_id = '" . session('seller_id') . "'";
            ...
            $sql = "SELECT MAX(order_id) FROM " . $this->sanmu->table('order_info') . " as o WHERE order_id < '$order[order_id]'";
            if ($agency_id > 0) {
            ...
            $sql = "SELECT MIN(order_id) FROM " . $this->sanmu->table('order_info') . " as o WHERE order_id > '$order[order_id]'";
            if ($agency_id > 0) {
                $sql .= " AND agency_id = '$agency_id'";
            }
            if (!empty($where)) {
            ...
            if ($order['user_id'] > 0) {
                ...
                if (!empty($user_info)) {
                    ...
                    if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                        $order['user_name'] = $this->sanmuRepository->stringToStar($order['user_name']);
                    }
                }
            }

            /* 取得所有办事处 */
            $sql = "SELECT agency_id, agency_name FROM " . $this->sanmu->table('agency');
            ...
            if ($order['from_ad'] == 0) {
                ...
                /* 查询广告的名称 */
                $ad_name = $this->db->getOne("SELECT ad_name FROM " . $this->sanmu->table('ad') . " WHERE ad_id='$order[from_ad]'");
                $order['referer'] = $GLOBALS['_LANG']['from_ad_js'] . $ad_name . ' (' . $GLOBALS['_LANG']['from'] . $order['referer'] . ')';
            }

            /* 此订单的发货备注(此订单的最后一条操作记录) */
            $sql = "SELECT action_note FROM " . $this->sanmu->table('order_action') .
                " WHERE order_id = '$order[order_id]' AND shipping_status = 1 ORDER BY log_time DESC";
            $order['invoice_note'] = $this->db->getOne($sql);

            /* 自提点信息 */
            $sql = "SELECT shipping_code FROM " . $this->sanmu->table('shipping') . " WHERE shipping_id = '$order[shipping_id]'";
            if ($this->db->getOne($sql) == 'cac') {
                $sql = "SELECT * FROM " . $this->sanmu->table('shipping_point') . " WHERE id IN (SELECT point_id FROM " . $this->sanmu->table('order_info') . " WHERE order_id='" . $order['order_id'] . "')";
                $order['point'] = $this->db->getRow($sql);
            }

            /* 判断当前订单是否是白条分期付订单 bylu */
            $sql = "SELECT stages_total,stages_one_price,is_stages FROM " . $this->sanmu->table('baitiao_log') . " WHERE order_id = '$order_id'";
            ...
            if ($order['invoice_type'] == 1) {
                $user_id = $order['user_id'];
                $sql = " SELECT * FROM " . $this->sanmu->table('users_vat_invoices_info') . " WHERE user_id = '$user_id' LIMIT 1";
            ...
            /*判断是否评论 by kong*/
            $order['is_comment'] = 0;
            $sql = " SELECT comment_id , add_time FROM" . $this->sanmu->table('comment') . " WHERE order_id = '" . $order['order_id'] . "' AND user_id = '" . $order['user_id'] . "'";
            ...
            if ($order['user_id'] > 0) {
                ...
                $sql = "SELECT rank_name FROM " . $this->sanmu->table('user_rank') . $where;
                ...
                $sql = "SELECT COUNT(*) " .
                    "FROM " . $this->sanmu->table('bonus_type') . " AS bt, " . $this->sanmu->table('user_bonus') . " AS ub " .
                ...
                // 地址信息
                $sql = "SELECT * FROM " . $this->sanmu->table('user_address') . " WHERE user_id = '$order[user_id]' AND address_id = '" . $user_info['address_id'] . "'";
                $address_list = $this->db->getAll($sql);

                if ($address_list) {
                    foreach ($address_list as $key => $row) {
                        if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                            $address_list[$key]['mobile'] = app(SanmuRepository::class)->stringToStar($row['mobile']);
                        }
                    }
                }
            ...
            /* 获取订单门店信息 */
            $sql = "SELECT store_id,pick_code,take_time  FROM" . $this->sanmu->table("store_order") . " WHERE order_id = '" . $order['order_id'] . "'";
            $stores = $this->db->getRow($sql);
            $order['store_id'] = $stores['store_id'];

            $offline_store = [];
            if ($order['store_id'] > 0) {
                $sql = "SELECT o.*,p.region_name as province,c.region_name as city,d.region_name as district FROM" . $this->sanmu->table('offline_store') . " AS o "
                    . "LEFT JOIN " . $this->sanmu->table('region') . " AS p ON p.region_id = o.province "
                    . "LEFT JOIN " . $this->sanmu->table('region') . " AS c ON c.region_id = o.city "
                    . "LEFT JOIN " . $this->sanmu->table('region') . " AS d ON d.region_id = o.district WHERE o.id = '" . $order['store_id'] . "'";
            ...
            FROM " . $this->sanmu->table('order_goods') . " AS o
                LEFT JOIN " . $this->sanmu->table('products') . " AS p
                    ON p.product_id = o.product_id
                LEFT JOIN " . $this->sanmu->table('goods') . " AS g
                    ON o.goods_id = g.goods_id
                LEFT JOIN " . $this->sanmu->table('category') . " AS c
                    ON g.cat_id = c.cat_id
				LEFT JOIN " . $this->sanmu->table('order_info') . " AS oi
            ...
            foreach ($res as $row) {
                ...
                $sql = "SELECT * FROM " . $this->sanmu->table($table_products) . " WHERE goods_id = '" . $row['goods_id'] . "'" . $type_files . " LIMIT 0, 1";
                ...
                $row['warehouse_name'] = $this->db->getOne("select region_name from " . $this->sanmu->table('region_warehouse') . " where region_id = '" . $row['warehouse_id'] . "'");
                ...
                $sql = "SELECT ret_id, agree_apply, return_status, refound_status FROM " . $this->sanmu->table('order_return') . " WHERE rec_id = '$row[rec_id]' AND return_type = 3 "; //仅退款
                ...
                //判断是否是贡云订单
                $sql = "SELECT COUNT(*) FROM" . $this->sanmu->table('order_cloud') . "WHERE rec_id = '" . $row['rec_id'] . "'";
...
//                $goods_where = " AND d.delivery_id = any(SELECT dg.delivery_id FROM " . $this->sanmu->table('delivery_goods') . " AS dg WHERE dg.goods_id = '" . $row['goods_id'] . "' AND goods_attr = '" . $row['goods_attr'] . "')";
//                $sql = "SELECT d.delivery_id, d.status FROM" . $this->sanmu->table('delivery_order') . " AS d WHERE d.order_sn = '" . $order['order_sn'] . "' $goods_where LIMIT 1";
//                $row['delivery'] = $this->db->getRow($sql);
                ...
                $row['formated_goods_bonus'] = sprintf($GLOBALS['_LANG']['average_bonus'], $this->sanmuRepository->getPriceFormat($row['goods_bonus']));
            ...
            /* 判断退换货订单申请是否通过 strat */
            $sql = "SELECT agree_apply FROM " . $this->sanmu->table('order_return') . " WHERE order_id = '$order[order_id]'";
             ...
             */
            $sql = "SELECT log_time  FROM " . $this->sanmu->table('order_action') . " WHERE order_id = '$order[order_id]' ";
            ...
            /* 取得订单操作记录 */
            $act_list = [];
            $sql = "SELECT * FROM " . $this->sanmu->table('order_action') . " WHERE order_id = '$order[order_id]' ORDER BY log_time DESC,action_id DESC";
            $res = $this->db->query($sql);
            foreach ($res as $row) {
            ...
            if ($order['pay_status'] == 2 && $order['shipping_status'] == 0) {
                $sql = " SELECT COUNT(*) FROM " . $this->sanmu->table('store_order') . " WHERE order_id = '$order[order_id]' AND store_id > 0 ";
            ...
            //商家店铺信息打印到订单和快递单上
            $sql = "select shop_name,country,province,city,district,shop_address,kf_tel from " . $this->sanmu->table('seller_shopinfo') . " where ru_id='" . $order['ru_id'] . "'";
            ...
            if (isset($_GET['print'])) {
                ...
                $this->smarty->assign('shop_url', $this->sanmu->seller_url());
            ...
            elseif (isset($_GET['shipping_print'])) {
                ...
                if (get_print_type($adminru['ru_id'])) {
                    ...
                    return sanmu_header("Location: $url\n");
                }
                //快递鸟、电子面单 end

                //发货地址所在地
                $region_array = [];
                $region = $this->db->getAll("SELECT region_id, region_name FROM " . $this->sanmu->table("region")); //打印快递单地区 by wu
                if (!empty($region)) {
                ...
                $this->smarty->assign('shop_address', $store['shop_address']);
                $this->smarty->assign('service_phone', $store['kf_tel']);
                $shipping = $this->db->getRow("SELECT * FROM " . $this->sanmu->table("shipping_tpl") . " WHERE shipping_id = '" . $order['shipping_id'] . "' and ru_id='" . $adminru['ru_id'] . "'");
                ...
                if ($shipping['print_model'] == 2) {
                    ...
                    $shipping_code = $this->db->getOne("SELECT shipping_code FROM " . $this->sanmu->table('shipping') . " WHERE shipping_id='" . $order['shipping_id'] . "'");
                ...
                //查询订单是否有商品退款
                $sql = "SELECT count(*) FROM " . $this->sanmu->table('order_return') . " WHERE order_id = '$order_id'";
                $this->db->getOne($sql);
        ...
        elseif ($_REQUEST['act'] == 'return_info') {
            ...
            if ($agency_id > 0) {
                ...
                /* 取当前办事处信息 */
                $sql = "SELECT agency_name FROM " . $this->sanmu->table('agency') . " WHERE agency_id = '$agency_id' LIMIT 0, 1";
            ...
            if ($back_order['user_id'] > 0) {
                ...
                if (!empty($user_info)) {
                    ...
                    if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                        $back_order['user_name'] = $this->sanmuRepository->stringToStar($back_order['user_name']);
                    }
                }
            }
            ...
            foreach ($shipping_list as $key => $shipping) {
                $shipping_fee = $this->sanmuRepository->shippingFee($shipping['shipping_code'], unserialize($shipping['configure']), $total['weight'], $total['amount'], $total['number']); //计算运费
        ...
        elseif ($_REQUEST['act'] == 'delivery_list') {
            ...
            //http://sanmux.test/assets/seller/images/sort_desc.gif
        ...
        elseif ($_REQUEST['act'] == 'delivery_info') {
            ...
            if ($delivery_order['user_id'] > 0) {
                ...
                if (!empty($user_info)) {
                    ...
                    if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                        $delivery_order['user_name'] = $this->sanmuRepository->stringToStar($delivery_order['user_name']);
                    }
                }
            }
            ...
            /* 取得发货单商品 */
            $goods_sql = "SELECT dg.*, g.brand_id, g.goods_thumb FROM " . $this->sanmu->table('delivery_goods') . " AS dg " .
                "LEFT JOIN " . $this->sanmu->table('goods') . " AS g ON g.goods_id = dg.goods_id " .
            ...
            /* 取得订单操作记录 */
            $act_list = [];
            $sql = "SELECT * FROM " . $this->sanmu->table('order_action') . " WHERE order_id = '" . $delivery_order['order_id'] . "' AND action_place = 1 ORDER BY log_time DESC,action_id DESC";
            $res = $this->db->query($sql);
            foreach ($res as $row) {
                $row['order_status'] = $GLOBALS['_LANG']['os'][$row['order_status']];
                $row['pay_status'] = $GLOBALS['_LANG']['ps'][$row['pay_status']];
        ...
        elseif ($_REQUEST['act'] == 'delivery_ship') {
            ...
            $delivery_stock_sql = "SELECT G.model_attr, G.model_inventory, DG.goods_id, DG.delivery_id, DG.is_real, DG.send_number AS sums, G.goods_number AS storage, G.goods_name, DG.send_number," .
                " OG.goods_attr_id, OG.warehouse_id, OG.area_id, OG.area_city, OG.ru_id, OG.order_id, OG.product_id FROM " . $this->sanmu->table('delivery_goods') . " AS DG, " .
                $this->sanmu->table('goods') . " AS G, " .
                $this->sanmu->table('delivery_order') . " AS D, " .
                $this->sanmu->table('order_goods') . " AS OG " .
            ...
            for ($i = 0; $i < count($delivery_stock_result); $i++) {
                ...
                $sql = "SELECT * FROM " . $this->sanmu->table($table_products) . " WHERE goods_id = '" . $delivery_stock_result[$i]['goods_id'] . "'" . $type_files . " LIMIT 0, 1";
                $prod = $this->db->getRow($sql);
                    ...
                    if ($value['is_real'] != 0) {
                        ...
                        if (!empty($value['product_id'])) {
                            if ($value['model_attr'] == 1) {
                                $minus_stock_sql = "UPDATE " . $this->sanmu->table('products_warehouse') . "
                                            SET product_number = product_number - " . $value['sums'] . "
                                            WHERE product_id = " . $value['product_id'];
                            } elseif ($value['model_attr'] == 2) {
                                $minus_stock_sql = "UPDATE " . $this->sanmu->table('products_area') . "
                                            SET product_number = product_number - " . $value['sums'] . "
                                            WHERE product_id = " . $value['product_id'];
                            } else {
                                $minus_stock_sql = "UPDATE " . $this->sanmu->table('products') . "
                                            SET product_number = product_number - " . $value['sums'] . "
                                            WHERE product_id = " . $value['product_id'];
                            }
                        } else {
                            if ($value['model_inventory'] == 1) {
                                $minus_stock_sql = "UPDATE " . $this->sanmu->table('warehouse_goods') . "
                                ...
                                $minus_stock_sql = "UPDATE " . $this->sanmu->table('warehouse_area_goods') . "
                                            SET region_number = region_number - " . $value['sums'] . "
                                            WHERE goods_id = " . $value['goods_id'] . " AND region_id = " . $value['area_id'];
                            } else {
                                $minus_stock_sql = "UPDATE " . $this->sanmu->table('goods') . "
                        ...
                        $this->db->autoExecute($this->sanmu->table('goods_inventory_logs'), $logs_other, 'INSERT');
                    }
                }
            }
            ...
            $query = $this->db->autoExecute($this->sanmu->table('delivery_order'), $_delivery, 'UPDATE', "delivery_id = $delivery_id", 'SILENT');
            if (!$query) {
                /* 操作失败 */
                $links[] = ['text' => lang('seller/order.delivery_sn') . lang('seller/order.detail'), 'href' => 'order.php?act=delivery_info&delivery_id=' . $delivery_id];
            ...
            if ($order_finish) {
                ...
                if ($cfg == '1') {
                    ...
                    $this->smarty->assign('confirm_url', $this->sanmu->url() . 'user_order.php?act=order_detail&order_id=' . $order['order_id']); //by wu
                    $this->smarty->assign('send_msg_url', $this->sanmu->url() . 'user_message.php?act=message_list&order_id=' . $order['order_id']);
                ...
                if (file_exists(MOBILE_WECHAT)) {
                    ...
                    $order_url = sanmu_url('/#/user/orderDetail/' . $order_id);
        ...
        elseif ($_REQUEST['act'] == 'ajax_order_detection') {
            ...
            if (session()->has('is_ajax_detection') && session('is_ajax_detection') == 1) {
                ...
                if ($result['order'] && $result['order']['shipping_status'] == SS_SHIPPED) {
                    ...
                    $this->db->query("UPDATE " . $this->sanmu->table('order_info') .
                    ...
                    $sql = "SELECT order_id, ru_id, user_id, order_sn , order_status, shipping_status, pay_status, " .
                        ...
                        "FROM " . $this->sanmu->table('order_info') . " WHERE order_id = '" . $result['order']['order_id'] . "'";
        ...
        elseif ($_REQUEST['act'] == 'delivery_cancel_ship') {
            ...
            FROM " . $this->sanmu->table('delivery_goods') . " AS DG
        ...
        elseif ($_REQUEST['act'] == 'back_info') {
            ...
            /* 取得发货单商品 */
            $goods_sql = "SELECT bg.*, g.brand_id FROM " . $this->sanmu->table('back_goods') . " AS bg " .
                "LEFT JOIN " . $this->sanmu->table('goods') . " AS g ON g.goods_id = bg.goods_id " .
        ...
        elseif ($_REQUEST['act'] == 'edit_refound_amount') {
            ...
            if ($type == 1) {
                ...
                /* 退运费 */
                $sql = "SELECT shipping_fee FROM " . $this->sanmu->table('order_info') . " WHERE order_id = '$order_id'";
        ...
        elseif ($_REQUEST['act'] == 'edit_refound_value_card') {
            ...
            $sql = "SELECT vc_id, use_val FROM " . $this->sanmu->table('value_card_record') . " WHERE vc_id = '$vc_id' AND order_id = '$order_id' LIMIT 1";
        ...
        elseif ($_REQUEST['act'] == 'step_post') {
            ...
            if ('user' == $step) {
                ...
                do {
                    ...
                    if ($this->db->autoExecute($this->sanmu->table('order_info'), $order, 'INSERT', '', 'SILENT')) {
                        break;
                    } else {
                ...
                /* 插入 pay_log */
                $sql = 'INSERT INTO ' . $this->sanmu->table('pay_log') . " (order_id, order_amount, order_type, is_paid)" .
                    " VALUES ('$order_id', 0, '" . PAY_ORDER . "', 0)";
                $this->db->query($sql);

                /* 下一步 */
                return sanmu_header("Location: order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=goods\n");
            } /* 编辑商品信息 */
            elseif ('edit_goods' == $step) {
                if (isset($_POST['rec_id'])) {
                    foreach ($_POST['rec_id'] as $key => $rec_id) {
                        $sql = "SELECT warehouse_id, area_id FROM " . $this->sanmu->table('order_goods') . " WHERE rec_id = '$rec_id' LIMIT 1";
                        $order_goods = $this->db->getRow($sql);

                        $sql = "SELECT goods_number " .
                            'FROM ' . $this->sanmu->table('goods') .
            ...
            elseif ('add_goods' == $step) {
                ...
                if ($model_attr == 1) {
                    ...
                    $attr_leftJoin = 'LEFT JOIN ' . $this->sanmu->table('warehouse_attr') . " AS wap ON g.goods_attr_id = wap.goods_attr_id AND wap.warehouse_id = '$warehouse_id' ";
                } elseif ($model_attr == 2) {
                    $select = " waa.attr_price as area_attr_price, ";
                    $attr_leftJoin = 'LEFT JOIN ' . $this->sanmu->table('warehouse_area_attr') . " AS waa ON g.goods_attr_id = waa.goods_attr_id AND area_id = '$area_id' ";
                }
                ...
                if ($attr_list) {
                    ...
                    $sql = "SELECT g.attr_value, " . $select . " g.attr_price " .
                        'FROM ' . $this->sanmu->table('goods_attr') . " AS g " .
                        "LEFT JOIN" . $this->sanmu->table('attribute') . " AS a ON g.attr_id = a.attr_id " .
                        $attr_leftJoin .
                ...
                $sql = "SELECT * FROM " . $this->sanmu->table($table_products) . " WHERE goods_id = '$goods_id'" . $type_files . " LIMIT 0, 1";
                $prod = $this->db->getRow($sql);
                //ecmoban模板堂 --zhuo end
            ...
            elseif ('goods' == $step) {
                /* 下一步 */
                if (isset($_POST['next'])) {
                    return sanmu_header("Location: order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=consignee\n");
                } /* 完成 */
                elseif (isset($_POST['finish'])) {
                    ...
                    if (!empty($msgs)) {
                        ...
                        /* 跳转到订单详情 */
                        return sanmu_header("Location: order.php?act=info&order_id=" . $order_id . "\n");
                    }
                }
            } /* 保存收货人信息 */
            elseif ('consignee' == $step) {
                ...
                if (isset($_POST['next'])) {
                    ...
                    if ($this->flowUserService->existRealGoods($order_id)) {
                        /* 存在实体商品，去配送方式 */
                        return sanmu_header("Location: order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=shipping\n");
                    } else {
                        /* 不存在实体商品，去支付方式 */
                        return sanmu_header("Location: order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=payment\n");
                    }
                } elseif (isset($_POST['finish'])) {
                    ...
                    /* 完成 */
                    if ($agency_changed) {
                        return sanmu_header("Location: order.php?act=list\n");
                    } else {
                        return sanmu_header("Location: order.php?act=info&order_id=" . $order_id . "\n");
                    }
                }
            ...
            elseif ('payment' == $step) {
                ...
                if (isset($_POST['next'])) {
                    /* 下一步 */
                    return sanmu_header("Location: order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=other\n");
                } elseif (isset($_POST['finish'])) {
                    /* 初始化提示信息和链接 */
                    $msgs = [];
                    $links = [];
                ...
                if (isset($_POST['next'])) {
                    /* 下一步 */
                    return sanmu_header("Location: order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=money\n");
                } elseif (isset($_POST['finish'])) {
                    /* 完成 */
                    return sanmu_header("Location: order.php?act=info&order_id=" . $order_id . "\n");
                }
                                ...
                                if ($order['order_amount'] > 0) {
                                    if ($old_order['extension_code'] != 'exchange_goods') {
                                        ...
                                        if (isset($_POST['integral']) && intval($_POST['integral']) > 0) {
                                            ...
                                            $order['integral_money'] = $this->sanmuRepository->valueOfIntegral($order['integral']);
                                    ...
                                    if ($order['order_amount'] > 0) {
                                        ...
                                        $order['integral'] = $this->sanmuRepository->integralOfValue($order['integral_money']);
                                        $order['order_amount'] = 0;
                                    }
                                } else {
                                    /* 如果红包足以支付，把待付款金额设为0 */
                ...
                if ($old_order['user_id'] > 0) {
                    ...
                    if ($old_order['bonus_id'] != $order['bonus_id']) {
                        if ($old_order['bonus_id'] > 0) {
                            $sql = "UPDATE " . $this->sanmu->table('user_bonus') .
         ...
         */ elseif ($_REQUEST['act'] == 'edit_shipping') {
            ...
            $this->db->query("UPDATE " . $this->sanmu->table('order_return') . " SET out_shipping_name = '$shipping_id' , out_invoice_no ='$invoice_no'" .
                "WHERE ret_id = '$ret_id'");
        ...
        elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit') {
            ...
            if ($order_id > 0) {
                ...
                $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('order_goods') . " WHERE order_id = '$order_id'";
            ...
            elseif ('money' == $step) {
                ...
                if ($order['user_id'] > 0) {
                    ...
                    foreach ($user_bonus as $key => $row) {
                        $sql = "SELECT order_id FROM " . $this->sanmu->table('order_info') . " WHERE bonus_id = '" . $row['bonus_id'] . "'";
        ...
        elseif ($_REQUEST['act'] == 'search_area') {
            ...
            $sql = "SELECT region_id, region_name FROM " . $this->sanmu->table('region_warehouse') . " WHERE region_type = 1 AND parent_id = '$warehouse_id'";
        ...
        elseif ($_REQUEST['act'] == 'process') {
            ...
            if ('drop_order_goods' == $func) {
                ...
                /* 如果使用库存，且下订单时减库存，则修改库存 */
                if ($GLOBALS['_CFG']['use_storage'] == '1' && $GLOBALS['_CFG']['stock_dec_time'] == SDT_PLACE) {
                    $goods = $this->db->getRow("SELECT goods_id, goods_number FROM " . $this->sanmu->table('order_goods') . " WHERE rec_id = " . $rec_id);
                    $sql = "UPDATE " . $this->sanmu->table('goods') .
                ...
                /* 删除 */
                $sql = "DELETE FROM " . $this->sanmu->table('order_goods') .
                    " WHERE rec_id = '$rec_id' LIMIT 1";
                $this->db->query($sql);
            ...
            elseif ('cancel_order' == $func) {
                ...
                if ($step_act == 'add') {
                    /* 如果是添加，删除订单，返回订单列表 */
                    if ($order_id > 0) {
                        $sql = "DELETE FROM " . $this->sanmu->table('order_info') .
        ...
        elseif ($_REQUEST['act'] == 'merge') {
            ...
            if ($merchant_id > 0) {
                ...
                $sql = "SELECT o.order_sn, u.user_name " .
                    "FROM " . $this->sanmu->table('order_info') . " AS o " .
                    "LEFT JOIN " . $this->sanmu->table('users') . " AS u ON o.user_id = u.user_id " .
                    " LEFT JOIN " . $this->sanmu->table('order_goods') . " AS og ON o.order_id=og.order_id " .
                    " LEFT JOIN " . $this->sanmu->table('goods') . " AS g ON og.goods_id=g.goods_id " .
        ...
        elseif ($_REQUEST['act'] == 'operate') {
            ...
            elseif (isset($_POST['to_shipping'])) {
                ...
                if (!empty($invoice_no) && !$delivery_info) {
                    ...
                    $sql = "select add_time from " . $this->sanmu->table('order_info') . " WHERE order_sn = '" . $delivery['order_sn'] . "'";
                    ...
                    /* 发货单入库 */
                    $query = $this->db->autoExecute($this->sanmu->table('delivery_order'), $_delivery, 'INSERT', '', 'SILENT');
                        ...
                        if (!empty($goods_list)) {
                            foreach ($goods_list as $value) {
                                ...
                                if (empty($value['extension_code']) || $value['extension_code'] == 'virtual_card' || $value['extension_code'] == 'presale' || substr($value['extension_code'], 0, 7) == 'seckill') {
                                    ...
                                    $query = $this->db->autoExecute($this->sanmu->table('delivery_goods'), $delivery_goods, 'INSERT', '', 'SILENT');
                                    $sql = "UPDATE " . $this->sanmu->table('order_goods') . "
                ...
                AND goods_id = '" . $value['goods_id'] . "' ";
                                ...
                                elseif ($value['extension_code'] == 'package_buy') {
                                    foreach ($value['package_goods_list'] as $pg_key => $pg_value) {
                                        ...
                                        $query = $this->db->autoExecute($this->sanmu->table('delivery_goods'), $delivery_pg_goods, 'INSERT', '', 'SILENT');
                                        $sql = "UPDATE " . $this->sanmu->table('order_goods') . "
                                        SET send_number = " . $value['goods_number'] . "
                                        WHERE order_id = '" . $value['order_id'] . "'
                    ...
                    if (!empty($delivery_id)) {
                        ...
                        $delivery_id = $this->db->getOne("SELECT delivery_id FROM " . $this->sanmu->table('delivery_order') . " WHERE order_sn = '$order_sn'");
                        $delivery_order = $this->delivery_order_info($delivery_id);
                    } else {
                        return 'order does not exist';
                    }
                    ...
                    $sql = "SELECT concat(IFNULL(c.region_name, ''), '  ', IFNULL(p.region_name, ''), " .
                        ...
                        "FROM " . $this->sanmu->table('order_info') . " AS o " .
                        "LEFT JOIN " . $this->sanmu->table('region') . " AS c ON o.country = c.region_id " .
                        "LEFT JOIN " . $this->sanmu->table('region') . " AS p ON o.province = p.region_id " .
                        "LEFT JOIN " . $this->sanmu->table('region') . " AS t ON o.city = t.region_id " .
                        "LEFT JOIN " . $this->sanmu->table('region') . " AS d ON o.district = d.region_id " .
                    ...
                    /* 取得发货单商品 */
                    $goods_sql = "SELECT *
                  FROM " . $this->sanmu->table('delivery_goods') . "
            ...
            elseif (isset($_POST['ship'])) {
                ...
                /* 取得订单操作记录 */
                $act_list = [];
                $sql = "SELECT * FROM " . $this->sanmu->table('order_action') . " WHERE order_id = '$order_id' ORDER BY log_time DESC,action_id DESC";
                $res = $this->db->query($sql);
                foreach ($res as $row) {
                    $row['order_status'] = $GLOBALS['_LANG']['os'][$row['order_status']];
                    $row['pay_status'] = $GLOBALS['_LANG']['ps'][$row['pay_status']];
            ...
            elseif (isset($_POST['return'])) {
                $sql = "SELECT ret_id FROM " . $this->sanmu->table('order_return') . " WHERE order_id = '" . $order_id . "'";
                ...
                $sql = "SELECT vc_id, use_val FROM " . $this->sanmu->table('value_card_record') . " WHERE order_id = '" . $order['order_id'] . "' LIMIT 1";
                $value_card = $this->db->getRow($sql);
     ...
     */
            elseif (isset($_POST['refound'])) {
                ...
                $count_goods = $this->db->getAll(" SELECT rec_id ,goods_id FROM " . $this->sanmu->table("order_goods") . " WHERE order_id = '$order_id'");
                if (count($count_goods) > 1) {
                    foreach ($count_goods as $k => $v) {
                        $all_goods_id[] = $v['goods_id'];
                    }
                    $count_integral = $this->db->getOne(" SELECT sum(integral) FROM" . $this->sanmu->table("goods") . " WHERE  goods_id" . db_create_in($all_goods_id)); //获取该订单的全部可用积分
                    $return_integral = $this->db->getOne(' SELECT g.integral FROM' . $this->sanmu->table("goods") . " as g LEFT JOIN " . $this->sanmu->table("order_return") . " as o on o.goods_id = g.goods_id  WHERE o.ret_id = '$ret_id'"); //退货商品的可用积分
                ...
                $goods_number = $this->db->getOne(" SELECT goods_number FROM " . $this->sanmu->table("order_goods") . " WHERE rec_id = '$rec_id'"); //获取该商品的订单数量
                $return_number = $this->db->getOne(" SELECT return_number FROM " . $this->sanmu->table("order_return_extend") . " WHERE ret_id = '$ret_id'"); //获取退货数量
             ...
             */ elseif (isset($_POST['send_submit'])) {
                ...
                $sql = "SELECT shipping_name FROM " . $this->sanmu->table('shipping') . " WHERE shipping_id =" . $shipping_id;
            ...
            elseif (isset($_POST['assign'])) {
                ...
                if ($new_agency_id != $order['agency_id']) {
                    ...
                    foreach ($query_array as $value) {
                        $this->db->query("UPDATE " . $this->sanmu->table($value) . " SET agency_id = '$new_agency_id' " .
            ...
            elseif (isset($_POST['remove'])) {
                ...
                if (!$batch) {
                    ...
                    /* 删除订单 */
                    $this->db->query("DELETE FROM " . $this->sanmu->table('order_info') . " WHERE order_id = '$order_id'");
                    $this->db->query("DELETE FROM " . $this->sanmu->table('order_goods') . " WHERE order_id = '$order_id'");
                    $this->db->query("DELETE FROM " . $this->sanmu->table('order_action') . " WHERE order_id = '$order_id'");

                    $this->db->query("DELETE FROM " . $this->sanmu->table('store_order') . " WHERE order_id = '$order_id'");
            ...
            elseif (isset($_REQUEST['remove_invoice'])) {
                ...
                foreach ($delivery_id as $value_is) {
                    ...
                    // 更新：删除发货单
                    $sql = "DELETE FROM " . $this->sanmu->table('delivery_order') . " WHERE delivery_id = '$value_is'";
                    $this->db->query($sql);
                }
            ...
            elseif (isset($_REQUEST['remove_back'])) {
                ...
                if (is_array($back_id)) {
                    foreach ($back_id as $value_is) {
                        $sql = "DELETE FROM " . $this->sanmu->table('back_order') . " WHERE back_id = '$value_is'";
                        $this->db->query($sql);
                    }
                } else {
                    $sql = "DELETE FROM " . $this->sanmu->table('back_order') . " WHERE back_id = '$back_id'";
                    $this->db->query($sql);
                }
                /* 返回 */
                ...
                foreach ($order_sn_list as $order_sn) {
                    if ($order_sn) {
                        ...
                        $sql = "SELECT o.*, c.measure_unit, g.goods_number AS storage, o.goods_attr, IFNULL(b.brand_name, '') AS brand_name, g.bar_code " .
                            "FROM " . $this->sanmu->table('order_goods') . " AS o " .
                            "LEFT JOIN " . $this->sanmu->table('goods') . " AS g ON o.goods_id = g.goods_id " .
                            "LEFT JOIN " . $this->sanmu->table('brand') . " AS b ON g.brand_id = b.brand_id " .
                            'LEFT JOIN ' . $this->sanmu->table('category') . ' AS c ON g.cat_id = c.cat_id ' .
                            "WHERE o.order_id = '$order[order_id]' ";
                        $res = $this->db->query($sql);
                        foreach ($res as $row) {
                            $products = $this->goodsWarehouseService->getWarehouseAttrNumber($row['goods_id'], $row['goods_attr_id'], $row['warehouse_id'], $row['area_id'], $row['area_city'], $row['model_attr']);
                        ...
                        /* 取得商家信息 by  kong */
                        $sql = "select shop_name,country,province,city,shop_address,kf_tel from " . $this->sanmu->table('seller_shopinfo') . " where ru_id='" . $order['ru_id'] . "'";
                        $store = $this->db->getRow($sql);

                        $store['shop_name'] = $this->merchantCommonService->getShopName($order['ru_id'], 1);

                        $sql = "SELECT domain_name FROM " . $this->sanmu->table("seller_domain") . " WHERE ru_id = '" . $order['ru_id'] . "' AND  is_enable = 1"; //获取商家域名
                ...
                foreach ($delivery_id as $value_is) {
                    ...
                    if ($order_finish) {
                        ...
                        if ($cfg == '1') {
                            ...
                            $this->smarty->assign('send_date', local_date($GLOBALS['_CFG']['time_format'], gmtime()));
                            $this->smarty->assign('sent_date', local_date($GLOBALS['_CFG']['time_format'], gmtime()));
                            //$this->smarty->assign('confirm_url', $this->sanmu->url() . 'receive.php?id=' . $order['order_id'] . '&con=' . rawurlencode($order['consignee']));
                            $this->smarty->assign('confirm_url', $this->sanmu->url() . 'user_order.php?act=order_detail&order_id=' . $order['order_id']); //by wu
                            $this->smarty->assign('send_msg_url', $this->sanmu->url() . 'user_message.php?act=message_list&order_id=' . $order['order_id']);
                            $content = $this->smarty->fetch('str:' . $tpl['template_content']);
                            if (!$this->commonRepository->sendEmail($order['consignee'], $order['email'], $tpl['template_subject'], $content, $tpl['is_html'])) {
            ...
            $sql = "select log_id from {$this->sanmu->table('baitiao_log')} where order_id" . db_create_in(explode(',', $order_id));
            ...
            if ($require_note || isset($show_invoice_no) || isset($show_refund)) {
                ...
                if (!$batch) {
                    ...
                    if (!empty($_REQUEST['ret_id'])) {
                        return sanmu_header("Location: order.php?act=operate_post&order_id=" . $order_id .
                    ...
                    /* 多个订单 */
                    return sanmu_header("Location: order.php?act=batch_operate_post&order_id=" . $order_id .
                        "&operation=" . $operation . "&action_note=" . urlencode($action_note) . "\n");
                }
            }
        }
        ...
        elseif ($_REQUEST['act'] == 'batch_operate_post') {
            ...
            if ($order_id) {
                $order_id = $this->sanmuRepository->delStrComma($order_id);
            ...
            if ('confirm' == $operation) {
                foreach ($order_id_list as $id_order) {
                    $sql = "SELECT * FROM " . $this->sanmu->table('order_info') .
            ...
            elseif ('invalid' == $operation) {
                ...
                foreach ($order_id_list as $id_order) {
                    ...
                    /* 删除订单 */
                    $this->db->query("DELETE FROM " . $this->sanmu->table('order_info') . " WHERE order_id = '$order[order_id]'");
                    $this->db->query("DELETE FROM " . $this->sanmu->table('order_goods') . " WHERE order_id = '$order[order_id]'");
                    $this->db->query("DELETE FROM " . $this->sanmu->table('order_action') . " WHERE order_id = '$order[order_id]'");

                    $this->db->query("DELETE FROM " . $this->sanmu->table('store_order') . " WHERE order_id = '$order[order_id]'");
...
//    $action_note = $_REQUEST['action_note'];
        ...
        elseif ($_REQUEST['act'] == 'operate_post') {
            ...
            elseif ('pay' == $operation) {
                ...
                $sql = 'SELECT store_id FROM ' . $this->sanmu->table("store_order") . " WHERE order_id = '$order_id' LIMIT 1";
                ...
                //门店处理 付款成功发送短信
                $sql = 'SELECT store_id,pick_code,order_id FROM ' . $this->sanmu->table("store_order") . " WHERE order_id = '$order_id' LIMIT 1";
                $stores_order = $this->db->getRow($sql);
                $user_mobile_phone = '';
                $sql = "SELECT mobile_phone,user_name FROM " . $this->sanmu->table('users') . " WHERE user_id = '" . $order['user_id'] . "' LIMIT 1";
                ...
                if ($user_mobile_phone != '') {
                    //门店短信处理
                    $store_smsParams = '';
                    $sql = "SELECT id, country, province, city, district, stores_address, stores_name, stores_tel FROM " . $this->sanmu->table('offline_store') . " WHERE id = '" . $stores_order['store_id'] . "' LIMIT 1";
                ...
                if (($arr['order_status'] == OS_CONFIRMED || $arr['order_status'] == OS_SPLITED) && $arr['pay_status'] == PS_PAYED && (isset($arr['shipping_status']) && $arr['shipping_status'] == SS_RECEIVED)) {
                    ...
                    $seller_id = $this->db->getOne("SELECT ru_id FROM " . $this->sanmu->table('order_goods') . " WHERE order_id = '$order_id'", true);
                    $value_card = $this->db->getOne("SELECT use_val FROM " . $this->sanmu->table('value_card_record') . " WHERE order_id = '$order_id'", true);
                ...
                if (true) {
                    ...
                    foreach ($goods_list as $key => $value) {
                        ...
                        elseif ($value['extension_code'] == 'virtual_card' || $value['is_real'] == 0) {
                            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('virtual_card') . " WHERE goods_id = '" . $value['goods_id'] . "' AND is_saled = 0 ";
            ...
            elseif ('receive' == $operation) {
                ...
                //更改智能权重里的商品统计数量
                $sql = 'SELECT goods_id, goods_number FROM ' . $this->sanmu->table('order_goods') . ' WHERE order_id =' . $order_id;
                $res = $this->db->getAll($sql);

                foreach ($res as $val) {
                    $sql = 'SELECT user_id FROM ' . $this->sanmu->table('order_goods') . ' WHERE goods_id=' . $val['goods_id'] . ' GROUP BY user_id';
                ...
                if (!$bill_order) {
                    $seller_id = $this->db->getOne("SELECT ru_id FROM " . $this->sanmu->table('order_goods') . " WHERE order_id = '" . $order['order_id'] . "'", true);
                    $value_card = $this->db->getOne("SELECT use_val FROM " . $this->sanmu->table('value_card_record') . " WHERE order_id = '" . $order['order_id'] . "'", true);
            ...
            elseif ('agree_apply' == $operation) {
                ...
                $this->db->autoExecute($this->sanmu->table('order_return'), $arr, 'UPDATE', "rec_id = '$rec_id'");
            ...
            elseif ('receive_goods' == $operation) {
                ...
                $res = $this->db->autoExecute($this->sanmu->table('order_return'), $arr, 'UPDATE', "rec_id = '$rec_id'");
                if ($res) {
                    //获取退换货商品ID
                    $sql = 'SELECT goods_id,return_number FROM ' . $this->sanmu->table('return_goods') . ' WHERE rec_id = ' . $rec_id;
                    $res = $this->db->getRow($sql);
             ...
             */ elseif ('complete' == $operation) {
                ...
                $sql = "SELECT return_type FROM " . $this->sanmu->table('order_return') . " WHERE rec_id = '$rec_id'";
            ...
            elseif ('refound' == $operation) {
                ...
                if ($is_diff) {
                    ...
                    if ($return_count == 1) {
                        ...
                        //退还红包
                        $bonus = $order['bonus'];
                        $sql = "UPDATE " . $this->sanmu->table('user_bonus') . " SET used_time = '' , order_id = '' WHERE order_id = " . $order_id;
                        $this->db->query($sql);
                ...
                if ($order['pay_status'] != PS_UNPAYED) {
                    ...
                    if (!empty($_REQUEST['action_note'])) {
                        ...
                        $this->db->autoExecute($this->sanmu->table('order_return'), $return_status, 'UPDATE', "ret_id = '$ret_id'");
            ...
            elseif ('return' == $operation) {
                ...
                if ($order['user_id'] > 0) {
                    ...
                    $sql = "SELECT  goods_number, send_number FROM" . $this->sanmu->table('order_goods') . "
                ...
                if ($delivery_list) {
                    foreach ($delivery_list as $list) {
                        ...
                        $sql_back_goods = "INSERT INTO " . $this->sanmu->table('back_goods') . " (back_id, goods_id, product_id, product_sn, goods_name,goods_sn, is_real, send_number, goods_attr)
                        ...
                        if ($res) {
                            //获取退换货商品ID
                            $sql = 'SELECT goods_id,send_number FROM ' . $this->sanmu->table('back_goods') . ' WHERE back_id = ' . $back_id;
                            $res = $this->db->getRow($sql);
                ...
                /* 修改订单的发货单状态为退货 */
                $sql_delivery = "UPDATE " . $this->sanmu->table('delivery_order') . "
                         SET status = 1
                         WHERE status IN (0, 2)
                         AND order_id = '" . $order['order_id'] . "'";
                $this->db->query($sql_delivery, 'SILENT');

                $sql = "SELECT goods_id, send_number FROM " . $this->sanmu->table('order_goods') . " WHERE order_id = '" . $order['order_id'] . "' AND extension_code <> 'virtual_card' AND send_number >= 1";
                $goods_list = $this->db->getAll($sql);

                if ($goods_list) {
                    foreach ($goods_list as $key => $row) {
                        $sql = "UPDATE " . $this->sanmu->table('goods') . " SET sales_volume = sales_volume - '" . $row['send_number'] . "' WHERE goods_id = '" . $row['goods_id'] . "'";
                        $this->db->query($sql);
                    }
                }
        ...
        elseif ($_REQUEST['act'] == 'json') {
            ...
            if ($func == 'get_goods_info') {
                ...
                $leftJoin = " left join " . $this->sanmu->table('warehouse_goods') . " as wg on g.goods_id = wg.goods_id and wg.region_id = '$warehouse_id' ";
                $leftJoin .= " left join " . $this->sanmu->table('warehouse_area_goods') . " as wag on g.goods_id = wag.goods_id and wag.region_id = '$area_id' ";
                ...
                $sql = "SELECT g.goods_id, c.cat_name, g.goods_sn, g.goods_name, b.brand_name, g.market_price, g.model_attr, g.user_id, " .
                    ...
                    "FROM " . $this->sanmu->table('goods') . " AS g " .
                    $leftJoin .
                    "LEFT JOIN " . $this->sanmu->table('brand') . " AS b ON g.brand_id = b.brand_id " .
                    "LEFT JOIN " . $this->sanmu->table('category') . " AS c ON g.cat_id = c.cat_id " .
                ...
                $sql = "SELECT p.user_price, r.rank_name " .
                    "FROM " . $this->sanmu->table('member_price') . " AS p, " .
                    $this->sanmu->table('user_rank') . " AS r " .
                ...
                $sql = "SELECT * FROM " . $this->sanmu->table($table_products) . " WHERE goods_id = '$goods_id'" . $type_files . " LIMIT 1";
                $prod = $this->db->getRow($sql);
        ...
        elseif ($_REQUEST['act'] == 'search_users') {
            ...
            if ($id_name != '') {
                $sql = "SELECT user_id, user_name FROM " . $this->sanmu->table('users') .
                ...
                foreach ($res as $row) {
                    ...
                    if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                        $row['user_name'] = $this->sanmuRepository->stringToStar($row['user_name']);
                    }
        ...
        elseif ($_REQUEST['act'] == 'search_goods') {
            ...
            if ($goods_count) {
                $sql = "SELECT ru_id FROM " . $this->sanmu->table('order_goods') . " WHERE order_id = '$order_id'";
            ...
            if ($keyword != '') {
                $sql = "SELECT goods_id, goods_name, goods_sn, user_id FROM " . $this->sanmu->table('goods') .
                    " WHERE is_delete = 0" .
                    " AND is_on_sale = 1" .
                    $where .
                    " AND is_alone_sale = 1" .
        ...
        elseif ($_REQUEST['act'] == 'edit_invoice_no') {
            ...
            $sql = 'SELECT order_sn, order_status, shipping_status, pay_status, invoice_no FROM ' . $this->sanmu->table('order_info') . " WHERE order_id = '$order_id' LIMIT 1";
            $order = $this->db->getRow($sql);

            $sql = 'UPDATE ' . $this->sanmu->table('order_info') . " SET invoice_no='$no' WHERE order_id = '$order_id'";
            if ($this->db->query($sql)) {
                if (!empty($no) && $no != $order['invoice_no']) {
                    $sql = 'UPDATE ' . $this->sanmu->table('delivery_order') . " SET invoice_no = '$no' WHERE order_id = '$order_id'";
                    $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'edit_pay_note') {
            ...
            $sql = 'UPDATE ' . $this->sanmu->table('order_info') . " SET pay_note='$no' WHERE order_id = '$order_id'";
            if ($this->db->query($sql)) {
                if (empty($no)) {
                    return make_json_result('N/A');
                } else {
        ...
        elseif ($_REQUEST['act'] == 'update_info') {
            ...
            $order_id = $_REQUEST['order_id'];
            $sql = 'UPDATE ' . $this->sanmu->table('order_info') . 'set sign_time =' . $sign_time . ' WHERE order_id =' . $order_id;
            $this->db->query($sql);
        }
        ...
        elseif ($_REQUEST['act'] == 'set_grab') {
            ...
            if (isset($_REQUEST['checkboxes']) && count($_REQUEST['checkboxes']) > 0) {
                ...
                $store_order_info = get_store_order_info($order_id, 'order_id');
                if (empty($store_order_info)) {
                    $sql = " INSERT INTO " . $this->sanmu->table('store_order') . " (order_id, store_id, ru_id, is_grab_order, grab_store_list) " .
                        " VALUES ('$order_id', '0', '$ru_id', '1', '$grab_store_list') ";
                    $this->db->query($sql);
                } else {
                    $sql = " UPDATE " . $this->sanmu->table('store_order') . " SET grab_store_list = '$grab_store_list' WHERE order_id = '$order_id' ";
                    $this->db->query($sql);
                }
            } else {
                $sql = " DELETE FROM" . $this->sanmu->table('store_order') . " WHERE order_id = '$order_id' AND ru_id = '$ru_id' AND store_id = 0";
                $this->db->query($sql);
            }
        ...
        elseif ($_REQUEST['act'] == 'part_ship') {
            ...
            $sql = "SELECT o.*, g.model_inventory, g.model_attr AS model_attr, g.suppliers_id AS suppliers_id, g.goods_number AS storage, g.goods_thumb, o.goods_attr, IFNULL(b.brand_name, '') AS brand_name, p.product_sn, g.bar_code  " .
                "FROM " . $this->sanmu->table('order_goods') . " AS o " .
                "LEFT JOIN " . $this->sanmu->table('products') . " AS p ON o.product_id = p.product_id " .
                "LEFT JOIN " . $this->sanmu->table('goods') . " AS g ON o.goods_id = g.goods_id " .
                "LEFT JOIN " . $this->sanmu->table('brand') . " AS b ON g.brand_id = b.brand_id " .
    ...
    private function update_order_amount($order_id)
    {
        load_helper('order');
        //更新订单总金额
        $sql = "UPDATE " . $this->sanmu->table('order_info') .
    ...
    private function order_list($page = 0)
    {
        ...
        if ($result === false) {
            ...
            if ($filter['keywords']) {
                ...
                $where .= " OR (SELECT COUNT(*) FROM " . $this->sanmu->table('order_goods') . " AS iog WHERE iog.order_id = o.order_id " .
            ...
            if ($filter['store_search'] > -1) {
                if ($adminru['ru_id'] == 0) {
                    if ($filter['store_search'] > 0) {
                        ...
                        if ($filter['store_search'] > 1) {
                            $where .= " AND (SELECT COUNT(*) FROM " . $this->sanmu->table('merchants_shop_information') . ' AS msi ' .
            ...
            if ($filter['store_id'] > 0) {
                $where .= " AND (SELECT COUNT(*) FROM " . $this->sanmu->table('store_order') . " AS sto WHERE sto.order_id = o.order_id AND sto.store_id  = '" . $filter['store_id'] . "') > 0 ";
            }
            ...
            if ($filter['user_name']) {
                $where .= " AND (SELECT u.user_id FROM " . $this->sanmu->table('users') . " AS u WHERE u.user_name LIKE '%" . mysql_like_quote($filter['user_name']) . "%' LIMIT 1) = o.user_id";
            }
            ...
            $sql = "SELECT agency_id FROM " . $this->sanmu->table('admin_user') . " WHERE user_id = '" . session('seller_id') . "' AND action_list <> 'all'";
            $agency_id = $this->db->getOne($sql);
            ...
            if ($filter['order_cat']) {
                switch ($filter['order_cat']) {
                    case 'stages':
                        $where .= " AND (SELECT COUNT(*) FROM " . $this->sanmu->table('baitiao_log') . " AS b WHERE b.order_id = o.order_id) > 0 ";
                    ...
                    case 'zc':
                        ...
                        break;
                    case 'store':
                        $where .= " AND (SELECT COUNT(*) FROM " . $this->sanmu->table('store_order') . " AS s WHERE s.order_id = o.order_id) > 0 ";
            ...
            $page_size = request()->cookie('sanmucp_page_size');
            ...
            if (!empty($filter['start_take_time']) || !empty($filter['end_take_time'])) {
                ...
                $where .= " AND (SELECT COUNT(*) FROM " . $this->sanmu->table('order_action') . " AS oa WHERE o.order_id = oa.order_id $where_action) > 0";
            }
            ...
            switch ($filter['serch_type']) {
                ...
                case 100:
                    ...
                    break;
                case 101:
                    $where .= " AND (o.shipping_status = '" . SS_UNSHIPPED . "' OR o.shipping_status = '" . SS_PREPARING . "') AND o.pay_status = '" . PS_PAYED . "' AND o.order_status = '" . OS_CONFIRMED . "' AND (SELECT ore.ret_id FROM " . $this->sanmu->table('order_return') . " AS ore WHERE ore.order_id = o.order_id LIMIT 1) IS NULL "; //待发货
                    break;
                case 102:
                ...
                case 3:
                    ...
                    break;
                case 4:
                    $where .= "AND (SELECT COUNT(*) FROM " . $this->sanmu->table('order_return') . " AS ore WHERE ore.order_id = o.order_id AND ore.return_type IN(1, 3) AND ore.refound_status = 0 AND ore.return_status != 6) > 0 " .
            ...
            /* 记录总数 */
            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('order_info') . " AS o " .
                $where . $where_store;
            ...
            /* 查询 */
            $select = "(SELECT bai.is_stages FROM " . $this->sanmu->table('baitiao_log') . " as bai WHERE o.order_id=bai.order_id LIMIT 1) AS is_stages,";
        ...
        foreach ($row as $key => $value) {
            ...
            //判断订单是不是门店抢单订单
            $sql = "SELECT id FROM " . $this->sanmu->table('store_order') . " WHERE order_id = '" . $value['order_id'] . "'";
                    ...
                    if ($payment && strpos($payment['pay_code'], 'pay_') === false) {
                        ...
                        if (!is_null($payObj)) {
                            ...
                            if (is_callable([$payObj, 'orderQuery'])) {
                                ...
                                $sql = "SELECT order_status, shipping_status, pay_status, pay_time FROM " . $this->sanmu->table('order_info') . " WHERE order_id = '" . $value['order_id'] . "' LIMIT 1";
                                $order_info = $this->db->getRow($sql);
                                if ($order_info) {
            ...
            if ($GLOBALS['_CFG']['open_delivery_time'] == 1) {
                if (($value['order_status'] == OS_CONFIRMED || $value['order_status'] == OS_SPLITED) && $value['pay_status'] == PS_PAYED && $value['shipping_status'] == SS_SHIPPED && $value['chargeoff_status'] == 0) {
                    ...
                    if ($confirm_take_time > $delivery_time) { //自动确认发货操作
                        ...
                        $sql = "UPDATE " . $this->sanmu->table('order_info') . " SET order_status = '" . $value['order_status'] . "', " .
                        ...
                        $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('seller_bill_order') . " WHERE order_id = '" . $value['order_id'] . "'";
                        ...
                        if ($bill_order_info <= 0) {
                            ...
                            $value_card = $this->db->getOne("SELECT use_val FROM " . $this->sanmu->table('value_card_record') . " WHERE order_id = '" . $value['order_id'] . "'", true);
                    ...
                    if (empty($value['confirm_take_time'])) {
                        ...
                        if ($bill_order_count > 0 && $value['shipping_status'] == SS_RECEIVED) {
                            $sql = "SELECT MAX(log_time) AS log_time FROM " . $this->sanmu->table('order_action') . " WHERE order_id = '" . $value['order_id'] . "' AND shipping_status = '" . SS_RECEIVED . "'";
                            $confirm_take_time = $this->db->getOne($sql, true);
                            ...
                            $this->db->autoExecute($this->sanmu->table('order_info'), $log_other, 'UPDATE', "order_id = '" . $value['order_id'] . "'");
                            $this->db->autoExecute($this->sanmu->table('seller_bill_order'), $log_other, 'UPDATE', "order_id = '" . $value['order_id'] . "'");
                        }
                    }
                }
            }
            ...
            if ($value['chargeoff_status'] > 0) {
                $bill = $this->db->getRow(" SELECT scb.id, scb.bill_sn, scb.seller_id, scb.proportion, scb.commission_model FROM " . $this->sanmu->table('seller_bill_order') . " AS sbo " .
                    "LEFT JOIN " . $this->sanmu->table('seller_commission_bill') . " AS scb ON sbo.bill_id = scb.id " .
                    "WHERE sbo.order_id = '" . $value['order_id'] . "' ");
            } else {
                $sql = "SELECT bill_id, chargeoff_status FROM " . $this->sanmu->table('seller_bill_order') . " WHERE order_id = '" . $value['order_id'] . "' AND chargeoff_status > 0";
                $bill = $this->db->getRow($sql);

                if ($bill) {
                    $bill = $this->db->getRow("SELECT * FROM " . $this->sanmu->table('seller_commission_bill') .
                        "WHERE id = '" . $bill['bill_id'] . "'");

                    $sql = "UPDATE " . $this->sanmu->table('order_info') . " SET chargeoff_status = '" . $bill['chargeoff_status'] . "' WHERE order_id = '" . $value['order_id'] . "'";
                    $this->db->query($sql);
                }
            }
            ...
            //查商家ID
            $sql = "SELECT ru_id, extension_code AS iog_extension_code FROM " . $this->sanmu->table('order_goods') . " WHERE order_id = '" . $value['order_id'] . "' ";
            ...
            //查会员名称
            $sql = " SELECT user_name FROM " . $this->sanmu->table('users') . " WHERE user_id = '" . $value['user_id'] . "'";
            ...
            //判断是否为门店订单 by wu
            $is_store_order = $this->db->getOne(" SELECT COUNT(*) FROM " . $this->sanmu->table('store_order') . " WHERE order_id = '" . $value['order_id'] . "' ", true);
            $row[$key]['is_store_order'] = empty($is_store_order) ? 0 : 1;

            //判断是否为退换货订单 start
            $sql = "SELECT ret_id FROM " . $this->sanmu->table('order_return') . " WHERE order_id =" . $value['order_id'];
            ...
            if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                $row[$key]['mobile'] = $this->sanmuRepository->stringToStar($value['mobile']);
                $row[$key]['buyer'] = $this->sanmuRepository->stringToStar($row[$key]['buyer']);
                $row[$key]['tel'] = $this->sanmuRepository->stringToStar($value['tel']);
                $row[$key]['email'] = $this->sanmuRepository->stringToStar($value['email']);
            }
        }
    ...
    {
        $sql = 'SELECT *
            FROM ' . $this->sanmu->table('suppliers') . '
            WHERE is_check = 1
    ...
    private function get_order_goods($order)
    {
        ...
        $sql = "SELECT o.*, o.extension_code AS iog_extension_code, g.model_inventory, g.model_attr AS model_attr, g.suppliers_id AS suppliers_id, g.goods_number AS storage, g.goods_thumb, g.goods_img, o.goods_attr, IFNULL(b.brand_name, '') AS brand_name, p.product_sn, g.bar_code, g.goods_unit, " .
            ...
            "LEFT JOIN " . $this->sanmu->table('goods') . " AS g ON o.goods_id = g.goods_id " .
            "LEFT JOIN " . $this->sanmu->table('brand') . " AS b ON g.brand_id = b.brand_id " .
            "LEFT JOIN " . $this->sanmu->table('order_info') . " AS oi ON oi.order_id = o.order_id " .
            "WHERE o.order_id = '$order[order_id]' ";
        $res = $this->db->query($sql);

        if ($res) {
            foreach ($res as $row) {
                $sql = "SELECT ret_id FROM " . $this->sanmu->table('order_return') . " WHERE rec_id = '" . $row['rec_id'] . "'";
                ...
                if (empty($row['product_sn'])) {
                    ...
                    $sql = "SELECT product_sn FROM " . $this->sanmu->table($table) . " WHERE product_id = '" . $row['product_id'] . "'";
                ...
                if ($trade_id) {
                    $row['trade_url'] = $this->sanmuRepository->sanmuUrl("trade_snapshot.php?act=trade&tradeId=" . $trade_id . "&snapshot=true");
                }

                $row['back_order'] = return_order_info($row['ret_id']);

                //处理商品链接
                $row['url'] = $this->sanmuRepository->buildUri('goods', ['gid' => $row['goods_id']], $row['goods_name']);
                ...
                $sql = "SELECT is_reality, is_return, is_fast FROM " . $this->sanmu->table('goods_extend') . " WHERE goods_id = '" . $row['goods_id'] . "' LIMIT 1";
    ...
    private function get_package_goods_list($package_id)
    {
        $sql = "SELECT pg.goods_id, g.goods_name, (CASE WHEN pg.product_id > 0 THEN p.product_number ELSE g.goods_number END) AS goods_number, p.goods_attr, p.product_id, pg.goods_number AS
            ...
            FROM " . $this->sanmu->table('package_goods') . " AS pg
                LEFT JOIN " . $this->sanmu->table('goods') . " AS g ON pg.goods_id = g.goods_id
                LEFT JOIN " . $this->sanmu->table('products') . " AS p ON pg.product_id = p.product_id
        ...
        if ($good_product_str != '') {
            $sql = "SELECT ga.goods_attr_id, ga.attr_value, ga.attr_price, a.attr_name
                FROM " . $this->sanmu->table('goods_attr') . " AS ga, " . $this->sanmu->table('attribute') . " AS a
    ...
    private function order_delivery_num($order_id, $goods_id, $product_id = 0)
    {
        $sql = 'SELECT SUM(G.send_number) AS sums
            FROM ' . $this->sanmu->table('delivery_goods') . ' AS G, ' . $this->sanmu->table('delivery_order') . ' AS O
    ...
    private function get_order_finish($order_id)
    {
        ...
        $sql = 'SELECT COUNT(rec_id)
            FROM ' . $this->sanmu->table('order_goods') . '
            WHERE order_id = \'' . $order_id . '\'
    ...
    private function del_order_delivery($order_id)
    {
        ...
        $sql = 'DELETE O, G
            FROM ' . $this->sanmu->table('delivery_order') . ' AS O, ' . $this->sanmu->table('delivery_goods') . ' AS G
            WHERE O.order_id = \'' . $order_id . '\'
            AND O.status = 0
    ...
    private function del_delivery($order_id, $action_array)
    {
        ...
        if (in_array('back', $action_array)) {
            $sql = 'DELETE O, G
                FROM ' . $this->sanmu->table('back_order') . ' AS O, ' . $this->sanmu->table('back_goods') . ' AS G
                WHERE O.order_id = \'' . $order_id . '\'
    ...
    private function delivery_list()
    {
        ...
        if ($result === false) {
            ...
            if ($filter['goods_id']) {
                $where .= " AND (SELECT dg.goods_id FROM " . $this->sanmu->table('delivery_goods') . " AS dg WHERE dg.delivery_id = do.delivery_id LIMIT 1) = '" . $filter['goods_id'] . "' ";
            }
            if ($filter['consignee']) {
            ...
            $leftJoin = " LEFT JOIN " . $this->sanmu->table('order_info') . " AS o ON o.order_id = do.order_id ";

            /* 记录总数 */
            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('delivery_order') . " as do " . $leftJoin . $where;
            $filter['record_count'] = $this->db->getOne($sql);
            $filter['page_count'] = $filter['record_count'] > 0 ? ceil($filter['record_count'] / $filter['page_size']) : 1;
            ...
            $sql = "SELECT do.delivery_id, do.delivery_sn, do.order_sn, do.order_id, do.add_time, do.action_user, do.consignee, do.country,
                ...
                FROM " . $this->sanmu->table("delivery_order") . " as do " .
                $leftJoin . $where .
                " ORDER BY " . $filter['sort_by'] . " " . $filter['sort_order'] . "
                LIMIT " . ($filter['page'] - 1) * $filter['page_size'] . ", " . $filter['page_size'] . " ";
        ...
        foreach ($row as $key => $value) {
            ...
            $sql = "SELECT ru_id FROM " . $this->sanmu->table('order_goods') . " WHERE order_id = '" . $value['order_id'] . "' LIMIT 0,1";
    ...
    private function back_list()
    {
        ...
        $leftJoin = " LEFT JOIN " . $this->sanmu->table('order_info') . " AS o ON o.order_id = bo.order_id ";

        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('back_order') . " as bo " . $leftJoin . $where;
        $filter['record_count'] = $this->db->getOne($sql);
        $filter['page_count'] = $filter['record_count'] > 0 ? ceil($filter['record_count'] / $filter['page_size']) : 1;
        ...
        $sql = "SELECT bo.back_id, bo.delivery_sn, bo.order_sn, bo.order_id, bo.add_time, bo.action_user, bo.consignee, bo.country,
                ...
                FROM " . $this->sanmu->table("back_order") . " as bo " . $leftJoin .
            $where .
            " ORDER BY " . $filter['sort_by'] . " " . $filter['sort_order'] . "
                LIMIT " . ($filter['page'] - 1) * $filter['page_size'] . ", " . $filter['page_size'] . " ";
    ...
    private function package_sended($package_id, $goods_id, $order_id, $extension_code, $product_id = 0)
    {
        ...
        $sql = "SELECT SUM(DG.send_number)
            FROM " . $this->sanmu->table('delivery_goods') . " AS DG, " . $this->sanmu->table('delivery_order') . " AS o
    ...
    private function package_virtual_card_shipping($goods, $order_sn)
    {
        ...
        foreach ($goods as $virtual_goods_key => $virtual_goods_value) {
            ...
            $sql = "SELECT card_id, card_sn, card_password, end_date, crc32
                FROM " . $this->sanmu->table('virtual_card') . "
            ...
            /* 标记已经取出的卡片 */
            $sql = "UPDATE " . $this->sanmu->table('virtual_card') . " SET " .
                "is_saled = 1 ," .
                "order_sn = '$order_sn' " .
            ...
            /* 获取订单信息 */
            $sql = "SELECT order_id, order_sn, consignee, email FROM " . $this->sanmu->table('order_info') . " WHERE order_sn = '$order_sn'";
            $order = $this->db->GetRow($sql);
    ...
    private function del_order_invoice_no($order_id, $delivery_invoice_no)
    {
        ...
        $sql = "SELECT invoice_no
            FROM " . $this->sanmu->table('order_info') . "
            WHERE order_id = '$order_id'";
