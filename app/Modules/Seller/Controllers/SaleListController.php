...
class SaleListController extends InitController
{
    ...
    public function index()
    {
        ...
        if (isset($_REQUEST['act']) && ($_REQUEST['act'] == 'query' || $_REQUEST['act'] == 'download')) {
            ...
            if ($_REQUEST['act'] == 'download') {
                ...
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['order_status']) . "\t";
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['shipping_status']) . "\t\n";
                ...
                foreach ($goods_sales_list['sale_list_data'] as $key => $value) {
                    ...
                    echo sanmu_iconv(EC_CHARSET, 'GB2312', $value['order_status_format']) . "\t";
                    echo sanmu_iconv(EC_CHARSET, 'GB2312', $value['shipping_status_format']) . "\t";
                    echo "\n";
                }
            } else {
    ...
    private function get_sale_list($is_pagination = true)
    {
        ...
        $sql = "SELECT COUNT(og.goods_id) FROM " .
            $this->sanmu->table('order_info') . ' AS oi,' .
            $this->sanmu->table('order_goods') . ' AS og ' . $leftJoin .
            $where;
        ...
        $sql = 'SELECT og.goods_id, og.goods_sn, og.goods_name, og.goods_number AS goods_num, og.ru_id, og.goods_price ' .
            ...
            "FROM " . $this->sanmu->table('order_goods') . " AS og, " . $this->sanmu->table('order_info') . " AS oi " . $leftJoin .
