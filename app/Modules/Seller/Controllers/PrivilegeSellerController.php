...
class PrivilegeSellerController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'insert') {
            ...
            if (session('seller_id') > 0) {
                $res = $this->db->getRow(" SELECT ru_id,action_list FROM " . $this->sanmu->table("admin_user") . " WHERE user_id = '" . session('seller_id') . "'");
                $action_list = $res['action_list'];
                $ru_id = $res['ru_id'];
            }
            $sql = "SELECT nav_list FROM " . $this->sanmu->table('admin_user') . " WHERE action_list = 'all'";
            $row = $this->db->getRow($sql);
            $sql = "INSERT INTO " . $this->sanmu->table('admin_user') . " (user_name, email, password, add_time, nav_list,action_list,ru_id,parent_id) " .
        ...
        elseif ($_REQUEST['act'] == 'allot') {
            ...
            $this->smarty->assign('primary_cat', $GLOBALS['_LANG']['10_priv_admin']);
            /* 获得该管理员的权限 */
            $priv_str = $this->db->getOne("SELECT action_list FROM " . $this->sanmu->table('admin_user') . " WHERE user_id = '$_GET[id]'");
            ...
            $user_parent = $this->db->getRow("SELECT action_list, parent_id FROM " . $this->sanmu->table('admin_user') . " WHERE user_id = '$user_id'");
            $parent_priv = $this->db->getRow("SELECT action_list FROM " . $this->sanmu->table('admin_user') . " WHERE user_id = '" . $user_parent['parent_id'] . "'");
        ...
        elseif ($_REQUEST['act'] == 'update_allot') {
            ...
            /* 取得当前管理员用户名 */
            $admin_name = $this->db->getOne("SELECT user_name FROM " . $this->sanmu->table('admin_user') . " WHERE user_id = '" . intval($_POST['id']) . "'");
    ...
    private function get_admin_userlist($ru_id)
    {
        ...
        if ($result === false) {
            ...
            /* 记录总数 */
            $sql = 'SELECT COUNT(*) FROM ' . $this->sanmu->table('admin_user') . " AS au " . " WHERE 1 AND ru_id > 0 $where";
