...
class ShippingController extends InitController
{
    ...
    {
        $exc = new Exchange($this->sanmu->table('shipping'), $this->db, 'shipping_code', 'shipping_name');
        $menus = session('menus', '');
        ...
        elseif ($_REQUEST['act'] == 'recovery_default_template') {
            ...
            if (file_exists($modules)) {
                ...
                /* 恢复默认 */
                $this->db->query("UPDATE " . $this->sanmu->table('shipping_tpl') . " SET print_bg = '" . addslashes($modules['print_bg']) . "',  config_lable = '" . addslashes($modules['config_lable']) . "' WHERE shipping_id = '$shipping_id' and ru_id='" . $adminru['ru_id'] . "' LIMIT 1");
            }
