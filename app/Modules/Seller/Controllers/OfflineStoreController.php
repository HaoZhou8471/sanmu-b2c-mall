...
class OfflineStoreController extends InitController
{
    ...
    {
        /*交换对象*/
        $exc = new Exchange($this->sanmu->table("offline_store"), $this->db, 'id', 'stores_user', 'stores_name', 'is_confirm', 'stores_tel', 'stores_opening_hours');
        $sto = new Exchange($this->sanmu->table("store_user"), $this->db, 'id', 'stores_user', 'store_id');
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            if ($_REQUEST['act'] == 'add') {
                ...
                $sql = "SELECT * FROM " . $this->sanmu->table("offline_store") . " WHERE id = '$id' LIMIT 1";
                $offline_store = $this->db->getRow($sql);

                $store_user_info = $this->db->getRow("SELECT stores_user, email FROM " . $this->sanmu->table('store_user') . " WHERE store_id = '" . $offline_store['id'] . "' AND parent_id = 0");

                $offline_store['stores_user'] = $store_user_info['stores_user'];
                $offline_store['email'] = $store_user_info['email'];
                $offline_store['stores_img'] = $this->sanmuRepository->getImagePath($offline_store['stores_img']);
            ...
            if ($_REQUEST['act'] == 'insert') {
                ...
                $this->db->autoExecute($this->sanmu->table('offline_store'), $offline_store);
                ...
                if ($store_id) {
                    ...
                    $this->db->autoExecute($this->sanmu->table('store_user'), $store_user);
                ...
                $sql = "SELECT stores_img FROM " . $this->sanmu->table('offline_store') . " WHERE id = '$id'";
                ...
                $sql = "SELECT ec_salt FROM " . $this->sanmu->table('store_user') . " WHERE store_id = '$id' AND parent_id = 0";
                ...
                $this->db->autoExecute($this->sanmu->table('offline_store'), $offline_store, 'UPDATE', "id = '$id'");

                $sql = "UPDATE" . $this->sanmu->table('store_user') . " SET $where stores_user='$stores_user' , tel = '$stores_tel' ,email='$email' WHERE store_id = '$id' AND parent_id = 0";
                $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'remove') {
            ...
            $exc->drop($id);
            // 删除门店会员
            $sql = 'DELETE FROM ' . $this->sanmu->table('store_user') . " WHERE store_id = " . $id;
            $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'batch_remove') {
            ...
            if ($_REQUEST['batch_handle'] == 'open_batch' || $_REQUEST['batch_handle'] == 'off_batch') {
                ...
                $sql = 'UPDATE' . $this->sanmu->table('offline_store') . " SET is_confirm = '$is_confirm' WHERE id" . db_create_in($checkboxes);
                ...
                if (!empty($checkboxes)) {
                    /*处理上传图片*/
                    $sql = " SELECT stores_img FROM" . $this->sanmu->table("offline_store") . "WHERE id" . db_create_in($checkboxes);
                    ...
                    // 删除门店
                    $sql = 'DELETE FROM ' . $this->sanmu->table('offline_store') . " WHERE id" . db_create_in($checkboxes);
                    $res = $this->db->query($sql);
                    // 删除门店会员
                    $sql = 'DELETE FROM ' . $this->sanmu->table('store_user') . " WHERE store_id" . db_create_in($checkboxes);
                    $this->db->query($sql);
    ...
    private function get_offline_store_list($ru_id)
    {
        ...
        if ($result === false) {
            ...
            if ($filter['stores_user']) {
                $sql = "SELECT store_id FROM" . $this->sanmu->table('store_user') . "stores_user LIKE '%" . mysql_like_quote($filter['stores_user']) . "%' AND parent_id = 0";
            ...
            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('offline_store') . $where;
            ...
            $sql = "SELECT o.ru_id,o.id,o.stores_name,o.stores_address,o.stores_tel,o.stores_opening_hours,"
                ...
                . "FROM" . $this->sanmu->table('offline_store') . " AS o "
                . "LEFT JOIN " . $this->sanmu->table('region') . " AS a ON a.region_id = o.country "
                . "LEFT JOIN " . $this->sanmu->table('region') . " AS b ON b.region_id = o.province "
                . "LEFT JOIN " . $this->sanmu->table('region') . " AS c ON c.region_id = o.city "
                . "LEFT JOIN " . $this->sanmu->table('region') . " AS d ON d.region_id = o.district $where ORDER BY o.id ASC LIMIT " . $filter['start'] . "," . $filter['page_size'];
        ...
        foreach ($row as $k => $v) {
            ...
            $row[$k]['stores_user'] = $this->db->getOne("SELECT stores_user FROM" . $this->sanmu->table('store_user') . " WHERE store_id = '" . $v['id'] . "' AND parent_id = 0");
    ...
    private function get_data_list($type = 0)
    {
        ...
        if ($type != 0) {
            ...
            if ($result === false) {
                ...
                if (isset($filter['store_name']) && $filter['store_name']) {
                    $sql = "SELECT id FROM" . $this->sanmu->table('offline_store') . " WHERE stores_name LIKE '%" . mysql_like_quote($filter['store_name']) . "%'";
                ...
                $sql = "SELECT og.goods_id, og.order_id, og.goods_id, og.goods_name, og.ru_id, og.goods_sn, og.goods_price, o.add_time, " .
                    ...
                    " FROM " . $this->sanmu->table('order_goods') . " AS og " .
                    " LEFT JOIN " . $this->sanmu->table('order_info') . " AS o " . " ON o.order_id = og.order_id " .
                    "LEFT JOIN " . $this->sanmu->table("store_order") . " AS sto ON sto.order_id = o.order_id " .
                    " LEFT JOIN " . $this->sanmu->table('goods') . " AS g " . " ON g.goods_id = og.goods_id " .
        ...
        if ($type != 0) {
            for ($i = 0; $i < count($data_list); $i++) {
                ...
                $data_list[$i]['stores_name'] = $this->db->getOne("SELECT stores_name FROM " . $this->sanmu->table('offline_store') . "  WHERE id = '" . $data_list[$i]['store_id'] . "' ");
