...
class GoodsWarehouseAttrController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'edit_product_sn') {
            ...
            /* 修改 */
            $sql = "UPDATE " . $this->sanmu->table('products_warehouse') . " SET product_sn = '$product_sn' WHERE product_id = '$product_id'";
        ...
        elseif ($_REQUEST['act'] == 'edit_bar_code') {
            ...
            /* 修改 */
            $sql = "UPDATE " . $this->sanmu->table('products_warehouse') . " SET bar_code = '$bar_code' WHERE product_id = '$product_id'";
            $result = $this->db->query($sql);
            if ($result) {
                clear_cache_files();
        ...
        elseif ($_REQUEST['act'] == 'edit_product_number') {
            ...
            /* 修改货品库存 */
            $sql = "UPDATE " . $this->sanmu->table('products_warehouse') . " SET product_number = '$product_number' WHERE product_id = '$product_id'";
            $result = $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'product_remove') {
            ...
            /* 删除货品 */
            $sql = "DELETE FROM " . $this->sanmu->table('products_warehouse') . " WHERE product_id = '$product_id'";
            $result = $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'product_add_execute') {
            ...
            foreach ($product['product_sn'] as $key => $value) {
                ...
                /* 插入货品表 */
                $sql = "INSERT INTO " . $this->sanmu->table('products_warehouse') . " (goods_id, goods_attr, product_sn, bar_code, product_price, product_number, warehouse_id)  VALUES ('" . $product['goods_id'] . "', '$goods_attr', '$value', '" . $product['bar_code'][$key] . "', '" . $product['product_price'][$key] . "', '" . $product['product_number'][$key] . "', '" . $product['warehouse_id'] . "')";
    ...
    private function warehouse_product_list($goods_id = 0)
    {
        ...
        if ($result === false) {
            ...
            $filter['record_count'] = $this->db->getOne("SELECT COUNT(*) FROM " . $this->sanmu->table('region_warehouse') . $ex_where);

            /* 分页大小 */
            $filter = page_and_size($filter);
            $sql = "SELECT region_id, region_name " .
                " FROM " . $this->sanmu->table('region_warehouse') . $ex_where .
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            $sql = "select count(*) from " . $this->sanmu->table('products_warehouse') . " where goods_id = '$goods_id' and warehouse_id = '" . $warehouse_list[$i]['region_id'] . "'";
    ...
    private function product_warehouse_list($goods_id, $conditions = '', $warehouse_id)
    {
        ...
        if ($result === false) {
            ...
            /* 记录总数 */
            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('products_warehouse') . " AS p WHERE goods_id = $goods_id $where";
            $filter['record_count'] = $this->db->getOne($sql);

            $sql = "SELECT product_id, goods_id, goods_attr, product_sn, product_price, product_number, bar_code
                FROM " . $this->sanmu->table('products_warehouse') . " AS g
    ...
    private function product_goods_attr_list($goods_id)
    {
        ...
        $sql = "SELECT goods_attr_id, attr_value FROM " . $this->sanmu->table('goods_attr') . " WHERE goods_id = '$goods_id'";
        $results = $this->db->getAll($sql);
    ...
    private function get_product_warehouse_info($product_id, $filed = '')
    {
        ...
        $sql = "SELECT $filed FROM  " . $this->sanmu->table('products') . " WHERE product_id = '$product_id'";
    ...
    private function check_product_warehouse_sn_exist($product_sn, $product_id = 0, $ru_id = 0, $type = 0)
    {
        ...
        if ($type == 1) {
            $sql = "SELECT g.goods_id FROM " . $this->sanmu->table('goods') . " AS g WHERE g.bar_code='$product_sn' AND g.user_id = '$ru_id'";
            if ($this->db->getOne($sql)) {
                return true;    //重复
            }
        } else {
            $sql = "SELECT g.goods_id FROM " . $this->sanmu->table('goods') . " AS g WHERE g.goods_sn='$product_sn' AND g.user_id = '$ru_id'";
            if ($this->db->getOne($sql)) {
                return true;    //重复
            }
        }

        $where = " AND (SELECT g.user_id FROM " . $this->sanmu->table('goods') . " AS g WHERE g.goods_id = p.goods_id LIMIT 1) = '$ru_id'";

        if (empty($product_id)) {
            if ($type == 1) {
                $sql = "SELECT p.product_id FROM " . $this->sanmu->table('products_warehouse') . " AS p " . "
    ...
    private function product_warehouse_number_count($goods_id, $conditions = '', $warehouse_id = 0)
    {
        ...
        $sql = "SELECT product_number
            FROM " . $this->sanmu->table('products_warehouse') . "
    ...
    private function handle_goods_attr($goods_id, $id_list, $is_spec_list, $value_price_list)
    {
        ...
        foreach ($id_list as $key => $id) {
            ...
            // 插入或更新记录
            $sql = "SELECT goods_attr_id FROM " . $this->sanmu->table('goods_attr') . " WHERE goods_id = '$goods_id' AND attr_id = '$id' AND attr_value = '$value' LIMIT 0, 1";
            $result_id = $this->db->getOne($sql);
            if (!empty($result_id)) {
                $sql = "UPDATE " . $this->sanmu->table('goods_attr') . "
                ...
                $sql = "INSERT INTO " . $this->sanmu->table('goods_attr') . " (goods_id, attr_id, attr_value, attr_price) " .
    ...
    private function check_goods_attr_exist($goods_attr, $goods_id, $product_id = 0, $warehouse_id = 0)
    {
        ...
        if (empty($product_id)) {
            $sql = "SELECT product_id FROM " . $this->sanmu->table('products_warehouse') . "
    ...
    private function check_product_sn_exist($product_sn, $product_id = 0)
    {
        ...
        $sql = "SELECT goods_id FROM " . $this->sanmu->table('goods') . "WHERE goods_sn='$product_sn'";
        if ($this->db->getOne($sql)) {
            return true;    //重复
        }
