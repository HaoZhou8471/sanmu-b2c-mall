...
class TagManageController extends InitController
{
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
            ...
            if ($is_insert) {
                $sql = 'INSERT INTO ' . $this->sanmu->table('tag') . '(tag_id, goods_id, tag_words)' .
        ...
        elseif ($_REQUEST['act'] == 'batch_drop') {
            ...
            if (isset($_POST['checkboxes'])) {
                ...
                foreach ($_POST['checkboxes'] as $key => $id) {
                    $sql = "DELETE FROM " . $this->sanmu->table('tag') . " WHERE tag_id='$id'";
                    $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'remove') {
            ...
            /* 鑾峰彇鍒犻櫎鐨勬爣绛剧殑鍚嶇О */
            $tag_name = $this->db->getOne("SELECT tag_words FROM " . $this->sanmu->table('tag') . " WHERE tag_id = '$id'");
    ...
    private function tag_is_only($name, $tag_id, $goods_id = '')
    {
        if (empty($goods_id)) {
            $db = $GLOBALS['db'];
            $sql = 'SELECT goods_id FROM ' . $this->sanmu->table('tag') . " WHERE tag_id = '$tag_id'";
            $row = $this->db->getRow($sql);
            $goods_id = $row['goods_id'];
        }

        $sql = 'SELECT COUNT(*) FROM ' . $this->sanmu->table('tag') . " WHERE tag_words = '$name'" .
    ...
    {
        $db = $GLOBALS['db'];
        $sql = 'UPDATE ' . $this->sanmu->table('tag') . " SET tag_words = '$name'";
        if (!empty($goods_id)) {
    ...
    private function get_tag_list()
    {
        ...
        $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('tag');
        $filter['record_count'] = $this->db->getOne($sql);

        $filter = page_and_size($filter);

        $sql = "SELECT t.tag_id, u.user_name, t.goods_id, g.goods_name, t.tag_words " .
            "FROM " . $this->sanmu->table('tag') . " AS t " .
            "LEFT JOIN " . $this->sanmu->table('users') . " AS u ON u.user_id=t.user_id " .
            "LEFT JOIN " . $this->sanmu->table('goods') . " AS g ON g.goods_id=t.goods_id " .
    ...
    {
        $sql = 'SELECT t.tag_id, t.tag_words, t.goods_id, g.goods_name FROM ' . $this->sanmu->table('tag') . ' AS t' .
            ' LEFT JOIN ' . $this->sanmu->table('goods') . ' AS g ON t.goods_id=g.goods_id' .
            " WHERE tag_id = '$tag_id'";
        $row = $this->db->getRow($sql);
