...
class GoodsAutoController extends InitController
{
    ...
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            $sql = "DELETE FROM " . $this->sanmu->table('auto_manage') . " WHERE item_id = '$goods_id' AND type = 'goods'";
            $this->db->query($sql);
            ...
            $this->db->autoReplace($this->sanmu->table('auto_manage'), ['item_id' => $id, 'type' => 'goods',
    ...
    private function get_auto_goods($ru_id)
    {
        ...
        if ($result === false) {
            ...
            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('goods') . " g" . $where;
            ...
            /* 查询 */
            $sql = "SELECT g.*,a.starttime,a.endtime FROM " . $this->sanmu->table('goods') . " g LEFT JOIN " . $this->sanmu->table('auto_manage') . " a ON g.goods_id = a.item_id AND a.type='goods'" . $where .
