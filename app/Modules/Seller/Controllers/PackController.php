...
class PackController extends InitController
{
    ...
    public function index()
    {
        ...
        $exc = new Exchange($this->sanmu->table("pack"), $this->db, 'pack_id', 'pack_name');
        ...
        if ($_REQUEST['act'] == 'insert') {
            ...
            $this->sanmuRepository->getOssAddFile([DATA_DIR . '/packimg/' . $img_name]);

            /*插入数据*/
            $sql = "INSERT INTO " . $this->sanmu->table('pack') . "(pack_name, pack_fee, free_money, pack_desc, pack_img, user_id)
        ...
        if ($_REQUEST['act'] == 'edit') {
            ...
            $sql = "SELECT pack_id, pack_name, pack_fee, free_money, pack_desc, pack_img FROM " . $this->sanmu->table('pack') . " WHERE pack_id='$_REQUEST[id]'";
        ...
        if ($_REQUEST['act'] == 'drop_pack_img') {
            ...
            $this->sanmuRepository->getDelBatch('', $pack_id, ['pack_img'], 'pack_id', Pack::whereRaw(1), 0, DATA_DIR . '/packimg/'); //删除图片

            $sql = "UPDATE " . $this->sanmu->table('pack') . " SET pack_img = '' WHERE pack_id = '$pack_id'";
            $this->db->query($sql);
        ...
        if ($_REQUEST['act'] == 'remove') {
            ...
            $this->sanmuRepository->getDelBatch('', $id, ['pack_img'], 'pack_id', Pack::whereRaw(1), 0, DATA_DIR . '/packimg/'); //删除图片
    ...
    private function packs_list($ru_id)
    {
        ...
        if ($result === false) {
            ...
            $sql = "SELECT count(*) FROM " . $this->sanmu->table('pack') . $where;
            ...
            $sql = "SELECT pack_id, user_id, pack_name, pack_img, pack_fee, free_money, pack_desc" .
                " FROM " . $this->sanmu->table('pack') . $where .
