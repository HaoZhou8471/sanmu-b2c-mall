...
class MerchantsCommissionController extends InitController
{
    ...
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            /* 获得该管理员的权限 */
            $priv_str = $this->db->getOne("SELECT action_list FROM " . $this->sanmu->table('admin_user') . " WHERE user_id = '" . session('seller_id') . "'");
        ...
        elseif ($_REQUEST['act'] == 'edit') {
            ...
            $sql = "SELECT ms.*, mp.percent_value FROM " . $this->sanmu->table('merchants_server') . " AS ms " .
                "LEFT JOIN " . $this->sanmu->table('merchants_percent') . " AS mp ON mp.percent_id = ms.suppliers_percent" .
        ...
        if ($_REQUEST['act'] == 'merchant_download') {
            ...
            if (!empty($merchants_download_content)) {
                foreach ($merchants_download_content as $k => $merchants_order_list) {
                    ...
                    $content .= sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['label_commision_count']);
                    $content .= sanmu_iconv(EC_CHARSET, 'GB2312', $merchants_order_list['brokerage_amount']['all_price']) . "\t\n";
                    $content .= sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['label_settled']);
                    $content .= sanmu_iconv(EC_CHARSET, 'GB2312', $merchants_order_list['brokerage_amount']['is_settlement_price']) . "\t\n";
                    $content .= sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['label_not_settled']);
        ...
        elseif ($_REQUEST['act'] == 'bill_update') {
            ...
            if ($bill['bill_apply'] == 0) {
                ...
                $this->db->autoExecute($this->sanmu->table('seller_commission_bill'), $other, 'UPDATE', "id = '$bill_id'");
                ...
                if (isset($bill['negative_bill']['negative_id'])) {
                    ...
                    $this->db->autoExecute($this->sanmu->table('seller_negative_bill'), $negativeOther, 'UPDATE', "id " . db_create_in($bill['negative_bill']['negative_id']) . " AND commission_bill_id = 0");
                }
