...
class PresaleController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'insert_update') {
            ...
            if (isset($_POST['finish'])) {
                ...
                /* 结束预售活动，修改结束时间为当前时间 */
                $sql = "UPDATE " . $this->sanmu->table('presale_activity') .
                    " SET end_time = '" . gmtime() . "' " .
                ...
                $sql = "SELECT o.consignee, o.add_time, g.goods_number, o.order_sn, " .
                    ...
                    $this->sanmu->table('order_goods') . " AS g " .
                ...
                foreach ($res as $order) {
                    if (!empty($order['email'])) {
                        ...
                        $this->smarty->assign('shop_url', $this->sanmu->seller_url() . 'user_order.php?act=order_detail&order_id=' . $order['order_id']);
                ...
                $goods_name = $this->db->getOne("SELECT goods_name FROM " . $this->sanmu->table('goods') . " WHERE goods_id = '$goods_id'");

                $act_name = empty($_POST['act_name']) ? $goods_name : $this->sanmuRepository->subStr($_POST['act_name'], 0, 255, false);
                ...
                if ($presale_id > 0) {
                    ...
                    /* update */
                    $this->db->autoExecute($this->sanmu->table('presale_activity'), $presale, 'UPDATE', "act_id = '$presale_id'");
                    ...
                    /* insert */
                    $this->db->autoExecute($this->sanmu->table('presale_activity'), $presale, 'INSERT');
        ...
        elseif ($_REQUEST['act'] == 'batch_drop') {
            if (isset($_POST['checkboxes'])) {
                ...
                foreach ($_POST['checkboxes'] as $key => $id) {
                    ...
                    if ($presale['valid_order'] <= 0) {
                        /* 删除预售活动 */
                        $sql = "DELETE FROM " . $this->sanmu->table('presale_activity') .
        ...
        elseif ($_REQUEST['act'] == 'remove') {
            ...
            /* 删除预售活动 */
            $sql = "DELETE FROM " . $this->sanmu->table('presale_activity') . " WHERE act_id = '$id' LIMIT 1";
            $this->db->query($sql);
    ...
    private function presale_list($ru_id)
    {
        ...
        if ($result === false) {
            ...
            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('presale_activity') . " AS ga " .
                " WHERE 1 $where";
            ...
            /* 查询 */
            $sql = "SELECT ga.* " .
                "FROM " . $this->sanmu->table('presale_activity') . " AS ga " .
                " WHERE 1 $where " .
    ...
    {
        $sql = "SELECT * FROM " . $this->sanmu->table('presale_activity') .
    ...
    */
    ...
    {
        $sql = " SELECT shop_price FROM " . $this->sanmu->table('goods') . " WHERE goods_id = '$goods_id' ";
