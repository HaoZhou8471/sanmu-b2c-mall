...
class MerchantsUpgradeController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'application_grade' || $_REQUEST['act'] == 'edit') {
            ...
            if ($_REQUEST['act'] == 'edit') {
                ...
                foreach ($apply_criteria as $k => $v) {
                    if (stripos($v, 'images') === 0) {
                        $apply_criteria[$k] = $this->sanmuRepository->getImagePath($v);
                    }
                }
                ...
                /*判断是否存在未支付未失效申请*/
                $sql = "SELECT apply_id FROM " . $this->sanmu->table('seller_apply_info') . " WHERE ru_id = '" . $adminru['ru_id'] . "' AND apply_status = 0 AND is_paid = 0 LIMIT 1";
            ...
            $grade_info = $this->db->getRow("SELECT entry_criteria,grade_name FROM " . $this->sanmu->table('seller_grade') . " WHERE id = '$grade_id'");
        ...
        elseif ($_REQUEST['act'] == 'insert_submit' || $_REQUEST['act'] == 'update_submit') {
            ...
            if ($_REQUEST['act'] == 'insert_submit') {
                ...
                $sql = 'INSERT INTO' . $this->sanmu->table("seller_apply_info") . $key . " VALUES" . $value;
                $this->db->query($sql);
                ...
                if ($_REQUEST['act'] == 'update_submit') {
                    $sql = "SELECT pay_status FROM" . $this->sanmu->table("seller_apply_info") . "WHERE apply_id = '$apply_id' limit 1";
                    if ($this->db->getOne($sql) == 1) {
                        return show_message($GLOBALS['_LANG']['apply_complete_pay_cant_operate']);
                    }
                }

                $sql = "UPDATE" . $this->sanmu->table('seller_apply_info') . " SET payable_amount = '$payable_amount', back_price = '$back_price', total_amount = '$all_count_charge',pay_fee='$payment_info[pay_fee]',fee_num = '$fee_num',entry_criteria='$valus',pay_id='$pay_id' WHERE apply_id = '$apply_id' AND apply_sn = '$apply_sn'";
                $this->db->query($sql);
                ...
                if ($payment_info) {
                    if ($payment_info['pay_code'] == 'balance') {
                        ...
                        if ($user_money > $payable_amount) {
                            /*修改申请的支付状态 */
                            $sql = " UPDATE " . $this->sanmu->table('seller_apply_info') . " SET is_paid = 1 ,pay_time = '" . gmtime() . "' ,pay_status = 1 WHERE apply_id= '" . $apply_id . "'";
                            $this->db->query($sql);

                            //记录支付log
                            $sql = "UPDATE " . $this->sanmu->table('pay_log') . "SET is_paid = 1 WHERE order_id = '" . $apply_id . "' AND order_type = '" . PAY_APPLYGRADE . "'";
                            $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'checkorder') {
            ...
            $sql = "SELECT pay_status, pay_id FROM " . $this->sanmu->table('seller_apply_info') . " WHERE apply_id = '$apply_id' LIMIT 1";
        ...
        elseif ($_REQUEST['act'] == 'grade_load') {
            ...
            if ($apply_id > 0) {
                $sql = "SELECT apply_sn,pay_fee,pay_id,payable_amount FROM " . $this->sanmu->table('seller_apply_info')
        ...
        elseif ($_REQUEST['act'] == 'supplier_info') {
            ...
            if (empty($_FILES['front_of_id_card']['size'])) {
                ...
                $this->sanmuRepository->getOssAddFile([$supplier_info['front_of_id_card']]);
            }
            ...
            if (empty($_FILES['reverse_of_id_card']['size'])) {
                ...
                $this->sanmuRepository->getOssAddFile([$supplier_info['reverse_of_id_card']]);
            }
            ...
            if (empty($_FILES['suppliers_logo']['size'])) {
                ...
                $this->sanmuRepository->getOssAddFile([$supplier_info['suppliers_logo']]);
            }

            $count_suppliers = $this->db->getOne("SELECT COUNT(*) FROM " . $this->sanmu->table('suppliers') . " WHERE user_id = '$user_id' ");
            if ($count_suppliers && $action == 'update') {
                if ($this->db->autoExecute($this->sanmu->table('suppliers'), $supplier_info, 'UPDATE', "user_id='$user_id' ")) {
                ...
                if ($this->db->autoExecute($this->sanmu->table('suppliers'), $supplier_info, 'INSERT')) {
                    $link[] = ['text' => $GLOBALS['_LANG']['back'], 'href' => 'merchants_upgrade.php?act=suppliers_apply'];
    ...
    private function get_pzd_list()
    {
        ...
        if ($result === false) {
            ...
            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('seller_grade') . " WHERE is_open = 1";
            $filter['record_count'] = $this->db->getOne($sql);
            $filter = page_and_size($filter);
            /* 获活动数据 */
            $sql = "SELECT * FROM" . $this->sanmu->table('seller_grade') . " WHERE is_open = 1  ORDER BY id ASC LIMIT " . $filter['start'] . "," . $filter['page_size'];
        ...
        foreach ($row as $k => $v) {
            if ($v['entry_criteria']) {
                ...
                foreach ($entry_criteria as $key => $val) {
                    $sql = "SELECT criteria_name FROM" . $this->sanmu->table('entry_criteria') . " WHERE id = '" . $val . "'";
