...
class AdsController extends InitController
{
    ...
    public function index()
    {
        ...
        $exc = new Exchange($this->sanmu->table("ad"), $this->db, 'ad_id', 'ad_name');
        $menus = session('menus', '');
        ...
        if ($act == 'list') {
            ...
            $sql = 'SELECT position_id, position_name, ad_width, ad_height ' .
                'FROM ' . $this->sanmu->table('ad_position') . $where;// 改判断条件 商家广告位置由admin设置公共属性 liu
        ...
        elseif ($act == 'insert') {
            ...
            /* 查看广告名称是否有重复 */
            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('ad') . " AS a, " .
                $this->sanmu->table('ad_position') . " AS p " .
            ...
            $this->sanmuRepository->getOssAddFile([DATA_DIR . '/afficheimg/' . $ad_code]);

            $public_ruid = $adminru['ru_id'];    //ecmoban模板堂 --zhuo
            /* 插入数据 */
            $sql = "INSERT INTO " . $this->sanmu->table('ad') . " (position_id,media_type,ad_name,is_new,is_hot,is_best,public_ruid,ad_link,ad_code,start_time,end_time,link_man,link_email,link_phone,click_count,enabled, link_color, ad_type, goods_name)
    VALUES ('$position_id',
            '$type',
            '$ad_name',
            '$is_new',
        ...
        elseif ($act == 'edit') {
            ...
            /* 获取广告数据 */
            $sql = "SELECT * FROM " . $this->sanmu->table('ad') . " WHERE ad_id='" . $id . "'";
        ...
        elseif ($act == 'update') {
            ...
            /* 更新信息 */
            $sql = "UPDATE " . $this->sanmu->table('ad') . " SET " .
        ...
        elseif ($act == 'add_js') {
            ...
            $js_code .= ' src=' . '"' . $this->sanmu->seller_url() . 'affiche.php?act=js&type=' . $type . '&ad_id=' . $id . '"' . '></script>';

            $site_url = $this->sanmu->seller_url() . 'affiche.php?act=js&type=' . $type . '&ad_id=' . $id;
        ...
        elseif ($act == 'remove') {
            ...
            if ((strpos($img, 'http://') === false) && (strpos($img, 'https://') === false)) {
                ...
                $this->sanmuRepository->getOssDelFile([DATA_DIR . '/afficheimg/' . $img_name]);
    ...
    private function get_adslist($ru_id)
    {
        ...
        /* 获得总记录数据 */
        $sql = 'SELECT COUNT(*) FROM ' . $this->sanmu->table('ad') . ' AS ad ' . 'LEFT JOIN ' . $this->sanmu->table('ad_position') . ' AS p ON p.position_id = ad.position_id ' . $where;
        ...
        $sql = 'SELECT ad.*, COUNT(o.order_id) AS ad_stats, p.position_name, p.user_id ' .
            'FROM ' . $this->sanmu->table('ad') . 'AS ad ' .
            'LEFT JOIN ' . $this->sanmu->table('ad_position') . ' AS p ON p.position_id = ad.position_id ' .
            'LEFT JOIN ' . $this->sanmu->table('order_info') . " AS o ON o.from_ad = ad.ad_id $where " .
            'GROUP BY ad.ad_id ' .
    ...
    private function get_ad_model($pid)
    {
        ...
        //广告位信息
        $sql = " select * from " . $this->sanmu->table('ad_position') . " where position_id='" . $pid . "' limit 1";
        ...
        if (!empty($position_info['position_model'])) {
            ...
            if (in_array('cat_id', $ad_model_structure) && in_array('num_id', $ad_model_structure)) {
                ...
                //搜索已添加广告
                $sql = " select ad_name from " . $this->sanmu->table('ad') . " where ad_name like '%" . $ad_model . "%'";
    ...
    private function getCatList($catId = 0)
    {
        ...
        $sql = " select cat_id,cat_name from " . $this->sanmu->table('category') . $where;
