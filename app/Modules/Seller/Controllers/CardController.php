...
class CardController extends InitController
{
    ...
    public function index()
    {
        ...
        $exc = new Exchange($this->sanmu->table("card"), $this->db, 'card_id', 'card_name');
        ...
        elseif ($act == 'remove') {
            ...
            $this->sanmuRepository->getDelBatch('', $card_id, ['card_img'], 'card_id', Card::whereRaw(1), 0, DATA_DIR . '/cardimg/'); //删除图片
        ...
        elseif ($act == 'add') {
            ...
            $this->sanmuRepository->getOssAddFile([DATA_DIR . '/cardimg/' . $img_name]);

            /*插入数据*/
            $sql = "INSERT INTO " . $this->sanmu->table('card') . "(card_name, card_fee, free_money, card_desc, card_img, user_id)
        ...
        elseif ($act == 'edit') {
            ...
            $sql = "SELECT card_id, card_name, card_fee, free_money, card_desc, card_img FROM " . $this->sanmu->table('card') . " WHERE card_id='$id'";
        ...
        elseif ($act == 'drop_card_img') {
            ...
            $this->sanmuRepository->getDelBatch('', $card_id, ['card_img'], 'card_id', Card::whereRaw(1), 0, DATA_DIR . '/cardimg/'); //删除图片
            $sql = "UPDATE " . $this->sanmu->table('card') . " SET card_img = '' WHERE card_id = '$card_id'";
            $this->db->query($sql);
    ...
    private function cards_list($ru_id)
    {
        ...
        if ($result === false) {
            ...
            /* 分页大小 */
            $sql = "SELECT count(*) FROM " . $this->sanmu->table('card') . $where;
            ...
            $sql = "SELECT card_id, card_name, card_img, card_fee, free_money, card_desc, user_id" .
                " FROM " . $this->sanmu->table('card') . $where .
