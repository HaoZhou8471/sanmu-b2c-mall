...
class PrivilegeKefuController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'remove') {
            ...
            $sql = "SELECT user_name, chat_status FROM  " . $this->sanmu->table('im_service') . " WHERE id = " . $id . " AND status = 1";
            ...
            //删除操作
            $sql = "UPDATE" . $this->sanmu->table('im_service') . " SET status = 0  WHERE id = " . $id;
        ...
        elseif ($_REQUEST['act'] == 'trash') {
            ...
            $ids = implode(',', $ids);
            $sql = "UPDATE" . $this->sanmu->table('im_service') . " SET status = 0  WHERE id in (" . $ids . ")";
        ...
        elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
            ...
            $sql = "SELECT user_name FROM  " . $this->sanmu->table('admin_user') . " WHERE user_id=" . $services;
            ...
            if ($_REQUEST['act'] == 'insert') {
                $sql = "SELECT user_name, status FROM  " . $this->sanmu->table('im_service') . " WHERE user_id=" . $services;

                $res = $this->db->getRow($sql);
                if ($res['status'] == 1) {
                    return sys_msg($GLOBALS['_LANG']['this_admin_was_kefu'], 1);
                } elseif ($res['status'] === 0 || $res['status'] === '0') {
                    $sql = "UPDATE" . $this->sanmu->table('im_service') . " SET nick_name='{$services_name}', post_desc='{$services_desc}', status=1  WHERE user_id=" . $services;
                    $res = $this->db->query($sql);
                } else {
                    $sql = "INSERT INTO " . $this->sanmu->table('im_service') . "(user_id, user_name, nick_name, post_desc, chat_status, status) " .
                ...
                $sql = "UPDATE" . $this->sanmu->table('im_service') . " SET nick_name='{$services_name}', post_desc='{$services_desc}', status=1  WHERE id=" . $id;
    ...
    private function services_list()
    {
        ...
        /** 获得总记录数据 */
        $sql = 'SELECT COUNT(s.id) FROM ' . $this->sanmu->table('im_service') . ' s'
            . " INNER JOIN " . $this->sanmu->table('admin_user') . " a ON a.user_id = s.user_id  AND a.ru_id = " . $adminru['ru_id']
            . ' WHERE s.status = 1';
        $filter['record_count'] = $this->db->getOne($sql);

        $filter = page_and_size($filter);
        $list = [];
        $sql = 'SELECT s.* FROM ' . $this->sanmu->table('im_service') . ' s'
            . " INNER JOIN " . $this->sanmu->table('admin_user') . " a ON a.user_id = s.user_id  AND a.ru_id = " . $adminru['ru_id']
            . ' WHERE s.status = 1';
    ...
    {
        $sql = "SELECT ru_id FROM " . $this->sanmu->table("admin_user") . " WHERE user_id = '" . session('seller_id') . "'";

        $ruId = $this->db->getOne($sql);

        $sql = "SELECT a.user_id, a.user_name FROM " . $this->sanmu->table("admin_user") . " a"
            . " LEFT JOIN " . $this->sanmu->table("im_service") . " s ON a.user_id = s.user_id AND s.status = 1"
            . " WHERE a.ru_id = " . $ruId;
    ...
    {
        $sql = "SELECT id,  customer_id, goods_id, store_id, start_time, end_time FROM " . $this->sanmu->table("im_dialog") . " WHERE services_id = {$id}";
        ...
        foreach ($res as $k => $v) {
            ...
            if ($v['goods_id'] > 0) {
                $sql = "SELECT goods_name, goods_thumb FROM " . $this->sanmu->table("goods") . " WHERE goods_id = " . $v['goods_id'];
                $goods = $this->db->getRow($sql);
                $res[$k]['goods_name'] = $goods['goods_name'];
                $res[$k]['goods_thumb'] = $this->format_goods_pic($goods['goods_thumb']);
            }

            $sql = "SELECT user_name FROM " . $this->sanmu->table("users") . " WHERE user_id = " . $v['customer_id'];
    ...
    private function message_list($customer_id, $service_id, $page = 0, $keyword = '', $date = '')
    {
        ...
        $sql = "SELECT message, user_type, FROM_UNIXTIME(add_time) AS add_time FROM " . $this->sanmu->table("im_message") . " WHERE ((from_user_id = " . $customer_id . " AND to_user_id = " . $service_id . ") OR (from_user_id = " . $service_id . " AND to_user_id = " . $customer_id . ")";
        $sqlCount = "SELECT count(id) FROM " . $this->sanmu->table("im_message") . " WHERE ((from_user_id = " . $customer_id . " AND to_user_id = " . $service_id . ") OR (from_user_id = " . $service_id . " AND to_user_id = " . $customer_id . ")";
    ...
    {
        $sql = "SELECT count(id) FROM " . $this->sanmu->table("im_dialog");
        $adminru = get_admin_ru_id();
    ...
    {
        $sql = "SELECT COUNT(DISTINCT customer_id) FROM " . $this->sanmu->table("im_dialog");
        $adminru = get_admin_ru_id();
    ...
    {
        $sql = 'SELECT id, nick_name, post_desc FROM ' . $this->sanmu->table("im_service") . '  WHERE status = 1 AND id = ' . $id;
        $res = $this->db->getRow($sql);
        return $res;
    }
    ...
    {
        $sql = "SELECT u.user_name, s.nick_name, FROM_UNIXTIME(start_time) AS start_time FROM " . $this->sanmu->table("im_dialog") . " d"
            . " LEFT JOIN " . $this->sanmu->table("im_service") . " s ON d.services_id = s.id"
            . " LEFT JOIN " . $this->sanmu->table("users") . " u ON d.customer_id = u.user_id"
            . " WHERE d.id = " . $id;
