...
class WechatController extends BaseController
{
                ...
                if ($msgtype == 'image' || $msgtype == 'voice') {
                    ...
                    if (!empty($mediaInfo['article_id'])) {
                        ...
                        foreach ($artids as $key => $val) {
                            ...
                            $artinfo['content'] = isset($artinfo['content']) ? $this->sanmuRepository->subStr(strip_tags(html_out($artinfo['content'])), 100) : '';
                            ...
                            $articles[$key]['url'] = empty($artinfo['link']) ? sanmu_url('/#/wechatMedia/' . $artinfo['id']) : strip_tags(html_out($artinfo['link']));
                        }
                    } else {
                        $articles[0]['title'] = $mediaInfo['title'];
                        $articles[0]['description'] = empty($mediaInfo['digest']) ? $this->sanmuRepository->subStr(strip_tags(html_out($mediaInfo['content'])), 100) : $mediaInfo['digest'];
                        $articles[0]['picurl'] = empty($mediaInfo['file']) ? '' : $this->wechatHelperService->get_wechat_image_path($mediaInfo['file']);
                        $articles[0]['url'] = empty($mediaInfo['link']) ? sanmu_url('/#/wechatMedia/' . $mediaInfo['id']) : strip_tags(html_out($mediaInfo['link']));
                    }
    ...
    public function article()
    {
        ...
        if (!empty($list)) {
            foreach ($list as $key => $val) {
                ...
                $list[$key]['content'] = !empty($val['digest']) ? $val['digest'] : (!empty($val['content']) ? $this->sanmuRepository->subStr(strip_tags(html_out($val['content'])), 50) : '');
            }
        }
    ...
    public function article_edit()
    {
        ...
        if (request()->isMethod('POST')) {
            ...
            $pic_path = $this->sanmuRepository->editUploadImage($pic_path);
            ...
            $file_arr = $this->sanmuRepository->transformOssFile($file_arr);
    ...
    public function mass_message()
    {
        ...
        if (request()->isMethod('POST')) {
            ...
            if (!empty($article_info['article_id'])) {
                ...
                foreach ($articles as $key => $val) {
                    ...
                    $article[$key]['content_source_url'] = empty($artinfo['link']) ? sanmu_url('/#/wechatMedia/' . $artinfo['id']) : strip_tags(html_out($artinfo['link']));
                ...
                $article[0]['content_source_url'] = empty($article_info['link']) ? sanmu_url('/#/wechatMedia/' . $article_info['id']) : strip_tags(html_out($article_info['link']));
        ...
        if ($article) {
            foreach ($article as $key => $val) {
                ...
                $article[$key]['content'] = empty($val['digest']) ? (empty($val['content']) ? '' : $this->sanmuRepository->subStr(strip_tags(html_out($val['content'])), 50)) : $val['digest'];
            }
        }
            ...
            if ('image' == $type) {
                ...
                if (!empty($result['list'])) {
                    foreach ($result['list'] as $key => $val) {
                        ...
                        $result['list'][$key]['content'] = empty($val['digest']) ? (empty($val['content']) ? '' : $this->sanmuRepository->subStr(strip_tags(html_out($val['content'])), 50)) : $val['digest'];
                        // 多图文的子图文不重复处理
                        if (empty($val['article_id'])) {
