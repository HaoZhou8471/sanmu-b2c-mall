...
class OrderStatsController extends InitController
{
    ...
    public function index()
    {
        ...
        $this->sanmuRepository->helpersLang(['statistic', 'common'], 'seller');
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            $sql = 'SELECT SUM(click_count) FROM ' . $this->sanmu->table('goods') . ' WHERE is_delete = 0' . $where;
        ...
        elseif ($_REQUEST['act'] == 'list_old') {
            ...
            /* 取得商品总点击数量 */
            $sql = 'SELECT SUM(click_count) FROM ' . $this->sanmu->table('goods') . ' WHERE is_delete = 0 AND user_id = ' . $adminru['ru_id'];
            $click_count = floatval($this->db->getOne($sql));
    ...
    private function get_orderinfo($start_date, $end_date, $type = 0)
    {
        ...
        if ($type == 1) {
            /* 未确认订单数 */
            $sql = 'SELECT IFNULL(SUM(' . $this->orderService->orderAmountField('o.') . '), 0) FROM ' . $GLOBALS['sanmu']->table('order_info') . " as o " .
    ...
    private function get_order_general($type = 0)
    {
        ...
        $sql = "SELECT count(*) as total_order_num, " . $total_fee . " FROM " . $this->sanmu->table('order_info') . ' as o' .
            " WHERE 1 " . $this->orderService->orderQuerySql('real_pay', 'o.') .
    ...
    private function get_pay_type($start_date, $end_date, $ru_id)
    {
        $sql = 'SELECT i.pay_id, p.pay_name, i.pay_time ' .
            'FROM ' . $this->sanmu->table('payment') . ' AS p, ' . $this->sanmu->table('order_info') . ' AS i ' .
    ...
    private function get_shipping_type($start_date, $end_date, $ru_id)
    {
        $sql = 'SELECT sp.shipping_id, sp.shipping_name AS ship_name, i.shipping_time ' .
            'FROM ' . $this->sanmu->table('shipping') . ' AS sp, ' . $this->sanmu->table('order_info') . ' AS i ' .
    ...
    private function get_area($countries, $pro = 0)
    {
        ...
        if ($countries == 1 && $pro == 0) {
            $sql = "SELECT COUNT(*) AS area_num ,r.region_name AS region_name FROM " . $this->sanmu->table('order_info') . " AS i " .
                " LEFT JOIN " . $this->sanmu->table('region') . " AS r ON r.region_id = i.province" .
                " WHERE i.country = '$countries' " . $where . " GROUP BY i.province";
        } elseif ($countries == 1 && $pro > 0) {
            $sql = "SELECT COUNT(*) AS area_num ,r.region_name AS region_name FROM " . $this->sanmu->table('order_info') . " AS i " .
                " LEFT JOIN " . $this->sanmu->table('region') . " AS r ON r.region_id = i.city" .
