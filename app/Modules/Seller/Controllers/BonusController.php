...
class BonusController extends InitController
{
    ...
    public function index()
    {
        ...
        /* 初始化$exc对象 */
        $exc = new Exchange($this->sanmu->table('bonus_type'), $this->db, 'type_id', 'type_name');
        ...
        $this->smarty->assign('url', $this->sanmu->seller_url());
        ...
        if ($act == 'remove') {
            ...
            $bonus_arr = $this->db->getRow("SELECT * FROM " . $this->sanmu->table('bonus_type') . " WHERE type_id = '$id'");
            ...
            /* 更新商品信息 */
            $this->db->query("UPDATE " . $this->sanmu->table('goods') . " SET bonus_type_id = 0 WHERE bonus_type_id = '$id'");

            /* 删除用户的红包 */
            $this->db->query("DELETE FROM " . $this->sanmu->table('user_bonus') . " WHERE bonus_type_id = '$id'");
        ...
        if ($act == 'insert') {
            ...
            /* 检查类型是否有重复 */
            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('bonus_type') . " WHERE type_name='$type_name' AND user_id = '$adminru[ru_id]'";
            ...
            */
            ...
            $sql = "INSERT INTO " . $this->sanmu->table('bonus_type') . " (type_name, valid_period,date_type,type_money,send_start_date,send_end_date,use_start_date,use_end_date,send_type, usebonus_type, user_id, min_amount,  min_goods_amount)
    VALUES ('$type_name',
            '$valid_period',
            '$data_type',
            '$type_money',
        ...
        if ($act == 'edit') {
            ...
            $bonus_arr = $this->db->getRow("SELECT * FROM " . $this->sanmu->table('bonus_type') . " WHERE type_id = '$type_id'");
...
//    }
        ...
        if ($act == 'update') {
            ...
            $sql = "UPDATE " . $this->sanmu->table('bonus_type') . " SET " .
                "type_name       = '$type_name', " .
                "date_type       = '$data_type', " .
        ...
        if ($act == 'send') {
            ...
            if ($send_by == SEND_BY_USER) {
                ...
                /* 查询此红包类型信息 */
                $bonus_type = $this->db->GetRow("SELECT type_id, type_name FROM " . $this->sanmu->table('bonus_type') .
                    " WHERE type_id='$id'");
                ...
                $bonus = [];
                $sql = 'SELECT type_id, type_name, type_money FROM ' . $this->sanmu->table('bonus_type') .
                    ' WHERE send_type = 4';
...
//                    }
        ...
        if ($act == 'send_by_print') {
            ...
            for ($i = 0, $j = 0; $i < $bonus_sum; $i++) {
                ...
                $this->db->query("INSERT INTO " . $this->sanmu->table('user_bonus') . " (bonus_type_id, bonus_sn, bonus_password) VALUES('$bonus_typeid', '$bonus_sn', '$bonus_password')");
        ...
        if ($act == 'gen_excel') {
            ...
            if (EC_CHARSET != 'gbk') {
                ...
                echo sanmu_iconv('UTF8', 'GB2312', $GLOBALS['_LANG']['bind_password']) . "\t";
                echo sanmu_iconv('UTF8', 'GB2312', $GLOBALS['_LANG']['type_money']) . "\t";
                echo sanmu_iconv('UTF8', 'GB2312', $GLOBALS['_LANG']['type_name']) . "\t";
                echo sanmu_iconv('UTF8', 'GB2312', $GLOBALS['_LANG']['use_enddate']) . "\t\n";
            } else {
        ...
        if ($act == 'get_goods_list') {
            $filters = sanmu_decode(request()->input('JSON', ''));
        ...
        if ($act == 'add_bonus_goods') {
            ...
            foreach ($add_ids as $key => $val) {
                $sql = "UPDATE " . $this->sanmu->table('goods') . " SET bonus_type_id='$type_id' WHERE goods_id='$val'";
        ...
        if ($act == 'drop_bonus_goods') {
            ...
            $this->db->query("UPDATE " . $this->sanmu->table('goods') . " SET bonus_type_id = 0 " .
        ...
        if ($act == 'search_users') {
            ...
            $sql = "SELECT user_id, user_name, mobile_phone, email FROM " . $this->sanmu->table('users') .
            ...
            if ($row) {
                foreach ($row as $key => $val) {
                    if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                        $row[$key]['mobile_phone'] = $this->sanmuRepository->stringToStar($val['mobile_phone']);
                        $row[$key]['user_name'] = $this->sanmuRepository->stringToStar($val['user_name']);
                        $row[$key]['email'] = $this->sanmuRepository->stringToStar($val['email']);
                    }
                }
            }
        ...
        elseif ($act == 'remove_bonus') {
            ...
            $this->db->query("DELETE FROM " . $this->sanmu->table('user_bonus') . " WHERE bonus_id='$id'");
        ...
        if ($act == 'batch') {
            ...
            if (request()->exists('checkboxes')) {
                ...
                if (request()->exists('drop')) {
                    $sql = "DELETE FROM " . $this->sanmu->table('user_bonus') . " WHERE bonus_id " . db_create_in($bonus_id_list);
                    $this->db->query($sql);
    ...
    private function get_type_list($ru_id)
    {
        ...
        $sql = "SELECT bonus_type_id, COUNT(*) AS sent_count" .
            " FROM " . $this->sanmu->table('user_bonus') .
        ...
        if ($result === false) {
            ...
            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('bonus_type') . " AS bt " . $where;
            $filter['record_count'] = $this->db->getOne($sql);

            /* 分页大小 */
            $filter = page_and_size($filter);

            $sql = "SELECT bt.* FROM " . $this->sanmu->table('bonus_type') . " AS bt " . $where . " ORDER BY $filter[sort_by] $filter[sort_order]";
    ...
    private function get_bonus_list()
    {
        ...
        $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('user_bonus') . $where;
        $filter['record_count'] = $this->db->getOne($sql);
        ...
        $sql = "SELECT ub.*, u.user_name, u.email, u.mobile_phone, o.order_sn, bt.type_name " .
            " FROM " . $this->sanmu->table('user_bonus') . " AS ub " .
            " LEFT JOIN " . $this->sanmu->table('bonus_type') . " AS bt ON bt.type_id=ub.bonus_type_id " .
            " LEFT JOIN " . $this->sanmu->table('users') . " AS u ON u.user_id=ub.user_id " .
            " LEFT JOIN " . $this->sanmu->table('order_info') . " AS o ON o.order_id=ub.order_id $where " .
    ...
    {
        $sql = "SELECT * FROM " . $this->sanmu->table('bonus_type') .
