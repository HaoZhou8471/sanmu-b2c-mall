...
class GoodsLibController extends InitController
{
    ...
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            //清楚商品零时货品表数据
            $sql = "DELETE FROM" . $GLOBALS['sanmu']->table('products_changelog') . "WHERE admin_id = '" . $admin_id . "'";
            $GLOBALS['db']->query($sql);

            //清楚商品属性
            $sql = "DELETE FROM" . $GLOBALS['sanmu']->table('goods_attr') . "WHERE admin_id = '" . $admin_id . "' AND goods_id = 0";
            $GLOBALS['db']->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'lib_list') {
            ...
            //商家ID，商家等级赋值
            $sql = " SELECT grade_id FROM " . $this->sanmu->table('merchants_grade') . " WHERE ru_id = '$ru_id' ";
        ...
        elseif ($_REQUEST['act'] == 'seller_import') {
            ...
            if ($standard == 1) {
                $table = 'wholesale';
                $sql = " SELECT is_delivery,is_return,is_free  FROM " . $this->sanmu->table('wholesale_extend') . " WHERE goods_id  = '$goods_id' ";
                $goods = $this->db->getRow($sql);

                $select = ',goods_cause,retail_price as shop_price';
            }
            $sql = " SELECT goods_name,cat_id,brand_id $select FROM " . $this->sanmu->table($table) . " WHERE goods_id  = '$goods_id' ";
        ...
        elseif ($_REQUEST['act'] == 'goods_import_action') {
            ...
            if ($standard == 1) {
                $sql = " SELECT * FROM " . $this->sanmu->table('wholesale') . " WHERE goods_id = '$lib_goods_id' ";
                $goods = $this->db->getRow($sql);
                if (empty($goods_name)) {
                ...
                $sql = "INSERT INTO " . $this->sanmu->table('goods') .
                    ...
                    "pinyin_keyword,'$goods_cause',warn_number,goods_product_tag,goods_unit,'$freight','$shipping_fee','$tid','$user_cat', '$review_status' FROM" . $this->sanmu->table('wholesale') . "WHERE goods_id = '$lib_goods_id'";
                $this->db->query($sql);
                $goods_id = $this->db->insert_id();

                //同步扩展信息
                $sql = "INSERT INTO" . $this->sanmu->table('goods_extend') . "(goods_id,width,height,depth,origincountry,originplace,assemblycountry,barcodetype,"
                    ...
                    . "catena,isbasicunit,packagetype,grossweight,netweight,netcontent,licensenum,healthpermitnum FROM" . $this->sanmu->table('wholesale_extend') . "WHERE goods_id = '$lib_goods_id'";
                $this->db->query($sql);
                //获取标准产品库商品相册
                $res = $this->db->getAll(" SELECT img_desc, img_url, thumb_url, img_original FROM " . $this->sanmu->table('suppliers_goods_gallery') . " WHERE goods_id = '$lib_goods_id' ");
            } else {
                $sql = " SELECT * FROM " . $this->sanmu->table('goods_lib') . " WHERE goods_id = '$lib_goods_id' ";
                $goods = $this->db->getRow($sql);
                if (empty($goods_name)) {
                    $goods_name = $goods['goods_name'];
                }
                $goods['goods_from'] = $goods['goods_from'] ?? 0;
                if (!($this->db->getOne(" SELECT goods_id FROM " . $this->sanmu->table('goods') . " WHERE goods_id ='" . $goods['lib_goods_id'] . "' AND user_id = '$ru_id' "))) {
                    $goods_thumb = $this->copy_img($goods['goods_thumb'], 'goods_thumb', $lib_goods_id);
                    $goods_img = $this->copy_img($goods['goods_img'], 'goods', $lib_goods_id);
                    $original_img = $this->copy_img($goods['original_img'], 'goods', $lib_goods_id);
                    ...
                    $goods_id = $this->db->insert_id();
                    //获取本地产品库商品相册
                    $res = $this->db->getAll(" SELECT img_desc, img_url, thumb_url, img_original FROM " . $this->sanmu->table('goods_lib_gallery') . " WHERE goods_id = '$lib_goods_id' ");
                } else {
            ...
            //商品属性处理
            $sql = "UPDATE" . $this->sanmu->table('goods_attr') . "SET goods_id = '$goods_id' WHERE goods_id = 0 AND admin_id = '$admin_id'";
            $this->db->query($sql);
            ...
            $sql = "SELECT goods_attr,product_sn,bar_code,product_number,product_price,product_market_price,product_promote_price,product_warn_number,warehouse_id,area_id,admin_id FROM " .
                $this->sanmu->table('products_changelog') . " WHERE 1 AND admin_id = '" . session('seller_id') . "' AND goods_id = 0";
            ...
            //清楚商品零时货品表数据
            $sql = "DELETE FROM" . $this->sanmu->table('products_changelog') . " WHERE goods_id = 0 AND admin_id = '$admin_id'";
            $this->db->query($sql);
            ...
            if ($res) {
                foreach ($res as $k => $v) {
                    ...
                    $sql = " INSERT INTO " . $this->sanmu->table('goods_gallery') .
        ...
        if ($_REQUEST['act'] == 'batch') {
            ...
            if ($_POST['checkboxes']) {
                $sql = " SELECT goods_id, goods_name FROM " . $this->sanmu->table($table) . " WHERE goods_id " . db_create_in($_POST['checkboxes']);
                ...
                foreach ($_POST['goods_id'] as $goods_id) {
                    ...
                    if ($standard_goods == 1) {
                        ...
                        $sql = "INSERT INTO " . $this->sanmu->table('goods') .
                            ...
                            "pinyin_keyword,warn_number,goods_product_tag,goods_unit, '$review_status' FROM" . $this->sanmu->table('wholesale') . "WHERE goods_id = '$lib_goods_id'";
        ...
        elseif ($_REQUEST['act'] == 'edit_product_sn') {
            ...
            /* 修改货品库存 */
            $sql = "UPDATE " . $this->sanmu->table($table) . " SET product_sn = '$product_sn' WHERE product_id = '$product_id'";
            $result = $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'edit_product_bar_code') {
            ...
            if (!empty($bar_code)) {
                ...
                /* 修改 */
                $sql = "UPDATE " . $this->sanmu->table($table) . " SET bar_code = '$bar_code' WHERE product_id = '$product_id'";
                $result = $this->db->query($sql);
                if ($result) {
                    clear_cache_files();
    ...
    */
    private function copy_img($image = '', $type = 'goods', $goods_id)
    {
        ...
        $this->sanmuRepository->getOssAddFile([$new_name]);
        return $new_name;
    }
}
