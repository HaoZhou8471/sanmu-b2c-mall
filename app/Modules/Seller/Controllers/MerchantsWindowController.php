...
class MerchantsWindowController extends InitController
{
    public function index()
    {
        ...
        $exc = new Exchange($this->sanmu->table("seller_shopwindow"), $this->db, 'id', 'win_name');
        //获取店铺模板
        $adminru = get_admin_ru_id();
        $sql = "select * from " . $this->sanmu->table('seller_shopinfo') . " where ru_id='" . $adminru['ru_id'] . "'";
        ...
        $sql = "select count(*) from " . $this->sanmu->table('seller_shopinfo') . " where ru_id = '" . $adminru['ru_id'] . "'";
        ...
        elseif ($_REQUEST['act'] == 'add') {
            ...
            $sql = "SELECT seller_theme FROM " . $this->sanmu->table('seller_shopinfo') . " WHERE ru_id='" . $adminru['ru_id'] . "'";
            ...
            //检查名称是否重复
            $sql = 'SELECT COUNT(*) FROM ' . $this->sanmu->table('seller_shopwindow') . " WHERE win_name = '$_POST[winname]' and ru_id='" . $adminru['ru_id'] . "'";
            ...
            /*插入数据*/
            if ($win_name) {
                $sql = "INSERT INTO " . $this->sanmu->table('seller_shopwindow') . "(win_type,win_goods_type,win_order,win_name, is_show, win_color, win_img,win_img_link,ru_id,win_custom,seller_theme) " .
        ...
        elseif ($_REQUEST['act'] == 'edit') {
            ...
            $sql = "SELECT * FROM " . $this->sanmu->table('seller_shopwindow') . " WHERE id='$_REQUEST[id]' and ru_id='" . $adminru['ru_id'] . "'";
            $seller_win = $this->db->GetRow($sql);
        ...
        elseif ($_REQUEST['act'] == 'edit_win_name') {
            ...
            /* 检查名称是否重复 */
            $sql = 'SELECT COUNT(*) FROM ' . $this->sanmu->table('seller_shopwindow') . " WHERE win_name = '$name' and ru_id='" . session('ru_id') . "'";
        ...
        elseif ($_REQUEST['act'] == 'remove') {
            ...
            /* 删除该橱窗的图标 */
            $sql = "SELECT win_img FROM " . $this->sanmu->table('seller_shopwindow') . " WHERE id = '$id' and ru_id='" . $adminru['ru_id'] . "'";
        ...
        elseif ($_REQUEST['act'] == 'insert_win_goods') {
            ...
            $linked_window = sanmu_decode($_GET['JSON'], true);
            $id = $linked_window[0];
        ...
        elseif ($_REQUEST['act'] == 'drop_win_goods') {
            ...
            if ($checkboxes) {
                if ($type == 1) {
                    ...
                    $sql = "DELETE FROM " . $this->sanmu->table('seller_shopwindow') . " WHERE win_type = 1 AND id in($id)";
                    $this->db->query($sql);
                }
            }
    ...
    {
        $adminru = get_admin_ru_id();
        $sql = "SELECT * FROM " . $this->sanmu->table('seller_shopwindow') . ' WHERE ru_id = \'' . $adminru['ru_id'] . '\' AND seller_theme = \'' . $seller_theme . '\' AND	win_type = 1';
    ...
    {
        $adminru = get_admin_ru_id();
        $sql = "select * from " . $this->sanmu->table('seller_shopwindow') . " where id='$id' and ru_id='" . $adminru['ru_id'] . "'";
