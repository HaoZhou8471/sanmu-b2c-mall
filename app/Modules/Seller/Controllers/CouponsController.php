...
class CouponsController extends InitController
{
    ...
    public function index()
    {
        ...
        $cou = new Exchange($this->sanmu->table('coupons'), $this->db, 'cou_id', 'cou_name');
        ...
        if ($_REQUEST['act'] == 'remove') {
            ...
            $cou_arr = $this->db->getRow("SELECT ru_id FROM " . $this->sanmu->table('coupons') . " WHERE cou_id = '$id' LIMIT 1");
        ...
        if ($_REQUEST['act'] == 'insert') {
            ...
            /* 插入数据库。 */
            $sql = "INSERT INTO " . $this->sanmu->table('coupons') . " (
    ...
    VALUES (
            ...
            if ($cou_type == 5) {
                ...
                $sql = "INSERT INTO" . $this->sanmu->table('coupons_region') . "(`cou_id`,`region_list`) VALUES ('$cou_id','$region_list')";
                $this->db->query($sql);
            }
            /* 清除缓存 */
            clear_cache_files();
        ...
        if ($_REQUEST['act'] == 'edit') {
            ...
            $cou_arr = $this->db->getRow("SELECT * FROM " . $this->sanmu->table('coupons') . " WHERE cou_id = '$cou_id'");
        ...
        if ($_REQUEST['act'] == 'update') {
            ...
            $this->db->autoExecute($this->sanmu->table('coupons'), $record, 'UPDATE', "cou_id = '$cou_id'");
            //录入包邮券，不包邮地区
            if ($cou_type == VOUCHER_SHIPPING) {
                $region_list = !empty($_REQUEST['free_value']) ? trim($_REQUEST['free_value']) : '';
                $sql = "SELECT COUNT(*) FROM" . $this->sanmu->table('coupons_region') . "WHERE cou_id = '$cou_id'";
                $count_free = $this->db->getOne($sql);
                if ($count_free > 0) {
                    $sql = "UPDATE" . $this->sanmu->table('coupons_region') . "SET region_list = '$region_list' WHERE cou_id = '$cou_id'";
                } else {
                    $sql = "INSERT INTO" . $this->sanmu->table('coupons_region') . "(`cou_id`,`region_list`) VALUES ('$cou_id','$region_list')";
                }
                $this->db->query($sql);
        ...
        if ($_REQUEST['act'] == 'change_order') {
            ...
            $sql = "UPDATE " . $this->sanmu->table('coupons') . "SET cou_order='" . $cou_order . "' WHERE cou_id='" . $cou_id . "'";
...
//                    }
        ...
        elseif ($_REQUEST['act'] == 'remove_coupons') {
            $cou_id = intval($_GET['cou_id']);
            if ($cou_id) {
                $res = $this->db->query("DELETE FROM " . $this->sanmu->table('coupons') . " WHERE cou_id='$cou_id'");
                if ($res) {
                    return 'ok';
                }
            }
            ...
            if ($uc_id) {
                $cou_id = $this->db->getOne("SELECT cou_id FROM " . $this->sanmu->table('coupons_user') . "WHERE uc_id='" . $uc_id . "'");
                $res = $this->db->query("DELETE FROM " . $this->sanmu->table('coupons_user') . " WHERE uc_id='$uc_id'");
    ...
    {
        /* 获得所有优惠券的发放数量 */
        $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('coupons') . "";
        ...
        $sql = "SELECT * FROM " . $this->sanmu->table('coupons') . " $filter[sort_order]";
    ...
    {
        $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('coupons_user') . " WHERE cou_id ='" . $cou_id . "' ";
        ...
        $sql = "SELECT * FROM" . $this->sanmu->table('coupons_user') . "WHERE cou_id='" . $cou_id . "' ORDER BY uc_id ASC";
        $row = $this->db->getAll($sql);
        ...
        foreach ($row as $key => $val) {
            ...
            //订单号
            if ($val['order_id']) {
                $row[$key]['order_sn'] = $this->db->getOne("SELECT order_sn FROM" . $this->sanmu->table('order_info') . " WHERE order_id=" . $val['order_id']);
            }

            //所属会员
            if ($val['user_id']) {
                $row[$key]['user_name'] = $this->db->getOne("SELECT user_name FROM" . $this->sanmu->table('users') . " WHERE user_id=" . $val['user_id']);
            }
        }
    ...
    private function get_coupons_info2($cou_id)
    {
        ...
        $sql = "SELECT * FROM" . $this->sanmu->table('coupons_user') . "WHERE cou_id='" . $cou_id . "' ORDER BY uc_id ASC LIMIT {$filter['start']} , {$filter['page_size']}";
        $row = $this->db->getAll($sql);
