...
class SaleGeneralController extends InitController
{
    ...
    public function index()
    {
        ...
        $this->sanmuRepository->helpersLang('statistic', 'seller');
        ...
        elseif ($_REQUEST['act'] == 'download') {
            ...
            /* 文件标题 */
            echo sanmu_iconv(EC_CHARSET, 'GB2312', $filename . $GLOBALS['_LANG']['sales_statistics']) . "\t\n";

            /* 订单数量, 销售出商品数量, 销售金额 */
            echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['goods_steps_name']) . "\t";
            echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['goods_name']) . "\t";
            echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['pro_code']) . "\t";
            echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['category']) . "\t";
            echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['amount']) . "\t";
            echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['unit_price']) . "\t";
            echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['total_amount']) . "\t";
            echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['sale_date']) . "\t\n";

            foreach ($data_list as $data) {
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $data['shop_name']) . "\t";
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $data['goods_name']) . "\t";
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $data['goods_sn']) . "\t";
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $data['cat_name']) . "\t";
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $data['goods_number']) . "\t";
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $data['goods_price']) . "\t";
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $data['total_fee']) . "\t";
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $data['add_time']) . "\t";
                echo "\n";
            }
        }
    }
    ...
    private function get_data_list($type = 0)
    {
        ...
        if ($type != 0) {
            ...
            if ($result === false) {
                ...
                if (!empty($filter['cat_name'])) {
                    $sql = "SELECT cat_id FROM " . $this->sanmu->table('category') . " WHERE cat_name = '" . $filter['cat_name'] . "'";
                ...
                if (!empty($filter['cat_name'])) {
                    $where_order .= " AND (SELECT g.cat_id FROM " . $this->sanmu->table('goods') . " AS g WHERE g.goods_id = og.goods_id LIMIT 1) = '$cat_id'";
                }
                ...
                $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('goods') . " AS g WHERE " . $goods_where .
                    " AND (SELECT og.goods_id FROM " . $this->sanmu->table('order_goods') . " AS og " .
                    " LEFT JOIN " . $this->sanmu->table('order_info') . " AS o ON og.order_id = o.order_id" .
        ...
        if ($type != 0) {
            for ($i = 0; $i < count($data_list); $i++) {
                ...
                $data_list[$i]['cat_name'] = $this->db->getOne("SELECT c.cat_name FROM " . $this->sanmu->table('category') . " AS c, " .
                    $this->sanmu->table('goods') . " AS g" . " WHERE c.cat_id = g.cat_id AND g.goods_id = '" . $data_list[$i]['goods_id'] . "' ");
