...
class GoodsExportController extends InitController
{
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'goods_export') {
            ...
            if ($GLOBALS['_CFG']['shop_province'] || $GLOBALS['_CFG']['shop_city']) {
                $sql = "SELECT region_id,  region_name FROM " . $this->sanmu->table('region') . " WHERE region_id IN ('" . $GLOBALS['_CFG']['shop_province'] . "',  '" . $GLOBALS['_CFG']['shop_city'] . "')";
            ...
            $sql = "SELECT g.goods_id, g.goods_name, g.shop_price, g.goods_number, g.goods_desc, g.goods_img " .
                " FROM " . $this->sanmu->table('goods') . " AS g " . $where;
            ...
            if (EC_CHARSET != 'utf-8') {
                $content = sanmu_iconv(EC_CHARSET, 'utf-8', $content);
            }
        ...
        elseif ($_REQUEST['act'] == 'import_taobao') {
        ...
        } elseif ($_REQUEST['act'] == 'act_export_sanmumall') {
            /* 检查权限 */
            admin_priv('goods_export');
            ...
            if (isset($_POST) && !empty($_POST)) {
                ...
                $filter = sanmu_decode($_REQUEST['filter']);
                $arr = $this->get_export_step_where_sql($filter);
                $where = $arr['where'];
            }

            $page_size = 50; // 默认50张/页
            $sql = "SELECT count(*) FROM " . $this->sanmu->table('goods') . " AS g LEFT JOIN " . $this->sanmu->table('merchants_shop_brand') . " AS b " .
            ...
            if ($result['page'] <= ceil($count / $result['page_size'])) {
                ...
                $sql = "SELECT g.*, b.brand_name " .
                    " FROM " . $this->sanmu->table('goods') . " AS g LEFT JOIN " . $this->sanmu->table('brand') . " AS b " .
                ...
                $content = '"' . implode('","', $GLOBALS['_LANG']['sanmumall']) . "\"\n";
                ...
                $zip->add_file(sanmu_iconv(EC_CHARSET, $charset, $content), 'goods_list.csv');
            ...
            $where = $arr['where'];
            $page_size = 50; // 默认50张/页
            $sql = "SELECT count(*) FROM " . $this->sanmu->table('goods') . " AS g LEFT JOIN " . $this->sanmu->table('brand') . " AS b " .
                "ON g.brand_id = b.brand_id" . $where;
            $count = $this->db->getOne($sql);
            ...
            if (EC_CHARSET == 'utf-8') {
                $zip->add_file(sanmu_iconv('UTF8', 'GB2312', $content), 'goods_list.csv');
            } else {
        ...
        elseif ($_REQUEST['act'] == 'get_goods_fields') {
            ...
            foreach ($res as $row) {
                ...
                $sql = "SELECT `attr_id`, `attr_value` FROM " . $this->sanmu->table('goods_attr') . " WHERE `goods_id` = '" . $row['goods_id'] . "'";
            ...
            $filters = sanmu_decode($_REQUEST['JSON']);
    ...
    private function set_goods_field_name($array, $lang)
    {
        ...
        foreach ($array as $key => $value) {
            if (isset($lang[$value])) {
                ...
                $tmp_fields[$key] = $this->db->getOne("SELECT `attr_name` FROM " . $this->sanmu->table('attribute') . " WHERE `attr_id` = '" . intval($value) . "'");
            }
        }
        return $tmp_fields;
    }
    ...
    {
        $res = !empty($filter['filter']) ? sanmu_decode($filter['filter'], true) : [];
        if (!empty($res)) {
