...
class VisualEditingController extends InitController
{
    ...
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'first') {
            ...
            if (empty($code)) {
                $sql = "SELECT seller_templates FROM " . $this->sanmu->table('seller_shopinfo') . " WHERE ru_id = '" . $adminru['ru_id'] . "'";
                ...
                if ((isset($file['error']) && $file['error'] == 0) || (!isset($file['error']) && $file['tmp_name'] != 'none')) {
                    ...
                    if (!check_file_type($file['tmp_name'], $file['name'], $allow_file_types)) {
                        ...
                        if (move_upload_file($file['tmp_name'], $file_name)) {
                            ...
                            if ($bgtype) {
                                $theme = '';
                                $sql = "SELECT id ,img_file FROM" . $this->sanmu->table('templates_left') . " WHERE ru_id = '" . $adminru['ru_id'] . "' AND seller_templates = '$suffix' AND type = '$bgtype' AND theme = '$theme'";
                                ...
                                if ($templates_left['id'] > 0) {
                                    if ($templates_left['img_file'] != '') {
                                        ...
                                        $this->sanmuRepository->getOssDelFile([$old_oss_img_url]);
                                        sanmu_unlink(storage_public($templates_left['img_file']));
                                    }
                                    $sql = "UPDATE" . $this->sanmu->table('templates_left') . " SET img_file = '$oss_img_url' WHERE ru_id = '" . $adminru['ru_id'] . "' AND seller_templates = '$suffix' AND id='" . $templates_left['id'] . "' AND type = '$bgtype' AND theme = '$theme'";
                                    $this->db->query($sql);
                                } else {
                                    $sql = "INSERT INTO" . $this->sanmu->table('templates_left') . " (`ru_id`,`seller_templates`,`img_file`,`type`) VALUES ('" . $adminru['ru_id'] . "','$suffix','$oss_img_url','$bgtype')";
                                    $this->db->query($sql);
                                }
                            }
        ...
        elseif ($_REQUEST['act'] == 'release') {
            ...
            if ($type == 1) {
                ...
                if ($apply_id > 0) {
                    $sql = "SELECT temp_id,pay_id FROM" . $this->sanmu->table("seller_template_apply") . "WHERE apply_id = '$apply_id'";
                    $seller_template_apply = $this->db->getRow($sql);

                    $temp_id = $seller_template_apply['temp_id'];
                }
                $sql = "SELECT temp_mode,temp_cost,temp_code,temp_id FROM" . $this->sanmu->table("template_mall") . "WHERE temp_id = '$temp_id'";
                ...
                if ($template_mall['temp_mode'] == 1 && $template_mall['temp_cost'] > 0) {
                    ...
                    $error = 2;
                    //判断是否已经购买过
                    $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('seller_template_apply') . "WHERE pay_status = 1 AND apply_status = 1 AND temp_id = '$temp_id' AND apply_id != '$apply_id' AND ru_id = '" . $adminru['ru_id'] . "'";
                ...
                if ($error != 4) {
                    //更新模板使用数量
                    $sql = "UPDATE" . $this->sanmu->table('template_mall') . "SET sales_volume = sales_volume+1 WHERE temp_id = '$temp_id'";
                    $this->db->query($sql);
                }
            }
            if ($suffix && $type == 0) {
                $sql = "UPDATE" . $this->sanmu->table('seller_shopinfo') . " SET seller_templates = '$suffix' WHERE ru_id = '" . $adminru['ru_id'] . "'";
        ...
        elseif ($_REQUEST['act'] == 'purchase_temp') {
            ...
            if ($pay_id > 0) {
                if ($temp_id > 0) {
                    ...
                    //获取模板详细信息
                    $sql = "SELECT temp_mode,temp_cost,temp_code FROM" . $this->sanmu->table("template_mall") . "WHERE temp_id = '$temp_id'";
                    $template_mall = $this->db->getRow($sql);
                    ...
                    //入库
                    if ($apply_id > 0) {
                        $sql = "UPDATE" . $this->sanmu->table('seller_template_apply') . "SET pay_id = '$pay_id',total_amount = '" . $apply_info['order_amount'] . "',pay_fee = '" . $payment_info['pay_fee'] . "' WHERE apply_id = '$apply_id'";
                        $this->db->query($sql);
                        $apply_info['log_id'] = $this->db->getOne("SELECT log_id FROM" . $this->sanmu->table('pay_log') . "WHERE order_id = '$apply_id' AND order_type = '" . PAY_APPLYTEMP . "' LIMIT 1");
                        $apply_sn = $this->db->getOne("SELECT apply_sn FROM" . $this->sanmu->table('seller_template_apply') . "WHERE apply_id = '$apply_id'");
                    } else {
                        ...
                        $sql = 'INSERT INTO' . $this->sanmu->table("seller_template_apply") . $key . " VALUES" . $value;
                        $this->db->query($sql);
                        $apply_id = $this->db->insert_id();
                        $apply_info['log_id'] = insert_pay_log($apply_id, $apply_info['order_amount'], $type = PAY_APPLYTEMP, 0); //记录支付日志
                    }

                    return sanmu_header("Location: visual_editing.php?act=temp_pay&apply_id=" . $apply_id . "\n");
                } else {
            ...
            if ($apply_id == 0) {
                return sanmu_header("Location: visual_editing.php?act=template_apply_list\n");
            }
            load_helper('order');
            load_helper('payment');
            load_helper('clips');
            ...
            $sql = "SELECT apply_id,pay_fee, total_amount as order_amount ,ru_id,apply_sn as order_sn,pay_id ,temp_code,temp_id FROM" . $this->sanmu->table('seller_template_apply') . "WHERE apply_id = '$apply_id'";
            ...
            if ($payment_info['pay_code'] == 'balance') {
                //查询出当前用户的剩余余额;
                $user_money = $this->db->getOne("SELECT user_money FROM " . $this->sanmu->table('users') . " WHERE user_id='" . $adminru['ru_id'] . "'");
                //如果用户余额足够支付订单;
                if ($user_money > $apply_info['order_amount']) {
                    /* 修改申请的支付状态 */
                    $sql = " UPDATE " . $this->sanmu->table('seller_template_apply') . " SET pay_status = 1 ,pay_time = '" . gmtime() . "'  , apply_status = 1 WHERE apply_id= '" . $apply_id . "'";
                    $this->db->query($sql);

                    //记录支付log
                    $sql = "UPDATE " . $this->sanmu->table('pay_log') . "SET is_paid = 1 WHERE order_id = '" . $apply_id . "' AND order_type = '" . PAY_APPLYTEMP . "'";
                    $this->db->query($sql);
                    ...
                    //更新模板使用数量
                    $sql = "UPDATE" . $this->sanmu->table('template_mall') . "SET sales_volume = sales_volume+1 WHERE temp_id = '" . $apply_info['temp_id'] . "'";
                    $this->db->query($sql);
                    if ($apply_id > 0) {
 ...
 */
        elseif ($_REQUEST['act'] == 'checkorder') {
            ...
            $sql = "SELECT pay_status, pay_id FROM " . $this->sanmu->table('seller_template_apply') . " WHERE apply_id = '$apply_id' LIMIT 1";
        ...
        elseif ($_REQUEST['act'] == 'templates') {
            ...
            if (empty($tpl_arr)) {
                ...
                $sql = "UPDATE " . $this->sanmu->table('seller_shopinfo') . " SET seller_templates = '$new_suffix' WHERE ru_id=" . $adminru['ru_id'];
                $this->db->query($sql);
            }
            /*获取店铺正在使用的模板名称*/
            $sql = "SELECT seller_templates FROM " . $this->sanmu->table('seller_shopinfo') . " WHERE ru_id=" . $adminru['ru_id'];
        ...
        elseif ($_REQUEST['act'] == 'generate') {
            ...
            $sql = "SELECT id  FROM" . $this->sanmu->table('templates_left') . " WHERE ru_id = '" . $adminru['ru_id'] . "' AND seller_templates = '$suffix' AND type='$type' AND theme = '$theme'";
            $id = $this->db->getOne($sql);
            if ($id > 0) {
                $sql = "UPDATE " . $this->sanmu->table('templates_left') . " SET seller_templates = '$suffix',bg_color = '$bg_color' ,if_show = '$is_show',bgrepeat='$bgshow',align= '$bgalign',type='$type' WHERE ru_id = '" . $adminru['ru_id'] . "' AND seller_templates = '$suffix' AND id='$id' AND type='$type' AND theme = '$theme'";
            } else {
                $sql = "INSERT INTO " . $this->sanmu->table('templates_left') . " (`ru_id`,`seller_templates`,`bg_color`,`if_show`,`bgrepeat`,`align`,`type`) VALUES ('" . $adminru['ru_id'] . "','$suffix','$bg_color','$is_show','$bgshow','$bgalign','$type')";
            }
            if ($this->db->query($sql) == true) {
                $result['error'] = 1;
            } else {
        ...
        elseif ($_REQUEST['act'] == 'remove_img') {
            ...
            $sql = "UPDATE " . $this->sanmu->table('templates_left') . " SET img_file = '' WHERE ru_id = '" . $adminru['ru_id'] . "' AND type = '$type' AND seller_templates = '$suffix' AND theme = ''";
            $this->db->query($sql);
        } /*编辑模板信息*/
        elseif ($_REQUEST['act'] == 'edit_information') {
            ...
            $this->sanmuRepository->getOssAddFile($images_list);
        ...
        elseif ($_REQUEST['act'] == 'removeTemplate') {
            ...
            if ($default_tem == $code) {
                ...
                $this->sanmuRepository->getOssDelFile($file['screenshot']);
                $this->sanmuRepository->getOssDelFile($file['template']);
        ...
        elseif ($_REQUEST['act'] == 'defaultTemplate') {
            ...
            return sanmu_header("Location:visual_editing.php?act=templates\n");
        } /*备份*/
        ...
        elseif ($_REQUEST['act'] == 'downloadModal') {
            ...
            if (!isset($GLOBALS['_CFG']['open_oss'])) {
                $sql = "SELECT value FROM " . $this->sanmu->table('shop_config') . " WHERE code = 'open_oss'";
            ...
            if ($is_oss && $server_model) {
                if ($topic_type == 'topic_type') {
                    ...
                    $this->sanmuRepository->getOssAddFile($file_list);

                    sanmu_unlink(storage_public(DATA_DIR . '/sc_file/topic/topic_' . $adminru['ru_id'] . "/" . $code . ".php"));

                    $id_data = read_static_cache('urlip_list');

                    if ($pin_region_list !== false) {
                        $this->sanmuRepository->getDelVisualTemplates($id_data, $code, 'del_topictemplates', $adminru['ru_id']);
                    }
                } else {
                    ...
                    sanmu_unlink(storage_public(DATA_DIR . '/sc_file/sellertemplates/seller_tem_' . $adminru['ru_id'] . '/' . $code . ".php"));

                    $id_data = read_static_cache('urlip_list');

                    if ($pin_region_list !== false) {
                        $this->sanmuRepository->getDelVisualTemplates($id_data, $code, 'del_sellertemplates', $adminru['ru_id']);
                    }
                }
            }
            /* 存入OSS end */
        ...
        elseif ($_REQUEST['act'] == 'remove') {
            ...
            $sql = "DELETE FROM" . $this->sanmu->table('seller_template_apply') . "WHERE apply_id = '$apply_id' AND ru_id = '" . $adminru['ru_id'] . "'AND pay_status = 0";
            $this->db->query($sql);
