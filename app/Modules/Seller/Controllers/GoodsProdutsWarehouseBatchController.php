...
class GoodsProdutsWarehouseBatchController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'upload') {
            ...
            if ($_FILES['file']['name']) {
                ...
                foreach ($attr_names as $k => $v) {
                    $attr_names[$k] = sanmu_iconv('GBK', 'UTF8', $v);
                }
        ...
        elseif ($_REQUEST['act'] == 'ajax_insert') {
            ...
            if (session()->has('goods_list') && session('goods_list')) {
                ...
                $goods_list = $this->sanmu->page_array($page_size, $page, $goods_list);
                ...
                if (!empty($result['list']['goods_attr'])) {
                    ...
                    //查询数据是否已经存在;
                    $sql = "SELECT product_id from " . $this->sanmu->table('products_warehouse') . " WHERE goods_id = '" . $result['list']['goods_id'] . "'" .
                        " AND warehouse_id = '" . $result['list']['warehouse_id'] . "'" . $where;
                    $product_id = $this->db->getOne($sql);

                    if ($product_id) {
                        $this->db->autoExecute($this->sanmu->table('products_warehouse'), $other, 'UPDATE', "product_id = '$product_id'");
                        ...
                        if ($GLOBALS['_CFG']['goods_attr_price'] == 1) {
                            ...
                            $this->db->autoExecute($this->sanmu->table('goods'), $goods_other, 'UPDATE', "goods_id = '" . $result['list']['goods_id'] . "' AND product_id = '$product_id' AND product_table = 'products_warehouse'");
                        }
                        ...
                        $this->db->autoExecute($this->sanmu->table('products_warehouse'), $other, 'INSERT');
        ...
        elseif ($_REQUEST['act'] == 'download') {
            ...
            if (isset($GLOBALS['_LANG']['upload_product'])) {
                ...
                if ($_GET['charset'] == 'zh-CN' || $_GET['charset'] == 'zh-TW') {
                    ...
                    echo sanmu_iconv(EC_CHARSET, $to_charset, $data);
                } else {
    ...
    {
        $sql = "SELECT a.attr_id, a.attr_name FROM " . $this->sanmu->table('goods_attr') . " AS ga " .
            " LEFT JOIN " . $this->sanmu->table('attribute') . " as a ON ga.attr_id = a.attr_id" .
    ...
    {
        $sql = "select goods_attr_id, attr_value from " . $this->sanmu->table('goods_attr') . " where goods_id = '$goods_id' and attr_id = '$attr_id' order by goods_attr_id asc";
        $res = $this->db->getAll($sql);
