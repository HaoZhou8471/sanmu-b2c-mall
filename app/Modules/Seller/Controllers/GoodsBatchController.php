...
class GoodsBatchController extends InitController
{
    ...
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'add') {
            ...
            /* 取得可选语言 */
            $download_list = $this->sanmuRepository->getDdownloadTemplate(resource_path('lang'));

            $data_format_array = [
                'sanmumall' => $GLOBALS['_LANG']['export_sanmumall'],
        ...
        elseif ($_REQUEST['act'] == 'upload') {
            ...
            $max_id = $this->db->getOne("SELECT MAX(goods_id) FROM " . $this->sanmu->table('goods'));
            ...
            if ($_POST['data_cat'] == 'sanmumall') {
                foreach ($data as $line) {
                    ...
                    if (($_POST['charset'] != 'UTF8') && (strpos(strtolower(EC_CHARSET), 'utf') === 0)) {
                        $line = sanmu_iconv($_POST['charset'], 'UTF8', $line);
                    }
                ...
                foreach ($data as $key => $line) {
                    ...
                    if (($_POST['charset'] == 'UTF8') && (strpos(strtolower(EC_CHARSET), 'utf') == 0)) {
                        $line = sanmu_iconv($_POST['charset'], 'GBK', $line);
                    }
        ...
        elseif ($_REQUEST['act'] == 'insert') {
            ...
            if (isset($_POST['checked'])) {
                ...
                /* 查询品牌列表 */
                $brand_list = [];
                $sql = "SELECT brand_id, brand_name FROM " . $this->sanmu->table('brand');
                ...
                foreach ($_POST['checked'] as $key => $value) {
                        ...
                        if (!empty($field_value)) {
                            ...
                            elseif ($field == 'brand_name') {
                                if (isset($brand_list[$field_value])) {
                                    ...
                                    $sql = "INSERT INTO " . $this->sanmu->table('brand') . " (brand_name) VALUES ('" . addslashes($field_value) . "')";
                                    $this->db->query($sql);
                    ...
                    $this->db->autoExecute($this->sanmu->table('goods'), $field_arr, 'INSERT');
                    ...
                    if (!empty($field_arr['original_img']) || !empty($field_arr['goods_img']) || !empty($field_arr['goods_thumb'])) {
                        ...
                        //修改商品图
                        $this->db->query("UPDATE " . $this->sanmu->table('goods') . " SET goods_img = '$goods_img', goods_thumb = '$goods_thumb', original_img = '$original_img' WHERE goods_id='" . $goods_gallery['goods_id'] . "'");

                        //添加商品相册图
                        if ($GLOBALS['_CFG']['auto_generate_gallery']) {
                            $this->db->autoExecute($this->sanmu->table('goods_gallery'), $goods_gallery, 'INSERT');
                        }
                    }
                }
            }
        ...
        elseif ($_REQUEST['act'] == 'edit') {
            ...
            if ($_POST['select_method'] == 'cat') {
                ...
                $sql = "SELECT DISTINCT goods_id FROM " . $this->sanmu->table('goods') .
            ...
            $sql = "SELECT DISTINCT goods_id, goods_sn, goods_name, market_price, shop_price, goods_number, integral, give_integral, brand_id, is_real,model_attr FROM " . $this->sanmu->table('goods') . $where;
            ...
            foreach ($goods_list as $key => $val) {
                ...
                $sql = "SELECT * FROM " . $this->sanmu->table($table_products) . "WHERE goods_id = '" . $val['goods_id'] . "'";
            ...
            /* 取得会员价格 */
            $member_price_list = [];
            $sql = "SELECT DISTINCT goods_id, user_rank, user_price FROM " . $this->sanmu->table('member_price') . $where;
            $res = $this->db->query($sql);
            foreach ($res as $row) {
            ...
            $sql = "SELECT rank_id, rank_name, discount " .
                "FROM " . $this->sanmu->table('user_rank') .
            ...
            if ($_POST['edit_method'] == 'each') {
                ...
                if (!empty($_POST['goods_id'])) {
                    foreach ($_POST['goods_id'] as $goods_id) {
                        $sql = "SELECT model_attr FROM" . $this->sanmu->table('goods') . "WHERE goods_id = '" . $goods_id . "' LIMIT 1";
                        ...
                        if (!empty($_POST['product_number'][$goods_id])) {
                            ...
                            foreach ($_POST['product_number'][$goods_id] as $key => $value) {
                                ...
                                $sql = "UPDATE" . $this->sanmu->table($table_products) . "SET product_number = '$value' WHERE goods_id = '$goods_id' AND product_id = " . $key . $table_where;
                                $this->db->query($sql);
                        ...
                        $this->db->autoExecute($this->sanmu->table('goods'), $goods, 'UPDATE', "goods_id = '$goods_id'");
                        ...
                        if (!empty($_POST['rank_id'])) {
                            foreach ($_POST['rank_id'] as $rank_id) {
                                ...
                                $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('member_price') . " WHERE goods_id = '$goods_id' AND user_rank = '$rank_id'";
                                if ($this->db->getOne($sql) > 0) {
                                    if ($rank['user_price'] < 0) {
                                        $this->db->query("DELETE FROM " . $this->sanmu->table('member_price') . " WHERE goods_id = '$goods_id' AND user_rank = '$rank_id'");
                                    } else {
                                        $this->db->autoExecute($this->sanmu->table('member_price'), $rank, 'UPDATE', "goods_id = '$goods_id' AND user_rank = '$rank_id'");
                                    }
                                } else {
                                    if ($rank['user_price'] >= 0) {
                                        $this->db->autoExecute($this->sanmu->table('member_price'), $rank, 'INSERT');
                                    }
                                }
                            }
                        }
        ...
        elseif ($_REQUEST['act'] == 'download') {
            ...
            if (isset($GLOBALS['_LANG']['upload_goods'])) {
                ...
                if ($_GET['charset'] == 'zh-CN' || $_GET['charset'] == 'zh-TW') {
                    ...
                    echo sanmu_iconv(EC_CHARSET, $to_charset, join(',', $GLOBALS['_LANG']['upload_goods']));
                } else {
