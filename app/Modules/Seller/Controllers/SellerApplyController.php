...
class SellerApplyController extends InitController
{
    ...
    public function index()
    {
        ...
        $exc = new Exchange($this->sanmu->table("seller_apply_info"), $this->db, 'apply_id', 'apply_sn');
...
//    admin_priv('seller_apply');
            ...
            /*获取申请信息*/
            $seller_apply_info = $this->db->getRow("SELECT * FROM" . $this->sanmu->table('seller_apply_info') . " WHERE apply_id = '$apply_id' LIMIT 1");
            if ($seller_apply_info['pay_id'] > 0) {
                $seller_apply_info['pay_name'] = $this->db->getOne("SELECT pay_name FROM " . $this->sanmu->table('payment') . " WHERE pay_id = '" . $seller_apply_info['pay_id'] . "'");
            }
            ...
            /*获取标准信息*/
            $entry_criteria = $this->db->getOne("SELECT entry_criteria FROM " . $this->sanmu->table('seller_grade') . " WHERE id = '$grade_id'");
        ...
        elseif ($_REQUEST['act'] == 'operation') {
            ...
            if ($is_confirm != 0) {
                ...
                if ($cfg == '1') {
                    ...
                    $shopinfo = $this->db->getRow("SELECT shop_name,seller_email FROM " . $this->sanmu->table('seller_shopinfo') . " WHERE ru_id = '$ru_id' LIMIT 1 ");
                    $grade['shop_name'] = $shopinfo['shop_name'];
                    $grade['email'] = $shopinfo['seller_email'];
                    $grade['grade_name'] = $this->db->getOne("SELECT grade_name FROM" . $this->sanmu->table('seller_grade') . " WHERE id = '" . $_REQUEST['garde_id'] . "'");
                ...
                $apply = $this->db->getRow("SELECT pay_status , payable_amount ,back_price, apply_sn FROM " . $this->sanmu->table('seller_apply_info') . " WHERE apply_id = '$apply_id'");//
                /*编辑商家等级*/
                if ($is_confirm == 1) {

                    /*更新商家权限*/
                    $action_list = $this->db->getRow("SELECT action_list FROM" . $this->sanmu->table('merchants_privilege') . " WHERE grade_id = '$grade_id' LIMIT 1");
                    $sql = "UPDATE" . $this->sanmu->table('admin_user') . " SET action_list = " . $action_list['action_list'] . " WHERE ru_id = '$ru_id'";
                    $this->db->query($sql);

                    /*判断是否已经存在等级，是则修改，不是则插入*/
                    $sql = " SELECT id FROM " . $this->sanmu->table('merchants_grade') . " WHERE ru_id = '$ru_id'";
                    if ($this->db->getOne($sql) > 0) {
                        $this->db->query("UPDATE " . $this->sanmu->table('merchants_grade') . " SET grade_id = '$grade_id',add_time = '" . gmtime() . "' ,amount = '$total_amount' , year_num = '$year_num' WHERE ru_id = '$ru_id'");
                    } else {
                        $this->db->query("INSERT INTO  " . $this->sanmu->table('merchants_grade') . " (`ru_id`,`grade_id`,`add_time`,`amount`,`year_num`) VALUES ('$ru_id','$grade_id','" . gmtime() . "','$total_amount','$year_num')");
                    }
                    /*退款*/
                    ...
                    if ($is_confirm == 2 || $is_confirm == 3) {
                        $sql = "DELETE FROM" . $this->sanmu->table("merchants_grade") . " WHERE ru_id = '$ru_id' AND grade_id = '$grade_id'";
                        $this->db->table($sql);
                    }
            ...
            /*修改申请状态*/
            $sql = 'UPDATE' . $this->sanmu->table('seller_apply_info') . " SET apply_status = '$is_confirm' , confirm_time = '$confirm_time',reply_seller= '$reply_seller',is_paid = '$is_paid' ,pay_status = '$is_paid' , pay_time = '" . gmtime() . "'  WHERE apply_id = '$apply_id'";
        ...
        elseif ($_REQUEST['act'] == 'remove') {
            ...
            /* 判断标准类型，如果上传文件，删除文件 */
            $entry_criteria_info = $this->db->getRow("SELECT entry_criteria FROM " . $this->sanmu->table('seller_apply_info') . " WHERE apply_id = '$id' LIMIT 1");
            ...
            if (!empty($entry_criteria)) {
                foreach ($entry_criteria as $k => $v) {
                    $type = $this->db->getOne(" SELECT type FROM" . $this->sanmu->table('entry_criteria') . " WHERE id = '$k'");//获取标准类型
                    /*如果是文件上传，删除文件*/
                    if ($type == 'file' && $v != '') {
    ...
    private function get_pzd_list()
    {
        ...
        if ($result === false) {
            ...
            if ($grade_name) {
                $filter['grade_id'] = $this->db->getOne("SELECT id FROM" . $this->sanmu->table('seller_grade') . " WHERE grade_name LIKE '%" . $grade_name . "%'");
            }
            ...
            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('seller_apply_info') . $where;
            $filter['record_count'] = $this->db->getOne($sql);
            $filter = page_and_size($filter);
            /* 获活动数据 */
            $sql = "SELECT * FROM" . $this->sanmu->table('seller_apply_info') . $where . "  ORDER BY add_time ASC LIMIT " . $filter['start'] . "," . $filter['page_size'];
        ...
        foreach ($row as $k => $v) {
            ...
            $row[$k]['grade_name'] = $this->db->getOne("SELECT grade_name FROM " . $this->sanmu->table('seller_grade') . " WHERE id = '" . $v['grade_id'] . "'");
            $row[$k]['add_time'] = local_date('Y-m-d H:i:s', $v['add_time']);
            if ($v['pay_id'] > 0) {
                $row[$k]['pay_name'] = $this->db->getOne("SELECT pay_name FROM " . $this->sanmu->table('payment') . " WHERE pay_id = '" . $v['pay_id'] . "'");
            }
    ...
    private function get_entry_criteria($entry_criteria = '')
    {
        ...
        $rel = '';
        if (!empty($entry_criteria)) {
            $sql = " SELECT id,criteria_name FROM" . $this->sanmu->table('entry_criteria') . " WHERE id " . db_create_in($entry_criteria);
            $rel = $this->db->getAll($sql);
            foreach ($rel as $k => $v) {
                $child = $this->db->getAll(" SELECT * FROM" . $this->sanmu->table("entry_criteria") . " WHERE parent_id = '" . $v['id'] . "'");
