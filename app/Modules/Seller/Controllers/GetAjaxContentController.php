...
class GetAjaxContentController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'addmodule') {
            ...
            if ($_REQUEST['spec_attr']) {
                ...
                if (!empty($_REQUEST['spec_attr'])) {
                    $spec_attr = sanmu_decode($_REQUEST['spec_attr'], true);
                }
            }
            ...
            if ($count) {
                for ($i = 0; $i < $count; $i++) {
                    ...
                    if (isset($link[$i]) && $link[$i]) {
                        $arr[$i]['link'] = $this->sanmuRepository->setRewriteUrl($link[$i]);
                    } else {
        ...
        elseif ($_REQUEST['act'] == 'changedgoods') {
            ...
            if (!empty($goods_list)) {
                foreach ($goods_list as $k => $v) {
                    ...
                    $goods_list[$k]['url'] = $this->sanmuRepository->buildUri('goods', ['gid' => $v['goods_id']], $v['goods_name']);
        ...
        elseif ($_REQUEST['act'] == 'navigator') {
            ...
            $sql = "SELECT name, url " .
                " FROM " . $this->sanmu->table('merchants_nav') . $where .
                " ORDER by vieworder";
            $navigator = $this->db->getAll($sql);
            foreach ($navigator as $k => $v) {
                if ($v['url']) {
                    $navigator[$k]['url'] = $this->sanmuRepository->setRewriteUrl($v['url']);
                }
            }
            ...
            if ($seller_domain) {
                $index_url = $this->sanmuRepository->sellerUrl($adminru['ru_id']);
            } else {
        ...
        elseif ($_REQUEST['act'] == 'album_move_back') {
            ...
            $sql = "UPDATE" . $this->sanmu->table('pic_album') . " SET album_id = '" . $album_id . "' WHERE pic_id = '" . $pic_id . "' AND ru_id = '" . $adminru['ru_id'] . "'";
            $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'add_albun_pic') {
            ...
            $exc = new Exchange($this->sanmu->table("gallery_album"), $this->db, 'album_id', 'album_mame');
        ...
        elseif ($_REQUEST['act'] == 'filter_list') {
            ...
            if ($search_type == "goods") {
                ...
                $filters = sanmu_decode(urldecode(json_encode($filters)));
            ...
            elseif ($search_type == 'get_content') {
                ...
                $filters = sanmu_decode(json_encode($filters));
        ...
        elseif ($_REQUEST['act'] == 'add_win_goods') {
            ...
            $sql = "select win_goods from " . $this->sanmu->table('seller_shopwindow') . " where id='$id'";
            ...
            $sql = "update " . $this->sanmu->table('seller_shopwindow') . " set win_goods='$win_goods' where id='$id'";
            $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'drop_win_goods') {
            ...
            $win_goods = $this->db->getOne("select win_goods from " . $this->sanmu->table('seller_shopwindow') . " where id='$id'");
            ...
            $sql = "update " . $this->sanmu->table('seller_shopwindow') . " set win_goods='$new_win_goods' where id='$id'";
            $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'upload_img') {
            ...
            if ($act_type == 'goods_img') {
                ...
                if (empty($goods_id)) {
                    ...
                    $this->db->autoExecute($this->sanmu->table('goods'), $result['data'], 'UPDATE', "goods_id = '$goods_id'");
                }

                $this->sanmuRepository->getOssAddFile($result['data']);
                ...
                if ($img) {
                    ...
                    $gallery_count = $this->db->getOne("SELECT MAX(img_desc) FROM " . $this->sanmu->table('goods_gallery') . " WHERE goods_id = '$goods_id'");
                    $img_desc = $gallery_count + 1;

                    $gallery_images = [$gallery_img, $gallery_thumb, $img];

                    $sql = "INSERT INTO " . $this->sanmu->table('goods_gallery') . " (goods_id, img_url, img_desc, thumb_url, img_original) " .
                        "VALUES ('$goods_id', '$gallery_img', $img_desc, '$gallery_thumb', '$img')";
                    $this->db->query($sql);
                    $thumb_img_id[] = $this->db->insert_id();
                    $this->sanmuRepository->getOssAddFile([$gallery_img, $gallery_thumb, $img]);
                ...
                if ($GLOBALS['_CFG']['open_oss'] == 1 && $bucket_info['is_delimg'] == 1) {
                    if ($result['data']) {
                        ...
                        sanmu_unlink($goods_images);
                    }

                    if ($gallery_images) {
                        sanmu_unlink($gallery_images);
                    }
                }
                /* 删除本地商品图片 start */
                ...
                if ($goods_id > 0) {
                    /* 图片列表 */
                    $sql = "SELECT * FROM " . $this->sanmu->table('goods_gallery') . " WHERE goods_id = '$goods_id' ORDER BY img_desc ASC";
                } else {
                ...
                /* 结束处理 start */
                $this_img_info = $this->db->getRow(" SELECT * FROM " . $this->sanmu->table('goods_gallery') . " WHERE goods_id = '$goods_id' ORDER BY img_id DESC LIMIT 1 ");
            ...
            if ($groupCount < 1000) {
                ...
                if (!empty($goods_ids)) {
                    foreach ($goods_ids as $key => $val) {
                        $sql = "SELECT id FROM " . $this->sanmu->table('group_goods') . " WHERE parent_id = '$goods_id' AND goods_id = '$val'  LIMIT 1";
                        if (!$this->db->getOne($sql)) {
                            $price_goods = 0;
                            if ($price == 0) {
                                $price_goods = $this->db->getOne("SELECT shop_price FROM" . $this->sanmu->table('goods') . " WHERE goods_id = '$val'");
                            } else {
                                $price_goods = $price;
                            }
        ...
        elseif ($_REQUEST['act'] == 'drop_link_desc') {
            ...
            if (empty($linked_goods)) {
                $sql = "delete from " . $this->sanmu->table('link_desc_temporary') . " where 1";
                $this->db->query($sql);
            }
        ...
        elseif ($_REQUEST['act'] == 'edit_navname') {
            $result = ['error' => '', 'pic_id' => '', 'content' => ''];
            $exc = new Exchange($this->sanmu->table("merchants_nav"), $this->db, 'id', 'name');
            ...
            if ($id > 0 && !empty($nav_name)) {
                ...
                if (!$is_only) {
                    ...
                    $sql = "UPDATE" . $this->sanmu->table('merchants_nav') . " SET name = '$nav_name' WHERE id = '$id' AND ru_id = " . $adminru['ru_id'];
                    $this->db->query($sql);
                    $result['error'] = 1;
        ...
        elseif ($_REQUEST['act'] == 'edit_navurl') {
            ...
            $id = isset($_REQUEST['id']) ? intval($_REQUEST['id']) : 0;
            if ($id > 0) {
                $sql = "UPDATE" . $this->sanmu->table('merchants_nav') . " SET url = '$url' WHERE id = '$id' AND ru_id = " . $adminru['ru_id'];
                $this->db->query($sql);
                $result['error'] = 1;
                $result['content'] = $GLOBALS['_LANG']['edit_success'];
            } else {
        ...
        elseif ($_REQUEST['act'] == 'edit_navvieworder') {
            ...
            if ($id > 0) {
                if (preg_match('/^\d+$/i', $order)) {
                    $sql = "UPDATE" . $this->sanmu->table('merchants_nav') . " SET vieworder = '$order' WHERE id = '$id' AND ru_id = " . $adminru['ru_id'];
                    $this->db->query($sql);
                    $result['error'] = 1;
                    $result['content'] = $GLOBALS['_LANG']['edit_success'];
                } else {
        ...
        elseif ($_REQUEST['act'] == 'remove_nav') {
            ...
            $id = isset($_REQUEST['id']) ? intval($_REQUEST['id']) : 0;
            if ($id > 0) {
                $row = $this->db->getRow("SELECT ctype,cid,type FROM " . $this->sanmu->table('merchants_nav') . " WHERE id = '$id' LIMIT 1");

                if ($row['type'] == 'middle' && $row['ctype'] && $row['cid']) {
                    $this->set_show_in_nav($row['ctype'], $row['cid'], 0);
                }

                $sql = " DELETE FROM " . $this->sanmu->table('merchants_nav') . " WHERE id='$id' LIMIT 1";
                $this->db->query($sql);
                $result['error'] = 1;
        ...
        elseif ($_REQUEST['act'] == 'add_nav') {
            ...
            if (!empty($name)) {
                ...
                if (!$is_only) {
                    ...
                    $sql = "INSERT INTO" . $this->sanmu->table("merchants_nav") . "(`name`,`url`,`ifshow`,`type`,`ru_id`,`vieworder`) VALUES('$name','$link',1,'middle','" . $adminru['ru_id'] . "',50)";
                    $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'edit_ifshow') {
            ...
            if ($id > 0) {
                ...
                $sql = "UPDATE" . $this->sanmu->table('merchants_nav') . " SET ifshow = '$val' WHERE id = '$id' AND ru_id = " . $adminru['ru_id'];
                $this->db->query($sql);
                $result['error'] = 1;
                $result['id'] = $id;
                $html_ifshow = "'" . $val . "'";
        ...
        elseif ($_REQUEST['act'] == 'gallery_album_list') {
            ...
            if ($gallery_album_list || $album_id > 0) {
                ...
                $sql = "SELECT album_mame FROM " . $this->sanmu->table('gallery_album') . " "
                    . " WHERE ru_id = '" . $adminru['ru_id'] . "' AND  album_id = '" . $album_id . "'";
                $album_mame = $this->db->getOne($sql);
                $where = "";
                $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('pic_album') . " WHERE ru_id = '" . $adminru['ru_id'] . "' AND album_id = '" . $album_id . "'";
                $filter['record_count'] = $this->db->getOne($sql);
                $filter = page_and_size($filter, 2);
                $where = "LIMIT " . $filter['start'] . "," . $filter['page_size'];
                $sql = "SELECT * FROM " . $this->sanmu->table('pic_album') . " WHERE ru_id = '" . $adminru['ru_id'] . "' AND album_id ='" . $album_id . "' ORDER BY pic_id ASC $where";
        ...
        elseif ($_REQUEST['act'] == 'insert_goodsImg') {
            ...
            $thumb_img_id = [];
            $img_default = 1;
            $sql = "SELECT pic_file,pic_thumb,pic_image FROM" . $this->sanmu->table('pic_album') . "WHERE pic_id in(" . $pic_id . ")";
            $img_list = $this->db->getAll($sql);
            ...
            if ($img_list) {
                ...
                foreach ($img_list as $key => $val) {
                    ...
                    if ($inid == 'gallery_album') {
                        $img_desc_new = 1;
                        if ($goods_id > 0) {
                            $sql = "UPDATE" . $this->sanmu->table('goods_gallery') . " SET img_desc = img_desc+1 WHERE goods_id = '$goods_id'";
                            $this->db->query($sql);
                        }
                    } else {
                        $gallery_count = $this->goodsManageService->getGoodsGalleryCount($goods_id);
            ...
            if ($goods_id > 0) {
                /* 图片列表 */
                $sql = "SELECT img_desc,thumb_url,goods_id,external_url,img_id FROM " . $this->sanmu->table('goods_gallery') . " WHERE goods_id = '$goods_id' ORDER BY img_desc ASC";
            } else {
                $where = '';
                ...
                $sql = "SELECT img_desc,thumb_url,goods_id,external_url,img_id FROM " . $this->sanmu->table('goods_gallery') . " WHERE goods_id='' $where ORDER BY img_desc ASC";
            }
        ...
        elseif ($_REQUEST['act'] == 'homeFloor') {
                ...
                if (!empty($result['leftBanner'])) {
                    foreach ($result['leftBanner'] as $k => $v) {
                        ...
                        if ($result['leftBannerLink'][$k]) {
                            $AdvBanner[$k]['leftBannerLink'] = $this->sanmuRepository->setRewriteUrl($result['leftBannerLink'][$k]);
                        } else {
            ...
            if (!empty($AdvNum['leftAdv']) && $AdvNum['leftAdv'] > 0) {
                for ($k = 0; $k < $AdvNum['leftAdv']; $k++) {
                    ...
                    if (isset($result['leftAdvLink'][$k])) {
                        $AdvLeft[$k]['leftAdvLink'] = $this->sanmuRepository->setRewriteUrl($result['leftAdvLink'][$k]);
                    } else {
            ...
            if (!empty($AdvNum['rightAdv']) && $AdvNum['rightAdv'] > 0) {
                for ($k = 0; $k < $AdvNum['rightAdv']; $k++) {
                    ...
                    if (isset($result['leftBannerLink'][$k])) {
                        $AdvRight[$k]['rightAdvLink'] = $this->sanmuRepository->setRewriteUrl($result['rightAdvLink'][$k]);
                    } else {
            ...
            if (!empty($result['cateValue'])) {
                ...
                foreach ($result['cateValue'] as $k => $v) {
                    ...
                    $arr = [];
                    if ($v > 0) {
                        $sql = "SELECT cat_name,cat_id FROM " . $this->sanmu->table('category') . " WHERE cat_id = '$v'";
            ...
            if ($result['brand_ids']) {
                ...
                $sql = "SELECT b.brand_id,b.brand_name,b.brand_logo,b.site_url FROM " . $this->sanmu->table('brand') . " as b left join " . $this->sanmu->table('brand_extend') . " AS be on b.brand_id=be.brand_id " . $where;
                ...
                if (!empty($brand_list)) {
                    foreach ($brand_list as $key => $val) {
                        if ($val['site_url'] && strlen($val['site_url']) > 8) {
                            ...
                            $brand_list[$key]['url'] = $this->sanmuRepository->buildUri('brandn', ['bid' => $val['brand_id']], $val['brand_name']);
                        }
                        $val['brand_logo'] = DATA_DIR . '/brandlogo/' . $val['brand_logo'];
            ...
            if ($result['moded'] == 'homeFloorFour' || $result['moded'] == 'homeFloorFive' || $result['moded'] == 'homeFloorSeven' ||
                ...
                $sql = "SELECT g.promote_start_date, g.promote_end_date, g.promote_price, g.goods_name, g.goods_id, g.goods_thumb, g.shop_price, g.market_price, g.original_img  FROM " .
                    $this->sanmu->table('goods') . " AS g " . $where_goods . " ORDER BY g.sort_order, g.goods_id DESC " . $limit;
                ...
                if (!empty($goods_list)) {
                    foreach ($goods_list as $key => $val) {
                        ...
                        if ((strpos($goods_list[$key]['goods_thumb'], 'http://') === false && strpos($goods_list[$key]['goods_thumb'], 'https://') === false)) {
                            $goods_list[$key]['goods_thumb'] = $this->sanmu->url() . $goods_list[$key]['goods_thumb'];
                        }

                        if ((strpos($goods_list[$key]['original_img'], 'http://') === false && strpos($goods_list[$key]['original_img'], 'https://') === false)) {
                            $goods_list[$key]['original_img'] = $this->sanmu->url() . $goods_list[$key]['original_img'];
                        }
                        $goods_list[$key]['url'] = $this->sanmuRepository->buildUri('goods', ['gid' => $val['goods_id']], $val['goods_name']);
                    }
                }
                $this->smarty->assign("goods_list", $goods_list);
        ...
        elseif ($_REQUEST['act'] == 'getsearchgoodsDiv') {
            ...
            if ($goods_ids) {
                ...
                $sql = "SELECT  g.goods_name, g.goods_id  FROM " .
                    $this->sanmu->table('goods') . " AS g " . $where;
                $goods_list = $this->db->getAll($sql);
                //重定义goods_id
                if (!empty($goods_list)) {
        ...
        elseif ($_REQUEST['act'] == 'add_seckill_goods') {
            ...
            if ($goods_ids) {
                ...
                foreach ($goods_ids_arr as $val) {
                    $sql = "SELECT id FROM " . $this->sanmu->table('seckill_goods') . " WHERE goods_id = '$val' AND sec_id = '$sec_id' AND tb_id = '$tb_id'";
                    if (!$this->db->getOne($sql)) {
                        $sql = "INSERT INTO " . $this->sanmu->table('seckill_goods') . " (sec_id, tb_id, goods_id) " .
                ...
                //获取秒杀商品ID列表
                $sql = " SELECT GROUP_CONCAT(goods_id) FROM " . $this->sanmu->table("seckill_goods") . " WHERE sec_id = '$sec_id' AND tb_id = '$tb_id' ";
        ...
        elseif ($_REQUEST['act'] == 'add_goods_type_cat') {
            $exc_cat = new Exchange($this->sanmu->table("goods_type_cat"), $this->db, 'cat_id', 'cat_name');
            $result = ['error' => 0, 'message' => '', 'content' => ''];
            ...
            //获取入库分类的层级
            if ($parent_id > 0) {
                $sql = "SELECT level FROM" . $this->sanmu->table("goods_type_cat") . " WHERE cat_id = '$parent_id' LIMIT 1";
            ...
            $this->db->autoExecute($this->sanmu->table('goods_type_cat'), $cat_info, 'INSERT');
        ...
        elseif ($_REQUEST['act'] == 'add_goods_type') {
            $result = ['error' => 0, 'message' => '', 'content' => ''];
            $parent_id = !empty($_REQUEST['attr_parent_id']) ? intval($_REQUEST['attr_parent_id']) : 0;
            $goods_type['cat_name'] = $this->sanmuRepository->subStr($_POST['cat_name'], 60);
            $goods_type['attr_group'] = $this->sanmuRepository->subStr($_POST['attr_group'], 255);
            $goods_type['user_id'] = $adminru['ru_id'];
            $goods_type['c_id'] = $parent_id;
            $this->db->autoExecute($this->sanmu->table('goods_type'), $goods_type);
        ...
        elseif ($_REQUEST['act'] == 'attribute_add') {
            ...
            $exc = new Exchange($this->sanmu->table("attribute"), $this->db, 'attr_id', 'attr_name');
            ...
            $this->db->autoExecute($this->sanmu->table('attribute'), $attr, 'INSERT');
            $attr_id = $this->db->insert_id();

            $sql = "SELECT MAX(sort_order) AS sort FROM " . $this->sanmu->table('attribute') . " WHERE cat_id = '$cat_id'";
            ...
            if (empty($attr['sort_order']) && !empty($sort)) {
                ...
                $this->db->autoExecute($this->sanmu->table('attribute'), $attr, 'UPDATE', "attr_id = '$attr_id'");
            }
    ...
    {
        $adminru = get_admin_ru_id();
        $sql = "select id,win_goods from " . $this->sanmu->table('seller_shopwindow') . " where id='$id' and ru_id='" . $adminru['ru_id'] . "'";
        ...
        if ($win_info['id'] > 0) {
            ...
            $goods = [];
            if ($goods_ids) {
                $sql = "select goods_id,goods_name from " . $this->sanmu->table('goods') . " where user_id='" . $adminru['ru_id'] . "' and goods_id in ($goods_ids)";
    ...
    {
        $sql = "SELECT `attr_id`, `cat_id`, `attr_name` FROM " . $this->sanmu->table('attribute') . " ";
        if (!empty($cat_id)) {
    ...
    {
        if ($type == 'c') {
            $tablename = $this->sanmu->table('category');
        } else {
            $tablename = $this->sanmu->table('article_cat');
        }
    ...
    private function get_brandlist($filters)
    {
        ...
        $sql = "SELECT b.brand_id, b.brand_name,b.brand_logo, COUNT(*) AS goods_num " .
            "FROM " . $this->sanmu->table('brand') . "AS b " .
            " LEFT JOIN " . $this->sanmu->table('goods') . " AS g ON g.brand_id = b.brand_id AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 " .
            " LEFT JOIN " . $this->sanmu->table('goods_cat') . " AS gc ON g.goods_id = gc.goods_id " .
