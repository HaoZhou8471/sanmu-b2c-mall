...
class GoodsAreaAttrBatchController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'ajax_insert') {
            ...
            if (session()->has('goods_list') && session('goods_list')) {
                ...
                //查询数据是否已经存在;
                $sql = "SELECT id FROM " . $this->sanmu->table('warehouse_area_attr') . " WHERE goods_id = '" . $result['list']['goods_id'] . "' AND area_id = '" . $result['list']['area_id'] . "'" .
                ...
                if ($this->db->getOne($sql, true)) {
                    ...
                    $this->db->autoExecute($this->sanmu->table('warehouse_area_attr'), $other, 'UPDATE', "goods_id = '" . $result['list']['goods_id'] . "' AND area_id = '" . $result['list']['area_id'] . "' AND goods_attr_id = '" . $result['list']['goods_attr_id'] . "' $where");
                    $result['status_lang'] = '<span style="color: red;">' . $GLOBALS['_LANG']['updated_data_success'] . '</span>';
                } else {
                    $other['admin_id'] = session('seller_id');

                    $this->db->autoExecute($this->sanmu->table('warehouse_area_attr'), $other, 'INSERT');
    ...
    private function get_goods_attr_list($goods_id)
    {
        ...
        $sql = "SELECT ga.goods_attr_id, ga.goods_id, ga.attr_id, ga.attr_value, ga.attr_sort, ga.admin_id, war.attr_price AS attr_price FROM " . $this->sanmu->table("goods_attr") . " AS ga " .
            " LEFT JOIN " . $this->sanmu->table("warehouse_area_attr") . " AS war ON ga.goods_attr_id = war.goods_attr_id AND ga.goods_id = war.goods_id" .
