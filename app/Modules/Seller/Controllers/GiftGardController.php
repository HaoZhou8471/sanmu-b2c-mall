...
class GiftGardController extends InitController
{
    ...
    public function index()
    {
        ...
        /* 初始化$exc对象 */
        $exc = new Exchange($this->sanmu->table('gift_gard_type'), $this->db, 'gift_id', 'gift_name');
        ...
        if ($_REQUEST['act'] == 'edit_type_money') {
            ...
            $sql = "UPDATE " . $this->sanmu->table('user_gift_gard') . " SET express_no='$val' WHERE gift_gard_id='$id'";
        ...
        if ($_REQUEST['act'] == 'confirm_ship') {
            ...
            if (!empty($id)) {
                if ($this->db->getOne("SELECT COUNT(*) FROM " . $this->sanmu->table('user_gift_gard') . " WHERE status = '3' AND gift_gard_id='$id'")) {
                    // 			return make_json_error('此订单已经完成，不能取消发货');
                } elseif ($this->db->getOne("SELECT COUNT(*) FROM " . $this->sanmu->table('user_gift_gard') . " WHERE status = '1' AND gift_gard_id='$id'")) {
                    $sql = "UPDATE " . $this->sanmu->table('user_gift_gard') . " SET status='2' WHERE gift_gard_id='$id'";

                    $this->db->query($sql);
                    $val = 2;
                } elseif ($this->db->getOne("SELECT COUNT(*) FROM " . $this->sanmu->table('user_gift_gard') . " WHERE status = '2' AND gift_gard_id='$id'")) {
                    $sql = "UPDATE " . $this->sanmu->table('user_gift_gard') . " SET status='1' WHERE gift_gard_id='$id'";
                    $this->db->query($sql);
                    $val = 1;
                } else {
        ...
        if ($_REQUEST['act'] == 'remove') {
            ...
            $bonus_arr = $this->db->getRow("SELECT ru_id FROM " . $this->sanmu->table('gift_gard_type') . " WHERE gift_id = '$id'");
            ...
            /* 删除礼品卡 */
            $this->db->query("DELETE FROM " . $this->sanmu->table('user_gift_gard') . " WHERE gift_id = '$id'");
        ...
        if ($_REQUEST['act'] == 'gift_insert') {
            ...
            /* 检查礼品卡是否有重复 */
            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('user_gift_gard') . " WHERE gift_sn='$gift_sn'";
            if ($this->db->getOne($sql) > 0) {
                $link[] = ['text' => $GLOBALS['_LANG']['go_back'], 'href' => 'javascript:history.back(-1)'];
                return sys_msg($GLOBALS['_LANG']['gift_name_exist'], 0, $link);
            }

            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('user_gift_gard') . " WHERE gift_password='$gift_pwd'";
            if ($this->db->getOne($sql) > 0) {
            ...
            /* 插入数据库。 */
            $sql = "INSERT INTO " . $this->sanmu->table('user_gift_gard') . " (gift_sn, gift_password,gift_id)
	VALUES ('$gift_sn',
	'$gift_pwd',
	'$gift_id')";
        ...
        if ($_REQUEST['act'] == 'insert') {
            ...
            /* 检查类型是否有重复 */
            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('gift_gard_type') . " WHERE gift_name='$type_name' AND ru_id = '$adminru[ru_id]'";
            if ($this->db->getOne($sql) > 0) {
                $link[] = ['text' => $GLOBALS['_LANG']['go_back'], 'href' => 'javascript:history.back(-1)'];
            ...
            /* 插入数据库。 */
            $sql = "INSERT INTO " . $this->sanmu->table('gift_gard_type') . " (ru_id, gift_name, gift_menory,gift_start_date,gift_end_date, gift_number)
            ...
            /* 生成红包序列号 */
            $num = $this->db->getOne("SELECT MAX(gift_sn) FROM " . $this->sanmu->table('user_gift_gard'));
            $num = $num ? floor($num / 1) : 8000000;

            for ($i = 1, $j = 0; $i <= $gift_number; $i++) {
                $gift_sn = ($num + $i);
                $gift_pwd = $this->generate_password(6);
                $this->db->query("INSERT INTO " . $this->sanmu->table('user_gift_gard') . " (gift_sn, gift_password, gift_id) VALUES('$gift_sn', '$gift_pwd', '$gift_id')");
        ...
        if ($_REQUEST['act'] == 'edit') {
            ...
            $bonus_arr = $this->db->getRow("SELECT * FROM " . $this->sanmu->table('gift_gard_type') . " WHERE gift_id = '$type_id'");
        ...
        if ($_REQUEST['act'] == 'update') {
            ...
            $this->db->autoExecute($this->sanmu->table('gift_gard_type'), $record, 'UPDATE', "gift_id = '$type_id'");
            ...
            for ($i = 1, $j = 0; $i <= $gift_number; $i++) {
                $gift_sn = ($num + $i);
                $gift_pwd = $this->generate_password(6);
                $this->db->query("INSERT INTO " . $this->sanmu->table('user_gift_gard') . " (gift_sn, gift_password, gift_id) VALUES('$gift_sn', '$gift_pwd', '$type_id')");
        ...
        if ($_REQUEST['act'] == 'send') {
            ...
            /* 查询此红包类型信息 */
            $bonus_type = $this->db->GetRow("SELECT gift_id, gift_name FROM " . $this->sanmu->table('gift_gard_type') .
                " WHERE gift_id='$id'");
            ...
            /* 查询其他红包类型的商品 */
            $sql = "SELECT goods_id FROM " . $this->sanmu->table('goods') .
        ...
        if ($_REQUEST['act'] == 'send_by_user') {
            ...
            if (isset($_REQUEST['send_rank'])) {
                ...
                if ($rank_id > 0) {
//                    $sql = "SELECT min_points, max_points, special_rank FROM " . $this->sanmu->table('user_rank') . " WHERE rank_id = '$rank_id'";
//                    $row = $this->db->getRow($sql);
//                    if ($row['special_rank']) {
                    /* 特殊会员组处理 */
                    $sql = 'SELECT COUNT(*) FROM ' . $this->sanmu->table('users') . " WHERE user_rank = '$rank_id'";
                    $send_count = $this->db->getOne($sql);
                    if ($validated_email) {
                        $sql = 'SELECT user_id, email, user_name FROM ' . $this->sanmu->table('users') .
...
//                    } else {
//                        $sql = 'SELECT COUNT(*) FROM ' . $this->sanmu->table('users') .
...
//                    }
            ...
            foreach ($user_list as $key => $val) {
                ...
                if ($this->add_to_maillist($val['user_name'], $val['email'], $tpl['template_subject'], $content, $tpl['is_html'])) {
                    /* 向会员红包表录入数据 */
                    $sql = "INSERT INTO " . $this->sanmu->table('user_bonus') .
        ...
        if ($_REQUEST['act'] == 'send_by_print') {
            ...
            /* 生成红包序列号 */
            $num = $this->db->getOne("SELECT MAX(bonus_sn) FROM " . $this->sanmu->table('user_bonus'));
            $num = $num ? floor($num / 10000) : 100000;

            for ($i = 0, $j = 0; $i < $bonus_sum; $i++) {
                $bonus_sn = ($num + $i) . str_pad(mt_rand(0, 9999), 4, '0', STR_PAD_LEFT);
                $this->db->query("INSERT INTO " . $this->sanmu->table('user_bonus') . " (bonus_type_id, bonus_sn) VALUES('$bonus_typeid', '$bonus_sn')");
        ...
        if ($_REQUEST['act'] == 'gen_excel') {
            ...
            $type_name = $this->db->getOne("SELECT type_name FROM " . $this->sanmu->table('bonus_type') . " WHERE type_id = '$tid'");

            /* 文件名称 */
            $bonus_filename = $type_name . '_bonus_list';
            if (EC_CHARSET != 'gbk') {
                $bonus_filename = sanmu_iconv('UTF8', 'GB2312', $bonus_filename);
            }
            ...
            /* 文件标题 */
            if (EC_CHARSET != 'gbk') {
                echo sanmu_iconv('UTF8', 'GB2312', $GLOBALS['_LANG']['bonus_excel_file']) . "\t\n";
                /* 红包序列号, 红包金额, 类型名称(红包名称), 使用结束日期 */
                echo sanmu_iconv('UTF8', 'GB2312', $GLOBALS['_LANG']['record_id']) . "\t";
                echo sanmu_iconv('UTF8', 'GB2312', $GLOBALS['_LANG']['bonus_sn']) . "\t";
                echo sanmu_iconv('UTF8', 'GB2312', $GLOBALS['_LANG']['address']) . "\t";
                echo sanmu_iconv('UTF8', 'GB2312', $GLOBALS['_LANG']['consignee_name']) . "\t";
                echo sanmu_iconv('UTF8', 'GB2312', $GLOBALS['_LANG']['mobile']) . "\t";
                echo sanmu_iconv('UTF8', 'GB2312', $GLOBALS['_LANG']['gift_user_name']) . "\t";
                echo sanmu_iconv('UTF8', 'GB2312', $GLOBALS['_LANG']['gift_goods_name']) . "\t";
                echo sanmu_iconv('UTF8', 'GB2312', $GLOBALS['_LANG']['gift_user_time']) . "\t";
                echo sanmu_iconv('UTF8', 'GB2312', $GLOBALS['_LANG']['confirm_ship']) . "\t";
                echo sanmu_iconv('UTF8', 'GB2312', $GLOBALS['_LANG']['express_no']) . "\t";
                echo sanmu_iconv('UTF8', 'GB2312', $GLOBALS['_LANG']['status']) . "\t\n";
            } else {
            ...
            $sql = "SELECT ub.bonus_id, ub.bonus_type_id, ub.bonus_sn, bt.type_name, bt.type_money, bt.use_end_date " .
                "FROM " . $this->sanmu->table('user_bonus') . " AS ub, " . $this->sanmu->table('bonus_type') . " AS bt " .
            ...
            foreach ($res as $val) {
                ...
                if (!isset($code_table[$val['type_name']])) {
                    if (EC_CHARSET != 'gbk') {
                        $code_table[$val['type_name']] = sanmu_iconv('UTF8', 'GB2312', $val['type_name']);
                    } else {
        ...
        if ($_REQUEST['act'] == 'add_bonus_goods') {
            ...
            $add_ids = sanmu_decode($_GET['add_ids'], true);
            $args = sanmu_decode($_GET['JSON'], true);

            $type_id = explode(',', trim($args));
            foreach ($type_id as $key => $val) {
                $add_ids_str = '';
                $add_ids_str = implode(',', $add_ids);
                $sql = "SELECT config_goods_id FROM " . $this->sanmu->table('user_gift_gard') . " WHERE gift_gard_id='$val'";
                $config_goods = $this->db->getRow($sql);
                if ($config_goods['config_goods_id'] && $add_ids_str) {
                    $add_ids_str .= ',' . $config_goods['config_goods_id'];
                }
                $sql = "UPDATE " . $this->sanmu->table('user_gift_gard') . " SET config_goods_id='$add_ids_str' WHERE gift_gard_id='$val'";
        ...
        if ($_REQUEST['act'] == 'drop_bonus_goods') {
            ...
            $drop_ids = sanmu_decode($_GET['drop_ids'], true);
            $args = sanmu_decode($_GET['JSON'], true);
            ...
            foreach ($type_id as $key => $val) {
                ...
                $this->db->query("UPDATE " . $this->sanmu->table('user_gift_gard') . " SET config_goods_id='$config_goods_id' WHERE gift_gard_id='$val'");
            }
        ...
        if ($_REQUEST['act'] == 'take_excel') {
            ...
            /* 文件标题 */
            echo sanmu_iconv(EC_CHARSET, $file_name) . "\t\n";

            /* 商品名称,订单号,商品数量,销售价格,销售日期 */
            echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['record_id']) . "\t";
            echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['gift_card_sn']) . "\t";
            echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['address']) . "\t";
            echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['consignee_name']) . "\t";
            echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['mobile']) . "\t";
            echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['gift_user_name']) . "\t";
            echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['gift_goods_name']) . "\t";
            echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['gift_user_time']) . "\t";
            echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['express_no']) . "\t";
            echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['status']) . "\t\n";

            foreach ($gift_gard_list['item'] as $key => $value) {
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $value['gift_gard_id']) . "\t";
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $value['gift_sn']) . "\t";
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $value['address']) . "\t";
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $value['consignee_name']) . "\t";
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $value['mobile']) . "\t";
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $value['user_name']) . "\t";
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $value['goods_name']) . "\t";
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $value['user_time']) . "\t";
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $value['express_no']) . "\t";
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['transaction_tip_4']) . "\t";
                echo "\n";
            }
        }
        ...
        if ($_REQUEST['act'] == 'export_gift_gard') {
            ...
            /* 商品名称,订单号,商品数量,销售价格,销售日期 */
            echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['record_id']) . "\t";
            echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['card_number']) . "\t";
            echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['password']) . "\t";
            echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['money']) . "\t";
            echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['card_name']) . "\t\n";

            foreach ($gift_list['item'] as $key => $value) {
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $value['gift_gard_id']) . "\t";
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $value['gift_sn']) . "\t";
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $value['gift_password']) . "\t";
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $value['gift_menory']) . "\t";
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $value['gift_name']) . "\t";
                echo "\n";
            }
        }
        ...
        elseif ($_REQUEST['act'] == 'remove_bonus') {
            ...
            $sql = "SELECT  COUNT(*) FROM " . $this->sanmu->table('user_gift_gard') . " WHERE gift_gard_id='$id' AND user_id > 0";
            $num = $this->db->getOne($sql);

            if ($num > 0) {
                $this->db->query("UPDATE " . $this->sanmu->table('user_gift_gard') . "SET is_delete = 0  WHERE gift_gard_id='$id'");
            } else {
                $this->db->query("DELETE FROM " . $this->sanmu->table('user_gift_gard') . " WHERE gift_gard_id='$id'");
            }
        ...
        elseif ($_REQUEST['act'] == 'remove_take') {
            ...
            $this->db->query("UPDATE " . $this->sanmu->table('user_gift_gard') . " SET goods_id=0, user_id=0, address='', consignee_name='', mobile='', status=0, express_no='' WHERE gift_gard_id='$id'");

            $this->db->query("DELETE FROM " . $this->sanmu->table('gift_gard_log') . " WHERE gift_gard_id  = '$id' AND handle_type='gift_gard'");//删除操作记录  bu kong
            ...
            if (isset($_POST['checkboxes'])) {
                ...
                if (isset($_POST['drop'])) {
                    foreach ($bonus_id_list as $key => $val) {
                        $sql = "SELECT  COUNT(*) FROM " . $this->sanmu->table('user_gift_gard') . " WHERE gift_gard_id='$val' AND user_id > 0";
                        $num = $this->db->getOne($sql);
                        if ($num > 0) {
                            $this->db->query("UPDATE " . $this->sanmu->table('user_gift_gard') . "SET is_delete = 0  WHERE gift_gard_id='$val'");
                        } else {
                            $this->db->query("DELETE FROM " . $this->sanmu->table('user_gift_gard') . " WHERE gift_gard_id='$val'");
                        }
                    }
    ...
    private function get_type_list($ru_id)
    {
        ...
        $sql = "SELECT gift_id, COUNT(*) AS sent_count" .
            " FROM " . $this->sanmu->table('gift_gard_type') .
            " GROUP BY gift_id";
        $res = $this->db->query($sql);
        ...
        if ($result === false) {
            ...
            if ($filter['store_search'] > -1) {
                if ($ru_id == 0) {
                    if ($filter['store_search'] > 0) {
                        ...
                        if ($filter['store_search'] > 1) {
                            $where .= " AND (SELECT msi.user_id FROM " . $this->sanmu->table('merchants_shop_information') . ' as msi ' .
            ...
            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('gift_gard_type') . " AS ggt" . $where;
            $filter['record_count'] = $this->db->getOne($sql);

            /* 分页大小 */
            $filter = page_and_size($filter);

            $sql = "SELECT * FROM " . $this->sanmu->table('gift_gard_type') . " AS ggt" . " $where ORDER BY $filter[sort_by] $filter[sort_order]";
        ...
        foreach ($res as $row) {
            ...
            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('user_gift_gard') . " WHERE gift_id=$row[gift_id]";
    ...
    {
        $type_arr = $this->db->getRow("SELECT config_goods_id FROM " . $this->sanmu->table('user_gift_gard') . " WHERE gift_gard_id='$type_id'");

        $row = [];
        if ($type_arr['config_goods_id']) {
            $sql = "SELECT goods_id, goods_name FROM " . $this->sanmu->table('goods') .
    ...
    private function get_bonus_list($ru_id)
    {
        ...
        $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('user_gift_gard') . " AS ub" .
            " LEFT JOIN " . $this->sanmu->table('gift_gard_type') . " AS bt ON ub.gift_id = bt.gift_id" .
        ...
        $sql = "SELECT ub.*, bt.ru_id, u.user_name, u.email, o.goods_name, bt.gift_name, bt.gift_menory " .
            " FROM " . $this->sanmu->table('user_gift_gard') . " AS ub " .
            " LEFT JOIN " . $this->sanmu->table('gift_gard_type') . " AS bt ON bt.gift_id=ub.gift_id " .
            " LEFT JOIN " . $this->sanmu->table('users') . " AS u ON u.user_id=ub.user_id " .
            " LEFT JOIN " . $this->sanmu->table('goods') . " AS o ON o.goods_id=ub.goods_id $where $delete_where2 $srarch_where2 " .
    ...
    {
        $sql = "SELECT * FROM " . $this->sanmu->table('gift_gard_type') .
    ...
    private function send_bonus_mail($bonus_type_id, $bonus_id_list)
    {
        ...
        $sql = "SELECT b.bonus_id, u.user_name, u.email " .
            "FROM " . $this->sanmu->table('user_bonus') . " AS b, " .
            $this->sanmu->table('users') . " AS u " .
    ...
    private function add_to_maillist($username, $email, $subject, $content, $is_html)
    {
        ...
        $template_id = $this->db->getOne("SELECT template_id FROM " . $this->sanmu->table('mail_templates') . " WHERE template_code = 'send_bonus'");
        $sql = "INSERT INTO " . $this->sanmu->table('email_sendlist') . " ( email, template_id, email_content, pri, last_send) VALUES ('$email', $template_id, '$content', 1, '$time')";
        $this->db->query($sql);
        return true;
    }
    ...
    private function generate_password($length = 6)
    {
        ...
        if ($this->db->getOne("SELECT * FROM " . $this->sanmu->table('user_gift_gard') . " WHERE gift_password='$password'")) {
    ...
    private function get_admin_gift_gard_log($id = 0)
    {
        ...
        if ($result === false) {
            ...
            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('gift_gard_log') . " WHERE gift_gard_id = '" . $filter['id'] . "' AND handle_type='gift_gard'";
            $filter['record_count'] = $this->db->getOne($sql);
            $filter = page_and_size($filter);
            $sql = "SELECT a.id,a.addtime,b.user_name,a.delivery_status,a.gift_gard_id FROM" . $this->sanmu->table('gift_gard_log') . " AS a LEFT JOIN " . $this->sanmu->table('admin_user') . " AS b ON a.admin_id = b.user_id WHERE a.gift_gard_id = '" . $filter['id'] . "' AND a.handle_type='gift_gard'  ORDER BY a.addtime DESC LIMIT " . $filter['start'] . "," . $filter['page_size'];
            set_filter($filter, $sql);
        } else {
            $sql = $result['sql'];
            $filter = $result['filter'];
        ...
        foreach ($row as $k => $v) {
            ...
            if ($v['gift_gard_id']) {
                $row[$k]['gift_sn'] = $this->db->getOne(" SELECT gift_sn FROM " . $this->sanmu->table('user_gift_gard') . " WHERE gift_gard_id = '" . $v['gift_gard_id'] . "'");
            }
        }
