...
class IndexController extends InitController
{
    ...
    public function index()
    {
        ...
        if ($_REQUEST['act'] == '') {
            ...
            $ru_id = $this->db->getOne("SELECT ru_id FROM " . $this->sanmu->table('admin_user') . " WHERE user_id='$user_id'");

            //上架、删除、下架、 库存预警 的商品, 包含虚拟商品;
            $seller_goods_info['is_sell'] = $this->db->getOne("SELECT COUNT(*) FROM " . $this->sanmu->table('goods') . " WHERE user_id ='$ru_id' AND is_on_sale = 1 AND is_delete = 0");
            $seller_goods_info['is_delete'] = $this->db->getOne("SELECT COUNT(*) FROM " . $this->sanmu->table('goods') . " WHERE user_id ='$ru_id' AND is_delete = 1");
            $seller_goods_info['is_on_sale'] = $this->db->getOne("SELECT COUNT(*) FROM " . $this->sanmu->table('goods') . " WHERE user_id ='$ru_id' AND is_on_sale = 0 AND is_delete = 0");
            $seller_goods_info['is_warn'] = $this->db->getOne("SELECT COUNT(*) FROM " . $this->sanmu->table('goods') . " WHERE user_id ='$ru_id' AND goods_number <= warn_number AND is_delete = 0");

            //总发布商品数;
            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('goods') . " WHERE user_id ='$ru_id'";
            ...
            /* 已完成的订单 */
            $order['finished'] = $this->db->getOne('SELECT count(*) FROM ' . $this->sanmu->table('order_info') . " as oi " .
                " WHERE 1 AND oi.shipping_status = 2 " . $where_og);
            $status['finished'] = CS_FINISHED;

            /* 待发货的订单： */
            $order['await_ship'] = $this->db->getOne('SELECT count(*) FROM ' . $this->sanmu->table('order_info') . " as oi " .
                " WHERE 1  AND (oi.shipping_status = '" . SS_UNSHIPPED . "' OR oi.shipping_status = '" . SS_PREPARING . "') AND oi.pay_status = '" . PS_PAYED . "' AND oi.order_status = '" . OS_CONFIRMED . "' AND (SELECT ore.ret_id FROM " . $this->sanmu->table('order_return') . " AS ore WHERE ore.order_id = oi.order_id LIMIT 1) IS NULL  " . $where_og);
            $status['await_ship'] = CS_AWAIT_SHIP;

            /* 待付款的订单： */
            $order['await_pay'] = $this->db->getOne('SELECT count(*) FROM ' . $this->sanmu->table('order_info') . " as oi " .
                " WHERE 1 AND oi.pay_status = 0 AND oi.order_status = 1 " . $where_og);
            $status['await_pay'] = CS_AWAIT_PAY;

            /* “未确认”的订单 */
            $order['unconfirmed'] = $this->db->getOne('SELECT count(*) FROM ' . $this->sanmu->table('order_info') . " as oi " .
                " WHERE 1 AND oi.order_status = 0 " . $where_og);
            $status['unconfirmed'] = OS_UNCONFIRMED;

            /* “交易中的”的订单(配送方式非"已收货"的所有订单) */
            $order['shipped_deal'] = $this->db->getOne('SELECT count(*) FROM ' . $this->sanmu->table('order_info') . " as oi " .
                " WHERE  shipping_status<>" . SS_RECEIVED . $where_og);
            $status['shipped_deal'] = SS_RECEIVED;

            /* “部分发货”的订单 */
            $order['shipped_part'] = $this->db->getOne('SELECT count(*) FROM ' . $this->sanmu->table('order_info') . " as oi " .
            ...
            $order['stats'] = $this->db->getRow('SELECT COUNT(*) AS oCount, IFNULL(SUM(oi.order_amount), 0) AS oAmount' .
                ' FROM ' . $this->sanmu->table('order_info') . " as oi" . " where 1 " . $where_og);

            //待评价订单
            $signNum0 = $this->get_order_no_comment($ru_id, 0);
            $this->smarty->assign('no_comment', $signNum0);
            //订单纠纷
            $sql = "SELECT COUNT(*) FROM" . $this->sanmu->table('complaint') . "WHERE complaint_state > 0 AND ru_id = '$ru_id'";
            ...
            $where_bg = '';
            if ($ru_id > 0) {
                $leftJoin_bg = " left join " . $this->sanmu->table('goods') . " as g on bg.goods_id = g.goods_id ";
                $where_bg = " and g.user_id = " . $ru_id;
            }
            //ecmoban模板堂 --zhuo end
            $sql = 'SELECT COUNT(*) FROM ' . $this->sanmu->table('booking_goods') . "as bg " .
                $leftJoin_bg .
            ...
            /* 退款申请 */
            $this->smarty->assign('new_repay', $this->db->getOne('SELECT COUNT(*) FROM ' . $this->sanmu->table('user_account') . ' WHERE process_type = ' . SURPLUS_RETURN . ' AND is_paid = 0 '));
            ...
            /* 商家帮助 */
            $sql = "SELECT * FROM " . $this->sanmu->table('article') . "WHERE cat_id = '" . $GLOBALS['_CFG']['seller_index_article'] . "' ";
            ...
            /* 单品销售数量排名(已付款的才算数) */
            $sql = "SELECT goods_id ,goods_name,sales_volume AS goods_shipping_total FROM" . $this->sanmu->table('goods') .
            ...
            $this->smarty->assign('shop_url', urlencode($this->sanmu->seller_url()));
        ...
        elseif ($_REQUEST['act'] == 'merchants_first') {
            ...
            $sql = "select notice from " . $this->sanmu->table('seller_shopinfo') . " where ru_id = 0 LIMIT 1";
            $seller_notice = $this->db->getOne($sql);
            $this->smarty->assign('seller_notice', $seller_notice);

            $host = $this->sanmuRepository->hostDomain();
            $this->smarty->assign('host', $host);

            //获取入驻商家店铺信息 wang 商家入驻
            $sql = "select ss.*,sq.* from " . $this->sanmu->table('seller_shopinfo') . " as ss " .
                " left join " . $this->sanmu->table('seller_qrcode') . " as sq on sq.ru_id = ss.ru_id " .
            ...
            //获取店铺二级域名 by kong
            $domain_name = $this->db->getOne(" SELECT domain_name FROM" . $this->sanmu->table("seller_domain") . " WHERE ru_id='" . $adminru['ru_id'] . "'");
            ...
            $adminru['ru_id'] == 0 ? $shop_information['is_sanmu'] = true : $shop_information['is_sanmu'] = false;//判断当前商家是平台,还是入驻商家 bylu
            ...
            $this->smarty->assign('http', $this->sanmu->http());
        ...
        elseif ($_REQUEST['act'] == 'merchants_second') {
            ...
            if (!empty($domain_name)) {
                $sql = " SELECT count(id) FROM " . $this->sanmu->table("seller_domain") . " WHERE domain_name = '" . $domain_name . "' AND ru_id !='" . $adminru['ru_id'] . "'";
            ...
            $sql = "SELECT ss.shop_logo, ss.logo_thumb, ss.street_thumb, ss.brand_thumb, sq.qrcode_thumb FROM " . $this->sanmu->table('seller_shopinfo') . " as ss " .
                " left join " . $this->sanmu->table('seller_qrcode') . " as sq on sq.ru_id=ss.ru_id " .
            ...
            $domain_id = $this->db->getOne("SELECT id FROM " . $this->sanmu->table('seller_domain') . " WHERE ru_id ='" . $adminru['ru_id'] . "'"); //by kong
            /* 二级域名绑定  by kong  satrt */
            if ($domain_id > 0) {
                $this->db->autoExecute($this->sanmu->table('seller_domain'), $seller_domain, 'UPDATE', "ru_id='" . $adminru['ru_id'] . "'");
            } else {
                $this->db->autoExecute($this->sanmu->table('seller_domain'), $seller_domain, 'INSERT');
            }
            /* 二级域名绑定  by kong  end */
                        ...
                        if (move_upload_file($file['tmp_name'], $file_name)) {
                            ...
                            if (!empty($qrcode_thumb)) {
                                ...
                                if (isset($store['qrcode_thumb']) && $store['qrcode_thumb']) {
                                    ...
                                    sanmu_unlink(storage_public($store['qrcode_thumb']));
                                }
                            }
            ...
            $this->sanmuRepository->getOssAddFile($oss_img);

            $admin_user = [
                'email' => $seller_email
            ];

            $this->db->autoExecute($this->sanmu->table('admin_user'), $admin_user, 'UPDATE', "user_id = '" . session('seller_id') . "'");

            if ($data_op == 'add') {
                if (!$store) {
                    $this->db->autoExecute($this->sanmu->table('seller_shopinfo'), ['ru_id' => $adminru['ru_id']], 'INSERT');
                    //处理修改数据 by wu start
                    $db_data = []; //数据库中数据
                    ...
                    if (!empty($diff_data)) { //有数据变化
                        ...
                        foreach ($diff_data as $key => $val) {
                            ...
                            $sql = "SELECT id FROM" . $this->sanmu->table('seller_shopinfo_changelog') . "WHERE data_key = '$key' AND ru_id = '" . $adminru['ru_id'] . "'";
                            if ($this->db->getOne($sql)) {
                                $this->db->autoExecute($this->sanmu->table('seller_shopinfo_changelog'), $changelog, 'update', "ru_id='" . $adminru['ru_id'] . "' AND data_key = '$key'");
                            } else {
                                $this->db->autoExecute($this->sanmu->table('seller_shopinfo_changelog'), $changelog, 'INSERT');
                            }
                        }
                    }
                    //处理修改数据 by wu end
                ...
                $sql = "select check_sellername from " . $this->sanmu->table('seller_shopinfo') . " where ru_id='" . $adminru['ru_id'] . "'";
                $seller_shop_info = $this->db->getRow($sql);
                ...
                if (isset($shop_info['logo_thumb']) && !empty($shop_info['logo_thumb'])) {
                    ...
                    sanmu_unlink(storage_public($store['logo_thumb']));
                }
                ...
                if (!empty($street_thumb)) {
                    ...
                    $shop_info['street_thumb'] = $street_thumb;
                    sanmu_unlink(storage_public($oss_street_thumb));
                }
                ...
                if (!empty($brand_thumb)) {
                    ...
                    $shop_info['brand_thumb'] = $brand_thumb;
                    sanmu_unlink(storage_public($oss_brand_thumb));
                }

                $this->sanmuRepository->getOssDelFile($oss_del);
                ...
                if ($diff_data_old) {
                    foreach ($diff_data_old as $key => $val) {
                        if ($key != 'shop_logo' && isset($oss_img[$key]) && !empty($oss_img[$key])) {
                            $val = str_replace(['../'], '', $val);
                            sanmu_unlink(storage_public($val));
                        }
                    }
                }
                ...
                if (!empty($diff_data)) { //有数据变化
                    ...
                    $this->db->autoExecute($this->sanmu->table('seller_shopinfo'), $review_status, 'UPDATE', "ru_id='" . $adminru['ru_id'] . "'");
                    //将修改数据插入日志
                    foreach ($diff_data as $key => $val) {
                        $changelog = ['data_key' => $key, 'data_value' => $val, 'ru_id' => $adminru['ru_id']];
                        $sql = "SELECT id FROM" . $this->sanmu->table('seller_shopinfo_changelog') . " WHERE data_key = '$key' AND ru_id = '" . $adminru['ru_id'] . "'";
        ...
        elseif ($_REQUEST['act'] == 'shop_top') {
            ...
            if ($seller_shop_info['id'] > 0) {
                //店铺头部
                $header_sql = "select content, headtype, headbg_img, shop_color from " . $this->sanmu->table('seller_shopheader') . " where seller_theme='" . $seller_shop_info['seller_theme'] . "' and ru_id = '" . $adminru['ru_id'] . "'";
                    ...
                    if ((isset($file['error']) && $file['error'] == 0) || (!isset($file['error']) && $file['tmp_name'] != 'none')) {
                        ...
                        if (!check_file_type($file['tmp_name'], $file['name'], $allow_file_types)) {
                            ...
                            if (move_upload_file($file['tmp_name'], $file_name)) {
                                ...
                                $this->sanmuRepository->getOssAddFile([$oss_img_url]);
                            } else {
            ...
            $sql = "SELECT headbg_img FROM " . $this->sanmu->table('seller_shopheader') . " WHERE ru_id='" . $adminru['ru_id'] . "' and seller_theme='" . $seller_theme . "'";
            ...
            //跟新店铺头部
            $sql = "update " . $this->sanmu->table('seller_shopheader') . " set content='$shop_header', shop_color='$shop_color', headbg_img='$img_url', headtype='$headtype' where ru_id='" . $adminru['ru_id'] . "' and seller_theme='" . $seller_theme . "'";
            $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'check_bill') {
            ...
            if ($data === false) {
                ...
                $scount = $this->db->getOne('SELECT COUNT(*) FROM ' . $this->sanmu->table('seller_shopinfo')); //会员数量
                $sql = 'SELECT COUNT(*) AS oCount, ' . "SUM(" . $this->orderService->orderAmountField('o.') . ") AS oAmount" . ' FROM ' . $this->sanmu->table('order_info') . " AS o WHERE o.main_count = 0 LIMIT 1";
                $order['stats'] = $this->db->getRow($sql);

                $ocount = $order['stats']['oCount']; //订单数量
                $oamount = $order['stats']['oAmount']; //总销售金额

                $goods['total'] = $this->db->GetOne('SELECT COUNT(*) FROM ' . $this->sanmu->table('goods') .
                    ' WHERE is_delete = 0 AND is_alone_sale = 1 AND is_real = 1');
                $gcount = $goods['total']; //商品数量
                $ecs_user = $this->db->getOne('SELECT COUNT(*) FROM ' . $this->sanmu->table('users')); //会员数量

                $ecs_template = $this->db->getOne('SELECT value FROM ' . $this->sanmu->table('shop_config') . ' WHERE code = \'template\''); //当前使用模板
                $style = $this->db->getOne('SELECT value FROM ' . $this->sanmu->table('shop_config') . ' WHERE code = \'stylename\'');  //当前模板样式
                if ($style == '') {
                    $style = '0';
                }
                $ecs_style = $style;
                $shop_url = urlencode($this->sanmu->url()); //当前url
        ...
        elseif ($_REQUEST['act'] == 'change_user_menu') {
            ...
            if (!$change) {
                $user_menu[] = $action;
                $sql = " UPDATE " . $this->sanmu->table('seller_shopinfo') . " set user_menu = '" . implode(',', $user_menu) . "' WHERE ru_id = '" . $adminru['ru_id'] . "' ";
        ...
        elseif ($_REQUEST['act'] == 'clear_cache') {
            $sql = "UPDATE " . $this->sanmu->table('shop_config') . " SET value = 0 WHERE code = 'is_downconfig'";
            $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'upload_store_img') {
            ...
            if ($_FILES['img']['name']) {
                ...
                $this->sanmuRepository->getOssAddFile([$img_name]);

                if ($img_name) {
                    $result['error'] = 1;
                    $result['content'] = get_image_path($img_name);
                    //删除原图片
                    $store_user_img = $this->db->getOne(" SELECT admin_user_img FROM " . $this->sanmu->table('admin_user') . " WHERE user_id = '" . $admin_id . "' ");
                    @unlink(storage_public($store_user_img));
                    //插入新图片
                    $sql = " UPDATE " . $this->sanmu->table('admin_user') . " SET admin_user_img = '{$result['content']}' WHERE user_id = '" . $admin_id . "' ";
                    $this->db->query($sql);
                }
            }
            return response()->json($result);
    ...
    private function get_sales($day_num, $ru_id = 0)
    {
        ...
        $sql = "SELECT IFNULL(SUM(" . $this->orderService->orderAmountField('o.') . "),0) AS 'ga', COUNT(o.order_id) AS 'oi' " .
            ...
            " LEFT JOIN " . $this->sanmu->table('baitiao_log') . " AS bai ON o.order_id=bai.order_id WHERE o.add_time BETWEEN " . $date_start . ' AND ' . $date_end . " AND o.referer NOT IN('touch', 'mobile')" .
    ...
    private function get_move_sales($day_num, $ru_id = 0)
    {
        ...
        $sql = " SELECT IFNULL(SUM(" . $this->orderService->orderAmountField('o.') . "),0) AS 'ga', COUNT(o.order_id) AS 'oi'" .
            " FROM " . $this->sanmu->table('order_info') . " AS o " .
            "LEFT JOIN " . $this->sanmu->table('baitiao_log') . " AS bai ON o.order_id=bai.order_id WHERE o.add_time BETWEEN " . $date_start . ' AND ' . $date_end . " AND o.referer IN('touch', 'mobile')" .
            " AND o.ru_id = '$ru_id'" . $where . " LIMIT 1";
        $row = $this->db->getRow($sql);
    ...
    private function viewip($ru_id)
    {
        ...
        $sql = "SELECT COUNT(i.ipid) AS ip " . " FROM " . $this->sanmu->table('source_ip') . " AS i " .
            "LEFT JOIN " . $this->sanmu->table('seller_shopinfo') . "AS s ON i.storeid = s.ru_id " .
    ...
    */
    private function query_sales($ru_id = 0, $where = '')
    {
        $sql = " SELECT COUNT(oi.order_id) order_total,IFNULL(SUM(" . $this->orderService->orderAmountField('oi.') . "),0) money_total FROM " .
            $this->sanmu->table('order_info') . "oi " .
    ...
    */
    private function surplus_time($ru_id)
    {
        if (session()->has('verify_time') && session('verify_time')) {
            $sql = " SELECT ru_id, grade_id, add_time, year_num FROM " . $this->sanmu->table('merchants_grade') . " WHERE ru_id = '$ru_id' " .
            ...
            if ($enter_overtime <= $time) {//审核过期
                $sql = " UPDATE " . $this->sanmu->table('merchants_shop_information') . " SET merchants_audit = 0 WHERE user_id = '$ru_id' ";
                $this->db->query($sql);
