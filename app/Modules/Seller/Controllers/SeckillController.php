...
class SeckillController extends InitController
{
    ...
    public function index()
    {
        ...
        /* 初始化$exc对象 */
        $exc = new Exchange($this->sanmu->table('seckill'), $this->db, 'sec_id', 'acti_title');
        $exc_sg = new Exchange($this->sanmu->table('seckill_goods'), $this->db, 'id', 'sec_id');
        ...
        elseif ($_REQUEST['act'] == 'remove') {
            ...
            if ($sec_id) {
                $res = $exc->drop($sec_id);
                if ($res) {
                    $this->db->query(" DELETE FROM " . $this->sanmu->table('seckill_goods') . " WHERE sec_id='$sec_id' ");
                }
            }
        ...
        elseif ($_REQUEST['act'] == 'sg_remove') {
            ...
            $sql = " SELECT sec_id, tb_id FROM " . $this->sanmu->table("seckill_goods") . " WHERE id = '$id' ";
        ...
        elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
            ...
            if ($_REQUEST['act'] == 'insert') {
                ...
                /* 插入数据库。 */
                $sql = "INSERT INTO " . $this->sanmu->table('seckill') . " (acti_title, begin_time, acti_time, is_putaway, add_time, ru_id, review_status)
		VALUES ('$acti_title', '$begin_time', '$acti_time', '$is_putaway', '$add_time', '$ru_id', $review_status)";
                ...
                /* 修改入库。 */
                $sql = "UPDATE " . $this->sanmu->table('seckill') . " SET " .
        ...
        elseif ($_REQUEST['act'] == 'toggle_putaway') {
            ...
            /* 修改 */
            $sql = "UPDATE " . $this->sanmu->table('seckill') . " SET is_putaway = '$val' WHERE sec_id = '$id'";
    ...
    */
    private function get_seckill_list($ru_id)
    {
        ...
        if ($result === false) {
            ...
            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('seckill') . $where;
    ...
    */
    ...
    {
        $sql = " SELECT sec_id, begin_time, acti_title, acti_time, is_putaway, ru_id, review_status FROM " . $this->sanmu->table('seckill') .
    ...
    {
        $sql = " SELECT * FROM " . $this->sanmu->table('seckill_time_bucket');
