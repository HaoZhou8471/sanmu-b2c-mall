...
function handle_goods_area($goods_id)
{
    $sql = "UPDATE " . $GLOBALS['sanmu']->table('link_area_goods') . " SET " .
        " goods_id = '$goods_id' " .
        " WHERE goods_id = '0'" .
        " AND ru_id = (SELECT ru_id FROM " . $GLOBALS['sanmu']->table('admin_user') . "  WHERE user_id = '" . session('store_user_id') . "') ";
    $GLOBALS['db']->query($sql);
}
...
function goods_list($is_delete, $real_goods = 1, $conditions = '')
{
    ...
    if ($result === false) {
        ...
        if ($sel_mode == 1 && !empty($brand_keyword)) {
            if ($adminru['ru_id'] > 0) {
                /* 商家品牌 */
                $filter['brand_id'] = $GLOBALS['db']->getALL("SELECT bid FROM " . $GLOBALS['sanmu']->table('merchants_shop_brand') . " WHERE brandName LIKE '%$brand_keyword%' ");
            } else {
                if ($filter['sel_brand'] == 2) {
                    ...
                    /* 自营品牌 */
                    $filter['brand_id'] = $GLOBALS['db']->getAll("SELECT brand_id FROM " . $GLOBALS['sanmu']->table('brand') . " WHERE brand_name LIKE '%$brand_keyword%' ");
                }
            }
        ...
        $sql = "SELECT g.goods_id, g.goods_name, g.model_price, g.model_inventory, g.user_id, g.goods_type, g.goods_sn, g.shop_price, g.is_on_sale, g.is_best, g.is_new, g.is_hot, g.sort_order, g.goods_number, g.integral, " .
            ...
            " left join " . $GLOBALS['sanmu']->table('goods_cat') . " as gc on g.goods_id = gc.goods_id" . " WHERE g.is_delete='$is_delete' $where" . " group by g.goods_id";
    ...
    for ($i = 0; $i < $count; $i++) {
        ...
        if (!empty($row[$i]['url']) && (strpos($row[$i]['url'], 'http://') === false && strpos($row[$i]['url'], 'https://') === false && strpos($row[$i]['url'], 'errorImg.png') === false)) {
            $row[$i]['url'] = $GLOBALS['sanmu']->stores_url() . $row[$i]['url'];
        }
    }
...
{
    //获得商品的扩展信息by wang
    $extend_sql = "SELECT `is_reality`,`is_return`,`is_fast` FROM " . $GLOBALS['sanmu']->table('goods_extend') . " WHERE goods_id='$goods_id'";
...
function product_number_count($goods_id, $conditions = '', $warehouse_id = 0)
{
    ...
    $sql = "SELECT product_number
            FROM " . $GLOBALS['sanmu']->table('store_products') . "
            WHERE goods_id = '$goods_id'
            " . $conditions;
...
function product_list($goods_id, $conditions = '')
{
    ...
    if ($result === false) {
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['sanmu']->table('store_products') . " AS p WHERE goods_id = $goods_id $where";
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);

        $sql = "SELECT product_id, goods_id, goods_attr, product_sn, product_number
                FROM " . $GLOBALS['sanmu']->table('store_products') . " AS g
...
function get_product_info($product_id, $filed = '')
{
    ...
    $sql = "SELECT $filed FROM  " . $GLOBALS['sanmu']->table('store_products') . " WHERE product_id = '$product_id'";
...
function check_goods_attr_exist($goods_attr, $goods_id, $product_id = 0)
{
    ...
    if (empty($product_id)) {
        $sql = "SELECT product_id FROM " . $GLOBALS['sanmu']->table('store_products') . "
...
function check_product_sn_exist($product_sn, $product_id = 0, $ru_id = 0, $store_id = 0)
{
    ...
    $sql = "SELECT goods_id FROM " . $GLOBALS['sanmu']->table('goods') . "WHERE goods_sn='$product_sn' AND user_id = '$ru_id'";
