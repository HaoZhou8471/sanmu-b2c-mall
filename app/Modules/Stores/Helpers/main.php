...
function get_position_list()
{
    ...
    $sql = 'SELECT position_id, position_name, ad_width, ad_height ' .
        'FROM ' . $GLOBALS['sanmu']->table('ad_position') . $ruCat;
    $res = $GLOBALS['db']->query($sql);
...
function goods_type_list($selected, $goods_id = 0)
{
    ...
    $sql = 'SELECT cat_id, cat_name FROM ' . $GLOBALS['sanmu']->table('goods_type') . ' WHERE enabled = 1' . $ruCat;
    $res = $GLOBALS['db']->query($sql);
...
function get_add_edit_link_desc($linked_array, $type = 0, $id = 0)
{
    if ($linked_array) {
        ...
        $sql = "select goods_id from " . $GLOBALS['sanmu']->table('link_desc_temporary') . " where 1 limit 0, 1";
...
function get_bacth_category($cat_name, $cat, $ru_id)
{
    for ($i = 0; $i < count($cat_name); $i++) {
        if (!empty($cat_name)) {
            ...
            if ($GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table('category'), $cat) !== false) {
                $cat_id = $GLOBALS['db']->insert_id();
                if ($cat['show_in_nav'] == 1) {
                    $vieworder = $GLOBALS['db']->getOne("SELECT max(vieworder) FROM " . $GLOBALS['sanmu']->table('nav') . " WHERE type = 'middle'");
                    $vieworder += 2;
                    //显示在自定义导航栏中
                    $sql = "INSERT INTO " . $GLOBALS['sanmu']->table('nav') .
                        " (name,ctype,cid,ifshow,vieworder,opennew,url,type)" .
                        " VALUES('" . $cat['cat_name'] . "', 'c', '$cat_id','1','$vieworder','0', '" . app(SanmuRepository::class)->buildUri('category', ['cid' => $cat_id], $cat['cat_name']) . "','middle')";
                    $GLOBALS['db']->query($sql);
                }
                ...
                if ($ru_id > 0) {
                    ...
                    $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table('merchants_category'), $parent, 'INSERT');
                }
...
function return_order_list()
{
    ...
    if ($result === false) {
        ...
        if ($filter['user_name']) {
            ...
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['sanmu']->table('order_return') . " AS r, " . $GLOBALS['sanmu']->table('order_info') . " as o " . $where . " AND r.order_id = o.order_id";
        }
        ...
        $sql = "SELECT o.order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid, og.ru_id," .
            ...
            "LEFT JOIN " . $GLOBALS['sanmu']->table('order_goods') . " AS og ON og.order_id=o.order_id " .
    ...
    foreach ($row as $key => $value) {
        ...
        $sql = "select return_number from " . $GLOBALS['sanmu']->table('return_goods') . " where rec_id = '" . $value['rec_id'] . "'";
...
/*function get_merchants_permissions($admin_id = 0, $string = ''){
    $sql = "SELECT action_list FROM " .$GLOBALS['sanmu']->table('admin_user'). " WHERE user_id = '$admin_id'";
...
function get_favourable_dateout_count($ru_id)
{
    ...
    $sql = "SELECT count(*) FROM " . $GLOBALS['sanmu']->table('favourable_activity') . " WHERE user_id = '$ru_id' AND (end_time - 24 * 60 * 60 * 3) < '$firstSecToday'";
    return $GLOBALS['db']->getOne($sql);
}
...
function store_priv($priv_str, $msg_type = '', $msg_output = true)
{
    ...
    $sql = " SELECT store_action FROM " . $GLOBALS['sanmu']->table('store_user') . " WHERE id = '$store_user_id' ";
...
function get_store_ru_id()
{
    ...
    $sql = "SELECT ru_id FROM" . $GLOBALS['sanmu']->table('store_user') . "WHERE id = '$admin_id'";
    return $GLOBALS['db']->getOne($sql);
}
...
function seller_shop_cat($user_id = 0)
{
    $seller_shop_cat = '';
    if ($user_id) {
        $sql = "SELECT user_shopMain_category FROM " . $GLOBALS['sanmu']->table('merchants_shop_information') . " WHERE user_id = '$user_id'";
