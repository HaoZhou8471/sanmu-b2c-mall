...
class OrderController extends InitController
{
    ...
    public function index()
    {
        ...
        $ru_id = $this->db->getOne(" SELECT ru_id FROM " . $this->sanmu->table('offline_store') . " WHERE id = '$store_id' ");
        $this->smarty->assign("app", "order");

        //设置logo
        $sql = "SELECT value FROM " . $this->sanmu->table('shop_config') . " WHERE code = 'stores_logo'";
        ...
        elseif ($_REQUEST['act'] == 'deal_store_order') {
            ...
            if ($operate == 'back') {
                ...
                //更新到数据库
                $sql = "UPDATE " . $GLOBALS['sanmu']->table('store_order') . " SET grab_store_list = '$grab_store_list' WHERE id = $id";
        ...
        elseif ($_REQUEST['act'] == 'pick_goods') {
            ...
            if ($pick_code) {
                $store_order_info = get_store_order_info($id);
                //判断订单是否已付款
                $sql = " SELECT order_status, pay_status, shipping_status, pay_id, user_id FROM " . $this->sanmu->table('order_info') . " WHERE order_id = '$order_id' ";
                ...
                if ($order_info['pay_status'] != 2 && !in_array($payment['pay_code'], ['bank', 'cod'])) {
                    ...
                    $sql = " UPDATE " . $this->sanmu->table('order_info') . " SET order_status = '" . OS_SPLITED . "', pay_status = '" . PS_PAYED . "', shipping_status = '" . SS_RECEIVED . "' " .
        ...
        elseif ($_REQUEST['act'] == 'grab_order') {
            ...
            if (empty($store_order_info['store_id'])) {
                ...
                $sql = " UPDATE " . $this->sanmu->table('store_order') . " SET store_id = '$store_id' WHERE id = '$id' ";
                if ($this->db->query($sql)) {
        ...
        elseif ($_REQUEST['act'] == 'delivery') {
            ...
            if ($invoice_no) {
                ...
                /* 发货单入库 */
                $this->db->autoExecute($this->sanmu->table('delivery_order'), $_delivery, 'INSERT', '', 'SILENT');
                    ...
                    if (!empty($goods_list)) {
                        foreach ($goods_list as $value) {
                            ...
                            if (empty($value['extension_code']) || $value['extension_code'] == 'virtual_card') {
                                ...
                                $this->db->autoExecute($this->sanmu->table('delivery_goods'), $delivery_goods, 'INSERT', '', 'SILENT');
                ...
                AND goods_id = '" . $value['goods_id'] . "' ";
                            ...
                            elseif ($value['extension_code'] == 'package_buy') {
                                foreach ($value['package_goods_list'] as $pg_key => $pg_value) {
                                    ...
                                    $this->db->autoExecute($this->sanmu->table('delivery_goods'), $delivery_pg_goods, 'INSERT', '', 'SILENT');
                                    $sql = "UPDATE " . $this->sanmu->table('order_goods') . "
                SET send_number = " . $value['goods_number'] . "
                WHERE order_id = '" . $value['order_id'] . "'
                        ...
                        if ($value['is_real'] != 0) {
                            ...
                            if (!empty($value['goods_attr_id'])) {
                                ...
                                if (is_spec($spec)) {
                                    ...
                                    $minus_stock_sql = "UPDATE " . $this->sanmu->table('store_products') . "
                                ...
                                $minus_stock_sql = "UPDATE " . $this->sanmu->table('store_goods') . "
                ...
                if ($order_finish) {
                    ...
                    if ($cfg == '1') {
                        ...
                        $this->smarty->assign('confirm_url', $this->sanmu->stores_url() . 'user_order.php?act=order_detail&order_id=' . $order['order_id']); //by wu
                        $this->smarty->assign('send_msg_url', $this->sanmu->stores_url() . 'user_message.php?act=message_list&order_id=' . $order['order_id']);
    ...
    private function package_sended($package_id, $goods_id, $order_id, $extension_code, $product_id = 0)
    {
        ...
        $sql = "SELECT SUM(DG.send_number)
            FROM " . $GLOBALS['sanmu']->table('delivery_goods') . " AS DG, " . $GLOBALS['sanmu']->table('delivery_order') . " AS o
    ...
    private function get_order_goods($order, $store_id = 0)
    {
        ...
        $sql = "SELECT o.*, g.model_inventory, g.model_attr AS model_attr, g.suppliers_id AS suppliers_id, g.goods_number AS storage, g.goods_thumb, o.goods_attr, IFNULL(b.brand_name, '') AS brand_name, p.product_sn, g.bar_code  " .
            "FROM " . $GLOBALS['sanmu']->table('order_goods') . " AS o " .
            "LEFT JOIN " . $GLOBALS['sanmu']->table('products') . " AS p ON o.product_id = p.product_id " .
            "LEFT JOIN " . $GLOBALS['sanmu']->table('goods') . " AS g ON o.goods_id = g.goods_id " .
            "LEFT JOIN " . $GLOBALS['sanmu']->table('brand') . " AS b ON g.brand_id = b.brand_id " .
            "WHERE o.order_id = '$order[order_id]' ";

        $res = $GLOBALS['db']->query($sql);

        //当前域名协议
        $http = $GLOBALS['sanmu']->http();
    ...
    {
        $sql = "SELECT region_number FROM " . $GLOBALS['sanmu']->table($table) . " WHERE region_id = '$warehouse_id' AND goods_id = '$goods_id'";
    ...
    private function get_package_goods_list($package_id)
    {
        $sql = "SELECT pg.goods_id, g.goods_name, (CASE WHEN pg.product_id > 0 THEN p.product_number ELSE g.goods_number END) AS goods_number, p.goods_attr, p.product_id, pg.goods_number AS
            ...
            FROM " . $GLOBALS['sanmu']->table('package_goods') . " AS pg
                LEFT JOIN " . $GLOBALS['sanmu']->table('goods') . " AS g ON pg.goods_id = g.goods_id
                LEFT JOIN " . $GLOBALS['sanmu']->table('products') . " AS p ON pg.product_id = p.product_id
        ...
        if ($good_product_str != '') {
            $sql = "SELECT ga.goods_attr_id, ga.attr_value, ga.attr_price, a.attr_name
                FROM " . $GLOBALS['sanmu']->table('goods_attr') . " AS ga, " . $GLOBALS['sanmu']->table('attribute') . " AS a
    ...
    private function store_order_list($ru_id = 0, $store_id = 0)
    {
        ...
        $where .= " AND (select count(*) from " . $GLOBALS['sanmu']->table('order_info') . " as oi1 WHERE oi1.main_order_id = so.order_id) = 0 ";  //主订单下有子订单时，则主订单不显示
        ...
        /* 获得总记录数据 */
        $sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['sanmu']->table('store_order') . ' AS so, ' .
        ...
        $sql = 'SELECT so.*, o.order_id, o.order_sn, o.add_time, o.consignee, o.order_status, o.shipping_status, o.pay_status, so.store_id, ' .
            ...
            'FROM ' . $GLOBALS['sanmu']->table('store_order') . ' AS so, ' .
            $GLOBALS['sanmu']->table('order_info') . ' AS o ' .
            $where . " AND o.order_id = so.order_id " .
    ...
    {
        $sql = " SELECT og.order_id, og.goods_id, og.goods_name, og.goods_price, og.goods_number, g.goods_thumb, og.goods_sn, og.goods_attr, og.product_id FROM " . $GLOBALS['sanmu']->table('order_goods') . " AS og " .
        ...
        foreach ($order_goods_list as $key => $val) {
            ...
            $order_goods_list[$key]['url'] = $this->sanmuRepository->buildUri('goods', ['gid' => $val['goods_id']], $val['goods_name']);

            //获得商品货号
            $sql = "SELECT product_sn FROM " . $GLOBALS['sanmu']->table('products') . " WHERE product_id = '{$val['product_id']}'";
    ...
    {
        $sql = " SELECT * FROM " . $GLOBALS['sanmu']->table('offline_store') . " WHERE id = '$store_id' ";
    ...
    private function get_order_finish($order_id)
    {
        ...
        $sql = 'SELECT COUNT(rec_id)
            FROM ' . $GLOBALS['sanmu']->table('order_goods') . '
            WHERE order_id = \'' . $order_id . '\'
    ...
    private function delivery_order_info($delivery_id, $delivery_sn = '')
    {
        ...
        $sql = "SELECT * FROM " . $GLOBALS['sanmu']->table('delivery_order');
        if ($delivery_id > 0) {
    ...
    private function get_all_delivery_finish($order_id)
    {
        ...
        if (!$this->get_order_finish($order_id)) {
            ...
            $sql = "SELECT COUNT(delivery_id)
                FROM " . $GLOBALS['sanmu']->table('delivery_order') . "
                WHERE order_id = '$order_id'
                AND status = 2 ";
            $sum = $GLOBALS['db']->getOne($sql);
            // 全部发货
