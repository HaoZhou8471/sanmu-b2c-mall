...
class StoreAssistantController extends InitController
{
    ...
    {
        $exc = new Exchange($this->sanmu->table("store_user"), $this->db, 'id', 'stores_user', 'store_id', 'email');
        $sto = new Exchange($this->sanmu->table("offline_store"), $this->db, 'id', 'stores_user', 'stores_name', 'is_confirm', 'stores_tel', 'stores_opening_hours');
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            if ($_REQUEST['act'] == 'edit') {
                $sql = "SELECT * FROM" . $this->sanmu->table('store_user') . " WHERE id = '$id'";
            ...
            if ($_REQUEST['act'] == 'insert') {
                ...
                $ec_salt = rand(1, 9999);
                $time = gmtime();
                $parent_id = $this->db->getOne("SELECT id FROM" . $this->sanmu->table('store_user') . " WHERE store_id = '$store_id' AND ru_id = '$ru_id' AND parent_id = 0");
                $sql = "INSERT INTO" . $this->sanmu->table('store_user') . "(`ru_id`,`store_id`,`parent_id`,`stores_user`,`stores_pwd`,`ec_salt`,`add_time`,`tel`,`email`,`store_action`) "
                ...
                $sql = "SELECT ec_salt FROM" . $this->sanmu->table('store_user') . " WHERE id = '$id'";
                ...
                /* 权限 */
                $user_action = $this->db->getOne("SELECT store_action FROM" . $this->sanmu->table('store_user') . " WHERE id = '$id'");
                if ($user_action != 'all') {
                ...
                $sql = "UPDATE" . $this->sanmu->table('store_user') . " SET $where stores_user = '$store_user',tel = '$tel',email = '$email'" . $set_action . " WHERE id = '$id'";
            ...
            $sql = "SELECT stores_name,country,province,city,district,stores_address,stores_tel,stores_opening_hours,stores_traffic_line,stores_img FROM" . $this->sanmu->table('offline_store') . " WHERE id='$store_id'";
            $store_info = $this->db->getRow($sql);

            if (isset($store_info['stores_img']) && !empty($store_info['stores_img'])) {
                $store_info['stores_img'] = $this->sanmuRepository->getImagePath($store_info['stores_img']);
            }
            ...
            $sql = "UPDATE" . $this->sanmu->table('offline_store') . " SET stores_name='$stores_name',country='$country'"
    ...
    private function get_store_user($ru_id = 0, $store_id = 0)
    {
        ...
        if ($result === false) {
            ...
            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('store_user') . $where;
            $filter['record_count'] = $this->db->getOne($sql);
            $filter = page_and_size($filter);
            /* 获活动数据 */
            $sql = "SELECT id,stores_user,tel,email,add_time FROM " . $this->sanmu->table('store_user') . " $where ORDER BY id ASC LIMIT " . $filter['start'] . "," . $filter['page_size'];
    ...
    {
        //获取权限列表
        $sql = " SELECT * FROM " . $this->sanmu->table('store_action');
        $store_action = $this->db->getAll($sql);

        //获取店员权限
        $sql = " SELECT store_action FROM " . $this->sanmu->table('store_user') . " WHERE id = '$store_user_id' ";
