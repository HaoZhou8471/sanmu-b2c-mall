...
class IndexController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($act == 'check_order') {
            $ru_id = $GLOBALS['db']->getOne(" SELECT ru_id FROM " . $GLOBALS['sanmu']->table('offline_store') . " WHERE id = '" . session('stores_id') . "'");
            $where = "";
            //区分自营或是店铺
            if ($ru_id > 0) {
                //只获取上级店铺的订单
            ...
            $where .= " AND (select count(*) from " . $this->sanmu->table('order_info') . " as oi WHERE oi.main_order_id = o.order_id) = 0 ";  //主订单下有子订单时，则主订单不显示
            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('store_order') . " as o, " .
                $this->sanmu->table('order_info') . " AS oi1 " .
