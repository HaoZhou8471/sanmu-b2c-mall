...
class GoodsController extends InitController
{
    ...
    public function index()
    {
        ...
        $ru_id = $this->db->getOne(" SELECT ru_id FROM " . $this->sanmu->table('offline_store') . " WHERE id = '$store_id'", true);
        ...
        elseif ($_REQUEST['act'] == 'info') {
            ...
            /* 取出商品信息 */
            $sql = "SELECT goods_sn, goods_name, goods_type, shop_price, model_attr, goods_thumb FROM " . $this->sanmu->table('goods') . " WHERE goods_id = '$goods_id'";
            ...
            if (!is_array($products_sn)) {
                ...
                if ($goods_sn) {
                    foreach ($products_sn as $val) {
                        ...
                        $sql = "SELECT goods_id FROM " . $this->sanmu->table('store_products') . "WHERE product_sn='$val'";
        ...
        elseif ($_REQUEST['act'] == 'product_add_execute') {
            ...
            /* 更新常规库存 start */
            $sql = " SELECT id FROM " . $this->sanmu->table('store_goods') . " WHERE 1 " . $where;
            $have_data = $this->db->getOne($sql);
            if ($have_data) {
                $sql = " UPDATE " . $this->sanmu->table('store_goods') . " SET goods_number = '$goods_number' WHERE 1 " . $where;
                $this->db->query($sql);
            } else {
                $sql = " INSERT INTO " . $this->sanmu->table('store_goods') . " (id, goods_id, store_id, ru_id, goods_number) VALUES " .
            ...
            if ($have_goods_attr) {
                ...
                /* 取出商品信息 */
                $sql = "SELECT goods_sn, goods_name, goods_type, shop_price FROM " . $this->sanmu->table('goods') . " WHERE goods_id = '" . $product['goods_id'] . "'";
                $goods = $this->db->getRow($sql);
                if (empty($goods)) {
                    return make_json_response('', 0, $GLOBALS['_LANG']['sys']['wrong'] . $GLOBALS['_LANG']['cannot_found_goods']);
                }
                ...
                foreach ($product['product_sn'] as $key => $value) {
                    ...
                    /* 插入货品表 */
                    $sql = "INSERT INTO " . $this->sanmu->table('store_products') . " (goods_id, goods_attr, product_sn, product_number, ru_id, store_id)  VALUES ('" . $product['goods_id'] . "', '$goods_attr', '$value', '" . $product['product_number'][$key] . "', '$ru_id', '$store_id' )";
                    ...
                    //货品号为空 自动补货品号
                    if (empty($value)) {
                        $sql = "UPDATE " . $this->sanmu->table('store_products') . "
                ...
                //清空属性库存
                $sql = "DELETE FROM" . $this->sanmu->table('store_products') . "WHERE goods_id = '$goods_id' AND store_id = '$store_id'";
                $this->db->query($sql);
            }
        ...
        elseif ($_REQUEST['act'] == 'product_remove') {
            ...
            /* 删除货品 */
            $sql = "DELETE FROM " . $this->sanmu->table('store_products') . " WHERE product_id = '$product_id'";
        ...
        elseif ($_REQUEST['act'] == 'edit_product_sn') {
            ...
            /* 修改 */
            $sql = "UPDATE " . $this->sanmu->table('store_products') . " SET product_sn = '$product_sn' WHERE product_id = '$product_id'";
            $result = $this->db->query($sql);
            if ($result) {
                clear_cache_files();
        ...
        elseif ($_REQUEST['act'] == 'edit_product_number') {
            ...
            /* 修改货品库存 */
            $sql = "UPDATE " . $this->sanmu->table('store_products') . " SET product_number = '$product_number' WHERE product_id = '$product_id'";
            $result = $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'batch_goods_number') {
            ...
            if (!empty($checkboxes)) {
                foreach ($checkboxes as $v) {
                    $sql = "SELECT goods_id,goods_sn FROM" . $this->sanmu->table("goods") . " WHERE goods_id = '$v'";
                    $goods = $this->db->getRow($sql);
                    if ($goods['goods_id'] > 0) {
                        //清空默认库存
                        $sql = "DELETE FROM" . $this->sanmu->table('store_goods') . "WHERE goods_id = '$v' AND store_id = '$store_id'";
                        $this->db->query($sql);
                        //清空属性库存
                        $sql = "DELETE FROM" . $this->sanmu->table('store_products') . "WHERE goods_id = '$v' AND store_id = '$store_id'";
                        $this->db->query($sql);
                        //商品默认库存入库
                        $sql = "INSERT INTO" . $this->sanmu->table('store_goods') . "(`goods_id`,`store_id`,`ru_id`,`goods_number`,`extend_goods_number`) SELECT '$v','$store_id','$ru_id',goods_number,'' FROM" . $this->sanmu->table('goods') . "WHERE goods_id = '$v'";
                        $this->db->query($sql);
                        //商品属性库存入库
                        $sql = "SELECT * FROM" . $this->sanmu->table('products') . "WHERE goods_id = '$v'";
                        $products = $this->db->getAll($sql);
                        if (!empty($products)) {
                            foreach ($products as $key => $val) {
                                $sql = "INSERT INTO" . $this->sanmu->table('store_products') . "(`goods_id`,`store_id`,`ru_id`,`product_number`,`product_sn`,`goods_attr`) VALUES ('$v','$store_id','$ru_id','" . $val['product_number'] . "','','" . $val['goods_attr'] . "')";
                                $this->db->query($sql);
                                //货品号为空 自动补货品号
    ...
    private function store_goods_list($ru_id = 0, $store_id = 0)
    {
        ...
        foreach ($res as $rows) {
            ...
            $rows['url'] = $this->sanmuRepository->buildUri('goods', ['gid' => $rows['goods_id']], $rows['goods_name']);
