...
class IndexController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'set_statistical_chart') {
            ...
            //获取系统数据 start
            $no_main_order = " AND (SELECT count(*) FROM " . $GLOBALS['sanmu']->table('wholesale_order_info') . " AS oi2 WHERE oi2.main_order_id = oi.order_id) = 0 ";  //主订单下有子订单时，则主订单不显示    if ($adminru['ru_id'] > 0) {
            $sql = 'SELECT DATE_FORMAT(FROM_UNIXTIME(oi.add_time + ' . $time_diff . '),"%y-%m-%d") AS day,COUNT(*) AS count,SUM(oi.order_amount) AS money FROM ' . $this->sanmu->table('wholesale_order_info') . " AS oi" . ' WHERE oi.pay_status = 2 AND oi.add_time BETWEEN ' . $date_start . ' AND ' . $date_end . $no_main_order . ' AND oi.suppliers_id = "' . $adminru['suppliers_id'] . '" GROUP BY day ORDER BY day ASC ';
