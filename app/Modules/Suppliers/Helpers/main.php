...
{
    app(SanmuRepository::class)->helpersLang('log_action', 'suppliers');
    ...
    $sql = 'INSERT INTO ' . $GLOBALS['sanmu']->table('admin_log') . ' (log_time, user_id, log_info, ip_address) ' .
...
function goods_type_list($selected, $goods_id = 0, $type = 'html')
{
    ...
    $sql = 'SELECT cat_id, cat_name ,c_id FROM ' . $GLOBALS['sanmu']->table('goods_type') . ' WHERE enabled = 1' . $ruCat;
            ...
            if (!empty($upFile)) {
                if ($res['id'] > 0) {
                    if ($upFile != $res['attr_img']) { //更新图片之前将上一张图片删除
                        sanmu_unlink(storage_public($res['attr_img']));
                    }
...
function return_order_list()
{
    ...
    if ($result === false) {
        ...
        $sql = "SELECT agency_id FROM " . $GLOBALS['sanmu']->table('admin_user') . " WHERE user_id = '" . session('supply_id') . "'";
        ...
        if ($filter['user_name']) {
            ...
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['sanmu']->table('wholesale_order_return') . " AS r, " . $GLOBALS['sanmu']->table('wholesale_order_info') . " as o " . $where . " AND r.order_id = o.order_id";
        }
