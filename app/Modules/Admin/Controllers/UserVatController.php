...
class UserVatController extends InitController
{
    ...
    public function index()
    {
        ...
        /* 初始化$exc对象 */
        $exc = new Exchange($this->sanmu->table('users_vat_invoices_info'), $this->db, 'id', 'user_id');
    ...
    private function vat_list()
    {
        ...
        $sql = " SELECT COUNT(*) FROM " . $this->sanmu->table('users_vat_invoices_info') . " AS t " . $where;
        $filter['record_count'] = $this->db->getOne($sql);

        /* 分页大小 */
        $filter = page_and_size($filter);

        $sql = "SELECT * FROM " . $this->sanmu->table('users_vat_invoices_info') . " $where ORDER BY " . $filter['sort_by'] . ' ' . $filter['sort_order'];
        ...
        if ($res) {
            foreach ($res as $row) {
                ...
                if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                    $row['company_telephone'] = $this->sanmuRepository->stringToStar($row['company_telephone']);
                    $row['consignee_mobile_phone'] = $this->sanmuRepository->stringToStar($row['consignee_mobile_phone']);
                }
    ...
    {
        $id = isset($_REQUEST['id']) ? intval($_REQUEST['id']) : 0;
        $sql = " SELECT * FROM " . $this->sanmu->table('users_vat_invoices_info') . " WHERE id = '$id' ";
    ...
    {
        $sql = "SELECT * FROM " . $this->sanmu->table('users_vat_invoices_info') .
            " WHERE id = '$vat_id'";
