...
class ShippingAreaController extends InitController
{
    ...
    {
        $exc = new Exchange($this->sanmu->table('shipping_area'), $this->db, 'shipping_area_id', 'shipping_area_name');
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            if (!empty($list) && $shipping_code == "cac") {
                foreach ($list as $key => $val) {
                    $sql = "SELECT name FROM " . $this->sanmu->table("shipping_point") . " WHERE shipping_area_id='" . $val['shipping_area_id'] . "'";
        ...
        elseif ($_REQUEST['act'] == 'add' && !empty($_REQUEST['shipping'])) {
            ...
            $shipping = $this->db->getRow("SELECT shipping_name, shipping_code FROM " . $this->sanmu->table('shipping') . " WHERE shipping_id='$_REQUEST[shipping]'");
            ...
            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table("shipping_area") .
            ...
            if ($this->db->getOne($sql) > 0) {
                ...
                $sql = "INSERT INTO " . $this->sanmu->table('shipping_area') .
                ...
                if ($shipping_code == "cac") {
                    ...
                    $sql = "INSERT INTO " . $this->sanmu->table('area_region') . " (shipping_area_id, region_id) VALUES ('$new_id', '$district')";
                    $this->db->query($sql);
                } else {
                    /* 添加选定的城市和地区 */
                    if (isset($_POST['regions']) && is_array($_POST['regions'])) {
                        foreach ($_POST['regions'] as $key => $val) {
                            $sql = "INSERT INTO " . $this->sanmu->table('area_region') . " (shipping_area_id, region_id,ru_id) VALUES ('$new_id', '$val','" . $adminru['ru_id'] . "')";
                            $this->db->query($sql);
                        }
                    }
                }
                ...
                if ($point_name) {
                    foreach ($point_name as $key => $val) {
                        ...
                        $sql = "INSERT INTO " . $this->sanmu->table('shipping_point') . " (shipping_area_id, name, user_name, mobile,address,img_url,anchor,line) " .
        ...
        elseif ($_REQUEST['act'] == 'edit') {
            ...
            $sql = "SELECT a.shipping_name, a.shipping_code, a.support_cod, b.* " .
                "FROM " . $this->sanmu->table('shipping') . " AS a, " . $this->sanmu->table('shipping_area') . " AS b " .
            ...
            $sql = "SELECT a.region_id, r.region_name " .
                "FROM " . $this->sanmu->table('area_region') . " AS a, " . $this->sanmu->table('region') . " AS r " .
            ...
            //省份城市区域---上门取货
            $sql = "SELECT region_id FROM " . $this->sanmu->table('area_region') . " WHERE shipping_area_id = '$_REQUEST[id]'";
            $region_id = $this->db->getOne($sql);
            if ($region_id && $row['shipping_code'] == "cac") {
                //区域
                $sql = "SELECT * FROM " . $this->sanmu->table('region') . " WHERE region_id = '$region_id'";
                $district = $this->db->getRow($sql);

                $sql = "SELECT * FROM " . $this->sanmu->table('region') . " WHERE parent_id = '$district[parent_id]'";
                $district_all = $this->db->getAll($sql);
                //城市
                $sql = "SELECT * FROM " . $this->sanmu->table('region') . " WHERE region_id = '$district[parent_id]'";
                $city = $this->db->getRow($sql);

                $sql = "SELECT * FROM " . $this->sanmu->table('region') . " WHERE parent_id = '$city[parent_id]'";
                $city_all = $this->db->getAll($sql);
                //省份
                $sql = "SELECT * FROM " . $this->sanmu->table('region') . " WHERE region_id = '$city[parent_id]'";
                $province = $this->db->getRow($sql);

                $sql = "SELECT * FROM " . $this->sanmu->table('region') . " WHERE parent_id = '$province[parent_id]'";
            ...
            if ($this->db->getOne($sql) > 0) {
                ...
                $shipping_code = $this->db->getOne("SELECT shipping_code FROM " . $this->sanmu->table('shipping') . " WHERE shipping_id='" . intval($_POST['shipping']) . "'");
                $shipping_name = Str::studly($shipping_code);
                $plugin = plugin_path('Shipping/' . $shipping_name . '/config.php');
                ...
                $sql = "UPDATE " . $this->sanmu->table('shipping_area') .
                ...
                if ($point_name && $mobile && $address) {
                    foreach ($point_name as $key => $val) {
                        ...
                        if ($_POST['point_id'][$key]) {
                            $sql = "UPDATE " . $this->sanmu->table('shipping_point') . "SET name='$point_name[$key]' , user_name='$user_name[$key]' ," .
                ...
                $sql = "SELECT region_id, parent_id FROM " . $this->sanmu->table('region');
                $res = $this->db->query($sql);
                foreach ($res as $row) {
                ...
                /* 清除原有的城市和地区 */
                $this->db->query("DELETE FROM " . $this->sanmu->table("area_region") . " WHERE shipping_area_id='$shipping_area_id' and ru_id='" . $adminru['ru_id'] . "'");
                ...
                if ($shipping_code == "cac") {
                    ...
                    $sql = "INSERT INTO " . $this->sanmu->table('area_region') . " (shipping_area_id, region_id) VALUES ('$shipping_area_id', '$district')";
                    $this->db->query($sql);
                } else {
                    /* 添加选定的城市和地区 */
                    foreach ($selected_regions as $key => $val) {
                        $sql = "INSERT INTO " . $this->sanmu->table('area_region') . " (shipping_area_id, region_id,ru_id) VALUES ('$shipping_area_id', '$val','" . $adminru['ru_id'] . "')";
                        $this->db->query($sql);
                    }
                }
        ...
        elseif ($_REQUEST['act'] == 'multi_remove') {
            ...
            if (isset($_POST['checkboxes']) && count($_POST['checkboxes']) > 0) {
                ...
                foreach ($_POST['checkboxes'] as $v) {
                    $this->db->query("DELETE FROM " . $this->sanmu->table('shipping_area') . " WHERE shipping_area_id='$v' and ru_id='" . $adminru['ru_id'] . "'");
                    $i++;
                }
        ...
        elseif ($_REQUEST['act'] == 'remove_area') {
            ...
            $exc->drop($id);
            $this->db->query('DELETE FROM ' . $this->sanmu->table('shipping_area') . ' WHERE shipping_area_id=' . $id . " and ru_id='" . $adminru['ru_id'] . "'");
            $this->db->query('DELETE FROM ' . $this->sanmu->table('area_region') . ' WHERE shipping_area_id=' . $id . " and ru_id='" . $adminru['ru_id'] . "'");
        ...
        elseif ($_REQUEST['act'] == 'remove_point') {
            ...
            $id = intval($_GET['id']);
            $name = $exc->get_name($id);
            $sql = "DELETE FROM " . $this->sanmu->table('shipping_point') . " WHERE id='$id'";
    ...
    {
        $sql = "SELECT * FROM " . $this->sanmu->table('shipping_area') . " where ru_id='$ru_id' ";
        if ($shipping_id > 0) {
        ...
        foreach ($res as $row) {
            $sql = "SELECT r.region_name " .
                "FROM " . $this->sanmu->table('area_region') . " AS a, " .
                $this->sanmu->table('region') . " AS r " .
