...
class SetFloorBrandController extends InitController
{
    public function index()
    {
        ...
        if ($act == 'list') {
            ...
            $sql = "DELETE FROM " . $this->sanmu->table('floor_content') . " WHERE filename = '$filename' AND theme = '$theme' AND id =  '$cat_id' AND region = '$region'";
            $this->db->query($sql);
    ...
    private function get_floor_content($curr_template, $filename, $id = 0, $region = '')
    {
        ...
        $sql = "select * from " . $this->sanmu->table('floor_content') . $where . " and filename='$filename' and theme='$curr_template'";
    ...
    {
        $sql = "select * from " . $this->sanmu->table('floor_content') . " where filename='$filename' and theme='$curr_template' group by filename,theme,id";

        $row = $this->db->getAll($sql);
        foreach ($row as $key => $val) {
            $row[$key]['brand_list'] = $this->db->getAll("select b.brand_id, b.brand_name from " . $this->sanmu->table('brand') . " AS b, " .
                $this->sanmu->table('floor_content') . " AS fc " .
            ...
            $row[$key]['cat_name'] = $this->db->getOne("select cat_name from " . $this->sanmu->table('category') . " where cat_id='$val[id]' limit 1");
        }
    ...
    {
        $sql = "select region,id from " . $this->sanmu->table('template') . " where filename='$filename' and theme='$curr_template' and region='$region'";
        ...
        foreach ($res as $key => $row) {
            ...
            $arr[$key]['cat_name'] = $this->db->getOne("select cat_name from " . $this->sanmu->table('category') . " where cat_id = '" . $row['id'] . "' limit 1");
        }
