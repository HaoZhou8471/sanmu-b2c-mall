...
class UserAccountController extends InitController
{
    ...
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            /* 获得支付方式列表 */
            $payment = [];
            $sql = "SELECT pay_id, pay_name FROM " . $this->sanmu->table('payment') .
        ...
        elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit') {
            ...
            if ($_REQUEST['act'] == 'edit') {
                /* 取得余额信息 */
                $user_account = $this->db->getRow("SELECT * FROM " . $this->sanmu->table('user_account') . " WHERE id = '$id'");

                // 如果是负数，去掉前面的符号
                $user_account['amount'] = str_replace('-', '', $user_account['amount']);

                /* 取得会员名称 */
                $sql = "SELECT user_name FROM " . $this->sanmu->table('users') . " WHERE user_id = '$user_account[user_id]'";
            ...
            if ($user_account && $user_account['pay_id'] == 0 && $user_account['payment']) {
                $sql = 'SELECT pay_id FROM ' . $this->sanmu->table('payment') .
            ...
            if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                $user_name = $this->sanmuRepository->stringToStar($user_name);
            }
        ...
        elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
            ...
            $user_id = $this->db->getOne("SELECT user_id FROM " . $this->sanmu->table('users') . " WHERE user_name = '$user_name'");
            ...
            $sql = 'SELECT pay_name FROM ' . $this->sanmu->table('payment') .
                " WHERE pay_id = '$pay_id'";
            ...
            if ($_REQUEST['act'] == 'insert') {
                ...
                $this->db->autoExecute($this->sanmu->table('user_account'), $other, 'INSERT');
                ...
                /* 更新数据表 */
                $sql = "UPDATE " . $this->sanmu->table('user_account') . " SET " .
                    "admin_note   = '$admin_note', " .
                    "user_note    = '$user_note', " .
                    "payment      = '$payment' " .
                    "WHERE id      = '$id'";
            ...
            if ($process_type == 0 && $is_paid == 0) {
                ...
                /* 取支付方式信息 */
                $payment_info = [];
                $payment_info = $this->db->getRow('SELECT * FROM ' . $this->sanmu->table('payment') .
        ...
        elseif ($_REQUEST['act'] == 'check') {
            ...
            /* 如果参数不合法，返回 */
            if ($id == 0) {
                return sanmu_header("Location: user_account.php?act=list\n");
            }

            /* 查询当前的预付款信息 */
            $account = $this->db->getRow("SELECT * FROM " . $this->sanmu->table('user_account') . " WHERE id = '$id'");
            ...
            if ($account['process_type'] == 1 || $account['payment'] == lang('admin/user_account.remittance')) {
                $sql = "SELECT real_name, bank_card, self_num, bank_name  FROM " . $this->sanmu->table('users_real') . " WHERE user_id = '$account[user_id]'";
            ...
            //by wang获得用户账目的扩展信息
            $account['fields'] = $this->db->getRow("SELECT * FROM " . $this->sanmu->table('user_account_fields') . " WHERE user_id = '$account[user_id]' and account_id='$id'");
            ...
            $sql = "SELECT user_name, mobile_phone FROM " . $this->sanmu->table('users') . " WHERE user_id = '" . $account['user_id'] . "' LIMIT 1";
            ...
            if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0 && $user_info) {
                ...
                $user_info['mobile_phone'] = $this->sanmuRepository->stringToStar($user_info['mobile_phone']);
            }

            $sql = "SELECT * FROM " . $this->sanmu->table('users_real') . " WHERE user_id = '" . $account['user_id'] . "' LIMIT 1";
            $users_real = $this->db->getRow($sql);

            if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0 && $users_real) {
                $users_real['bank_mobile'] = $this->sanmuRepository->stringToStar($users_real['bank_mobile']);
            }
        ...
        elseif ($_REQUEST['act'] == 'action') {
            ...
            if ($account['is_paid'] == 0) {
                ...
                if (intval($account['user_id']) > 0) {
                    $sql = "SELECT mobile_phone, email,user_name FROM" . $this->sanmu->table('users') . " WHERE user_id = '" . $account['user_id'] . "' LIMIT 1";
                    $user_info = $this->db->getRow($sql);
                }
                ...
                if ($is_paid == 1 && $account['process_type'] == 1) {
                    ...
                    if ($order_note && isset($order_note[1]) && $order_note[1]) {
                        ...
                        if ($order['ru_id']) {
                            ...
                            $this->db->autoExecute($this->sanmu->table('merchants_account_log'), $log, 'INSERT');

                            $sql = "UPDATE " . $this->sanmu->table('seller_shopinfo') . " SET seller_money = seller_money + '" . $log['user_money'] . "' WHERE ru_id = '" . $order['ru_id'] . "'";
                            $this->db->query($sql);
                        }
                    }
        ...
        elseif ($_REQUEST['act'] == 'remove') {
            ...
            $sql = "SELECT u.user_name FROM " . $this->sanmu->table('users') . " AS u, " .
                $this->sanmu->table('user_account') . " AS ua " .
                " WHERE u.user_id = ua.user_id AND ua.id = '$id' ";
            $user_name = $this->db->getOne($sql);
            $sql = "DELETE FROM " . $this->sanmu->table('user_account') . " WHERE id = '$id'";
        ...
        elseif ($_REQUEST['act'] == 'batch') {
            ...
            if ($checkboxes) {
                foreach ($checkboxes as $id) {
                    ...
                    if ($account['is_paid'] == 0) {
                        //获取申请会员信息
                        $sql = "SELECT mobile_phone, email,user_name FROM" . $this->sanmu->table('users') . " WHERE user_id = '" . $account['user_id'] . "'";
                        $user_info = $this->db->getRow($sql);
    ...
    {
        $sql = "SELECT user_money, frozen_money FROM " . $this->sanmu->table('users') .
    ...
    private function account_list()
    {
        ...
        if ($result === false) {
            ...
            if ($filter['keywords']) {
                ...
                $leftJoin = "LEFT JOIN" . $GLOBALS['sanmu']->table('users') . ' AS u ON ua.user_id = u.user_id';
            }
            /* 　时间过滤　 */
            ...
            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('user_account') . " AS ua " . $leftJoin . $where;
            ...
            $sql = 'SELECT ua.*, u.user_name FROM ' .
                $this->sanmu->table('user_account') . ' AS ua LEFT JOIN ' .
                $this->sanmu->table('users') . ' AS u ON ua.user_id = u.user_id' .
        ...
        foreach ($list as $key => $value) {
            ...
            if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                $list[$key]['user_name'] = $this->sanmuRepository->stringToStar($value['user_name']);
            }
        }
