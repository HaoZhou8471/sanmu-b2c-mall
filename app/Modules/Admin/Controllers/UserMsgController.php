...
class UserMsgController extends InitController
{
    ...
    public function index()
    {
        ...
        /*初始化数据交换对象 */
        $exc = new Exchange($this->sanmu->table("feedback"), $this->db, 'msg_id', 'msg_title');
        ...
        if ($_REQUEST['act'] == 'add') {
            ...
            $order_sn = $this->db->getOne("SELECT order_sn FROM " . $this->sanmu->table('order_info') . " WHERE order_id = '$order_id'");

            /* 获取关于订单所有信息 */
            $sql = "SELECT msg_id, user_name, msg_title, msg_type, msg_time, msg_content" .
                " FROM " . $this->sanmu->table('feedback') .
        ...
        if ($_REQUEST['act'] == 'insert') {
            $sql = "INSERT INTO " . $this->sanmu->table('feedback') . "(parent_id, user_id, user_name, user_email, msg_title, msg_type, msg_content, msg_time, message_img, order_id)" .
                " VALUES (0, '$_POST[user_id]', '" . session('admin_name') . "', ' ', " .
                " '$_POST[msg_title]', 5, '$_POST[msg_content]', '" . gmtime() . "', '', '" . intval($_POST['order_id']) . "')";

            $this->db->query($sql);

            return sanmu_header("Location: user_msg.php?act=add&order_id=$_POST[order_id]&user_id=$_POST[user_id]\n");
        }
        ...
        if ($_REQUEST['act'] == 'remove_msg') {
            ...
            $order_id = empty($_GET['order_id']) ? 0 : intval($_GET['order_id']);
            $user_id = empty($_GET['user_id']) ? 0 : intval($_GET['user_id']);
            $sql = "SELECT user_id, order_id, message_img FROM " . $this->sanmu->table('feedback') . " WHERE msg_id='$msg_id'";
            $row = $this->db->getRow($sql);
            if ($row) {
                if ($row['user_id'] == $user_id && $row['order_id'] == $order_id) {
                    if ($row['message_img']) {
                        sanmu_unlink(storage_public(DATA_DIR . '/feedbackimg/' . $row['message_img']));
                    }
                    $sql = "DELETE FROM " . $this->sanmu->table('feedback') . " WHERE msg_id=$msg_id LIMIT 1";
                    $this->db->query($sql);
                }
            }

            return sanmu_header("Location: user_msg.php?act=add&order_id=$_GET[order_id]&user_id=$_GET[user_id]\n");
        }
        /*------------------------------------------------------ */
        //-- 更新留言的状态为显示或者        禁止
        /*------------------------------------------------------ */
        if ($_REQUEST['act'] == 'check') {
            if ($_REQUEST['check'] == 'allow') {
                /* 允许留言显示 */
                $sql = "UPDATE " . $this->sanmu->table('feedback') . " SET msg_status = 1 WHERE msg_id = '$_REQUEST[id]'";
                $this->db->query($sql);

                /* 清除缓存 */
                clear_cache_files();

                return sanmu_header("Location: user_msg.php?act=view&id=$_REQUEST[id]\n");
            } else {
                /* 禁止留言显示 */
                $sql = "UPDATE " . $this->sanmu->table('feedback') . " SET msg_status = 0 WHERE msg_id = '$_REQUEST[id]'";
                $this->db->query($sql);
...
//    $this->smarty->assign($sort_flag['tag'], $sort_flag['img']);
        ...
        elseif ($_REQUEST['act'] == 'remove') {
            ...
            if ($exc->drop($msg_id)) {
                /* 删除图片 */
                if (!empty($img)) {
                    sanmu_unlink(storage_public(DATA_DIR . '/feedbackimg/' . $img));
                }
                $sql = "DELETE FROM " . $this->sanmu->table('feedback') . " WHERE parent_id = '$msg_id' LIMIT 1";
        ...
        if ($_REQUEST['act'] == 'batch') {
            ...
            if (isset($_POST['checkboxes'])) {
                switch ($action) {
                    case 'remove':
                        $this->db->query("DELETE FROM " . $this->sanmu->table('feedback') . " WHERE " . db_create_in($_POST['checkboxes'], 'msg_id'));
                        $this->db->query("DELETE FROM " . $this->sanmu->table('feedback') . " WHERE " . db_create_in($_POST['checkboxes'], 'parent_id'));
                        break;

                    case 'allow':
                        $this->db->query("UPDATE " . $this->sanmu->table('feedback') . " SET msg_status = 1  WHERE " . db_create_in($_POST['checkboxes'], 'msg_id'));
                        break;

                    case 'deny':
                        $this->db->query("UPDATE " . $this->sanmu->table('feedback') . " SET msg_status = 0,msg_area =1  WHERE " . db_create_in($_POST['checkboxes'], 'msg_id'));
                        break;
        ...
        elseif ($_REQUEST['act'] == 'view') {
            ...
            if (empty($parent_id)) {
                $sql = "INSERT INTO " . $this->sanmu->table('feedback') . " (msg_title, msg_time, user_id, user_name , " .
                ...
                $sql = "UPDATE " . $this->sanmu->table('feedback') . " SET user_email = '" . $_POST['user_email'] . "', msg_content='" . $_POST['msg_content'] . "', msg_time = '" . gmtime() . "' WHERE msg_id = '$parent_id'";
                $this->db->query($sql);
            }
            ...
            if (!empty($_POST['send_email_notice']) or isset($_POST['remail'])) {
                ...
                $this->smarty->assign('shop_name', "<a href='" . $this->sanmu->url() . "'>" . $GLOBALS['_CFG']['shop_name'] . '</a>');
        ...
        elseif ($_REQUEST['act'] == 'drop_file') {
            ...
            /* 删除上传的文件 */
            $file = $_GET['file'];
            sanmu_unlink(storage_public(DATA_DIR . '/feedbackimg/' . $file));

            $id = isset($_GET['id']) && !empty($_GET['id']) ? intval($_GET['id']) : 0;

            /* 更新数据库 */
            $this->db->query("UPDATE " . $this->sanmu->table('feedback') . " SET message_img = '' WHERE msg_id = '$id'");
    ...
    private function msg_list()
    {
        ...
        $sql = "SELECT count(*) FROM " . $this->sanmu->table('feedback') . " AS f" .
        ...
        $sql = "SELECT f.msg_id, f.user_name, f.msg_title, f.msg_type, f.order_id, f.msg_status, f.msg_time, f.msg_area, COUNT(r.msg_id) AS reply " .
            "FROM " . $this->sanmu->table('feedback') . " AS f " .
            "LEFT JOIN " . $this->sanmu->table('feedback') . " AS r ON r.parent_id=f.msg_id " .
        ...
        foreach ($msg_list as $key => $value) {
            if ($value['order_id'] > 0) {
                $msg_list[$key]['order_sn'] = $this->db->getOne("SELECT order_sn FROM " . $this->sanmu->table('order_info') . " WHERE order_id= " . $value['order_id']);
            }
            ...
            if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                $msg_list[$key]['user_name'] = $this->sanmuRepository->stringToStar($value['user_name']);
            }
        }
    ...
    private function get_feedback_detail($id)
    {
        $sql = "SELECT T1.*, T2.msg_id AS reply_id, T2.user_name  AS reply_name, u.email AS reply_email, " .
            ...
            "FROM " . $this->sanmu->table('feedback') . " AS T1 " .
            "LEFT JOIN " . $this->sanmu->table('admin_user') . " AS u ON u.user_id='" . session('admin_id') . "' " .
            "LEFT JOIN " . $this->sanmu->table('feedback') . " AS T2 ON T2.parent_id=T1.msg_id " .
            "WHERE T1.msg_id = '$id'";
        ...
        if ($msg) {
            ...
            if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                $msg['user_name'] = $this->sanmuRepository->stringToStar($msg['user_name']);
                $msg['user_email'] = $this->sanmuRepository->stringToStar($msg['user_email']);
            }
        }
