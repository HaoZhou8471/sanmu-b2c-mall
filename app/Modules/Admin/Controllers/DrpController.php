...
class DrpController extends BaseController
{
    ...
    public function drp_set_qrcode(Request $request, DrpConfigService $drpConfigService)
    {
        ...
        if (!empty($info)) {
            $info['backbround'] = $this->sanmuRepository->getImagePath($info['backbround']);
        ...
        if (!empty($imgList)) {
            foreach ($imgList as $key => $val) {
                $imgList[$key] = $this->sanmuRepository->getImagePath($val);
            }
        }
        // 添加默认背景图至开头
    ...
    public function drp_aff_list()
    {
        ...
        if (!empty($user_list)) {
            foreach ($user_list as $k => $val) {
                ...
                if (isset($this->config['show_mobile']) && $this->config['show_mobile'] == 0) {
                    $user_list[$k]['user_name'] = $this->sanmuRepository->stringToStar($val['user_name']);
                    $user_list[$k]['email'] = $this->sanmuRepository->stringToStar($val['email']);
                }
