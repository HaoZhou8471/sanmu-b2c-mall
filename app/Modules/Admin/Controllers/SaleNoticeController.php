    ...
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            foreach ($list['item'] as $key => $val) {
                $list['item'][$key]['goods_link'] = $this->sanmu->url() . "goods.php?id=" . $val['goods_id'];
            }
        ...
        if ($_REQUEST['act'] == 'view') {
            ...
            if (!empty($detail)) {
                ...
                if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                    $detail['user_name'] = $this->sanmuRepository->stringToStar($detail['user_name']);
                    $detail['cellphone'] = $this->sanmuRepository->stringToStar($detail['cellphone']);
                    $detail['email'] = $this->sanmuRepository->stringToStar($detail['email']);
                }
        ...
        if ($_REQUEST['act'] == 'action') {
            ...
            if (!empty($email) && !empty($detail['email'])) {
                ...
                $this->smarty->assign('goods_link', $this->sanmu->url() . "goods.php?id=" . $detail['goods_id']);
            ...
            if (!empty($resms) && !empty($detail['cellphone']) && $GLOBALS['_CFG']['sms_price_notice'] == 1) {
                ...
                $smsParams = [
                    ...
                    'goodsname' => $this->sanmuRepository->subStr($detail['goods_name'], 20),
            ...
            return sanmu_header("Location: sale_notice.php?act=list");
        }
