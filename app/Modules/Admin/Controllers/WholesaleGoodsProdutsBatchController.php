...
class WholesaleGoodsProdutsBatchController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'ajax_insert') {
            ...
            if (session()->has('goods_list') && session('goods_list')) {
                ...
                //查询数据是否已经存在;
                $sql = "SELECT product_id FROM " . $GLOBALS['sanmu']->table('wholesale_products') . " WHERE goods_id = '" . $result['list']['goods_id'] . "'" .
                    " AND goods_attr = '" . $result['list']['goods_attr'] . "'";

                if ($GLOBALS['db']->getOne($sql, true)) {
                    $this->db->autoExecute($this->sanmu->table('wholesale_products'), $other, 'UPDATE', "goods_id = '" . $result['list']['goods_id'] . "' AND goods_attr = '" . $result['list']['goods_attr'] . "'");
                    ...
                    $this->db->autoExecute($this->sanmu->table('wholesale_products'), $other, 'INSERT');
    ...
    {
        $sql = "SELECT a.attr_id, a.attr_name FROM " . $GLOBALS['sanmu']->table('goods_attr') . " AS ga " .
    ...
    {
        $sql = "select goods_attr_id, attr_value from " . $GLOBALS['sanmu']->table('goods_attr') . " where goods_id = '$goods_id' and attr_id = '$attr_id' order by goods_attr_id asc";
        $res = $GLOBALS['db']->getAll($sql);
