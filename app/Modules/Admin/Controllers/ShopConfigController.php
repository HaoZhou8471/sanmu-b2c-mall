...
class ShopConfigController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'post') {
            ...
            $arr = [];
            $sql = 'SELECT id, value FROM ' . $this->sanmu->table('shop_config');
            ...
            /* 处理上传文件 */
            $file_var_list = [];
            $sql = "SELECT * FROM " . $this->sanmu->table('shop_config') . " WHERE parent_id > 0 AND type = 'file'";
            $res = $this->db->query($sql);
            foreach ($res as $row) {
                ...
                if ((isset($file['error']) && $file['error'] == 0) || (!isset($file['error']) && $file['tmp_name'] != 'none')) {
                    ...
                    if (!check_file_type($file['tmp_name'], $file['name'], $allow_file_types)) {
                        ...
                        if (move_upload_file($file['tmp_name'], $file_name)) {
                            $file_name = $dir_name;
                            $sql = "SELECT value FROM" . $this->sanmu->table("shop_config") . " WHERE code = '$code'";
                            ...
                            if ($file_name) {
                                ...
                                if ($olde_value != $file_name && $olde_value != "../images/errorImg.png" && $olde_value != '' && strpos($olde_value, 'http://') === false && strpos($olde_value, 'https://') === false) {
                                    ...
                                    $this->sanmuRepository->getOssDelFile([$oss_olde_file]);
                                    //做判断，判断文件是否存在，如果存在则删除

                                    $oss_olde_file = get_image_path($oss_olde_file);
                                    sanmu_unlink($oss_olde_file);
                                }
                                $this->sanmuRepository->getOssAddFile([$oss_file_name]);
                            }

                            $sql = "UPDATE " . $this->sanmu->table('shop_config') . " SET value = '$file_name' WHERE code = '$code'";
                            $this->db->query($sql);
                        } else {
            ...
            if (!empty($invoice_list['type'])) {
                ...
                $sql = "UPDATE " . $this->sanmu->table('shop_config') . " SET value = '" . serialize($invoice) . "' WHERE code = 'invoice_type'";
                $this->db->query($sql);
            }

            if (empty($invoice_list['type']) && empty($invoice_list['rate'])) {
                $sql = "UPDATE " . $this->sanmu->table('shop_config') . " SET value = '' WHERE code = 'invoice_type'";
                $this->db->query($sql);
            }
            ...
            $sql = 'SELECT id, code, value FROM ' . $this->sanmu->table('shop_config') . " WHERE code IN('shop_name','shop_title','shop_desc','shop_keywords','shop_address','qq','ww','service_phone','msn','service_email','sms_shop_mobile','icp_number','lang', 'certi', 'shop_country', 'shop_province', 'shop_city', 'shop_address', 'shop_district')";
            ...
            if (!in_array($type, $back_array)) {
                ...
                foreach ($update_arr as $key => $val) {
                    $sql = "UPDATE " . $this->sanmu->table('seller_shopinfo') . " SET " . $key . " = '" . $val . "' WHERE ru_id = 0 ";
                    $this->db->query($sql);
                }
            }
        ...
        elseif ($_REQUEST['act'] == 'del') {
            ...
            if (isset($filename) && !empty($filename)) {
                ...
                $this->sanmuRepository->getOssDelFile([$oss_file_name]);
            }

            //删除文件
            if (in_array($code, ['admin_login_logo', 'admin_logo', 'seller_login_logo', 'seller_logo', 'stores_login_logo', 'stores_logo', 'order_print_logo'])) {
                sanmu_unlink(public_path('/assets/' . $filename));
            } else {
                sanmu_unlink(storage_public($filename));
            }

            $sql = "UPDATE " . $this->sanmu->table('shop_config') . " SET value = '' WHERE code = '$code'";
            $this->db->query($sql);
    ...
    {
        if (!empty($key)) {
            $sql = "UPDATE " . $this->sanmu->table('shop_config') . " SET value='$val' WHERE code='$key'";
    ...
    private function get_settings($groups = null, $excludes = null)
    {
        ...
        /* 取出全部数据：分组和变量 */
        $sql = "SELECT * FROM " . $this->sanmu->table('shop_config') .
                            ...
                            if ($item['type'] == 'file' && in_array($item['code'], $code_arr) && $item['value']) {
                                ...
                                if (strpos($item['value'], '../') === false) {
                                    if (in_array($item['code'], ['admin_login_logo', 'admin_logo', 'seller_login_logo', 'seller_logo', 'stores_login_logo', 'stores_logo', 'order_print_logo'])) {
                                        $item['value'] = $this->sanmu->url() . 'assets/' . $item['value'];
                                    } else {
