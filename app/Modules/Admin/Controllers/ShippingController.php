...
class ShippingController extends InitController
{
    ...
    {
        $exc = new Exchange($this->sanmu->table('shipping'), $this->db, 'shipping_id', 'shipping_name');
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            $modules = $this->sanmuRepository->readModules(plugin_path('Shipping'));

            for ($i = 0; $i < count($modules); $i++) {

                /* 检查该插件是否已经安装 */
                $sql = "SELECT shipping_id, shipping_name, shipping_desc, insure, support_cod,shipping_order FROM " . $this->sanmu->table('shipping') . " WHERE shipping_code='" . $modules[$i]['code'] . "' ORDER BY shipping_order";
        ...
        elseif ($_REQUEST['act'] == 'date_insert') {
            ...
            $sql = "INSERT INTO " . $this->sanmu->table('shipping_date') . "(start_date, end_date, select_day)VALUES('$shipping_date_start', '$shipping_date_end', $later_day)";
            $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'date_edit') {
            ...
            if (empty($shipping_id)) {
                return sanmu_header("location: shipping.php?act=date_list\n");
            }

            $sql = "SELECT * FROM " . $this->sanmu->table('shipping_date') . " WHERE shipping_date_id='$shipping_id'";
        ...
        elseif ($_REQUEST['act'] == 'date_update') {
            ...
            $sql = "UPDATE " . $this->sanmu->table('shipping_date') . " SET start_date='$shipping_date_start', end_date='$shipping_date_end', select_day='$later_day' WHERE shipping_date_id='$shipping_id'";
        ...
        elseif ($_REQUEST['act'] == 'date_remove') {
            ...
            $sql = "DELETE FROM " . $this->sanmu->table('shipping_date') . " WHERE shipping_date_id='$shipping_id'";
            if (($this->db->query($sql))) {
                /* 提示信息 */
                $link[0]['text'] = $GLOBALS['_LANG']['back_list_page'];
                $link[0]['href'] = 'shipping.php?act=date_list';
        ...
        elseif ($_REQUEST['act'] == 'install') {
            ...
            /* 检查该配送方式是否已经安装 */
            $sql = "SELECT shipping_id FROM " . $this->sanmu->table('shipping') . " WHERE shipping_code = '$code'";
            $id = $this->db->GetOne($sql);

            if ($id > 0) {
                /* 该配送方式已经安装过, 将该配送方式的状态设置为 enable */
                $this->db->query("UPDATE " . $this->sanmu->table('shipping') . " SET enabled = 1 WHERE shipping_code = '$code' LIMIT 1");
            } else {
                ...
                if (file_exists($modules)) {
                    ...
                    $sql = "INSERT INTO " . $this->sanmu->table('shipping') . " (" .
        ...
        elseif ($_REQUEST['act'] == 'uninstall') {
            ...
            /* 获得该配送方式的ID */
            $row = $this->db->GetRow("SELECT shipping_id, shipping_name, print_bg FROM " . $this->sanmu->table('shipping') . " WHERE shipping_code='$code'");
            $shipping_id = $row['shipping_id'];
            $shipping_name = $row['shipping_name'];

            /* 删除 shipping_fee 以及 shipping 表中的数据 */
            if ($row) {
                $all = $this->db->getCol("SELECT shipping_area_id FROM " . $this->sanmu->table('shipping_area') . " WHERE shipping_id='$shipping_id'");
                if (!empty($all)) {
                    $in = db_create_in(join(',', $all));
                    $this->db->query("DELETE FROM " . $this->sanmu->table('area_region') . " WHERE shipping_area_id $in");
                }

                $this->db->query("DELETE FROM " . $this->sanmu->table('shipping_area') . " WHERE shipping_id='$shipping_id'");
                $this->db->query("DELETE FROM " . $this->sanmu->table('shipping') . " WHERE shipping_id='$shipping_id'");

                $this->db->query("DELETE FROM " . $this->sanmu->table('goods_transport_express') . " WHERE shipping_id='$shipping_id'");
                $this->db->query("DELETE FROM " . $this->sanmu->table('goods_transport_tpl') . " WHERE shipping_id='$shipping_id'");
        ...
        elseif ($_REQUEST['act'] == 'print_index') {
            ...
            /* 检查该插件是否已经安装 取值 */
            $sql = "SELECT * FROM " . $this->sanmu->table('shipping') . " WHERE shipping_id = '$shipping_id' LIMIT 0,1";
            $row = $this->db->GetRow($sql);
            if ($row) {
                $sql = "SELECT * FROM " . $this->sanmu->table('shipping_tpl') . " WHERE shipping_id='$shipping_id' and ru_id='" . $adminru['ru_id'] . "'";
        ...
        elseif ($_REQUEST['act'] == 'recovery_default_template') {
            ...
            /* 取配送代码 */
            $sql = "SELECT shipping_code FROM " . $this->sanmu->table('shipping') . " WHERE shipping_id = '$shipping_id'";
            ...
            if ($code) {
                ...
                if (file_exists($modules)) {
                    ...
                    /* 恢复默认 */
                    $this->db->query("UPDATE " . $this->sanmu->table('shipping_tpl') . " SET print_bg = '" . addslashes($modules['print_bg']) . "',  config_lable = '" . addslashes($modules['config_lable']) . "' " .
        ...
        elseif ($_REQUEST['act'] == 'print_upload') {
            ...
            if (!empty($_FILES['bg']['name'])) {
                ...
                if (move_upload_file($_FILES['bg']['tmp_name'], $target)) {
                    ...
                    // 上传oss
                    $this->sanmuRepository->getOssAddFile([$src]);
                }
            }

            //保存
            $sql = "UPDATE " . $this->sanmu->table('shipping_tpl') . " SET print_bg = '$src' WHERE shipping_id = '$shipping_id' and ru_id='" . $adminru['ru_id'] . "'";
        ...
        elseif ($_REQUEST['act'] == 'print_del') {
            ...
            /* 检查该插件是否已经安装 取值 */
            $sql = "SELECT print_bg FROM " . $this->sanmu->table('shipping_tpl') . " WHERE shipping_id = '$shipping_id' LIMIT 0,1";
            $row = $this->db->GetRow($sql);
            if ($row) {
                if ($row['print_bg'] != '' && !$this->is_print_bg_default($row['print_bg'])) {
                    sanmu_unlink(storage_public($row['print_bg']));

                    // 删除oss
                    $this->sanmuRepository->getOssDelFile([$row['print_bg']]);
                }

                $sql = "UPDATE " . $this->sanmu->table('shipping_tpl') . " SET print_bg = '' WHERE shipping_id = '$shipping_id' and ru_id='" . $adminru['ru_id'] . "'";
                $this->db->query($sql);
            } else {
        ...
        elseif ($_REQUEST['act'] == 'edit_print_template') {
            ...
            /* 检查该插件是否已经安装 */
            $sql = "SELECT * FROM " . $this->sanmu->table('shipping') . " WHERE shipping_id=$shipping_id";
            $row = $this->db->GetRow($sql);
            if ($row) {
                $sql = "SELECT * FROM " . $this->sanmu->table('shipping_tpl') . " WHERE shipping_id='$shipping_id' and ru_id='" . $adminru['ru_id'] . "'";
                $ship_tpl = $this->db->GetRow($sql);
                if (!$ship_tpl) {
                    $sql = "INSERT INTO " . $this->sanmu->table('shipping_tpl') . " (shipping_id, ru_id, print_bg, update_time) VALUES ('$shipping_id','" . $adminru['ru_id'] . "',''," . gmtime() . ")";
                    $this->db->query($sql);
                }
                $ship_tpl['shipping_print'] = !empty($ship_tpl['shipping_print']) ? $ship_tpl['shipping_print'] : '';
                $ship_tpl['print_bg'] = !empty($ship_tpl['print_bg']) ? get_image_path($ship_tpl['print_bg']) : '';
        ...
        elseif ($_REQUEST['act'] == 'do_edit_print_template') {
            ...
            if ($print_model == 2) {
                //所见即所得模式
                $this->db->query("UPDATE " . $this->sanmu->table('shipping_tpl') . " SET config_lable = '" . $_POST['config_lable'] . "', print_model = '$print_model' WHERE shipping_id = '$shipping_id' and ru_id='" . $adminru['ru_id'] . "'");
            } elseif ($print_model == 1) {
                //代码模式
                $template = !empty($_POST['shipping_print']) ? $_POST['shipping_print'] : '';
                $this->db->query("UPDATE " . $this->sanmu->table('shipping_tpl') . " SET shipping_print = '" . $template . "', print_model = '$print_model' WHERE shipping_id = '$shipping_id' and ru_id='" . $adminru['ru_id'] . "'");
            }
        ...
        elseif ($_REQUEST['act'] == 'account_save') {
            ...
            if (get_shipping_conf($data['shipping_id'], $data['ru_id'])) {
                $this->db->autoExecute($this->sanmu->table('kdniao_eorder_config'), $data, 'UPDATE', "shipping_id = '$data[shipping_id] ' and ru_id = '$data[ru_id]'");
            } else {
                $this->db->autoExecute($this->sanmu->table('kdniao_eorder_config'), $data, 'INSERT');
            }
        ...
        elseif ($_REQUEST['act'] == 'account_submit') {
            ...
            $data = [];
            $data['shipping_id'] = empty($_REQUEST['shipping_id']) ? 0 : intval($_REQUEST['shipping_id']);
            $data['sanmu_province'] = empty($_REQUEST['sanmu_province']) ? 0 : intval($_REQUEST['sanmu_province']);
            $data['sanmu_city'] = empty($_REQUEST['sanmu_city']) ? 0 : intval($_REQUEST['sanmu_city']);
            $data['sanmu_district'] = empty($_REQUEST['sanmu_district']) ? 0 : intval($_REQUEST['sanmu_district']);
            $data['shipper_code'] = empty($_REQUEST['shipper_code']) ? '' : trim($_REQUEST['shipper_code']);
            ...
            if (get_kdniao_customer_account($data['shipping_id'], $data['ru_id'])) {
                $this->db->autoExecute($this->sanmu->table('kdniao_customer_account'), $data, 'UPDATE', "shipping_id = '$data[shipping_id] ' and ru_id = '$data[ru_id]'");
            } else {
                $this->db->autoExecute($this->sanmu->table('kdniao_customer_account'), $data, 'INSERT');
            }
            ...
            $format_data['CityName'] = get_table_date('region', "region_id='$data[sanmu_city]'", ['region_name'], 2);
            $format_data['Name'] = $data['name'];
            $format_data['CityCode'] = isset($_REQUEST['city_code']) ? $_REQUEST['city_code'] : '';
            $format_data['Tel'] = $data['tel'];
            $format_data['ExpAreaName'] = get_table_date('region', "region_id='$data[sanmu_district]'", ['region_name'], 2);
            $format_data['Mobile'] = $data['mobile'];
            $format_data['ExpAreaCode'] = isset($_REQUEST['exp_area_code']) ? $_REQUEST['exp_area_code'] : '';
            $format_data['ProvinceName'] = get_table_date('region', "region_id='$data[sanmu_province]'", ['region_name'], 2);
            ...
            $result = sanmu_decode($jsonResult, true);
    ...
    {
        $sql = "SELECT * FROM " . $this->sanmu->table('shipping_date');
