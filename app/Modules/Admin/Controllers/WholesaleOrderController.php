...
class WholesaleOrderController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'info') {
            ...
            if ($order['user_id'] > 0) {
                ...
                // 地址信息
                $sql = "SELECT * FROM " . $this->sanmu->table('user_address') . " WHERE user_id = '$order[user_id]'";
            ...
            FROM " . $this->sanmu->table('wholesale_order_goods') . " AS o
			LEFT JOIN " . $this->sanmu->table('wholesale_products') . " AS p ON p.product_id = o.product_id
			LEFT JOIN " . $this->sanmu->table('wholesale') . " AS w ON w.goods_id = o.goods_id 
			LEFT JOIN " . $this->sanmu->table('wholesale_cat') . " AS c ON w.cat_id = c.cat_id
			LEFT JOIN " . $this->sanmu->table('suppliers') . " AS s ON s.suppliers_id = w.suppliers_id
            ...
            if (isset($_GET['print'])) {
                ...
                $this->smarty->assign('shop_url', $this->sanmu->url());
            ...
            elseif (isset($_GET['shipping_print'])) {
                ...
                $this->smarty->assign('shop_address', $store['shop_address']);
                $this->smarty->assign('service_phone', $store['kf_tel']);
                $shipping = $this->db->getRow("SELECT * FROM " . $this->sanmu->table("shipping_tpl") . " WHERE shipping_id = '" . $order['shipping_id'] . "' and ru_id='" . $order['ru_id'] . "'");
                //打印单模式
                if ($shipping['print_model'] == 2) {
                    ...
                    $sql = "select oi.ru_id from " . $GLOBALS['sanmu']->table('order_info') . " as oi " .
                        " where oi.order_id = '" . $order['order_id'] . "'";
                    $ru_id = $GLOBALS['db']->getOne($sql);

                    if ($ru_id > 0) {
                        $sql = "select shoprz_brandName, shopNameSuffix from " . $GLOBALS['sanmu']->table('merchants_shop_information') . " where user_id = '$ru_id'";
        ...
        elseif ($_REQUEST['act'] == 'get_goods_info') {
            ...
            $sql = "SELECT o.*, w.goods_thumb, w.goods_sn, w.brand_id, w.goods_number AS storage, o.goods_attr, oi.order_sn " .
                "FROM " . $this->sanmu->table('wholesale_order_goods') . " AS o " .
                "LEFT JOIN " . $this->sanmu->table('wholesale') . " AS w ON w.goods_id = o.goods_id " .
                "LEFT JOIN " . $this->sanmu->table('wholesale_order_info') . " AS oi ON oi.order_id = o.order_id " .
        ...
        elseif ($_REQUEST['act'] == 'operate') {
            ...
            if (isset($_POST['remove'])) {
                foreach ($order_id_list as $id_order) {
                    ...
                    if ($order['order_status'] == 0 && $order['pay_status'] == 0 && $order['shipping_status'] == 0) {
                        $this->db->query("DELETE FROM " . $this->sanmu->table('wholesale_order_info') . " WHERE order_id = '$order[order_id]'");
                        $this->db->query("DELETE FROM " . $this->sanmu->table('wholesale_order_goods') . " WHERE order_id = '$order[order_id]'");
                        $this->db->query("DELETE FROM " . $this->sanmu->table('wholesale_order_action') . " WHERE order_id = '$order[order_id]'");

                        $this->db->query("DELETE FROM " . $this->sanmu->table('wholesale_order_info') . " WHERE order_id = '$order[main_order_id]'");
                        $this->db->query("DELETE FROM " . $this->sanmu->table('wholesale_order_goods') . " WHERE order_id = '$order[main_order_id]'");
                        $this->db->query("DELETE FROM " . $this->sanmu->table('wholesale_order_action') . " WHERE order_id = '$order[main_order_id]'");
                ...
                foreach ($order_sn_list as $order_sn) {
                    ...
                    if ($order['ru_id'] > 0) {
                        ...
                        if ($order_child > 0) {
                            ...
                            $seller_shopinfo = $this->db->getRow("SELECT shop_address ,kf_tel FROM" . $this->sanmu->table("seller_shopinfo") . " WHERE ru_id = '$order[ru_id]' LIMIT 1");//获取商家地址，电话
                    ...
                    $sql = "SELECT agency_id FROM " . $this->sanmu->table('admin_user') . " WHERE user_id = '" . session('admin_id') . "'";
                    ...
                    $sql = "SELECT o.*, c.measure_unit, g.goods_unit, g.goods_number AS storage, o.goods_attr, IFNULL(b.brand_name, '') AS brand_name, g.bar_code " .
                        ...
                        'LEFT JOIN ' . $GLOBALS['sanmu']->table('category') . ' AS c ON g.cat_id = c.cat_id ' .
                        "WHERE o.order_id = '$order[order_id]' ";
                    $res = $this->db->getAll($sql);
            ...
            if ($order_id > 0) {
                $sql = "SELECT pay_status FROM" . $this->sanmu->table('wholesale_order_info') . "WHERE order_id = '$order_id'";
                ...
                if ($pay_status == 2) {
                    ...
                    $sql = "SELECT log_id FROM" . $this->sanmu->table('pay_log') . "WHERE order_id = '$order_id' AND order_type = '" . PAY_WHOLESALE . "'";
        ...
        elseif ($_REQUEST['act'] == 'delivery_info') {
            ...
            /* 取得发货单商品 */
            $goods_sql = "SELECT dg.*, g.goods_thumb, g.brand_id  FROM " . $this->sanmu->table('wholesale_delivery_goods') .
                " AS dg LEFT JOIN " . $this->sanmu->table('wholesale') . " as g ON dg.goods_id = g.goods_id " .
            ...
            /* 取得订单操作记录 */
            $act_list = array();
            $sql = "SELECT * FROM " . $this->sanmu->table('wholesale_order_action') . " WHERE order_id = '" . $delivery_order['order_id'] . "' AND action_place = 1 ORDER BY log_time DESC,action_id DESC";
            $res = $this->db->getAll($sql);
    ...
    private function wholesale_order_list()
    {
        ...
        if ($result === false) {
            ...
            if ($adminru['suppliers_id'] > 0) {
                $where .= " AND (SELECT og.suppliers_id FROM " . $GLOBALS['sanmu']->table('wholesale_order_goods') . ' as og' . " WHERE og.order_id = o.order_id LIMIT 1) = '" . $adminru['suppliers_id'] . "' ";
            }

            if ($filter['source'] == 'start' || $adminru['ru_id'] > 0 || $filter['keywords']) {
                $no_main_order = " and (select count(*) from " . $GLOBALS['sanmu']->table('wholesale_order_info') . " as oi2 where oi2.main_order_id = o.order_id) = 0 ";  //主订单下有子订单时，则主订单不显示
            }
            ...
            if ($filter['user_name']) {
                $where .= " AND (SELECT u.user_id FROM " . $GLOBALS['sanmu']->table('users') . " AS u WHERE u.user_name LIKE '%" . mysql_like_quote($filter['user_name']) . "%' LIMIT 1) = o.user_id";
            }
            ...
            if (empty($filter['start_take_time']) || empty($filter['end_take_time'])) {
                if ($adminru['suppliers_id'] == 0) {
                    $where_store = " AND (SELECT COUNT(*) FROM " . $GLOBALS['sanmu']->table('wholesale_order_goods') . " AS og " . " WHERE o.order_id = og.order_id LIMIT 1) > 0 " .
                        " AND (SELECT COUNT(*) FROM " . $GLOBALS['sanmu']->table('wholesale_order_info') . " AS oi2 WHERE oi2.main_order_id = o.order_id) = 0";
                }
            }

            /* 记录总数 */
            if (!empty($filter['start_take_time']) || !empty($filter['end_take_time'])) {
                $sql = "SELECT o.order_id FROM " . $GLOBALS['sanmu']->table('wholesale_order_info') . " AS o " .
                    $leftJoin .
                    "LEFT JOIN " . $GLOBALS['sanmu']->table('wholesale_order_action') . " AS oa ON o.order_id = oa.order_id " .
                    $where . $where_store . $no_main_order . " GROUP BY o.order_id";

                $record_count = count($GLOBALS['db']->getAll($sql));
            } elseif (!empty($filter['keywords'])) {
                $leftJoin .= " LEFT JOIN " . $GLOBALS['sanmu']->table('wholesale_order_goods') . " AS iog ON iog.order_id = o.order_id ";
                ...
                $sql = "SELECT COUNT(*) FROM " . $GLOBALS['sanmu']->table('wholesale_order_info') . " AS o " .
                    $leftJoin .
            ...
            if (!empty($filter['keywords']) && empty($filter['user_name'])) {
                ...
                $groupBy = " GROUP BY o.order_id ";
                $leftJoin .= " LEFT JOIN " . $GLOBALS['sanmu']->table('wholesale_order_action') . " AS oa ON o.order_id = oa.order_id ";
            }
            ...
            $sql = "SELECT o.order_id, o.main_order_id, o.order_sn, o.add_time, o.order_status," .
                ...
                " FROM " . $GLOBALS['sanmu']->table('wholesale_order_info') . " AS o " .
                $leftJoin .
        ...
        foreach ($row as $key => $value) {
            $row[$key]['pay_name'] = $GLOBALS['db']->getOne("SELECT pay_name FROM" . $GLOBALS['sanmu']->table('payment') . "WHERE pay_id = '" . $value['pay_id'] . "'");
            $row[$key]['pay_time'] = local_date($GLOBALS['_CFG']['time_format'], $value['pay_time']);
            //查会员名称
            $sql = " SELECT user_name FROM " . $GLOBALS['sanmu']->table('users') . " WHERE user_id = '" . $value['user_id'] . "'";
    ...
    private function wholesale_delivery_list()
    {
        ...
        if ($result === false) {
            ...
            if ($filter['goods_id']) {
                $where .= " AND (SELECT dg.goods_id FROM " . $GLOBALS['sanmu']->table('delivery_goods') . " AS dg WHERE dg.delivery_id = do.delivery_id LIMIT 1) = '" . $filter['goods_id'] . "' ";
            }
            if ($filter['consignee']) {
            ...
            /* 记录总数 */
            $sql = "SELECT COUNT(DISTINCT delivery_id) FROM " . $GLOBALS['sanmu']->table('wholesale_delivery_order') . " as do " . $where;
            ...
            $sql = "SELECT DISTINCT do.delivery_id, do.delivery_sn, do.order_sn, do.order_id, do.add_time, do.action_user, do.consignee, do.country,
                ...
                FROM " . $GLOBALS['sanmu']->table("wholesale_delivery_order") . " as do " .
                " LEFT JOIN " . $GLOBALS['sanmu']->table('suppliers') . " AS s ON s.suppliers_id = do.suppliers_id " .
                $where .
                " ORDER BY " . $filter['sort_by'] . " " . $filter['sort_order'] . "
                LIMIT " . ($filter['page'] - 1) * $filter['page_size'] . ", " . $filter['page_size'] . " ";
            set_filter($filter, $sql);
    ...
    private function wholesale_delivery_info($delivery_id, $delivery_sn = '')
    {
        ...
        if ($delivery) {
            ...
            if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                $delivery['mobile'] = $this->sanmuRepository->stringToStar($delivery['mobile']);
                $delivery['tel'] = $this->sanmuRepository->stringToStar($delivery['tel']);
                $delivery['email'] = $this->sanmuRepository->stringToStar($delivery['email']);
            }
