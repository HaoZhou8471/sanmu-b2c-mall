...
class SuppliersController extends InitController
{
    ...
    {
        /* 初始化 $exc 对象 */
        $exc = new Exchange($this->sanmu->table("admin_user"), $this->db, 'user_id', 'user_name');
        $exc_sup = new Exchange($this->sanmu->table("suppliers"), $this->db, 'suppliers_id', 'suppliers_name');
        ...
        elseif ($_REQUEST['act'] == 'remove') {
            ...
            if ($suppliers['suppliers_id']) {
                /* 判断供货商是否存在订单 */
                $sql = "SELECT COUNT(*)
                FROM " . $this->sanmu->table('order_info') . "AS O, " . $this->sanmu->table('order_goods') . " AS OG, " . $this->sanmu->table('goods') . " AS G
                ...
                $sql = "DELETE FROM " . $this->sanmu->table('suppliers') . "
            WHERE suppliers_id = '$id'";
                $this->db->query($sql);
                ...
                foreach ($table_array as $value) {
                    $sql = "DELETE FROM " . $this->sanmu->table($value) . " WHERE suppliers_id = '$id'";
        ...
        elseif ($_REQUEST['act'] == 'is_check') {
            ...
            if ($suppliers['suppliers_id']) {
                ...
                $this->db->autoExecute($this->sanmu->table('suppliers'), $_suppliers, '', "suppliers_id = '$id'");
                clear_cache_files();
        ...
        elseif ($_REQUEST['act'] == 'batch') {
            ...
            if (empty($_POST['checkboxes'])) {
                ...
                WHERE suppliers_id " . db_create_in($ids);
                    ...
                    $table_array = array('admin_user', 'delivery_order', 'back_order');
                    foreach ($table_array as $value) {
                        $sql = "DELETE FROM " . $this->sanmu->table($value) . " WHERE suppliers_id " . db_create_in($ids) . " ";
                        $this->db->query($sql, 'SILENT');
                    }
        ...
        elseif ($_REQUEST['act'] == 'edit') {
            ...
            $sql = "SELECT s.*, au.user_name FROM " . $this->sanmu->table('suppliers') . " AS s LEFT JOIN " . $this->sanmu->table('admin_user') . " AS au ON s.suppliers_id = au.suppliers_id WHERE s.suppliers_id = '$id'";
            ...
            if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0 && $suppliers) {
                $suppliers['real_name'] = $this->sanmuRepository->stringToStar($suppliers['real_name']);
                $suppliers['mobile_phone'] = $this->sanmuRepository->stringToStar($suppliers['mobile_phone']);
                $suppliers['email'] = $this->sanmuRepository->stringToStar($suppliers['email']);
                $suppliers['user_name'] = $this->sanmuRepository->stringToStar($suppliers['user_name']);
            }
        ...
        elseif ($_REQUEST['act'] == 'update') {
            ...
            $sql = " SELECT user_id FROM " . $this->sanmu->table('suppliers') . " WHERE suppliers_id = '$suppliers_id' ";
            $user_id = $this->db->getOne($sql);

            /* 判断名称是否重复 */
            $sql = "SELECT suppliers_id FROM " . $this->sanmu->table('suppliers') . "  WHERE suppliers_name = '" . $user_name . "' AND suppliers_id <> '$suppliers_id'";
            ...
            if ($review_status == 3) {
                ...
                $sql = " SELECT user_name FROM " . $this->sanmu->table('admin_user') . " WHERE suppliers_id = '$suppliers_id' AND parent_id = '0' ";
            ...
            if ($review_status == 3 && $suppliers_id) {//插入admin_user表数据
                ...
                $sql = " SELECT user_id, user_name, password FROM " . $this->sanmu->table('admin_user') . " WHERE ru_id = '$user_id' AND suppliers_id = '$suppliers_id' ";
                ...
                if ($row['user_id']) {
                    if (($admin_user['user_name'] != $row['user_name']) || (isset($admin_user['password']) && $admin_user['password'] != $row['password'])) {
                        ...
                        $this->db->autoExecute($this->sanmu->table('admin_user'), $admin_user, 'UPDATE', "user_id = '" . $row['user_id'] . "'");
                    }
                } else {
                    ...
                    $this->db->autoExecute($this->sanmu->table('admin_user'), $admin_user, 'INSERT');
                }
            ...
            /* 保存供货商信息 */
            $this->db->autoExecute($this->sanmu->table('suppliers'), $suppliers, 'UPDATE', "suppliers_id = '$suppliers_id'");
        ...
        elseif ($_REQUEST['act'] == 'allot') {
            ...
            $user_id = $this->db->getOne(" SELECT user_id FROM " . $this->sanmu->table('suppliers') . " WHERE suppliers_id = '$suppliers_id' ");
            /* 获得该管理员的权限 */
            $priv_str = $this->db->getOne("SELECT action_list FROM " . $this->sanmu->table('admin_user') . " WHERE ru_id = '$user_id' AND suppliers_id = '$suppliers_id'");
            ...
            /* 获取权限的分组数据 */
            $sql_query = "SELECT action_id, parent_id, action_code,relevance FROM " . $this->sanmu->table('admin_action') .
            ...
            if ($priv_arr) {
                /* 按权限组查询底级的权限名称 */
                $sql = "SELECT action_id, parent_id, action_code,relevance FROM " . $this->sanmu->table('admin_action') .
        ...
        elseif ($_REQUEST['act'] == 'update_allot') {
            ...
            /* 取得当前管理员用户名 */
            $admin_name = $this->db->getOne("SELECT user_name FROM " . $this->sanmu->table('admin_user') . " WHERE ru_id = '$ru_id' AND suppliers_id = '$suppliers_id' ");
            /* 更新管理员的权限 */
            $act_list = @join(",", $_POST['action_code']);
            $sql = "UPDATE " . $this->sanmu->table('admin_user') . " SET action_list = '$act_list', role_id = '' " .
    ...
    private function suppliers_list()
    {
        ...
        if ($result === false) {
            ...
            /* 记录总数 */
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['sanmu']->table('suppliers') . $where;
            ...
            $sql = "SELECT suppliers_id, suppliers_name, suppliers_desc, is_check, real_name, company_name, add_time, review_status, region_id, user_id,suppliers_percent
                FROM " . $GLOBALS['sanmu']->table("suppliers") . $where .
        ...
        if ($res) {
            foreach ($res as $k => $v) {
                ...
                $res[$k]['user_name'] = $GLOBALS['db']->getOne(" SELECT user_name FROM " . $GLOBALS['sanmu']->table('admin_user') . " WHERE parent_id = '0' AND suppliers_id = '" . $v['suppliers_id'] . "' LIMIT 1 ");
                $res[$k]['region_name'] = $GLOBALS['db']->getOne(" SELECT region_name FROM " . $GLOBALS['sanmu']->table('region') . " WHERE region_id = '" . $v['region_id'] . "' ");

                if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                    $res[$k]['real_name'] = $this->sanmuRepository->stringToStar($v['real_name']);
                }
            }
        }
