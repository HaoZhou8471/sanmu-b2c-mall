...
class UpgradeController extends InitController
{
    ...
    public function index(Request $request)
    {
        ...
        $sanmu_version = ShopConfig::where('code', 'sanmu_version')->value('value');
        $sanmu_version = $sanmu_version ? $sanmu_version : '';

        // 当前版本
        $current_version = $sanmu_version;
    ...
    protected function patchList($current_version)
    {
        ...
        $metadata = sanmu_decode($content, true);
    ...
    protected function upgrade($version)
    {
        ...
        // 更新版本到数据库
        ShopConfig::where('code', 'sanmu_version')->update([
            'value' => $version
        ]);
    }
