...
class TemplateMallController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'confirm_operation') {
            ...
            //获取订单信息
            $sql = "SELECT ru_id,temp_id,temp_code FROM" . $this->sanmu->table("seller_template_apply") . "WHERE apply_id = '$apply_id'";
            ...
            //更新模板使用数量
            $sql = "UPDATE" . $this->sanmu->table('template_mall') . "SET sales_volume = sales_volume+1 WHERE temp_id = '" . $seller_template_apply['temp_id'] . "'";
            $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'remove') {
            $apply_id = !empty($_REQUEST['id']) ? intval($_REQUEST['id']) : 0;
            $sql = "DELETE FROM" . $this->sanmu->table('seller_template_apply') . "WHERE apply_id = '$apply_id' AND pay_status = 0";
            $this->db->query($sql);
