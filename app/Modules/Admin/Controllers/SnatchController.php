...
class SnatchController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'add') {
            ...
            /* 检查商品是否存在 */
            $sql = "SELECT goods_name FROM " . $this->sanmu->table('goods') . " WHERE goods_id = '$_POST[goods_id]'";
        ...
        elseif ($_REQUEST['act'] == 'update') {
            ...
            if (empty($_POST['goods_id'])) {
                ...
                $_POST['goods_name'] = $this->db->getOne("SELECT goods_name FROM " . $this->sanmu->table('goods') . "WHERE goods_id= '" . $_POST['goods_id'] . "'");
            }
            if (empty($_POST['start_price'])) {
                $_POST['start_price'] = 0;
            }
            ...
            $this->db->autoExecute($this->sanmu->table('goods_activity'), $record, 'UPDATE', "act_id = '$act_id' AND act_type = " . GAT_SNATCH);
        ...
        elseif ($_REQUEST['act'] == 'batch') {
            ...
            if (isset($_POST['type'])) {
                ...
                elseif ($_POST['type'] == 'review_to') {
                    ...
                    $sql = "UPDATE " . $this->sanmu->table('goods_activity') . " SET review_status = '$review_status' "
                        . " WHERE act_id " . db_create_in($ids);
    ...
    private function get_snatch_detail()
    {
        ...
        /* 获得记录总数以及总页数 */
        $sql = "SELECT count(*) FROM " . $this->sanmu->table('snatch_log') . $where;
        $filter['record_count'] = $this->db->getOne($sql);
        ...
        $sql = "SELECT s.log_id, u.user_name, s.bid_price, s.bid_time " .
            " FROM " . $this->sanmu->table('snatch_log') . " AS s " .
            " LEFT JOIN " . $this->sanmu->table('users') . " AS u ON s.user_id = u.user_id  " . $where .
