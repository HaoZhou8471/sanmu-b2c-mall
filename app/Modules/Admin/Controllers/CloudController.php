...
class CloudController extends InitController
{
    public function index()
    {
        ...
        $data['shop_url'] = urlencode($this->sanmu->url());
        $data['admin_url'] = urlencode($this->sanmu->url() . ADMIN_PATH);
        ...
        if ($act == 'menu_api') {
                ...
                if (!empty($api_str)) {
                    ...
                    if (!empty($api_arr) && $api_arr['error'] == 0 && md5($api_arr['content']) == $api_arr['hash']) {
                        ...
                        if ($data['charset'] != 'UTF-8') {
                            $api_arr['content'] = sanmu_iconv('UTF-8', $data['charset'], $api_arr['content']);
                        }
            ...
            if (!empty($_GET['link'])) {
                ...
                if (!empty($url['host'])) {
                    return sanmu_header("Location: " . $url['scheme'] . "://" . $url['host'] . $url['path'] . "?" . $url['query'] . $query . "\n");
                }
            }
