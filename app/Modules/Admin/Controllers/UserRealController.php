...
class UserRealController extends InitController
{
    ...
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            $sql = "SELECT ur.*, u.user_name, u.user_id FROM " . $this->sanmu->table('users_real') . " AS ur "
                . " JOIN " . $this->sanmu->table('users') . " AS u ON ur.user_id = u.user_id "
            ...
            if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0 && isset($user_real_info['user_name'])) {
                $user_real_info['bank_mobile'] = $this->sanmuRepository->stringToStar($user_real_info['bank_mobile']);
                $user_real_info['user_name'] = $this->sanmuRepository->stringToStar($user_real_info['user_name']);
            }
            ...
            if ($user_id > 0) {
                $sql = "SELECT real_id FROM " . $this->sanmu->table('users_real') . " WHERE user_id = '$user_id' AND user_type = '$user_type'";
                $real_id = $this->db->getOne($sql);
                if ($real_id) {
                    $this->db->autoExecute($this->sanmu->table('users_real'), $post_user_real, 'UPDATE', "real_id = '$real_id'");
                    ...
                    $this->db->autoExecute($this->sanmu->table('users_real'), $post_user_real, 'INSERT');
        ...
        elseif ($_REQUEST['act'] == 'batch') {
            ...
            if (isset($_POST['type'])) {
                ...
                if ($_POST['type'] == 'batch_remove') {
                    $sql = "DELETE FROM " . $this->sanmu->table('users_real') .
                ...
                elseif ($_POST['type'] == 'review_to') {
                    ...
                    $sql = "UPDATE " . $this->sanmu->table('users_real') . " SET review_status = '$review_status', review_content = '$review_content', review_time = '$time' "
                        . " WHERE real_id " . db_create_in($real_id_arr);
        ...
        elseif ($_REQUEST['act'] == 'remove') {
            ...
            if ($real_id > 0) {
                $sql = "DELETE FROM " . $this->sanmu->table('users_real') . " WHERE real_id = '$real_id'";
                if ($this->db->query($sql)) {
                    /* 记录管理员操作 */
    ...
    private function users_real_list()
    {
        ...
        if ($result === false) {
            ...
            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('users_real') . "AS ur "
                . " JOIN " . $this->sanmu->table('users') . " AS u ON ur.user_id = u.user_id " . $ex_where;
            $filter['record_count'] = $this->db->getOne($sql);

            /* 分页大小 */
            $filter = page_and_size($filter);
            $sql = "SELECT ur.*,u.user_name " .
                " FROM " . $this->sanmu->table('users_real') . "as ur "
                . " JOIN " . $this->sanmu->table('users') . " AS u ON ur.user_id = u.user_id "
                . $ex_where .
        ...
        for ($i = 0; $i < count($users_real_list); $i++) {
            ...
            if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                $users_real_list[$i]['bank_mobile'] = $this->sanmuRepository->stringToStar($users_real_list[$i]['bank_mobile']);
                $users_real_list[$i]['user_name'] = $this->sanmuRepository->stringToStar($users_real_list[$i]['user_name']);
            }
        }
