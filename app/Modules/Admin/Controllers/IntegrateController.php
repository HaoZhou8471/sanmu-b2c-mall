...
class IntegrateController extends InitController
{
     ...
     * @param SanmuRepository $sanmuRepository
     */
    ...
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'list') {
            $modules = $this->sanmuRepository->readModules(plugin_path('Integrates'));
            for ($i = 0; $i < count($modules); $i++) {
                $modules[$i]['code'] = $modules[$i]['code'] === 'passport' ? 'sanmumall' : $modules[$i]['code'];
            ...
            $allow_set_points = 0; // $GLOBALS['_CFG']['integrate_code'] == 'sanmumall' ? 0 : 1;
        ...
        if ($_REQUEST['act'] == 'install') {
            ...
            if ($_GET['code'] == 'sanmumall' || $_GET['code'] == 'ecjia') {
        ...
        if ($_REQUEST['act'] == 'save_uc_config_first') {
            ...
            $data = $this->db->getRow("SHOW TABLE STATUS LIKE '" . $GLOBALS['sanmu']->prefix . 'users' . "'");
