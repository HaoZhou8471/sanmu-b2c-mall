...
class UserAccountManageController extends InitController
{
    ...
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            $sql = " SELECT IFNULL(SUM(user_money), 0) AS user_money, IFNULL(SUM(frozen_money), 0) AS frozen_money FROM " .
                $this->sanmu->table('account_log') . " WHERE `change_time` >= " . $start_date . " AND `change_time` < " . ($end_date + 86400);
            ...
            $sql = "SELECT IFNULL(SUM(o.surplus), 0) AS surplus, IFNULL(SUM(o.integral_money), 0) AS integral_money FROM " .
                $this->sanmu->table('order_info') . " AS o WHERE 1 AND o.main_count = 0 AND o.add_time >= " . $start_date . " AND o.add_time < " . ($end_date + 86400);
            $money_list = $this->db->getRow($sql);
    ...
    {
        $sql = " SELECT IFNULL(SUM(amount), 0) AS total_amount FROM " . $this->sanmu->table('user_account') . " AS a, " . $this->sanmu->table('users') . " AS u " .
    ...
    private function order_list()
    {
        ...
        if ($result === false) {
            ...
            $filter['record_count'] = $this->db->getOne("SELECT COUNT(*) FROM " . $this->sanmu->table('order_info') . " AS o, " . $this->sanmu->table('users') . " AS u " . $ex_where);

            /* 分页大小 */
            $filter = page_and_size($filter);

            $sql = "SELECT o.order_id, o.order_sn, u.user_name, o.surplus, o.integral_money, o.add_time FROM " .
                $this->sanmu->table('order_info') . " AS o," . $this->sanmu->table('users') . " AS u " . $ex_where .
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                $order_list[$i]['user_name'] = $this->sanmuRepository->stringToStar($order_list[$i]['user_name']);
            }
        }
