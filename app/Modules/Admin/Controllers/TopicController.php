...
class TopicController extends InitController
{
    ...
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit') {
            ...
            if (!$isadd) {
                $sql = "SELECT * FROM " . $this->sanmu->table('topic') . " WHERE topic_id = '$topic_id'";
                ...
                if ((isset($file['error']) && $file['error'] == 0) || (!isset($file['error']) && $file['tmp_name'] != 'none')) {
                    ...
                    if (!check_file_type($file['tmp_name'], $file['name'], $allow_file_types)) {
                        ...
                        if (move_upload_file($file['tmp_name'], $file_path)) {
                            ...
                            $this->sanmuRepository->getOssAddFile([$file_name]);
                            ...
                            if ($bgtype) {
                                ...
                                $sql = "SELECT id ,img_file FROM" . $this->sanmu->table('templates_left') . " WHERE ru_id = '" . $tem_RuId . "' AND seller_templates = '$suffix' AND type = '$bgtype' AND theme = '$theme'";
                                ...
                                if ($templates_left['id'] > 0) {
                                    ...
                                    $sql = "UPDATE" . $this->sanmu->table('templates_left') . " SET img_file = '$content_file' WHERE ru_id = '" . $tem_RuId . "' AND seller_templates = '$suffix' AND id='" . $templates_left['id'] . "' AND type = '$bgtype' AND theme = '$theme'";
                                    $this->db->query($sql);
                                } else {
                                    $sql = "INSERT INTO" . $this->sanmu->table('templates_left') . " (`ru_id`,`seller_templates`,`img_file`,`type`,`theme`) VALUES ('" . $tem_RuId . "','$suffix','$content_file','$bgtype','$theme')";
                                    $this->db->query($sql);
                                }
                            }
        ...
        elseif ($_REQUEST['act'] == 'generate') {
            ...
            $sql = "SELECT id  FROM" . $this->sanmu->table('templates_left') . " WHERE ru_id = '" . $tem_RuId . "' AND seller_templates = '$suffix' AND type='$type' AND theme = '$theme'";
            $id = $this->db->getOne($sql);
            if ($id > 0) {
                $sql = "UPDATE " . $this->sanmu->table('templates_left') . " SET seller_templates = '$suffix',bg_color = '$bg_color' ,if_show = '$is_show',bgrepeat='$bgshow',align= '$bgalign',type='$type' WHERE ru_id = '" . $tem_RuId . "' AND seller_templates = '$suffix' AND id='$id' AND type='$type' AND theme = '$theme'";
            } else {
                $sql = "INSERT INTO " . $this->sanmu->table('templates_left') . " (`ru_id`,`seller_templates`,`bg_color`,`if_show`,`bgrepeat`,`align`,`type`) VALUES ('" . $tem_RuId . "','$suffix','$bg_color','$is_show','$bgshow','$bgalign','$type')";
            }
        ...
        elseif ($_REQUEST['act'] == 'remove_img') {
            ...
            $sql = "UPDATE " . $this->sanmu->table('templates_left') . " SET img_file = '' WHERE ru_id = '" . $tem_RuId . "' AND type = '$type' AND seller_templates = '$suffix' AND theme = '$theme'";
            $this->db->query($sql);
        }
        ...
        elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
            ...
            if ($is_insert) {
                ...
                $this->db->AutoExecute($this->sanmu->table('topic'), $record, 'INSERT');
            } else {
                ...
                $this->db->AutoExecute($this->sanmu->table('topic'), $record, 'UPDATE', "topic_id='$topic_id'");
            }
        ...
        elseif ($_REQUEST['act'] == 'batch') {
            ...
            if (isset($_POST['type'])) {
                ...
                if ($_POST['type'] == 'batch_remove') {
                    ...
                    $this->sanmuRepository->getDelBatch($_POST['checkboxes'], $id, ['topic_img', 'title_pic'], 'topic_id', Topic::whereRaw(1), 1);
                    $sql = "DELETE FROM " . $this->sanmu->table('topic') . " WHERE ";
                ...
                elseif ($_POST['type'] == 'review_to') {
                    ...
                    $sql = "UPDATE " . $this->sanmu->table('topic') . " SET review_status = '$review_status' "
    ...
    private function get_topic_list()
    {
        ...
        if ($result === false) {
            ...
            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('topic') . " AS t " . " WHERE $where";
            $filter['record_count'] = $this->db->getOne($sql);

            /* 分页大小 */
            $filter = page_and_size($filter);

            $sql = "SELECT t.* FROM " . $this->sanmu->table('topic') . " AS t " . " WHERE $where ORDER BY $filter[sort_by] $filter[sort_order]";
        ...
        foreach ($query as $topic) {
            $topic['start_time'] = local_date('Y-m-d H:i:s', $topic['start_time']);
            $topic['end_time'] = local_date('Y-m-d H:i:s', $topic['end_time']);
            $topic['url'] = $this->sanmu->url() . 'topic.php?topic_id=' . $topic['topic_id'];
