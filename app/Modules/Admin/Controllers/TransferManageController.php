...
class TransferManageController extends InitController
{
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'sql_basic') {
            ...
            $basic = sanmu_decode($_REQUEST['basic']);
        ...
        elseif ($_REQUEST['act'] == 'is_ajax') {
            ...
            if (isset($_GET['start'])) {
                ...
                if ($s_db_retain == 0) {
                    $arr = explode(",", $table);
                    foreach ($arr as $vv) {
                        $this->db->query("TRUNCATE TABLE " . $this->sanmu->table($vv));
                    }
                }
    ...
    private function supplier_batch($table, $page = 1, $page_size = 10, $silent = true, $s_db_retain = 0)
    {
        ...
        foreach ($res as $row) {
            if ($s_db_retain == 0) {
                $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('admin_user') . " WHERE ru_id = '$row[user_id]'";
                if ($this->db->getOne($sql) > 0) {
                    $sql = "DELETE FROM " . $this->sanmu->table('admin_user') . " WHERE ru_id = '$row[user_id]'";
                    $this->db->query($sql);
                }
                //ecs_user
                $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('users') . " WHERE user_id = '$row[user_id]'";
                if ($this->db->getOne($sql) == 0) {
                    continue;
                }
                //supplier_admin_user
                ...
                $sql = "INSERT INTO " . $this->sanmu->table('admin_user') . "(user_name, ru_id, email, password, ec_salt, add_time, last_login, last_ip," .
            ...
            $sql = "INSERT INTO " . $this->sanmu->table('merchants_shop_information') . "(shop_id, user_id, rz_shopName, steps_audit, merchants_audit) " .
                "VALUES ('$row[supplier_id]', '$row[user_id]', '$row[supplier_name]', 1, '$row[status]')";
            $this->db->query($sql);

            //merchants_steps_fields
            $sql = "INSERT INTO " . $this->sanmu->table('merchants_steps_fields') . "(user_id, agreement, contactName, contactPhone, contactEmail," .
        ...
        foreach ($res as $row) {
            ...
            foreach ($field_list as $field) {
                ...
                if ($table == 'goods' && $field == 'user_id') {
                    $table_field = get_table_file_name($this->sanmu->table('suppliers'), 'user_id');
                    if ($table_field['bool']) {
            ...
            $result = $this->db->autoExecute($this->sanmu->table($table), $field_arr, 'INSERT');
