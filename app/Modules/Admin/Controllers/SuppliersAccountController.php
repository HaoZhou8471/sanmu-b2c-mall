...
class SuppliersAccountController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'update_check') {
            ...
            if ($log_status > 0 && $log_status <= 2) {
                if ($log_info['log_type'] == 5) {
                    ...
                    if ($log_status == 1) {
                        /* 改变商家金额 */
                        $sql = " UPDATE " . $this->sanmu->table('suppliers') . " SET suppliers_money = suppliers_money + " . $log_info['frozen_money'] . " WHERE suppliers_id = '" . $log_info['suppliers_id'] . "'";
                        $this->db->query($sql);
                    ...
                    /* 改变商家资金记录 */
                    $sql = " UPDATE " . $this->sanmu->table('suppliers_account_log_detail') . " SET is_paid = $log_status, admin_note = '$log_reply', log_type = '$log_type' WHERE log_id = '$log_id'";
                    $this->db->query($sql);
                } else {
                    if ($log_info['suppliers_frozen'] < $log_info['amount'] && isset($log_info['payment_info']['pay_code']) && $log_info['payment_info']['pay_code'] != 'bank') {
                        ...
                        if (isset($log_info['payment_info']['pay_code']) && $log_info['payment_info']['pay_code'] == 'bank') {
                            ...
                            /* 改变商家金额 */
                            $sql = " UPDATE " . $this->sanmu->table('suppliers') . " SET suppliers_money = suppliers_money + " . $log_info['amount'] . " WHERE suppliers_id = '" . $log_info['suppliers_id'] . "'";
                            $this->db->query($sql);
                            ...
                            if ($log_info['deposit_mode'] == 1) {
                                /* 改变会员金额 */
                                $sql = " UPDATE " . $this->sanmu->table('users') . " SET user_money = user_money + " . $log_info['amount'] . " WHERE user_id = '" . $log_info['ru_id'] . "'";
                                $this->db->query($sql);
                            }
                            /* 转账至前台会员余额账户 end */

                            /* 改变商家金额 */
                            $sql = " UPDATE " . $this->sanmu->table('suppliers') . " SET frozen_money = frozen_money - " . $log_info['amount'] . " WHERE suppliers_id = '" . $log_info['suppliers_id'] . "'";
                            $this->db->query($sql);
                            ...
                            if ($log_info['deposit_mode'] == 1) {
                                ...
                                $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table('account_log'), $user_account_log, 'INSERT');
                            }
                            /* 转账至前台会员余额账户 end */
                        ...
                        /* 改变会员金额 */
                        $sql = " UPDATE " . $this->sanmu->table('suppliers_account_log_detail') . " SET is_paid = '$log_status', $update_set admin_note = '$log_reply', log_type = '$log_type' WHERE log_id = '$log_id'";
                        $this->db->query($sql);
                    }
                }
            } else {
        ...
        elseif ($_REQUEST['act'] == 'edit_seller') {
            ...
            $sql = "SELECT  sal.suppliers_money, sal.frozen_money, sal.suppliers_id, sal.suppliers_name FROM " . $GLOBALS['sanmu']->table('suppliers') . " AS sal " .
        ...
        elseif ($_REQUEST['act'] == 'insert') {
            ...
            if (!empty($sc_guid) && !empty($sc_rand) && request()->hasCookie('seller_account_cookie')) {
                ...
                $sql = "SELECT suppliers_id, suppliers_money, frozen_money FROM" . $this->sanmu->table("suppliers") . " WHERE suppliers_id = '$suppliers_id' LIMIT 1";
                $suppliers_info = $this->db->getRow($sql);
                ...
                $change_desc = $this->sanmuRepository->subStr($_POST['change_desc'], 255, false);
    ...
    private function get_suppliers_seller_account()
    {
        ...
        if ($result === false) {
            ...
            if ($filter['keywords']) {
                $sql = "SELECT suppliers_id FROM" . $GLOBALS['sanmu']->table('suppliers') . " WHERE (suppliers_name LIKE '%" . mysql_like_quote($filter['keywords']) . "%' OR suppliers_desc LIKE '%" . mysql_like_quote($filter['keywords']) . "%') ";
            ...
            $sql = "SELECT count(*) FROM " . $GLOBALS['sanmu']->table('suppliers') . " AS sal " .
                " $ex_where";
