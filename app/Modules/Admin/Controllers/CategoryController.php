    ...
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'delete_icon') {
            ...
            if (!empty($cat_info)) {
                ...
                sanmu_unlink(storage_public($cat_info['cat_icon']));
                ...
                if ($cat_id > 0) {
                    ...
                    if ($cat['show_in_nav'] == 1) {
                        ...
                        $other = [
                            ...
                            'vieworder' => $vieworder,
                            'opennew' => 0,
                            'url' => $this->sanmuRepository->buildUri('category', ['cid' => $cat_id], $cat['cat_name']),
                            'type' => 'middle'
                        ];
                        Nav::insert($other);
                    }
        ...
        if ($_REQUEST['act'] == 'update') {
            ...
            if (isset($_FILES['cat_icon']) && !empty($_FILES['cat_icon']['name'])) {
                ...
                if ($cat_info['cat_icon']) {
                    sanmu_unlink(storage_public($cat_info['cat_icon']));
                    $this->sanmuRepository->getOssDelFile([$cat_info['cat_icon']]);
                }
            }
            //上传分类菜单图标 by wu end
            ...
            if (isset($_FILES['touch_catads']) && !empty($_FILES['touch_catads']['name'])) {
                ...
                if ($cat_info['touch_catads']) {
                    sanmu_unlink(storage_public($cat_info['touch_catads']));
                    $this->sanmuRepository->getOssDelFile([$cat_info['touch_catads']]);
                }
            }
