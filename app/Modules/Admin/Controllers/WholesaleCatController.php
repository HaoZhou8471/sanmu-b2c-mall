...
class WholesaleCatController extends InitController
{
    ...
    public function index()
    {
        ...
        $exc = new Exchange($this->sanmu->table("wholesale_cat"), $this->db, 'cat_id', 'cat_name');
        ...
        if ($_REQUEST['act'] == 'insert') {
            ...
            /* 入库的操作 */
            if ($this->db->autoExecute($this->sanmu->table('wholesale_cat'), $cat) !== false) {
                $this->db->insert_id();
        ...
        if ($_REQUEST['act'] == 'update') {
            ...
            if ($this->db->autoExecute($this->sanmu->table('wholesale_cat'), $cat, 'UPDATE', "cat_id = '$cat_id'")) {
                clear_cache_files(); // 清除缓存
        ...
        if ($_REQUEST['act'] == 'remove') {
            ...
            $cat_name = $this->db->getOne('SELECT cat_name FROM ' . $this->sanmu->table('wholesale_cat') . " WHERE cat_id = '$cat_id'");

            /* 当前分类下是否有子分类 */
            $cat_count = $this->db->getOne('SELECT COUNT(*) FROM ' . $this->sanmu->table('wholesale_cat') . " WHERE parent_id = '$cat_id'");

            /* 当前分类下是否存在商品 */
            $goods_count = $this->db->getOne('SELECT COUNT(*) FROM ' . $this->sanmu->table('wholesale') . " WHERE cat_id = '$cat_id'");

            /* 如果不存在下级子分类和商品，则删除 */
            if ($cat_count == 0 && $goods_count == 0) {
                /* 删除分类 */
                $sql = 'DELETE FROM ' . $this->sanmu->table('wholesale_cat') . " WHERE cat_id = '$cat_id'";
    ...
    {
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['sanmu']->table('wholesale_cat') .
    ...
    private function cat_update($cat_id, $args)
    {
        ...
        return $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table('presale_cat'), $args, 'update', "cat_id='$cat_id'");
    }
    ...
    {
        $sql = " SELECT cat_id,is_show, cat_name, parent_id, sort_order FROM " . $GLOBALS['sanmu']->table('wholesale_cat') . " WHERE parent_id = '$pid' ";
