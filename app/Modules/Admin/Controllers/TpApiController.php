...
class TpApiController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'kdniao_update') {
            ...
            $sql = " UPDATE " . $this->sanmu->table('shop_config') . " SET value = '$client_id' WHERE code = 'kdniao_client_id' ";
            $this->db->query($sql);
            $sql = " UPDATE " . $this->sanmu->table('shop_config') . " SET value = '$appkey' WHERE code = 'kdniao_appkey' ";
            $this->db->query($sql);

            $sql = " UPDATE " . $GLOBALS['sanmu']->table('shop_config') . " SET value = '$kdniao_account_use' WHERE code = 'kdniao_account_use' ";
        ...
        elseif ($_REQUEST['act'] == 'kdniao_print') {
            ...
            if (!empty($order_sn)) {
                $sql = " SELECT order_id FROM " . $this->sanmu->table('order_info') . " WHERE order_sn " . db_create_in($order_sn);
            ...
            //判断快递是否一样
            $sql = " SELECT shipping_id FROM " . $this->sanmu->table('order_info') . " WHERE order_id " . db_create_in($order_ids);
            ...
            if ($order_ids && $order_ids[0]) {
                ...
                foreach ($order_ids as $order_id) {
                    ...
                    if (isset($result['Order']['LogisticCode'])) {
                        $sql = " UPDATE " . $this->sanmu->table('order_info') . " SET invoice_no = '{$result['Order']['LogisticCode']}' WHERE order_id = '$order_id' ";
                        $this->db->query($sql);
                    }
                }
            }
        ...
        elseif ($_REQUEST['act'] == 'order_print_setting_remove') {
            ...
            $exc = new Exchange($this->sanmu->table("order_print_setting"), $this->db, 'id', 'ru_id');
            $exc->drop($id);
            //clear_cache_files();
        ...
        elseif ($_REQUEST['act'] == 'edit_order_printer') {
            ...
            $sql = " UPDATE " . $this->sanmu->table('order_print_setting') . " SET printer = '$val' WHERE ru_id = '{$adminru['ru_id']}' AND id = '$id' ";
            $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'edit_print_width') {
            ...
            $sql = " UPDATE " . $this->sanmu->table('order_print_setting') . " SET width = '$val' WHERE ru_id = '{$adminru['ru_id']}' AND id = '$id' ";
            $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'edit_sort_order') {
            ...
            $sql = " UPDATE " . $this->sanmu->table('order_print_setting') . " SET sort_order = '$val' WHERE ru_id = '{$adminru['ru_id']}' AND id = '$id' ";
            $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'toggle_order_is_default') {
            ...
            $sql = " UPDATE " . $this->sanmu->table('order_print_setting') . " SET is_default = '$val' WHERE ru_id = '{$adminru['ru_id']}' AND id = '$id' ";
            $this->db->query($sql);

            if ($val) {
                $sql = " UPDATE " . $this->sanmu->table('order_print_setting') . " SET is_default = '0' WHERE ru_id = '{$adminru['ru_id']}' AND id <> '$id' ";
                $this->db->query($sql);
            }
        ...
        elseif ($_REQUEST['act'] == 'order_print_setting_insert' || $_REQUEST['act'] == 'order_print_setting_update') {
            ...
            /* 检查是否重复 */
            $sql = " SELECT id FROM " . $this->sanmu->table('order_print_setting') . " WHERE ru_id = '{$adminru['ru_id']}' AND specification = '{$data['specification']}' AND id <> '$id' LIMIT 1 ";
            ...
            /* 插入、更新 */
            if ($id > 0) {
                $this->db->autoExecute($this->sanmu->table('order_print_setting'), $data, 'UPDATE', "id = '$id'");
                $msg = $GLOBALS['_LANG']['edit_success'];
            } else {
                $this->db->autoExecute($this->sanmu->table('order_print_setting'), $data, 'INSERT');
                $id = $this->db->insert_id();
                $msg = $GLOBALS['_LANG']['add_success'];
            }
            /* 默认设置 */
            if ($data['is_default']) {
                $this->db->autoExecute($this->sanmu->table('order_print_setting'), ['is_default' => 0], 'UPDATE', "id <> '$id'");
            }
        ...
        elseif ($_REQUEST['act'] == 'order_print') {
            ...
            if ($order_type == 'order' || empty($action_id)) {
                $table = $this->sanmu->table('order_info');
            } else {
                $table = $this->sanmu->table('wholesale_order_info');
            }
            $order_ids = [];
            if (!empty($order_id)) {
                $order_ids[] = $order_id;
    ...
    private function get_order_print_setting($ru_id)
    {
        ...
        /* 获得总记录数据 */
        $sql = 'SELECT COUNT(*) FROM ' . $this->sanmu->table('order_print_setting') . ' AS ops ' . $where;
        ...
        /* 获得数据 */
        $arr = [];
        $sql = 'SELECT ops.* FROM ' . $this->sanmu->table('order_print_setting') . 'AS ops ' .
