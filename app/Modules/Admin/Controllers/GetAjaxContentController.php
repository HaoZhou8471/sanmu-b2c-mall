...
class GetAjaxContentController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'add_seckill_goods') {
            ...
            $this->sanmuRepository->helpersLang(['seckill'], 'admin');
        ...
        elseif ($_REQUEST['act'] == 'ajax_allot') {
            ...
            $this->sanmuRepository->helpersLang('priv_action', 'admin');
        ...
        elseif ($_REQUEST['act'] == 'addmodule') {
            ...
            if ($_REQUEST['spec_attr']) {
                ...
                if (!empty($_REQUEST['spec_attr'])) {
                    $spec_attr = sanmu_decode($_REQUEST['spec_attr']);
                    $spec_attr = $this->sanmuRepository->objectToArray($spec_attr);
                }
            }
            ...
            if ($result['hierarchy'] == 3) {
                ...
                if ($result['activity_type'] == 'exchange') {
                    ...
                    $leftjoin = " LEFT JOIN " . $this->sanmu->table('exchange_goods') . " AS ga ON ga.goods_id=g.goods_id ";
                    ...
                    $leftjoin = " LEFT JOIN " . $this->sanmu->table('presale_activity') . " AS ga ON ga.goods_id=g.goods_id ";
                    ...
                    $leftjoin = " LEFT JOIN " . $this->sanmu->table('goods_activity') . " AS ga ON ga.goods_id=g.goods_id ";
                ...
                $sql = "SELECT g.promote_start_date, g.promote_end_date, g.promote_price, g.goods_name, g.goods_id, g.goods_thumb, g.shop_price, g.market_price, g.original_img $search FROM " .
                    $this->sanmu->table('goods') . " AS g " . $leftjoin . $where . $limit;
                ...
                if (!empty($goods_list)) {
                    foreach ($goods_list as $key => $val) {
                        ...
                        if ($result['activity_type'] == 'snatch') {
                            $goods_list[$key]['url'] = $this->sanmuRepository->buildUri('snatch', array('sid' => $val['act_id']));
                        } elseif ($result['activity_type'] == 'auction') {
                            $goods_list[$key]['url'] = $this->sanmuRepository->buildUri('auction', array('auid' => $val['act_id']));
                            ...
                            $goods_list[$key]['url'] = $this->sanmuRepository->buildUri('group_buy', array('gbid' => $val['act_id']));;
                        } elseif ($result['activity_type'] == 'exchange') {
                            $goods_list[$key]['url'] = $this->sanmuRepository->buildUri('exchange_goods', array('gid' => $val['goods_id']), $val['goods_name']);
                            $goods_list[$key]['exchange_integral'] = $_LANG['label_integral'] . $val['exchange_integral'];
                        } elseif ($result['activity_type'] == 'presale') {
                            $goods_list[$key]['url'] = $this->sanmuRepository->buildUri('presale', array('act' => 'view', 'presaleid' => $val['act_id']), $val['goods_name']);
                        } else {
                            $goods_list[$key]['url'] = $this->sanmuRepository->buildUri('goods', array('gid' => $val['goods_id']), $val['goods_name']);
                        }
        ...
        elseif ($_REQUEST['act'] == 'insertVipEdit') {
            ...
            for ($i = 0; $i < $count; $i++) {
                if ($quick_url[$i]) {
                    $arr[$i]['quick_url'] = $this->sanmuRepository->setRewriteUrl($quick_url[$i]);
                } else {
        ...
        elseif ($_REQUEST['act'] == 'homeAdvInsert') {
            ...
            for ($i = 0; $i < $count; $i++) {
                ...
                $arr[$i]['subtitle'] = $subtitle[$i];
                if ($url[$i]) {
                    $arr[$i]['url'] = $this->sanmuRepository->setRewriteUrl($url[$i]);
                } else {
                    $arr[$i]['url'] = $url[$i];
                }
        ...
        elseif ($_REQUEST['act'] == 'homeFloor') {
            ...
            if (!empty($result['cateValue'])) {
                ...
                foreach ($result['cateValue'] as $k => $v) {
                    ...
                    if ($v > 0) {
                        ...
                        $arr['url'] = $this->sanmuRepository->buildUri('category', ['cid' => $arr['cat_id']], $arr['cat_name']);
                        $cat_tow[] = $arr;
                    }
                }
            ...
            if ($result['moded'] == 'homeFloorFour' || $result['moded'] == 'homeFloorFive' || $result['moded'] == 'homeFloorSeven' ||
                ...
                $adminru = get_admin_ru_id();
                if ($adminru['rs_id'] > 0) {
                    $res = $this->sanmuRepository->getWhereRsid($res, 'user_id', $adminru['rs_id']);
                }
                // end
                $limit = 8;
                $goods_num = -1;
                    ...
                    if ($article_list) {
                        foreach ($article_list as $key => $val) {
                            ...
                            $article_list[$key]['url'] = $val['open_type'] != 1 ?
                                $this->sanmuRepository->buildUri('article', ['aid' => $val['article_id']], $val['title']) : trim($val['file_url']);
            ...
            if ($result['barndAdv']) {
                foreach ($result['barndAdv'] as $k => $v) {
                    ...
                    if (isset($result['barndAdvLink'][$k]) && $result['barndAdvLink'][$k]) {
                        $adv[$k]['barndAdvLink'] = $this->sanmuRepository->setRewriteUrl($result['barndAdvLink'][$k]);
                    } else {
        ...
        elseif ($_REQUEST['act'] == 'honePromo') {
            ...
            if (!empty($spec_attr['goods_ids'])) {
                if (!empty($PromotionType)) {
                    ...
                    $sql = "SELECT g.promote_start_date, g.promote_end_date, g.promote_price, g.goods_name, g.goods_id, g.goods_thumb, g.shop_price, g.market_price, g.original_img $search FROM " .
                        $this->sanmu->table('goods') . " AS g " . $leftjoin . $where;
                    ...
                    $sql = "SELECT g.promote_start_date, g.promote_end_date, g.promote_price, g.goods_name, g.goods_id, g.goods_thumb,"
                        . " g.shop_price, g.market_price, g.original_img , sg.sec_price,sg.id FROM" . $this->sanmu->table('goods') . " AS g "
                        . "LEFT JOIN " . $this->sanmu->table('seckill_goods') . " AS sg ON sg.goods_id=g.goods_id "
                        . "LEFT JOIN " . $this->sanmu->table('seckill') . "AS s ON s.sec_id=sg.sec_id" . $where;
                } else {
                    if (empty($spec_attr['title'])) {
                ...
                if (!empty($goods_list)) {
                    ...
                    foreach ($goods_list as $key => $val) {
                        ...
                        if (!empty($PromotionType)) {
                            if ($PromotionType == 'snatch') {
                                $goods_list[$key]['url'] = $this->sanmuRepository->buildUri('snatch', ['sid' => $val['act_id']]);
                            } elseif ($PromotionType == 'auction') {
                                $goods_list[$key]['url'] = $this->sanmuRepository->buildUri('auction', ['auid' => $val['act_id']]);
                                $ext_info = unserialize($val['ext_info']);
                                $auction = array_merge($val, $ext_info);
                                $goods_list[$key]['promote_price'] = price_format($auction['start_price']);
                            } elseif ($PromotionType == 'group_buy') {
                                ...
                                $group_buy = array_merge($val, $ext_info);
                                $goods_list[$key]['promote_price'] = price_format($group_buy['price_ladder'][0]['price']);
                                $goods_list[$key]['url'] = $this->sanmuRepository->buildUri('group_buy', ['gbid' => $val['act_id']]);
                            } elseif ($PromotionType == 'exchange') {
                                $goods_list[$key]['url'] = $this->sanmuRepository->buildUri('exchange_goods', ['gid' => $val['goods_id']], $val['goods_name']);
                                $goods_list[$key]['exchange_integral'] = $GLOBALS['_LANG']['label_integral'] . $val['exchange_integral'];
                            } elseif ($PromotionType == 'presale') {
                                $goods_list[$key]['url'] = $this->sanmuRepository->buildUri('presale', ['act' => 'view', 'presaleid' => $val['act_id']], $val['goods_name']);
                            }
                            ...
                            $goods_list[$key]['url'] = $this->sanmuRepository->buildUri('seckill', ['act' => "view", 'secid' => $val['id']], $val['goods_name']);
                            $goods_list[$key]['list_url'] = $this->sanmuRepository->buildUri('seckill', ['act' => "list", 'secid' => $val['id']], $val['goods_name']);
                        } else {
                            $goods_list[$key]['url'] = $this->sanmuRepository->buildUri('goods', ['gid' => $val['goods_id']], $val['goods_name']);
