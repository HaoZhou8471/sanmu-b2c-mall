    ...
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'reply') {
            ...
            if ($comment_info && isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                $comment_info['email'] = $this->sanmuRepository->stringToStar($comment_info['email']);
                $comment_info['user_name'] = $this->sanmuRepository->stringToStar($comment_info['user_name']);
            }

            if ($admin_info && isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                $admin_info['email'] = $this->sanmuRepository->stringToStar($admin_info['email']);
            }
