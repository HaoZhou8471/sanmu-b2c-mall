...
class UserBaitiaoLogController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'log_list') {
            ...
            //所有待还款白条的总额和条数
            $sql = "SELECT SUM(b.stages_one_price * (b.stages_total - b.yes_num)) AS total_amount, COUNT(log_id) AS numbers , SUM(b.stages_one_price * b.yes_num) AS already_amount FROM " . $this->sanmu->table('baitiao_log') . " AS b " .
            ...
            $user_id = isset($_REQUEST['user_id']) ? intval($_REQUEST['user_id']) : 0;
            //会员白条信息
            $bt = "SELECT * FROM " . $this->sanmu->table('baitiao') . " WHERE user_id='$user_id'";

            $bt_info = $this->db->getRow($bt);
            $bt_info['format_amount'] = isset($bt_info['amount']) ? price_format($bt_info['amount'], false) : 0;
            //待还款总额(不含分期金额)
            $bt_sun = "SELECT SUM(o.order_amount) FROM " . $this->sanmu->table('baitiao_log') . " AS b LEFT JOIN " . $this->sanmu->table('order_info') . "  AS o ON b.order_id=o.order_id WHERE b.user_id='$user_id' AND b.is_repay=0 AND  b.is_stages<>1  AND is_refund=0 ";
            $repay_sun_amount = $this->db->getOne($bt_sun);

            //所有待还款白条的总额和条数
            $bt_repay = "SELECT SUM(o.order_amount) AS total_amount,COUNT(log_id) AS numbers , SUM(b.stages_one_price * b.yes_num) AS already_amount FROM " . $this->sanmu->table('baitiao_log') . " AS b " .
                "LEFT JOIN " . $this->sanmu->table('order_info') . "  AS o ON b.order_id=o.order_id " .
        ...
        elseif ($_REQUEST['act'] == 'bt_add_tp') {
            ...
            if ($user_id > 0) {
                //会员信息
                $user_sql = "SELECT user_name,user_id FROM " . $this->sanmu->table('users') . " WHERE user_id='$user_id'";
                $user_info = $this->db->getRow($user_sql);

                if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0 && isset($user_info['user_name'])) {
                    $user_info['user_name'] = $this->sanmuRepository->stringToStar($user_info['user_name']);
                }

                $bt_sql = "SELECT b.*,u.user_name,u.user_id FROM " . $this->sanmu->table('baitiao') . " AS b LEFT JOIN " . $this->sanmu->table('users') . " AS u ON u.user_id=b.user_id WHERE b.user_id='$user_id'";
                $bt_info = $this->db->getRow($bt_sql);

                if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0 && isset($bt_info['user_name'])) {
                    $bt_info['user_name'] = $this->sanmuRepository->stringToStar($bt_info['user_name']);
                }
        ...
        elseif ($_REQUEST['act'] == 'bt_edit') {
            ...
            if ($user_id > 0) {
                $bt_sql = "SELECT baitiao_id FROM " . $this->sanmu->table('baitiao') . " WHERE user_id='$user_id'";
                $bt_info = $this->db->getOne($bt_sql);
                if ($bt_info) {
                    $bt_up_sql = "update " . $this->sanmu->table('baitiao') . " set amount='$amount',repay_term='$repay_term',over_repay_trem='$over_repay_trem',add_time=" . gmtime() . " where baitiao_id='$bt_info.baitiao_id'";
                    ...
                    $bt_insert_sql = "INSERT INTO " . $this->sanmu->table('baitiao') . " (user_id,amount,repay_term,over_repay_trem,add_time) VALUES ('$user_id','$amount','$repay_term','$over_repay_trem'," . gmtime() . ")";
        ...
        elseif ($_REQUEST['act'] == 'batch_remove') {
            ...
            $sql = "DELETE FROM " . $this->sanmu->table('baitiao') .
        ...
        elseif ($_REQUEST['act'] == 'batch_remove_log') {
            ...
            $sql = "SELECT log_id,is_repay,baitiao_id,user_id FROM " . $this->sanmu->table('baitiao_log') . " WHERE log_id " . db_create_in(join(',', $_POST['checkboxes']));
            ...
            if ($bt_log) {
                ...
                foreach ($bt_log as $key => $val) {
                    if ($val['is_repay']) {
                        $del_sql = "DELETE FROM " . $this->sanmu->table('baitiao_log') .
        ...
        elseif ($_REQUEST['act'] == 'remove') {
            ...
            $sql = "SELECT baitiao_id FROM " . $this->sanmu->table('baitiao') . " WHERE baitiao_id = '" . $_GET['baitiao_id'] . "'";
            $baitiao_id = $this->db->getOne($sql);
            if ($baitiao_id > 0) {
                $sql = "delete from " . $this->sanmu->table('baitiao') . " where baitiao_id = '" . $baitiao_id . "'";
                $del_ok = $this->db->query($sql);
                if ($del_ok) {
                    /* 记录管理员操作 */
        ...
        elseif ($_REQUEST['act'] == 'remove_log') {
            ...
            $sql = "SELECT log_id,is_repay,baitiao_id,user_id,is_refund FROM " . $this->sanmu->table('baitiao_log') . " WHERE log_id = '" . $_GET['log_id'] . "'";
            $bt_log = $this->db->getRow($sql);
            if ($bt_log['log_id'] > 0) {
                if ($bt_log['is_repay'] || $bt_log['is_refund']) {//已退款白条订单页可以删除;
                    $sql = "DELETE FROM " . $this->sanmu->table('baitiao_log') . " WHERE is_repay=1 OR is_refund=1 AND log_id = '" . $bt_log['log_id'] . "'";
                    $del_ok = $this->db->query($sql);
    ...
    private function baitiao_list()
    {
        ...
        if ($result === false) {
            ...
            $filter['record_count'] = $this->db->getOne("SELECT COUNT(*) FROM " . $this->sanmu->table('baitiao') . "AS b LEFT JOIN " . $this->sanmu->table('users') . " AS u ON b.user_id=u.user_id " . $ex_where);

            /* 分页大小 */
            $filter = page_and_size($filter);
            $sql = "SELECT b.*,u.user_name " .
                " FROM " . $this->sanmu->table('baitiao') . "as b left join " . $this->sanmu->table('users') . " as u on b.user_id=u.user_id " . $ex_where .
        ...
        if ($baitiao_list) {
            foreach ($baitiao_list as $key => $row) {
                if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                    $baitiao_list[$key]['user_name'] = $this->sanmuRepository->stringToStar($row['user_name']);
                }
            }
        }
    ...
    private function get_baitiao_list($user_id = 0)
    {
        ...
        if ($result === false) {
            ...
            $filter['record_count'] = $this->db->getOne("SELECT COUNT(b.log_id) AS total FROM " . $this->sanmu->table('baitiao_log') . " AS b LEFT JOIN " . $this->sanmu->table('order_info') . "  AS o ON b.order_id=o.order_id  $ex_where ORDER BY b.log_id DESC");

            /* 分页大小 */
            $filter = page_and_size($filter);
            $sql = "SELECT b.*,o.order_sn, o.money_paid AS order_amount FROM " . $this->sanmu->table('baitiao_log') . " AS b LEFT JOIN " . $this->sanmu->table('order_info') . "  AS o ON b.order_id=o.order_id  $ex_where ORDER BY b.log_id DESC" .
                " LIMIT " . $filter['start'] . ',' . $filter['page_size'];
