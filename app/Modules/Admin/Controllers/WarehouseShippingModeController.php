...
class WarehouseShippingModeController extends InitController
{
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            $sql = "select region_id from " . $this->sanmu->table('region_warehouse') . " where regionId = '$region_id'";
        ...
        elseif ($_REQUEST['act'] == 'update') {
            ...
            $sql = "select shipping_id from " . $this->sanmu->table('warehouse_freight') . " where id = '$id'";
            $shipping = $this->db->getOne($sql);

            $shipping_code = $this->db->getOne("SELECT shipping_code FROM " . $this->sanmu->table('shipping') . " WHERE shipping_id='$shipping'");
            ...
            $sql = "UPDATE " . $this->sanmu->table('warehouse_freight') .
        ...
        elseif ($_REQUEST['act'] == 'remove') {
            ...
            $sql = "DELETE FROM " . $this->sanmu->table('warehouse_freight') . " WHERE id = '" . intval($_GET['id']) . "'";
            $this->db->query($sql);
    ...
    private function shipping_mode_list($shipping_id, $region_id)
    {
        ...
        if ($result === false) {
            ...
            $sql = "SELECT wf.id, rw1.region_name as region_name1, rw2.region_name as region_name2 " .
                " FROM " . $this->sanmu->table('warehouse_freight') . " AS wf" .
                " LEFT JOIN " . $this->sanmu->table('region_warehouse') . " as rw1 ON wf.warehouse_id = rw1.region_id" .
                " LEFT JOIN " . $this->sanmu->table('shipping') . " as s ON wf.shipping_id = s.shipping_id" .
                " LEFT JOIN " . $this->sanmu->table('region_warehouse') . " as rw2 ON wf.region_id = rw2.regionId" .
                $ex_where;
