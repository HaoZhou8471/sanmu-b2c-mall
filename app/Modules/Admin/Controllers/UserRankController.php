...
class UserRankController extends InitController
{
    ...
    {
        $exc = new Exchange($this->sanmu->table("user_rank"), $this->db, 'rank_id', 'rank_name');
        $exc_user = new Exchange($this->sanmu->table("users"), $this->db, 'user_rank', 'user_rank');
...
//                'special_rank' => 0,//1.4.2 1.4.3修改为只显示普通等级
        ...
        elseif ($_REQUEST['act'] == 'query') {
            $ranks = [];
            $ranks = $this->db->getAll("SELECT * FROM " . $this->sanmu->table('user_rank') . " WHERE special_rank = 0 order by rank_id");
            $this->smarty->assign('rank_count', count($ranks));
            $this->smarty->assign('user_ranks', $ranks);
...
//            $min_points = $this->db->getOne(" SELECT MAX(min_points) FROM " . $this->sanmu->table('user_rank') . " WHERE special_rank = 0 ");
...
//            }
            ...
            $sql = "INSERT INTO " . $this->sanmu->table('user_rank') . "( " .
...
//            $sql = "SELECT * FROM" . $this->sanmu->table("user_rank") . " WHERE rank_id='$id'";
...
////            }
//
//            $sql = " UPDATE " . $this->sanmu->table('user_rank') . " SET min_points = '$max_points' + 1 WHERE min_points = '$old_max_points' + 1 OR rank_id > '$id' LIMIT 1 ";
//            $this->db->query($sql);
//
//            $sql = "UPDATE" . $this->sanmu->table('user_rank') . " SET rank_name = '$rank_name' "
...
//            return sys_msg($GLOBALS['_LANG']['edit'] . $GLOBALS['_LANG']['success'], 0, $lnk);
        ...
        elseif ($_REQUEST['act'] == 'update_user_rank_set') {
            ...
            $this->db->autoExecute($this->sanmu->table('shop_config'), $open_user_rank, 'update', "code='open_user_rank_set'");

            //更新是否开启等级积分将会自动清零有效期
            $clear_rank_point['value'] = $_POST['clear_rank_point'] ? intval($_POST['clear_rank_point']) : 12;
            $this->db->autoExecute($this->sanmu->table('shop_config'), $clear_rank_point, 'update', "code='clear_rank_point'");
        }
    }
