...
class SuppliersCommissionController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'toggle_on_settlement') {
            ...
            $sql = "UPDATE " . $this->sanmu->table('wholesale_order_info') . " SET is_settlement = '$on_sale' WHERE order_id = '$order_id'";
            $this->db->query($sql);

            $this->db->autoExecute($this->sanmu->table('suppliers_account_log_detail'), $other, 'INSERT');
        ...
        if ($_REQUEST['act'] == 'merchant_download') {
            ...
            if (!empty($merchants_download_content)) {
                foreach ($merchants_download_content as $k => $merchants_order_list) {
                    ...
                    $content .= sanmu_iconv(EC_CHARSET, 'GB2312', lang('admin/suppliers_commission.total_sum'));
                    $content .= sanmu_iconv(EC_CHARSET, 'GB2312', $valid['brokerage_amount']) . "\t\n";
                    $content .= sanmu_iconv(EC_CHARSET, 'GB2312', lang('admin/suppliers_commission.settled_s'));
                    $content .= sanmu_iconv(EC_CHARSET, 'GB2312', $valid['formated_is_settlement_amout']) . "\t\n";
                    $content .= sanmu_iconv(EC_CHARSET, 'GB2312', lang('admin/suppliers_commission.unsettled_s'));
                    $content .= sanmu_iconv(EC_CHARSET, 'GB2312', $valid['formated_no_settlement_amout']) . "\t\n";
    ...
    private function getSuppliersCommissionOrderList($suppliers_id = 0, $page = 0)
    {
        ...
        if ($result === false) {
            ...
            //有主订单事  去除主订单
            $where .= " AND (select count(*) from " . $GLOBALS['sanmu']->table('wholesale_order_info') . " as oi2 where oi2.main_order_id = oi.order_id limit 0, 1) = 0";

            $where .= ' AND  ((oi.pay_status =2 AND oi.order_status = 1) OR (oi.pay_status = 5 AND oi.order_status = 4)) ';//查询已支付，一完成的订单 或者部分退款订单

            /* 查询记录总数，计算分页数 */
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['sanmu']->table('wholesale_order_info') . " AS oi" . $where;
            ...
            $sql = "SELECT oi.return_amount,oi.add_time,oi.suppliers_id,oi.consignee,oi.address,oi.user_id,order_amount,oi.order_sn,oi.order_id,oi.is_settlement FROM " .
                $GLOBALS['sanmu']->table('wholesale_order_info') . " AS oi" . $where .
    ...
    private function getSuppliersCommissionList()
    {
        ...
        if ($result === false) {
            ...
            /* 查询记录总数，计算分页数 */
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['sanmu']->table('suppliers');
            $filter['record_count'] = $GLOBALS['db']->getOne($sql);

            $filter = page_and_size($filter);

            /* 查询记录 */
            $sql = "SELECT suppliers_id,suppliers_name,company_name,company_address,mobile_phone FROM " . $GLOBALS['sanmu']->table('suppliers') .
                " ORDER BY $filter[sort_by] $filter[sort_order] " .
                " LIMIT " . $filter['start'] . ",$filter[page_size]";
        ...
        foreach ($res as $key => $rows) {
            ...
            if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                $rows['mobile_phone'] = $this->sanmuRepository->stringToStar($rows['mobile_phone']);
            }
