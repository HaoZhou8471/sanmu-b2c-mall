...
class SuppliersGoodsController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy') {
            ...
            if ($is_add) {
                ...
                /* 商品信息 */
                $sql = "SELECT * FROM " . $this->sanmu->table('goods') . " WHERE goods_id = '$_REQUEST[goods_id]'";
                ...
                $sql = "SELECT cat_id FROM " . $this->sanmu->table('goods_cat') . " WHERE goods_id = '$_REQUEST[goods_id]'";
                ...
                /* 图片列表 */
                $sql = "SELECT * FROM " . $this->sanmu->table('goods_gallery') . " WHERE goods_id = '$goods[goods_id]'";
        ...
        elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
            ...
            if ($is_insert) {
                if ($code == '') {
                    $sql = "INSERT INTO " . $this->sanmu->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                ...
                if ($proc_thumb && $goods_img && $row['goods_img'] && !$this->goods_parse_url($row['goods_img'])) {
                    sanmu_unlink(storage_public($row['goods_img']));
                    sanmu_unlink(storage_public($row['original_img']));
                }

                if ($proc_thumb && $goods_thumb && $row['goods_thumb'] && !$this->goods_parse_url($row['goods_thumb'])) {
                    sanmu_unlink(storage_public($row['goods_thumb']));
                }

                $sql = "UPDATE " . $this->sanmu->table('goods') . " SET " .
            ...
            if ((isset($_POST['attr_id_list']) && isset($_POST['attr_value_list'])) || (empty($_POST['attr_id_list']) && empty($_POST['attr_value_list']))) {
                ...
                $sql = "SELECT * FROM " . $this->sanmu->table('goods_attr') . " WHERE goods_id = '$goods_id'";
                ...
                $sql = "UPDATE " . $this->sanmu->table('goods') . " SET keywords = '$keywords' WHERE goods_id = '$goods_id' LIMIT 1";
                ...
                foreach ($goods_attr_list as $attr_id => $attr_value_list) {
                    foreach ($attr_value_list as $attr_value => $info) {
                        if ($info['sign'] == 'insert') {
                            $sql = "INSERT INTO " . $this->sanmu->table('goods_attr') . " (attr_id, goods_id, attr_value, attr_price)" .
                                "VALUES ('$attr_id', '$goods_id', '$attr_value', '$info[attr_price]')";
                        } elseif ($info['sign'] == 'update') {
                            $sql = "UPDATE " . $this->sanmu->table('goods_attr') . " SET attr_price = '$info[attr_price]' WHERE goods_attr_id = '$info[goods_attr_id]' LIMIT 1";
                        } else {
            ...
            if ($goods_img !== false) {
                $this->db->query("UPDATE " . $this->sanmu->table('goods') . " SET goods_img = '$goods_img' WHERE goods_id='$goods_id'");
            }

            if ($original_img !== false) {
                $this->db->query("UPDATE " . $this->sanmu->table('goods') . " SET original_img = '$original_img' WHERE goods_id='$goods_id'");
            }

            if ($goods_thumb !== false) {
                $this->db->query("UPDATE " . $this->sanmu->table('goods') . " SET goods_thumb = '$goods_thumb' WHERE goods_id='$goods_id'");
            }
            ...
            if (!$is_insert && isset($_POST['old_img_desc'])) {
                foreach ($_POST['old_img_desc'] as $img_id => $img_desc) {
                    $sql = "UPDATE " . $this->sanmu->table('goods_gallery') . " SET img_desc = '$img_desc' WHERE img_id = '$img_id' LIMIT 1";
                    $this->db->query($sql);
                }
            }
            ...
            if ($proc_thumb && !$GLOBALS['_CFG']['retain_original_img'] && !empty($original_img)) {
                $this->db->query("UPDATE " . $this->sanmu->table('goods') . " SET original_img='' WHERE `goods_id`='{$goods_id}'");
                $this->db->query("UPDATE " . $this->sanmu->table('goods_gallery') . " SET img_original='' WHERE `goods_id`='{$goods_id}'");
                @unlink('../' . $original_img);
                @unlink('../' . $img);
            }
        ...
        elseif ($_REQUEST['act'] == 'drop_image') {
            ...
            /* 删除图片文件 */
            $sql = "SELECT img_url, thumb_url, img_original " .
                " FROM " . $GLOBALS['sanmu']->table('goods_gallery') .
                " WHERE img_id = '$img_id'";
            ...
            /* 删除数据 */
            $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('goods_gallery') . " WHERE img_id = '$img_id' LIMIT 1";
        ...
        elseif ($_REQUEST['act'] == 'add_link_goods') {
            ...
            $linked_array = sanmu_decode($_GET['add_ids']);
            $linked_goods = sanmu_decode($_GET['JSON']);
        ...
        elseif ($_REQUEST['act'] == 'drop_link_goods') {
            ...
            $drop_goods = sanmu_decode($_GET['drop_ids']);
        ...
        elseif ($_REQUEST['act'] == 'add_group_goods') {
            ...
            $fittings = sanmu_decode($_GET['add_ids']);
            $arguments = sanmu_decode($_GET['JSON']);
            $goods_id = $arguments[0];
            $price = $arguments[1];

            foreach ($fittings as $val) {
                $sql = "INSERT INTO " . $this->sanmu->table('group_goods') . " (parent_id, goods_id, goods_price, admin_id) " .
        ...
        elseif ($_REQUEST['act'] == 'drop_group_goods') {
            ...
            $fittings = sanmu_decode($_GET['drop_ids']);
            $arguments = sanmu_decode($_GET['JSON']);
            $goods_id = $arguments[0];
            $price = $arguments[1];
        ...
        elseif ($_REQUEST['act'] == 'get_article_list') {
            $filters = (array)sanmu_decode(json_str_iconv($_GET['JSON']));
        ...
        elseif ($_REQUEST['act'] == 'add_goods_article') {
            ...
            $articles = sanmu_decode($_GET['add_ids']);
            $arguments = sanmu_decode($_GET['JSON']);
            $goods_id = $arguments[0];
        ...
        elseif ($_REQUEST['act'] == 'drop_goods_article') {
            ...
            $articles = sanmu_decode($_GET['drop_ids']);
            $arguments = sanmu_decode($_GET['JSON']);
            $goods_id = $arguments[0];

            $sql = "DELETE FROM " . $this->sanmu->table('goods_article') . " WHERE " . db_create_in($articles, "article_id");
            $this->db->query($sql);
    ...
    {
        $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('volume_price') .
        ...
        foreach ($price_list as $key => $price) {
            ...
            if (!empty($price)) {
                $sql = "INSERT INTO " . $GLOBALS['sanmu']->table('volume_price') .
