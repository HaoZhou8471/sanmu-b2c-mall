...
class VoteController extends InitController
{
    public function index()
    {
        ...
        $exc = new Exchange($this->sanmu->table("vote"), $this->db, 'vote_id', 'vote_name');
        $exc_opn = new Exchange($this->sanmu->table("vote_option"), $this->db, 'option_id', 'option_name');
        ...
        elseif ($_REQUEST['act'] == 'add') {
            ...
            /* 查看广告名称是否有重复 */
            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('vote') . " WHERE vote_name='$_POST[vote_name]'";
            if ($this->db->getOne($sql) == 0) {
                /* 插入数据 */
                $sql = "INSERT INTO " . $this->sanmu->table('vote') . " (vote_name, start_time, end_time, can_multi, vote_count)
        ...
        elseif ($_REQUEST['act'] == 'edit') {
            ...
            /* 获取数据 */
            $vote_arr = $this->db->GetRow("SELECT * FROM " . $this->sanmu->table('vote') . " WHERE vote_id='$_REQUEST[id]'");
            ...
            /* 更新信息 */
            $sql = "UPDATE " . $this->sanmu->table('vote') . " SET " .
        ...
        elseif ($_REQUEST['act'] == 'new_option') {
            ...
            if (!empty($option_name)) {
                /* 查看调查标题是否有重复 */
                $sql = 'SELECT COUNT(*) FROM ' . $this->sanmu->table('vote_option') .
                    " WHERE option_name = '$option_name' AND vote_id = '$vote_id'";
                if ($this->db->getOne($sql) != 0) {
                    return make_json_error($GLOBALS['_LANG']['vote_option_exist']);
                } else {
                    $sql = 'INSERT INTO ' . $this->sanmu->table('vote_option') . ' (vote_id, option_name, option_count) ' .
        ...
        elseif ($_REQUEST['act'] == 'remove') {
            ...
            if ($exc->drop($id)) {
                /* 同时删除调查选项 */
                $this->db->query("DELETE FROM " . $this->sanmu->table('vote_option') . " WHERE vote_id = '$id'");
                clear_cache_files();
        ...
        elseif ($_REQUEST['act'] == 'remove_option') {
            ...
            $id = intval($_GET['id']);
            $vote_id = $this->db->getOne('SELECT vote_id FROM ' . $this->sanmu->table('vote_option') . " WHERE option_id='$id'");
    ...
    private function get_votelist()
    {
        ...
        /* 记录总数以及页数 */
        $sql = 'SELECT COUNT(*) FROM ' . $this->sanmu->table('vote');
        $filter['record_count'] = $this->db->getOne($sql);

        $filter = page_and_size($filter);

        /* 查询数据 */
        $sql = 'SELECT * FROM ' . $this->sanmu->table('vote') . ' ORDER BY vote_id DESC';
    ...
    private function get_optionlist($id)
    {
        ...
        $sql = 'SELECT option_id, vote_id, option_name, option_count, option_order' .
            ' FROM ' . $this->sanmu->table('vote_option') .
