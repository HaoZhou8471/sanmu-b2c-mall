 ...
 * 短信模板管理程序
 */
class SanmusmsConfigureController extends InitController
{
    ...
    public function index()
    {
        ...
        $this->smarty->assign('menu_select', ['action' => '01_system', 'current' => 'sanmusms_configure']);
        /*------------------------------------------------------ */
        //-- 阿里大鱼短信模板列表
        /*------------------------------------------------------ */
        if ($act == 'list') {
            $this->smarty->assign('action_link', ['text' => $GLOBALS['_LANG']['alidayu_add'], 'href' => 'sanmusms_configure.php?act=add']);
            $this->smarty->assign('action_link2', ['text2' => $GLOBALS['_LANG']['alidayu_set'], 'href' => 'sanmusms_configure.php?act=set_up']);

            /* 模板赋值 */
            $this->smarty->assign('ur_here', $GLOBALS['_LANG']['sanmusms_configure']);
            ...
            /* 列表页面 */
            return $this->smarty->display('sanmusms_configure_list.dwt');
        }
        ...
        elseif ($act == 'query') {
            ...
            return make_json_result($this->smarty->fetch('sanmusms_configure_list.dwt'), '', ['filter' => $sms_list['filter'], 'page_count' => $sms_list['page_count']]);
        }
        ...
        /*------------------------------------------------------ */
        if ($act == 'add') {
            $this->smarty->assign('action_link', ['text' => $GLOBALS['_LANG']['alidayu_list'], 'href' => 'sanmusms_configure.php?act=list']);
            ...
            /* 列表页面 */
            return $this->smarty->display('sanmusms_configure_info.dwt');
        }
        ...
        if ($act == 'set_update') {
            ...
            /* 提示信息 */
            $link[] = ['text' => $GLOBALS['_LANG']['go_back'], 'href' => 'sanmusms_configure.php?act=list'];
        ...
        elseif ($act == 'insert' || $act == 'update') {
            ...
            if ($id) {
                ...
                $href = 'sanmusms_configure.php?act=edit&id=' . $id;
                ...
                $href = 'sanmusms_configure.php?act=list';
