...
class TouchTopicController extends InitController
{
    ...
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit') {
            ...
            if (!$isadd) {
                $sql = "SELECT * FROM " . $this->sanmu->table('touch_topic') . " WHERE topic_id = '$topic_id'";
        ...
        elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
            ...
            $this->sanmuRepository->getOssAddFile([$topic_img, $title_pic]);

            $tmp_data = sanmu_decode($_POST['topic_data'], true);
            ...
            if ($is_insert) {
                ...
                $this->db->autoExecute($this->sanmu->table('touch_topic'), $record, 'INSERT');
            } else {
                ...
                $this->db->autoExecute($this->sanmu->table('touch_topic'), $record, 'UPDATE', "topic_id = '$topic_id'");
            }
            ...
            //删除图片
            $this->sanmuRepository->getDelBatch($_POST['checkboxes'], intval($_GET['id']), ['topic_img', 'title_pic'], 'topic_id', TouchTopic::whereRaw(1), 1);

            $sql = "DELETE FROM " . $this->sanmu->table('touch_topic') . " WHERE ";
    ...
    private function get_topic_list()
    {
        ...
        if ($result === false) {
            ...
            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('touch_topic') . " AS t " . " WHERE $where";
            $filter['record_count'] = $this->db->getOne($sql);

            /* 分页大小 */
            $filter = page_and_size($filter);

            $sql = "SELECT t.* FROM " . $this->sanmu->table('touch_topic') . " AS t " . " WHERE $where ORDER BY $filter[sort_by] $filter[sort_order]";
