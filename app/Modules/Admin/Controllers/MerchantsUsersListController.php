...
class MerchantsUsersListController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'insert_shop' || $_REQUEST['act'] == 'update_shop') {
            ...
            if ($shopinfo) {
                ...
                if ($merchants_audit == 1) {
                    ...
                    $this->db->autoExecute($this->sanmu->table('seller_shopinfo'), $seller_shopinfo);
                }
            }
        ...
        elseif ($_REQUEST['act'] == 'ajax_initialize_rank') {
            ...
            $seller_grade_list = seller_grade_list();
            $grade_list = $this->sanmu->page_array($page_size, $page, $seller_grade_list);
        ...
        elseif ($_REQUEST['act'] == 'seller_shopinfo') {
            ...
            //引入首页语言包
            $this->sanmuRepository->helpersLang('index', 'admin');
            ...
            if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0 && $seller_shop_info) {
                $seller_shop_info['mobile'] = $this->sanmuRepository->stringToStar($seller_shop_info['mobile']);
                $seller_shop_info['seller_email'] = $this->sanmuRepository->stringToStar($seller_shop_info['seller_email']);
                $seller_shop_info['kf_tel'] = $this->sanmuRepository->stringToStar($seller_shop_info['kf_tel']);
            }
        ...
        elseif ($_REQUEST['act'] == 'save_seller_shopinfo') {
            ...
            //$this->sanmuRepository->getOssAddFile([$street_thumb, $brand_thumb]);
        ...
        elseif ($_REQUEST['act'] == 'see_shopinfo') {
            ...
            $shop_information['is_sanmu'] = $adminru['ru_id'] == 0 ? true : false; //判断当前商家是平台,还是入驻商家
            // 申请时间、开店时间、到期时间
            ...
            if (isset($shop_information['get_users']) && !empty($shop_information['get_users'])) {
                ...
                if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                    $shop_information['mobile_phone'] = $this->sanmuRepository->stringToStar($shop_information['mobile_phone']);
                    $shop_information['user_name'] = $this->sanmuRepository->stringToStar($shop_information['user_name']);
                    $shop_information['email'] = $this->sanmuRepository->stringToStar($shop_information['email']);
                }
            }
