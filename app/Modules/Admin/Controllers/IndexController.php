...
namespace App\Modules\Sanmu\Controllers;
...
class IndexController extends InitController
{
    ...
    public function index()
    {
        ...
        /* 校验授权 start */
        $is_empower = $this->sanmuRepository->checkEmpower();
        ...
        if ($_REQUEST['act'] == '') {
            ...
            $this->smarty->assign('shop_url', urlencode($this->sanmu->url()));
        ...
        elseif ($_REQUEST['act'] == 'set_clear_cache') {
            ...
            if ($chkGroup == 'all' || $sessGroup == 'all') {
                ...
                foreach ($action_code as $k => $v) {
                    //商城配置
                    if ($v == 'shop_config') {
                        sanmu_unlink(storage_path('framework/temp/static_caches/shop_config.php'));
                    }
                    if ($v == 'category') {
        ...
        elseif ($_REQUEST['act'] == 'main') {
            ...
            if (session()->has('shop_guide') && session('shop_guide') === true) {
                ...
                return sanmu_header("Location: ./index.php?act=first\n");
            }
            ...
            //今日销售总额
            //付款金额
            $sql = 'SELECT  SUM(oi.money_paid) AS sales FROM ' . $this->sanmu->table('order_info') . " AS oi" . ' WHERE oi.add_time BETWEEN ' . $today_start . ' AND ' . $today_end . '  AND oi.supplier_id = 0  ' . $this->orderService->orderQuerySql('queren', "oi.") . $where_og;
            $today['money_paid_money'] = $this->db->GetOne($sql);

            //余额金额
            $sql = 'SELECT  SUM(surplus) AS sales FROM ' . $this->sanmu->table('order_info') . " AS oi" . ' WHERE oi.add_time BETWEEN ' . $today_start . ' AND ' . $today_end . '  AND oi.supplier_id=0 ' . $this->orderService->orderQuerySql('queren', "oi.") . $where_og;
...
//    $today_start = mktime(0,0,0,date('m'),date('d'),date('Y'));
            ...
            $this->smarty->assign('ecs_url', $this->sanmu->url());
        ...
        elseif ($_REQUEST['act'] == 'shop_top') {
            ...
            if (is_null($data)) {
                ...
                $ecs_style = $style;
                $shop_url = urlencode($GLOBALS['sanmu']->url()); //当前url
                ...
                $httpData = [
                    'domain' => $GLOBALS['sanmu']->get_domain(), //当前域名
                ...
                Http::doPost($sanmu_url, $argument);
        ...
        elseif ($_REQUEST['act'] == 'first') {
            ...
            //获取支付方式
            $modules = $this->sanmuRepository->readModules(plugin_path('Payment'));
        ...
        elseif ($_REQUEST['act'] == 'second') {
            ...
            //设置配送方式
            if (!empty($shipping)) {
                $shop_add = $this->sanmuRepository->readModules(plugin_path('Shipping'));
        ...
        elseif ($_REQUEST['act'] == 'merchants_first') {
            ...
            $shop_information['is_sanmu'] = isset($shop_information['is_sanmu']) ? $shop_information['is_sanmu'] : '';
                        ...
                        if (move_upload_file($file['tmp_name'], $file_name)) {
                            ...
                            if (!empty($qrcode_thumb)) {
                                ...
                                if (isset($store['qrcode_thumb']) && $store['qrcode_thumb']) {
                                    ...
                                    sanmu_unlink(storage_public($del_qrcode_thumb));
                                }
                            }
        ...
        elseif ($_REQUEST['act'] == 'check_order') {
            ...
            if (admin_priv('order_view', '', false)) {
                ...
                $arr['await_ship'] = $this->db->getOne('SELECT count(*) FROM ' . $GLOBALS['sanmu']->table('order_info') . " AS oi " .
                    " WHERE 1 " . $this->orderService->orderQuerySql('await_ship', 'oi.') . " AND (SELECT ore.ret_id FROM " . $GLOBALS['sanmu']->table('order_return') . " AS ore WHERE ore.order_id = oi.order_id LIMIT 1) IS NULL " . $where_og);
            }
        ...
        elseif ($_REQUEST['act'] == 'cloud_services') {
            ...
            if (strpos($http, 'https://') === false) {
                $Loaction = "http://market.sanmumall.cn/";
            } else {
                $Loaction = "https://market.sanmumall.cn/";
            }
        ...
        elseif ($_REQUEST['act'] == 'upload_store_img') {
            ...
            if ($_FILES['img']['name']) {
                ...
                if ($img_name) {
                    ...
                    sanmu_unlink(storage_public($store_user_img));
