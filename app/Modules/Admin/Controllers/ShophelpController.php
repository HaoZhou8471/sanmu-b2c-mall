...
class ShophelpController extends InitController
{
    public function index()
    {
        ...
        /*初始化数据交换对象 */
        $exc_article = new Exchange($this->sanmu->table("article"), $this->db, 'article_id', 'title');
        $exc_cat = new Exchange($this->sanmu->table("article_cat"), $this->db, 'cat_id', 'cat_name');
        ...
        if ($_REQUEST['act'] == 'insert') {
            ...
            /* 插入数据 */
            $add_time = gmtime();
            $sql = "INSERT INTO " . $this->sanmu->table('article') . "(title, cat_id, article_type, content, add_time, author) VALUES('$title', '$cat_id', '$article_type','$editor','$add_time', '_SHOPHELP' )";
            $this->db->query($sql);
        ...
        if ($_REQUEST['act'] == 'edit') {
            ...
            /* 取文章数据 */
            $sql = "SELECT article_id,title, cat_id, article_type, is_open, author, author_email, keywords, content FROM " . $this->sanmu->table('article') . " WHERE article_id='$article_id'";
        ...
        elseif ($_REQUEST['act'] == 'remove_art') {
            ...
            $id = intval($_GET['id']);
            $cat_id = $this->db->getOne('SELECT cat_id FROM ' . $this->sanmu->table('article') . " WHERE article_id='$id'");
        ...
        elseif ($_REQUEST['act'] == 'add_catname') {
            ...
            if (!empty($cat_name)) {
                if ($exc_cat->num("cat_name", $cat_name) != 0) {
                    ...
                    $sql = "INSERT INTO " . $this->sanmu->table('article_cat') . " (cat_name, cat_type) VALUES ('$cat_name', 0)";
                    $this->db->query($sql);

                    admin_log($cat_name, 'add', 'shophelpcat');

                    return sanmu_header("Location: shophelp.php?act=query\n");
                }
            } else {
                return make_json_error($GLOBALS['_LANG']['js_languages']['no_catname']);
            }

            return sanmu_header("Location: shophelp.php?act=list_cat\n");
        }
    ...
    private function get_shophelp_list()
    {
        ...
        $sql = 'SELECT cat_id, cat_name, sort_order' .
            ' FROM ' . $this->sanmu->table('article_cat') .
            ' WHERE cat_type = 0 ORDER BY sort_order';
        $res = $this->db->query($sql);
        foreach ($res as $rows) {
            $sql = 'SELECT COUNT(*) FROM ' . $this->sanmu->table('article') . " WHERE cat_id = '" . $rows['cat_id'] . "'";
