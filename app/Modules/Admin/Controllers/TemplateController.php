...
class TemplateController extends InitController
{
    ...
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            /* 清除不需要的模板设置 */
            $available_code = [];
            $sql = "DELETE FROM " . $this->sanmu->table('template') . " WHERE 1 ";
        ...
        if ($_REQUEST['act'] == 'setup') {
            ...
            $sql = "SELECT region, library, sort_order, id, number, type, floor_tpl FROM " . $this->sanmu->table('template') . " " .
        ...
        if ($_REQUEST['act'] == 'setting') {
            ...
            $curr_template = $GLOBALS['_CFG']['template'];
            $this->db->query("DELETE FROM " . $this->sanmu->table('template') . " WHERE remarks = '' AND filename = '$_POST[template_file]' AND theme = '$curr_template'");
            ...
            foreach ($_POST['regions'] as $key => $val) {
                ...
                if (!in_array($key, $GLOBALS['dyna_libs']) and (isset($_POST['display'][$key]) and $_POST['display'][$key] == 1 or $number > 0)) {
                    $sql = "INSERT INTO " . $this->sanmu->table('template') .
        ...
        if ($_REQUEST['act'] == 'install') {
            ...
            $sql = "UPDATE " . $this->sanmu->table('shop_config') . " SET value = '$tpl_name' WHERE code = 'template'";
            $step_one = $this->db->query($sql, 'SILENT');
            $sql = "UPDATE " . $this->sanmu->table('shop_config') . " SET value = '$tpl_fg' WHERE code = 'stylename'";
        ...
        if ($_REQUEST['act'] == 'backup_setting') {
            ...
            $sql = "SELECT DISTINCT(remarks) FROM " . $this->sanmu->table('template') . " WHERE theme = '" . $GLOBALS['_CFG']['template'] . "' AND remarks > ''";
            ...
            $sql = "SELECT DISTINCT(filename) FROM " . $this->sanmu->table('template') . " WHERE theme = '" . $GLOBALS['_CFG']['template'] . "' AND remarks = ''";
            $col = $this->db->getCol($sql);
            $files = [];
            if ($col) {
                foreach ($col as $val) {
        ...
        if ($_REQUEST['act'] == 'act_backup_setting') {
            ...
            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('template') . " WHERE remarks='$remarks' AND theme = '" . $GLOBALS['_CFG']['template'] . "'";
            ...
            $sql = "INSERT INTO " . $this->sanmu->table('template') .
                ...
                " FROM " . $this->sanmu->table('template') .
        ...
        if ($_REQUEST['act'] == 'del_backup') {
            ...
            if ($remarks) {
                $sql = "DELETE FROM " . $this->sanmu->table('template') . " WHERE remarks='$remarks' AND theme = '" . $GLOBALS['_CFG']['template'] . "'";
                $this->db->query($sql);
            }
        ...
        if ($_REQUEST['act'] == 'restore_backup') {
            ...
            if ($remarks) {
                ...
                if ($arr) {
                    ...
                    /* 文件修改成功后，恢复数据库 */
                    $sql = "DELETE FROM " . $this->sanmu->table('template') .
                        " WHERE remarks = '' AND  theme = '" . $GLOBALS['_CFG']['template'] . "'" .
