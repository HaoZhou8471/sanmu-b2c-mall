...
class UsersController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'add') {
            ...
            /* 取出注册扩展字段 */
            $sql = 'SELECT * FROM ' . $this->sanmu->table('reg_fields') . ' WHERE type < 2 AND display = 1 ORDER BY dis_order, id';
        ...
        elseif ($_REQUEST['act'] == 'insert') {
            ...
            if (!empty($other['mobile_phone'])) {
                $sql = "SELECT user_id FROM " . $this->sanmu->table('users') . " WHERE mobile_phone = '" . $other['mobile_phone'] . "'";
            ...
            /*把新注册用户的扩展信息插入数据库*/
            $sql = 'SELECT id FROM ' . $this->sanmu->table('reg_fields') . ' WHERE type = 0 AND display = 1 ORDER BY dis_order, id';   //读出所有扩展字段的id
            ...
            if ($extend_field_str) {      //插入注册扩展数据
                $sql = 'INSERT INTO ' . $this->sanmu->table('reg_extend_info') . ' (`user_id`, `reg_field_id`, `content`) VALUES' . $extend_field_str;
                $this->db->query($sql);
            }
            ...
            $this->db->autoExecute($this->sanmu->table('users'), $other, 'UPDATE', "user_name = '$username'");
        ...
        elseif ($_REQUEST['act'] == 'edit') {
            ...
            $sql = "SELECT u.user_name, u.sex, u.birthday, u.pay_points, u.rank_points, u.user_rank , " .
                ...
                " FROM " . $this->sanmu->table('users') . " u LEFT JOIN " . $this->sanmu->table('users') . " u2 ON u.parent_id = u2.user_id " .
                " LEFT JOIN " . $this->sanmu->table('users_real') . " r ON u.user_id = r.user_id " .
            ...
            if ($row) {
                ...
                if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                    $row['mobile_phone'] = $this->sanmuRepository->stringToStar($row['mobile_phone']);
                    $user['user_name'] = $this->sanmuRepository->stringToStar($user['user_name']);
                    $user['email'] = $this->sanmuRepository->stringToStar($user['email']);
                }
            ...
            $sql = 'SELECT reg_field_id, content ' .
                'FROM ' . $this->sanmu->table('reg_extend_info') .
                ...
                if (empty($affiliate['config']['separate_by'])) {
                    ...
                    if ($num) {
                        for ($i = 1; $i <= $num; $i++) {
                            $count = 0;
                            if ($up_uid) {
                                $sql = "SELECT user_id FROM " . $this->sanmu->table('users') . " WHERE parent_id IN($up_uid)";
        ...
        elseif ($_REQUEST['act'] == 'update') {
            ...
            if (!empty($password)) {
                $sql = "UPDATE " . $this->sanmu->table('users') . "SET `ec_salt`='0' WHERE user_name= '" . $username . "'";
                $this->db->query($sql);
            }
            /* 更新用户扩展字段的数据 */
            $sql = 'SELECT id FROM ' . $this->sanmu->table('reg_fields') . ' WHERE type = 0 AND display = 1 ORDER BY dis_order, id';   //读出所有扩展字段的id
            ...
            foreach ($fields_arr as $val) {       //循环更新扩展用户信息
                ...
                if (isset($_POST[$extend_field_index])) {
                    ...
                    $sql = 'SELECT * FROM ' . $this->sanmu->table('reg_extend_info') . "  WHERE reg_field_id = '$val[id]' AND user_id = '$user_id'";
                    if ($this->db->getOne($sql)) {      //如果之前没有记录，则插入
                        $sql = 'UPDATE ' . $this->sanmu->table('reg_extend_info') . " SET content = '$temp_field_content' WHERE reg_field_id = '$val[id]' AND user_id = '$user_id'";
                    } else {
                        $sql = 'INSERT INTO ' . $this->sanmu->table('reg_extend_info') . " (`user_id`, `reg_field_id`, `content`) VALUES ('$user_id', '$val[id]', '$temp_field_content')";
                    }
                    $this->db->query($sql);
                }
            }
            ...
            //验证手机是否存在
            if (!empty($other['mobile_phone'])) {
                $sql = "SELECT user_id FROM " . $this->sanmu->table('users') . " WHERE mobile_phone = '$other[mobile_phone]' AND user_id != '$id'";
                if ($this->db->getOne($sql) > 0) {
            ...
            if ($id > 0) {
                $this->db->autoExecute($this->sanmu->table('users'), $other, 'UPDATE', "user_id = '$id'");
            } else {
                $this->db->autoExecute($this->sanmu->table('users'), $other, 'UPDATE', "user_name = '$username'");
            }
        ...
        elseif ($_REQUEST['act'] == 'batch_remove') {
            ...
            if (isset($_POST['checkboxes'])) {
                ...
                $priv_str = $this->db->getOne("SELECT action_list FROM " . $this->sanmu->table('admin_user') . " WHERE user_id = '" . session('admin_id') . "'");
                if ($priv_str != 'all') {
                    foreach ($_POST['checkboxes'] as $key => $val) {
                        $sql = "SELECT id FROM " . $this->sanmu->table('seller_shopinfo') . " WHERE ru_id = '$val'";
                ...
                $sql = "SELECT user_name FROM " . $this->sanmu->table('users') . " WHERE user_id " . db_create_in($_POST['checkboxes']);
        ...
        elseif ($_REQUEST['act'] == 'remove') {
            ...
            $sql = "SELECT user_name FROM " . $this->sanmu->table('users') . " WHERE user_id = '$user_id'";
            ...
            /* 只有超级管理员才可以删除商家会员 by wu */
            $sql = "SELECT shop_id FROM " . $this->sanmu->table('merchants_shop_information') . " WHERE user_id = '$user_id'";
        ...
        elseif ($_REQUEST['act'] == 'address_list') {
            ...
            $sql = "SELECT a.*, c.region_name AS country_name, p.region_name AS province, ct.region_name AS city_name, d.region_name AS district_name " .
                " FROM " . $this->sanmu->table('user_address') . " as a " .
                " LEFT JOIN " . $this->sanmu->table('region') . " AS c ON c.region_id = a.country " .
                " LEFT JOIN " . $this->sanmu->table('region') . " AS p ON p.region_id = a.province " .
                " LEFT JOIN " . $this->sanmu->table('region') . " AS ct ON ct.region_id = a.city " .
                " LEFT JOIN " . $this->sanmu->table('region') . " AS d ON d.region_id = a.district " .
                " WHERE user_id='$id'";
            ...
            if ($address) {
                foreach ($address as $key => $row) {
                    if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                        $address[$key]['tel'] = $this->sanmuRepository->stringToStar($row['tel']);
                        $address[$key]['mobile'] = $this->sanmuRepository->stringToStar($row['mobile']);
                    }
                }
            }
        ...
        elseif ($_REQUEST['act'] == 'remove_parent') {
            ...
            $sql = "UPDATE " . $this->sanmu->table('users') . " SET parent_id = 0 WHERE user_id = '" . $_GET['id'] . "'";
            $this->db->query($sql);

            /* 记录管理员操作 */
            $sql = "SELECT user_name FROM " . $this->sanmu->table('users') . " WHERE user_id = '" . $_GET['id'] . "'";
            $username = $this->db->getOne($sql);
            ...
            for ($i = 1; $i <= $num; $i++) {
                ...
                if ($count) {
                    $sql = "SELECT user_id, user_name, '$i' AS level, email, is_validated, user_money, frozen_money, rank_points, pay_points, reg_time " .
                        " FROM " . $this->sanmu->table('users') . " WHERE user_id IN($up_uid)" .
        ...
        elseif ($_REQUEST['act'] == 'batch_log') {
            ...
            if (empty($_POST['checkboxes'])) {
                ...
                /* 删除记录 */
                $sql = "DELETE FROM " . $this->sanmu->table('users_log') .
    ...
    private function get_user_log()
    {
        ...
        if ($result === false) {
            ...
            /* 查询记录总数，计算分页数 */
            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('users_log') . $where;
            $filter['record_count'] = $this->db->getOne($sql);
            $filter = page_and_size($filter);

            /* 查询记录 */

            $sql = "SELECT log_id,user_id,change_time,change_type,ip_address,change_city,logon_service,admin_id FROM" . $this->sanmu->table('users_log')
        ...
        foreach ($res as $rows) {
            ...
            if ($rows['admin_id'] > 0) {
                $sql = 'SELECT user_name FROM' . $this->sanmu->table('admin_user') . " WHERE user_id = '" . $rows['admin_id'] . "'";
        ...
        if ($result === false) {
            ...
            if ($filter['store_search'] != 0) {
                if ($adminru['ru_id'] == 0) {
                    ...
                    if ($filter['store_search'] > 1) {
                        $ex_where .= " AND (SELECT msi.user_id FROM " . $this->sanmu->table('merchants_shop_information') . ' as msi ' .
            ...
            if ($filter['rank']) {
//                $sql = "SELECT min_points, max_points, special_rank FROM " . $this->sanmu->table('user_rank') . " WHERE rank_id = '$filter[rank]'";
//                $row = $this->db->getRow($sql);
...
//                }
            ...
            $filter['record_count'] = $this->db->getOne("SELECT COUNT(*) FROM " . $this->sanmu->table('users') . " AS u " . $ex_where);
            ...
            $sql = "SELECT * FROM " . $this->sanmu->table('users') . " AS u " . $ex_where .
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                $user_list[$i]['mobile_phone'] = $this->sanmuRepository->stringToStar($user_list[$i]['mobile_phone']);
                $user_list[$i]['user_name'] = $this->sanmuRepository->stringToStar($user_list[$i]['user_name']);
                $user_list[$i]['email'] = $this->sanmuRepository->stringToStar($user_list[$i]['email']);
            }
            $user_list[$i]['user_picture'] = $this->sanmuRepository->getImagePath($user_list[$i]['user_picture']);
        }
    ...
    private function user_update($user_id, $args)
    {
        ...
        return $this->db->autoExecute($this->sanmu->table('users'), $args, 'update', "user_id='$user_id'");
    }
}
