    ...
    public function index()
    {
        ...
        elseif ($act == 'insert_gallery_url') {
            ...
            if (!empty($external_url)) {
                // 验证商品图片外链格式
                $res = $this->sanmuRepository->checkImageUrl($external_url);
                if ($res == false) {
                    $result['error'] = 2;
        ...
        elseif ($act == 'home_adv') {
            ...
            if ($result['mode'] == 'h-brand') {
                ...
                $now = gmtime();
                //获取秒杀时间段
                $sql = " SELECT id, title, begin_time, end_time FROM " . $GLOBALS['sanmu']->table('seckill_time_bucket') . " ORDER BY begin_time ASC ";
        ...
        elseif ($act == 'add_goods_type_cat') {
            ...
            if ($type == 'add_goods_type_cat' || $type == 'add_goods_type') {
                ...
                $this->sanmuRepository->helpersLang('attribute', 'admin');
        ...
        elseif ($act == 'ajaxBrand') {
            ...
            $this->sanmuRepository->helpersLang('brand', 'admin');

            $result = ['content' => '', 'mode' => ''];

            $this->smarty->assign('is_need', $this->config['template'] == 'ecmoban_sanmu2017' ? 1 : 0);
            $this->smarty->assign("temp", $act);
            $this->smarty->assign("lang", $GLOBALS['_LANG']);
        ...
        elseif ($act == 'brand_insert') {
            ...
            $this->sanmuRepository->getOssAddFile([DATA_DIR . '/brandlogo/' . $img_name]);
            /* 品牌专区大图 by wu start */
            $index_img = isset($_FILES['index_img']) ? basename($image->upload_image($_FILES['index_img'], 'indeximg')) : 0;
            $this->sanmuRepository->getOssAddFile([DATA_DIR . '/indeximg/' . $index_img]);
            /* 品牌专区大图 by wu end */

            /* 品牌背景图 start */
            $brand_bg = isset($_FILES['brand_bg']) ? basename($image->upload_image($_FILES['brand_bg'], 'brandbg')) : 0;
            $this->sanmuRepository->getOssAddFile([DATA_DIR . '/brandbg/' . $brand_bg]);
            /* 品牌背景图 end */
        ...
        elseif ($act == 'ajaxTransport') {
            ...
            $this->sanmuRepository->helpersLang('goods_transport', 'admin');
        ...
        elseif ($act == 'ajaxArticle') {
            ...
            $this->sanmuRepository->helpersLang('article', 'admin');
        ...
        elseif ($act == 'ajaxArea') {
            ...
            $this->sanmuRepository->helpersLang('area_manage', 'admin');

            $exc = new Exchange($this->sanmu->table('region'), $db, 'region_id', 'region_name');
            $result = ['content' => '', 'mode' => ''];
            $is_ajax = 1;
        ...
        elseif ($act == 'submit_empower') {
            ...
            $res = $this->sanmuRepository->sanmuEmpower($AppKey, $activate_time);
        ...
        elseif ($act == 'ajaxCate') {
            ...
            $this->sanmuRepository->helpersLang('category', 'admin');
        ...
        elseif ($act == 'cate_insert') {
            $result = ['content' => '', 'message' => '', 'error' => 0];
            $exc = new Exchange($this->sanmu->table("category"), $db, 'cat_id', 'cat_name');
            ...
            if (!empty($_FILES['cat_icon']['name'])) {
                ...
                $this->sanmuRepository->getOssAddFile([$cat['cat_icon']]); //oss存储图片
            }
            //上传分类菜单图标 by wu end
