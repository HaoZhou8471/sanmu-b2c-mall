    ...
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'view') {
            ...
            if (!$is_multi) {
                ...
                $sql = "SELECT COUNT(*) AS access_count, area FROM " . $this->sanmu->table('stats') .
                    " WHERE access_time >= '$start_date' AND access_time < " . ($end_date + 86400) .
                ...
                $sql = "SELECT access_time, area FROM " . $this->sanmu->table('stats') .
                    " WHERE $where";
                $res = $this->db->query($sql);
                $area_arr = [];
                foreach ($res as $val) {
            ...
            if (!$is_multi) {
                ...
                $sql = "SELECT COUNT(*) AS access_count, referer_domain FROM " . $this->sanmu->table('stats') .
                    " WHERE access_time >= '$start_date' AND access_time <= " . ($end_date + 86400) .
                ...
                $sql = "SELECT access_time, referer_domain FROM " . $this->sanmu->table('stats') .
                    " WHERE $where";
        ...
        elseif ($act = 'download') {
            ...
            if (EC_CHARSET != 'gbk') {
                echo sanmu_iconv(EC_CHARSET, 'gbk', $data) . "\t";
            } else {
                echo $data . "\t";
            }
        }
