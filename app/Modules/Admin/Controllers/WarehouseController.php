...
class WarehouseController extends InitController
{
    ...
    {
        $exc = new Exchange($this->sanmu->table('region_warehouse'), $this->db, 'region_id', 'region_name');
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            if ($region_id > 0) {
                ...
                /* 获取顶级仓库名称 */
                $regionType = $this->db->getOne("SELECT MAX(region_type) FROM" . $this->sanmu->table('region_warehouse'));
                for ($i = 0; $i < $regionType; $i++) {
                    $sql = "SELECT parent_id ,region_id FROM " . $this->sanmu->table('region_warehouse') . " WHERE region_id = '$region_id'";
        ...
        elseif ($_REQUEST['act'] == 'ship_list') {
            ...
            /* 获取商家设置的配送方式 by wu */
            $sql = " select ru_id, shipping_id from " . $this->sanmu->table("seller_shopinfo") . " where ru_id='" . $adminru['ru_id'] . "' ";
            ...
            foreach ($shipping_list as $key => $val) {
                $sql = "SELECT shipping_desc, insure, support_cod FROM " . $this->sanmu->table('shipping') . " WHERE shipping_id = '$val[shipping_id]' ";
        ...
        elseif ($_REQUEST['act'] == 'remove_tpl') {
            ...
            $sql = "DELETE FROM " . $this->sanmu->table('warehouse_freight_tpl') . " WHERE id='$id'";
            ...
            $sql = "DELETE FROM " . $this->sanmu->table('warehouse_freight_tpl') . " WHERE id in (" . $ids . ")";
            if ($this->db->query($sql)) {
        ...
        if ($_REQUEST['act'] == 'tpl_list') {
            ...
            //上门取货
            $sql = "SELECT shipping_code,shipping_name FROM " . $this->sanmu->table("shipping") . " WHERE shipping_id=" . $shipping_id;
        ...
        elseif ($_REQUEST['act'] == 'ship_tpl') {
            ...
            $sql = "SELECT a.shipping_name, a.shipping_code, a.support_cod, b.* " .
                " FROM " . $this->sanmu->table('warehouse_freight_tpl') . " AS b " .
                " left join " . $this->sanmu->table('shipping') . " AS a on a.shipping_id=b.shipping_id " .
            ...
            if (!empty($row)) {
                ...
                if (!empty($row) && $row['shipping_code'] == "cac") {
                    $sql = "SELECT * FROM " . $this->sanmu->table('shipping_point') . " WHERE shipping_area_id='{$row['shipping_area_id']}'";
                ...
                $shipping = $this->db->getRow("SELECT shipping_name, shipping_code FROM " . $this->sanmu->table('shipping') . " WHERE shipping_id='$shipping_id'");
            ...
            //设置仓库状态 by wu
            $sql = " SELECT warehouse_id from " . $this->sanmu->table('warehouse_freight_tpl') . " where id='$id' and shipping_id='$shipping_id' and user_id='" . $adminru['ru_id'] . "' ";
            ...
            $sql = " SELECT region_id from " . $this->sanmu->table('warehouse_freight_tpl') . " where id='$id' and shipping_id='$shipping_id' and user_id='" . $adminru['ru_id'] . "' ";
            $region_list = $res = $this->db->getOne($sql);
            if (!empty($region_list)) {
                $sql = " SELECT region_id,region_name from " . $this->sanmu->table('region') . " where region_id in (" . $region_list . ") ";
        ...
        elseif ($_REQUEST['act'] == 'freight_tpl_insert') {
            ...
            $sql = "select warehouse_id,region_id from " . $this->sanmu->table('warehouse_freight_tpl') . " where shipping_id = '$shipping_id' and user_id = '" . $adminru['ru_id'] . "'" . $where;
            ...
            $shipping_code = $this->db->getOne("SELECT shipping_code FROM " . $this->sanmu->table('shipping') .
            ...
            $sql = " select * from " . $this->sanmu->table('warehouse_freight_tpl') . " where shipping_id='$shipping_id' and user_id='" . $adminru['ru_id'] . "' ";
            $tpl_status = $this->db->getRow($sql);
            if (empty($tpl_status) || empty($id)) {
                $this->db->autoExecute($this->sanmu->table('warehouse_freight_tpl'), $other, 'INSERT');
                $add_to_mess = $GLOBALS['_LANG']['template_add_success'];
            } else {
                $this->db->autoExecute($this->sanmu->table('warehouse_freight_tpl'), $other, 'UPDATE', ' id= ' . $id . ' and user_id = "' . $adminru['ru_id'] . '" and shipping_id= ' . $shipping_id);
        ...
        elseif ($_REQUEST['act'] == 'add_area') {
            ...
            /* 查看区域是否重复 */
            $sql = "SELECT region_id FROM " . $this->sanmu->table('region_warehouse') . " WHERE regionId = '$regionId' ";
            $res = $this->db->getOne($sql);

            if ($res > 0 && $regionId) {
                return make_json_error($GLOBALS['_LANG']['region_name_exist']);
            } else {
                $sql = "select region_id from " . $this->sanmu->table('region_warehouse') . " where region_name = '$region_name' AND region_type <> 1 AND regionId = '$regionId'";
                $res = $this->db->getOne($sql);
            ...
            $sql = "INSERT INTO " . $this->sanmu->table('region_warehouse') . " (regionId, parent_id, region_name, region_code, region_type) " .
            ...
            if ($this->db->query($sql, 'SILENT')) {
                ...
                if ($parent_id) {
                    $sql = "SELECT * FROM " . $this->sanmu->table('region') . " WHERE parent_id = '$regionId'";
                    ...
                    foreach ($region_list1 as $key => $row) {
                        ...
                        $sql = "SELECT * FROM " . $this->sanmu->table('region') . " WHERE parent_id = '" . $row['region_id'] . "'";
                        ...
                        foreach ($region_list2 as $k => $r) {
                            ...
                            $sql = "SELECT * FROM " . $this->sanmu->table('region') . " WHERE parent_id = '" . $r['region_id'] . "'";
        ...
        elseif ($_REQUEST['act'] == 'drop_area') {
            ...
            $sql = "SELECT * FROM " . $this->sanmu->table('region_warehouse') . " WHERE region_id = '$id'";
            ...
            if (is_array($delete_region)) {
                foreach ($delete_region as $v) {
                    $sql = " SELECT regionId FROM " . $this->sanmu->table('region_warehouse') . " WHERE region_id = '$v' ";
                    $regionIds[] = $this->db->getOne($sql);
                }
            }

            $sql = " DELETE FROM " . $this->sanmu->table("warehouse_freight") . " WHERE region_id " . db_create_in($regionIds);
            $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'freight') {
            ...
            $sql = "select region_name from " . $this->sanmu->table('region_warehouse') . " where region_id = '$region_id'";
            ...
            $sql = "select regionId from " . $this->sanmu->table('region_warehouse') . " where region_id = '$region_id'";
        ...
        elseif ($_REQUEST['act'] == 'freight_insert') {
            ...
            $sql = "select regionId from " . $this->sanmu->table('region_warehouse') . " where regionId = '$regionId'";
            $regionId = $this->db->getOne($sql);
            ...
            $sql = "SELECT id FROM " . $this->sanmu->table('warehouse_freight') . " WHERE warehouse_id = '$warehouse_id' and shipping_id = '$shipping_id' and region_id = '$regionId' AND user_id = '" . $adminru['ru_id'] . "'";
            $id = $this->db->getOne($sql);

            if ($id) {
                $this->db->autoExecute($this->sanmu->table('warehouse_freight'), $other, 'UPDATE', "id='$id'");
                $add_to_mess = $GLOBALS['_LANG']['freight_edit_success'];
            } else {
                $this->db->autoExecute($this->sanmu->table('warehouse_freight'), $other, 'INSERT');
        ...
        elseif ($_REQUEST['act'] == 'get_freight_area') {
            ...
            $sql = "SELECT s.*, wf.id, wf.configure, rw1.region_name as region_name1, rw2.region_name as region_name2 FROM " . $this->sanmu->table('warehouse_freight') . " AS wf" .
                " LEFT JOIN " . $this->sanmu->table('shipping') . " AS s ON wf.shipping_id = s.shipping_id" .
                " LEFT JOIN " . $this->sanmu->table('region_warehouse') . " AS rw1 ON wf.warehouse_id = rw1.region_id" .
                " LEFT JOIN " . $this->sanmu->table('region_warehouse') . " AS rw2 ON wf.region_id = rw2.regionId" .
            ...
            if ($shipping) {
                ...
                $sql = "SELECT shipping_name, shipping_code FROM " . $this->sanmu->table('shipping') . " WHERE shipping_id='$shipping_id'";
                $shipping = $this->db->getRow($sql);
    ...
    private function new_region_id($region_id)
    {
        ...
        $sql = "SELECT region_id FROM " . $this->sanmu->table("region_warehouse") . "WHERE parent_id " . db_create_in($region_id);
    ...
    {
        $sql = "SELECT region_id, region_name FROM " . $this->sanmu->table('region') . " WHERE region_type = '$type'";
        $res = $this->db->getAll($sql);
    ...
    {
        $sql = "select region_name from " . $this->sanmu->table('region') . " where region_id = '$region_id'";
    ...
    {
        $sql = "SELECT region_id, parent_id, region_name FROM " . $this->sanmu->table('region_warehouse') . " WHERE region_id = '$region_id'";
        $res = $this->db->getAll($sql);
