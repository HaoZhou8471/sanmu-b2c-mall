...
class ValueCardController extends InitController
{
    ...
    public function index()
    {
        ...
        /* 初始化$exc对象 */
        $exc = new Exchange($this->sanmu->table('value_card_type'), $this->db, 'id', 'name');
        ...
        if ($_REQUEST['act'] == 'vc_type_add' || $_REQUEST['act'] == 'vc_type_edit') {
            ...
            if ($_REQUEST['act'] == 'vc_type_add') {
                ...
                $sql = " SELECT * FROM " . $this->sanmu->table('value_card_type') . " WHERE id = '$id' ";
        ...
        if ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
            ...
            if ($id > 0) {
                $sql = " UPDATE " . $this->sanmu->table('value_card_type') . " SET " .
                    " name = '$name', " .
                    " vc_desc = '$vc_desc', " .
                    " vc_limit = '$vc_limit', " .
                    " vc_value = '$vc_value', " .
        ...
        if ($_REQUEST['act'] == 'remove') {
            ...
            $sql = " SELECT COUNT(*) FROM " . $this->sanmu->table('value_card') . " WHERE tid = '$id' AND user_id > 0 ";
            $row = $this->db->getOne($sql);
            if ($row > 0) {
                return make_json_error($GLOBALS['_LANG']['notice_remove_type_error']);
            } else {
                $exc->drop($id);
                $sql = " DELETE FROM " . $this->sanmu->table('value_card') . " WHERE tid = '$id' ";
                $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'batch') {
            ...
            if (empty($_POST['checkboxes'])) {
                ...
                //检查是否存在已绑定用户的储值卡 如果有则无法删除
                $sql = " SELECT COUNT(*) FROM " . $this->sanmu->table('value_card') . " WHERE tid" . db_create_in($ids) . " AND user_id > 0 ";
                $row = $this->db->getOne($sql);
                if (isset($_POST['drop'])) {
                    if ($row > 0) {
                        ...
                        /* 删除记录 */
                        $sql = "DELETE FROM " . $this->sanmu->table('value_card_type') .
                            " WHERE id " . db_create_in($ids);
                        $res = $this->db->query($sql);
                        if ($res) {
                            $sql = " DELETE FROM " . $this->sanmu->table('value_card') . " WHERE tid " . db_create_in($ids);
                            $this->db->query($sql);
                        }
        ...
        if ($_REQUEST['act'] == 'remove_vc') {
            ...
            //检查是否存在已绑定用户的储值卡 如果有则无法删除
            $sql = " SELECT user_id FROM " . $this->sanmu->table('value_card') . " WHERE vid = '$id' ";
            $row = $this->db->getOne($sql);
            if ($row > 0) {
                return make_json_error($GLOBALS['_LANG']['notice_remove_vc_error']);
            } else {
                $sql = " DELETE FROM " . $this->sanmu->table('value_card') . " WHERE vid = '$id' ";
                $this->db->query($sql);
        ...
        if ($_REQUEST['act'] == 'export_vc_list') {
            ...
            $arr = [];
            $sql = " SELECT vc.vid,vc.value_card_sn,vc.value_card_password,vc.vc_value,vc.bind_time, t.name, u.user_name FROM " . $this->sanmu->table('value_card') . " AS vc " .
                " LEFT JOIN " . $this->sanmu->table('value_card_type') . " AS t ON vc.tid = t.id " .
                " LEFT JOIN " . $this->sanmu->table('users') . " AS u ON u.user_id = vc.user_id " .
                $where;
        ...
        if ($_REQUEST['act'] == 'send_value_card') {
            ...
            $sql = " SELECT vc_value, vc_prefix FROM " . $this->sanmu->table('value_card_type') . " WHERE id = '$tid' ";
            $row = $this->db->getRow($sql);
            ...
            /* 生成储值卡序列号 */
            $num = $this->db->getOne(" SELECT MAX(SUBSTRING(value_card_sn,$prefix_len+1)) FROM " . $this->sanmu->table('value_card') . " WHERE tid = '$tid' AND LENGTH(value_card_sn) = '$length' ");
            ...
            for ($i = 0, $j = 0; $i < $send_sum; $i++) {
                ...
                $this->db->query("INSERT INTO " . $this->sanmu->table('value_card') . " (tid, value_card_sn, value_card_password, vc_value, card_money) VALUES('$tid', '$value_card_sn', '$value_card_password', '$row[vc_value]', '$row[vc_value]')");
                $j++;
            }
        ...
        if ($_REQUEST['act'] == 'select_merchants') {
            ...
            $sql = " SELECT ru_id FROM " . $this->sanmu->table('seller_shopinfo') . " WHERE ru_id > 0 ";
    ...
    private function vc_type_list()
    {
        ...
        if ($result === false) {
            ...
            $sql = " SELECT COUNT(*) FROM " . $this->sanmu->table('value_card_type') . " AS t " . $where;
            $filter['record_count'] = $this->db->getOne($sql);

            /* 分页大小 */
            $filter = page_and_size($filter);

            $sql = "SELECT * FROM " . $this->sanmu->table('value_card_type') . " AS t" . " $where ORDER BY $filter[sort_by] $filter[sort_order]";
    ...
    private function vc_list()
    {
        ...
        if ($result === false) {
            ...
            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('value_card') . $where;
            $filter['record_count'] = $this->db->getOne($sql);

            /* 分页大小 */
            $filter = page_and_size($filter);

            $sql = " SELECT vc.*, t.name, u.user_name FROM " . $this->sanmu->table('value_card') . " AS vc " .
                " LEFT JOIN " . $this->sanmu->table('value_card_type') . " AS t ON vc.tid = t.id " .
                " LEFT JOIN " . $this->sanmu->table('users') . " AS u ON u.user_id = vc.user_id " .
                $where .
    ...
    {
        $sql = " SELECT COUNT(*) FROM " . $this->sanmu->table('value_card') . " WHERE tid = '$id' ";
    ...
    {
        $sql = " SELECT COUNT(*) FROM " . $this->sanmu->table('value_card') . " WHERE tid = '$id' AND user_id > 0 AND bind_time > 0 ";
        return $this->db->getOne($sql);
    }
}
