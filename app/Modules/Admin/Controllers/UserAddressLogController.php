...
class UserAddressLogController extends InitController
{
    ...
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            $sql = "SELECT rank_id, rank_name, min_points FROM " . $this->sanmu->table('user_rank') . " ORDER BY min_points ASC ";
        ...
        elseif ($_REQUEST['act'] == 'edit') {
            ...
            if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0 && $consignee) {
                $consignee['mobile'] = $this->sanmuRepository->stringToStar($consignee['mobile']);
                $consignee['email'] = $this->sanmuRepository->stringToStar($consignee['email']);
            }
        ...
        elseif ($_REQUEST['act'] == 'update') {
            ...
            //更新到收货地址表中
            $this->db->autoExecute($this->sanmu->table('user_address'), $other, 'UPDATE', "address_id = '$address_id' and user_id = '$user_id'");
    ...
    private function user_address_list_log()
    {
        ...
        if ($result === false) {
            ...
            $filter['record_count'] = $this->db->getOne("SELECT COUNT(*) FROM " . $this->sanmu->table('user_address') . " as a " .
                "left join " . $this->sanmu->table('users') . " as u on a.user_id = u.user_id " . $ex_where);
            ...
            $sql = "SELECT concat(IFNULL(c.region_name, ''), '  ', IFNULL(p.region_name, ''), " .
                ...
                " FROM " . $this->sanmu->table('user_address') . " as a left join" .

                $this->sanmu->table('users') . " as u on a.user_id = u.user_id " .

                "LEFT JOIN " . $this->sanmu->table('region') . " AS c ON a.country = c.region_id " .
                "LEFT JOIN " . $this->sanmu->table('region') . " AS p ON a.province = p.region_id " .
                "LEFT JOIN " . $this->sanmu->table('region') . " AS t ON a.city = t.region_id " .
                "LEFT JOIN " . $this->sanmu->table('region') . " AS d ON a.district = d.region_id " .
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                $address_list[$i]['user_name'] = $this->sanmuRepository->stringToStar($address_list[$i]['user_name']);
                $address_list[$i]['mobile'] = $this->sanmuRepository->stringToStar($address_list[$i]['mobile']);
                $address_list[$i]['email'] = $this->sanmuRepository->stringToStar($address_list[$i]['email']);
            }
    ...
    {
        $sql = "SELECT * FROM " . $this->sanmu->table('user_address') .
    ...
    {
        $sql = 'SELECT region_id, region_name FROM ' . $this->sanmu->table('region') .
    ...
    {
        if ($open == 1) {
            $sql = "delete from " . $this->sanmu->table('user_address') . " where address_id = " . $address_id['address_id'];
            $this->db->query($sql);
        } else {
            if (count($address_id) > 0) {
                for ($i = 0; $i < count($address_id); $i++) {
                    $sql = "delete from " . $this->sanmu->table('user_address') . " where address_id = " . $address_id[$i];
                    $this->db->query($sql);
                }
            }
        }
