...
class VisualhomeController extends InitController
{
    ...
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            if ($adminru['rs_id'] > 0) {
                $sql = "SELECT rs_id FROM" . $this->sanmu->table('home_templates') . "WHERE code = '$code'";
            ...
            if (empty($code)) {
                $sql = "SELECT value FROM" . $this->sanmu->table('shop_config') . " WHERE code= 'hometheme' AND store_range = '" . $GLOBALS['_CFG']['template'] . "'";
            ...
            if (!file_exists($des . "/" . $code . "/nav_html.php") && !file_exists($des . "/" . $code . "/temp/nav_html.php")) {
                ...
                $sql = "SELECT id, name, ifshow, vieworder, opennew, url, type" .
                    " FROM " . $this->sanmu->table('nav') . "WHERE type = 'middle'";
            ...
            $replace_data = [
                'http://localhost/ecmoban_sanmu2.0.5_20170518/',
                'http://localhost/ecmoban_sanmu2.2.6_20170727/',
                'http://localhost/ecmoban_sanmu2.3/'
            ];
            $arr['out'] = str_replace($replace_data, $this->sanmu->url(), $arr['out']);
        ...
        elseif ($_REQUEST['act'] == 'edit_information') {
            ...
            if ($html === false) {
                ...
                if ($check == 1 && $template_type != 'seller') {
                    ...
                    if ($temp_id == 0) {
                        //模板入库
                        $sql = "INSERT INTO" . $this->sanmu->table('home_templates') . "(`rs_id`,`code`,`theme`) VALUES ('" . $adminru['rs_id'] . "','$tem','" . $GLOBALS['_CFG']['template'] . "')";
                        $this->db->query($sql);
                    }
                    //  卖场首页模板 end
                ...
                elseif ($template_type == 'seller') {
                    if ($temp_id > 0) {
                        $sql = "UPDATE" . $this->sanmu->table('template_mall') . "SET temp_mode = '$temp_mode',temp_cost='$temp_cost' WHERE temp_id = '$temp_id' AND temp_code = '$tem'";
                    } else {
                        $time = gmtime();
                        $sql = "INSERT INTO" . $this->sanmu->table('template_mall') . "(`temp_mode`,`temp_cost`,`temp_code`,`add_time`) VALUES('$temp_mode','$temp_cost','$tem','$time')";
                    }
                    $this->db->query($sql);
                }
                $result['error'] = 0;
        ...
        elseif ($_REQUEST['act'] == 'removeTemplate') {
            ...
            if ($default_tem == $code && $template_type != 'seller') {
                ...
                if ($rmdir == true) {
                    //删除模板对应的左侧信息
                    $sql = "DELETE FROM" . $this->sanmu->table('templates_left') . "WHERE seller_templates = '$code' AND theme = '$theme'";
                    $this->db->query($sql);
                    $result['error'] = 0;

                    if ($template_type == 'seller') {
                        $sql = "DELETE FROM" . $this->sanmu->table('template_mall') . "WHERE temp_code = '$code' AND temp_id = '$temp_id'";
                        $this->db->query($sql);
                    } else {
                        //
                        $sql = "DELETE FROM" . $this->sanmu->table('home_templates') . "WHERE code = '$code' AND temp_id = '$temp_id'";
                        $this->db->query($sql);
                    }
                } else {
                    $result['error'] = 1;
        ...
        elseif ($_REQUEST['act'] == 'setupTemplate') {
            ...
            // start
            //获取卖场id
            $sql = "SELECT rs_id FROM" . $this->sanmu->table('home_templates') . "WHERE temp_id = '$temp_id'";
            $rs_id = $this->db->getOne($sql);
            ...
            if ($rs_id != $adminru['rs_id']) {
                ...
                //判断模板是否存在
                $sql = "UPDATE" . $this->sanmu->table('home_templates') . "SET is_enable = 1 WHERE temp_id = '$temp_id' AND theme = '" . $GLOBALS['_CFG']['template'] . "' AND rs_id = '$rs_id'";
                $this->db->query($sql);

                //修正正在使用的模板的状态
                $sql = "UPDATE" . $this->sanmu->table('home_templates') . "SET is_enable = 0 WHERE temp_id != '$temp_id' AND is_enable = 1 AND theme = '" . $GLOBALS['_CFG']['template'] . "'  AND rs_id = '$rs_id'";
                $this->db->query($sql);
            }
            // end
            return response()->json($result);
        ...
        elseif ($_REQUEST['act'] == 'downloadModal') {
            ...
            if ($is_oss && $server_model && $new == 0) {
                ...
                if ($id_data !== false) {
                    $this->sanmuRepository->getDelVisualTemplates($id_data, $code);
                }
            }
            /* 存入OSS end */
        ...
        elseif ($_REQUEST['act'] == 'bonusAdv') {
            ...
            $sql = "SELECT id ,img_file FROM" . $this->sanmu->table('templates_left') . " WHERE ru_id = 0 AND seller_templates = '$suffix' AND type = '$bgtype' AND theme = '$theme' LIMIT 1";
            ...
            if ($templates_left['id'] > 0) {
                ...
                if ($oss_img_url != '') {
                    if ($templates_left['img_file'] != '') {
                        ...
                        $this->sanmuRepository->getOssDelFile([$templates_left['img_file']]);
                    }
                    $fileurl = ",img_file = '$file_name'";
                }
                $sql = "UPDATE" . $this->sanmu->table('templates_left') . " SET fileurl = '$adv_url' $fileurl WHERE ru_id = 0 AND seller_templates = '$suffix' AND id='" . $templates_left['id'] . "' AND type = '$bgtype' AND theme = '$theme'";
                $this->db->query($sql);
            } else {
                $sql = "INSERT INTO" . $this->sanmu->table('templates_left') . " (`ru_id`,`seller_templates`,`img_file`,`type`,`theme`,`fileurl`) VALUES (0,'$suffix','$file_name','$bgtype','$theme','$adv_url')";
                $this->db->query($sql);
            }
        ...
        elseif ($_REQUEST['act'] == 'delete_adv') {
            ...
            $sql = "DELETE FROM" . $this->sanmu->table("templates_left") . "WHERE ru_id = 0 AND seller_templates = '$suffix' AND type = '$bgtype' AND theme = '$theme'";
            $this->db->query($sql);
            return response()->json($result);
        } //同步首页可视化模板
        elseif ($_REQUEST['act'] == 'update_home_temp') {
            ...
            if ($GLOBALS['_CFG']['update_home_temp'] == 1) {
                $enableTem = $this->db->getOne("SELECT value FROM" . $this->sanmu->table('shop_config') . " WHERE code= 'hometheme' AND store_range = '" . $GLOBALS['_CFG']['template'] . "'");
                /*默认模板*/
                $dir = storage_public(DATA_DIR . '/home_templates/' . $GLOBALS['_CFG']['template'] . '/');
                if (file_exists($dir)) {
                    ...
                    while ($file = readdir($template_dir)) {
                        if ($file != '.' && $file != '..' && $file != '.svn' && $file != 'index.htm') {
                            ...
                            $sql = "INSERT INTO" . $this->sanmu->table('home_templates') . "(`code`,`theme`,`is_enable`) VALUES('$file','" . $GLOBALS['_CFG']['template'] . "','$is_enable')";
                            $this->db->query($sql);
                        }
                    }
                ...
                $sql = "UPDATE" . $this->sanmu->table('shop_config') . "SET value = 0 WHERE code = 'update_home_temp'";
                $this->db->query($sql);
