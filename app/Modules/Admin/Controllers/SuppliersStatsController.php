...
class SuppliersStatsController extends InitController
{
    ...
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            echo sanmu_iconv(EC_CHARSET, 'GB2312', $data) . "\t";
            exit;
        }
    }
    ...
    private function get_suppliers_id_orderinfo($start_date, $end_date, $suppliers_id = 0)
    {
        ...
        $sql = 'SELECT o.order_id FROM ' . $GLOBALS['sanmu']->table('wholesale_order_info') . " as o " .
            ...
            " AND (select count(*) from " . $GLOBALS['sanmu']->table('wholesale_order_info') . " as oi2 where oi2.main_order_id = o.order_id) = 0";  //主订单下有子订单时，则主订单不显示
    ...
    private function get_order_general($type = 0)
    {
        ...
        $sql = "SELECT count(*) as total_order_num, SUM(o.order_amount) AS total_turnover,$today_amout,$today_num FROM " . $GLOBALS['sanmu']->table('wholesale_order_info') . ' as o' .
    ...
    private function get_pay_type($start_date = '', $end_date = '', $suppliers_id = 0)
    {
        ...
        $sql = 'SELECT i.pay_id, p.pay_name, i.pay_time ' .
            'FROM ' . $GLOBALS['sanmu']->table('payment') . ' AS p, ' . $GLOBALS['sanmu']->table('wholesale_order_info') . ' AS i ' .
            "WHERE p.pay_id = i.pay_id AND i.pay_status =2 AND i.order_status = 1 " .
            "AND i.add_time >= '$start_date' AND i.add_time <= '$end_date' " .
            $where . " AND (SELECT COUNT(*) FROM " . $GLOBALS['sanmu']->table('wholesale_order_info') . " AS oi2 WHERE oi2.main_order_id = i.order_id) = 0 " . //主订单下有子订单时，则主订单不显示;
    ...
    private function get_shipping_type($start_date = '', $end_date = '', $suppliers_id = 0)
    {
        ...
        $sql = 'SELECT sp.shipping_id, sp.shipping_name AS ship_name, i.shipping_time ' .
            'FROM ' . $GLOBALS['sanmu']->table('shipping') . ' AS sp, ' . $GLOBALS['sanmu']->table('wholesale_order_info') . ' AS i ' .
