...
class DatabaseController extends InitController
{
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'backup') {
            $tables = $this->db->GetCol("SHOW TABLES LIKE '" . mysql_like_quote($this->sanmu->prefix) . "%'");
        ...
        if ($_REQUEST['act'] == 'dumpsql') {
            ...
            switch ($type) {
                case 'full':
                    $except = [$this->sanmu->prefix . 'sessions', $this->sanmu->prefix . 'sessions_data'];
                    $temp = $this->db->GetCol("SHOW TABLES LIKE '" . mysql_like_quote($this->sanmu->prefix) . "%'");
                ...
                case 'stand':
                    ...
                    foreach ($temp as $table) {
                        $tables[$this->sanmu->prefix . $table] = -1;
                    }
                    $dump->put_tables_list($run_log, $tables);
                    break;
        ...
        if ($_REQUEST['act'] == 'optimize') {
            ...
            $db_ver = $db_ver_arr;
            $ret = $this->db->query("SHOW TABLE STATUS LIKE '" . mysql_like_quote($this->sanmu->prefix) . "%'");
