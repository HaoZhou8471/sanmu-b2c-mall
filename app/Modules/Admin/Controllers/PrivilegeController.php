...
class PrivilegeController extends InitController
{
    ...
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'login') {
            cookie()->queue('sanmuActionParam', '', 0);
        ...
        elseif ($_REQUEST['act'] == 'signin') {
            ...
            setcookie('sanmuActionParam', 'index|main', gmtime()); //登录后添加默认cookie信息
            ...
            if ($row) {
                ...
                $last_ip = $this->sanmuRepository->sanmuIp();
        ...
        elseif ($_REQUEST['act'] == 'update' || $_REQUEST['act'] == 'update_self') {
            ...
            if ($ru_id && $GLOBALS['_CFG']['sms_seller_signin'] == '1') {
                ...
                if (empty($shopinfo['mobile'])) {
                    $field = get_table_file_name($this->sanmu->table('merchants_steps_fields'), 'contactPhone');
                ...
                if ($adminru['ru_id'] == 0 && $template['template_content'] != '') {
                    $field = get_table_file_name($this->sanmu->table('merchants_steps_fields'), 'contactEmail');
