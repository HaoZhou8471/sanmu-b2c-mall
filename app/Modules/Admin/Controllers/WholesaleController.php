...
class WholesaleController extends InitController
{
    ...
    public function index()
    {
        ...
        $this->sanmuRepository->helpersLang(['goods'], 'admin');

        $exc = new Exchange($this->sanmu->table('wholesale'), $this->db, 'goods_id', 'goods_name');
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('wholesale_goods_attr') . " WHERE goods_id = 0 AND admin_id = '" . session('admin_id') . "'";
        ...
        elseif ($_REQUEST['act'] == 'update_review_status') {
            ...
            $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table('wholesale'), $other, "UPDATE", "goods_id = '$goods_id'");
        ...
        elseif ($_REQUEST['act'] == 'batch_add') {
            ...
            $sql = "SELECT rank_id, rank_name FROM " . $this->sanmu->table('user_rank') .
        ...
        elseif ($_REQUEST['act'] == 'batch_add_insert') {
            ...
            $sql = "SELECT goods_name, goods_id FROM " . $this->sanmu->table('goods') .
            ...
            if (isset($_POST['rank_id'])) {
                ...
                foreach ($_POST['rank_id'] as $rank_id) {
                    $sql = "SELECT COUNT(act_id) AS num, goods_id FROM " . $this->sanmu->table('wholesale') .
                        " WHERE goods_id IN ($dst_goods) " .
            ...
            foreach ($_POST['dst_goods_lists'] as $goods_value) {
                ...
                /* 保存数据 */
                $this->db->autoExecute($this->sanmu->table('wholesale'), $_wholesale, 'INSERT');
        ...
        elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit') {
            ...
            $this->smarty->assign('goods_type_name', $GLOBALS['db']->getOne(" SELECT cat_name FROM " . $GLOBALS['sanmu']->table('goods_type') . " WHERE cat_id = '0' "));
        ...
        elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
            ...
            $sql = "SELECT goods_name FROM " . $this->sanmu->table('goods') .
            ...
            //判断是否存在商品
            $sql = "SELECT goods_id FROM " . $this->sanmu->table('wholesale') . " WHERE goods_id = '$goods_id'";
            ...
            if (isset($_POST['rank_id'])) {
                foreach ($_POST['rank_id'] as $rank_id) {
                    $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('wholesale') .
                        " WHERE goods_id = '$goods_id' " .
                        " AND CONCAT(',', rank_ids, ',') LIKE CONCAT('%,', '$rank_id', ',%')";
                    if (!$is_add) {
            ...
            if ($goods_id) {
                ...
                $extend = $this->db->getOne("select count(goods_id) from " . $this->sanmu->table('wholesale_extend') . " where goods_id='$goods_id'");
                if ($extend > 0) {
                    //跟新商品扩展信息
                    $extend_sql = "update " . $this->sanmu->table('wholesale_extend') . " SET `is_delivery`='$is_delivery',`is_return`='$is_return',`is_free`='$is_free' WHERE goods_id='$goods_id'";
                } else {
                    //插入商品扩展信息
                    $extend_sql = "INSERT INTO " . $this->sanmu->table('wholesale_extend') . "(`goods_id`, `is_delivery`, `is_return`, `is_free`) VALUES ('$goods_id','$is_delivery','$is_return','$is_free')";
                }
            ...
            if ($is_add) {
                if ($have_goods) {
                    ...
                    $this->db->autoExecute($this->sanmu->table('wholesale'), $wholesale, 'INSERT');
                ...
                $this->db->autoExecute($this->sanmu->table('wholesale'), $wholesale, 'UPDATE', "act_id = '$wholesale[act_id]'");
            }
            ...
            if ((isset($_POST['attr_id_list']) && isset($_POST['attr_value_list'])) || (empty($_POST['attr_id_list']) && empty($_POST['attr_value_list']))) {
                ...
                $sql = "SELECT attr_id, attr_index FROM " . $this->sanmu->table('attribute') . " WHERE cat_id = '0'";
                ...
                FROM " . $this->sanmu->table('wholesale_goods_attr') . " AS ga
                ...
                foreach ($goods_attr_list as $attr_id => $attr_value_list) {
                    foreach ($attr_value_list as $attr_value => $info) {
                        if ($info['sign'] == 'insert') {
                            $sql = "INSERT INTO " . $this->sanmu->table('wholesale_goods_attr') . " (attr_id, goods_id, attr_value, attr_sort)" .
                                "VALUES ('$attr_id', '$goods_id', '$attr_value', '$info[attr_sort]')";
                        } elseif ($info['sign'] == 'update') {
                            $sql = "UPDATE " . $this->sanmu->table('wholesale_goods_attr') . " SET attr_sort = '$info[attr_sort]' WHERE goods_attr_id = '$info[goods_attr_id]' LIMIT 1";
                        } else {
                            $table = 'products';

                            $where = " AND goods_id = '$goods_id'";
                            $this->sanmu->get_del_find_in_set($info['goods_attr_id'], $where, $table, 'goods_attr', '|');

                            $sql = "DELETE FROM " . $this->sanmu->table('wholesale_goods_attr') . " WHERE goods_attr_id = '" . $info['goods_attr_id'] . "' LIMIT 1";
                        }
                        $this->db->query($sql);
                    }
                }
            ...
            foreach ($product['product_sn'] as $key => $value) {
                ...
                if ($product['product_id'][$key]) {
                    ...
                    if ($goods_product['product_number'] != $product['product_number'][$key]) {
                        ...
                        $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table('goods_inventory_logs'), $logs_other, 'INSERT');
                    }

                    $sql = "UPDATE " . $GLOBALS['sanmu']->table($table) . " SET product_number = '" . $product['product_number'][$key] . "'" .
                    ...
                    /* 插入货品表 */
                    $sql = "INSERT INTO " . $GLOBALS['sanmu']->table($table) .
                    ...
                    if (!$GLOBALS['db']->query($sql)) {
                        ...
                        //货品号为空 自动补货品号
                        if (empty($value)) {
                            $sql = "UPDATE " . $GLOBALS['sanmu']->table($table) . "
        ...
        elseif ($_REQUEST['act'] == 'get_goods_info') {
            ...
            $goods_type = $this->db->getOne(" SELECT goods_type FROM " . $this->sanmu->table('goods') . " WHERE goods_id = '$goods_id' ");
            $attribute = get_goods_specifications_list($goods_id);
            ...
            $this->smarty->assign('goods_type_name', $GLOBALS['db']->getOne(" SELECT cat_name FROM " . $GLOBALS['sanmu']->table('goods_type') . " WHERE cat_id = '$goods_type' "));
        ...
        elseif ($_REQUEST['act'] == 'drop_product') {
            ...
            /* 删除数据 */
            $sql = "DELETE FROM " . $GLOBALS['sanmu']->table($table) . " WHERE product_id = '$product_id' LIMIT 1";
            $GLOBALS['db']->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'set_attribute_table' || $_REQUEST['act'] == 'wholesale_attribute_query') {
            ...
            $region_name = $GLOBALS['db']->getOne(" SELECT region_name FROM " . $GLOBALS['sanmu']->table('region_warehouse') . " WHERE region_id ='$region_id' ");
            ...
            //商品基本信息
            $goods_info = $GLOBALS['db']->getRow(" SELECT market_price, shop_price, model_attr FROM " . $GLOBALS['sanmu']->table("goods") . " WHERE goods_id = '$goods_id' ");
            ...
            if (count($attr_arr) > 0) {
                ...
                foreach ($attr_arr as $key => $val) {
                    $sql = "SELECT attr_name, attr_type FROM " . $GLOBALS['sanmu']->table('attribute') . " WHERE attr_id ='$key' LIMIT 1";
                    ...
                    foreach ($val as $k => $v) {
                        ...
                        if (!$data) {
                            $sql = "SELECT MAX(goods_attr_id) AS goods_attr_id FROM " . $GLOBALS['sanmu']->table('wholesale_goods_attr') . " WHERE 1 ";
                            $max_goods_attr_id = $GLOBALS['db']->getOne($sql);
                            $attr_sort = $max_goods_attr_id + 1;

                            $sql = " INSERT INTO " . $GLOBALS['sanmu']->table('wholesale_goods_attr') . " (goods_id, attr_id, attr_value, attr_sort, admin_id) " .
                                " VALUES " .
        ...
        elseif ($_REQUEST['act'] == 'goods_info') {
            ...
            if (!empty($_REQUEST['spec_attr'])) {
                $spec_attr = sanmu_decode(stripslashes($_REQUEST['spec_attr']), true);
            }
            ...
            if ($spec_attr['goods_ids']) {
                ...
                if (!empty($goods_info)) {
                    ...
                    $sql = "SELECT g.goods_name,g.goods_id,g.goods_thumb,g.original_img,g.shop_price FROM " . $this->sanmu->table('goods') . " AS g " . $where;
        ...
        elseif ($_REQUEST['act'] == 'changedgoods') {
            ...
            //已参加活动的商品不在列表内
            $sql = " SELECT goods_id FROM " . $this->sanmu->table('wholesale') . " WHERE user_id = '" . $adminru['ru_id'] . "' ";
            ...
            if ($type == 1) {
                ...
                $sql = "SELECT g.promote_start_date, g.promote_end_date, g.promote_price, g.goods_name, g.goods_id, g.goods_thumb, g.shop_price, g.market_price, g.original_img FROM " .
                    $this->sanmu->table('goods') . " AS g " . $where . $sort;
                $goods_list = $this->db->getAll($sql);
            }
            ...
            if (!empty($goods_list)) {
                foreach ($goods_list as $k => $v) {
                    ...
                    $goods_list[$k]['url'] = $this->sanmuRepository->buildUri('goods', array('gid' => $v['goods_id']), $v['goods_name']);
                    $goods_list[$k]['shop_price'] = price_format($v['shop_price']);
        ...
        elseif ($_REQUEST['act'] == 'edit_product_number') {
            ...
            /* 修改货品库存 */
            $sql = "UPDATE " . $this->sanmu->table('wholesale_products') . " SET product_number = '$product_number' WHERE product_id = '$product_id'";
        ...
        elseif ($_REQUEST['act'] == 'edit_product_price') {
            ...
            /* 修改 */
            $sql = "UPDATE " . $this->sanmu->table('wholesale_products') . " SET product_price = '$product_price' WHERE product_id = '$product_id'";
            $result = $this->db->query($sql);
            if ($result) {
                clear_cache_files();
        ...
        elseif ($_REQUEST['act'] == 'edit_attr_sort') {
            ...
            /* 修改 */
            $sql = "UPDATE " . $this->sanmu->table('wholesale_goods_attr') . " SET attr_sort = '$attr_sort' WHERE goods_attr_id = '$goods_attr_id'";
            $result = $this->db->query($sql);
            if ($result) {
                clear_cache_files();
        ...
        elseif ($_REQUEST['act'] == 'edit_attr_price') {
            ...
            /* 修改 */
            $sql = "UPDATE " . $this->sanmu->table('wholesale_goods_attr') . " SET attr_price = '$attr_price' WHERE goods_attr_id = '$goods_attr_id'";
            $result = $this->db->query($sql);
            if ($result) {
                clear_cache_files();
        ...
        elseif ($_REQUEST['act'] == 'edit_product_sn') {
            ...
            /* 修改 */
            $sql = "UPDATE " . $this->sanmu->table('wholesale_products') . " SET product_sn = '$product_sn' WHERE product_id = '$product_id'";
            $result = $this->db->query($sql);
            if ($result) {
                clear_cache_files();
            ...
            foreach ($attr_value_list as $key => $attr_value) {
                if ($attr_value) {
                    if ($goods_attr_id[$key]) {
                        $sql = "UPDATE " . $this->sanmu->table('wholesale_goods_attr') . " SET attr_value = '$attr_value' WHERE goods_attr_id = '" . $goods_attr_id[$key] . "' LIMIT 1";
                    } else {
                        $sql = "SELECT MAX(attr_sort) AS attr_sort FROM " . $GLOBALS['sanmu']->table('wholesale_goods_attr') . " WHERE attr_id = '$attr_id'" . $where;
                        ...
                        $sql = "INSERT INTO " . $this->sanmu->table('wholesale_goods_attr') . " (attr_id, goods_id, attr_value, attr_sort, admin_id)" .
        ...
        elseif ($_REQUEST['act'] == 'del_goods_attr') {
            ...
            $sql = "DELETE FROM " . $GLOBALS['sanmu']->table("wholesale_goods_attr") . " WHERE $where";
            $GLOBALS['db']->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'insert_attr_img') {
            ...
            if (!empty($_FILES['attr_img_flie'])) {
                ...
                $this->sanmuRepository->getOssAddFile(array($other['attr_img_flie']));
            } else {
                $other['attr_img_flie'] = '';
            }
            ...
            if (empty($other['attr_img_flie'])) {
                ...
                sanmu_unlink(storage_public($goods_attr_info['attr_img_flie']));
            }
            ...
            if ($other['attr_checked'] == 1) {
                $this->db->autoExecute($this->sanmu->table('wholesale_goods_attr'), array('attr_checked' => 0), 'UPDATE', 'attr_id = ' . $attr_id . ' and goods_id = ' . $goods_id);
                $result['is_checked'] = 1;
            }

            $this->db->autoExecute($this->sanmu->table('wholesale_goods_attr'), $other, 'UPDATE', 'goods_attr_id = ' . $goods_attr_id . ' and attr_id = ' . $attr_id . ' and goods_id = ' . $goods_id);
        ...
        elseif ($_REQUEST['act'] == 'drop_attr_img') {
            ...
            $sql = "select attr_img_flie from " . $this->sanmu->table('wholesale_goods_attr') . " where goods_attr_id = '$goods_attr_id'";
            $attr_img_flie = $this->db->getOne($sql);

            $this->sanmuRepository->getOssDelFile(array($attr_img_flie));

            sanmu_unlink(storage_public($attr_img_flie));
            $other['attr_img_flie'] = '';
            $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table('wholesale_goods_attr'), $other, "UPDATE", "goods_attr_id = '$goods_attr_id'");
        ...
        elseif ($_REQUEST['act'] == 'choose_attrImg') {
            ...
            $sql = "SELECT attr_gallery_flie FROM " . $GLOBALS['sanmu']->table('wholesale_goods_attr') . " WHERE goods_attr_id = '$goods_attr_id' AND goods_id = '$goods_id'";
            $attr_gallery_flie = $GLOBALS['db']->getOne($sql);

            /* 删除数据 */
            $sql = "SELECT img_id, thumb_url, img_url FROM " . $GLOBALS['sanmu']->table('goods_gallery') . " WHERE goods_id = '$goods_id'";
        ...
        elseif ($_REQUEST['act'] == 'insert_gallery_attr') {
            ...
            if (!empty($gallery_id)) {
                ...
                $sql = "UPDATE " . $this->sanmu->table('wholesale_goods_attr') . " SET attr_gallery_flie = '" . $img['img_url'] . "' WHERE goods_attr_id = '$goods_attr_id' AND goods_id = '$goods_id'";
                $this->db->query($sql);
            } else {
                $result['error'] = 1;
            }
