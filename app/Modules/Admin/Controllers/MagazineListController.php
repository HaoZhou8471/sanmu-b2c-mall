            ...
            if (!empty($template_id)) {
                if (SEND_LIST == $rank_array['0']) {
                    ...
                    if (SEND_USER == $rank_array['0']) {
                        $count_sql = 'SELECT COUNT(*) FROM ' . $this->sanmu->table('users') . 'WHERE is_validated = 1';
                        $email_sql = 'SELECT email FROM ' . $this->sanmu->table('users') . "WHERE is_validated = 1 LIMIT $start,100";
                    } else/*if ($row['special_rank'])*/ {
                        $count_sql = 'SELECT COUNT(*) FROM ' . $this->sanmu->table('users') . 'WHERE is_validated = 1 AND user_rank = ' . $rank_array['1'];
                        $email_sql = 'SELECT email FROM ' . $this->sanmu->table('users') . 'WHERE is_validated = 1 AND user_rank = ' . $rank_array['1'] . " LIMIT $start,100";
//                    } else {
//                        $count_sql = 'SELECT COUNT(*) ' .
//                            'FROM ' . $this->sanmu->table('users') . ' AS u LEFT JOIN ' . $this->sanmu->table('user_rank') . ' AS ur ' .
