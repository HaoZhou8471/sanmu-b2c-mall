...
class TouchAdsController extends InitController
{
    ...
    public function index()
    {
        ...
        $exc = new Exchange($this->sanmu->table("touch_ad"), $this->db, 'ad_id', 'ad_name');
        ...
        elseif ($act == 'insert') {
            ...
            /* 查看广告名称是否有重复 */
            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('touch_ad') . " AS a, " .
                $this->sanmu->table('touch_ad_position') . " AS p " .
            ...
            /* 插入数据 */
            $sql = "INSERT INTO " . $this->sanmu->table('touch_ad') . " (position_id,media_type,ad_name,is_new,is_hot,is_best,public_ruid,ad_link,ad_code,start_time,end_time,link_man,link_email,link_phone,click_count,enabled, link_color, ad_type, goods_name)
    VALUES ('$_POST[position_id]',
        ...
        elseif ($act == 'edit') {
            ...
            /* 获取广告数据 */
            $sql = "SELECT * FROM " . $this->sanmu->table('touch_ad') . " WHERE ad_id='" . intval($_REQUEST['id']) . "'";
        ...
        elseif ($act == 'update') {
            ...
            $this->sanmuRepository->getOssAddFile([DATA_DIR . '/afficheimg/' . $ad_images]);

            $ad_code = str_replace(['../' . DATA_DIR . '/afficheimg/', DATA_DIR . '/afficheimg/', storage_public()], '', $ad_code);

            /* 更新信息 */
            $sql = "UPDATE " . $this->sanmu->table('touch_ad') . " SET " .
        ...
        elseif ($act == 'add_js') {
            ...
            $js_code .= ' src=' . '"' . $this->sanmu->url() . 'affiche.php?act=js&type=' . $_REQUEST['type'] . '&ad_id=' . intval($_REQUEST['id']) . '"' . '></script>';

            $site_url = $this->sanmu->url() . 'affiche.php?act=js&type=' . $_REQUEST['type'] . '&ad_id=' . intval($_REQUEST['id']);
        ...
        elseif ($act == 'remove') {
            ...
            if ((strpos($img, 'http://') === false) && (strpos($img, 'https://') === false)) {
                ...
                sanmu_unlink(storage_public(DATA_DIR . '/afficheimg/' . $img_name));
            }
