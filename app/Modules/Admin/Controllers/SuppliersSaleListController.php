...
class SuppliersSaleListController extends InitController
{
    ...
    public function index()
    {
        ...
        $this->sanmuRepository->helpersLang(['statistic'], 'admin');
        ...
        if (isset($_REQUEST['act']) && ($_REQUEST['act'] == 'query' || $_REQUEST['act'] == 'download')) {
            ...
            if ($_REQUEST['act'] == 'download') {
                ...
                /* 文件标题 */
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $_REQUEST['start_date'] . $GLOBALS['_LANG']['to'] . $_REQUEST['end_date'] . $GLOBALS['_LANG']['sales_list']) . "\t\n";

                /* 商品名称,订单号,商品数量,销售价格,销售日期 */
                echo sanmu_iconv(EC_CHARSET, 'GB2312', lang('admin/suppliers_sale_list.supplier_name')) . "\t";
                echo sanmu_iconv(EC_CHARSET, 'GB2312', lang('admin/suppliers_sale_list.number')) . "\t";
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['goods_name']) . "\t";
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['order_sn']) . "\t";
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['amount']) . "\t";
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['sell_price']) . "\t";
                echo sanmu_iconv(EC_CHARSET, 'GB2312', lang('admin/suppliers_sale_list.sum_price')) . "\t";
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['sell_date']) . "\t\n";

                foreach ($goods_sales_list['sale_list_data'] as $key => $value) {
                    echo sanmu_iconv(EC_CHARSET, 'GB2312', $value['shop_name']) . "\t";
                    echo sanmu_iconv(EC_CHARSET, 'GB2312', $value['goods_sn']) . "\t";
                    echo sanmu_iconv(EC_CHARSET, 'GB2312', $value['goods_name']) . "\t";
                    echo sanmu_iconv(EC_CHARSET, 'GB2312', '[ ' . $value['order_sn'] . ' ]') . "\t";
                    echo sanmu_iconv(EC_CHARSET, 'GB2312', $value['goods_num']) . "\t";
                    echo sanmu_iconv(EC_CHARSET, 'GB2312', $value['sales_price']) . "\t";
                    echo sanmu_iconv(EC_CHARSET, 'GB2312', $value['total_fee']) . "\t";
                    echo sanmu_iconv(EC_CHARSET, 'GB2312', $value['sales_time']) . "\t";
                    echo "\n";
                }
                exit;
            }
    ...
    private function get_sale_list($is_pagination = true)
    {
        ...
        $where .= " and (SELECT COUNT(*) FROM " . $GLOBALS['sanmu']->table('wholesale_order_info') . " AS oi2 WHERE oi2.main_order_id = oi.order_id) = 0 AND oi.order_id = og.order_id ";  //主订单下有子订单时，则主订单不显示
        ...
        $sql = "SELECT COUNT(og.goods_id) FROM " .
            $GLOBALS['sanmu']->table('wholesale_order_info') . ' AS oi,' .
            $GLOBALS['sanmu']->table('wholesale_order_goods') . ' AS og ' . $leftJoin .
            $where;
        ...
        $sql = 'SELECT og.goods_id, og.goods_sn, og.goods_name, og.goods_number AS goods_num, oi.suppliers_id, og.goods_price ' .
            ...
            "FROM " . $GLOBALS['sanmu']->table('wholesale_order_goods') . " AS og, " . $GLOBALS['sanmu']->table('wholesale_order_info') . " AS oi " . $leftJoin .
