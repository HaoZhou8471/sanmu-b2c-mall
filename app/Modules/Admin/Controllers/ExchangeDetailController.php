    ...
    public function index()
    {
        ...
        if (isset($_REQUEST['act']) && ($_REQUEST['act'] == 'detail_query' || $_REQUEST['act'] == 'download')) {
            ...
            if ($_REQUEST['act'] == 'download') {
                ...
                /* 文件标题 */
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['store_integral_detail']) . "\t\n";

                /* 商品名称,订单号,商品数量,销售价格,销售日期 */
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['goods_steps_name']) . "\t";
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['gift_consumption_score']) . "\t";
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['gift_grade_integral']) . "\t\n";

                foreach ($exchange_detail['detail'] as $key => $value) {
                    echo sanmu_iconv(EC_CHARSET, 'GB2312', $value['shop_name']) . "\t";
                    echo sanmu_iconv(EC_CHARSET, 'GB2312', $value['give_integral']) . "\t";
                    echo sanmu_iconv(EC_CHARSET, 'GB2312', $value['rank_integral']) . "\t";
                    echo "\n";
                }
                exit;
            }
        ...
        if (isset($_REQUEST['act']) && ($_REQUEST['act'] == 'order_query' || $_REQUEST['act'] == 'order_download')) {
            if ($_REQUEST['act'] == 'order_download') {
                ...
                /* 文件标题 */
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['give_integral_order_list']) . "\t\n";

                /* 商品名称,订单号,商品数量,销售价格,销售日期 */
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['order_sn']) . "\t";
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['goods_steps_name']) . "\t";
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['goods_number']) . "\t";
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['give_integral']) . "\t";
                echo sanmu_iconv(EC_CHARSET, 'GB2312', $GLOBALS['_LANG']['rank_integral']) . "\t\n";
                ...
                foreach ($order_list['item'] as $key => $value) {
                    ...
                    echo sanmu_iconv(EC_CHARSET, 'GB2312', $value['goods_number']) . "\t";
                    echo sanmu_iconv(EC_CHARSET, 'GB2312', $value['give_integral']) . "\t";
                    echo sanmu_iconv(EC_CHARSET, 'GB2312', $value['rank_integral']) . "\t";
                    echo "\n";
                }
