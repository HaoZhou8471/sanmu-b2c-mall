...
class ViewSendlistController extends InitController
{
    ...
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            $id = (int)$_REQUEST['id'];
            $sql = "DELETE FROM " . $this->sanmu->table('email_sendlist') . " WHERE id = '$id' LIMIT 1";
            $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'batch_remove') {
            ...
            if (isset($_POST['checkboxes'])) {
                $sql = "DELETE FROM " . $this->sanmu->table('email_sendlist') . " WHERE id " . db_create_in($_POST['checkboxes']);
                $this->db->query($sql);
        ...
        elseif ($_REQUEST['act'] == 'batch_send') {
            /* 检查权限 */
            if (isset($_POST['checkboxes'])) {
                $sql = "SELECT * FROM " . $this->sanmu->table('email_sendlist') . "WHERE id " . db_create_in($_POST['checkboxes']) . " ORDER BY pri DESC, last_send ASC LIMIT 1";
                ...
                $sql = "SELECT * FROM " . $this->sanmu->table('email_sendlist') . "WHERE id " . db_create_in($_POST['checkboxes']) . " ORDER BY pri DESC, last_send ASC";
                $res = $this->db->query($sql);
                foreach ($res as $row) {
                    //发送列表不为空，邮件地址为空
                    if (!empty($row['id']) && empty($row['email'])) {
                        $sql = "DELETE FROM " . $this->sanmu->table('email_sendlist') . " WHERE id = '$row[id]'";
                        $this->db->query($sql);
                        continue;
                    }

                    //查询相关模板
                    $sql = "SELECT * FROM " . $this->sanmu->table('mail_templates') . " WHERE template_id = '$row[template_id]'";
                    ...
                    if ($rt['template_id'] && $rt['template_content']) {
                        if ($this->commonRepository->sendEmail('', $row['email'], $rt['template_subject'], $rt['template_content'], $rt['is_html'])) {
                            ...
                            if ($row['error'] < 3) {
                                $time = time();
                                $sql = "UPDATE " . $this->sanmu->table('email_sendlist') . " SET error = error + 1, pri = 0, last_send = '$time' WHERE id = '$row[id]'";
                            } else {
                                //将出错超次的纪录删除
                                $sql = "DELETE FROM " . $this->sanmu->table('email_sendlist') . " WHERE id = '$row[id]'";
                            }
        ...
        elseif ($_REQUEST['act'] == 'all_send') {
            $sql = "SELECT * FROM " . $this->sanmu->table('email_sendlist') . " ORDER BY pri DESC, last_send ASC LIMIT 1";
            $row = $this->db->getRow($sql);
            ...
            $sql = "SELECT * FROM " . $this->sanmu->table('email_sendlist') . " ORDER BY pri DESC, last_send ASC";
            $res = $this->db->query($sql);
            foreach ($res as $row) {
                //发送列表不为空，邮件地址为空
                if (!empty($row['id']) && empty($row['email'])) {
    ...
    private function get_sendlist()
    {
        ...
        if ($result === false) {
            ...
            $sql = "SELECT count(*) FROM " . $this->sanmu->table('email_sendlist') . " e LEFT JOIN " . $this->sanmu->table('mail_templates') . " m ON e.template_id = m.template_id";
            ...
            /* 查询 */
            $sql = "SELECT e.id, e.email, e.pri, e.error, FROM_UNIXTIME(e.last_send) AS last_send, m.template_subject, m.type FROM " . $this->sanmu->table('email_sendlist') . " e LEFT JOIN " . $this->sanmu->table('mail_templates') . " m ON e.template_id = m.template_id" .
        ...
        if ($listdb) {
            foreach ($listdb as $key => $val) {
                if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                    $listdb[$key]['email'] = $this->sanmuRepository->stringToStar($val['email']);
                }
            }
        }
