...
class VirtualCardController extends InitController
{
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'replenish') {
            ...
            if (empty($_REQUEST['goods_id'])) {
                ...
                $goods_name = $this->db->GetOne("SELECT goods_name From " . $this->sanmu->table('goods') . " WHERE goods_id='" . $_REQUEST['goods_id'] . "' AND is_real = 0 AND extension_code='virtual_card' ");
                if (empty($goods_name)) {
                    $link[] = ['text' => $GLOBALS['_LANG']['go_back'], 'href' => 'virtual_card.php?act=list'];
        ...
        elseif ($_REQUEST['act'] == 'edit_replenish') {
            ...
            $sql = "SELECT T1.card_id, T1.goods_id, T2.goods_name,T1.card_sn, T1.card_password, T1.end_date, T1.crc32 FROM " .
                $this->sanmu->table('virtual_card') . " AS T1, " . $this->sanmu->table('goods') . " AS T2 " .
            ...
            if ($card['crc32'] == 0 || $card['crc32'] == crc32(AUTH_KEY)) {
                $card['card_sn'] = sanmu_decrypt($card['card_sn']);
                $card['card_password'] = sanmu_decrypt($card['card_password']);
            } elseif ($card['crc32'] == crc32(OLD_AUTH_KEY)) {
                $card['card_sn'] = sanmu_decrypt($card['card_sn'], OLD_AUTH_KEY);
                $card['card_password'] = sanmu_decrypt($card['card_password'], OLD_AUTH_KEY);
            } else {
                $card['card_sn'] = '***';
                $card['card_password'] = '***';
            }
            ...
            /* 加密后的 */
            $coded_card_sn = sanmu_encrypt($_POST['card_sn']);
            $coded_card_password = sanmu_encrypt($_POST['card_password']);

            /* 在前后两次card_sn不一致时，检查是否有重复记录,一致时直接更新数据 */
            if ($_POST['card_sn'] != $_POST['old_card_sn']) {
                $sql = "SELECT count(*) FROM " . $this->sanmu->table('virtual_card') . " WHERE goods_id='" . intval($_POST['goods_id']) . "' AND card_sn='$coded_card_sn'";
            ...
            if (empty($_POST['old_card_sn'])) {
                ...
                $add_date = gmtime();
                $sql = "INSERT INTO " . $this->sanmu->table('virtual_card') . " (goods_id, card_sn, card_password, end_date, add_date, crc32) " .
                    "VALUES ('$_POST[goods_id]', '$coded_card_sn', '$coded_card_password', '$end_date', '$add_date', '" . crc32(AUTH_KEY) . "')";
                $this->db->query($sql);

                /* 如果添加成功且原卡号为空时商品库存加1 */
                if (empty($_POST['old_card_sn'])) {
                    $sql = "UPDATE " . $this->sanmu->table('goods') . " SET goods_number= goods_number+1 WHERE goods_id='$_POST[goods_id]'";
                    $this->db->query($sql);
                }
                ...
                /* 更新数据 */
                $end_date = strtotime($_POST['end_dateYear'] . "-" . $_POST['end_dateMonth'] . "-" . $_POST['end_dateDay']);
                $sql = "UPDATE " . $this->sanmu->table('virtual_card') . " SET card_sn='$coded_card_sn', card_password='$coded_card_password', end_date='$end_date' " .
        ...
        elseif ($_REQUEST['act'] == 'batch_drop_card') {
            ...
            $sql = "DELETE FROM " . $this->sanmu->table('virtual_card') . " WHERE card_id " . db_create_in(implode(',', $_POST['checkboxes']));
        ...
        elseif ($_REQUEST['act'] == 'batch_insert') {
            ...
            foreach ($_POST['checked'] as $key) {
                $rec['card_sn'] = sanmu_encrypt($_POST['card_sn'][$key]);
                $rec['card_password'] = sanmu_encrypt($_POST['card_password'][$key]);
                ...
                $this->db->AutoExecute($this->sanmu->table('virtual_card'), $rec, 'INSERT');
                $i++;
            }
        ...
        elseif ($_REQUEST['act'] == 'submit_change') {
            ...
            if (isset($_POST['old_string']) && isset($_POST['new_string'])) {
                ...
                $sql = "SELECT card_id, card_sn, card_password FROM " . $this->sanmu->table('virtual_card') . " WHERE crc32 = '$old_crc32'";
                $res = $this->db->query($sql);
                foreach ($res as $row) {
                    $row['card_sn'] = sanmu_encrypt(sanmu_decrypt($row['card_sn'], $_POST['old_string']), $_POST['new_string']);
                    $row['card_password'] = sanmu_encrypt(sanmu_decrypt($row['card_password'], $_POST['old_string']), $_POST['new_string']);
                    $row['crc32'] = $new_crc32;
                    $this->db->autoExecute($this->sanmu->table('virtual_card'), $row, 'UPDATE', 'card_id = ' . $row['card_id']);
                }
        ...
        elseif ($_REQUEST['act'] == 'toggle_sold') {
            ...
            $sql = "UPDATE " . $this->sanmu->table('virtual_card') . " SET is_saled= '$val' WHERE card_id='$id'";

            if ($this->db->query($sql, 'SILENT')) {
                /* 修改商品库存 */
                $sql = "SELECT goods_id FROM " . $this->sanmu->table('virtual_card') . " WHERE card_id = '$id' LIMIT 1";
        ...
        elseif ($_REQUEST['act'] == 'remove_card') {
            ...
            $row = $this->db->GetRow('SELECT card_sn, goods_id FROM ' . $this->sanmu->table('virtual_card') . " WHERE card_id = '$id'");

            $sql = 'DELETE FROM ' . $this->sanmu->table('virtual_card') . " WHERE card_id = '$id'";
            if ($this->db->query($sql, 'SILENT')) {
                /* 修改商品数量 */
        ...
        elseif ($_REQUEST['act'] == 'start_change') {
            ...
            $sql = "SELECT crc32, count(*) AS cnt FROM " . $this->sanmu->table('virtual_card') . " GROUP BY crc32";
            $res = $this->db->query($sql);
            foreach ($res as $row) {
                $stat['all'] += $row['cnt'];
        ...
        elseif ($_REQUEST['act'] == 'on_change') {
            ...
            foreach ($res as $row) {
                $row['card_sn'] = sanmu_encrypt(sanmu_decrypt($row['card_sn'], OLD_AUTH_KEY));
                $row['card_password'] = sanmu_encrypt(sanmu_decrypt($row['card_password'], OLD_AUTH_KEY));
                $row['crc32'] = $new_crc32;

                if (!$this->db->autoExecute($this->sanmu->table('virtual_card'), $row, 'UPDATE', 'card_id = ' . $row['card_id'])) {
            ...
            // 查询是否还有未更新的
            $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('virtual_card') . " WHERE crc32 = '$old_crc32' ";
    ...
    private function get_replenish_list()
    {
        ...
        if (!empty($filter['keyword'])) {
            if ($filter['search_type'] == 'card_sn') {
                $where .= " AND card_sn = '" . sanmu_encrypt($filter['keyword']) . "'";
            } else {
                $where .= " AND order_sn LIKE '%" . mysql_like_quote($filter['keyword']) . "%' ";
            }
        }

        $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('virtual_card') . " WHERE 1 $where";
        ...
        foreach ($all as $key => $row) {
            if ($row['crc32'] == 0 || $row['crc32'] == crc32(AUTH_KEY)) {
                $row['card_sn'] = sanmu_decrypt($row['card_sn']);
                $row['card_password'] = sanmu_decrypt($row['card_password']);
            } elseif ($row['crc32'] == crc32(OLD_AUTH_KEY)) {
                $row['card_sn'] = sanmu_decrypt($row['card_sn'], OLD_AUTH_KEY);
                $row['card_password'] = sanmu_decrypt($row['card_password'], OLD_AUTH_KEY);
            } else {
                $row['card_sn'] = '***';
                $row['card_password'] = '***';
            }
    ...
    {
        $sql = "SELECT COUNT(*) FROM " . $this->sanmu->table('virtual_card') . " WHERE goods_id = '$goods_id' AND is_saled = 0";
        $goods_number = $this->db->getOne($sql);

        $sql = "UPDATE " . $this->sanmu->table('goods') . " SET goods_number = '$goods_number' WHERE goods_id = '$goods_id' AND extension_code='virtual_card'";
