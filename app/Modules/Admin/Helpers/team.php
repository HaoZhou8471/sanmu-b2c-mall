...
function team_category_goods($tc_id = 0, $keywords = '', $size = 10, $page = 1, $intro = '', $sort, $order, $brand, $min, $max)
{
    ...
    $leftJoin = '';
    if ($brand > 0) {
        $leftJoin .= "LEFT JOIN " . $GLOBALS['sanmu']->table('brand') . " AS b " . "ON b.brand_id = g.brand_id ";
    ...
    $res = $GLOBALS['sanmu']->selectLimit($sql, $size, ($page - 1) * $size);
...
function get_good_comment($id, $rank = null, $hasgoods = 0, $start = 0, $size = 10)
{
    ...
    $sql = "SELECT * FROM " . $GLOBALS['sanmu']->table('comment') . " WHERE id_value = '" . $id . "' and comment_type = 0 and status = 1 and parent_id = 0 " . $where . " ORDER BY comment_id DESC LIMIT $start, $size";
    ...
    if ($comment) {
        ...
        foreach ($comment as $key => $row) {
            ...
            if ($row['order_id'] && $hasgoods) {
                $sql = "SELECT o.goods_id, o.goods_name, o.goods_attr, g.goods_img FROM " . $GLOBALS['sanmu']->table('order_goods') . " o LEFT JOIN " . $GLOBALS['sanmu']->table('goods') . " g ON o.goods_id = g.goods_id WHERE o.order_id = '" . $row['order_id'] . "' ORDER BY rec_id DESC";
        ...
        /* 取得已有回复的评论 */
        if ($ids) {
            $sql = 'SELECT * FROM ' . $GLOBALS['sanmu']->table('comment') . " WHERE parent_id IN( $ids )";
            $res = $GLOBALS['sanmu']->query($sql);
            foreach ($res as $row) {
...
function get_good_comment_as($id, $rank = null, $hasgoods = 0, $start = 0, $size = 10)
{
    ...
    $sql = "SELECT * FROM " . $GLOBALS['sanmu']->table('comment') . " WHERE id_value = '" . $id . "' and comment_type = 0 and status = 1 and parent_id = 0 " . $where;
...
function get_goods_attr_ajax($goods_id, $goods_attr, $goods_attr_id)
{
    ...
    if ($goods_attr) {
        ...
        $sql = "SELECT ga.goods_attr_id, ga.attr_id, ga.attr_value  FROM " . $GLOBALS['sanmu']->table('goods_attr') . " AS ga" .
            " LEFT JOIN " . $GLOBALS['sanmu']->table('attribute') . " AS a ON ga.attr_id = a.attr_id " .
...
{
    $sql = 'SELECT g.*, tg.team_price, tg.team_num,tg.astrict_num FROM ' . $GLOBALS['sanmu']->table('team_goods') . 'AS tg LEFT JOIN ' . $GLOBALS['sanmu']->table('goods') . ' AS g ON tg.goods_id = g.goods_id ' . "WHERE tg.goods_id = $goods and tg.id = $t_id  and is_team = 1";
...
{
    $sql = 'SELECT tg.id,tg.team_price,tg.team_num,tg.astrict_num,tg.is_team FROM ' . $GLOBALS['sanmu']->table('team_log') . 'AS tl LEFT JOIN ' . $GLOBALS['sanmu']->table('team_goods') . ' AS tg ON tl.t_id = tg.id ' . "WHERE  tl.team_id = $team_id";
...
{
    $sql = "select tl.team_id, tl.start_time,o.team_parent_id,g.goods_id,tg.validity_time ,tg.team_num from " . $GLOBALS['sanmu']->table('team_log') . " as tl LEFT JOIN " . $GLOBALS['sanmu']->table('order_info') . " as o ON tl.team_id = o.team_id LEFT JOIN  " . $GLOBALS['sanmu']->table('goods') . " as g ON tl.goods_id = g.goods_id LEFT JOIN " . $GLOBALS['sanmu']->table('team_goods') . " AS tg ON tl.t_id = tg.id  " . " where tl.goods_id = $goods_id and tl.status <1 and tl.is_show = 1 and o.extension_code ='team_buy' and o.team_parent_id > 0 and pay_status = 2 and tg.is_team = 1";
...
{
    if ($failure == 2) {
        $sql = "SELECT count(order_id) as num  FROM " . $GLOBALS['sanmu']->table('order_info') . " WHERE team_id = '" . $team_id . "' AND extension_code = 'team_buy' ";
    } else {
        $sql = "SELECT count(order_id) as num  FROM " . $GLOBALS['sanmu']->table('order_info') . " WHERE team_id = '" . $team_id . "' AND extension_code = 'team_buy' and (pay_status = '" . PS_PAYED . "' or order_status = 4) ";
    }
...
function my_team_goods($user_id, $type = 1, $page = 1, $size = 10)
{
    ...
    $sql = "select o.order_id,o.order_status,o.pay_status,t.goods_id,t.team_id,t.start_time,t.status,g.goods_name,g.goods_thumb,g.goods_img,tg.validity_time,tg.team_num,tg.team_price,tg.limit_num from " . $GLOBALS['sanmu']->table('order_info') . " as o left join " . $GLOBALS['sanmu']->table('team_log') . " as t on o.team_id = t.team_id left join " . $GLOBALS['sanmu']->table('team_goods') . " as tg on t.t_id = tg.id left join " . $GLOBALS['sanmu']->table('goods') . " as g on g.goods_id = tg.goods_id" . " where o.user_id =$user_id and o.extension_code ='team_buy'  and t.is_show = 1 $where  ORDER BY o.add_time DESC ";
    $goods_list = $GLOBALS['db']->getAll($sql);
    $total = is_array($goods_list) ? count($goods_list) : 0;
    $res = $GLOBALS['sanmu']->selectLimit($sql, $size, ($page - 1) * $size);
...
function get_cart_value($flow_type = 0)
{
    ...
    $sql = "SELECT c.rec_id FROM " . $GLOBALS['sanmu']->table('cart') .
        " AS c LEFT JOIN " . $GLOBALS['sanmu']->table('goods') .
...
function flow_available_points($cart_value)
{
    ...
    $sql = "SELECT SUM(g.integral * c.goods_number) " .
        "FROM " . $GLOBALS['sanmu']->table('cart') . " AS c, " . $GLOBALS['sanmu']->table('goods') . " AS g " .
        "WHERE " . $c_sess . " AND c.goods_id = g.goods_id AND c.is_gift = 0 AND g.integral > 0 $where" .
        "AND c.rec_type = '" . CART_GENERAL_GOODS . "'";

    $val = intval($GLOBALS['db']->getOne($sql));

    $val = app(SanmuRepository::class)->integralOfValue($val);
...
/*function get_select_category($cat_id = 0, $relation = 0, $self = true)
{
    ...
    if ($relation == 0) {
        ...
        $sql = " select parent_id from " . $GLOBALS['sanmu']->table('category') . " where cat_id='" . $cat_id . "' ";
        ...
        $sql = " select cat_id from " . $GLOBALS['sanmu']->table('category') . " where parent_id='" . $cat_id . "' ";
...
{
    $sql = "select * from " . $GLOBALS['sanmu']->table('goods_attr') . " where goods_id='$goods_id' and goods_attr_id='$attr_id'";
...
function team_get_tree($tree_id = 0)
{
    $three_arr = [];
    $where = "";
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['sanmu']->table('team_category') . " WHERE parent_id = '$tree_id' AND status = 1" . $where;
    if ($GLOBALS['db']->getOne($sql) || $tree_id == 0) {
        $child_sql = 'SELECT id, name, parent_id,status ' . ' FROM ' . $GLOBALS['sanmu']->table('team_category') .
...
function flow_cart_stock($arr, $store_id = 0)
{
    ...
    foreach ($arr as $key => $val) {
        ...
        $sql = "SELECT `goods_id`, `goods_attr_id`, `extension_code`, `warehouse_id` FROM" . $GLOBALS['sanmu']->table('cart') .
        ...
        $sql = "select IF(g.model_inventory < 1, g.goods_number, IF(g.model_inventory < 2, wg.region_number, wag.region_number)) AS goods_number " .
            " from " . $GLOBALS['sanmu']->table('goods') . " as g " .
            " left join " . $GLOBALS['sanmu']->table('warehouse_goods') . " as wg on g.goods_id = wg.goods_id" .
            " left join " . $GLOBALS['sanmu']->table('warehouse_area_goods') . " as wag on g.goods_id = wag.goods_id" .
        ...
        if (intval($GLOBALS['_CFG']['use_storage']) > 0 && $goods['extension_code'] != 'package_buy' && $store_id == 0) {
            ...
            if (!empty($row['product_id'])) {
                ...
                $sql = "SELECT product_number FROM " . $GLOBALS['sanmu']->table($table_products) . " WHERE goods_id = '" . $row['goods_id'] . "' and product_id = '" . $row['product_id'] . "'";
            ...
            $sql = "SELECT goods_number,ru_id FROM" . $GLOBALS['sanmu']->table("store_goods") . " WHERE store_id = '$store_id' AND goods_id = '" . $row['goods_id'] . "' ";
...
function team_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0)
{
    ...
    if ($re_type == true) {
        ...
        foreach ($options as $key => $value) {
            $options[$key]['url'] = app(SanmuRepository::class)->buildUri('article_cat', ['acid' => $value['cat_id']], $value['cat_name']);
        }
        return $options;
    }
}
...
function team_log_account_change($user_id, $shop_money = 0, $frozen_money = 0, $rank_points = 0, $pay_points = 0, $change_desc = '', $change_type)
{
    if ($change_type == ACT_TRANSFERRED) {
        ...
        $GLOBALS['sanmu']->autoExecute($GLOBALS['sanmu']->table('account_log'), $account_log, 'INSERT');
    }
    if ($change_type == ACT_TRANSFERRED) {
        /* 更新用户信息 */
        $sql = "UPDATE " . $GLOBALS['sanmu']->table('users') .
            " SET user_money = user_money - ('$shop_money')," .
            " frozen_money = frozen_money - ('$frozen_money')," .
            " rank_points = rank_points - ('$rank_points')," .
            " pay_points = pay_points - ('$pay_points')" .
            " WHERE user_id = '$user_id' LIMIT 1";
        $GLOBALS['sanmu']->query($sql);
    }
}
...
function team_order_action_change($order_id, $action_user = 'admin', $order_status = 0, $shipping_status = 0, $pay_status = 0, $action_note = '')
{
    ...
    $GLOBALS['sanmu']->autoExecute($GLOBALS['sanmu']->table('order_action'), $action_log, 'INSERT');
...
function teamCategories($tc_id = 0)
{
    ...
    if ($team_list) {
        foreach ($team_list as $key => $val) {
            ...
            $list[$key]['tc_img'] = app(SanmuRepository::class)->getImagePath($val['tc_img']);
        }
    }
