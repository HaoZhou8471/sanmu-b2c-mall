...
{
    $sql = "SELECT * FROM " . $GLOBALS['sanmu']->table('user_rank') .
        " ORDER BY min_points";
...
function get_member_price_list($goods_id)
{
    ...
    $sql = "SELECT user_rank, user_price FROM " .
        $GLOBALS['sanmu']->table('member_price') .
...
function handle_goods_attr($goods_id, $id_list, $is_spec_list, $value_price_list)
{
    ...
    foreach ($id_list as $key => $id) {
        ...
        // 插入或更新记录
        $sql = "SELECT goods_attr_id FROM " . $GLOBALS['sanmu']->table('goods_attr') . " WHERE goods_id = '$goods_id' AND attr_id = '$id' AND attr_value = '$value' LIMIT 0, 1";
        $result_id = $GLOBALS['db']->getOne($sql);
        if (!empty($result_id)) {
            $sql = "UPDATE " . $GLOBALS['sanmu']->table('goods_attr') . "
            ...
            $sql = "INSERT INTO " . $GLOBALS['sanmu']->table('goods_attr') . " (goods_id, attr_id, attr_value, attr_price) " .
...
function handle_member_price($goods_id, $rank_list, $price_list)
{
    ...
    foreach ($rank_list as $key => $rank) {
        ...
        // 插入或更新记录
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['sanmu']->table('member_price') .
            " WHERE goods_id = '$goods_id' AND user_rank = '$rank'";
        if ($GLOBALS['db']->getOne($sql) > 0) {
            /* 如果会员价格是小于0则删除原来价格，不是则更新为新的价格 */
            if ($price < 0) {
                $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('member_price') .
                    " WHERE goods_id = '$goods_id' AND user_rank = '$rank' LIMIT 1";
            } else {
                $sql = "UPDATE " . $GLOBALS['sanmu']->table('member_price') .
            ...
            if ($price == -1) {
                ...
                $sql = "INSERT INTO " . $GLOBALS['sanmu']->table('member_price') . " (goods_id, user_rank, user_price) " .
...
{
    /* 查询现有的扩展分类 */
    $sql = "SELECT cat_id FROM " . $GLOBALS['sanmu']->table('goods_cat') .
        " WHERE goods_id = '$goods_id'";
    ...
    if ($delete_list) {
        $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('goods_cat') .
            " WHERE goods_id = '$goods_id' " .
    ...
    foreach ($add_list as $cat_id) {
        // 插入记录
        $sql = "INSERT INTO " . $GLOBALS['sanmu']->table('goods_cat') .
            " (goods_id, cat_id) " .
...
{
    $sql = "UPDATE " . $GLOBALS['sanmu']->table('link_goods') . " SET " .
        " goods_id = '$goods_id' " .
        " WHERE goods_id = '0'" .
...
{
    $sql = "UPDATE " . $GLOBALS['sanmu']->table('group_goods') . " SET " .
        " parent_id = '$goods_id' " .
        " WHERE parent_id = '0'" .
        " AND admin_id = '" . session('admin_id') . "'";
    $GLOBALS['db']->query($sql);
...
{
    $sql = "UPDATE " . $GLOBALS['sanmu']->table('goods_article') . " SET " .
        " goods_id = '$goods_id' " .
        " WHERE goods_id = '0'" .
        " AND admin_id = '" . session('admin_id') . "'";
    $GLOBALS['db']->query($sql);
...
{
    $sql = "UPDATE " . $GLOBALS['sanmu']->table('link_area_goods') . " SET " .
        " goods_id = '$goods_id' " .
        " WHERE goods_id = '0'" .
        " AND ru_id = (SELECT ru_id FROM " . $GLOBALS['sanmu']->table('admin_user') . "  WHERE user_id = '" . session('admin_id') . "') ";
    $GLOBALS['db']->query($sql);
}
...
function handle_gallery_image($goods_id, $image_files, $image_descs, $image_urls, $single_id = 0, $files_type = 0)
{
    ...
    foreach ($image_descs as $key => $img_desc) {
        ...
        if ($flag) {
            ...
            $sql = "INSERT INTO " . $GLOBALS['sanmu']->table('goods_gallery') . " (goods_id, img_url, img_desc, thumb_url, img_original, " . $files_type . ") " .
            ...
            if ($proc_thumb && !$GLOBALS['_CFG']['retain_original_img'] && !empty($img_original)) {
                $GLOBALS['db']->query("UPDATE " . $GLOBALS['sanmu']->table('goods_gallery') . " SET img_original='' WHERE `goods_id`='$goods_id'");
                @unlink('../' . $img_original);
            }
        ...
        if ($img_url) {
            app(SanmuRepository::class)->getOssAddFile([$img_url, $thumb_url, $img_original]);
        }
    }
}
...
{ //ecmoban模板堂 --zhuo  $content = ''
    if ($goods_id) {
        ...
        if ($field == 'is_on_sale') {
            if ($value == 1) {
                $sql = "SELECT act_id FROM " . $GLOBALS['sanmu']->table('presale_activity') . " WHERE goods_id " . db_create_in($goods_id);
                if ($GLOBALS['db']->getOne($sql, true)) {
                    $GLOBALS['db']->query("DELETE FROM " . $GLOBALS['sanmu']->table('presale_activity') . " WHERE goods_id " . db_create_in($goods_id));
                    $GLOBALS['db']->query("DELETE FROM " . $GLOBALS['sanmu']->table('cart') . " WHERE goods_id " . db_create_in($goods_id));
                }
            } else {
                $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('cart') . " WHERE goods_id " . db_create_in($goods_id);
                $GLOBALS['db']->query($sql);
            }
        }
...
function check_goods_sn_exist($goods_sn, $goods_id = 0)
{
    ...
    if (empty($goods_id)) {
        $sql = "SELECT goods_id FROM " . $GLOBALS['sanmu']->table('goods') . "
...
function get_attr_list($cat_id, $goods_id = 0)
{
    ...
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_input_type, a.attr_type, a.attr_values, v.attr_value, v.attr_price, v.attr_sort, v.attr_checked " .
        ...
        "LEFT JOIN " . $GLOBALS['sanmu']->table('goods_attr') . " AS v " .
...
function get_goods_type_specifications()
{
    ...
    $sql = "SELECT DISTINCT cat_id
            FROM " . $GLOBALS['sanmu']->table('attribute') . "
            WHERE attr_type = 1";
    $row = $GLOBALS['db']->GetAll($sql);
...
function get_linked_goods($goods_id)
{
    $sql = "SELECT lg.link_goods_id AS goods_id, g.goods_name, lg.is_double " .
        "FROM " . $GLOBALS['sanmu']->table('link_goods') . " AS lg, " .
        $GLOBALS['sanmu']->table('goods') . " AS g " .
...
function get_group_goods($goods_id)
{
    $sql = "SELECT gg.id, gg.goods_id, gg.group_id, g.goods_name ,gg.goods_price,g.shop_price " .
        "FROM " . $GLOBALS['sanmu']->table('group_goods') . " AS gg, " .
        $GLOBALS['sanmu']->table('goods') . " AS g " .
...
function get_goods_articles($goods_id)
{
    $sql = "SELECT g.article_id, a.title " .
        "FROM " . $GLOBALS['sanmu']->table('goods_article') . " AS g, " .
        $GLOBALS['sanmu']->table('article') . " AS a " .
...
function goods_list($is_delete = 0, $real_goods = 1, $conditions = '', $review_status = 0, $real_division = 0)
{
    ...
    if ($result === false) {
        ...
        if ($filter['brand_keyword']) {
            $filter['brand_id'] = $GLOBALS['db']->getAll("SELECT GROUP_CONCAT(brand_id) AS brand_id FROM " . $GLOBALS['sanmu']->table('brand') . " WHERE brand_name LIKE '%" . $filter['brand_keyword'] . "%' ");
        ...
        if ($filter['store_search'] != 0) {
            if ($adminru['ru_id'] == 0) {
                ...
                if ($filter['store_search'] > 1 && $filter['store_search'] != 4) {
                    $where .= " AND (SELECT msi.user_id FROM " . $GLOBALS['sanmu']->table('merchants_shop_information') . ' as msi ' .
        ...
        if ($filter['cat_type'] != 'seller' && $filter['cat_id']) {
            ...
            $leftjoin = " LEFT JOIN " . $GLOBALS['sanmu']->table('goods_cat') . " AS gc ON g.goods_id = gc.goods_id";
        }
        ...
        if ($filter['cat_type'] != 'seller' && $filter['cat_id']) {
            ...
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['sanmu']->table('goods') . " AS g " .
                $leftjoin .
            ...
            $sql = "SELECT SUM(CASE WHEN " . $goods_where . " THEN 1 ELSE 0 END) FROM " . $GLOBALS['sanmu']->table('goods') . " AS g " .
                $leftjoin .
                " WHERE 1";
        ...
        $sql = "SELECT g.goods_id,g.goods_thumb, g.goods_name, g.user_id, g.brand_id, g.goods_type, g.goods_sn, g.shop_price, " .
            ...
            " FROM " . $GLOBALS['sanmu']->table('goods') . " AS g " .
            $leftjoin .
    ...
    for ($i = 0; $i < $count; $i++) {
        ...
        if ($row[$i]['goods_type'] == 0) {
            $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('products') . " WHERE goods_id = '" . $row[$i]['goods_id'] . "'";
            $GLOBALS['db']->query($sql);

            $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('products_area') . " WHERE goods_id = '" . $row[$i]['goods_id'] . "'";
            $GLOBALS['db']->query($sql);

            $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('products_warehouse') . " WHERE goods_id = '" . $row[$i]['goods_id'] . "'";
            $GLOBALS['db']->query($sql);

            $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('goods_attr') . " WHERE goods_id = '" . $row[$i]['goods_id'] . "'";
            $GLOBALS['db']->query($sql);
        } else {
            $sql = "SELECT ga.goods_attr_id FROM " . $GLOBALS['sanmu']->table('goods_attr') . " AS ga," .
                $GLOBALS['sanmu']->table('attribute') . " AS a" .
        ...
        $row[$i]['url'] = app(SanmuRepository::class)->buildUri('goods', ['gid' => $row[$i]['goods_id']], $row[$i]['goods_name']);

        if (!empty($row[$i]['url']) && (strpos($row[$i]['url'], 'http://') === false && strpos($row[$i]['url'], 'https://') === false && strpos($row[$i]['url'], 'errorImg.png') === false)) {
            $row[$i]['url'] = $GLOBALS['sanmu']->url() . $row[$i]['url'];
        }
    }
...
{
    //判断商品类型 by wu
    $goods_model = $GLOBALS['db']->getOne(" SELECT model_price FROM " . $GLOBALS['sanmu']->table("goods") . " WHERE goods_id = '$goods_id' ");
    if ($goods_model == 1) {
    ...
    $sql = "SELECT product_number
            FROM " . $GLOBALS['sanmu']->table($table) . "
            WHERE goods_id = '$goods_id'
            " . $conditions;
...
function product_goods_attr_list($goods_id)
{
    ...
    $sql = "SELECT goods_attr_id, attr_value FROM " . $GLOBALS['sanmu']->table('goods_attr') . " WHERE goods_id = '$goods_id'";
...
function get_goods_specifications_list($goods_id)
{
    ...
    $sql = "SELECT g.goods_attr_id, g.attr_value, g.attr_id, a.attr_name
            FROM " . $GLOBALS['sanmu']->table('goods_attr') . " AS g
                LEFT JOIN " . $GLOBALS['sanmu']->table('attribute') . " AS a
...
function product_list($goods_id, $conditions = '')
{
    ...
    if ($result === false) {
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['sanmu']->table('products') . " AS p WHERE goods_id = $goods_id $where";
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);

        $sql = "SELECT product_id, goods_id, goods_attr, product_sn, bar_code, product_price, product_number
                FROM " . $GLOBALS['sanmu']->table('products') . " AS g
...
function get_product_info($product_id, $filed = '', $goods_model = 0, $is_attr = 0)
{
    ...
    $sql = "SELECT $filed FROM  " . $GLOBALS['sanmu']->table($table) . " WHERE product_id = '$product_id'";
...
function get_product_attr_list($goods_attr_id = 0, $goods_id = 0, $goods_model = 0, $warehouse_id = 0, $area_id = 0, $area_city = 0)
{
    ...
    if ($goods_model == 1) {
        ...
        $leftJion = ' LEFT JOIN ' . $GLOBALS['sanmu']->table('warehouse_attr') . ' AS wa ON wa.goods_attr_id = ga.goods_attr_id ' . $where;
        ...
        $leftJion = ' LEFT JOIN ' . $GLOBALS['sanmu']->table('warehouse_area_attr') . ' AS waa ON waa.goods_attr_id = ga.goods_attr_id ' . $where;
        $select = ", waa.attr_price AS attr_price, area_id, waa.id";
    } else {
        $select = ", ga.attr_price AS attr_price";
    }

    $sql = "SELECT  ga.goods_attr_id, ga.attr_id, ga.attr_value $select FROM  " . $GLOBALS['sanmu']->table('goods_attr') . " AS ga " .
        ' LEFT JOIN ' . $GLOBALS['sanmu']->table('attribute') . ' AS a ON a.attr_id = ga.attr_id ' .
        $leftJion .
...
function check_goods_specifications_exist($goods_id)
{
    ...
    $sql = "SELECT COUNT(a.attr_id)
            FROM " . $GLOBALS['sanmu']->table('attribute') . " AS a, " . $GLOBALS['sanmu']->table('goods') . " AS g
...
function check_goods_attr_exist($goods_attr, $goods_id, $product_id = 0, $region_id = 0)
{
    ...
    if (empty($product_id)) {
        $sql = "SELECT product_id FROM " . $GLOBALS['sanmu']->table($table) . "
                WHERE 1 " . $set . "
...
function check_product_sn_exist($product_sn, $product_id = 0, $ru_id = 0, $goods_model = 0, $warehouse_id = 0, $area_id = 0, $area_city = 0)
{
    ...
    $sql = "SELECT g.goods_id FROM " . $GLOBALS['sanmu']->table('goods') . " AS g WHERE g.goods_sn='$product_sn' AND g.user_id = '$ru_id'";
    if ($GLOBALS['db']->getOne($sql)) {
        return true;    //重复
    }

    $where .= " AND (SELECT g.user_id FROM " . $GLOBALS['sanmu']->table('goods') . " AS g WHERE g.goods_id = p.goods_id LIMIT 1) = '$ru_id'";

    if (empty($product_id)) {
        $sql = "SELECT p.product_id FROM " . $GLOBALS['sanmu']->table($table) . " AS p " . "
...
function check_product_bar_code_exist($product_bar_code, $product_id = 0, $ru_id = 0, $goods_model = 0, $warehouse_id = 0, $area_id = 0, $area_city = 0)
{
    ...
    if (!empty($product_id)) {
        $sql = "SELECT g.user_id FROM " . $GLOBALS['sanmu']->table($table) . " AS p, " .
            $GLOBALS['sanmu']->table('goods') . " AS g" .
    ...
    $sql = "SELECT g.goods_id FROM " . $GLOBALS['sanmu']->table('goods') . " AS g WHERE g.bar_code = '$product_bar_code' AND g.user_id = '$ru_id'";
    if ($GLOBALS['db']->getOne($sql)) {
        return true;    //重复
    }
...
{
    $sql = " SELECT recently_cat FROM " . $GLOBALS['sanmu']->table("admin_user") . " WHERE user_id = '$admin_id' LIMIT 5 ";
...
function get_img_file_list($is_ajax = 0, $file_dir)
{
    ...
    if ($file_list) {
        foreach ($file_list as $key => $row) {
            if (!is_dir($row) && app(SanmuRepository::class)->strLen($row) == 6 && !in_array($row, $file_dir)) {
                $arr[$key]['file_name'] = $dir . "/" . $row;
                $arr[$key]['goods_img'] = $GLOBALS['sanmu']->get_file_list($dir . "/" . $row . "/goods_img");
                $arr[$key]['source_img'] = $GLOBALS['sanmu']->get_file_list($dir . "/" . $row . "/source_img");
                $arr[$key]['thumb_img'] = $GLOBALS['sanmu']->get_file_list($dir . "/" . $row . "/thumb_img");
            }
        }

        $arr = $GLOBALS['sanmu']->page_array($page_size, $page, $arr);
    }
...
function get_img_file($dir, $file_parent_dir, $file_dir, $strlen = 6)
{
    ...
    if ($file_list) {
        foreach ($file_list as $key => $row) {
            if (!is_dir($row) && app(SanmuRepository::class)->strLen($row) == $strlen && !in_array($row, $file_parent_dir)) {
                $arr[$key]['file_name'] = $dir . "/" . $row;
...
function get_add_seckill_goods($sec_id, $tb_id)
{
    ...
    if ($result === false) {
        ...
        $sql = " SELECT COUNT(*) FROM " . $GLOBALS['sanmu']->table('seckill_goods') . " AS sg LEFT JOIN " .
            $GLOBALS['sanmu']->table('goods') . " AS g ON sg.goods_id = g.goods_id  " . $where .
        ...
        $sql = " SELECT sg.id, sg.sec_id, sg.tb_id, sg.sec_num, sg.sec_limit, sg.sec_price, g.goods_name, g.shop_price FROM " .
            $GLOBALS['sanmu']->table('seckill_goods') . " AS sg LEFT JOIN " . $GLOBALS['sanmu']->table('goods') . " AS g ON sg.goods_id = g.goods_id  " . $where .
    ...
    $sql = " SELECT GROUP_CONCAT(sg.goods_id) FROM " . $GLOBALS['sanmu']->table('seckill_goods') . " AS sg " . $where;
...
{
    $sql = " SELECT recently_cat FROM " . $GLOBALS['sanmu']->table('admin_user') . " WHERE user_id = '$admin_id' ";
    $recently_cat = $GLOBALS['db']->getOne($sql);
    ...
    $sql = "UPDATE " . $GLOBALS['sanmu']->table('admin_user') .
...
function lib_goods_list($real_goods = 1, $conditions = '', $review_status = 0, $real_division = 0)
{
    ...
    if ($result === false) {
        ...
        /* 记录总数 */
        $sql = "SELECT $select FROM " . $GLOBALS['sanmu']->table('goods_lib') . " AS g " .
            " WHERE $where " . $groupby;
        ...
        $sql = " SELECT g.goods_id, g.lib_cat_id, g.goods_thumb, g.goods_name, g.brand_id, g.goods_type, g.goods_sn, g.shop_price, g.sort_order, g.is_real, g.bar_code, g.is_on_sale,go.freight,go.tid " .
            " FROM " . $GLOBALS['sanmu']->table('goods_lib') . " AS g " .
            " LEFT JOIN " . $GLOBALS['sanmu']->table('goods') . " AS go ON g.goods_id=go.goods_id " .
...
{
    $sql = " SELECT cat_name FROM " . $GLOBALS['sanmu']->table('goods_lib_cat') . " WHERE cat_id = '$cat_id' ";
