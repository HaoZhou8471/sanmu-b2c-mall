...
{
    $sql = 'SELECT start_time,end_time,status FROM ' . $GLOBALS['sanmu']->table('bargain_goods') . " WHERE id = $bargain_id";
    $bargain = $GLOBALS['db']->getRow($sql);
    return $bargain;
}
...
{
    $sql = 'SELECT * FROM ' . $GLOBALS['sanmu']->table('bargain_statistics_log') . " WHERE bargain_id = $bargain_id and user_id =$user_id and status != 1 ";
    $bargain = $GLOBALS['db']->getRow($sql);
    return $bargain;
}
...
{
    $sql = 'SELECT user_id,subtract_price FROM ' . $GLOBALS['sanmu']->table('bargain_statistics') . " WHERE bs_id = $bs_id and user_id =$user_id ";
    $bargain = $GLOBALS['db']->getRow($sql);
    if ($bargain) {
        //用户名、头像
...
{
    $sql = 'SELECT user_id ,add_time,subtract_price FROM ' . $GLOBALS['sanmu']->table('bargain_statistics') . " WHERE bs_id = $bs_id order by add_time desc ";
...
{
    $sql = 'SELECT subtract_price FROM ' . $GLOBALS['sanmu']->table('bargain_statistics') . " WHERE bs_id = $bs_id order by add_time desc limit 0,10 ";
...
function get_bargain_goods_hot()
{
    ...
    $sql = 'SELECT bg.id,bg.bargain_name,bg.goods_id,bg.start_time,bg.end_time,bg.target_price,bg.total_num,g.user_id, g.goods_name, g.shop_price, g.market_price, g.goods_thumb , g.goods_img,g.goods_brief FROM ' . $GLOBALS['sanmu']->table('bargain_goods') . 'AS bg LEFT JOIN ' . $GLOBALS['sanmu']->table('goods') . ' AS g ON bg.goods_id = g.goods_id ' .
...
{
    $sql = 'SELECT min(target_price) as target_price FROM ' . $GLOBALS['sanmu']->table('activity_goods_attr') . " WHERE bargain_id = $bargain_id ";
...
function bargain_get_goods_attr_id($where_select = [], $select = [], $attr_type = 0, $retuen_db = 0)
{
    ...
    $sql = " SELECT $select FROM " . $GLOBALS['sanmu']->table('goods_attr') . " AS ga, " .
...
function get_product_info_by_attr_bargain($bargain_id = 0, $goods_id = 0, $attr_arr = [], $goods_model = 0, $region_id = 0)
{
    if (!empty($attr_arr)) {
        ...
        if ($bargain_id > 0) {
            $sql = " SELECT * FROM " . $GLOBALS['sanmu']->table('activity_goods_attr') . " WHERE bargain_id = '$bargain_id'  AND goods_id = '$goods_id' and product_id = '" . $product_info['product_id'] . "'  LIMIT 1 ";
...
{
    $sql = "select model_attr from " . $GLOBALS['sanmu']->table('goods') . " where goods_id='$goods_id'";
...
function bargain_target_price($bargain_id = 0, $goods_id = 0, $spec = [], $warehouse_id = 0, $area_id = 0)
{
    if (!empty($spec)) {
        ...
        if ($GLOBALS['_CFG']['goods_attr_price'] == 1) {
            ...
            $sql = 'SELECT product_id FROM ' . $GLOBALS['sanmu']->table($table) . " WHERE $where";
            $product_id = $GLOBALS['db']->getOne($sql);
            if ($product_id) {
                $sql = 'SELECT target_price FROM ' . $GLOBALS['sanmu']->table('activity_goods_attr') . " WHERE bargain_id = '$bargain_id' and goods_id = '$goods_id' and product_id = '$product_id' ";
