...
{
    $bonus = [];
    $sql = 'SELECT type_id, type_name, type_money FROM ' . $GLOBALS['sanmu']->table('pay_card_type') . " WHERE type_id = '$id' LIMIT 1 ";
    $res = $GLOBALS['db']->query($sql);
...
function goods_type_list($selected, $goods_id = 0, $type = 'html', $c_id = '')
{
    ...
    $ruCat = '';
    if ($goods_id > 0) {
        $sql = "select user_id from " . $GLOBALS['sanmu']->table('goods') . " where goods_id = '$goods_id'";
...
function get_add_attr_values($attr_id, $type = 0, $list = [], $attr_input_type = 0)
{
    if ($attr_input_type == 1) {
        ...
        $sql = "SELECT GROUP_CONCAT(ga.attr_value) AS attr_values FROM " . $GLOBALS['sanmu']->table('attribute') . " AS a, " .
            $GLOBALS['sanmu']->table('goods_attr') . " AS ga " .
...
function get_add_edit_link_desc($linked_array, $type = 0, $id = 0)
{
    ...
    if ($linked_array) {
        ...
        if ($id > 0) {
            $sql = "SELECT goods_id, ru_id FROM " . $GLOBALS['sanmu']->table('link_goods_desc') . " WHERE id = '$id'";
        ...
        $sql = "SELECT goods_id FROM " . $GLOBALS['sanmu']->table('link_desc_temporary') . " WHERE ru_id = '$ru_id'";
...
function return_order_list()
{
    ...
    if ($result === false) {
        ...
        $sql = "SELECT agency_id FROM " . $GLOBALS['sanmu']->table('admin_user') . " WHERE user_id = '" . session('admin_id') . "'";
        ...
        if ($GLOBALS['_CFG']['region_store_enabled']) {
            //卖场
            $filed = " (SELECT og.ru_id FROM " . $GLOBALS['sanmu']->table('order_goods') . ' as og ' . " WHERE og.order_id = o.order_id LIMIT 1) ";
        ...
        if ($filter['user_name']) {
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['sanmu']->table('order_return') . " AS o ," .
                $GLOBALS['sanmu']->table('users') . " AS u " . $where;
        } else {
            ...
            $sql = "SELECT COUNT(DISTINCT ret_id) FROM " . $GLOBALS['sanmu']->table('order_return') . " AS r, " . $GLOBALS['sanmu']->table('order_info') . " as o " . $where . " AND r.order_id = o.order_id";
        }
        ...
        $sql = "SELECT DISTINCT r.ret_id ,o.order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid, o.goods_amount, o.discount," .
            ...
            "LEFT JOIN " . $GLOBALS['sanmu']->table('users') . " AS u ON u.user_id=o.user_id  " . $where .
    ...
    foreach ($row as $key => $value) {
        ...
        //获取订单总数
        $sql = "SELECT COUNT(goods_number)  FROM" . $GLOBALS['sanmu']->table('order_goods') . "WHERE order_id = '" . $value['order_id'] . "'";
        ...
        if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
            $row[$key]['buyer'] = app(SanmuRepository::class)->stringToStar($row[$key]['buyer']);
            $row[$key]['phone'] = app(SanmuRepository::class)->stringToStar($value['phone']);
        }
    }
...
function get_order_detection_list($is_ajax = 0)
{
    ...
    if ($result === false) {
        ...
        if ($is_ajax == 1) {
            if ($filter['order_id_list']) {
                ...
                /* 记录log */
                $sql = "SELECT order_sn,order_status,pay_status,confirm_take_time FROM " . $GLOBALS['sanmu']->table('order_info') . " WHERE order_id " . db_create_in($filter['order_id_list']);
        ...
        $sql = "SELECT COUNT(DISTINCT o.order_id) FROM " . $GLOBALS['sanmu']->table('order_info') . " AS o " .
            $where . $where_store;
    ...
    foreach ($row as $key => $value) {
        ...
        if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
            $row[$key]['buyer'] = app(SanmuRepository::class)->stringToStar($value['buyer']);
            $row[$key]['mobile'] = app(SanmuRepository::class)->stringToStar($value['mobile']);
        }
    }
...
{
    $domain = $GLOBALS['sanmu']->http() . request()->server('SERVER_NAME');
    ...
    if ($attribute_list) {
        foreach ($attribute_list as $key => $val) {
            ...
            if ($val['attr_type'] > 0) {
                ...
                if ($attr_values) {
                    for ($i = 0; $i < count($attr_values); $i++) {
                        $sql = "SELECT goods_attr_id, attr_price, attr_sort FROM " . $GLOBALS['sanmu']->table('goods_attr') . " WHERE goods_id = '$goods_id' AND attr_value = '" . $attr_values[$i] . "' AND attr_id = '" . $val['attr_id'] . "' LIMIT 1";
...
function get_product_info_by_attr($goods_id = 0, $attr_arr = [], $goods_model = 0, $region_id = 0, $changelog = 0, $city_id = 0)
{
    if (!empty($attr_arr)) {
        ...
        $sql = " SELECT * FROM " . $GLOBALS['sanmu']->table($table) . " WHERE 1 $set AND goods_id = '$goods_id' " . $where . " LIMIT 1 ";
