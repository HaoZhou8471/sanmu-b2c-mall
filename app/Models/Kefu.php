...
class Kefu extends Model
{
    ...
    public static function getWait($ru_id = 0)
    {
        ...
        foreach ($waitMessage as $k => $v) {
            ...
            if (!empty($user)) {
                ...
                $waitMessage[$k]['avatar'] = empty($user['user_picture']) ? '' : app(SanmuRepository::class)->getImagePath($user['user_picture']);
            } else {
                unset($waitMessage[$k]);
            }
        }
    ...
    public static function userInfo($uid)
    {
        ...
        if (!empty($res)) {
            $res['avatar'] = empty($res['user_picture']) ? '' : app(SanmuRepository::class)->getImagePath($res['user_picture']);
        ...
        foreach ($messageList as $k => $v) {
            ...
            if ($messageList[$k]['goods_id'] > 0) {
                ...
                if (!empty($res)) {
                    ...
                    $messageList[$k]['goods']['shop_price'] = app(SanmuRepository::class)->getPriceFormat($res['shop_price']);
                    $messageList[$k]['goods']['url'] = $rootPath . '/goods.php?id=' . $v['goods_id'];
                    $messageList[$k]['goods']['goods_thumb'] = empty($res['goods_thumb']) ? '' : app(SanmuRepository::class)->getImagePath($res['goods_thumb']);
                }
            }
            ...
            if (!empty($users)) {
                ...
                $messageList[$k]['user_picture'] = empty($users['user_picture']) ? '' : app(SanmuRepository::class)->getImagePath($users['user_picture']);
            } else {
                unset($messageList[$k]);
            }
        }
    ...
    public static function getGoods($gid)
    {
        ...
        if ($goods) {
            $goods['goods_thumb'] = empty($goods['goods_thumb']) ? '' : app(SanmuRepository::class)->getImagePath($goods['goods_thumb']);
        }
                ...
                if (preg_match('/^(http|https)/is', $url)) {
                    if (preg_match('/(goods|goods.php)/i', $url)) { //验证是否有goods 控制器
                        ...
                        if (!empty($goods_info)) {
                            ...
                            $goods_name = app(SanmuRepository::class)->subStr($goods_info['goods_name'], 50);
