...
class CommonRepository extends Base
{
    ...
    public function sendEmail($name = '', $email = '', $subject = '', $content = '', $type = 0, $notification = false)
    {
        ...
        if ($this->config['mail_charset'] != EC_CHARSET) {
            $name = sanmu_iconv(EC_CHARSET, $this->config['mail_charset'], $name);
            $subject = sanmu_iconv(EC_CHARSET, $this->config['mail_charset'], $subject);
            $content = sanmu_iconv(EC_CHARSET, $this->config['mail_charset'], $content);
            $shop_name = sanmu_iconv(EC_CHARSET, $this->config['mail_charset'], $this->config['shop_name']);
        }
        ...
        if (file_exists(MOBILE_DRP)) {
            ...
            if ($drp_affiliate == 1) {
                ...
                if (!empty($config['config']['expire'])) {
                    ...
                    cache()->put('sanmumall_affiliate_drp_id' . $uid, intval($parent_id), $expiresAt);
                } else {
                    // 过期时间为 1 天
                    cache()->put('sanmumall_affiliate_drp_id' . $uid, intval($parent_id), Carbon::now()->addDays(1));
                }
            }
        }
    }
    ...
    {
        $parent_id = cache('sanmumall_affiliate_drp_id' . $uid);
        ...
        if (!is_null($parent_id)) {
            ...
            if ($forget > 0) {
                cache()->forget('sanmumall_affiliate_drp_id' . $uid); // 失效
            }
        ...
        if (file_exists(MOBILE_DRP)) {
            ...
            if ($drp_affiliate == 1) {
                ...
                if (!empty($config['config']['expire'])) {
                    ...
                    // 过期时间（以分钟为单位）
                    cookie()->queue('sanmumall_affiliate_drp_id', intval($uid), $minutes);
                } else {
                    // 过期时间（以分钟为单位）
                    $minutes = 24 * 60;
                    cookie()->queue('sanmumall_affiliate_drp_id', intval($uid), $minutes);
    ...
    {
        $uid = request()->cookie('sanmumall_affiliate_drp_id');
        ...
        if (!is_null($uid) && $uid > 0) {
            ...
            if ($count > 0) {
                ...
                cookie()->queue('sanmumall_affiliate_drp_id', '', 1);
            }
        }
    ...
    public function setUserAffiliate($uid = 0)
    {
        ...
        if (!empty($uid) && $config['on'] == 1) {
            if (!empty($config['config']['expire'])) {
                ...
                // 过期时间（以分钟为单位）
                cookie()->queue('sanmumall_affiliate_uid', intval($uid), $minutes);
            } else {
                // 过期时间（以分钟为单位）
                $minutes = 24 * 60;
                cookie()->queue('sanmumall_affiliate_uid', intval($uid), $minutes);
    ...
    {
        $uid = request()->cookie('sanmumall_affiliate_uid');
        ...
        if (!is_null($uid) && $uid > 0) {
            ...
            if ($count > 0) {
                ...
                cookie()->queue('sanmumall_affiliate_uid', '', 1);
            }
        }
