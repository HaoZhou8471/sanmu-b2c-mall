...
use App\Kernel\Repositories\Common\SanmuRepository as Base;
use App\Models\MerchantsShopInformation;
use App\Models\RsRegion;

/**
 * Class SanmuRepository
 ...
 * @method sanmuEmpower($AppKey, $activate_time) 生成授权证书
 ...
 * @method sanmuConfig($str = '') $str默认值空，多个示例:xx, xx, xx字符串组成
 * @method sanmuUrl($str = '') 获取网站地址[域名]
 ...
 * @method sanmuHttp($server = '') 获取http|https
 ...
 */
class SanmuRepository extends Base
{
    /**
     * 重写 URL 地址
     *
     ...
     */
    public function buildUri($app, $params, $append = '', $page = 0, $keywords = '', $size = 0)
{
    static $rewrite = NULL;
    ...
    return $this->sanmuUrl($uri, config('app.url'));
}   
    ...
    public function descImagesPreg($text_desc = '', $str_file = 'goods_desc', $is_mobile_div = 0)
    {
        ...
        $pathImage = [
            $this->sanmuUrl('storage/' . $image_dir . '/'),
            $this->sanmuUrl($image_dir . '/')
        ];

        $pathData = [
            $this->sanmuUrl('storage/' . $data_dir . '/'),
            $this->sanmuUrl($data_dir . '/')
        ];

        $uploads = $this->sanmuUrl('storage/uploads/');
        ...
        if ($desc_img) {
            ...
            if ($img_list && $endpoint) {
                foreach ($img_list as $key => $row) {
                    $row = trim($row);
                    if ($this->config['open_oss'] == 1) {
                        if (strpos($row, $this->sanmuUrl('storage/' . $image_dir)) !== false || strpos($row, $this->sanmuUrl($image_dir)) !== false) {
                            $row = str_replace($pathImage, '', $row);
                            $arr[] = 'storage/' . $image_dir . '/' . $row;

                            $text_desc = str_replace($pathImage, $endpoint . $image_dir . '/', $text_desc);
                        } elseif (strpos($row, $this->sanmuUrl('storage/' . $data_dir)) !== false || strpos($row, $this->sanmuUrl($data_dir)) !== false) {
                        ...
                        if (strpos($row, 'http://') !== false || strpos($row, 'https://') !== false) {
                            if (strpos($row, 'storage/' . $image_dir) !== false || strpos($row, $image_dir) !== false) {
                                ...
                                $text_desc = str_replace($pathImage, $this->sanmuUrl('storage/' . $image_dir . '/'), $text_desc);
                            } elseif (strpos($row, 'storage/' . $data_dir) !== false || strpos($row, $data_dir) !== false) {
                                $row = str_replace($pathData, '', $row);
                                $arr[] = 'storage/' . $data_dir . '/' . $row;

                                $text_desc = str_replace($pathData, $this->sanmuUrl('storage/' . $data_dir . '/'), $text_desc);
                            } elseif (strpos($row, $uploads) !== false) {
                                $arr[] = 'storage/uploads/' . $row;
                            }
                        } else {
                            if (strpos($row, 'storage') !== false) {
                                $arr[] = $row;
                                $text_desc = str_replace($row, $this->sanmuUrl($row), $text_desc);
                            } else {
                                $arr[] = 'storage/' . $row;
                                $text_desc = str_replace($row, $this->sanmuUrl('storage/' . $row), $text_desc);
                            }
                        }
                    }
                }
    ...
    {
        if (!empty($url)) {
            $prex_patch = rtrim($this->sanmuUrl(), '/') . '/';
    ...
    {
        $url = false;
        $rewrite = intval($this->sanmuConfig('rewrite'));
        ...
        if ($rewrite && !empty($urlArr[0]) && strpos($urlArr[0], '.php')) {
            ...
            //rewrite
            if ($idType == null) {
                $url = $this->sanmuUrl($app . '.html', config('app.url'));
            } else {
        ...
        if ($url) {
            ...
            if ((strpos($initUrl, 'http://') === false && strpos($initUrl, 'https://') === false)) {
                return $this->sanmuUrl($initUrl, config('app.url'));
            } else {
                return $initUrl;
            }
        }
