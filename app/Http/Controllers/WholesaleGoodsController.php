...
class WholesaleGoodsController extends InitController
{
    ...
    public function index()
    {
        ...
        if ($GLOBALS['_CFG']['two_code'] && isset($goods['goods_id']) && $goods['goods_id'] > 0) {
            ...
            $this->sanmuRepository->helpersLang('goods');
        ...
        $this->smarty->assign('im_user_id', 'sanmu' . session('user_id')); //登入用户ID;
        /*  @author-bylu  end */
        ...
        if ($seller_recommend) {
            ...
            $seller_recommend['goods_url'] = $this->sanmuRepository->buildUri('wholesale_goods', array('aid' => $seller_recommend['goods_id']), $seller_recommend['goods_name']);
    ...
    private function see_more_wholesale($suppliers_id, $goods_id)
    {
        ...
        if ($res) {
            foreach ($res as $k => $v) {
                ...
                $res[$k]['goods_url'] = $this->sanmuRepository->buildUri('wholesale_goods', array('aid' => $v['goods_id']), $v['goods_name']);
                $res[$k]['goods_name'] = $v['goods_name'];
                $res[$k]['goods_thumb'] = $this->sanmuRepository->getImagePath($v['goods_thumb']); //处理图片地址
            }
        }
