...
class OnlineController extends InitController
{
    ...
    public function index()
    {
        ...
        if ($act == 'service') {
            ...
            if ($basic_info) {
                ...
                if (empty($basic_info['kf_logo']) || $basic_info['kf_logo'] == 'http://') {
                    $basic_info['kf_logo'] = 'http://sanmu-kf.oss-cn-shanghai.aliyuncs.com/sanmu_kf/p16812444.jpg';
                }
            ...
            if ($user_id) {
                ...
                $user_info['user_id'] = 'sanmu' . $user_id;
                if (empty($user_info['user_picture'])) {
                    $user_logo = $this->sanmuRepository->getImagePath('sanmu_kf/sanmu_kf_user_logo.jpg');
                } else {
                    $user_logo = $this->sanmuRepository->getImagePath($user_info['user_picture']);
                }
            } else {
         ...
         */
        if ($act == 'history') {
            $request = sanmu_decode(request()->input('q', ''), true);
