...
class InstallController extends InitController
{
    ...
    public function index()
    {
        ...
        switch ($step) {
            ...
            case 'check':
                ...
                $templates_root = [
                    'dwt' => resource_path('views/themes/ecmoban_sanmu2017/'),
                    'lbi' => resource_path('views/themes/ecmoban_sanmu2017/library/')
                ];
            ...
            case 'setting_ui':
                ...
                session([
                    'setting_ui' => [
                        ...
                        'timezones' => $timezones,
                        'userinterface' => empty($_GET['ui']) ? 'sanmumall' : $_GET['ui']
                    ]
                ]);
                ...
                $userinterface = isset($_GET['ui']) && !empty($_GET['ui']) ? $_GET['ui'] : 'sanmumall';
            ...
            case 'do_others':
                ...
                $integrate = isset($_POST['userinterface']) ? trim($_POST['userinterface']) : 'sanmumall';
    ...
    private function api_request($url, $apiget)
    {
        ...
        $install_api = 'http://sanmu.ecmoban.com/ecmoban_sc/sanmu_pms/sc_admin.php?c=Api&a=';

        $api_comment = app(Http::class)->doGet($install_api . $url . $apiget);
        $api_str = substr($api_comment, 3);

        $api_arr = sanmu_decode($api_str, true);
