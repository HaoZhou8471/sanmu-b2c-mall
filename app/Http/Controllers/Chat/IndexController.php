...
class IndexController extends InitController
{
    ...
    public function orderList()
    {
        ...
        foreach ($goodsList as $k => $v) {
            ...
            $goodsList[$k]['goods_url'] = $this->sanmuRepository->sanmuUrl('goods.php?id=' . $v['goods_id']);
        }
 ...
 FROM " . db_config('prefix') . "im_message WHERE " . $dialogSql . " ORDER BY add_time DESC, id DESC";
            ...
            if ($v['user_type'] == 1) {
                ...
                if (empty($users['user_picture'])) {
                    ...
                    if (strpos($users['user_picture'], 'http') !== false) {
                        ...
                        $services[$k]['avatar'] = $this->sanmuRepository->sanmuUrl($users['user_picture']);
                    }
                }
            }
    ...
    public function serviceChatData()
    {
        ...
        if (!empty($goods)) {
            $goods['goods_thumb'] = get_image_path($goods['goods_thumb']);
            $goods['goods_url'] = $this->sanmuRepository->sanmuUrl('goods.php?id=' . $goods['goods_id']);
        }
