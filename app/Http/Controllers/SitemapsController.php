...
class SitemapsController extends InitController
{
    ...
    public function index()
    {
        if (file_exists(storage_public(DATA_DIR . '/sitemap.dat')) && time() - filemtime(storage_public(DATA_DIR . '/sitemap.dat')) < 86400) {
            ...
            if ($res) {
                foreach ($res as $row) {
                    if (strpos($this->sanmuRepository->buildUri('category', ['cid' => $row['cat_id']], $row['cat_name']), $site_url) === false) {
                        $cat_loc = "$site_url/" . $this->sanmuRepository->buildUri('category', ['cid' => $row['cat_id']], $row['cat_name']);
                    } else {
                        $cat_loc = $this->sanmuRepository->buildUri('category', ['cid' => $row['cat_id']], $row['cat_name']);
                    }
            ...
            if ($res) {
                foreach ($res as $row) {
                    if (strpos($this->sanmuRepository->buildUri('article_cat', ['acid' => $row['cat_id']], $row['cat_name']), $site_url) === false) {
                        $art_cat_loc = "$site_url/" . $this->sanmuRepository->buildUri('article_cat', ['acid' => $row['cat_id']], $row['cat_name']);
                    } else {
                        $art_cat_loc = $this->sanmuRepository->buildUri('article_cat', ['acid' => $row['cat_id']], $row['cat_name']);
                    }
            ...
            if ($res) {
                foreach ($res as $row) {
                    if (strpos($this->sanmuRepository->buildUri('goods', ['gid' => $row['goods_id']], $row['goods_name']), $site_url) === false) {
                        $goods_loc = "$site_url/" . $this->sanmuRepository->buildUri('goods', ['gid' => $row['goods_id']], $row['goods_name']);
                    } else {
                        $goods_loc = $this->sanmuRepository->buildUri('goods', ['gid' => $row['goods_id']], $row['goods_name']);
                    }
