...
class ActivityController extends InitController
{
    ...
    public function index()
    {
        ...
        $this->sanmuRepository->helpersLang(['shopping_flow', 'user']);
        ...
        if ($act == 'view') {
            ...
            /* 跳转H5 start */
            $Loaction = sanmu_url('/#/activity/detail/' . $act_id);
                    ...
                    case 0:
                        ...
                        if (is_array($row['gift'])) {
                            foreach ($row['gift'] as $k => $v) {
                                ...
                                if ($goods) {
                                    $goods = $goods->toArray();
                                    $row['gift'][$k]['thumb'] = $this->sanmuRepository->getImagePath($goods['goods_thumb']);
                                    $row['gift'][$k]['price'] = $this->sanmuRepository->getPriceFormat($v['price']);
                                }
                            }
                        }
                        break;
