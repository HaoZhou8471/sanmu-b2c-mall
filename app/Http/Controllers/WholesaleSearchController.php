    ...
    public function index()
    {
        ...
        if ($res) {
            foreach ($res as $key => $row) {
                ...
                if ($display == 'grid') {
                    $arr[$row['goods_id']]['goods_name'] = $GLOBALS['_CFG']['goods_name_length'] > 0 ? $this->sanmuRepository->subStr($row['goods_name'], $GLOBALS['_CFG']['goods_name_length']) : $row['goods_name'];
                } else {
                ...
                $arr[$row['goods_id']]['suppliers_url'] = $this->sanmuRepository->buildUri('wholesale_suppliers', array('sid' => $suppliers['suppliers_id']));
                // 供应商获取配置客服QQ
                ...
                $arr[$row['goods_id']]['url'] = $this->sanmuRepository->buildUri('wholesale_goods', array('aid' => $row['goods_id']), $row['goods_name']);
            }
        }
