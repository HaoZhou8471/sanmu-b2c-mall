...
class GoodsController extends InitController
{
    ...
    public function index()
    {
        ...
        /* 跳转H5 start */
        $Loaction = sanmu_url('/#/goods/' . $goods_id);
        ...
        if (!empty($act) && $act == 'price') {
            ...
            if ($goods_id == 0) {
                ...
                $shop_price = $this->goodsCommonService->getFinalPrice($goods_id, $number, true, $attr_id,  $area_id, $area_city);
                ...
                //属性价格
                if ($attr_id) {
                    $spec_price = $this->goodsCommonService->getFinalPrice($goods_id, $number, true, $attr_id, $ $area_id, $area_city, 1, 0, 0, $res['show_goods'], $product_promote_price);
                } else {
                    $spec_price = 0;
                }
            ...
            $fittings_list = get_goods_fittings([$goods_id],   $area_id, $area_city);

            if ($fittings_list) {
                $fittings_attr = $attr_id;

                $goods_fittings = get_goods_fittings_info($goods_id,  $area_id, $area_city, '', 1, '', $fittings_attr);
            ...
            $limit = 1;
            $area_position_list = get_goods_user_area_position($goods['user_id'],   $area_id, $area_city, $this->city_id, $get_attr_id, $goods_id, 0, 0, 1, 0, $limit);
...
*/
        ...
        if ($shop_information) {
            ...
            if ($goods['user_id'] == 0) {
                //判断平台是否开启了IM在线客服
                $kf_im_switch = SellerShopinfo::where('user_id', 0)->value('contact_caiwu');
                if ($kf_im_switch) {
        ...
        $this->smarty->assign('im_user_id', 'sanmu' . $user_id); //登入用户ID;
        /*  @author-bylu  end */
        ...
        //$this->smarty->assign('extend_info', get_goods_extend_info($goods_id));  
     ...
     $this->smarty->assign('get_categories_tree', $get_categories_tree); 
	              $parent_cat_id = $this->initService->get_parent_cat_id($goods['cat_id']);
				  $parent_cat_id = $this->initService->get_child_cat($parent_cat_id);
             $this->smarty->assign('child_cat',$parent_cat_id);
			 
			 
			 $get_cat_brands  = $this->initService->get_cat_brands($goods['cat_id']);
			  $this->smarty->assign('get_cat_brands',$get_cat_brands); 
