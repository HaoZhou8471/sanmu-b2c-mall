...
class AjaxWholesaleController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($act == 'edit_wholesale_invoice') {
            ...
            if (empty($cart_goods) && empty($from) || !$this->flowUserService->checkConsigneeInfo($consignee, $flow_type) && empty($from)) {
                ...
                if ((!isset($this->config['can_invoice']) || $this->config['can_invoice'] == '1') && isset($this->config['invoice_content']) && trim($this->config['invoice_content']) != '' && $flow_type != CART_EXCHANGE_GOODS) {
                    ...
                    //抬头名称
                    $sql = "SELECT * FROM " . $GLOBALS['sanmu']->table('order_invoice') . " WHERE user_id='$user_id' LIMIT 10";
                    ...
                    /* 增票信息 */
                    $sql = " SELECT * FROM " . $GLOBALS['sanmu']->table('users_vat_invoices_info') . " WHERE user_id='$user_id' LIMIT 1 ";
        ...
        elseif ($act == 'wholesale_gotoInvoice') {
            ...
            /* 保存发票纳税人识别码 */
            if (empty($invoice_id)) {
                $sql = "SELECT invoice_id FROM " . $GLOBALS['sanmu']->table('order_invoice') . " WHERE inv_payee = '$inv_payee' AND user_id = '$user_id'";
                if (!$GLOBALS['db']->getOne($sql)) {
                    $sql = "INSERT INTO " . $GLOBALS['sanmu']->table('order_invoice') . " (`tax_id`) VALUES ('$tax_id')";
                    $GLOBALS['db']->query($sql);
                }
            } else {
                $sql = "UPDATE " . $GLOBALS['sanmu']->table('order_invoice') . " SET tax_id='$tax_id' WHERE invoice_id='$invoice_id'";
                $GLOBALS['db']->query($sql);
            }
            ...
            if (empty($cart_goods) && empty($from) || !$this->flowUserService->checkConsigneeInfo($consignee, $flow_type) && empty($from)) {
                ...
                if ($inv_content) {
                    if ($invoice_id > 0) {
                        $sql = "SELECT inv_payee FROM " . $GLOBALS['sanmu']->table('order_invoice') . " WHERE invoice_id='$invoice_id'";
