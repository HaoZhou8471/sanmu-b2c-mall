...
class AjaxUserController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($act == 'upload_user_picture') {
            ...
            if ($filename && $user_id > 0) {
                ...
                if (!empty($filename_arr)) {
                    ...
                    sanmu_unlink($filename_cropper);

                    $parent['user_picture'] = $filename_120;
                    Users::where('user_id', $user_id)->update($parent);

                    $this->sanmuRepository->getOssAddFile([$filename_120, $filename_48, $filename_24]);
                ...
                if (!empty($img_list)) {
                    foreach ($img_list as $key => $val) {
                        if ($img_id == $val['id']) {
                            ...
                            $this->sanmuRepository->getOssDelFile([$val['img_file']]);

                            sanmu_unlink(storage_public($val['img_file']));
                        }
                    }
                }
        ...
        elseif ($act == 'getUserInfo') {
            ...
            if (!empty($seckillid)) {
                $seckillid = sanmu_decode($seckillid, true);
            }
        ...
        elseif ($act == 'user_auction_gotopage') {
            ...
            if (count($id) == 1) {
                ...
                $id = explode("|", $id[1]);
                $auction = $this->sanmuRepository->getStrArray1($id);
        ...
        elseif ($act == 'user_snatch_gotopage') {
            ...
            if (count($id) == 1) {
                ...
                $id = explode("|", $id[1]);
                $snatch = $this->sanmuRepository->getStrArray1($id);
