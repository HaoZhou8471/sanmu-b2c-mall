...
class UserMessageController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($action == 'comment_list') {
            ...
            if ($img_list) {
                foreach ($img_list as $key => $val) {
                    $this->sanmuRepository->getOssDelFile([$val['comment_img'], $val['img_thumb']]);
                    sanmu_unlink(storage_public($val['comment_img']));
                    sanmu_unlink(storage_public($val['img_thumb']));
                }
            }
        ...
        elseif ($action == 'del_msg') {
            ...
            if ($is_order) {
                return sanmu_header("Location: user_message.php?act=message_list&is_order=1&order_id=$order_id\n");
            } else {
                return sanmu_header("Location: user_message.php?act=message_list&order_id=$order_id\n");
            }
        }
