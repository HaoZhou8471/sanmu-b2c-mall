...
class CrowdfundingController extends InitController
{
    ...
    public function index()
    {
        ...
        if ($zcgoods_id) {
            $Loaction = sanmu_url('/#/crowdfunding/detail/' . $zcgoods_id);
        } else {
            $Loaction = sanmu_url('/#/crowdfunding');
        }
        ...
        if ($act == 'detail') {
            ...
            if (empty($zhongchou)) {
                return sanmu_header("Location: crowdfunding.php\n");
            }
            ...
            if ($zhongchou) {
                ...
                $zhongchou['format_join_money'] = $this->sanmuRepository->getPriceFormat($zhongchou['join_money']);
                $zhongchou['format_join_money'] = str_replace(['<em>', '</em>'], ['<span>', '</span>'], $zhongchou['format_join_money']);

                $zhongchou['title_img'] = $this->sanmuRepository->getImagePath($zhongchou['title_img']);
                ...
                if (!empty($zhongchou['img'])) {
                    ...
                    if (!empty($zhongchou['img'])) {
                        foreach ($zhongchou['img'] as $k2 => $v2) {
                            $zhongchou['img'][$k2] = $this->sanmuRepository->getImagePath($v2);
                        }
                    }
                }
            }
            ...
            $base_url = $GLOBALS['sanmu']->http() . request()->server('SERVER_NAME') . request()->server('PHP_SELF');
            ...
            if ($GLOBALS['_CFG']['open_oss'] == 1) {
                $logo = $logo ? $this->sanmuRepository->getImagePath($logo) : '';
            } else {
            ...
            $this->sanmuRepository->getOssAddFile([$image]);

            $this->smarty->assign('weixin_img_url', $this->sanmuRepository->getImagePath($image));
                ...
                foreach ($zc_evolve_list as $k => &$v) {
                    ...
                    if (!empty($v['img'])) {
                        foreach ($v['img'] as $k2 => $v2) {
                            $v['img'][$k2] = $this->sanmuRepository->getImagePath($v2);
                        }
                    }
                }
            }
        ...
        if ($act == 'checkout') {
            ...
            if (empty($user_id)) {
                ...
                return sanmu_header("Location: user.php\n");
            }
            //获取收货地址 by wu
        ...
        if ($act == 'consignee') {
            ...
            $this->sanmuRepository->helpersLang(['user', 'shopping_flow']);
        ...
        if ($act == 'drop_consignee') {
            ...
            if ($this->userAddressService->dropConsignee($consignee_id, $user_id)) {
                return sanmu_header("Location: crowdfunding.php?act=consignee&gid=" . $gid . "\n");
            } else {
        ...
        if ($act == 'done') {
            ...
            $order['format_goods_amount'] = $this->sanmuRepository->getPriceFormat($order['goods_amount']);
            $order['format_shipping_fee'] = $this->sanmuRepository->getPriceFormat($order['shipping_fee']);
            $order['format_order_amount'] = $this->sanmuRepository->getPriceFormat($order['order_amount']);
