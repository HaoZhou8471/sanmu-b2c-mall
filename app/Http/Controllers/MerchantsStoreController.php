...
class MerchantsStoreController extends InitController
{
    ...
    public function index($shop = '')
    {
        ...
        if ($seller_domain) {
            ...
            $this->smarty->assign('shop_url', $this->sanmuRepository->sellerUrl($merchant_id));
        ...
        /* 跳转H5 start */
        $Loaction = sanmu_url('/#/shopHome/' . $merchant_id);
        ...
        /*记录访问者IP*/
        $realip = $this->sanmuRepository->sanmuIp();
        ...
        if (($merchant_id == 0 || $shop_id < 1) && $temp_code == '') {
            return sanmu_header("Location: " . config('app.url') . "\n");
        }
        ...
        */
            ...
            if (!empty($cou_data) && isset($cou_data)) {
                ...
                if ($cou_info['ru_id'] == $merchant_id) {
                    ...
                    if (!empty($rec_id) && isset($rec_id)) {
                        ...
                        $cou_info[0]['format_cou_money'] = $this->sanmuRepository->getPriceFormat($cou_info[0]['cou_money']);
                    }
                }
            } else {
                $cou_info = [];
        ...
        if (!empty($act) && $act == 'in_stock') {
            ...
            $basic_info['license_fileImg'] = isset($basic_info['license_fileImg']) && $basic_info['license_fileImg'] ? $this->sanmuRepository->getImagePath($basic_info['license_fileImg']) : '';
            //处理营业执照所在地
            if ($basic_info) {
            ...
            $this->smarty->assign('seller_qrcode_img', $this->sanmuRepository->getImagePath($image));
            ...
            $basic_info['shop_logo'] = $this->sanmuRepository->getImagePath($basic_info['shop_logo']);
        ...
        if (!$this->smarty->is_cached($dwt, $cache_id)) {
            ...
            if ($cat_id > 0) {
                ...
                if ($brands) {
                    foreach ($brands as $key => $val) {
                        ...
                        $shop_url = $this->sanmuRepository->sellerUrl($merchant_id, $build_uri);
            ...
            if ($seller_templates) {
                ...
                $head_temp['out'] = str_replace(' ../data / ', $this->sanmuRepository->getImagePath(DATA_DIR . "/"), $head_temp['out'], $i);
