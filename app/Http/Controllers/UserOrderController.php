...
class UserOrderController extends InitController
{
    ...
    )
    {
        ...
        $this->config = $sanmuRepository->sanmuConfig();
    ...
    public function index()
    {
        ...
        $this->sanmuRepository->helpersLang(['user']);
        ...
        if (in_array($action, $ui_arr)) {
            ...
            if (!in_array($action, $not_login_arr) || $user_id == 0) {
                ...
                $back_act = $this->sanmuRepository->sanmuUrl('user.php' . $referer);
                return sanmu_header('location:' . $back_act);
            }
        }
        ...
        if ($action == 'order_list' || $action == 'order_recycle') {
            ...
            if ($this->sanmuRepository->strLen($order_type) == $this->sanmuRepository->strLen('toBe_unconfirmed')) {
                $order_where = 1;
            } elseif ($this->sanmuRepository->strLen($order_type) == $this->sanmuRepository->strLen('toBe_pay')) {
                $order_where = 2;
            } elseif ($this->sanmuRepository->strLen($order_type) == $this->sanmuRepository->strLen('toBe_confirmed')) {
                $order_where = 3;
            } elseif ($this->sanmuRepository->strLen($order_type) == $this->sanmuRepository->strLen('toBe_finished')) {
                $order_where = 4;
            } else {
                $order_where = 0;
            }
        ...
        elseif ($action == 'order_detail' || $action == 'auction_order_detail') {
            ...
            //
            //延迟收货申请 start
            $order['sanmu_shipping_status'] = $order['shipping_status'];
            //获取延迟收货时间段 end
        ...
        elseif ($action == 'cancel_order') {
            ...
            if (cancel_order($order_id, $user_id)) {
                return sanmu_header("Location: user_order.php?act=order_list\n");
            } else {
        ...
        elseif ($action == 'complaint_apply') {
            ...
            if ($ru_id == 0) {
                ...
                if ($kf_im_switch) {
                    $orders['is_sanmu'] = true;
                } else {
                    $orders['is_sanmu'] = false;
                }
            } else {
                $orders['is_sanmu'] = false;
            }
        ...
        elseif ($action == 'trade') {
            ...
            if ($row['ru_id'] == 0) {
                ...
                $kf_im_switch = SellerShopinfo::where('ru_id', 0)->value('kf_im_switch');
                if ($kf_im_switch) {
                    $shop_information['is_sanmu'] = true;
                } else {
                    $shop_information['is_sanmu'] = false;
                }
            } else {
                $shop_information['is_sanmu'] = false;
            }
        ...
        elseif ($action == 'goods_order') {
            ...
            $formated_goods_amount = $price ? $this->sanmuRepository->getPriceFormat(array_sum($price), false) : $this->sanmuRepository->getPriceFormat(0);
...
//            $back_number = (int)request()->input('attr_num', 0);
        ...
        elseif ($action == 'cancel_return') {
            ...
            if (cancel_return($ret_id, $user_id)) {
                return sanmu_header("Location: user_order.php?act=return_list\n");
            } else {
        ...
        elseif ($action == 'affirm_received') {
            ...
            if (affirm_received($order_id, $user_id)) {
                ...
                if ($act == 'auction') {
                    return sanmu_header("Location: user_activity.php?act=auction\n");
                } elseif ($act == 'crowdfunding') {
                    return sanmu_header("Location: user_crowdfund.php?act=crowdfunding\n");
                } elseif ($act == 'info') {
