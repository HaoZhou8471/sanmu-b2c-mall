    ...
    public function index()
    {
        ...
        if ($act == 'js') {
            ...
            if (!empty($ad_info)) {
                /* 转换编码 */
                if ($charset != 'UTF8') {
                    $ad_info['ad_name'] = sanmu_iconv('UTF8', $charset, $ad_info['ad_name']);
                    $ad_info['ad_code'] = sanmu_iconv('UTF8', $charset, $ad_info['ad_code']);
                }
        ...
        /* ------------------------------------------------------ */
            ...
            if ($ad_id == '-1') {
                ...
                $uri = $this->sanmuRepository->buildUri('goods', ['gid' => $goods_id], $goods_name);

                return sanmu_header("Location: $uri\n");
            } else {
                /* 更新站内广告的点击次数 */
                ...
                if (!empty($ad_info['ad_link'])) {
                    $uri = (strpos($ad_info['ad_link'], 'http://') === false && strpos($ad_info['ad_link'], 'https://') === false) ? $this->sanmu->http() . urldecode($ad_info['ad_link']) : urldecode($ad_info['ad_link']);
                } else {
                    $uri = url('/');
                }
