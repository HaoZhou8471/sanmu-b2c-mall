...
class FlowController extends InitController
{
    ...
    public function index()
    {
        ...
        if ($step == 'link_buy') {
            ...
            return sanmu_header("Location:./flow.php\n");
        }
        ...
        elseif ($step == 'login') {
            ...
            //第三方登录判断
            if ($user_id > 0) {
                return sanmu_header("Location:./flow.php?step=consignee\n");
            }
        ...
        elseif ($step == 'consignee') {
            ...
            if (request()->server('REQUEST_METHOD') == 'GET') {
                ...
                return sanmu_header("Location: flow.php?step=checkout&direct_shopping=1\n");
            }
        }
            ...
            if ($act == 'balance') {
                ...
                if ($order_info && $user_money >= $order_amount) {
                    ...
                    if ($order_info['user_id'] > 0) {
                        ...
                        //付款成功,跳转到 支付成功页;
                        return sanmu_header("location:flow.php?step=pay_success&order_id=" . $order_info['order_id'] . "&store_id=$store_id");
                    }
                } else {
                    //余额不足;
                ...
                if (empty($order_info) || $logCont > 0) {
                    if ($logCont > 0) {
                        //付款成功,跳转到 支付成功页;
                        return sanmu_header("location:flow.php?step=pay_success&order_id=" . $order_id . "&store_id=" . $store_id);
                    } else {
                ...
                if (!$count) {
                    ...
                    return sanmu_header("Location:flow.php?step=order_reload" . $where_reload . "\n");
                }
                ...
                if (!$this->flowUserService->checkConsigneeInfo($consignee, $flow_type) && $store_id == 0) {
                    /* 如果不完整则转向到收货人信息填写界面 */
                    return sanmu_header("Location: user_address.php?act=address_list\n"); //by wu
                }
                ...
                /* 订单中的商品 */
                $cart_goods = $this->sanmuRepository->turnPluckFlattenOne($cart_goods_list);
...
HTML;
        ...
        elseif ($step == 'pdf') {
            ...
            $chat = $this->sanmuRepository->chatQq($shop_info);
...
HTML;
        ...
        /*------------------------------------------------------ */
            ...
            if ($one_step_buy == 1) {
                ...
                return sanmu_header("Location: flow.php?step=checkout" . $param . "\n");
            }
