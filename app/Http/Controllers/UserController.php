...
class UserController extends InitController
{
    ...
    public function index()
    {
        ...
        if ($require_login == 1) {
            //未登录提交数据。非正常途径提交数据！
            return sanmu_header('location:' . $this->sanmuRepository->sanmuUrl('user.php'));
        }
        ...
        if (in_array($action, $ui_arr)) {
            ...
            if ($user_id) {
                ...
                if (isset($info['is_validated']) && !$info['is_validated'] && $this->config['user_login_register'] == 1) {
                    ...
                    return sanmu_header('location:' . $Location);
                }
            }

            $count = AdminUser::where('user_id', session('user_id'))->count();
            if ($count) {
                $is_merchants = 1;
            } else {
                $is_merchants = 0;
        ...
        if ($action == 'register') {
            if ($this->config['shop_reg_closed'] == 1) {
                return sanmu_header('location:' . $this->sanmuRepository->sanmuUrl());
            }
            ...
            $register_url = $this->sanmuRepository->buildUri('article', ['aid' => $register_article_id], $GLOBALS['_LANG']['protocol_bind']);
        ...
        elseif ($action == 'email_send_succeed') {
            ...
            return sanmu_header('Location: ' . url('/') . '/' . "user.php");
        }
        ...
        /* ------------------------------------------------------ */
        elseif ($action == 'login') {
            $sanmu_token = get_sanmu_token();
            $this->smarty->assign('sanmu_token', $sanmu_token);
        ...
        elseif ($action == 'to_paid') {
            ...
            if ($value_card_id > 0) {
                ...
                return sanmu_header("location:user.php?act=value_card");
            }
        }
        ...
        elseif ($action == 'take_list') {
            ...
            if ($up_id) {
                return sanmu_header("Location: user.php?act=take_list\n");
            } else {
        ...
        elseif ($action == 'act_del_tag') {
            ...
            return sanmu_header("Location: user.php?act=tag_list\n");
        }
        ...
        elseif ($action == 'act_del_booking') {
            ...
            if ($id == 0 || $user_id == 0) {
                return sanmu_header("Location: user.php?act=booking_list\n");
            }
        ...
        elseif ($action == 'cancel') {
            ...
            $id = (int)request()->input('id', 0);
            if ($id == 0 || $user_id == 0) {
                return sanmu_header("Location: user.php?act=account_log\n");
            }
        ...
        elseif ($action == 'pay') {
            ...
            if ($payment_id == 0) {
                return sanmu_header("Location: user.php?act=account_deposit&id=" . $surplus_id . "\n");
            }
        ...
        elseif ($action == 'act_edit_surplus') {
            ...
            $user_id = $order_goods ? $order_goods['user_id'] : 0;
            ...
            if ($user_id == 0) {
                ...
                $sms_shop_mobile = SellerShopinfo::where('user_id', $user_id)->value('mobile');
            }
            ...
            if ($request['pay_status'] == 'auction') {
                return sanmu_header('Location: user_order.php?act=auction_order_detail&order_id=' . $order_id . "\n");
            } else {
                return sanmu_header('Location: user_order.php?act=order_detail&order_id=' . $order_id . "\n");
            }
        }
        ...
        elseif ($action == 'save_order_address') {
            ...
            if (save_order_address($address, $user_id)) {
                return sanmu_header('Location: user_order.php?act=order_detail&order_id=' . $address['order_id'] . "\n");
            } else {
        ...
        elseif ($action == 'merchants_upgrade_log') {
            ...
            $record_count = SellerApplyInfo::where('user_id', $user_id)->count();
        ...
        elseif ($action == 'application_grade' || $action == 'application_grade_edit') {
            ...
            if ($action == 'application_grade_edit') {
                ...
                if ($seller_apply_info['pay_id'] > 0 && $seller_apply_info['is_paid'] == 0 && $seller_apply_info['pay_status'] == 0) {
                    ...
                    if ($payment_info === false) {
                        ...
                        if (substr($payment_info['pay_code'], 0, 4) == 'pay_') {
                            ...
                            $apply['user_id'] = $seller_apply_info['user_id'];
                ...
                /*判断是否存在未支付未失效申请*/
                $seller_apply_info = SellerApplyInfo::where('user_id', $user_id)->where('apply_status', 0)->where('is_paid', 0)->first();
                $seller_apply_info = $seller_apply_info ? $seller_apply_info->toArray() : [];
        ...
        elseif ($action == 'grade_load') {
            ...
            if ($apply_id > 0) {
                $seller_apply_info = SellerApplyInfo::where('user_id', $user_id)->where('apply_id', $apply_id)->first();
                $seller_apply_info = $seller_apply_info ? $seller_apply_info->toArray() : [];
            ...
            elseif ($type == 'real_name') {
                ...
                if ($step == 'first') {
                    ...
                    if (empty($_FILES['front_of_id_card']['size'])) {
                        ...
                        $this->sanmuRepository->getOssAddFile([$front_name]);
                    }
                    ...
                    if (empty($_FILES['reverse_of_id_card']['size'])) {
                        ...
                        $this->sanmuRepository->getOssAddFile([$reverse_name]);
                    }
        ...
        elseif ($action == 'flow_inv_form') {
            ...
            $obj = sanmu_decode(request()->input('msg', ''));
            $arr = $this->sanmuRepository->objectToArray($obj);
        ...
        elseif ($action == 'goods_report') {
            ...
            $goods_info['url'] = $this->sanmuRepository->buildUri('goods', ['gid' => $goods_info['goods_id']], $goods_info['goods_name']);
