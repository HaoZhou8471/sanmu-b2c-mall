...
class SearchController extends InitController
{
    ...
    public function index()
    {
        ...
        if (request()->has('keywords')) {
            ...
            $Loaction = sanmu_url('/#/searchList?keywords=' . $keywords);
        } else {
            $Loaction = sanmu_url('/#/search/');
        }
        ...
        if (empty($encode)) {
            ...
            return sanmu_header("Location: search.php?encode=$string\n");
        } else {
        ...
        $brands_id = $this->sanmu->get_explode_filter($request['brand']); //过滤品牌参数
                ...
                $content = cache()->remember('search.dwt.' . $cache_id, $this->config['cache_time'], function () use ($user_id, $cat_id, $insert_keyword, $price_min, $price_max, $brands_id, $warehouse_id, $area_id, $area_city, $where_ext, $cou_id, $ur_here, $get_price_min, $get_price_max, $user_cou, $spec_goods_ids, $display, $goods_num, $size, $page, $sort, $order, $search_type, $is_ship, $is_self, $have, $re_brand, $re_goods_type, $re_sc_ds, $re_outstock, $request) {
                        ...
                        if ($price_grade) {
                            foreach ($price_grade as $key => $val) {
                                if ($val['sn'] != '') {
                                    ...
                                    $price_grade[$temp_key]['url'] = $brands[$temp_key]['url'] = $this->sanmuRepository->buildUri('search', ['bid' => $brands_id, 'chkw' => $request['keywords'], 'price_min' => $price_grade[$temp_key]['start'], 'price_max' => $price_grade[$temp_key]['end'], 'filter_attr' => $filter_attr_str, 'cou_id' => $cou_id], $ur_here);
                                    /* 判断价格区间是否被选中 */
                    ...
                    if ($brands_list) {
                        foreach ($brands_list as $key => $val) {
                            ...
                            $brands[$temp_key]['url'] = $this->sanmuRepository->buildUri('search', ['bid' => $val['brand_id'], 'chkw' => $request['keywords'], 'price_min' => $price_min, 'price_max' => $price_max, 'filter_attr' => $filter_attr_str, 'cou_id' => $cou_id], $val['brand_name']);
                    ...
                    if (empty($g_price) && ($price_min > 0 || $price_max > 0)) {
                        ...
                        $parray = [];
                        $parray['purl'] = $this->sanmuRepository->buildUri('search', ['bid' => $brands_id, 'chkw' => $request['keywords'], 'price_min' => 0, 'price_max' => 0, 'filter_attr' => $filter_attr_str, 'cou_id' => $cou_id], $ur_here);
                        ...
                        foreach ($cat_filter_attr as $key => $value) {
                            ...
                            if ($attributeInfo) {
                                ...
                                foreach ($attr_list as $k => $v) {
                                    ...
                                    $all_attr_list[$key]['attr_list'][$temp_key]['url'] = $this->sanmuRepository->buildUri('category', ['cid' => $cat_id, 'bid' => $brands_id, 'price_min' => $price_min, 'price_max' => $price_max, 'filter_attr' => $temp_arrt_url], $cat['cat_name']);
