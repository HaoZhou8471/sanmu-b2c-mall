...
class GalleryController extends InitController
{
    ...
    public function index()
    {
        ...
        $content = cache()->remember('gallery.dwt.' . $cache_id, $this->config['cache_time'], function () use ($goods_id) {
            ...
            if ($img_count == 0) {
                /* 如果没有图片，返回商品详情页 */
                return sanmu_header('Location: goods.php?id=' . $goods_id . "\n");
            } else {
                if ($img_list) {
                    foreach ($img_list as $key => $img) {
                        $galleryInfo['list'][] = [
                            'gallery_thumb' => $this->sanmuRepository->getImagePath($img_list[$key]['thumb_url']),
                            'gallery' => $this->sanmuRepository->getImagePath($img_list[$key]['img_url']),
