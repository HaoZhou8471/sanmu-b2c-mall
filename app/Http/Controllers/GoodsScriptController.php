    ...
    public function index()
    {
        ...
        if (!$this->smarty->is_cached($tpl, $cache_id)) {
            ...
            if ($type == '') {
                ...
                $_from = (!empty($_charset) && $_charset != 'UTF8') ? urlencode(sanmu_iconv('UTF-8', 'GBK', $sitename)) : urlencode(@$sitename);
            ...
            if ($res) {
                foreach ($res as $goods) {
                    ...
                    // 转换编码
                    $goods['goods_price'] = $this->sanmuRepository->getPriceFormat($goods['goods_price']);
                    if ($charset != EC_CHARSET) {
                        ...
                        $goods['goods_name'] = sanmu_iconv(EC_CHARSET, $charset, $tmp_goods_name);
                        $goods['goods_price'] = sanmu_iconv(EC_CHARSET, $charset, $goods['goods_price']);
                    }
                    $goods['goods_name'] = $this->config['goods_name_length'] > 0 ? $this->sanmuRepository->subStr($goods['goods_name'], $this->config['goods_name_length']) : $goods['goods_name'];
