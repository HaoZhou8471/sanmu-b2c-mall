...
class CategoryController extends InitController
{
    ...
    public function index()
    {
        ...
        //过滤品牌参数
        $brands_id = $this->sanmu->get_explode_filter((request()->input('brand', '')));
        ...
        $content = cache()->remember($dwt_name . '.dwt.' . $cache_id, $seconds, function () use ($cat_id, $cat_row, $dwt_name, $warehouse_id, $area_id, $area_city, $where_ext, $brands_id, $price_min, $price_max, $display, $user_id, $ship, $self, $have, $filter_attr, $filter_attr_str, $size, $page, $sort, $order) {
            ...
            if (($cat_row['parent_id'] == 0 && $cat_row['is_top_style'] == 1) == false) { //非顶级分类页和设置为顶级分类样式
                    ...
                    if ($price_grade) {
                        foreach ($price_grade as $key => $val) {
                            if ($val['sn'] != '') {
                                ...
                                $price_grade[$temp_key]['url'] = $this->sanmuRepository->buildUri('category', ['cid' => $cat_id, 'bid' => $brands_id, 'price_min' => $price_grade[$temp_key]['start'], 'price_max' => $price_grade[$temp_key]['end'], 'filter_attr' => $filter_attr_str], $cat['cat_name']);
                ...
                foreach ($brands_list as $key => $val) {
                    ...
                    $brands[$temp_key]['url'] = $this->sanmuRepository->buildUri('category', ['cid' => $cat_id, 'bid' => $val['brand_id'], 'price_min' => $price_min, 'price_max' => $price_max, 'filter_attr' => $filter_attr_str], $cat['cat_name']);
                ...
                if (empty($g_price) && ($price_min > 0 || $price_max > 0)) {
                    ...
                    $parray['purl'] = $this->sanmuRepository->buildUri('category', ['cid' => $cat_id, 'bid' => $brands_id, 'price_min' => 0, 'price_max' => 0, 'filter_attr' => $filter_attr_str], $cat['cat_name']);
