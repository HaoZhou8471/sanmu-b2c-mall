...
class SeckillController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($act == 'view') {
            ...
            /* 跳转H5 start */
            $Loaction = sanmu_url('/#/seckill/detail?seckill_id=' . $seckill_id);
            $uachar = $this->sanmuRepository->getReturnMobile($Loaction);
            ...
            if ($seckillInfo['is_end'] && !$seckillInfo['status']) {
                $Location = $this->sanmuRepository->buildUri('goods', ['gid' => $seckillInfo['goods_id']], $seckillInfo['goods_name']);
        ...
        elseif ($act == 'buy') {
            ...
            return sanmu_header("Location: flow.php?step=checkout\n");
        }
