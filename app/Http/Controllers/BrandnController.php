    ...
    public function index()
    {
        ...
        elseif ($act == 'new') {
            ...
            $content = cache()->remember('brandn_new.dwt.' . $cache_id, $this->config['cache_time'], function () use ($brand_id, $cate, $where_ext, $price_min, $price_max, $warehouse_id, $area_id, $area_city, $size, $page, $sort, $order, $display, $act, $is_ship, $is_self) {
                ...
                $goods = $this->sanmu->page_array($size, $page, $goods);
        ...
        elseif ($act == 'cancel') {
            ...
            if ($type == 0) {
                ...
                if ($count) {
                    return show_message($this->db->errorMsg(), $GLOBALS['_LANG']['back'], $this->sanmu->url, 'error');
                } else {
                    return sanmu_header("Location: user_collect.php?act=focus_brand\n");
                }
            }
        }
