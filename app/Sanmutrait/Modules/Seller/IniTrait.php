...
namespace App\Sanmutrait\Modules\Seller;
...
trait IniTrait
{
    protected function initialize()
    {
        ...
        /* 创建 SHOP 对象 */
        $this->sanmu = $GLOBALS['sanmu'] = new Shop();
        define('DATA_DIR', $this->sanmu->data_dir());
        define('IMAGE_DIR', $this->sanmu->image_dir());
        ...
        /* 如果有新版本，升级 */
        if (!isset($GLOBALS['_CFG']['sanmu_version'])) {
            $GLOBALS['_CFG']['sanmu_version'] = 'v1.0';
        }
        ...
        if ($_REQUEST['act'] != 'login' && $_REQUEST['act'] != 'signin' &&
            ...
            $admin_path = preg_replace('/:\d+/', '', $this->sanmu->seller_url()) . SELLER_PATH; //重置路径
        ...
        if ($seller_id > 0) {
            ...
            $admin_info['store_user_img'] = $admin_info && $admin_info['admin_user_img'] ? app(SanmuRepository::class)->getImagePath($admin_info['admin_user_img']) : '';
        ...
        $this->smarty->assign('site_url', str_replace(['http://', 'https://'], "", $this->sanmu->get_domain()));
        ...
        if ($adminru) {
            ...
            if ($domain_name && $row['is_enable']) {
                ...
                $head_shop_url = app(SanmuRepository::class)->buildUri('merchants_store', $head_build_uri, $head_shop_name);
