...
namespace App\Sanmutrait\Modules\Stores;
...
trait IniTrait
{
    protected function initialize()
    {
        ...
        if (!isset($GLOBALS['_CFG']['sanmu_version'])) {
            $GLOBALS['_CFG']['sanmu_version'] = 'v3.0';
        }
        ...
        if ($_REQUEST['act'] != 'login' && $_REQUEST['act'] != 'signin' &&
            $_REQUEST['act'] != 'forget_pwd' && $_REQUEST['act'] != 'reset_pwd' && $_REQUEST['act'] != 'check_order') {
            $admin_path = preg_replace('/:\d+/', '', $this->sanmu->stores_url()) . STORES_PATH; //重置路径
        ...
        if (!empty(session('store_user_id'))) {
            ...
            $store_user_info['store_user_img'] = $store_user_info && $store_user_info['store_user_img'] ? app(SanmuRepository::class)->getImagePath($store_user_info['store_user_img']) : '';
