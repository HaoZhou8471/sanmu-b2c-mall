...
class BargainController extends Controller
{
    ...
    public function detail(Request $request)
    {
        ...
        if (!empty($bs_id)) {
            ...
            if ($bargain_info) {
                ...
                $bargain_info['user_picture'] = $this->sanmuRepository->getImagePath($user_info->user_picture);
                ...
                $bargain_info['subtract_price'] = $this->sanmuRepository->getPriceFormat($bargain_info['subtract_price'], true); //砍掉价格
        ...
        $goodsInfo['goods_img'] = $this->sanmuRepository->getImagePath($goodsInfo['goods_img']);
        $goodsInfo['goods_thumb'] = $this->sanmuRepository->getImagePath($goodsInfo['goods_thumb']);
        $goodsInfo['shop_price'] = $this->sanmuRepository->getPriceFormat($goodsInfo['shop_price'], true);   //原价
        $goodsInfo['goods_price'] = $goodsInfo['shop_price'];
        $goodsInfo['target_price'] = $this->sanmuRepository->getPriceFormat($goodsInfo['target_price'], true);          //底价
        $goodsInfo['market_price_formated'] = $this->sanmuRepository->getPriceFormat($goodsInfo['market_price'], true);

        // 商品详情图 PC
        if (empty($goodsInfo['desc_mobile']) && !empty($goodsInfo['goods_desc'])) {
            $goodsInfo['goods_desc'] = $this->sanmuRepository->getContentImgReplace($goodsInfo['goods_desc']);
        }
        if (!empty($goodsInfo['desc_mobile'])) {
            // 处理手机端商品详情 图片（手机相册图） data/gallery_album/
            $goodsInfo['goods_desc'] = $this->sanmuRepository->getContentImgReplace($goodsInfo['desc_mobile'], 'goods_desc', 1);
        }
    ...
    public function property(Request $request)
    {
        ...
        $result['target_price'] = $this->sanmuRepository->getPriceFormat($result['target_price'], true);
        $result['goods_price'] = $this->sanmuRepository->getPriceFormat($result['goods_price'], true);
        //商品属性图片
    ...
    public function bid(Request $request)
    {
        ...
        if ($bargain['target_price'] == $bs_log['final_price']) {
            if (is_wechat_browser() && file_exists(MOBILE_WECHAT)) {
                ...
                $url = sanmu_url('/#/bargain/detail/' . $id);
