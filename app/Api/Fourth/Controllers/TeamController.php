...
class TeamController extends Controller
{
    ...
    public function detail(Request $request)
    {
        ...
        $goodsInfo['team_price'] = $this->sanmuRepository->getPriceFormat($goodsInfo['team_price'], true);
        $goodsInfo['shop_price'] = $this->sanmuRepository->getPriceFormat($goodsInfo['shop_price'], true);
        $goodsInfo['market_price'] = $this->sanmuRepository->getPriceFormat($goodsInfo['market_price'], true);
        /*获取商品规格参数*/
        ...
        if ($team_log) {
            foreach ($team_log as $key => $val) {
                ...
                $team_log[$key]['user_picture'] = $this->sanmuRepository->getImagePath($val['user_picture']);
    ...
    public function teamWait(Request $request)
    {
        ...
        if ($team_info) {
            ...
            foreach ($teamUser as $key => $val) {
                ...
                $teamUser[$key]['user_picture'] = $this->sanmuRepository->getImagePath($val['get_users']['user_picture']);
    ...
    public function teamUser(Request $request)
    {
        ...
        foreach ($teamUser as $key => $val) {
            ...
            $team_user[$key]['user_picture'] = $this->sanmuRepository->getImagePath($val['get_users']['user_picture']);
        }
            ...
            foreach ($order_list as $key => $val) {
                ...
                if (is_wechat_browser() && file_exists(MOBILE_WECHAT)) {
                    $pushData = [
                        ...
                        'keyword4' => ['value' => $this->sanmuRepository->getPriceFormat($order['team_price'], true), 'color' => '#173177']
                    ];
                    $url = sanmu_url('/#/user/orderDetail/' . $val['order_id']);
