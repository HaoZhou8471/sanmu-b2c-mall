...
class GroupBuyController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        foreach ($list as $key => $val) {
            $list[$key]['get_goods']['goods_img'] = $this->sanmuRepository->getImagePath($val['get_goods']['goods_img']);
            $list[$key]['get_goods']['goods_thumb'] = $this->sanmuRepository->getImagePath($val['get_goods']['goods_thumb']);
    ...
    public function detail(Request $request)
    {
        ...
        if ($list) {
            ...
            if (empty($list['goods']['desc_mobile']) && !empty($list['goods']['goods_desc'])) {
                $desc_preg = $this->sanmuRepository->descImagesPreg($list['goods']['goods_desc']);
                $list['goods']['goods_desc'] = $desc_preg['goods_desc'];
            }
            // 手机端详情
            if (!empty($list['goods']['desc_mobile'])) {
                $desc_preg = $this->sanmuRepository->descImagesPreg($list['goods']['desc_mobile'], 'desc_mobile', 1);
