...
class GoodsController extends Controller
{
    ...
    public function show(Request $request, CommonRepository $commonRepository)
    {
        ...
        if (file_exists(app_path('Custom/Distribute'))) {
            ...
            if ($data['is_distribution'] > 0 || $data['membership_card_id'] > 0) {
                ...
                $data['commission_money'] = empty($data['commission_money']) ? 0 : $this->sanmuRepository->getPriceFormat($data['commission_money']);
            }
        }
    ...
    public function FittingPrice(Request $request)
    {
        ...
        if (empty($goods)) {
            ...
            $res['spec_price_formated'] = $this->sanmuRepository->getPriceFormat($res['spec_price'], true);
            // 最终价格
            ...
            $res['goods_price_formated'] = $this->sanmuRepository->getPriceFormat($res['result'], true);
        }
        ...
        if ($attr_id) {
            ...
            if (!empty($attr_img['attr_img_flie'])) {
                $res['attr_img'] = $this->sanmuRepository->getImagePath($attr_img['attr_img_flie']);
            }
