...
class DistributionController extends Controller
{
        ...
        if (!empty($card_receive_value)) {
            foreach ($card_receive_value as $value) {
                ...
                if ($receive_type == 'integral') {
                    ...
                    if (!empty($value)) {
                        $content['buy_money'] = $this->sanmuRepository->getPriceFormat($value, true);
                    }
                    ...
                    if (!empty($value)) {
                        $content['price'] = $this->sanmuRepository->getPriceFormat($value, true);
                    }
    ...
    public function purchasepay(Request $request, RightsCardService $rightsCardService)
    {
        ...
        if ($buy_money > 0) {
            ...
            $result['order_amount_formated'] = $this->sanmuRepository->getPriceFormat($buy_money, true);
    ...
    public function wxapppurchasepay(Request $request, RightsCardService $rightsCardService)
    {
        ...
        $order['order_amount_formated'] = $this->sanmuRepository->getPriceFormat($order['order_amount'], true);
            ...
            foreach ($order as $key => $value) {
                ...
                if (isset($value['get_order']) && !empty($value['get_order'])) {
                    ...
                    if (!empty($order_goods)) {
                        foreach ($order_goods as $k => $val) {
                            ...
                            $goods_list[$k]['goods_price_format'] = $this->sanmuRepository->getPriceFormat($val['goods_price'], true);
                            $goods_list[$k]['goods_number'] = $val['goods_number'];
                            $goods_list[$k]['goods_name'] = $val['goods_name'];
                            $goods_list[$k]['goods_thumb'] = $this->sanmuRepository->getImagePath($val['goods_thumb']);
                        }
                    }
                }
                // 付费购买
                if (isset($value['get_drp_account_log']) && !empty($value['get_drp_account_log'])) {
                    ...
                    if (!empty($membership_card)) {
                        ...
                        $goods_list[0]['goods_price_format'] = $this->sanmuRepository->getPriceFormat($value['amount'], true);
                        $goods_list[0]['goods_number'] = 1;
                        $goods_list[0]['goods_name'] = $membership_card['name'];
                        $goods_list[0]['goods_thumb'] = empty($membership_card['background_img']) ? asset('img/membership_card_default.png') : $this->sanmuRepository->getImagePath($membership_card['background_img']);
                    }
                }

                if (isset($this->config['show_mobile']) && $this->config['show_mobile'] == 0) {
                    $result[$key]['buy_user_name'] = $this->sanmuRepository->stringToStar($result[$key]['buy_user_name']);
                }
                ...
                $result[$key]['money_format'] = $this->sanmuRepository->getPriceFormat($value['money'], true);
        ...
        if (!empty($order)) {
            ...
            if (isset($value['get_drp_account_log']) && !empty($value['get_drp_account_log'])) {
                ...
                if (!empty($membership_card)) {
                    ...
                    $goods_list[0]['goods_number'] = 1;
                    $goods_list[0]['goods_name'] = $membership_card['name'];
                    $goods_list[0]['goods_thumb'] = $this->sanmuRepository->getImagePath($membership_card['background_img']);
                }
            }

            if (isset($this->config['show_mobile']) && $this->config['show_mobile'] == 0) {
                $result['buy_user_name'] = $this->sanmuRepository->stringToStar($result['buy_user_name']);
            }
            ...
            $result['money_format'] = $this->sanmuRepository->getPriceFormat($value['money'], true);
...
//            'user_id' => 'required|integer',
        ...
        if ($teaminfo) {
            foreach ($teaminfo as $key => $value) {
                ...
                if (isset($value['get_drp_shop']['shop_portrait']) && $value['get_drp_shop']['shop_portrait']) {
                    $team_info[$key]['user_picture'] = $this->sanmuRepository->getImagePath($value['get_drp_shop']['shop_portrait']);
                } else {
                    $team_info[$key]['user_picture'] = $this->sanmuRepository->getImagePath($value['user_picture']);
                }
            }
        }
    ...
    public function teamDetail(Request $request)
    {
        ...
        if (isset($info['get_drp_shop']['shop_portrait']) && $info['get_drp_shop']['shop_portrait']) {
            $info['user_picture'] = $this->sanmuRepository->getImagePath($info['get_drp_shop']['shop_portrait']);
        } else {
            $info['user_picture'] = $this->sanmuRepository->getImagePath($info['user_picture']);
        }
    ...
    public function offlineUser(Request $request)
    {
        ...
        if ($user_list) {
            foreach ($user_list as $key => $value) {
                ...
                $user_list[$key]['user_picture'] = $this->sanmuRepository->getImagePath($value['user_picture']);
            }
        }
