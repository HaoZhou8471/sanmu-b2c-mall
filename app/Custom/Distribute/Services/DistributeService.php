        ...
        if (is_string($goods_ids)) {
            ...
            if ($list) {
                foreach ($list as $k => $goods) {
                    $list[$k]['goods_thumb'] = empty($goods['goods_thumb']) ? '' : $this->sanmuRepository->getImagePath($goods['goods_thumb']);
                    ...
                    $list[$k]['shop_price_formated'] = $this->sanmuRepository->getPriceFormat($price['shop_price']);
        ...
        if (!empty($result['list'])) {
            ...
            foreach ($result['list'] as $key => $val) {
                ...
                if (isset($val['drp_log']) && !empty($val['drp_log'])) {
                    $val['drp_log']['money_format'] = $this->sanmuRepository->getPriceFormat($val['drp_log']['money']);
