...
class GoodsRepository
{
    ...
    public function goods_info($goods_id = 0)
    {
        ...
        if ($info) {
            ...
            $info['goods_video_path'] = $this->sanmuRepository->getImagePath($info['goods_video']);
            $info['shop_name'] = app(MerchantCommonService::class)->getShopName($info['user_id'], 1); //店铺名称

            $info['goods_thumb'] = empty($info['goods_thumb']) ? '' : $this->sanmuRepository->getImagePath($info['goods_thumb']);
        }
