...
class CommissionServer extends Command
{
    ...
    public function commissionBillList($seller_id = 0)
    {
        ...
        $this->sanmuRepository->writeStaticCache('', $cache_name, $row, 'commission/bill/');
        ...
        if ($row) {
            ...
            for ($page = 1; $countpage >= $page; $page++) {
                $list = $this->sanmuRepository->readStaticCache('', $cache_name, 'commission/bill/');

                $arr = $this->sanmuRepository->pageArray($page_size, $page, $list);
