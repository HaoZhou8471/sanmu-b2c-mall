...
class TimeoutServer extends Command
{
    ...
    private function timeoutOrderPay($pay_effective_time = 0)
    {
        if ($pay_effective_time > 0) {
            ...
            $this->sanmuRepository->writeStaticCache('', $cache_name_file, $list, 'user_order_timeout/');

            if ($list) {

                for ($page = 1; $countpage >= $page; $page++) {

                    $list = $this->sanmuRepository->readStaticCache('', $cache_name_file, 'user_order_timeout/');
