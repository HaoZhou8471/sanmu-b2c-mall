...
function get_suppliers_info($suppliers_id = 0, $select = array())
{
    ...
    $sql = "SELECT $select FROM " . $GLOBALS['sanmu']->table('suppliers') . " WHERE suppliers_id = '$suppliers_id' LIMIT 1";
...
function log_suppliers_account_change($suppliers_id, $suppliers_money = 0, $frozen_money = 0)
{
    if ($suppliers_money || $frozen_money) {
        /* 更新用户信息 */
        $sql = "UPDATE " . $GLOBALS['sanmu']->table('suppliers') .
...
function get_suppliers_account_log()
{
    ...
    if ($result === false) {
        ...
        $sql = "SELECT count(*) FROM " . $GLOBALS['sanmu']->table('suppliers_account_log') . " AS sal " .
            " LEFT JOIN " . $GLOBALS['sanmu']->table('suppliers') . " AS s ON sal.user_id = s.suppliers_id " .
            " $ex_where";
        ...
        $sql = "SELECT sal.*, s.suppliers_name FROM " . $GLOBALS['sanmu']->table('suppliers_account_log') . " AS sal " .
            " LEFT JOIN " . $GLOBALS['sanmu']->table('suppliers') . " AS s ON sal.user_id = s.suppliers_id " .
            " $ex_where " .
...
function get_suppliers_account_log_detail($suppliers_id, $type = 0)
{
    ...
    if ($result === false) {
        ...
        if ($filter['order_sn']) {
            ...
            $ex_where .= " OR ";
            $ex_where .= " (SELECT order_sn FROM " . $GLOBALS['sanmu']->table("order_info") . " AS oi WHERE sal.order_id = oi.order_id LIMIT 1) = '" . $filter['order_sn'] . "')";
        }
        ...
        $sql = "SELECT count(*) FROM " . $GLOBALS['sanmu']->table('suppliers_account_log_detail') . " AS sal " .
            " LEFT JOIN " . $GLOBALS['sanmu']->table('suppliers') . " AS s ON sal.suppliers_id = s.suppliers_id " .
        ...
        $sql = "SELECT sal.*, s.suppliers_name FROM " . $GLOBALS['sanmu']->table('suppliers_account_log_detail') . " AS sal " .
            " LEFT JOIN " . $GLOBALS['sanmu']->table('suppliers') . " AS s ON sal.suppliers_id = s.suppliers_id " .
            " $ex_where AND sal.log_type IN($type)" .
            " ORDER BY " . $filter['sort_by'] . ' ' . $filter['sort_order'] .
            " LIMIT " . $filter['start'] . ',' . $filter['page_size'];
...
function get_suppliers_statistical_data($start_date = 0, $end_date = 0, $type = 'order', $suppliers_id = 0)
{
    ...
    $where_date = '';
    //获取系统数据 start
    $no_main_order = " AND (SELECT count(*) FROM " . $GLOBALS['sanmu']->table('wholesale_order_info') . " AS oi2 WHERE oi2.main_order_id = oi.order_id) = 0 "; //主订单下有子订单时，则主订单不显示
    if ($suppliers_id > 0) {
    ...
    $sql = 'SELECT DATE_FORMAT(FROM_UNIXTIME(oi.add_time + ' . $time_diff . '),"%y-%m-%d") AS day,COUNT(*) AS count,SUM(oi.order_amount) AS money FROM ' .
        $GLOBALS['sanmu']->table('wholesale_order_info') . " AS oi" . ' WHERE oi.pay_status =2 AND oi.order_status = 1 AND  oi.add_time BETWEEN ' . $date_start .
...
{
    $sql = "SELECT * FROM " . $GLOBALS['sanmu']->table('suppliers_account_log_detail') . " WHERE log_id = '$log_id' LIMIT 1";
