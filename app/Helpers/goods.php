    ...
    if ($sql || $parent_id == 0) {
        ...
        foreach ($res as $row) {
            if ($row['is_show']) {
                ...
                $cat_arr[$row['cat_id']]['url'] = app(SanmuRepository::class)->buildUri('category', ['cid' => $row['cat_id']], $row['cat_name']);

                if (isset($row['cat_id']) != null) {
                    $cat_arr[$row['cat_id']]['cat_id'] = app(CategoryService::class)->getChildTree($row['cat_id']);
                }
            }
        }
    }
...
function get_top10($cats = 0, $presale = '', $ru_id = 0, $warehouse_id = 0, $area_id = 0, $area_city = 0)
{
    ...
    $arr = app(SanmuRepository::class)->getAreaLinkGoods($arr, $area_id, $area_city);
    ...
    if ($arr) {
        foreach ($arr as $key => $row) {
            ...
            $arr[$key]['short_name'] = $GLOBALS['_CFG']['goods_name_length'] > 0 ?
                app(SanmuRepository::class)->subStr($row['goods_name'], $GLOBALS['_CFG']['goods_name_length']) : $row['goods_name'];
...
function assign_cat_goods($cat_id, $num = 0, $from = 'web', $order_rule = '', $return = 'cat', $warehouse_id = 0, $area_id = 0, $area_city = 0, $floor_sort_order = 0)
{
    ...
    $cat['url'] = app(SanmuRepository::class)->buildUri('category', ['cid' => $cat_id], $cat['name']);
    $cat['id'] = $cat_id;
    ...
    if ($res) {
        ...
        foreach ($res as $key => $value) {
            if ($key == 0) {
                ...
                $goods_res = app(SanmuRepository::class)->getAreaLinkGoods($goods_res, $area_id, $area_city);

                $areaCookie = app(AreaService::class)->areaCookie();
                $goods_res = app(SanmuRepository::class)->getWhereRsid($goods_res, 'user_id', 0, $areaCookie['city']);
                ...
                if ($goods_res) {
                    foreach ($goods_res as $idx => $row) {
                        ...
                        $goods_res[$idx]['shop_price'] = price_format($row['shop_price']);
                        $goods_res[$idx]['short_name'] = $GLOBALS['_CFG']['goods_name_length'] > 0 ? app(SanmuRepository::class)->subStr($row['goods_name'], $GLOBALS['_CFG']['goods_name_length']) : $row['goods_name'];
                        $goods_res[$idx]['url'] = app(SanmuRepository::class)->buildUri('goods', ['gid' => $row['goods_id']], $row['goods_name']);
        ...
        if ($res) {
            foreach ($res as $row) {
                ...
                if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                    $row['user_name'] = app(SanmuRepository::class)->stringToStar($row['user_name']);
                }
    ...
    if ($res) {
        ...
        foreach ($res as $row) {
            ...
            $arr[$temp_index]['parent_short_name'] = $GLOBALS['_CFG']['goods_name_length'] > 0 ?
                app(SanmuRepository::class)->subStr($row['parent_name'], $GLOBALS['_CFG']['goods_name_length']) : $row['parent_name']; //配件的基本件显示的名称
            ...
            $arr[$temp_index]['short_name'] = $GLOBALS['_CFG']['goods_name_length'] > 0 ?
                app(SanmuRepository::class)->subStr($row['goods_name'], $GLOBALS['_CFG']['goods_name_length']) : $row['goods_name']; //配件显示的名称
            ...
            $arr[$temp_index]['url'] = app(SanmuRepository::class)->buildUri('goods', ['gid' => $row['goods_id']], $row['goods_name']);
...
function get_goods_fittings_info($goods_id = 0, $warehouse_id = 0, $area_id = 0, $area_city = 0, $rev = '', $type = 0, $fittings_goods = 0, $fittings_attr = [], $user_id = 0, $userrank = [])
{
    ...
    $res = app(SanmuRepository::class)->getAreaLinkGoods($res, $area_id, $area_city);
    ...
    if ($res) {
        ...
        foreach ($res as $row) {
            ...
            $arr[$temp_index]['parent_short_name'] = $GLOBALS['_CFG']['goods_name_length'] > 0 ? app(SanmuRepository::class)->subStr($row['parent_name'], $GLOBALS['_CFG']['goods_name_length']) : $row['parent_name']; //配件的基本件显示的名称
            $arr[$temp_index]['goods_id'] = $row['goods_id']; //配件的商品ID
            $arr[$temp_index]['goods_name'] = $row['goods_name']; //配件的名称
            $arr[$temp_index]['comments_number'] = isset($row['comments_number']) ? $row['comments_number'] : 0;
            $arr[$temp_index]['sales_volume'] = $row['sales_volume'];
            $arr[$temp_index]['short_name'] = $GLOBALS['_CFG']['goods_name_length'] > 0 ? app(SanmuRepository::class)->subStr($row['goods_name'], $GLOBALS['_CFG']['goods_name_length']) : $row['goods_name']; //配件显示的名称
...
function seckill_info($seckill_id, $current_num = 0, $path = '')
{
    ...
    if ($GLOBALS['_CFG']['open_oss'] == 1) {
        $bucket_info = app(SanmuRepository::class)->getBucketInfo();
...
function seckill_goods_results($date = '', $cat_id = 0, $day = 0)
{
    ...
    if ($res) {
        foreach ($res as $k => $v) {
            ...
            if ($day > 0) {
                $res[$k]['url'] = app(SanmuRepository::class)->buildUri('seckill', ['act' => "view", 'secid' => $v['id'], 'tmr' => 1], $v['goods_name']);
            } else {
                $res[$k]['url'] = app(SanmuRepository::class)->buildUri('seckill', ['act' => "view", 'secid' => $v['id']], $v['goods_name']);
            }
