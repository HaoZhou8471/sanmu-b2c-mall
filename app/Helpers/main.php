        ...
        if (in_array($filename, ['category', 'goods', 'category_discuss', 'article_cat', 'article', 'brand', 'single_sun', 'store_street', 'presale', 'group_buy', 'exchange', 'seckill', 'snatch', 'wholesale_goods'])) {
            ...
            if (!empty($cat_arr)) {
                ...
                foreach ($cat_arr as $kval => $val) {
                    ...
                    if ($type == 'article_cat') {
                        $ur_here .= '<span class="arrow">></span>' . '<span> <a href="' . app(SanmuRepository::class)->buildUri($type, $args, $val['cat_name']) . '">' . htmlspecialchars($val['cat_name']) . '</a>' . '</span>';
                    }

                    if (!(isset($val['parent_id']) && $val['parent_id'] == 0)) {
                        $ur_here .= '<span class="arrow">></span>'; //by wang
                        $ur_here .= '<span class="breadcrumb-item ziji">';
                        $ur_here .= '<span class="filter-tag"><a href="' . app(SanmuRepository::class)->buildUri($type, $args, $val['cat_name']) . '">' . $val['cat_name'] . '</a><i class="sc-icon-right"></i></span>';
                        $cat_arr[$kval]['url'] = app(SanmuRepository::class)->buildUri($type, $args, $val['cat_name']); //url by wu
                        if (isset($val['cat_tree']) && $val['cat_tree']) {
                            ...
                            foreach ($val['cat_tree'] as $ckey => $crow) {
                                $ur_here .= '<li><a href="' . app(SanmuRepository::class)->buildUri($type, ['cid' => $crow['cat_id']], $crow['cat_name']) . '" title="' . $crow['cat_name'] . '">' . $crow['cat_name'] . '</a></li>';
                                $cat_arr[$kval]['cat_tree'][$ckey]['url'] = app(SanmuRepository::class)->buildUri($type, ['cid' => $crow['cat_id']], $crow['cat_name']); //url by wu
                            }
                            $ur_here .= "</ul>";
                            $ur_here .= '</div></div>';
                        }
...
)
{
    ...
    if ($pager['styleid'] == 0) {
        if (!empty($url_format)) {
            ...
            $pager['page_first'] = app(SanmuRepository::class)->buildUri($app, $uri_args, '', 1, $keywords);
            $pager['page_prev'] = app(SanmuRepository::class)->buildUri($app, $uri_args, '', $page_prev, $keywords);
            $pager['page_next'] = app(SanmuRepository::class)->buildUri($app, $uri_args, '', $page_next, $keywords);
            $pager['page_last'] = app(SanmuRepository::class)->buildUri($app, $uri_args, '', $page_count, $keywords);
        }
        $pager['array'] = [];
        ...
        if (!empty($url_format)) {
            ...
            $pager['page_first'] = ($page - $_offset > 1 && $_pagenum < $page_count) ? app(SanmuRepository::class)->buildUri($app, $uri_args, '', 1, $keywords) : '';
            $pager['page_prev'] = ($page > 1) ? app(SanmuRepository::class)->buildUri($app, $uri_args, '', $page_prev, $keywords) : '';
            $pager['page_next'] = ($page < $page_count) ? app(SanmuRepository::class)->buildUri($app, $uri_args, '', $page_next, $keywords) : '';
            $pager['page_last'] = ($_to < $page_count) ? app(SanmuRepository::class)->buildUri($app, $uri_args, '', $page_count, $keywords) : '';
            $pager['page_kbd'] = ($_pagenum < $page_count) ? true : false;
            $pager['page_number'] = [];
            for ($i = $_from; $i <= $_to; ++$i) {
                $pager['page_number'][$i] = app(SanmuRepository::class)->buildUri($app, $uri_args, '', $i, $keywords);
            }
        }
    }
...
function visit_stats()
{
    ...
    $os = get_os();
    $ip = app(SanmuRepository::class)->sanmuIp();
...
function save_searchengine_keyword($domain, $path)
{
    ...
    if (!empty($keywords)) {
        ...
        if (EC_CHARSET == 'utf-8' && in_array($searchengine, $gb_search)) {
            $keywords = sanmu_iconv('GBK', 'UTF8', $keywords);
        }
        if (EC_CHARSET == 'gbk' && !in_array($searchengine, $gb_search)) {
            $keywords = sanmu_iconv('UTF8', 'GBK', $keywords);
        }
...
/*   $get_categories_tree = get_categories_tree();
     $GLOBALS['smarty']->assign('get_categories_tree', $get_categories_tree); */ 
     //自定义左侧菜单
    ...
    $urlHtml = app(SanmuRepository::class)->getUrlHtml($urlHtmlKey);
    ...
    $GLOBALS['smarty']->assign('url_presale_new', app(SanmuRepository::class)->buildUri('presale', ['act' => 'new']));
    $GLOBALS['smarty']->assign('url_business_buy', app(SanmuRepository::class)->buildUri('wholesale', ['act' => 'buy']));
    $GLOBALS['smarty']->assign('url_presale_advance', app(SanmuRepository::class)->buildUri('presale', ['act' => 'advance']));
