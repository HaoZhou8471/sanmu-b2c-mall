...
function order_info($order_id, $order_sn = '', $seller_id = -1)
{
    ...
    if ($order) {
        ...
        if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
            $order['mobile'] = app(SanmuRepository::class)->stringToStar($order['mobile']);
            $order['tel'] = app(SanmuRepository::class)->stringToStar($order['tel']);
        }
    ...
    if ($res) {
        foreach ($res as $row) {
            ...
            if (isset($goods['get_goods_consumption']) && $goods['get_goods_consumption']) {
                $row['amount'] = app(SanmuRepository::class)->getGoodsConsumptionPrice($goods['get_goods_consumption'], $row['goods_amount']);
            } else {
            ...
            if ($row['order_extension_code'] == "presale" && !empty($row['extension_id'])) {
                $row['url'] = app(SanmuRepository::class)->buildUri('presale', ['act' => 'view', 'presaleid' => $row['extension_id']], $row['goods_name']);
            } elseif ($row['order_extension_code'] == "group_buy") {
                $row['url'] = app(SanmuRepository::class)->buildUri('group_buy', ['gbid' => $row['extension_id']]);
            } elseif ($row['order_extension_code'] == "snatch") {
                $row['url'] = app(SanmuRepository::class)->buildUri('snatch', ['sid' => $row['extension_id']]);
            } elseif (substr($row['extension_code'], 0, 7) == "seckill") {
                $row['url'] = app(SanmuRepository::class)->buildUri('seckill', ['act' => "view", 'secid' => $row['extension_id']]);
            } elseif ($row['order_extension_code'] == "auction") {
                $row['url'] = app(SanmuRepository::class)->buildUri('auction', ['auid' => $row['extension_id']]);
            } elseif ($row['order_extension_code'] == "exchange_goods") {
                $row['url'] = app(SanmuRepository::class)->buildUri('exchange_goods', ['gid' => $row['extension_id']]);
            } else {
                $row['url'] = app(SanmuRepository::class)->buildUri('goods', ['gid' => $row['goods_id']], $row['goods_name']);
            }

            $row['shop_name'] = app(MerchantCommonService::class)->getShopName($row['ru_id'], 1); //店铺名称
            $row['shopUrl'] = app(SanmuRepository::class)->buildUri('merchants_store', ['urid' => $row['ru_id']]);
...
function order_fee($order, $goods, $consignee, $type = 0, $cart_value = '', $pay_type = 0, $cart_goods_list = '', $warehouse_id = 0, $area_id = 0, $store_id = 0, $store_type = '', $user_id = 0, $rank_id = 0, $rec_type = CART_GENERAL_GOODS)
{
    ...
    if ($total['amount'] > 0 && $max_amount > 0 && $order['integral'] > 0) {
        $integral_money = app(SanmuRepository::class)->valueOfIntegral($order['integral']);

        // 使用积分支付
        $use_integral = min($total['amount'], $max_amount, $integral_money); // 实际使用积分支付的金额
        $total['amount'] -= $use_integral;
        $total['integral_money'] = $use_integral;
        $order['integral'] = app(SanmuRepository::class)->integralOfValue($use_integral);
    } else {
            ...
            foreach ($cart_goods as $key => $row) {
                ...
                if ($shipping) {
                    if (!empty($step) && in_array($step, $step_array)) {
                        ...
                        $str_shipping = app(SanmuRepository::class)->delStrComma($str_shipping);
...
function update_order($order_id = 0, $order = [])
{
    ...
    $old_order_amount = app(SanmuRepository::class)->changeFloat($old_order_amount);

    $request_surplus = isset($order['request_surplus']) ? floatval($order['request_surplus']) : 0;
    $request_surplus = app(SanmuRepository::class)->changeFloat($request_surplus);
    ...
    $new_amount = app(SanmuRepository::class)->changeFloat($new_amount);
        ...
        $arr = $arr->where(function ($query) use($where){
            $query->where(function ($query) use($where){
                $query->whereHas('getGoods', function ($query) use ($where) {
                    app(SanmuRepository::class)->getAreaLinkGoods($query, $where['area_id'], $where['area_city']);
                });
    ...
    if ($arr) {
        ...
        foreach ($arr as $key => $value) {
            ...
            if ($value['extension_code'] == 'package_buy') {
                ...
                if ($activity) {
                    $arr[$key]['goods_thumb'] = !empty($activity['activity_thumb']) ? get_image_path($activity['activity_thumb']) : app(SanmuRepository::class)->sanmuUrl('themes/ecmoban_sanmu2017/images/17184624079016pa.jpg');
                }
                ...
                //ecmoban模板堂 --zhuo start 商品金额促销
                if (isset($goods['get_goods_consumption']) && $goods['get_goods_consumption']) {
                    $value['amount'] = app(SanmuRepository::class)->getGoodsConsumptionPrice($goods['get_goods_consumption'], $value['subtotal']);
                } else {
                ...
                if ($value['rec_type'] == 1) {
                    ...
                    if ($group_buy) {
                        $arr[$key]['url'] = app(SanmuRepository::class)->buildUri('group_buy', ['gbid' => $group_buy['act_id']]);
                    ...
                    if ($presale) {
                        ...
                        $arr[$key]['url'] = app(SanmuRepository::class)->buildUri('presale', ['act' => 'view', 'presaleid' => $presale['act_id']], $presale['act_name']);
                    }
                } elseif ($value['rec_type'] == 4) {
                    $arr[$key]['url'] = app(SanmuRepository::class)->buildUri('exchange_goods', ['gid' => $value['goods_id']], $value['goods_name']);
                } else {
                    $arr[$key]['url'] = app(SanmuRepository::class)->buildUri('goods', ['gid' => $value['goods_id']], $value['goods_name']);
                }
...
function cloud_confirmorder($order_id)
{
    if ($order_id > 0) {
        ...
        if ($cloud_order) {
            ...
            $cloud_sanmu_appkey = ShopConfig::where('code', 'cloud_sanmu_appkey')->value('value');
            $cloud_order['notifyUrl'] = $GLOBALS['sanmu']->url() . "api.php?app_key=" . $cloud_sanmu_appkey . "&method=sanmu.order.confirmorder.post&format=json&interface_type=1";
...
function bonus_info($bonus_id, $bonus_psd = '', $cart_value = 0)
{
    ...
    if ($cart_value != 0 || !empty($cart_value)) {
        ...
        if (!empty($goods_user)) {
            ...
            $goods_user = app(SanmuRepository::class)->delStrComma($goods_user);
        ...
        foreach ($res as $key => $row) {
            ...
            if ($row['extension_code'] == 'package_buy') {
                ...
                if ($activity) {
                    ...
                    $row['goods_thumb'] = !empty($activity['activity_thumb']) ? get_image_path($activity['activity_thumb']) : app(SanmuRepository::class)->sanmuUrl('themes/ecmoban_sanmu2017/images/17184624079016pa.jpg');
                } else {
                    //移除无效超值礼包
                    Cart::where([
...
function get_cart_ru_goods_list($goods_list = [], $cart_value = '', $consignee = '', $store_id = 0, $user_id = 0)
{
    ...
    foreach ($goods_list as $key => $row) {
        ...
        $arr[$key]['url'] = app(SanmuRepository::class)->buildUri('merchants_store', ['urid' => $key], $ru_name);
        ...
        if ($key == 0) {
            ...
            if ($kf_im_switch) {
                $arr[$key]['is_sanmu'] = true;
            } else {
                $arr[$key]['is_sanmu'] = false;
            }
        } else {
            $arr[$key]['is_sanmu'] = false;
        }
        ...
        $chat = app(SanmuRepository::class)->chatQq($basic_info);
...
function get_ru_shippng_info($cart_goods, $cart_value, $ru_id, $consignee = '', $user_id = 0)
{
    ...
    if ($freight) {
        $freight = app(SanmuRepository::class)->delStrComma($freight);
    }
...
function get_virtual_goods_info($rec_id = 0)
{
    ...
    if ($virtual_info) {
        foreach ($virtual_info as $row) {
            $res['card_sn'] = sanmu_decrypt($row['card_sn']);
            $res['card_password'] = sanmu_decrypt($row['card_password']);
...
function merge_order($from_order_sn_arr, $to_order_sn)
{
    ...
    foreach ($from_order_sn_arr as $key => $from_order_sn) {
        ...
        $order['integral_money'] = app(SanmuRepository::class)->valueOfIntegral($order['integral']);
...
function return_order_info($ret_id = 0, $order_sn = '', $order_id = 0)
{
    ...
    if ($order) {
        ...
        $order['shop_url'] = app(SanmuRepository::class)->buildUri('merchants_store', ['urid' => $ru_id], $order['shop_name']);
...
function rec_goods($rec_id)
{
    ...
    $res['url'] = app(SanmuRepository::class)->buildUri('goods', ['gid' => $res['goods_id']], $res['goods_name']);
...
function favourable_list($user_rank, $user_id = -1, $fav_id = 0, $act_sel_id = [], $ru_id = -1, $uid = 0)
{
    ...
    $res = FavourableActivity::where('start_time', '<=', $now)
        ->where('end_time', '>=', $now);
...
function get_favourable_info($goods_id = 0, $ru_id = 0, $goods = [], $rank_id = 0)
{
    ...
    $res = FavourableActivity::where('start_time', '<=', $gmtime)
