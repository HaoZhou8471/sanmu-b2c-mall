...
function get_promotion_info($goods_id = '', $ru_id = 0, $goods = [])
{
    ...
    $gmtime = gmtime();
    $res = GoodsActivity::where('is_finished', 0)
...
function cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
    ...
    if ($res === NULL)
    {
        ...
        if ($data === false)
        {
            $sql = "SELECT c.cat_id, c.cat_name, c.measure_unit, c.parent_id, c.is_show, c.show_in_nav, c.grade, c.sort_order, COUNT(s.cat_id) AS has_children ".
                'FROM ' . app(Shop::class)->table('category') . " AS c ".
                "LEFT JOIN " . app(Shop::class)->table('category') . " AS s ON s.parent_id=c.cat_id ".
                "where c.is_virtual= '0' ".
                "GROUP BY c.cat_id ".
                'ORDER BY c.parent_id, c.sort_order ASC';
            $res = app(Mysql::class)->getAll($sql);

            $sql = "SELECT cat_id, COUNT(*) AS goods_num " .
                    " FROM " . app(Shop::class)->table('goods') .
                    " WHERE is_delete = 0  " .
                    " GROUP BY cat_id";
            $res2 = app(Mysql::class)->getAll($sql);

            $sql = "SELECT gc.cat_id, COUNT(*) AS goods_num " .
                    " FROM " . app(Shop::class)->table('goods_cat') . " AS gc , " . app(Shop::class)->table('goods') . " AS g " .
                    " WHERE g.goods_id = gc.goods_id AND g.is_delete = 0  " .
                    " GROUP BY gc.cat_id";
            $res3 = app(Mysql::class)->getAll($sql);
    ...
    else
    {
        foreach ($options AS $key => $value)
        {
            $options[$key]['url'] = app(DscRepository::class)->buildUri('category', array('cid' => $value['cat_id']), $value['cat_name']);
        }
 ...
 * 查询店铺分类
 */
function get_fine_store_category($options, $web_type, $array_type = 0, $ru_id = 0)
{
    $cat_array = [];
    ...
    if (isset($ecsCookie['list_history']) && !empty($ecsCookie['list_history'])) {
        ...
        if ($res) {
            foreach ($res as $row) {
                ...
                $app(DscRepository::class)->buildUri = [
                    'urid' => $row['user_id'],
                    'append' => $arr[$row['goods_id']]['rz_shopName'],
                ];

                $domain_url = app(MerchantCommonService::class)->getSellerDomainUrl($row['user_id'], $app(DscRepository::class)->buildUri);
...
{
    $basic_info = SellerShopinfo::where('user_id', $user_id);
...
function get_category_leve_one($cat_id = 0)
{
    if ($cat_id > 0)
    {
    ...
    */
     ...
     $res = Category::selectRaw('COUNT(*)')
           ->where('parent_id', $parent_id)
           ->where('is_show', 1);
    if (app(BaseRepository::class)->getToArrayFirst($res)|| $parent_id == 0)
    {
        /* 获取当前分类及其子分类 */
      $res = Category::select('cat_id', 'cat_name', 'parent_id', 'is_show')
        ...
        foreach ($res AS $row)
        {
            if ($row['is_show'])
            {
                ...
                $cat_arr[$row['cat_id']]['url']  = app(DscRepository::class)->buildUri('category', array('cid' => $row['cat_id']), $row['cat_name']);
...
function get_goods_activity_info($act_id = 0, $select = [])
{
    ...
    if ($activity) {
        $activity['goods_thumb'] = !empty($activity['activity_thumb']) ? get_image_path($activity['activity_thumb']) : app(DscRepository::class)->dscUrl('themes/sanmubuy/images/17184624079016pa.jpg');
    }
...
{
    $res = Category::select('keywords', 'sort_order', 'cat_desc')->where('cat_id', $cat_id);
    $res = app(BaseRepository::class)->getToArrayFirst($res);
