...
function sanmu_iconv($source_lang, $target_lang, $source_string = '')
{
    static $chs = null;
...
function json_str_iconv($str)
{
    if (EC_CHARSET != 'utf-8') {
        if (is_string($str)) {
            return addslashes(stripslashes(sanmu_iconv('utf-8', EC_CHARSET, $str)));
        } elseif (is_array($str)) {
...
function to_utf8_iconv($str)
{
    if (EC_CHARSET != 'utf-8') {
        if (is_string($str)) {
            return sanmu_iconv(EC_CHARSET, 'utf-8', $str);
        } elseif (is_array($str)) {
            foreach ($str as $key => $value) {
...
function write_static_file_cache($cache_name = '', $caches = '', $suffix = '', $path = '', $type = 0)
{
    ...
    if ($sel_config['open_memcached'] == 1 && $type == 0) {
        ...
        if ($is_oss == 1 && $server_model) {
            app(SanmuRepository::class)->getOssAddFile([$cache_file_path]);
        }
 ...
 * 删除文件
 */
function sanmu_unlink($file = '', $path = '')
{
    if ($file) {
        if (is_array($file)) {
...
/* 重返序列化 */
function sanmu_unserialize($serial_str)
{
...
function get_go_index($type = 0, $var = false)
{
    if ($type == 1) {
        if (!$var) {
            return sanmu_header("Location: " . $GLOBALS['sanmu']->url() . "\n");
        }
    } else {
...
function get_sanmu_token()
{
    ...
    $sanmu_token = MD5($sc_guid . "-" . $sc_rand);
    ...
    if ($cache) {
        if (isset($cache['category']) && isset($cache['category']['cache_path']) && !empty($cache['category']['cache_path'])) {
            if (isset($cache['category']['type']) && $cache['category']['type'] == 'add_edit') {
                 ...
                 * start
                 */
                sanmu_unlink(storage_path($cache['category']['cache_path'] . "category_tree_leve_one0.php"));
