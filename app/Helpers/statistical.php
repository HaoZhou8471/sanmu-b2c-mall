...
function shop_total_stats()
{
    ...
    /* 关联查询 */
    $leftJoin = " LEFT JOIN " . $GLOBALS['sanmu']->table('order_return') . " AS re ON re.order_id = o.order_id ";
    $leftJoin .= " LEFT JOIN " . $GLOBALS['sanmu']->table('merchants_shop_information') . " AS msi ON msi.user_id = o.ru_id ";
...
function get_statistical_new_user($search_data = array())
{
    ...
    //获取系统数据 start
    $sql = "SELECT FROM_UNIXTIME(u.reg_time+$time_diff,'%y-%m-%d') AS day, COUNT(*) AS count FROM " . $GLOBALS['sanmu']->table('users') . " AS u" . ' WHERE u.reg_time BETWEEN ' . $date_start . ' AND ' . $date_end . $where_data . ' GROUP BY day ORDER BY day ASC ';
...
function get_statistical_sale($search_data = array())
{
    ...
    $sql = " SELECT FROM_UNIXTIME(o.add_time+$time_diff,'%y-%m-%d') AS day, " .
        ...
        " FROM " . $GLOBALS['sanmu']->table('order_info') . " AS o" . ' WHERE o.main_count = 0 AND o.add_time BETWEEN ' . $date_start . ' AND ' . $date_end . $where_data . ' GROUP BY day ORDER BY day ASC ';
    $result = $GLOBALS['db']->getAll($sql);
...
function get_statistical_user_rank()
{
    ...
    $sql = " SELECT rank_id, rank_name, special_rank FROM " . $GLOBALS['sanmu']->table('user_rank');
...
function industry_analysis($page = 0)
{
    ...
    if ($result === false) {
        ...
        /* 关联查询 */
        $leftJoin = '';
        $leftJoin .= " LEFT JOIN " . $GLOBALS['sanmu']->table('goods') . " AS g ON g.cat_id = c.cat_id ";
        $leftJoin .= " LEFT JOIN " . $GLOBALS['sanmu']->table('order_goods') . " AS og ON og.goods_id = g.goods_id ";
        $leftJoin .= " LEFT JOIN " . $GLOBALS['sanmu']->table('order_info') . " AS o ON o.order_id = og.order_id ";

        /* 记录总数 */
        $sql = "SELECT c.cat_id FROM " . $GLOBALS['sanmu']->table('category') . " AS c " .
            $leftJoin .
        ...
        $sql = "SELECT c.cat_id, c.cat_name, " .
            ...
            " FROM " . $GLOBALS['sanmu']->table('category') . " AS c " .
            $leftJoin .
            $where_c . $where_o . $groupBy .
    ...
    if ($cat_list) {
        ...
        foreach ($cat_list as $key => $val) {
            ...
            $sql = " SELECT " .
                ...
                " FROM " . $GLOBALS['sanmu']->table('order_goods') . " as og " .
                " LEFT JOIN " . $GLOBALS['sanmu']->table('order_info') . " AS o ON o.order_id = og.order_id " .
                " LEFT JOIN " . $GLOBALS['sanmu']->table('goods') . " AS g ON g.goods_id = og.goods_id " .
                " LEFT JOIN " . $GLOBALS['sanmu']->table('category') . " AS c ON c.cat_id = g.cat_id " .
...
function get_statistical_today_trend($search_data = array())
{
    ...
    $sql = " SELECT FROM_UNIXTIME(o.add_time,'%y-%m-%d-%H') AS hour, " .
        statistical_field_order_num() . " AS volume, " .
        statistical_field_sale_money() . " AS money " .
        " FROM " . $GLOBALS['sanmu']->table('order_info') . " AS o" . ' WHERE o.main_count = 0 AND o.add_time BETWEEN ' . $date_start . ' AND ' . $date_end . $where_data . ' GROUP BY hour ORDER BY hour ASC ';
    $result = $GLOBALS['db']->getAll($sql);
...
function member_account_stats($page = 0)
{
    ...
    if ($result === false) {
        ...
        /* 关联查询 */
        $leftJoin = '';
        $leftJoin .= " LEFT JOIN " . $GLOBALS['sanmu']->table('user_rank') . " AS ur ON ur.rank_id = u.user_rank ";
        $leftJoin .= " LEFT JOIN " . $GLOBALS['sanmu']->table('account_log') . " AS al ON al.user_id = u.user_id ";

        /* 记录总数 */
        $sql = "SELECT u.user_id FROM " . $GLOBALS['sanmu']->table('users') . " AS u " .
            $leftJoin .
        ...
        $sql = "SELECT u.user_id, u.user_name, u.nick_name, u.user_money, u.frozen_money, ur.rank_name " .
            " FROM " . $GLOBALS['sanmu']->table('users') . " AS u " .
            $leftJoin .
            $where_u . $where_al . $groupBy .
            " ORDER BY $filter[sort_by] $filter[sort_order] " .
            " LIMIT " . ($filter['page'] - 1) * $filter['page_size'] . ",$filter[page_size]";
...
function merchants_commission_list()
{
    ...
    if ($result === false) {
        ...
        if ($filter['user_name']) {
            $sql = "SELECT GROUP_CONCAT(user_id) AS user_id FROM " . $GLOBALS['sanmu']->table('users') . " WHERE user_name LIKE '%" . mysql_like_quote($filter['user_name']) . "%'";
        ...
        if ($filter['cycle'] != -1) {
            ...
            $left_join .= " LEFT JOIN " . $GLOBALS['sanmu']->table('merchants_server') . " AS ms ON ms.user_id = mis.user_id ";
        }
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['sanmu']->table('merchants_shop_information') . " as mis " . $left_join . $where;
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);
        $filter['page_count'] = $filter['record_count'] > 0 ? ceil($filter['record_count'] / $filter['page_size']) : 1;

        /* 查询 */  //ecmoban模板堂 -zhuo suppliers_sn
        $sql = "SELECT mis.*, msf.companyName, msf.company_adress, msf.company_contactTel FROM " . $GLOBALS['sanmu']->table("merchants_shop_information") . " as mis " .
            " LEFT JOIN " . $GLOBALS['sanmu']->table('merchants_steps_fields') . " as msf on mis.user_id = msf.user_id " . $left_join . $where .
            " ORDER BY " . $filter['sort_by'] . " " . $filter['sort_order'];
    ...
    for ($i = 0; $i < count($row); $i++) {
        $sql = "SELECT server_id, suppliers_desc, suppliers_percent FROM " . $GLOBALS['sanmu']->table('merchants_server') . " WHERE user_id = '" . $row[$i]['user_id'] . "' LIMIT 1";
        ...
        $row[$i]['user_name'] = $GLOBALS['db']->getOne("SELECT user_name FROM " . $GLOBALS['sanmu']->table('users') . " WHERE user_id = '" . $row[$i]['user_id'] . "'");
        ...
        $sql = "SELECT ss.shop_name, ss.shop_address, ss.mobile, " .
            ...
            " FROM " . $GLOBALS['sanmu']->table('seller_shopinfo') . " AS ss " .
            "LEFT JOIN " . $GLOBALS['sanmu']->table('region') . " AS p ON ss.province = p.region_id " .
            "LEFT JOIN " . $GLOBALS['sanmu']->table('region') . " AS t ON ss.city = t.region_id " .
            "LEFT JOIN " . $GLOBALS['sanmu']->table('region') . " AS d ON ss.district = d.region_id " .
...
function settlement_total_stats()
{
    ...
    $left_join = '';
    $sql = " SELECT mis.user_id FROM " . $GLOBALS['sanmu']->table('merchants_shop_information') . " AS mis " . $left_join . $where;
    $seller = $GLOBALS['db']->getCol($sql);
    /* 计算销售总额 */
    $store_num = count($seller);
    if ($store_num > 0) {
        $sql = " SELECT SUM(" . order_commission_field('o.') . ") AS total_amount " .
            " FROM " . $GLOBALS['sanmu']->table('order_info') . " AS o WHERE o.ru_id " . db_create_in($seller);
...
function get_statistical_shop_area($search_data = array())
{
    ...
    if (isset($search_data['area']) && !empty($search_data['area'])) {
        $sql = " SELECT region_id FROM " . $GLOBALS['sanmu']->table('merchants_region_info') . " WHERE ra_id = '$search_data[area]' ";
    ...
    /* 关联查询 */
    $leftJoin = '';
    $leftJoin .= " LEFT JOIN " . $GLOBALS['sanmu']->table('merchants_shop_information') . " AS msi ON msi.user_id = spi.ru_id ";
    $leftJoin .= " LEFT JOIN " . $GLOBALS['sanmu']->table('region') . " AS r ON r.region_id = spi.province ";
    /* 数据查询 */
    $sql = "SELECT spi.province, r.region_name as province_name, " .
        statistical_field_shop_num() . " AS store_num " . //店铺数量
        " FROM " . $GLOBALS['sanmu']->table('seller_shopinfo') . " AS spi " .
        $leftJoin .
        $where_data . $groupBy;
    $result = $GLOBALS['db']->getAll($sql);
...
function list_download($config = array())
{
    ...
    echo sanmu_iconv(EC_CHARSET, 'GB2312', $config['thead'][0]) . "\t";
    echo sanmu_iconv(EC_CHARSET, 'GB2312', $config['thead'][1]) . "\t";
    echo sanmu_iconv(EC_CHARSET, 'GB2312', $config['thead'][2]) . "\t";
    echo sanmu_iconv(EC_CHARSET, 'GB2312', $config['thead'][3]) . "\t";
    echo sanmu_iconv(EC_CHARSET, 'GB2312', $config['thead'][4]) . "\t";
    echo sanmu_iconv(EC_CHARSET, 'GB2312', $config['thead'][5]) . "\t";
    echo sanmu_iconv(EC_CHARSET, 'GB2312', $config['thead'][6]) . "\t";
    echo sanmu_iconv(EC_CHARSET, 'GB2312', $config['thead'][7]) . "\t\n";

    foreach ($config['tdata'] as $data) {
        echo sanmu_iconv(EC_CHARSET, 'GB2312', $data['user_id'] ?? 0) . "\t";
        echo sanmu_iconv(EC_CHARSET, 'GB2312', $data['user_name'] ?? '') . "\t";
        echo sanmu_iconv(EC_CHARSET, 'GB2312', $data['total_num'] ?? 0) . "\t";
        echo sanmu_iconv(EC_CHARSET, 'GB2312', $data['total_fee'] ?? 0) . "\t";
        echo sanmu_iconv(EC_CHARSET, 'GB2312', $data['valid_num'] ?? 0) . "\t";
        echo sanmu_iconv(EC_CHARSET, 'GB2312', $data['valid_fee'] ?? 0) . "\t";
        echo sanmu_iconv(EC_CHARSET, 'GB2312', $data['return_num'] ?? 0) . "\t";
        echo sanmu_iconv(EC_CHARSET, 'GB2312', $data['return_fee'] ?? 0) . "\t";
        echo "\n";
    }
