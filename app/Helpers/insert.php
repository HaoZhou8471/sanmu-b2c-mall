    ...
    if (isset($ecsCookie['history']) && !empty($ecsCookie['history'])) {
        ...
        if ($res) {
            foreach ($res as $row) {
                ...
                $goods['short_name'] = $GLOBALS['_CFG']['goods_name_length'] > 0 ? app(SanmuRepository::class)->subStr($row['goods_name'], $GLOBALS['_CFG']['goods_name_length']) : $row['goods_name'];
                ...
                $goods['url'] = app(SanmuRepository::class)->buildUri('goods', ['gid' => $row['goods_id']], $row['goods_name']);
                //by wang
    ...
    if (isset($ecsCookie['history']) && !empty($ecsCookie['history'])) {
        ...
        if ($res) {
            foreach ($res as $key => $row) {
                ...
                $res[$key]['short_name'] = $GLOBALS['_CFG']['goods_name_length'] > 0 ? app(SanmuRepository::class)->subStr($row['goods_name'], $GLOBALS['_CFG']['goods_name_length']) : $row['goods_name'];
                ...
                $res[$key]['url'] = app(SanmuRepository::class)->buildUri('goods', ['gid' => $row['goods_id']], $row['goods_name']);
            }
        }
    }
...
function insert_member_info()
{
    ...
    $register = app(SanmuRepository::class)->buildUri('user', ['act' => 'register']);
    $GLOBALS['smarty']->assign('register', $register);

    $logout = app(SanmuRepository::class)->buildUri('user', ['act' => 'logout']);
...
function insert_category_tree_nav($arr = [])
{
   ...
   // $val = $GLOBALS['smarty']->fetch('library/category_tree_nav.lbi');
    $val = $GLOBALS['smarty']->fetch('library/page_header_index.lbi');
    $GLOBALS['smarty']->caching = $need_cache;
    $GLOBALS['smarty']->force_compile = $need_compile;
    ...
    if (isset($ecsCookie['history']) && !empty($ecsCookie['history'])) {
        ...
        if ($res) {
            foreach ($res as $key => $row) {
                ...
                $res[$key]['shopUrl'] = app(SanmuRepository::class)->buildUri('merchants_store', ['urid' => $row['user_id']]);
...
function insert_wholesale_cart_info()
{
    ...
    if ($row) {
        foreach ($row as $k => $v) {
            ...
            $arr[$k]['url'] = app(SanmuRepository::class)->buildUri('wholesale_goods', ['aid' => $v['goods_id']], $v['goods_name']);
...
{
    $str = '';
    if (!empty($COOKIE['ECS']['history']))
    {
        $where = db_create_in($COOKIE['ECS']['history'], 'goods_id');
...
function insert_cart_mes_num()
{
    ...
    $row = $row->where('rec_type', CART_GENERAL_GOODS);
