            ...
            foreach ($res as $row) {
                ...
                if ($articles) {
                    foreach ($articles as $key => $val) {
                        $articles[$key]['url'] = $val['open_type'] != 1 ?
                            app(SanmuRepository::class)->buildUri('article', ['aid' => $val['article_id']], $val['title']) : trim($val['file_url']);
                    }
                }
                ...
                if ($brands_list) {
                    foreach ($brands_list as $key => $val) {
                        ...
                        $brands_list[$temp_key]['url'] = app(SanmuRepository::class)->buildUri('category', ['cid' => $cat_id, 'bid' => $val['brand_id']], $cat['cat_name']);
    ...
    if (!empty($ecsCookie['history'])) {
        ...
        if ($res) {
            foreach ($res as $row) {
                ...
                $arr[$row['goods_id']]['url'] = app(SanmuRepository::class)->buildUri('goods', ['gid' => $row['goods_id']], $row['goods_name']);
                $arr[$row['goods_id']]['sales_volume'] = $row['sales_volume'];
                $arr[$row['goods_id']]['shop_name'] = app(MerchantCommonService::class)->getShopName($row['user_id'], 1); //店铺名称
                $arr[$row['goods_id']]['shopUrl'] = app(SanmuRepository::class)->buildUri('merchants_store', ['urid' => $row['user_id']]);
...
function presale_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
    ...
    if ($re_type == true) {
        ...
        foreach ($options as $key => $value) {
            //$options[$key]['url'] = app(SanmuRepository::class)->buildUri('category', array('cid' => $value['cid']), $value['c_name']);
        }
...
function get_guess_store($user_id, $limit = 0)
{
    ...
    if (empty($store_list) || count($store_list) < 4) {
        $row = OrderGoods::selectRaw("SUM(goods_number) AS total,  rec_id")
        ...
        $ru_id = $row ? app(BaseRepository::class)->getKeyPluck($row) : [];
    } else {
...
function get_desc_images_del($images_list = '')
{
    ...
    if ($images_list) {
        for ($i = 0; $i < count($images_list); $i++) {
            ...
            if (isset($img[1])) {
                sanmu_unlink(storage_public($image_dir . $img[1]));
            }
        }
    }
}
    ...
    if (!empty($img_list)) {
        foreach ($img_list as $k => $v) {
            if ($v['img_file']) {
                ...
                app(SanmuRepository::class)->getOssDelFile([$v['img_file']]);
...
function get_brand_url($brand_id = 0)
{
    ...
    if ($res) {
        $res['url'] = app(SanmuRepository::class)->buildUri('brand', ['bid' => $res['brand_id']], $res['brand_name']);
