...
function get_collection_brands($user_id, $record_count, $page, $pageFunc, $size = 5, $warehouse_id = 0, $area_id = 0, $area_city = 0)
{
    ...
    if ($res) {
        foreach ($res as $key => $row) {
            ...
            $brand_list[$row['rec_id']]['url'] = app(SanmuRepository::class)->buildUri('brandn', ['bid' => $row['brand_id'], 'act' => 'index'], $row['brand_name']);
    ...
    if ($res) {
        foreach ($res as $row) {
            ...
            if (isset($GLOBALS['display']) == 'grid') {
                $arr[$row['goods_id']]['goods_name'] = $GLOBALS['_CFG']['goods_name_length'] > 0 ? app(SanmuRepository::class)->subStr($row['goods_name'], $GLOBALS['_CFG']['goods_name_length']) : $row['goods_name'];
            } else {
    ...
    if ($res) {
        foreach ($res as $key => $row) {
            ...
            if ($ru_id == 0) {
                ...
                if ($kf_im_switch) {
                    $store_list[$key]['is_sanmu'] = true;
                } else {
                    $store_list[$key]['is_sanmu'] = false;
                }
            } else {
                $store_list[$key]['is_sanmu'] = false;
            }

            $chat = app(SanmuRepository::class)->chatQq($row);
...
function get_user_store_goods_list($user_id = 0, $warehouse_id = 0, $area_id = 0, $area_city = 0, $type = '', $sort = 'last_update', $order = 'DESC', $size = 10)
{
    ...
    if ($res) {
        foreach ($res as $key => $row) {
            ...
            $goods_list[$row['goods_id']]['url'] = app(SanmuRepository::class)->buildUri('goods', ['gid' => $row['goods_id']], $row['goods_name']);
...
function add_message($message)
{
    ...
    if ($message['upload']) {
        ...
        app(SanmuRepository::class)->getOssAddFile([$oss_img_name]);
    } else {
        $img_name = '';
    }
    ...
    if ($res) {
        foreach ($res as $row) {
            ...
            $booking[] = ['rec_id' => $row['rec_id'],
                ...
                'url' => app(SanmuRepository::class)->buildUri('goods', ['gid' => $row['goods_id']], $row['goods_name'])];
        }
    }
...
function get_account_log($user_id, $size, $start)
{
    ...
    if ($res) {
        foreach ($res as $rows) {
            ...
            $rows['short_admin_note'] = ($rows['admin_note'] > '') ? app(SanmuRepository::class)->subStr($rows['admin_note'], 30) : 'N/A';
            $rows['user_note'] = nl2br(htmlspecialchars($rows['user_note']));
            $rows['short_user_note'] = ($rows['user_note'] > '') ? app(SanmuRepository::class)->subStr($rows['user_note'], 30) : 'N/A';
...
function get_comment_list($user_id, $page_size, $start)
{
    ...
    if ($res) {
        foreach ($res as $row) {
            ...
            $row['goods_url'] = app(SanmuRepository::class)->buildUri('goods', ['gid' => $row['id_value']], $row['goods_name']);
            $comments[] = $row;
        }
    }
...
function get_user_order_comment_list($user_id, $sign = 0, $order_id = 0, $size = 0, $start = 0)
{
    ...
    if ($res) {
        foreach ($res as $key => $row) {
            ...
            $row['goods_url'] = app(SanmuRepository::class)->buildUri('goods', ['gid' => $row['goods_id']], $row['goods_name']);
