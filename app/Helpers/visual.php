            ...
            if (is_dir($src . '/' . $file)) {
                ...
                if ($type == 0) {
                    ...
                    if (in_array($prefix, $format)) {
                        ...
                        app(SanmuRepository::class)->getOssAddFile([$ossFile]);
                    }
                } else {
...
function get_home_adv_goods_list($where = [])
{
    ...
    if (isset($where['rs_id']) && $where['rs_id'] > 0) {
        $row = app(SanmuRepository::class)->getWhereRsid($row, 'user_id', $where['rs_id']);
    }
    ...
    if ($brand_id) {
        ...
        if ($table == 'goods') {
            ...
            if ($GLOBALS['_CFG']['region_store_enabled'] == 1) {
                $res = app(SanmuRepository::class)->getWhereRsid($res, 'user_id', $adminru['rs_id']);
            } else {
                if ($adminru['ru_id'] > 0) {
...
function get_down_hometemplates($suffix = '')
{
    if ($GLOBALS['_CFG']['open_oss'] && $GLOBALS['_CFG']['server_model'] && !empty($suffix)) {
        if (!file_exists(storage_public(DATA_DIR . '/sc_file/hometemplates/' . $suffix . ".php"))) {
            /* 下载目录下文件 */
            $oss_list = app(SanmuRepository::class)->getOssListFile(['prefix' => DATA_DIR . '/home_templates/' . $GLOBALS['_CFG']['template'] . '/' . $suffix . '/']);

            if (!is_array($oss_list)) {
                $oss_list = sanmu_decode($oss_list, true);
            ...
            /* 下载目录下文件 */
            $oss_bonusadv_list = app(SanmuRepository::class)->getOssListFile(['prefix' => DATA_DIR . '/home_templates/' . $GLOBALS['_CFG']['template'] . '/' . $suffix . '/images/bonusadv/']);

            if (!is_array($oss_list)) {
                $oss_bonusadv_list = sanmu_decode($oss_bonusadv_list, true);
            ...
            /* 下载目录下文件 */
            $oss_content_list = app(SanmuRepository::class)->getOssListFile(['prefix' => DATA_DIR . '/home_templates/' . $GLOBALS['_CFG']['template'] . '/' . $suffix . '/images/content/']);

            if (!is_array($oss_list)) {
                $oss_content_list = sanmu_decode($oss_content_list, true);
...
function get_down_topictemplates($topic_id = 0, $seller_id = 0)
{
    ...
    if ($GLOBALS['_CFG']['open_oss'] && $GLOBALS['_CFG']['server_model']) {
        if (!file_exists(storage_public('data/sc_file/topic/topic_' . $seller_id . "/" . "topic_" . $topic_id . ".php"))) {
            /* 下载目录下文件 */
            $oss_list = app(SanmuRepository::class)->getOssListFile(['prefix' => "data/topic/topic_" . $seller_id . "/" . "topic_" . $topic_id . '/']);
            ...
            /* 下载目录下文件 */
            $oss_images_list = app(SanmuRepository::class)->getOssListFile(['prefix' => "data/topic/topic_" . $seller_id . "/" . "topic_" . $topic_id . '/images/']);

            if (!is_array($oss_images_list)) {
                $oss_images_list = sanmu_decode($oss_images_list, true);
            ...
            /* 下载目录下文件 */
            $oss_content_list = app(SanmuRepository::class)->getOssListFile(['prefix' => "data/topic/topic_" . $seller_id . "/" . "topic_" . $topic_id . '/images/content/']);
...
function get_down_sellertemplates($merchant_id = 0, $tem = '')
{
    ...
    if ($GLOBALS['_CFG']['open_oss'] && $GLOBALS['_CFG']['server_model'] && !empty($tem)) {
        if (!file_exists(storage_public('data/sc_file/sellertemplates/seller_tem_' . $merchant_id . '/' . $tem . ".php"))) {
            /* 下载目录下文件 */
            $oss_list = app(SanmuRepository::class)->getOssListFile(['prefix' => 'data/seller_templates/seller_tem_' . $merchant_id . '/' . $tem . "/"]);
            ...
            /* 下载目录下文件 */
            $oss_css_list = app(SanmuRepository::class)->getOssListFile(['prefix' => 'data/seller_templates/seller_tem_' . $merchant_id . '/' . $tem . '/css/']);

            if (!is_array($oss_css_list)) {
                $oss_css_list = sanmu_decode($oss_css_list, true);
            ...
            /* 下载目录下文件 */
            $oss_images_list = app(SanmuRepository::class)->getOssListFile(['prefix' => 'data/seller_templates/seller_tem_' . $merchant_id . '/' . $tem . '/images/']);
            ...
            /* 下载目录下文件 */
            $oss_head_list = app(SanmuRepository::class)->getOssListFile(['prefix' => 'data/seller_templates/seller_tem_' . $merchant_id . '/' . $tem . '/images/head/']);

            if (!is_array($oss_head_list)) {
                $oss_head_list = sanmu_decode($oss_head_list, true);
            ...
            /* 下载目录下文件 */
            $oss_content_list = app(SanmuRepository::class)->getOssListFile(['prefix' => 'data/seller_templates/seller_tem_' . $merchant_id . '/' . $tem . '/images/content/']);
