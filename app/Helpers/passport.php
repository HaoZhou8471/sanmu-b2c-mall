...
function check_userinfo($user_name, $email)
{
    if (empty($user_name) || empty($email)) {
        return sanmu_header("Location: user.php?act=get_password\n");
    }
...
function send_pwd_email($uid, $user_name, $email, $code)
{
    ...
    $reset_email = $GLOBALS['sanmu']->url() . 'user.php?act=get_password&uid=' . $uid . '&code=' . $code;
...
function send_regiter_hash($user_id)
{
    ...
    $validate_email = $GLOBALS['sanmu']->url() . 'user.php?act=validate_email&hash=' . $hash;
...
function send_account_safe_hash($user_id, $type, $validated = 0)
{
    ...
    $template = get_mail_template('register_validate');
    $hash = register_hash('encode', $user_id);
    $validate_email = $GLOBALS['sanmu']->url() . 'user.php?act=account_safe&type=validated_email&hash=' . $hash;
    ...
    switch ($type) {
        case 'change_pwd':
            $validate_email = $GLOBALS['sanmu']->url() . 'user.php?act=account_safe&type=validated_email&mail_type=change_pwd&hash=' . $hash;
            break;
        case 'change_mail':
            $validate_email = $GLOBALS['sanmu']->url() . 'user.php?act=account_safe&type=validated_email&mail_type=change_mail&hash=' . $hash;
            break;
        case 'change_mobile':
            $validate_email = $GLOBALS['sanmu']->url() . 'user.php?act=account_safe&type=validated_email&mail_type=change_mobile&hash=' . $hash;
            break;
        case 'change_paypwd':
            $validate_email = $GLOBALS['sanmu']->url() . 'user.php?act=account_safe&type=validated_email&mail_type=change_paypwd&hash=' . $hash;
            break;
        ...
        case 'editmail':
            ...
            $validate_email = $GLOBALS['sanmu']->url() . 'user.php?act=account_safe&type=validated_email&mail_type=editmail' . $validated . '&hash=' . $hash;
...
function register_hash($operation, $key)
{
    if ($operation == 'encode') {
        $user_id = intval($key);
        $sql = "SELECT reg_time " .
            " FROM " . $GLOBALS['sanmu']->table('users') .
