...
function get_array_category_info($arr = [], $table = '')
{
    ...
    if ($arr) {
        $arr = app(SanmuRepository::class)->delStrComma($arr);
        $arr = app(BaseRepository::class)->getExplode($arr);
        ...
        foreach ($category_list as $key => $val) {
            $category_list[$key]['url'] = app(SanmuRepository::class)->buildUri($table, ['cid' => $val['cat_id']], $val['cat_name']);
        }
        return $category_list;
    } else {
        return false;
...
function get_del_edit_goods_img($goods_id, $result = [])
{
    ...
    if ($row) {
        ...
        app(SanmuRepository::class)->getOssDelFile($arr_img);
    }
}
    ...
    if (session()->has('goods.' . $admin_id) && $goods_list) {
        foreach ($goods_list as $key => $row) {
            ...
            if ($key == 0 && !$count) {
                ...
                $arr_img ? app(SanmuRepository::class)->getOssDelFile($arr_img) : [];
            }
        }
...
function get_del_goods_video()
{
    ...
    if ($goods_video) {
        ...
        if ($arr) {
            app(SanmuRepository::class)->getOssDelFile($arr);
        }
...
function lib_get_del_goods_gallery()
{
    ...
    if (session('thumb_img_id.' . session('admin_id'))) {
        ...
        if (!empty($res)) {
            foreach ($res as $k) {
                if ($k['img_url'] && strpos($k['img_url'], "data/gallery_album") === false) {
                    sanmu_unlink(storage_public($k['img_url']));
                    $arr_img[] = $k['img_url'];
                }
                if ($k['thumb_url'] && strpos($k['thumb_url'], "data/gallery_album") === false) {
                    sanmu_unlink(storage_public($k['thumb_url']));
                    $arr_img[] = $k['thumb_url'];
                }
                if ($k['img_original'] && strpos($k['img_original'], "data/gallery_album") === false) {
                    sanmu_unlink(storage_public($k['img_original']));
                    $arr_img[] = $k['img_original'];
                }
...
function get_statistical_data($start_date = 0, $end_date = 0, $type = 'order')
{
    ...
    if ($adminru['rs_id'] > 0) {
        $result = app(SanmuRepository::class)->getWhereRsid($result, 'ru_id', $adminru['rs_id']);
    } else {
    ...
    if ($order_delayed_list) {
        foreach ($order_delayed_list as $key => $value) {
            ...
            if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                $order_delayed_list[$key]['user_name'] = app(SanmuRepository::class)->stringToStar($order_delayed_list[$key]['user_name']);
            }
        }
    }
