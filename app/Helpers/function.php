...
function get_table_date($table = '', $where = 1, $date = [], $sqlType = 0)
{
    ...
    if (!empty($date)) {
        ...
        $sql = "SELECT " . $date . " FROM " . $GLOBALS['sanmu']->table($table) . " WHERE " . $where;
...
function get_Add_Drop_fields($date, $newDate = '', $table = '', $type = 'insert', $dateType = 'VARCHAR', $length = '', $IntType = 'NOT NULL', $comment = '')
{
    ...
    if (!empty($table) && !empty($date)) {
        ...
        //字段操作 start
        if ($type == 'insert') {
            $sql = "ALTER TABLE " . $GLOBALS['sanmu']->table($table) . " ADD `" . $date . "` " . $dateType . " " . $IntType . $comment;
        } elseif ($type == 'update') {
            $sql = "ALTER TABLE " . $GLOBALS['sanmu']->table($table) . " CHANGE `" . $date . "` `" . $newDate . "` " . $dateType . " " . $codingType . " " . $IntType . " " . $comment;
        } elseif ($type == 'delete') {
            $sql = "ALTER TABLE " . $GLOBALS['sanmu']->table($table) . " DROP `" . $date . "`";
        }
        //字段操作 end
...
function get_merchants_article_menu($cat_id)
{
    ...
    if ($res) {
        foreach ($res as $key => $row) {
            ...
            if ($row['open_type'] != 1) {
                $arr[$key]['url'] = app(SanmuRepository::class)->buildUri('merchants', ['mid' => $row['article_id']], $row['title']);
            } else {
...
function get_merchants_steps_fields_admin($table, $date, $dateType, $length, $notnull, $coding, $formName, $fields_sort, $tid)
{
    ...
    for ($i = 0; $i < count($date); $i++) {
        if (!empty($date[$i])) {
            ...
            //判断数据库表的字段是否存在
            $test = $GLOBALS['db']->query('Describe ' . $GLOBALS['sanmu']->table($table) . $date[$i]);
                ...
                if ($user_id > 0) {
                    ...
                    if ($arr[$i + 1]['textFields'] === 'contactPhone' || $arr[$i + 1]['textFields'] === 'contactEmail') {
                        if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                            $arr[$i + 1]['titles_centents'] = app(SanmuRepository::class)->stringToStar($arr[$i + 1]['titles_centents']);
                        }
                    }
        ...
        if ($category[$i][0]) {
            ...
            for ($j = 0; $j < count($twoChild); $j++) {
                if ($type == 0) {
                    ...
                    if ($category[$i][1]) {
                        $category[$i][1] = app(SanmuRepository::class)->delStrComma($category[$i][1]);
                        ...
                        if ($child_tree) {
                            foreach ($child_tree as $key => $row) {
                                ...
                                if ($ru_id) {
                                    ...
                                    $category[$i]['child_tree'][$key]['url'] = app(SanmuRepository::class)->buildUri('category', ['cid' => $row['cat_id']], $row['cat_name']);
                                }
...
function get_root_merchants_steps_title($pid = 0, $user_id = 0)
{
    ...
    if (isset($_FILES['ec_brandLogo']) && !empty($_FILES['ec_brandLogo'])) {
        ...
        app(SanmuRepository::class)->getOssAddFile([$brandLogo]);
    }
    ...
    if ($res) {
        foreach ($res as $key => $row) {
            ...
            if ($row['steps_style'] == 1) {
                ...
                app(SanmuRepository::class)->getOssAddFile([$ec_authorizeFile, $ec_shop_hypermarketFile]);
                    ...
                    if (session('user_id') > 0 && !isset($_REQUEST['searchBrandZhInput']) && !isset($_REQUEST['searchBrandEnInput'])) {
                        ...
                        if (!empty($brandName)) {
                            ...
                            if ($bRes > 0) {
                                ...
                                return sanmu_header("Location: " . $back_url . "\n");
                            } else {
...
function get_merchants_dt_file_insert_update($cat_id, $dt_id, $permanentFile, $permanent_date, $user_id)
{
    ...
    for ($i = 0; $i < count($cat_id); $i++) {
        ...
        app(SanmuRepository::class)->getOssAddFile([$pFile]);
...
function get_shop_brand_file($qInput, $qImg, $eDinput, $b_fid, $ec_shop_bid)
{
    ...
    for ($i = 0; $i < count($qInput); $i++) {
        if (isset($qImg['name'][$i]) && $qImg['name'][$i]) {
            ...
            app(SanmuRepository::class)->getOssAddFile([$qImg[$i]]);
    ...
    if ($formName) {
        ...
        for ($i = 0; $i < count($formName); $i++) {
            ...
            if (substr($formName[$i], -3) == 'Img') {
                ...
                app(SanmuRepository::class)->getOssAddFile([$setps_thumb]);
                //文本隐藏域数据
    ...
    if ($res) {
        ...
        if ($res) {
            foreach ($res as $key => $row) {
                ...
                if ($dtFile) {
                    $arr[$key]['permanent_file'] = app(SanmuRepository::class)->getImagePath($dtFile['permanent_file']);
...
//查询商品的默认配送方式运费金额
function get_goods_freight($goods, $warehouse_id = 0, $goods_region = [], $buy_number = 1, $shipping_code = 0)
{
    ...
    if ($res) {
        foreach ($res as $key => $row) {
            ...
            if ($row['og_extension_code'] == 'presale') {
                $arr[$key]['url'] = app(SanmuRepository::class)->buildUri('presale', ['act' => 'view', 'presaleid' => $row['extension_id']], $row['goods_name']);
            } elseif ($row['oi_extension_code'] == 'group_buy') {
                $arr[$key]['url'] = app(SanmuRepository::class)->buildUri('group_buy', ['gbid' => $row['extension_id']]);
            } elseif ($row['oi_extension_code'] == 'snatch') {
                $arr[$key]['url'] = app(SanmuRepository::class)->buildUri('snatch', ['sid' => $row['extension_id']]);
            } elseif ($row['oi_extension_code'] == 'seckill') {
                $arr[$key]['url'] = app(SanmuRepository::class)->buildUri('seckill', ['act' => "view", 'secid' => $row['extension_id']]);
            } elseif ($row['oi_extension_code'] == 'auction') {
                $arr[$key]['url'] = app(SanmuRepository::class)->buildUri('auction', ['auid' => $row['extension_id']]);
            } elseif ($row['oi_extension_code'] == 'exchange_goods') {
                $arr[$key]['url'] = app(SanmuRepository::class)->buildUri('exchange_goods', ['gid' => $row['extension_id']]);
            } else {
                $arr[$key]['url'] = app(SanmuRepository::class)->buildUri('goods', ['gid' => $row['goods_id']], $row['goods_name']);
            }
...
function get_goods_minMax_price($goods_id = 0, $warehouse_id = 0, $area_id = 0, $area_city = 0, $goods_price = 0, $market_price = 0, $type = 1)
{
...
function get_min_or_max_values($arr)
{
    ...
    if ($newArr['is_attr']) {
        $newArr['is_attr'] = app(SanmuRepository::class)->delStrComma($newArr['is_attr']);
    ...
    $newArr['fittings_minPrice'] = app(SanmuRepository::class)->delStrComma($newArr['fittings_minPrice']);
    $newArr['market_minPrice'] = app(SanmuRepository::class)->delStrComma($newArr['market_minPrice']);

    $newArr['fittings_maxPrice'] = app(SanmuRepository::class)->delStrComma($newArr['fittings_maxPrice']);
    $newArr['market_maxPrice'] = app(SanmuRepository::class)->delStrComma($newArr['market_maxPrice']);
...
function get_goods_transport_tpl($goodsInfo = [], $region = [], $shippingInfo = [], $goods_number = 1)
{
    ...
    if ($is_shipping) {
        ...
        $shippingFee = app(SanmuRepository::class)->shippingFee($shippingInfo['shipping_code'], $shippingInfo['configure'], $goods_weight, $shop_price, $goods_number);
...
function IM($appkey, $secretkey)
{
    ...
    if ($user_id > 0) {
        ...
        $userinfos->userid = 'sanmu' . $user_info['user_id'];
        $userinfos->password = 'sanmu' . $user_info['user_id'];
...
function get_coupons_type_info_res($filter = [], $cou_type ='' )
{
    ...
    //卖场
    if ($filter['rs_id'] > 0) {
        $res = app(SanmuRepository::class)->getWhereRsid($res, 'ru_id', $filter['rs_id']);
    }
...
function region_parent($region_id)
{
    ...
    if ($parent_id) {
        ...
        $result = app(SanmuRepository::class)->delStrComma($result);
    }
