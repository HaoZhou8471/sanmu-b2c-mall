    ...
    if ($res === null) {
        ...
        if ($data === false) {
            $sql = "SELECT w.cat_id, w.cat_name,w.is_show, w.parent_id, w.sort_order, COUNT(s.parent_id) AS has_children " .
                'FROM ' . $GLOBALS['sanmu']->table('wholesale_cat') . " AS w " .
                "LEFT JOIN " . $GLOBALS['sanmu']->table('wholesale_cat') . " AS s ON s.parent_id = w.cat_id " .
                "GROUP BY w.cat_id " .
...
{
    $sql = "SELECT goods_number, goods_price, (goods_number * goods_price) AS goods_amount FROM " . $GLOBALS['sanmu']->table('wholesale_order_goods') . " WHERE rec_id = '$rec_id'";
...
function get_wholesale_return_goods($ret_id)
{
    ...
    $sql = "SELECT rg.*, g.goods_thumb, g.brand_id FROM " . $GLOBALS['sanmu']->table('wholesale_return_goods') .
        " as rg  LEFT JOIN " . $GLOBALS['sanmu']->table('wholesale_order_return') . "as r ON rg.rec_id = r.rec_id " .
        " LEFT JOIN " . $GLOBALS['sanmu']->table('wholesale') . " AS g ON g.goods_id = rg.goods_id " .
...
{
    $sql = " SELECT GROUP_CONCAT(rec_id) AS rec_id FROM " . $GLOBALS['sanmu']->table('wholesale_order_goods') . " WHERE order_id = '$order_id'";
    $rec_list = $GLOBALS['db']->getOne($sql);
    $rec_list = !empty($rec_list) ? explode(",", $rec_list) : array();

    $sql = " SELECT GROUP_CONCAT(rec_id) AS rec_id FROM " . $GLOBALS['sanmu']->table('wholesale_order_return') . " WHERE order_id = '$order_id'";
...
function wholesale_return_order_info_byId($order_id, $refound = true)
{
    if (!$refound) {
        ...
        $sql = " SELECT count(*) FROM " . $GLOBALS['sanmu']->table('wholesale_order_return') . " WHERE order_id = '$order_id' AND refound_status = 0";
        $res = $GLOBALS['db']->getOne($sql);
    } else {
        $sql = " SELECT * FROM " . $GLOBALS['sanmu']->table('wholesale_order_return') . " WHERE order_id = '$order_id'";
        $res = $GLOBALS['db']->getAll($sql);
    }
...
function wholesale_order_refound_fee($order_id = 0, $ret_id = 0)
{
    ...
    $sql = "SELECT SUM(actual_return) AS actual_return FROM " . $GLOBALS['sanmu']->table('wholesale_order_return') . " WHERE order_id = '$order_id' " .
...
{
    $sql = "select * FROM " . $GLOBALS['sanmu']->table('wholesale_order_goods') . " WHERE rec_id =" . $rec_id;
...
function wholesale_order_refound($order, $refund_type, $refund_note, $refund_amount = 0, $operation = '')
{
    ...
    if (1 == $refund_type) {
        ...
        if ($user_id > 0) {
            ...
            if ($order['suppliers_id']) {
                ...
                if ($suppliers_info) {
                    ...
                    /*$GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table('suppliers_account_log'), $log, 'INSERT');

                    $sql = "UPDATE " . $GLOBALS['sanmu']->table('suppliers') . " SET suppliers_money = suppliers_money + '" . $log['user_money'] . "' WHERE suppliers_id = '" . $order['suppliers_id'] . "'";
        ...
        $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table('user_account'), $account, 'INSERT');
    ...
    elseif (5 == $refund_type) {
        ...
        //查询当前退款订单使用了多少余额支付;
        $surplus = $GLOBALS['db']->getOne('SELECT surplus FROM' . $GLOBALS['sanmu']->table('order_info') . 'WHERE order_id=' . $order['order_id']);

        //余额退余额,白条退白条;
        if ($surplus != 0.00) {
            log_account_change($user_id, $surplus, 0, 0, 0, lang('baitiao.baitiao') . $change_desc);
        } else {
            $baitiao_info = $GLOBALS['db']->getRow("SELECT * FROM " . $GLOBALS['sanmu']->table('baitiao_log') . "
        ...
        $sql = "update {$GLOBALS['sanmu']->table('baitiao_log')} set is_refund=1 where order_id='{$order['order_id']}'";
        $GLOBALS['db']->query($sql);
...
function wholesale_return_order_list()
{
    ...
    if ($result === false) {
        ...
        /* 记录总数 */
        if ($filter['user_name']) {
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['sanmu']->table('wholesale_order_return') . " AS o ," .
            ...
            $sql = "SELECT COUNT(DISTINCT ret_id) FROM " . $GLOBALS['sanmu']->table('wholesale_order_return') . " AS r, " . $GLOBALS['sanmu']->table('wholesale_order_info') . " as o " . $where . " AND r.order_id = o.order_id";
        }
        ...
        $sql = "SELECT DISTINCT r.ret_id, o.suppliers_id, o.order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, " .
            ...
            "FROM " . $GLOBALS['sanmu']->table('wholesale_order_return') . " AS r " .
            "LEFT JOIN " . $GLOBALS['sanmu']->table('wholesale_order_info') . " AS o ON r.order_id = o.order_id " .
    ...
    if ($row) {
        foreach ($row as $key => $value) {
            ...
            $sql = "SELECT return_number, refound FROM " . $GLOBALS['sanmu']->table('wholesale_return_goods') . " WHERE rec_id = '" . $value['rec_id'] . "' LIMIT 1";
            ...
            if (isset($GLOBALS['_CFG']['show_mobile']) && $GLOBALS['_CFG']['show_mobile'] == 0) {
                ...
                $row[$key]['tel'] = app(SanmuRepository::class)->stringToStar($value['tel']);
                $row[$key]['email'] = app(SanmuRepository::class)->stringToStar($value['email']);
            }
        }
    }
...
function wholesale_order_goods($order_id)
{
    $sql = "SELECT og.*, (og.goods_price * og.goods_number) AS subtotal,og.goods_price, g.is_shipping, g.goods_weight AS goodsweight, g.goods_img, g.goods_thumb, " .
        "g.goods_cause, g.is_shipping FROM " . $GLOBALS['sanmu']->table('wholesale_order_goods') . " AS og " .
        "LEFT JOIN " . $GLOBALS['sanmu']->table('wholesale') . " AS g ON og.goods_id = g.goods_id " .
    ...
    if ($res) {
        foreach ($res as $key => $row) {
            ...
            $row['url'] = app(SanmuRepository::class)->buildUri('wholesale_goods', array('aid' => $row['goods_id']), $row['goods_name']);
...
{
    $sql = "SELECT COUNT(*) FROM " . $GLOBALS['sanmu']->table('wholesale_order_return') . " WHERE rec_id=" . $rec_id;
    $is_refound = 0;
...
function get_wholesale_goods_attr_info_new($arr, $type = 'pice', $warehouse_id = 0, $area_id = 0)
{
    ...
    if (!empty($arr)) {
        ...
        $leftJoin = " left join " . $GLOBALS['sanmu']->table('wholesale') . " as g on g.goods_id = ga.goods_id";
        $sql = "SELECT ga.goods_attr_id, a.attr_name, ga.attr_value, ga.attr_price " .
            "FROM " . $GLOBALS['sanmu']->table('wholesale_goods_attr') . " AS ga " .
            $leftJoin .
...
{
    /* 查询订单信息，检查状态 */
    $sql = "SELECT user_id, return_sn , return_status, order_sn FROM " . $GLOBALS['sanmu']->table('wholesale_order_return') . " WHERE order_id = '$order_id'";
    ...
    elseif ($return_order['return_status'] == 4) {
        ...
        $sql = "UPDATE " . $GLOBALS['sanmu']->table('wholesale_order_return') . " SET return_status = '" . 4 . "' WHERE order_id = '$order_id'";
...
function get_wholesale_order_arr($goods_number_return = 0, $rec_id = 0, $order_goods = array(), $order_info = array())
{
    ...
    foreach ($order_goods as $k => $v) {
        ...
        $sql = "SELECT ret_id FROM" . $GLOBALS['sanmu']->table('wholesale_order_return') . " WHERE rec_id = '" . $v['rec_id'] . "' AND order_id = '" . $v['order_id'] . "' AND refound_status = 1";
