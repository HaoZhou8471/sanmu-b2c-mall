...
class CaptchaVerify
{
    ...
    public function __construct($config = array())
    {
        ...
        $this->sessionRepository = app(SessionRepository::class);
    }
    ...
    public function check($code = '', $seKey = '', $id = '', $type = '')
    {
        ...
        // 验证码不能为空
        $secode = $this->sessionRepository->sessionGet($key, 0);
        ...
        if ($now_time - $secode['verify_time'] > $this->expire) {
            $this->sessionRepository->sessionPut($key, null);
        ...
        if ($this->authcode(strtoupper($code)) == $secode['verify_code']) {
            if ($type != 'ajax') {
                $this->reset && $this->sessionRepository->sessionPut($key, null);
            }
    ...
    public function entry($id = '')
    {
        ...
        // 验证码使用随机字体
        $ttfPath = Storage::disk('public')->path('data/verify/' . ($this->useZh ? 'zhttfs' : 'ttfs') . '/');
        ...
        if (!empty($id)) {
            $this->sessionRepository->sessionPut($key . $id, $secode);
        } else {
            $this->sessionRepository->sessionPut($key, $secode);
        }
    ...
    {
        $path = Storage::disk('public')->path('data/verify/bgs/');
