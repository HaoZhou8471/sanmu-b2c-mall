...
class Wxpay
{
    ...
    public function get_code($order = [], $payment = [], $user_id = 0)
    {
        ...
        if (is_mobile_device()) {
            ...
            if (is_wechat_browser() === false) {
                ...
                $scene_info = json_encode(['h5_info' => ['type' => 'Wap', 'wap_url' => sanmu_url('/'), 'wap_name' => $this->config['shop_name']]]);
                    ...
                    if ($postdata['result_code'] == 'SUCCESS') {
                        ...
                        if(isset($rs['status']) && $rs['status'] == 'error') {
                            ...
                            if (!empty($type) && $type == 'wxapp') {
                                // 小程序模板消息推送
                                $attach = isset($postdata['attach']) ? sanmu_decode($postdata['attach'], true) : [];
    ...
    public function orderQuery($order = [])
    {
        ...
        if ($payLog['is_paid'] == 0) {
            ...
            if (isset($payLog['pay_trade_data']) && !empty($payLog['pay_trade_data'])) {
                $pay_trade_data = sanmu_decode($payLog['pay_trade_data'], true);
    ...
    protected function xmlToArray($xml)
    {
        ...
        return sanmu_decode(json_encode($xml_string), true);
    }
