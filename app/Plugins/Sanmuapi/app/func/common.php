...
namespace App\Plugins\Sanmuapi\app\func;
...
class common
{
     ...
     * @return string
     */
    public static function xml_encode($data, $root = 'sanmu', $item = 'item', $attr = '', $id = 'id', $encoding = 'utf-8')
    {
        if (is_array($attr)) {
            $_attr = array();
    ...
    public static function generate_goods_sn($goods_id)
    {
        ...
        $sql = "SELECT goods_sn FROM " . $GLOBALS['sanmu']->table('goods') .
    ...
    public static function integral_to_give($order)
    {
        ...
        if ($order['extension_code'] == 'group_buy') {
            ...
            $sql = "SELECT ext_info FROM" . $GLOBALS['sanmu']->table('goods_activity') . "WHERE act_id = '" . $order['extension_id'] . "'";
            ...
            $leftJoin .= " left join " . $GLOBALS['sanmu']->table('warehouse_goods') . " as wg on g.goods_id = wg.goods_id and wg.region_id = og.warehouse_id ";
            $leftJoin .= " left join " . $GLOBALS['sanmu']->table('warehouse_area_goods') . " as wag on g.goods_id = wag.goods_id and wag.region_id = og.area_id ";

            $give_integral = "IF(og.ru_id > 0, (SELECT sg.give_integral / 100 FROM " . $GLOBALS['sanmu']->table('merchants_grade') . " AS mg, " .
                $GLOBALS['sanmu']->table('seller_grade') . " AS sg " .
                " WHERE mg.grade_id = sg.id AND mg.ru_id = og.ru_id LIMIT 1), 1)";

            $rank_integral = "IF(og.ru_id > 0, (SELECT sg.rank_integral / 100 FROM " . $GLOBALS['sanmu']->table('merchants_grade') . " AS mg, " .
                $GLOBALS['sanmu']->table('seller_grade') . " AS sg " .
                " WHERE mg.grade_id = sg.id AND mg.ru_id = og.ru_id LIMIT 1), 1)";
            ...
            $sql = "SELECT SUM(og.goods_number * IF(IF(g.model_price < 1, g.give_integral, IF(g.model_price < 2, wg.give_integral, wag.give_integral)) > -1, IF(g.model_price < 1, g.give_integral, IF(g.model_price < 2, wg.give_integral, wag.give_integral)), og.goods_price * $give_integral)) AS custom_points," .
                ...
                "LEFT JOIN " . $GLOBALS['sanmu']->table('goods') . " AS g ON og.goods_id = g.goods_id " .
                $leftJoin .
    ...
    public static function log_account_change($user_id, $user_money = 0, $frozen_money = 0, $rank_points = 0, $pay_points = 0, $change_desc = '', $change_type = ACT_OTHER)
    {
        ...
        if ($is_go && ($user_money || $frozen_money || $rank_points || $pay_points)) {
            ...
            $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table('account_log'), $account_log, 'INSERT');
            ...
            /* 更新会员当前等级 start */
            $sql = "SELECT rank_points FROM " . $GLOBALS['sanmu']->table("users") . " WHERE user_id = '$user_id'";
            $user_rank_points = $GLOBALS['db']->getOne($sql, true);

            $sql = 'SELECT rank_id, discount FROM ' . $GLOBALS['sanmu']->table('user_rank') . " WHERE special_rank = 0 AND min_points <= '" . $user_rank_points . "' ORDER BY min_points DESC LIMIT 1";
            ...
            $sql = "UPDATE " . $GLOBALS['sanmu']->table('users') . "SET user_rank = '" . $rank_row['rank_id'] . "' WHERE user_id = '$user_id'";
            $GLOBALS['db']->query($sql);

            $sql = "UPDATE " . $GLOBALS['sanmu']->table('sessions') . "SET user_rank = '" . $rank_row['rank_id'] . "', discount= '" . $rank_row['discount'] . "' WHERE userid = '$user_id' AND adminid = 0";
            $GLOBALS['db']->query($sql);
        }
    }
    ...
    public static function send_order_bonus($order_id)
    {
        ...
        if ($bonus_list) {
            ...
            $sql = "SELECT u.user_id, u.user_name, u.email " .
                ...
                $GLOBALS['sanmu']->table('users') . " AS u " .
            ...
            foreach ($bonus_list as $bonus) {
                ...
                /* 修改用户红包 */
                $sql = "INSERT INTO " . $GLOBALS['sanmu']->table('user_bonus') . " (bonus_type_id, user_id) " .
    ...
    public static function order_bonus($order_id)
    {
        ...
        $sql = "SELECT b.type_id, b.type_money, SUM(o.goods_number) AS number " .
            ...
            $GLOBALS['sanmu']->table('bonus_type') . " AS b " .
            " WHERE o.order_id = '$order_id' " .
            " AND o.is_gift = 0 " .
        ...
        $sql = "SELECT oi.add_time, og.ru_id " .
            " FROM " . $GLOBALS['sanmu']->table('order_info') . "AS oi," .
            $GLOBALS['sanmu']->table('order_goods') . "AS og" .
        ...
        $sql = "SELECT type_id, type_name, type_money, IFNULL(FLOOR('$amount' / min_amount), 1) AS number " .
            "FROM " . $GLOBALS['sanmu']->table('bonus_type') .
    ...
    public static function send_order_coupons($order)
    {
        ...
        $sql = "SELECT * FROM " . $GLOBALS['sanmu']->table('coupons') .
        ...
        foreach ($coupons_buy_info as $k => $v) {
            ...
            $num = $GLOBALS['db']->getOne(" SELECT COUNT(uc_id) FROM " . $GLOBALS['sanmu']->table('coupons_user') . " WHERE cou_id='" . $v['cou_id'] . "'");
            if ($v['cou_total'] <= $num) {
                continue;
            }

            //当前用户已经领取的数量,超过允许领取的数量则不再返券
            $cou_user_num = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['sanmu']->table('coupons_user') . " WHERE user_id='" . $order['user_id'] . "' AND cou_id ='" . $v['cou_id'] . "' AND is_use = 0");

            if ($cou_user_num < $v['cou_user_num']) {

                //获取订单商品详情
                $sql = " SELECT GROUP_CONCAT(og.goods_id) AS goods_id, GROUP_CONCAT(g.cat_id) AS cat_id FROM " . $GLOBALS['sanmu']->table('order_goods') . " AS og," . $GLOBALS['sanmu']->table('goods') . " AS g" . " WHERE og.goods_id = g.goods_id AND order_id='" . $order['order_id'] . "'";
                ...
                if ($order['goods_amount'] >= $v['cou_get_man']) {
                    ...
                    //返券
                    if ($flag) {
                        $GLOBALS['db']->query("INSERT INTO " . $GLOBALS['sanmu']->table('coupons_user') . " (`user_id`,`cou_id`,`uc_sn`) VALUES ('" . $order['user_id'] . "','" . $v['cou_id'] . "','" . $v['uc_sn'] . "') ");
                    }
                }
            }
        }
    ...
    public static function get_cou_children($cat = '')
    {
        ...
        if ($cat) {
            ...
            $catlist = app(SanmuRepository::class)->delStrComma($catlist, 0, -1);
            $catlist = array_unique(explode(",", $catlist));
            $catlist = implode(",", $catlist);
            $cat = implode(",", $cat);
            $catlist = !empty($catlist) ? $catlist . "," . $cat : $cat;

            $catlist = app(SanmuRepository::class)->delStrComma($catlist);
        }
    ...
    public static function get_one_user_rank($user_id)
    {
        ...
        $sql = 'SELECT u.user_money,u.email, u.pay_points, u.user_rank, u.rank_points, ' .
            ...
            ' LEFT JOIN ' . $GLOBALS['sanmu']->table('user_bonus') . ' AS ub' .
            ' ON ub.user_id = u.user_id AND ub.used_time = 0 ' .
            ' LEFT JOIN ' . $GLOBALS['sanmu']->table('bonus_type') . ' AS b' .
        ...
        if ($row = $GLOBALS['db']->getRow($sql)) {
            ...
            if ($row['user_rank'] > 0) {
                $sql = "SELECT special_rank from " . $GLOBALS['sanmu']->table('user_rank') . "where rank_id='$row[user_rank]'";
                if ($GLOBALS['db']->getOne($sql) === '0' || $GLOBALS['db']->getOne($sql) === null) {
                    $sql = "update " . $GLOBALS['sanmu']->table('users') . "set user_rank='0' where user_id='$user_id'";
                    $GLOBALS['db']->query($sql);
                    $row['user_rank'] = 0;
                }
            }
            ...
            if ($row['user_rank'] == 0) {
                ...
                $sql = 'SELECT rank_id, discount FROM ' . $GLOBALS['sanmu']->table('user_rank') . " WHERE special_rank = 0 AND min_points <= '" . intval($row['rank_points']) . "' ORDER BY min_points DESC LIMIT 1";
                if ($row = $GLOBALS['db']->getRow($sql)) {
                    return $row['rank_id'];
                } else {
                    return false;
                ...
                // 特殊等级
                $sql = 'SELECT rank_id, discount FROM ' . $GLOBALS['sanmu']->table('user_rank') . " WHERE rank_id = '$row[user_rank]' LIMIT 1";
                if ($row = $GLOBALS['db']->getRow($sql)) {
                    return $row['rank_id'];
                } else {
                    return false;
        ...
        if ($code) {
            foreach ($control_table as $k => $v) {
                if ($v['cloud_code'] == $code && $v['code']) {
                    $sql = "SELECT * FROM" . $GLOBALS['sanmu']->table('shipping') . "WHERE shipping_code = '" . $v['code'] . "' LIMIT 1";
    ...
    {
        $sql = 'SELECT region_id, region_name FROM ' . $GLOBALS['sanmu']->table('region') .
