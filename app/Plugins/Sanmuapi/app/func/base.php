...
class base
{
    ...
    public static function get_join_table($table = '', $join_on = '', $select = '', $where = 1, $result_type = 0)
    {
        ...
        foreach ($table as $key => $row) {
            if ($key == 0) {
                $left .= $GLOBALS['sanmu']->table($row['table']) . " AS " . $row['alias'] . ",";
            } else {
                $left .= " LEFT JOIN " . $GLOBALS['sanmu']->table($row['table']) . " AS " . $row['alias'] . ",";
            }
    ...
    public static function get_link_seller_brand($brand_id = 0, $type = 0)
    {
        if ($type == 1) {
             ...
             * 商家品牌ID
             */
            $sql = "SELECT GROUP_CONCAT(bid) AS brand_id FROM " . $GLOBALS['sanmu']->table('link_brand') . " WHERE brand_id = '$brand_id'";
        } else {
            /**
             * 平台品牌ID
             */
            $sql = "SELECT GROUP_CONCAT(brand_id) AS brand_id FROM " . $GLOBALS['sanmu']->table('link_brand') . " WHERE bid = '$brand_id'";
        }
