...
namespace App\Plugins\Sanmuapi\app\controller;

use App\Plugins\Sanmuapi\app\func\common;
use App\Plugins\Sanmuapi\app\func\base;
use App\Plugins\Sanmuapi\app\model\goodsModel;
...
class goods extends goodsModel
{
    ...
    public function __construct($where = array())
    {
        ...
        if ($this->method == 'sanmu.goods.area.info.get') {
            $this->alias = 'wag.';
        }

        $this->where = goodsModel::get_where($this->where_val, $this->alias);
        if ($this->method != 'sanmu.goods.batchinsert.post') {
            ...
            if (!isset($select['a_id'])) {
                if (isset($select['region_sn']) && !empty($select['region_sn'])) {
                    ...
                    if (isset($select['province_name']) && $select['province_name']) {
                        $sql = "SELECT region_id FROM " . $GLOBALS['sanmu']->table('region_warehouse') . " WHERE region_type = 1 AND region_name = '" . $select['province_name'] . "'";
                        ...
                        $sql = "SELECT region_id FROM " . $GLOBALS['sanmu']->table('region_warehouse') . " WHERE region_type = 2 AND region_name = '" . $select['city_name'] . "'";
                        $region_id = $GLOBALS['db']->getOne($sql);
            ...
            if (!isset($select['a_id'])) {
                if (strlen($this->where) != 1) {
                    ...
                    if (!$info) {
                        ...
                        $is_region = 1;
                        if (isset($select['province_name']) && $select['province_name']) {
                            $sql = "SELECT region_id FROM " . $GLOBALS['sanmu']->table('region_warehouse') . " WHERE region_type = 1 AND province_name = '" . $select['province_name'] . "'";
                            $region_id = $GLOBALS['db']->getOne($sql);
