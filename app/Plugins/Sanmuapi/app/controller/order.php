...
use App\Plugins\Sanmuapi\app\model\orderModel;
...
class order extends orderModel
{
    ...
    public function get_order_goods_delete($table)
    {
        if (isset($this->order_sn) && !empty($this->order_sn)) {
            ...
            $sql = "DELETE FROM " . $GLOBALS['sanmu']->table($table['order']) . " WHERE order_id " . db_create_in($order_id);
    ...
    {
        $sql = "SELECT GROUP_CONCAT(order_id) AS order_id FROM " . $GLOBALS['sanmu']->table($table) . " WHERE order_sn " . db_create_in($this->order_sn);
