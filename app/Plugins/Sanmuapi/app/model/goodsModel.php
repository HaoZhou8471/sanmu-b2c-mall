...
namespace App\Plugins\Sanmuapi\app\model;

use App\Plugins\Sanmuapi\app\func\common;
use App\Plugins\Sanmuapi\app\func\base;
use App\Plugins\Sanmuapi\languages\goodsLang;
...
abstract class goodsModel extends common
{
    ...
    public function get_select_list($table, $select, $where, $page_size, $page, $sort_by, $sort_order)
    {
        ...
        if ($table == 'warehouse_area_goods') {
            ...
            if (!empty($region_where)) {
                $left_join = " LEFT JOIN " . $GLOBALS['sanmu']->table('region_warehouse') . " AS r1 ON r1.region_id = wag.region_id " . $region_where;
            }

            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['sanmu']->table('warehouse_area_goods') . " AS wag " .
                $left_join .
                " WHERE " . $where;
            ...
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['sanmu']->table($table) . " WHERE " . $where;
            $result['record_count'] = $GLOBALS['db']->getOne($sql);
        ...
        if ($table == 'warehouse_area_goods') {
            ...
            $sql = "SELECT wag.*, r1.region_name FROM " . $GLOBALS['sanmu']->table('warehouse_area_goods') . " AS wag " .
                " LEFT JOIN " . $GLOBALS['sanmu']->table('region_warehouse') . " AS r1 ON r1.region_id = wag.region_id " . $region_where .
                " WHERE " . $where;
            $result['list'] = $GLOBALS['db']->getAll($sql);
        } else {
            $sql = "SELECT " . $select . " FROM " . $GLOBALS['sanmu']->table($table) . " WHERE " . $where;
            $result['list'] = $GLOBALS['db']->getAll($sql);
        }
    ...
    public function get_select_info($table, $select, $where)
    {
        ...
        if ($table == 'warehouse_area_goods') {
            ...
            $sql = "SELECT " . $select . " FROM " . $GLOBALS['sanmu']->table($table) . " WHERE " . $where . " LIMIT 1";
            $goods = $GLOBALS['db']->getRow($sql);
        }
    ...
    public function get_insert($table, $select, $format)
    {
        ...
        $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table($table), $select, "INSERT");
    ...
    public function get_more_insert($table, $select, $format)
    {
        ...
        $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table($first_table), $first_select, "INSERT");
        ...
        for ($i = 0; $i < count($table); $i++) {
            if ($i > 0 && $table[$i]) {
                ...
                $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table($table[$i]), $select[$i], "INSERT");
            }
        }
    ...
    public function get_update($table, $select, $where, $format, $info = [])
    {
        ...
        $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table($table), $select, "UPDATE", $where);
    ...
    public function get_more_update($table, $select, $where, $format, $info = [])
    {
        ...
        $first_table = $table[0];
        $first_select = $select[0];
        $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table($first_table), $first_select, "UPDATE", $where);
        ...
        for ($i = 0; $i < count($table); $i++) {
            if ($i > 0 && $table[$i]) {
                ...
                $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table($table[$i]), $select[$i], "UPDATE", $where);
            }
        }
    ...
    public function get_more_delete($table, $where, $format)
    {
        ...
        if (strlen($where) != 1) {
            for ($i = 0; $i < count($table); $i++) {
                $sql = "DELETE FROM " . $GLOBALS['sanmu']->table($table[$i]) . " WHERE " . $where;
                $GLOBALS['db']->query($sql);
            }
    ...
    public function get_goods_batch_insert($table, $select, $format)
    {
        ...
        if (!empty($select)) {
            ...
            //获取贡云属性类型id
            $sql = "SELECT cat_id FROM" . $GLOBALS['sanmu']->table('goods_type') . "WHERE cat_name = 'cloud' LIMIT 1";
            ...
            if ($cat_id == 0 || $cat_id == '') {
                ...
                $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table('goods_type'), $goods_type, "INSERT");
                $cat_id = $GLOBALS['db']->insert_id();
            }

            if (isset($select)) {
                $select = is_array($select) ? $select : sanmu_decode($select, true);
                ...
                $id = 0;
                    ...
                    //判断是重新推送过来的商品
                    $sql = "SELECT goods_id FROM" . $GLOBALS['sanmu']->table($table) . "WHERE cloud_id = '" . $v['id'] . "' LIMIT 1";
                    $goods_id = $GLOBALS['db']->getOne($sql);
                    //已经存在则更新商品信息
                    if ($goods_id > 0) {
                        $id = $goods_id;
                        $sql_where = " goods_id = '$id'";
                        $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table($table), $goods, "UPDATE", $sql_where);
                        //清空商品相册
                        $sql = "SELECT img_original , img_url , thumb_url FROM" . $GLOBALS['sanmu']->table('goods_gallery') . "WHERE goods_id = '$id'";
                        ...
                        if (!empty($goods_gallery_list)) {
                            foreach ($goods_gallery_list as $gallery_list_key => $gallery_list_val) {
                                if ($gallery_list_val['img_original'] != '' && strpos($gallery_list_val['img_original'], 'http://') === false && strpos($gallery_list_val['img_original'], 'https://') === false) {
                                    sanmu_unlink(storage_public($gallery_list_val['img_original']));
                                }
                                if ($gallery_list_val['img_url'] != '' && strpos($gallery_list_val['img_url'], 'http://') === false && strpos($gallery_list_val['img_url'], 'https://') === false) {
                                    sanmu_unlink(storage_public($gallery_list_val['img_url']));
                                }
                                if ($gallery_list_val['thumb_url'] != '' && strpos($gallery_list_val['thumb_url'], 'http://') === false && strpos($gallery_list_val['thumb_url'], 'https://') === false) {
                                    sanmu_unlink(storage_public($gallery_list_val['thumb_url']));
                                }
                            }
                        }
                        $sql = "DELETE FROM" . $GLOBALS['sanmu']->table('goods_gallery') . "WHERE goods_id = '$id'";
                        $GLOBALS['db']->query($sql);
                    } else {
                        $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table($table), $goods, "INSERT");
                        $id = $GLOBALS['db']->insert_id();
                    }
                    //商品id为0时  跳出循环
                    if ($id == 0) {
                    ...
                    if ($v['goodsDetailList']) {
                        ...
                        $sql_where = " goods_id = '$id'";
                        $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table($table), $arr, "UPDATE", $sql_where);
                    }
                    ...
                    if ($v['goodsNavigateList']) {
                        ...
                        foreach ($v['goodsNavigateList'] as $gallery_key=>$gallery_val) {
                            ...
                            if (!empty($gallery_val['navigateImage']) && ($gallery_val['navigateImage'] != 'http://') && (strpos($gallery_val['navigateImage'], 'http://') !== false || strpos($gallery_val['navigateImage'], 'https://') !== false)) {
                                ...
                                $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table('goods_gallery'), $goods_gallery, "INSERT");
                                //把第一张相册图片保存为商品图片
                                if ($i == 1) {
                                    ...
                                    $sql_where = " goods_id = '$id'";
                                    $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table($table), $goods_arr, "UPDATE", $sql_where);
                                }
                            }
                        }
                    }
                    ...
                    //属性入库
                    if ($v['specDesc']) {
                        $specDesc = sanmu_decode($v['specDesc'], true);
                        if (!empty($specDesc)) {
                            foreach ($specDesc as $specDesc_key => $specDesc_val) {
                                ...
                                //判断是否存在该类型
                                $sql = "SELECT attr_id FROM" . $GLOBALS['sanmu']->table('attribute') . "WHERE (cloud_attr_id = '" . $specDesc_val['specificationId'] . "' OR attr_name = '" . addslashes($specDesc_val['name']) . "') AND cat_id = '$cat_id' LIMIT 1";
                                $attr_id = $GLOBALS['db']->getOne($sql);

                                if ($attr_id == 0 || $attr_id == '') {
                                    $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table('attribute'), $specDesc_arr, "INSERT");
                                    $attr_id = $GLOBALS['db']->insert_id();
                                } else {
                                    $sql_where = " attr_id = '$attr_id'";
                                    $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table('attribute'), $specDesc_arr, "UPDATE", $sql_where);
                                }
                                ...
                                if (!empty($specDesc_val['value'])) {
                                    foreach ($specDesc_val['value'] as $value_key => $value_val) {
                                        ...
                                        //判断是否存在该类型
                                        $sql = "SELECT goods_attr_id FROM" . $GLOBALS['sanmu']->table('goods_attr') . "WHERE cloud_id = '" . $value_val['specificationDetailId'] . "' AND goods_id = '$id' LIMIT 1";
                                        $goods_attr_id = $GLOBALS['db']->getOne($sql);

                                        if ($goods_attr_id == 0 || $goods_attr_id == '') {
                                            $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table('goods_attr'), $value_arr, "INSERT");
                                            $goods_attr_id = $GLOBALS['db']->insert_id();
                                        } else {
                                            $sql_where = " goods_attr_id = '$goods_attr_id'";
                                            $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table('goods_attr'), $value_arr, "UPDATE", $sql_where);
                                        }
                                    }
                                }
                            }
                    ...
                    if ($v['products']) {
                        ...
                        foreach ($products as $products_key => $products_val) {
                            ...
                            if ($products_val['specInfo']) {
                                //获取本地属性id
                                $sql = "SELECT goods_attr_id FROM" . $GLOBALS['sanmu']->table('goods_attr') . "WHERE cloud_id in(" . $products_val['specInfo'] . ") AND goods_id = '$id' ";
                                $goods_attr_arr = $GLOBALS['db']->getCol($sql);
                                if ($goods_attr_arr) {

                                //清除货品表
                                    $sql = "DELETE FROM" . $GLOBALS['sanmu']->table('products') . " WHERE goods_id = '$id' AND cloud_product_id = '" . intval($products_val['id']) . "'";
                                    $GLOBALS['db']->query($sql);
                                ...
                                $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table('products'), $insert_attr, "INSERT");
    ...
    public function get_goodsnotification_update($table, $select, $format)
    {
        ...
        if (!empty($select)) {
            ...
            $goods_id = 0;
            if ($select['goodsId'] > 0) {
                $sql = "SELECT goods_id FROM" . $GLOBALS['sanmu']->table('goods') . "WHERE cloud_id = '" . $select['goodsId'] . "' LIMIT 1";
                $goods_id = $GLOBALS['db']->getOne($sql);
            }
            //获取订单id
            $order = array();
            if ($select['orderSn']) {
                $sql = "SELECT oi.order_id , oi.order_sn , oi.user_id,oi.surplus,oi.money_paid,oi.bonus_id,oi.integral_money,oi.bonus,oi.coupons FROM" . $GLOBALS['sanmu']->table('order_goods')
                    . "AS og LEFT JOIN " . $GLOBALS['sanmu']->table('order_cloud')
                    . "AS oc ON oc.rec_id = og.rec_id LEFT JOIN " . $GLOBALS['sanmu']->table('order_info')
            ...
            switch ($select['mainType']) {
                case 101://商品上架通知
                    if ($goods_id > 0) {
                        $sql = "UPDATE " . $GLOBALS['sanmu']->table('goods') .
                ...
                case 102://商品下架通知
                    if ($goods_id > 0) {
                        $sql = "UPDATE " . $GLOBALS['sanmu']->table('goods') .
                            ...
                            "WHERE goods_id = '$goods_id'";
                        $GLOBALS['db']->query($sql);
                        $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('cart') . " WHERE goods_id = '$goods_id'";
                        $GLOBALS['db']->query($sql);
                    } else {
                        $common_data = array(
                            'code' => $goodsLang['msg_failure']['code'],
                ...
                case 201://订单取消通知
                    if (!empty($order)) {
                        foreach ($order as $k => $v) {
                            ...
                            /* 标记订单为“无效” */
                            $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table('order_info'), array('order_status' => OS_INVALID), 'UPDATE', "order_id = '" . $v['order_id'] . "'");
                            /* 记录log */
                            $log_time = gmtime();
                            $sql = 'INSERT INTO ' . $GLOBALS['sanmu']->table('order_action') .
                                ...
                                'FROM ' . $GLOBALS['sanmu']->table('order_info') . " WHERE order_sn = '" . $v['order_sn'] . "'";
                            $GLOBALS['db']->query($sql);
    ...
    public function return_user_surplus_integral_bonus($order)
    {
        ...
        if ($order['user_id'] > 0 && $order['surplus'] > 0) {
            ...
            $GLOBALS['db']->query("UPDATE " . $GLOBALS['sanmu']->table('order_info') . " SET `order_amount` = '0' WHERE `order_id` =" . $order['order_id']);
        }
        ...
        if ($order['bonus_id'] > 0) {
            $sql = "UPDATE " . $GLOBALS['sanmu']->table('user_bonus') .
        ...
        /*退优惠券*/
        if ($order['coupons'] > 0) {
            $sql = "UPDATE " . $GLOBALS['sanmu']->table('coupons_user') .
        ...
        $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table('order_info'), $arr, 'UPDATE', "order_id = '" . $order['order_id'] . "'");
    }
}
