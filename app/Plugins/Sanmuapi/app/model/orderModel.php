...
use App\Plugins\Sanmuapi\languages\orderLang;
...
abstract class orderModel extends common
{
    ...
    {
        $method = [
            'sanmu.order.goods.list.get',
            'sanmu.order.goods.info.get',
            'sanmu.order.goods.update.post',
            'sanmu.order.goods.del.get',
        ];
        ...
        if ($val['seller_id'] > 0 && (isset($val['method']) && $val['method'] == 'sanmu.order.list.get')) {
    ...
    private function get_take_time($start_take_time = '', $end_take_time = '', $alias = '')
    {
        ...
        if ((!empty($start_take_time) && $start_take_time != -1) || (!empty($end_take_time) && $end_take_time != -1)) {
            ...
            $where .= " AND (SELECT COUNT(*) FROM " . $GLOBALS['sanmu']->table('order_action') . " AS oa WHERE {$alias}order_id = oa.order_id $where_action) > 0";
        }
    ...
    public function get_select_list($table, $select, $where, $page_size, $page, $sort_by, $sort_order, $alias = '')
    {
        ...
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['sanmu']->table($table) . $table_alias . " WHERE " . $where;
        ...
        $sql = "SELECT " . $select . " FROM " . $GLOBALS['sanmu']->table($table) . $table_alias . " WHERE " . $where;
    ...
    public function get_select_info($table, $select, $where, $alias = '')
    {
        ...
        $sql = "SELECT " . $select . " FROM " . $GLOBALS['sanmu']->table($table) . $table_alias . " WHERE " . $where . " LIMIT 1";
    ...
    public function get_insert($table, $select, $format)
    {
        ...
        if ($table == 'order_info') {
            ...
            if (isset($select['pay_name']) && !empty($select['pay_name']) && $pay_id == 0) {
                ...
                $sql = "SELECT * FROM " . $GLOBALS['sanmu']->table('payment') . " WHERE pay_name = '$pay_name' LIMIT 1";
    ...
    public function get_confirmorder($table, $order_select, $format)
    {
        ...
        if (!empty($order_select)) {
            ...
            //$sql = "SELECT * FROM".$GLOBALS['sanmu']->table('shipping')."WHERE shipping_code = '" . $order_select['expressCode']. "' LIMIT 1";
            ...
            $sql = "SELECT oi.postscript,oi.best_time,oi.mobile,oi.tel,oi.zipcode,oi.email,oi.address,oi.consignee,oi.how_oos,oi.add_time,oi.order_id,oi.user_id,oi.country,oi.province,oi.city,oi.district,oi.agency_id,oi.insure_fee,oi.shipping_fee,oi.order_sn FROM" .
                $GLOBALS['sanmu']->table('order_info') . "AS oi LEFT JOIN" .
                $GLOBALS['sanmu']->table('order_goods') . " AS og ON oi.order_id = og.order_id LEFT JOIN" .
                $GLOBALS['sanmu']->table('order_cloud') . " AS oc ON og.rec_id = oc.rec_id WHERE oc.apiordersn = '" . $order_select['orderSn'] . "'";
            ...
            /* 发货单入库 */
            $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table('delivery_order'), $_delivery, 'INSERT', '', 'SILENT');
            ...
            if ($delivery_id > 0) {
                ...
                $sql = "SELECT o.goods_number,o.goods_id,o.product_id,o.goods_name,o.goods_sn, o.goods_attr,g.is_real, IFNULL(b.brand_name, '') AS brand_name, p.product_sn " .
                    ...
                    "LEFT JOIN " . $GLOBALS['sanmu']->table('order_cloud') . " AS oc ON o.rec_id = oc.rec_id " .
                ...
                if (!empty($goods_list)) {
                    ...
                    foreach ($goods_list as $key => $val) {
                        ...
                        $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table('delivery_goods'), $delivery_goods, 'INSERT', '', 'SILENT');
                        //分单操作
                        $split_action_note .= sprintf($orderLang['conf_message']['split_action_note'], $val['goods_sn'], $val['goods_number']) . "<br/>";
                        //更新订单发货数量
                        $sql = "UPDATE " . $GLOBALS['sanmu']->table('order_goods') . "
        ...
        /* 标记订单为已确认 “发货中” */
        /* 更新发货时间 */
        $sql = "SELECT COUNT(rec_id) FROM" . $GLOBALS['sanmu']->table('order_goods') . "WHERE order_id = '" . $_delivery['order_id'] . "' AND goods_number >= send_number";
        $order_finish = $GLOBALS['db']->getOne($sql);
        $shipping_status = SS_SHIPPED_ING;
        $sql = "SELECT mobile,email,consignee,order_id,order_status,pay_status,insure_fee,pay_id,invoice_no,pay_time,order_sn,extension_id,extension_code,goods_amount,user_id FROM" . $GLOBALS['sanmu']->table('order_info') . "WHERE order_id = '" . $_delivery['order_id'] . "'";
        ...
        $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table('order_info'), $arr, 'UPDATE', "order_id = '" . $_delivery['order_id'] . "'");
        /* 分单操作 */
        ...
        //发货
        //获取发货单信息
        $sql = "SELECT * FROM " . $GLOBALS['sanmu']->table('delivery_order') . " WHERE delivery_id = '$delivery_id' LIMIT 0, 1";
        ...
        if ($delivery_order['user_id'] > 0) {
            $sql = "SELECT user_name FROM " . $GLOBALS['sanmu']->table('users') .
        ...
        $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table('delivery_order'), $_delivery, 'UPDATE', "delivery_id = $delivery_id", 'SILENT');
        ...
        $GLOBALS['db']->autoExecute($GLOBALS['sanmu']->table('order_info'), $arr, 'UPDATE', "order_id = '$order_id'");
        ...
        if ($order_finish) {
            ...
            if ($cfg == '1') {
                ...
                $sql = 'SELECT template_subject, is_html, template_content FROM ' . $GLOBALS['sanmu']->table('mail_templates') . " WHERE template_code = 'deliver_notice'";
                ...
                $GLOBALS['smarty']->assign('confirm_url', $GLOBALS['sanmu']->url() . 'user_order.php?act=order_detail&order_id=' . $order['order_id']); //by wu
                $GLOBALS['smarty']->assign('send_msg_url', $GLOBALS['sanmu']->url() . 'user_message.php?act=message_list&order_id=' . $order['order_id']);
            ...
            if ($GLOBALS['_CFG']['sms_order_shipped'] == '1' && $order['mobile'] != '') {
                ...
                //短信接口参数
                $sql = "SELECT shop_name FROM" . $GLOBALS['sanmu']->table('seller_shopinfo') . " WHERE ru_id = 0";
                $shop_name = $GLOBALS['db']->getOne($sql);

                $sql = "SELECT user_name FROM" . $GLOBALS['sanmu']->table('users') . "WHERE user_id = '" . $order['user_id'] . "'";
