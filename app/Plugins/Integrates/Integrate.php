...
class Integrate
{
    ...
    public function __construct($cfg = [])
    {
        ...
        if (empty($cfg['db_host'])) {
            $this->db_name = $GLOBALS['sanmu']->db_name;
            $this->prefix = $GLOBALS['sanmu']->prefix;
            $this->db = &$GLOBALS['db'];
        }
    }
    ...
    public function edit_user($cfg, $forget_pwd = '0')
    {
        ...
        if ((!empty($cfg['email'])) && $this->field_email != 'NULL') {
            ...
            if ($this->db->getOne($sql, true) == 0) {
                // 新的E-mail
                $sql = "UPDATE " . $GLOBALS['sanmu']->table($this->user_table) . " SET is_validated = 0 WHERE user_name = '$cfg[post_username]'";
                $this->db->query($sql);
            }
        ...
        if (isset($cfg['old_password']) && !empty($cfg['old_password']) && !empty($cfg['post_username'])) {
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['sanmu']->table($this->user_table) . " WHERE user_name = '" . $cfg['post_username'] . "' AND 'password' = '" . $cfg['old_password'] . "'";
            if ($this->db->getOne($sql, true) > 0) {
                return true;
            } else {
                return false;
    ...
    public function remove_user($post_id)
    {
        ...
        if ($this->need_sync || (isset($this->is_sanmumall) && $this->is_sanmumall)) {
            /* 如果需要同步或是sanmumall插件执行这部分代码 */
            $sql = "SELECT user_id FROM " . $GLOBALS['sanmu']->table($this->user_table) . " WHERE ";
            $sql .= (is_array($post_id)) ? db_create_in($post_id, 'user_name') : "user_name='" . $post_id . "' LIMIT 1";
            $col = $GLOBALS['db']->getCol($sql);

            if ($col) {
                $sql = "UPDATE " . $GLOBALS['sanmu']->table($this->user_table) . " SET parent_id = 0 WHERE " . db_create_in($col, 'parent_id'); //将删除用户的下级的parent_id 改为0
                $GLOBALS['db']->query($sql);
                $sql = "DELETE FROM " . $GLOBALS['sanmu']->table($this->user_table) . " WHERE " . db_create_in($col, 'user_id'); //删除用户
                $GLOBALS['db']->query($sql);
                /* 删除用户订单 */
                $sql = "SELECT order_id FROM " . $GLOBALS['sanmu']->table('order_info') . " WHERE " . db_create_in($col, 'user_id');
                $GLOBALS['db']->query($sql);
                $col_order_id = $GLOBALS['db']->getCol($sql);
                if ($col_order_id) {
                    $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('order_info') . " WHERE " . db_create_in($col_order_id, 'order_id');
                    $GLOBALS['db']->query($sql);
                    $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('order_goods') . " WHERE " . db_create_in($col_order_id, 'order_id');
                    $GLOBALS['db']->query($sql);
                }

                /* 删除用户白条 */
                $sql = "SELECT baitiao_id FROM " . $GLOBALS['sanmu']->table('baitiao') . " WHERE " . db_create_in($col, 'user_id');
                $GLOBALS['db']->query($sql);
                $col_baitiao_id = $GLOBALS['db']->getCol($sql);
                if ($col_baitiao_id) {
                    $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('baitiao_log') . " WHERE " . db_create_in($col_baitiao_id, 'baitiao_id'); //删除白条记录
                    $GLOBALS['db']->query($sql);
                    $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('baitiao_pay_log') . " WHERE " . db_create_in($col_baitiao_id, 'baitiao_id'); //删除白条支付记录
                    $GLOBALS['db']->query($sql);
                }

                $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('booking_goods') . " WHERE " . db_create_in($col, 'user_id'); //删除用户
                $GLOBALS['db']->query($sql);
                $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('collect_goods') . " WHERE " . db_create_in($col, 'user_id'); //删除会员收藏商品
                $GLOBALS['db']->query($sql);
                $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('collect_store') . " WHERE " . db_create_in($col, 'user_id'); //删除会员收藏店铺
                $GLOBALS['db']->query($sql);
                $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('collect_brand') . " WHERE " . db_create_in($col, 'user_id'); //删除会员收藏品牌
                $GLOBALS['db']->query($sql);
                $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('feedback') . " WHERE " . db_create_in($col, 'user_id'); //删除用户留言
                $GLOBALS['db']->query($sql);
                $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('user_address') . " WHERE " . db_create_in($col, 'user_id'); //删除用户地址
                $GLOBALS['db']->query($sql);
                $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('user_bonus') . " WHERE " . db_create_in($col, 'user_id'); //删除用户红包
                $GLOBALS['db']->query($sql);
                $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('user_account') . " WHERE " . db_create_in($col, 'user_id'); //删除用户帐号金额
                $GLOBALS['db']->query($sql);
                $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('tag') . " WHERE " . db_create_in($col, 'user_id'); //删除用户标记
                $GLOBALS['db']->query($sql);
                $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('account_log') . " WHERE " . db_create_in($col, 'user_id'); //删除用户日志
                $GLOBALS['db']->query($sql);
                $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('value_card') . " WHERE " . db_create_in($col, 'user_id'); //删除用户储值卡
                $GLOBALS['db']->query($sql);
                $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('coupons_user') . " WHERE " . db_create_in($col, 'user_id'); //删除用户优惠券
                $GLOBALS['db']->query($sql);

                $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('users_vat_invoices_info') . " WHERE " . db_create_in($col, 'user_id'); //删除用户发票信息
                $GLOBALS['db']->query($sql);

                $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('user_bank') . " WHERE " . db_create_in($col, 'user_id'); //删除用户银行卡
                $GLOBALS['db']->query($sql);
                $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('users_paypwd') . " WHERE " . db_create_in($col, 'user_id'); //删除用户支付密码
                $GLOBALS['db']->query($sql);
                $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('users_real') . " WHERE " . db_create_in($col, 'user_id'); //删除用户实名认证信息
                $GLOBALS['db']->query($sql);
                $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('users_auth') . " WHERE " . db_create_in($col, 'user_id'); //删除第三方会员绑定表
                $GLOBALS['db']->query($sql);

                $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('connect_user') . " WHERE " . db_create_in($col, 'user_id'); //删除第三方会员绑定表
                $GLOBALS['db']->query($sql);

                if (file_exists(MOBILE_WECHAT)) {
                    // 删除微信通关联表
                    $sql = "SELECT uid FROM " . $GLOBALS['sanmu']->table('wechat_user') . " WHERE " . db_create_in($col, 'ect_uid');
                    $GLOBALS['db']->query($sql);
                    $col_uid = $GLOBALS['db']->getCol($sql);
                    if ($col_uid) {
                        // 微信粉丝普通消息记录
                        $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('wechat_custom_message') . " WHERE " . db_create_in($col_uid, 'uid');
                        $GLOBALS['db']->query($sql);
                    }

                    $sql = "SELECT openid FROM " . $GLOBALS['sanmu']->table('wechat_user') . " WHERE " . db_create_in($col, 'ect_uid');
                    $GLOBALS['db']->query($sql);
                    $col_openid = $GLOBALS['db']->getCol($sql);
                    if ($col_openid) {
                        // 微信粉丝标签
                        $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('wechat_user_tag') . " WHERE " . db_create_in($col_openid, 'openid');
                        $GLOBALS['db']->query($sql);
                        // 微信粉丝中奖记录
                        $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('wechat_prize') . " WHERE " . db_create_in($col_openid, 'openid');
                        $GLOBALS['db']->query($sql);
                        // 微信粉丝签到记录
                        $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('wechat_point') . " WHERE " . db_create_in($col_openid, 'openid');
                        $GLOBALS['db']->query($sql);
                        // 微信粉丝模板消息记录
                        $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('wechat_template_log') . " WHERE " . db_create_in($col_openid, 'openid');
                        $GLOBALS['db']->query($sql);
                    }

                    $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('wechat_user') . " WHERE " . db_create_in($col, 'ect_uid'); //删除微信通会员
                    $GLOBALS['db']->query($sql);
                }

                if (file_exists(MOBILE_DRP)) {
                    $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('drp_shop') . " WHERE " . db_create_in($col, 'user_id'); //删除分销会员
                    $GLOBALS['db']->query($sql);

                    $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('drp_affiliate_log') . " WHERE " . db_create_in($col, 'user_id'); //删除分销会员分成记录 已丢弃
                    $GLOBALS['db']->query($sql);

                    $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('drp_log') . " WHERE " . db_create_in($col, 'user_id'); //删除分销会员分成记录
                    $GLOBALS['db']->query($sql);

                    $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('drp_type') . " WHERE " . db_create_in($col, 'user_id'); //删除分销会员商品分类记录
                    $GLOBALS['db']->query($sql);

                    $sql = "DELETE FROM " . $GLOBALS['sanmu']->table('drp_transfer_log') . " WHERE " . db_create_in($col, 'user_id'); //删除分销会员提现记录
                    $GLOBALS['db']->query($sql);
                }
            }
        }

        if (isset($this->sanmumall) && $this->sanmumall) {
            /* 如果是sanmumall插件直接退出 */
            return;
        }
    ...
    public function set_cookie($username = '', $remember = null, $is_oath = 1, $unionid = '')
    {
        ...
        if ($unionid) {
            ...
            $select = ", conu.open_id";
            $leftJoin = " LEFT JOIN " . $GLOBALS['sanmu']->table('connect_user') . " AS conu ON u.user_id = conu.user_id";
        }
        ...
        if (empty($username)) {
            ...
            if ($is_oath == 1) {
                ...
                $sql = "SELECT u.user_id, u.user_name, u.nick_name, u.password, u.email " . $select . " FROM " . $GLOBALS['sanmu']->table($this->user_table) . " AS u " .
                    $leftJoin .
                    " WHERE $where LIMIT 1";
...
//                cookie()->queue('ECS[password]', $row['password'], $time);//密码敏感 不存cookie
    ...
    public function sync($username, $password = '', $md5password = '')
    {
        ...
        $sql = "SELECT user_name, email, password, sex, birthday" .
            " FROM " . $GLOBALS['sanmu']->table($this->user_table) .
        ...
        if (empty($profile)) {
            ...
            if (empty($md5password)) {
                $sql = "INSERT INTO " . $GLOBALS['sanmu']->table($this->user_table) .
            ...
            if (empty($values)) {
                ...
                $sql = "UPDATE " . $GLOBALS['sanmu']->table($this->user_table) .
                    " SET " . implode(", ", $values) .
                    " WHERE user_name='$username'";
